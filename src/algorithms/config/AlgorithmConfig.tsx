...
// 中医五诊算法配置管理     提供统一的配置管理接口，支持动态配置更新和环境适配     @author 索克生活技术团队   @version 1.0.0
export interface AlgorithmConfigOptions {;
  version?: string;
  startTime?: number;
  // 各诊法配置 *   looking?: LookingConfig; */
  listening?: ListeningConfig;
  inquiry?: InquiryConfig;
  palpation?: PalpationConfig;
  calculation?: CalculationConfig;
  // 融合和分析配置 *   fusion?: FusionConfig; */
  syndrome?: SyndromeConfig;
  constitution?: ConstitutionConfig;
  treatment?: TreatmentConfig;
  // 系统配置 *   knowledgeBase?: KnowledgeBaseConfig; */
  qualityControl?: QualityControlConfig;
  monitoring?: MonitoringConfig;
  // 性能配置 *   performance?: PerformanceConfig; */
}
export interface LookingConfig { enabled: boolean,
  models: {tongueAnalysis: ModelConfig,
...
export interface ListeningConfig { enabled: boolean,
  models: {voiceAnalysis: ModelConfig,
...
export interface InquiryConfig { enabled: boolean,
  models: {symptomAnalysis: ModelConfig,
...
export interface PalpationConfig { enabled: boolean,
  models: {pulseAnalysis: ModelConfig,
...
export interface CalculationConfig { enabled: boolean,
  models: {lunarCalculation: ModelConfig,
    fiveElementsAnalysis: ModelConfig,
    yinYangAnalysis: ModelConfig}
  calculation: { precision: number,
    timezone: string,
    calendar: "gregorian" | "lunar" | "both"};
  confidence: { threshold: number,
    historicalDepth: number}
}
export interface FusionConfig { enabled: boolean,
...
export interface SyndromeConfig { enabled: boolean,
  models: {patternRecognition: ModelConfig,
    syndromeClassification: ModelConfig};
  analysis: { maxSyndromes: number,
    minConfidence: number,
    includeSubSyndromes: boolean};
}
export interface ConstitutionConfig { enabled: boolean,
  models: {constitutionClassification: ModelConfig,
    bodyTypeAnalysis: ModelConfig};
  analysis: { constitutionTypes: string[],
    adaptiveWeighting: boolean,
    ageFactors: boolean};
}
export interface TreatmentConfig { enabled: boolean,
  models: {recommendationEngine: ModelConfig,
...
export interface QualityControlConfig { enabled: boolean,
  checks: {dataValidation: boolean,
...
export interface PerformanceConfig { maxConcurrentSessions: number,
  timeoutMs: number,
...
// 算法配置管理类export class AlgorithmConfig {;
  ...
  constructor(options: Partial<AlgorithmConfigOptions /> = {}) {/    this.version = options.version || "1.0.0";
    ...
    // 初始化各诊法配置 *     this.looking = this.initializeLookingConfig(options.looking); */
    ...
    // 初始化融合和分析配置 *     this.fusion = this.initializeFusionConfig(options.fusion); */
    ...
    // 初始化系统配置 *     this.knowledgeBase = this.initializeKnowledgeBaseConfig( */
  ...
  // /    初始化望诊配置  private initializeLookingConfig(config?: Partial<LookingConfig />/  );: LookingConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        tongueAnalysis: config?.models?.tongueAnalysis ;|;|this.getDefaultModelConfig("tongue_analysis"),
  ...
  // /    初始化闻诊配置  private initializeListeningConfig(config?: Partial<ListeningConfig />/  );: ListeningConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        voiceAnalysis: config?.models?.voiceAnalysis ;|;|this.getDefaultModelConfig("voice_analysis"),
        breathingAnalysis: config?.models?.breathingAnalysis ||this.getDefaultModelConfig("breathing_analysis"),
  ...
  // /    初始化问诊配置  private initializeInquiryConfig(config?: Partial<InquiryConfig />/  );: InquiryConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        symptomAnalysis: config?.models?.symptomAnalysis ;|;|this.getDefaultModelConfig("symptom_analysis"),
        nlpProcessing: config?.models?.nlpProcessing ||this.getDefaultModelConfig("nlp_processing"),
  ...
  // /    初始化切诊配置  private initializePalpationConfig(config?: Partial<PalpationConfig />/  );: PalpationConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        pulseAnalysis: config?.models?.pulseAnalysis ;|;|this.getDefaultModelConfig("pulse_analysis"),
        pressureAnalysis: config?.models?.pressureAnalysis ||this.getDefaultModelConfig("pressure_analysis"),
  ...
  // /    初始化算诊配置  private initializeCalculationConfig(config?: Partial<CalculationConfig />/  );: CalculationConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        lunarCalculation: config?.models?.lunarCalculation ;|;|this.getDefaultModelConfig("lunar_calculation"),
        fiveElementsAnalysis: config?.models?.fiveElementsAnalysis ||this.getDefaultModelConfig("five_elements"),
      ...
      calculation: {
        ...
        timezone: config?.calculation?.timezone || "Asia/Shanghai",/        calendar: config?.calculation?.calendar || "both"
      },
      confidence: {
  ...
  // /    初始化融合配置  private initializeFusionConfig(config?: Partial<FusionConfig />): FusionConfig  {/    return {
      enabled: config?.enabled ?? true,
  ...
  // /    初始化辨证配置  private initializeSyndromeConfig(config?: Partial<SyndromeConfig />/  );: SyndromeConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        patternRecognition: config?.models?.patternRecognition ;|;|this.getDefaultModelConfig("pattern_recognition"),
        syndromeClassification: config?.models?.syndromeClassification ||this.getDefaultModelConfig("syndrome_classification")},
      analysis: {
  ...
  // /    初始化体质配置  private initializeConstitutionConfig(config?: Partial<ConstitutionConfig />/  );: ConstitutionConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        constitutionClassification: config?.models?.constitutionClassification ;|;|this.getDefaultModelConfig("constitution_classification"),
        bodyTypeAnalysis: config?.models?.bodyTypeAnalysis ||this.getDefaultModelConfig("body_type_analysis")},
      analysis: {
        constitutionTypes: config?.analysis?.constitutionTypes || ["平和质",
          "气虚质",
          "阳虚质",
          "阴虚质",
          "痰湿质",
  ...
  // /    初始化治疗配置  private initializeTreatmentConfig(config?: Partial<TreatmentConfig />/  );: TreatmentConfig  {
    return {
      enabled: config?.enabled ?? true,
      models: {
        recommendationEngine: config?.models?.recommendationEngine ;|;|this.getDefaultModelConfig("recommendation_engine"),
        herbFormulation: config?.models?.herbFormulation ||this.getDefaultModelConfig("herb_formulation"),
  ...
  // /    初始化知识库配置  private initializeKnowledgeBaseConfig(config?: Partial<KnowledgeBaseConfig />/  );: KnowledgeBaseConfig  {
    return {
      version: config?.version || "1.0.0",
      updateInterval: config?.updateInterval || 86400000, // 24小时 *       sources: config?.sources |;| ;["tcm_classics", */
        "modern_research",
        "clinical_data"
      ],
      caching: {
        enabled: config?.caching?.enabled ?? true,
        ttl: config?.caching?.ttl || 3600000, // 1小时 *         maxSize: config?.caching?.maxSize || 1000 */
      }
    };
  }
  // /    初始化质量控制配置  private initializeQualityControlConfig(config?: Partial<QualityControlConfig />/  );: QualityControlConfig  {
    return {
      enabled: config?.enabled ?? true,
      checks: {
  ...
  // /    初始化监控配置  private initializeMonitoringConfig(config?: Partial<MonitoringConfig />/  ): MonitoringConfig  {
    return {
      enabled: config?.enabled ?? true,
      metrics: {
      ...
      reporting: {
        interval: config?.reporting?.interval || 300000, // 5分钟 *         destination: config?.reporting?.destination || "console", */
        format: config?.reporting?.format || "json"}
    ;};
  }
  // /    初始化性能配置  private initializePerformanceConfig(config?: Partial<PerformanceConfig />/  ): PerformanceConfig  {
    return {
  ...
  // /    获取默认模型配置  private getDefaultModelConfig(modelName: string): ModelConfig  {
    return {
      name: modelName,
      version: "1.0.0",
      path: `/models/${modelName}`,/      type: "tensorflow",
      device: "auto",
      batchSize: 32,
      timeout: 1000;0
    ;};
  }
  // /    更新配置  public update(newConfig: Partial<AlgorithmConfigOptions />);: void  {/    // 实现配置热更新逻辑 *      *// 注意：某些配置可能需要重启算法模块才能生效* *     Object.assign( * *//
      this,
      new AlgorithmConfig({
        ...this.toJSON(),
        ...newConfig
  ...
  // /    导出配置为JSON  public toJSON();: AlgorithmConfigOptions {
    return {
      ...
      performance: this.performanc;e
    ;};
  }
  // /    验证配置有效性  public validate();: { valid: boolean, errors: string[]  ; } {
    const errors: string[] = [];
    // 验证权重总和 *     const weights = this.fusion.weigh;t;s; */
    const totalWeight = Object.values(weights).reduce(;
      (sum, weigh;t;); => sum + weight,
      0
    );
    if (Math.abs(totalWeight - 1.0) > 0.01) {
      errors.push(`融合权重总和应为1.0，当前为${totalWeight}`);
    }
    // 验证置信度阈值 *     const confidenceThresholds = [ */
    ...
    // 验证性能配置 *     if (this.performance.maxConcurrentSessions <= 0) { */
