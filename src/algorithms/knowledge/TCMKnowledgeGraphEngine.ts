 ...
 * 中医知识图谱推理引擎
 */
export class TCMKnowledgeGraphEngine extends EventEmitter {
  private entities: Map<string, TCMEntity> = new Map();
  ...
  public async diagnose(input: DiagnosisInput): Promise<DiagnosisResult> {
    ...
    try {
      ...
      const result: DiagnosisResult = {
        primarySyndrome: rankedSyndromes[0],
  ...
  private async recognizeSymptoms(symptoms: string[]): Promise<TCMEntity[]> {
    ...
    for (const symptom of symptoms) {
      ...
      if (matches.length > 0) {
        ...
        // 创建未知症状实体
        const unknownSymptom: TCMEntity = {
  ...
  private processTongueEvidence(tongueAnalysis: any): any {
    return {
      color: tongueAnalysis.color,
      coating: tongueAnalysis.coating,
      texture: tongueAnalysis.texture,
      moisture: tongueAnalysis.moisture,
      syndromeIndicators: this.mapTongueToSyndromes(tongueAnalysis)
    };
  }
  ...
  private processPulseEvidence(pulseAnalysis: any): any {
    return {
      type: pulseAnalysis.type,
      rate: pulseAnalysis.rate,
      rhythm: pulseAnalysis.rhythm,
      strength: pulseAnalysis.strength,
      syndromeIndicators: this.mapPulseToSyndromes(pulseAnalysis)
    };
  }
  ...
  private generateLifestyleAdvice(syndrome: TCMEntity): TreatmentRecommendation | null {
    const adviceMap: Record<string, string> = {
      'syn_qi_deficiency': '适当运动，避免过度劳累，规律作息',
      'syn_yin_deficiency': '避免熬夜，多食滋阴润燥食物，保持心情平和',
      'syn_blood_stasis': '适当活动，避免久坐，注意保暖',
      'syn_damp_heat': '饮食清淡，避免辛辣油腻，保持环境通风'
    };
    ...
    if (advice) {
      return {
        type: 'lifestyle',
        entity: {
          id: `lifestyle_${syndrome.id}`,
          name: '生活调理',
          type: EntityType.TREATMENT,
          properties: { advice }
        },
        confidence: (syndrome.confidence || 0) * 0.8,
        notes: advice
      };
    }
  ...
  private performDifferentialDiagnosis(syndromes: TCMEntity[], evidence: any): DifferentialDiagnosis[] {
    return syndromes.map(syndrome => {
      ...
      return {
        syndrome,
        probability: syndrome.confidence || 0,
        supportingEvidence,
  ...
  private generateCacheKey(input: DiagnosisInput): string {
    return JSON.stringify({
      ...
      tongue: input.tongueAnalysis,
      pulse: input.pulseAnalysis,
