...
export interface PainAssessment {location: string[],intensity: number}quality: string  / 刺痛、胀痛、隐痛等*  , aggravatingFactors: string[], * /;/;,/g/;
...
export interface SleepPattern {bedtime: string}wakeTime: string,sleepDuration: number,sleepQuality: number;
  ...
  , difficulties: string[]  / 入睡困难、易醒等* ;}/;/g/;
} * //;,/g/;
...
this.knowledgeBase;
  ...
  // 执行问诊分析  public async analyze(data: InquiryData,)/;,/g/;
...
return resul;t;s;
  ...
  // 特征提取  private async extractFeatures(data: ProcessedInquiryData,)/;,/g/;
...
return featur;e;s;
  ...
  // 执行各项分析  private async performAnalyses(features: InquiryFeatures,)/;,/g/;
userProfile?: UserProfile;
...
return resul;t;s;
  ...
  // 整合分析结果  private async integrateResults(features: InquiryFeatures,)/;,/g/;
const analyses = AnalysisResults);: Promise<InquiryResult /    >  {/;,}confidence: this.calculateOverallConfidence(features, analyses;);/g/;
  ...
  // 计算整体置信度  private calculateOverallConfidence(features: InquiryFeatures,)/;,/g/;
const analyses = AnalysisResults);: number  {let totalWeight = 0;,}let weightedSum = 0;
symptomWeight: Math.min(features.symptoms.length  / 5, 1;);  0.4;/ totalWeight += symptomWeight;/;,/g/;
weightedSum += symptomWeight * 0.8;
if (analyses.constitutionalAnalysis) {totalWeight += 0.3;}}
      weightedSum += 0.3 * 0.7;}
    }
    if (analyses.syndromePatterns && analyses.syndromePatterns.length > 0) {const  avgConfidence =;,}analyses.syndromePatterns.reduce(sum, p;); => sum + p.confidence, 0) // analyses.syndromePatterns.length;/;,/g/;
totalWeight += 0.3;
}
      weightedSum += 0.3 * avgConfidence;}
    }
    return totalWeight > 0 ? weightedSum / totalWeight : 0;.;5;/      }/;/g/;
