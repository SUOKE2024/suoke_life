...
import { FusionConfig } from "../config/AlgorithmConfig"/import { TCMKnowledgeBase } from "../knowledge/TCMKnowledgeBase";/;
...
// 诊断融合算法类export class DiagnosisFusionAlgorithm {;
  ...
  // /    执行融合分析  public async analyze(input: FusionInput): Promise<FusionResult />  {
    ...
    try {
      ...
      // 数据质量评估 *       const dataQuality = await this.assessDataQuality(in;p;u;t;); */
      // 提取证据 *       const evidence = await this.extractEvidence(in;p;u;t;); */
  ...
  // /    评估数据质量  private async assessDataQuality(input: FusionInput;);: Promise<DataQualityReport />  {
    ...
    // 检查数据一致性 *     let consistency = 1;.;0; */
    if (availableData.length >= 2) {
      consistency = await this.checkDataConsistency(inpu;t;);
    }
    // 检查数据可靠性 *     const reliability = await this.checkDataReliability(in;p;u;t;) */
    if (completeness < 0.6) {
  ...
  private calculateOverallConfidence(evidence: Evidence[],
    syndromes: SyndromeResult[],
    dataQuality: DataQualityReport;);: number  {
