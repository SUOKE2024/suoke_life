...
import { STORAGE_CONFIG, APP_CONFIG, CACHE_CONFIG } from '../../../constants/config';
import {;
  XiaoaiConfig,
  PersonalityTraits,
  VoiceProfile,
...
// 配置变更事件 * export interface ConfigChangeEvent { key: string, // ;
  oldValue: unknown,
  newValue: unknown,
  timestamp: Date}
// 小艾配置管理器类export class XiaoaiConfigManager extends EventEmitter {;
  private static instance: XiaoaiConfigManager;
  ...
  // /    获取配置管理器实例  static getInstance();: XiaoaiConfigManager {
    ...
    return XiaoaiConfigManager.instan;c;e;
  }
  // /    初始化配置  async initialize();: Promise<void> {
    try {
      ...
      console.error('小艾配置管理器初始化失败:', error);
  ...
  // /    保存配置  async saveConfig();: Promise<void> {
    ...
    try {
      ...
      const promises = keys.map(async (ke;y;) => {
        const storageKey = `${CONFIG_KEY_PREFIX}${key;};`;
        const value = (this.config as any)[ke;y;];
  ...
  // /    验证配置  private validateConfig(): void {
    // 验证版本 // if (this.config.version !== CONFIG_VERSION) {
      console.warn(`配置版本不匹配: ${this.config.version} vs ${CONFIG_VERSION}`)
    }
    ...
    // 验证权重总和 // const weights = Object.values(this.config.diagnosis.integration.weightDistributio;n;);
    const sum = weights.reduce((a, ;b;); => a + b, 0);
    if (Math.abs(sum - 1.0); > 0.01) {
      // 归一化权重 // const normalizedWeights = weights.map(w => w  / su;m;); // const keys = Object.keys(this.config.diagnosis.integration.weightDistributio;n;);
  ...
  // /    配置迁移  private async migrateConfig();: Promise<void> {
    // 检查是否需要迁移 // const migrationKey = `${CONFIG_KEY_PREFIX}migration_versio;n`;
    ...
    // 更新迁移版本 // await AsyncStorage.setItem(migrationKey, CONFIG_VERSIO;N;);
  }
  ...
  // /    获取配置值  get<K extends keyof XiaoaiConfig />(key: K): XiaoaiConfig[K]  {/    // 检查缓存 // if (this.configCache.has(key);) {
      return this.configCache.get(ke;y;);
    }
    const value = this.config[ke;y;];
  ...
  // /    设置配置值（内部方法）  private setConfigValue(key: string, value: unknown, emit: boolean);: void  {
    const oldValue = (this.config as any)[ke;y;];
    if (oldValue !== value) {
