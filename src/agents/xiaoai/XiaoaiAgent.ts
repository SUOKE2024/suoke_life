import { XiaoaiChatDiagnosisIntegrator } from "./core/XiaoaiChatDiagnosisIntegrator";""/;,"/g"/;
import { accessibilityServiceClient } from "./services/AccessibilityServiceClient";""/;,"/g"/;
import { diagnosisServiceClient } from "./services/DiagnosisServiceClient";""/;"/g"/;

// 基础类型定义/;,/g/;
interface ChatContext {const userId = string;,}sessionId?: string;
hasImages?: boolean;
hasAudio?: boolean;
...
interface ChatResponse {const text = string;,}suggestions?: string[];
...
interface FourDiagnosisResults {inquiry?: any;,}look?: LookResult;
listen?: ListenResult;
}
}
...
interface XiaoaiAgent {processMessage(message: string, context: ChatContext): Promise<ChatResponse>;,}analyzeHealthData(data: any): Promise<any>;
...
if (audioType === 'voice') {';,}const return = await diagnosisServiceClient.listen.analyzeVoice(processedAudioData);'';
  ...
  ): Promise<IntegratedDiagnosis> {try {}      // 综合四诊结果/;,/g/;
...
const async = enableAccessibilityFeature(feature: any): Promise<void> {';,}try {';,}if (feature.type === 'voice_assistance') {';}}'';
  ...
  const async = getAccessibilityStatus(): Promise<any> {try {}      // 检查无障碍服务状态/;,/g/;
