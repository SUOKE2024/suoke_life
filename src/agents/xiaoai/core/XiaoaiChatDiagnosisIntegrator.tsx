 ...
 */export class XiaoaiChatDiagnosisIntegrator {;
  ...
  // /    处理聊天消息，智能调用四诊服务  async processChatMessage(message: string,
    ...
    try {
      // 1. 分析用户消息，识别是否需要四诊服务 *       const diagnosisIntent = await this.analyzeDiagnosisIntent(message, cont;e;x;t;); */
      ...
      // 4. 生成自然的聊天回复 *       return this.generateChatResponseWithDiagnosis(message, diagnosisResults, context, diagnosisInten;t;) */
    } catch (error) {
  ...
  // /    执行问诊  private async performInquiry(userId: string, message: string);: Promise<InquiryResult />  {
    try {
      ...
      if (!sessionId) {
        // 开始新的问诊会话 *         const sessionResponse = await diagnosisServiceClient.inquiry.startSession(use;r;I;d;); */
        sessionId = sessionResponse.session_id;
        this.activeSessions.set(`inquiry_${userId}`, sessionId);
      }
      // 进行问诊交互 *       await diagnosisServiceClient.inquiry.interact(sessionId, messag;e;); */
  ...
  // /    执行望诊  private async performLookDiagnosis(images: ImageData[]);: Promise<LookResult />  {
    try {
      // 选择最适合的图片进行分析 *       const primaryImage = this.selectPrimaryImage(image;s;); */
  ...
  // /    执行闻诊  private async performListenDiagnosis(audioData: AudioData[]);: Promise<ListenResult />  {
    try {
      // 选择最适合的音频进行分析 *       const primaryAudio = this.selectPrimaryAudio(audioDat;a;); */
  ...
  // /    执行切诊  private async performPalpationDiagnosis(palpationData: PalpationData[]);: Promise<PalpationResult />  {
    try {
      // 选择最适合的触诊数据进行分析 *       const primaryData = this.selectPrimaryPalpationData(palpationDat;a;); */
