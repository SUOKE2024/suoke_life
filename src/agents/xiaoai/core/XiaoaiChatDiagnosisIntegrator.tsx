...
/////
  ...
  LookResult,
  ListenResult,
  { PalpationResult } from "../types;// * 小艾智能体聊天中的四诊调用集成器"////
 * 负责在聊天过程中智能识别并调用相应的四诊服务
 export class XiaoaiChatDiagnosisIntegrator   {private activeSessions: Map<string, any> = new Map();
  // 处理聊天消息，智能调用四诊服务  async processChatMessage(message: string,
    ...
    try {
      // 1. 分析用户消息，识别是否需要四诊服务 // const diagnosisIntent = await this.analyzeDiagnosisIntent(message, conte;x;t;);
      // 2. 根据意图调用相应的四诊服务 // const diagnosisResults: FourDiagnosisResults = {}
      ...
      // 3. 如果有多个诊断结果，进行四诊合参 // if (this.hasMultipleDiagnosisResults(diagnosisResults)) {
        diagnosisResults.integrated = await this.performFourDiagnosisIntegration(diagnosisResult;s;);
      }
      // 4. 生成自然的聊天回复 // return this.generateChatResponseWithDiagnosis(message, diagnosisResults, context, diagnosisIntent;);
    } catch (error) {
  ...
  // 分析用户消息的四诊意图  private async analyzeDiagnosisIntent(message: string,
    ...
    const needsPalpationDiagnosis = context.hasPalpationData ||;
                                   ...
                                   message.includes(按压") ||"
                                   message.includes("触诊);"
    // 计算置信度 // const confidence = this.calculateIntentConfidence(;
  ...
  // 执行问诊  private async performInquiry(userId: string, message: string): Promise<InquiryResult /////    >  {
    try {
      // 检查是否已有活跃的问诊会话 // let sessionId = this.activeSessions.get(`inquiry_${userId}`);
      if (!sessionId) {
        // 开始新的问诊会话 // const sessionResponse = await diagnosisServiceClient.inquiry.startSession(user;I;d;);
        sessionId = sessionResponse.session_id;
        this.activeSessions.set(`inquiry_${userId}`, sessionId);
      }
      // 进行问诊交互 // await diagnosisServiceClient.inquiry.interact(sessionId, message;);
      // 获取当前的问诊结果（不结束会话，保持连续对话） // / 这里可能需要调用一个获取当前状态的API，而不是结束会话* // * // 暂时使用模拟数据* // return { * /////;
  ...
  // 执行望诊  private async performLookDiagnosis(images: ImageData[]): Promise<LookResult /////    >  {
    try {
      // 选择最适合的图片进行分析 // const primaryImage = this.selectPrimaryImage(images;);
      return await diagnosisServiceClient.look.analyzeImage(primaryIm;a;g;e;);
    } catch (error) {
      throw error;
    }
  }
  // 执行闻诊  private async performListenDiagnosis(audioData: AudioData[]): Promise<ListenResult /////    >  {
    try {
      // 选择最适合的音频进行分析 // const primaryAudio = this.selectPrimaryAudio(audioData;);
      return await diagnosisServiceClient.listen.analyzeAudio(primaryAu;d;i;o;);
    } catch (error) {
      throw error;
    }
  }
  // 执行切诊  private async performPalpationDiagnosis(palpationData: PalpationData[]): Promise<PalpationResult /////    >  {
    try {
      // 选择最适合的触诊数据进行分析 // const primaryData = this.selectPrimaryPalpationData(palpationData;);
      return await diagnosisServiceClient.palpation.analyzePalpation(primaryD;a;t;a;);
    } catch (error) {
      throw error;
    }
  }
  // 执行四诊合参  private async performFourDiagnosisIntegration(diagnosisResults: FourDiagnosisResults);: Promise<IntegratedDiagnosis /////    >  {
    // 这里实现四诊合参的逻辑 // / 目前返回模拟数据，后续需要实现真正的中医四诊合参算法* // * /////
    ...
    return { overallAssessment: `综合四诊分析：${evidences.join(；")  }`,";
      tcmDiagnosis: {syndrome: "待进一步分析,",pathogenesis: "基于四诊合参的病机分析",treatment: 个性化治疗方案",";
        prognosis: "良好},";
  ...
  // 生成包含诊断结果的聊天回复  private generateChatResponseWithDiagnosis(originalMessage: string,
    ...
    // 根据紧急程度调整回复语气 // if (intent.urgencyLevel === high" || intent.urgencyLevel === "emergency) {
    ...
    // 根据诊断结果生成具体回复 // if (diagnosisResults.integrated) {
      ...
      // 单个诊断结果的处理 // if (diagnosisResults.inquiry) {
        responseText += \n\n基于我们的对话，我了解了你的症状情况。""
      }
      if (diagnosisResults.look) {
    ...
    // 生成后续建议的操作 // if (intent.needsInquiry && !diagnosisResults.inquiry) {
      actions.push({
        type: "inquiry,",
  ...
  // 生成错误回复  private generateErrorResponse(message: string, error: unknown): ChatResponse  {
    return {text: "抱歉，我在分析你的情况时遇到了一些技术问题。请稍后再试，或者换个方式描述你的症状。",suggestions: [重新描述症状", "稍后再试, "联系技术支持"],timestamp: Date.now();};
  }
  // 辅助方法  private calculateIntentConfidence(symptomCount: number,
    ...
    confidence += symptomCount * 0.1 // 每个症状增加0.1 // confidence += (needsInquiry ? 0.2 : 0) ////
  ...
  private selectPrimaryImage(images: ImageData[]): ImageData  {
    // 优先选择舌象图片，其次是面部图片 // const tongueImage = images.find(img => img.type === tongue";); "
  ...
  private selectPrimaryAudio(audioData: AudioData[]): AudioData  {
    // 优先选择咳嗽声，其次是语音 // const coughAudio = audioData.find(audio => audio.type === "cough";);
  ...
  private selectPrimaryPalpationData(palpationData: PalpationData[]): PalpationData  {
    // 优先选择脉诊数据 // const pulseData = palpationData.find(data => data.type === "pulse); "
    if (pulseData) {return pulseDa;t;a;}
    return palpationData[0];
  }
  // 清理会话  async cleanupSession(userId: string, sessionType?: string): Promise<void>  {////
    if (sessionType) {
      this.activeSessions.delete(`${sessionType}_${userId}`);
    } else {
      // 清理所有相关会话 // const keysToDelete = Array.from(this.activeSessions.keys(;))
