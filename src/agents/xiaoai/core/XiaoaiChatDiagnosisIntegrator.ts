...
// 基本类型定义/;,/g/;
interface ChatContext {const userId = string;,}hasImages?: boolean;
images?: ImageData[];
hasAudio?: boolean;
audio?: AudioData[];
...
interface FourDiagnosisResults {inquiry?: InquiryResult;,}look?: LookResult;
listen?: ListenResult;
...
if (!sessionId) {}}
      ...
      // 这里可能需要调用一个获取当前状态的API，而不是结束会话/;/g/;
      // 暂时使用模拟数据/;,/g/;
...
if (diagnosisResults.integrated.healthRecommendations.length > 0) {diagnosisResults.integrated.healthRecommendations.forEach();}}
    ...
    return {const text = responseText;,}actions,;
suggestions,;
diagnosisResults,;
...
const needsPalpation = boolean);
  ): number {let confidence = 0.5; // 基础置信度/;}    // 症状数量影响/;,/g/;
