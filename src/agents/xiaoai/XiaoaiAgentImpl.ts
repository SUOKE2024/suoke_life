...
export class XiaoaiAgentImpl extends AgentBase {
  ...
  async processMessage(message: string, context: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      return this.createErrorResponse('抱歉，我暂时无法处理您的请求，请稍后再试。', error, context);
    }
  }
  ...
  private async analyzeInput(message: string, context: AgentContext): Promise<any> {
    ...
    if (context.deviceInfo?.capabilities?.includes('camera')) {
      if (keywords.includes('舌头') || keywords.includes('舌诊') || keywords.includes('看舌')) {
        return { type: 'tongue_diagnosis', confidence: 0.95, multimodal: true };
      }
      
      if (keywords.includes('面色') || keywords.includes('气色') || keywords.includes('面诊')) {
        return { type: 'face_analysis', confidence: 0.90, multimodal: true };
      }
    }
    
    // 检查语音交互
    if (keywords.includes('语音') || keywords.includes('说话') || keywords.includes('听')) {
      return { type: 'voice_interaction', confidence: 0.88 };
    }
    
    // 检查医疗咨询
    if (keywords.includes('症状') || keywords.includes('不舒服') || keywords.includes('疼痛') || keywords.includes('咨询')) {
      return { type: 'medical_consultation', confidence: 0.85 };
    }
    
    // 检查无障碍服务
    if (keywords.includes('导盲') || keywords.includes('手语') || keywords.includes('无障碍') || keywords.includes('辅助')) {
      return { type: 'accessibility_request', confidence: 0.92 };
    }
    
    // 检查健康档案管理
    if (keywords.includes('档案') || keywords.includes('记录') || keywords.includes('病历')) {
      return { type: 'health_record', confidence: 0.80 };
    }
    
    // 检查AI推理请求
    if (keywords.includes('分析') || keywords.includes('推理') || keywords.includes('判断')) {
      return { type: 'ai_inference', confidence: 0.75 };
    }
    
    return { type: 'general_conversation', confidence: 0.60 };
  }

  private async handleVoiceInteraction(analysis: any, context: AgentContext): Promise<any> {
    // 处理语音交互
    return {
      message: '我支持多语种语音交互，包括方言识别。请开始说话，我会仔细聆听。',
      data: {
        voiceCapabilities: this.voiceEngine.speechRecognition,
        supportedLanguages: this.voiceEngine.speechRecognition.languages,
        supportedDialects: this.voiceEngine.speechRecognition.dialects,
  ...
  private async handleMedicalConsultation(analysis: any, context: AgentContext): Promise<any> {
    ...
    const recommendations = await this.generateMedicalRecommendations(symptoms, context);
    
    return {
      message: '基于您描述的症状，我为您提供以下分析和建议：',
      data: {
        symptoms: symptoms,
        analysis: recommendations.analysis,
        suggestions: recommendations.suggestions,
        urgencyLevel: recommendations.urgencyLevel,
        followUpRequired: recommendations.followUpRequired,
        disclaimer: '此建议仅供参考，如症状持续或加重，请及时就医。'
      }
    };
  }
  ...
  private async handleTongueDiagnosis(analysis: any, context: AgentContext): Promise<any> {
    ...
    return {
      message: '舌诊分析完成，以下是详细的中医舌诊报告：',
      data: {
        tongueQuality: tongueFeatures.quality,
        tongueCoating: tongueFeatures.coating,
        tongueShape: tongueFeatures.shape,
        tongueColor: tongueFeatures.color,
        tcmAnalysis: tongueFeatures.tcmAnalysis,
        constitution: tongueFeatures.suggestedConstitution,
        recommendations: tongueFeatures.recommendations,
        confidence: tongueFeatures.confidence
      }
    };
  }
  ...
  private async handleFaceAnalysis(analysis: any, context: AgentContext): Promise<any> {
    ...
    return {
      message: '面诊分析完成，以下是您的面色气色分析：',
      data: {
        faceColor: faceFeatures.color,
        complexion: faceFeatures.complexion,
        spirit: faceFeatures.spirit,
        emotions: faceFeatures.emotions,
        tcmAnalysis: faceFeatures.tcmAnalysis,
        healthIndicators: faceFeatures.healthIndicators,
        recommendations: faceFeatures.recommendations
      }
    };
  }
    ...
    return {
      ...
      data: {
        services: [
          ...
          {
            name: '手语识别',
            description: '手语转文字、文字转手语',
            features: ['实时识别', '双向翻译', '多种手语', '学习模式']
          },
          {
            name: '语音引导',
            description: '全程语音操作指导',
            features: ['操作提示', '功能说明', '结果播报', '错误提醒']
          },
          {
            name: '老年友好',
            description: '大字体、高对比度、简化操作',
            features: ['界面优化', '操作简化', '语音提醒', '紧急求助']
          }
        ],
  ...
  private async handleAIInference(analysis: any, context: AgentContext): Promise<any> {
    // 处理AI推理请求
    const inferenceResult = await this.performAIInference(analysis.message, context);
    
    return {
      message: '基于AI推理分析，为您提供以下洞察：',
      data: {
  ...
  private async handleGeneralConversation(message: string, context: AgentContext): Promise<any> {
    // 处理一般对话
    return {
      message: '您好！我是小艾，索克生活的AI助手。我可以为您提供语音交互、健康咨询、中医诊断和无障碍服务。请告诉我您需要什么帮助？',
      data: {
        availableServices: [
          '语音交互（支持方言）',
          '中医舌诊分析',
  ...
  private async generateMedicalRecommendations(symptoms: string[], context: AgentContext): Promise<any> {
    // 生成医疗建议
    return {
      analysis: '根据症状分析，可能是感冒引起的不适',
      suggestions: [
        '多休息，保证充足睡眠',
        '多喝温水，保持水分',
        '注意保暖，避免受凉',
  ...
  private async analyzeTongueImage(context: AgentContext): Promise<any> {
    ...
    return {
      ...
      shape: '正常',
      color: '淡红色',
      tcmAnalysis: '舌质淡红，苔薄白，属正常舌象',
      suggestedConstitution: '平和质',
      recommendations: ['保持现有生活习惯', '注意饮食均衡'],
      confidence: 0.88
    };
  }
  ...
  private async analyzeFaceImage(context: AgentContext): Promise<any> {
    ...
    return {
      ...
      spirit: '精神饱满',
      emotions: ['平静', '愉悦'],
      tcmAnalysis: '面色红润有光泽，精神状态良好',
      healthIndicators: ['气血充足', '脏腑功能正常'],
      recommendations: ['继续保持良好作息', '适当运动']
    };
  }

  private async performAIInference(message: string, context: AgentContext): Promise<any> {
    // 执行AI推理
    return {
      analysis: '基于多模态AI分析...',
      reasoning: ['数据分析', '模式识别', '知识推理'],
      confidence: 0.85,
