...
const API_CONFIG = {
  ...
  look: {
    ...
    timeout: 30000,
    retries: 3
  },listen: {baseUrl: "http://localhost:8000",timeout: 30000,retries: 3;
  },palpation: {baseUrl: "http://localhost:8002",timeout: 30000,retries: 3;
  },calculation: {baseUrl: "http://localhost:8003",timeout: 30000,retries: 3;
  };
};
...
export class InquiryServiceClientImpl implements InquiryServiceClient {
  ...
  async startSession(userId: string): Promise<string> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({user_id: userId,session_type: "comprehensive",language: "zh-CN";
        });
      },
      this.timeout,
      this.retries,
  ...
  async askQuestion(sessionId: string, question: string): Promise<InquiryResult> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({session_id: sessionId,user_input: question,interaction_type: "question";
        });
      },
      this.timeout,
      this.retries,
  ...
  async getSymptomAnalysis(sessionId: string): Promise<any> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({session_id: sessionId,analysis_type: "comprehensive";
        });
      },
      this.timeout,
      this.retries,
...
export class LookServiceClientImpl implements LookServiceClient {
  ...
  async analyzeFace(imageData: ImageData): Promise<LookResult> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({image_data: base64Data,image_format: imageData.format,analysis_type: "comprehensive",metadata: {width: imageData.width,height: imageData.height,timestamp: Date.now();
          }
        })
      },
      this.timeout,
  ...
  async analyzeTongue(imageData: ImageData): Promise<LookResult> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({image_data: base64Data,image_format: imageData.format,analysis_type: "tongue_diagnosis";
        });
      },
      this.timeout,
      this.retries,
...
export class ListenServiceClientImpl implements ListenServiceClient {
  ...
  async analyzeVoice(audioData: AudioData): Promise<ListenResult> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({audio_data: base64Data,audio_format: audioData.format,duration: audioData.duration,sample_rate: audioData.sampleRate,analysis_type: "comprehensive";
        });
      },
      this.timeout,
      this.retries,
  ...
  async analyzeBreathing(audioData: AudioData): Promise<ListenResult> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({audio_data: base64Data,audio_format: audioData.format,analysis_type: "breathing_pattern";
        });
      },
      this.timeout,
      this.retries,
...
export class PalpationServiceClientImpl implements PalpationServiceClient {
  ...
  async analyzePalpation(data: PalpationData): Promise<PalpationResult> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({palpation_type: data.type,sensor_data: data.sensorData,user_id: "current_user",metadata: data.metadata || {};
        });
      },
      this.timeout,
      this.retries,
  ...
  async startRealTimeMonitoring(userId: string): Promise<string> {
    ...
    const response = await apiRequest<any>(;
      url,{method: "POST",body: JSON.stringify({user_id: userId,monitoring_type: "real_time";
        });
      },
      this.timeout,
      this.retries,
...
export class DiagnosisServiceClientImpl implements DiagnosisServiceClient {
  ...
  async healthCheck(): Promise<{ [key: string]: boolean }> {
    ...
    const healthChecks = services.map(async (service) => {try {const config = API_CONFIG[service as keyof typeof API_CONFIG];
        ...
        const response = await fetch(`${config.baseUrl}/health`, {method: "GET",signal: controller.signal,headers: {"Content-Type": "application/json";
          };
        });
  ...
  async comprehensiveDiagnosis(data: {
    ...
    try {
      ...
      const comprehensiveResult = await apiRequest<any>(;
        calculationUrl,{method: "POST",body: JSON.stringify({user_id: data.userId,diagnosis_results: results,timestamp: Date.now();
          })
        },
