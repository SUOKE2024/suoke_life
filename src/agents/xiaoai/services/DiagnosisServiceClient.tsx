...
export class LookServiceClientImpl implements LookServiceClient {
  ...
  async analyzeFace(imageData: ImageData): Promise<LookResult> {
    ...
    return {
      ...
      faceFeatures: response.face_features || {},
      complexionAnalysis: response.complexion_analysis || {},
      tongueAnalysis: response.tongue_analysis || {},
      overallAssessment: response.overall_assessment || "面部分析完成",
      confidence: response.confidence || 0.8,
      recommendations: response.recommendations || []
    };
  }
  ...
  async analyzeTongue(imageData: ImageData): Promise<LookResult> {
    ...
    return {
      analysisId: response.analysis_id,
      tongueAnalysis: response.tongue_analysis || {},
      overallAssessment: response.overall_assessment || "舌诊分析完成",
      confidence: response.confidence || 0.8
    };
  }
}
...
export class ListenServiceClientImpl implements ListenServiceClient {
  ...
  async analyzeVoice(audioData: AudioData): Promise<ListenResult> {
    ...
    return {
      analysisId: response.analysis_id,
      voiceFeatures: response.voice_features || {},
      breathingPattern: response.breathing_pattern || {},
      overallAssessment: response.overall_assessment || "语音分析完成",
      confidence: response.confidence || 0.8
    };
  }
  ...
  async analyzeBreathing(audioData: AudioData): Promise<ListenResult> {
    ...
    const response = await apiRequest<any>(
      url,
      {
        ...
        body: JSON.stringify({
          audio_data: base64Data,
          audio_format: audioData.format,
          analysis_type: "breathing_pattern"
        })
      },
      this.timeout,
      this.retries,
    ...
    return {
      analysisId: response.analysis_id,
      breathingPattern: response.breathing_analysis || {},
      overallAssessment: response.overall_assessment || "呼吸音分析完成",
      confidence: response.confidence || 0.8
    };
  }
}
...
export class PalpationServiceClientImpl implements PalpationServiceClient {
  ...
  async analyzePalpation(data: PalpationData): Promise<PalpationResult> {
    ...
    return {
      ...
      overallAssessment: response.overall_assessment || "切诊分析完成",
      confidence: response.confidence || 0.8
    };
  }
...
export class DiagnosisServiceClientImpl implements DiagnosisServiceClient {
  ...
  async comprehensiveDiagnosis(data: {
    ...
    try {
      ...
      throw new DiagnosisApiError(
        `综合诊断失败: ${error instanceof Error ? error.message : '未知错误'}`,
        undefined,
        'comprehensive',
        true
      );
