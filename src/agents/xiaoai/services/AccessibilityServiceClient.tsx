...
code?: number}
  ...
  UserProfile,
  ImageData,
  { AudioData } from "../types";// // 无障碍服务配置 /////     const ACCESSIBILITY_API_CONFIG =  {baseUrl: "http:// , localhost:50051",  * // 无障碍服务地址 * / timeout: 30000 /////     ;};
// 无障碍服务响应类型 * export interface AccessibilityResponse { success: boolean////  ;
 /////    ;
  data?: unknown;
  error?: string}
export interface VoiceAssistanceResponse { recognized_text: string,response_text: string,response_audio: string;
...
export interface SignLanguageResponse { recognized_text: string,
  ...
  success: boolean;
  error?: string}
export interface AccessibleContentResponse { accessible_content: string,content_url: string,audio_content: string;
...
// 无障碍服务客户端   对应后端Python的AccessibilityClient实现export class AccessibilityServiceClient  {private baseUrl: string;
  ...
  // 处理语音输入，支持语音识别和语音辅助  async processVoiceInput(audioData: AudioData,
    ...
    try {
      ...
      return {recognized_text: ",",response_text: `语音处理失败: ${(error as Error).message}`,response_audio: ",",confidence: 0.0,success: false,error: (error as Error).messag;e;
      ;}
    }
  }
  ...
  // 管理用户的无障碍设置  async manageAccessibilitySettings(userId: string,
    ...
    try {
      ...
      return {current_preferences: {language: "zh-CN",voice_type: "female",speech_rate: 1.0,high_contrast: false,screen_reader: false,font_size: "medium";
