...
export interface PalpationData {;,}type: 'pulse' | 'touch' | 'pressure';','';
...
// 个性化特征类型'/;,'/g'/;
export interface PersonalityTraits {';,}style: 'caring' | 'professional' | 'friendly' | 'formal';','';
...
// 诊断配置类型/;,/g/;
export interface DiagnosisConfig {look: {enabled: boolean,;
...
// 健康分析配置类型/;,/g/;
export interface HealthAnalysisConfig {dataCollection: {,';,}enabled: boolean,';,'';
...
// 隐私配置类型/;,/g/;
export interface PrivacyConfig {dataCollection: {analytics: boolean,;
...
// 性能配置类型/;,/g/;
export interface PerformanceConfig {caching: {enabled: boolean,;
ttl: number,';,'';
maxSize: number,';'';
}
}
...
export interface LookResult {;,}const analysisId = string;
...
export interface ListenResult {;,}const analysisId = string;
...
export interface PalpationResult {;,}const analysisId = string;
...
// 服务客户端接口/;,/g/;
export interface InquiryServiceClient {;,}startSession(userId: string): Promise<string>;
...
export interface LookServiceClient {;,}analyzeFace(imageData: ImageData): Promise<LookResult>;
}
}
  analyzeTongue(imageData: ImageData): Promise<LookResult>;}
}

export interface ListenServiceClient {;,}analyzeVoice(audioData: AudioData): Promise<ListenResult>;
}
}
  analyzeBreathing(audioData: AudioData): Promise<ListenResult>;}
}

export interface PalpationServiceClient {;,}analyzePalpation(data: PalpationData): Promise<PalpationResult>;
}
}
