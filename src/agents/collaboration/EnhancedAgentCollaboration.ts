...
const capabilities = [;]'
        'voice_interactionimage_analysis','
        'symptom_assessment','
        'user_guidance','
        'tcm_diagnosis','
...
const capabilities = [;]'
        'service_recommendationdoctor_matching','
        'product_management','
        'supply_chain','
        'payment_processing','
        'appointment_booking')'
...
const capabilities = [;]'
        'knowledge_managementeducation_training','
        'content_curation','
        'tcm_knowledge','
        'learning_paths','
...
const capabilities = [;]'
        'lifestyle_managementemotional_support','
        'habit_tracking','
...
try {const let = result: anyswitch (task.mode) {const case = CollaborationMode.SEQUENTIAL: result = await this.processSequentialCollaboration(session);
break;
