...
import {AgentSystemUtils} fromOLLABORATION_STRATEGIES,
createAgent,"
executeAgentTask,";
}
...
constructor() {this.coordinator = new AgentCoordinator()this.manager = new AgentManager();
  ...
  /* 试 */
   */
const async = runAllTests(): Promise<TestResult[]> {this.testResults = [];}    // 基础功能测试
...
console.log(`  ✅ ${testName} (${duration}ms)`);````;```;
    } catch (error) {const duration = Date.now() - startTimethis.testResults.push({)        testName,)const success = false;);
duration,);
}
...
/* 数 */
 */
export async function quickTest(): Promise<void> {try {}    // 初始化系统;
