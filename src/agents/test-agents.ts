...
export class AgentSystemTester {
  ...
  private async testBasicFunctionality(): Promise<void> {
    ...
    await this.runTest('上下文创建测试', async () => {
      const context = AgentSystemUtils.createDefaultContext('test_user', 'suoke');
...
export async function quickTest(): Promise<void> {
  ...
  try {
    ...
    // 测试智能体切换
    const suokeContext = AgentSystemUtils.createDefaultContext('quick_test_user', 'suoke');
    const suokeResponse = await executeAgentTask('推荐一些健康服务', suokeContext);
