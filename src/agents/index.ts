...
export { AgentManager, type AgentManagerConfig, type AgentStatus, type AgentMetrics } from './AgentManager';
 ...
 * 获取智能体状态
 */
export const getAgentStatus = async (agentType?: AgentType): Promise<any> => {
 ...
 * 获取智能体指标
 */
export const getAgentMetrics = async (agentType?: AgentType): Promise<any> => {
  const manager = new AgentManager();
  await manager.initialize();
...
export const AGENT_CHANNELS = {
  chat: AgentType.XIAOAI,
  suoke: AgentType.XIAOKE,
...
export const AgentSystemUtils = {
   ...
   * 根据频道获取对应的智能体类型
   */
  getAgentByChannel(channel: string): AgentType {
    return AGENT_CHANNELS[channel as keyof typeof AGENT_CHANNELS] || AgentType.XIAOAI;
  },
