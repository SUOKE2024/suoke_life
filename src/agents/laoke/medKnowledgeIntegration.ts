import { medKnowledgeService, KnowledgeQuery, Constitution, Symptom, HealthRecommendation } from '../../services/medKnowledgeService';
...
export class LaokeKnowledgeIntegration {
  ...
  async analyzeConstitutionBySymptoms(symptoms: string[]): Promise<{
    ...
    try {
      ...
      const overallConfidence = topMatches.length > 0 
  ...
  async getPersonalizedAdvice(
    ...
    try {
      ...
      // 获取个性化推荐
      const recommendations = await medKnowledgeService.getPersonalizedRecommendations({
  ...
  private generateConstitutionReasoning(
    ...
    const reasoning = [
      `根据症状分析，您最可能的体质类型是${topMatch.constitution.name}（${topMatch.constitution.type}）。`,
      `匹配的症状包括：${topMatch.matchingSymptoms.join('、')}。`,
      `该体质的主要特征：${topMatch.constitution.characteristics.slice(0, 3).join('、')}。`,
      `建议关注：${topMatch.constitution.recommendations.slice(0, 2).join('、')}。`
    ];
  ...
  private generateCustomAdvice(
    ...
    const advice = [
      `基于您的${constitution.type}体质特点，为您提供以下个性化建议：`,
      '',
      '【饮食调养】',
      ...constitution.lifestyle.diet.slice(0, 3).map(item => `• ${item}`),
      '',
      '【运动养生】',
      ...constitution.lifestyle.exercise.slice(0, 3).map(item => `• ${item}`),
      '',
      '【情志调节】',
      ...constitution.lifestyle.emotion.slice(0, 2).map(item => `• ${item}`),
    ];
    
    if (userContext.currentSymptoms && userContext.currentSymptoms.length > 0) {
      advice.push('', '【针对性调理】');
      advice.push(`针对您当前的${userContext.currentSymptoms.join('、')}症状，建议加强相应的调理措施。`);
    }
  ...
  private generateTCMAnalysis(symptoms: Symptom[], constitutions: Constitution[]): string {
    ...
    if (constitutions.length > 0) {
      analysis.push(`体质倾向于${constitutions[0].type}，需要注意${constitutions[0].characteristics.slice(0, 2).join('、')}。`);
    }
  ...
  private generateGraphInsights(relationships: any[], neighbors: any[]): string[] {
    const insights = [
      '基于知识图谱分析，发现以下关联：',
      `相关实体数量：${neighbors.length}个`,
      `关系类型：${relationships.length}种`,
    ];
  ...
  private calculateHealthScore(assessmentData: any, symptomAnalysis: any): number {
    ...
    // 根据症状严重程度扣分
    const severeSymptoms = symptomAnalysis.symptoms.filter(
      (s: Symptom) => s.severity === 'severe'
    );
