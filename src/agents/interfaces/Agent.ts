...
/**
 * * 智能体响应接口
export interface AgentResponse {messageId: string;
  agentId: string;
  success: boolean;
  data?: any;
  error?: string;
...
/**
 * * 经验接口
export interface Experience {taskId: string;
  action: string;
  context: any;
  outcome: any;
...
/**
 * * 学习统计接口
export interface LearningStats {totalExperiences: number;
  successRate: number;
...
/**
 * * 认证配置接口
export interface AuthConfig {type: "none" | token" | "certificate;
  credentials?: any;
}
/**
 * * 资源限制接口
export interface ResourceLimits {maxMemoryMB: number;
...
/**
 * * 健康检查接口
export interface HealthCheck {name: string;
...
/**
 * * 智能体指标接口
export interface AgentMetrics {tasksProcessed: number;
...
export default {
  AgentStatus,AgentType,MessageType,MessagePriority,LogLevel;
};
  */////
