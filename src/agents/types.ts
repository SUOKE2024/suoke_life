...
// 智能体上下文/;,/g/;
export interface AgentContext {;,}const userId = string;
sessionId?: string;
...
currentChannel?: string; // 当前频道：首页、SUOKE、探索、LIFE/;/g/;
}
}
  [key: string]: any;}
}
...
export interface AvailabilityInfo {;,}const available = boolean;
schedule?: Schedule;
nextAvailable?: Date;
capacity?: number;
}
...
';,'';
export interface AppointmentAction {';,}const type = 'book' | 'reschedule' | 'cancel' | 'confirm';';,'';
...
constitution?: ConstitutionType; // 中医体质/;,/g/;
...
// 健康数据/;,/g/;
export interface HealthData {;,}vitals?: VitalSigns;
symptoms?: Symptom[];
...
// 医疗记录/;,/g/;
export interface MedicalRecord {id: string,';,}date: Date,';,'';
...
// 症状/;,/g/;
export interface Symptom {name: string}severity: 1 | 2 | 3 | 4 | 5; // 1-轻微, 5-严重/;,/g/;
...
// 活动数据'/;,'/g'/;
export interface ActivityData {';,}type: 'walking' | 'running' | 'cycling' | 'swimming' | 'other';','';
...
hydration?: number; // 毫升/;/g/;
}

// 餐食'/;,'/g'/;
export interface Meal {';,}type: 'breakfast' | 'lunch' | 'dinner' | 'snack';','';
foods: Food[],;
timestamp: Date,;
}
}
...
// 情绪数据'/;,'/g'/;
export interface MoodData {';,}mood: 'very_sad' | 'sad' | 'neutral' | 'happy' | 'very_happy';','';
stress: 1 | 2 | 3 | 4 | 5,;
...
// 智能体健康状态/;,/g/;
export interface AgentHealthStatus {';,}agentType: AgentType,';,'';
