...
export class AgentUsageExample {;
  // /    示例1: 小艾智能体 - 四诊协调与健康咨询;  public static async xiaoaiHealthConsultationExample() {
    // 1. 创建小艾智能体 *     const agentFactory = AgentFactory.getInstance;(;); */
    const xiaoai = await agentFactory.createXiaoaiAge;n;t;(;)
    `);
    .join(", ")}...`);
    ...
    const chatResponse = await apiService.xiaoaiChat({
      ...
      userId: "user123",
      sessionId: "session45;6"
    ;};);
    // 3. 四诊协调示例 *     const diagnosisResponse = await apiService.xiaoaiFourDiagnosis({ */;
      userId: "user123",
  ...
  // /    示例2: 小克智能体 - 服务管理与农产品定制;  public static async xiaokeServiceManagementExample() {
    ...
    // 1. 服务订阅管理 *     const subscriptionResponse = await apiService.xiaokeServiceManagement({ */
      userId: "user123",
