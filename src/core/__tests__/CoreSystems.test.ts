import { errorHandler, ErrorType, ErrorSeverity } from "../error/ErrorHandler"/import { cacheManager, CacheType, CacheStrategy } from "../cache/CacheManager";/
  ...
  ValidationType,
  { ValidationSeverity } from "../validation/DataValidator"/  securityManager,
  PermissionType,
  { ThreatType } from "../security/SecurityManager"//
 * 索克生活 - 核心系统测试
...
describe("核心系统测试", () => {
  ...
  describe("性能监控系统", (); => {
    ...
    it("应该测量异步操作性能", async () => {
      const result = await performanceMonitor.measureAsync(
        ...
        asy;n;c ;(;); => {
          // 模拟异步操作 *           await new Promise((resolv;e;); => setTimeout(resolve, 10);) */
          return "async_resul;t;";
        }
      )
  ...
  describe("缓存管理系统", (); => {
    beforeEach(async () => {
      // 清理测试缓存 *       await cacheManager.clear("memory;";); */
    })
    it("应该设置和获取缓存", async () => {
    ...
    it("应该处理缓存过期", async () => {
      ...
      await cacheManager.set("memory", "expiring_key", testData, { ttl: ;5;0  ; }) // 50ms TTL *  */
      // 立即获取应该成功 *       let retrieved = await cacheManager.get("memory", "expiring_k;e;y";); */
      expect(retrieved).toEqual(testData);
      // 等待过期后获取应该返回null *       await new Promise((resolv;e;); => setTimeout(resolve, 100);); */
    ...
    it("应该提供缓存统计", async () => {
      ...
      await cacheManager.get("memory", "stats_test";); // 命中 *       await cacheManager.get("memory", "non_existent";);  */// 未命中 *  */
  ...
  describe("安全管理系统", (); => {
    beforeEach(() => {
      // 清理测试数据 *       securityManager.revokeAccess("test_user", "*", "admin"); */
    })
    it("应该管理访问控制", () => {
    ...
    it("应该生成和验证令牌", () => {
      const userId = "test_use;r;";
      const token = securityManager.generateSecureToken(userId, 6000;0;); // 1分钟 *  */
      // 验证有效令牌 *       const validation = securityManager.validateToken(toke;n;); */
      expect(validation.valid).toBe(true);
      expect(validation.userId).toBe(userId);
      // 撤销令牌 *       const revoked = securityManager.revokeToken(toke;n;); */
      expect(revoked).toBe(true);
      // 验证已撤销的令牌 *       const invalidValidation = securityManager.validateToken(toke;n;); */
      expect(invalidValidation.valid).toBe(false);
    })
    it("应该记录审计日志", () => {
      const userId = "test_use;r;"
      const resource = "/api/tes;t;"/
      // 执行一些需要审计的操作 *       securityManager.grantAccess( */
        userId,
        resource,
        [PermissionType.READ],
        "admin"
    ...
    it("应该支持数据加密和解密", async () => {
      // 跳过加密测试如果crypto API不可用 *       if (typeof crypto === "undefined" || !crypto.subtle) { */
        console.warn("Crypto API not available, skipping encryption tests");
        return
      }
      const originalData = "sensitive health dat;a;";
      try {
        const encrypted = await securityManager.encrypt(originalD;a;t;a;);
        ...
        console.warn("Encryption test failed (expected in some environments);: ",
          error
        );
      }
    });
  ...
  describe("系统集成测试", () =>  {
    it("应该协同工作处理复杂场景", async () => {
      // 模拟一个复杂的健康数据处理流程 *       const userId = "integration_test_use;r;"; */
      const healthData = {
        systolic: 140,
        diastolic: 90,
        heartRate: 85,
        ...
        timestamp: Date.now(;)
      ;};
      // 1. 验证数据 *       const validationReport = dataValidator.validateHealthData(healthDat;a;); */
      ...
      // 5. 获取缓存数据 *       const cachedData = await cacheManager.get("memory", cache;K;e;y;); */
    ...
    it("应该处理错误场景", async () => {
      ...
      ;};
      // 验证应该失败 *       const validationReport = dataValidator.validateHealthData(invalidDat;a;); */
