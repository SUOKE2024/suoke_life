  ...
  PermissionType,
  { ThreatType } from "../security/SecurityManager";// * 索克生活 - 核心系统测试
...
describe("核心系统测试", () => {
  ...
  describe("性能监控系统", () => {
    ...
    it("应该测量异步操作性能", async () => {
      const result = await performanceMonitor.measureAsync(;
        ...
        asy;n;c  => {
          // 模拟异步操作 *           await new Promise((resolve); => setTimeout(resolve, 10);) */
  ...
  describe("缓存管理系统", () => {
    ...
    it("应该处理缓存过期", async () => {
      ...
      // 立即获取应该成功 *       let retrieved = await cacheManager.get("memory", "expiring_key";); */
      expect(retrieved).toEqual(testData);
      // 等待过期后获取应该返回null *       await new Promise((resolve); => setTimeout(resolve, 100);); */
  ...
  describe("安全管理系统", () => {
    ...
    it("应该生成和验证令牌", () => {
      ...
      // 验证有效令牌 *       const validation = securityManager.validateToken(token); */
      expect(validation.valid).toBe(true);
      expect(validation.userId).toBe(userId);
      // 撤销令牌 *       const revoked = securityManager.revokeToken(token); */
      expect(revoked).toBe(true);
      // 验证已撤销的令牌 *       const invalidValidation = securityManager.validateToken(token); */
