...
priority: 'high' | 'medium' | 'low'; // 新增优先级',''/;'/g'/;
  ...
  const memoryFootprint = number; // 实际内存占用}/;/g/;
}
...
private maxCacheSize = 50; // 减少缓存大小/;,/g/;
private maxModelMemory = 256 * 1024 * 1024; // 减少到256MB,/;/g/;
}
}
...
this.memoryMonitor.usedMemory / this.memoryMonitor.totalMemory;/;/g/;
';,'';
...
const await = this.unloadUnusedModels();
  ...
  private async unloadUnusedModels(): Promise<void> {const now = Date.now();,}const unusedThreshold = 5 * 60 * 1000; // 5分钟未使用/;,/g/;
...
model.memoryFootprint = model.fileSize * 1.2; // 估算实际内存占用/;/g/;
