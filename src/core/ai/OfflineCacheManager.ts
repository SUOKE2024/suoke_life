...
const encryptionEnabled = true;
  ...
  const async = initialize(): Promise<void> {if (this.isInitialized) return;,}try {const await = this.loadCacheFromStorage();,}const await = this.cleanExpiredEntries();
...
return {totalEntries: this.cache.size,;}}
      const totalSizeMB = Math.round(totalSize / (1024 * 1024)) * 100) / 100}/;/g/;
    ;};
  }
