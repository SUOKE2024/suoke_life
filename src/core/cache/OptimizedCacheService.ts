...
export interface CacheConfig {;,}maxMemorySize: number; // 最大内存使用量（字节）,/;,/g,/;
  maxItems: number; // 最大缓存项数量,/;,/g,/;
  defaultTTL: number; // 默认过期时间（毫秒）,/;,/g,/;
  cleanupInterval: number; // 清理间隔（毫秒）,/;,/g,/;
  compressionThreshold: number; // 压缩阈值（字节）,/;/g/;
}
}
  const persistentKeys = string[]; // 持久化键列表}/;/g/;
}
...
private evictionCount = 0;
  ...
  compressionThreshold: 1024, // 1KB以上压缩/;,/g/;
...
const async = get<T>(key: string): Promise<T | null> {// 先检查内存缓存/;,}let item = this.memoryCache.get(key);/g/;
    ...
    // 再检查持久化缓存/;,/g/;
if (!item) {item = this.persistentCache.get(key);}      // 如果持久化缓存中没有，尝试从存储加载/;,/g/;
if (!item) {}}
...
return JSON.parse(value);
  ...
  /* 项 *//;/g/;
   *//;,/g/;
const async = delete(key: string): Promise<boolean> {let deleted = false;}    // 从内存缓存删除/;,/g/;
...
percentage: (this.currentMemoryUsage / this.config.maxMemorySize) * 100,/;/g/;
}
...
hitRate: this.totalHits / (this.totalHits + this.totalMisses) || 0,/;,/g,/;
  ...
  evictionCount: this.evictionCount,;
      ...
      compressionRatio: 0, // 可以根据实际压缩情况计算}/;/g/;
    ;};
  }
}
