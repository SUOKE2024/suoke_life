...
GlobalErrorHandler.instance = new GlobalErrorHandler(config || defaultConfig);
  ...
  /* 误 */
  *//,/g,/;
  public: handleError(error: Error | ErrorInfo, context?: any): void {const errorInfo = this.normalizeError(error; context);}    // 记录错误
...
const stackTrace = error.stack;});
  ...
  /* 误 */
  *//,/g,/;
  public: handleApiError(status: number, message: string, endpoint?: string): void {const severity = status >= 500 ? ErrorSeverity.HIGH : ErrorSeverity.MEDIUMthis.handleError({)      const type = ErrorType.API_ERROR;)severity,);
);
}
...
return {const total = this.errorQueue.lengthbyType,
}
      bySeverity,}
...
timestamp: new Date(error.timestamp).toISOString(),
  ...
  /* 误 */
  */
private async reportError(error: ErrorInfo): Promise<void> {if (!this.config.reportingEndpoint) returntry {'await: fetch(this.config.reportingEndpoint, {',)method: "POST,")";}}
      const headers = {)"};
...
const body = JSON.stringify(error);});
  ...
  /* 户 */
  */
private notifyUser(error: ErrorInfo): void {if (error.severity === ErrorSeverity.LOW) returnconst userMessage = this.getUserFriendlyMessage(error);
