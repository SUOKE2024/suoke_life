...
export class PerformanceMonitor {
  ...
  public generateReport(timeRangeMs: number = 3600000): PerformanceReport {
    ...
    for (const [key, metricHistory] of this.metrics.entries()) {
      const recentMetrics = metricHistory.filter(m => m.timestamp >= startTime);
      ...
      recentMetrics.forEach(metric => {
        ...
        // 计算平均响应时间
        if (metric.category === PerformanceCategory.NETWORK || 
   ...
   * 清除指标历史
   */
  public clearMetrics(category?: PerformanceCategory): void {
    if (category) {
      // 清除特定类别的指标
  ...
  private setupDefaultThresholds(): void {
    // 网络性能阈值
    this.setThreshold(PerformanceCategory.NETWORK, 'api_response_time', 1000, 3000);
    this.setThreshold(PerformanceCategory.NETWORK, 'download_time', 5000, 15000);

    // 内存使用阈值
    this.setThreshold(PerformanceCategory.MEMORY, 'heap_used', 100, 200, 'MB');
    this.setThreshold(PerformanceCategory.MEMORY, 'heap_total', 150, 300, 'MB');

    // 渲染性能阈值
    this.setThreshold(PerformanceCategory.RENDER, 'frame_time', 16, 33);
    this.setThreshold(PerformanceCategory.RENDER, 'layout_time', 10, 20);

    // 智能体性能阈值
    this.setThreshold(PerformanceCategory.AGENT, 'response_time', 2000, 5000);
    this.setThreshold(PerformanceCategory.AGENT, 'processing_time', 1000, 3000);

    // 数据库性能阈值
    this.setThreshold(PerformanceCategory.DATABASE, 'query_time', 500, 2000);
    this.setThreshold(PerformanceCategory.DATABASE, 'connection_time', 1000, 3000);
  }
  ...
  private checkThreshold(metric: PerformanceMetric): void {
    ...
    if (metric.value >= metric.threshold.critical) {
      console.error(`🚨 CRITICAL: ${metric.name} = ${metric.value}${metric.unit} (threshold: ${metric.threshold.critical}${metric.unit})`);
    } else if (metric.value >= metric.threshold.warning) {
      console.warn(`⚠️ WARNING: ${metric.name} = ${metric.value}${metric.unit} (threshold: ${metric.threshold.warning}${metric.unit})`);
    }
  }
  ...
  private analyzeTrends(timeRangeMs: number): { improving: string[]; degrading: string[]; stable: string[] } {
    ...
    for (const [key, metricHistory] of this.metrics.entries()) {
      const recentMetrics = metricHistory.filter(m => m.timestamp >= now - timeRangeMs);
      
      if (recentMetrics.length < 10) continue; // 需要足够的数据点

      const firstHalf = recentMetrics.filter(m => m.timestamp < now - halfRange);
      const secondHalf = recentMetrics.filter(m => m.timestamp >= now - halfRange);
  ...
  private generateRecommendations(metrics: PerformanceMetric[]): string[] {
    ...
    // 分析智能体性能
    const agentMetrics = metrics.filter(m => m.category === PerformanceCategory.AGENT);
    ...
    // 分析渲染性能
    const renderMetrics = metrics.filter(m => m.category === PerformanceCategory.RENDER);
