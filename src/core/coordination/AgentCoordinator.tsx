...
/* 配 *//;/g/;
*/"/;,"/g"/;
export enum AgentType {";,}XIAOAI = 'XIAOAI', // 小艾 - 健康助手 & 首页聊天频道版主"/;,"/g"/;
XIAOKE = 'XIAOKE', // 小克 - 中医诊断专家"/;,"/g"/;
LAOKE = 'LAOKE', // 老克 - 资深健康顾问"/;"/g"/;
}
}
  SOER = 'SOER', // 索儿 - 生活方式指导师"}""/;"/g"/;
}";,"";
...
averageProcessingTime: number; // 毫秒,/;/g/;
}
}
...
dependencies?: string[]; // 依赖的其他任务ID;/;,/g/;
collaborators?: AgentType[]; // 需要协作的其他智能体/;,/g/;
result?: unknown;
error?: unknown;
}
...
requesterUserId: string,;
data: unknown,;
const options = {estimatedDuration?: number;,}dependencies?: string[];
}
...
private generateTaskId(): string {}
  ...
  private sortTaskQueue(): void {this.taskQueue.sort(a, b) => {}      // 优先级高的在前/;,/g/;
...
if (assignedAgent) {task.assignedAgent = assignedAgent;,}task.status = TaskStatus.ASSIGNED;
...
return agent.activeTasks.length < maxTasks;
  ...
  private monitorAgentHealth(): void {const now = Date.now();,}const healthTimeout = 30000; // 30秒/;,/g/;
...
task.assignedAgent = undefined;
  ...
  private optimizeTaskDistribution(): void {// 检查负载均衡/;,}const agents = Array.from(this.agents.values()).filter(;);/g/;
    ...
    const  avgLoad =;
agents.reduce(sum, agent) => sum + agent.currentLoad, 0) / agents.length;/;,/g/;
const threshold = 0.3; // 负载差异阈值/;,/g/;
for (const agent of agents) {if (agent.currentLoad > avgLoad + threshold) {}        // 尝试将一些任务转移给负载较低的智能体/;/g/;
};
this.redistributeTasks(agent.type);}
  ...
  private redistributeTasks(overloadedAgentType: AgentType): void {}}
    ...
    const tasksToRedistribute = overloadedAgent.activeTasks;
      ...
      .slice(0, Math.ceil(overloadedAgent.activeTasks.length * 0.2)); // 重分配20%的任务/;,/g/;
...
const totalTasks = perf.totalTasksCompleted + 1;
    // 更新成功率/;,/g/;
perf.successRate =;
      (perf.successRate * perf.totalTasksCompleted + (success ? 1 : 0)) //;,/g/;
totalTasks;
    // 更新平均响应时间/;,/g/;
perf.averageResponseTime =;
      (perf.averageResponseTime * perf.totalTasksCompleted + duration) //;,/g/;
totalTasks;
    // 更新完成任务数/;,/g/;
...
for (const task of dependentTasks) {if (this.areDependenciesMet(task)) {}        // 将任务重新加入队列/;,/g/;
...
this.taskQueue.push(task);}
  ...
  private handleTaskFailure(task: Task): void {// 记录失败原因/;,}errorHandler.logError(ErrorType.TASK_EXECUTION_ERROR, {)      taskId: task.id,);,}agentType: task.assignedAgent,);/g/;
    ...
    // 如果是关键任务，尝试重新分配/;,/g/;
if (task.priority >= TaskPriority.HIGH) {task.status = TaskStatus.PENDING;,}task.assignedAgent = undefined;
this.taskQueue.push(task);
}
      this.sortTaskQueue();}
    }
