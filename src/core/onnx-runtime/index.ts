...
// 便捷工具函数
import { ONNXInferenceEngine } from './ONNXInferenceEngine';
...
import { TensorProcessor } from './TensorProcessor';
...
export class ONNXRuntimeManager {
  ...
  async initialize(config?: InferenceConfig): Promise<void> {
    ...
    try {
      ...
      console.log('设备能力检测完成:', deviceCapabilities);
      ...
      console.log('ONNX Runtime管理器初始化完成');

    } catch (error) {
      console.error('ONNX Runtime管理器初始化失败:', error);
      throw error;
    }
  }
  ...
  async destroy(): Promise<void> {
    ...
    try {
      ...
      console.log('ONNX Runtime管理器已销毁');

    } catch (error) {
      console.error('销毁ONNX Runtime管理器失败:', error);
    }
  }
  ...
  getSystemStatus() {
    return {
      ...
      loadedModels: this.engine.getLoadedModels(),
      deviceCapabilities: this.deviceDetector.getLastDetectionResult(),
      performanceMetrics: this.engine.getPerformanceMetrics(),
      cacheStats: this.cache.getStats(),
      edgeComputeStats: this.edgeManager.getStats()
    };
  }
 ...
 * 创建ONNX Runtime管理器实例
 */
export function createONNXRuntimeManager(): ONNXRuntimeManager {
  return new ONNXRuntimeManager();
}
 ...
 */
export function getGlobalONNXRuntimeManager(): ONNXRuntimeManager {
  if (!globalManager) {
    globalManager = new ONNXRuntimeManager();
  }
...
export async function quickDeploy(
  ...
  const model = await manager.deployModel(modelPath, {
    quantize: true,
    optimize: true,
    cache: true
  });
