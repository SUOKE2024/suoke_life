 ...
 */
export class ONNXInferenceEngine extends EventEmitter {
  ...
  async loadModel(
    ...
    try {
      ...
      // 创建会话元数据
      const sessionMetadata: CustomInferenceSession = {
        sessionId,
        modelId: model.id,
        isActive: true,
        createdAt: new Date(),
      ...
      console.log(`模型加载完成: ${model.name}, 会话ID: ${sessionId}`);
      return sessionId;
  ...
  async unloadModel(sessionId: string): Promise<void> {
    ...
    try {
      ...
      console.log(`模型已卸载: ${sessionId}`);
    } catch (error) {
      console.error(`卸载模型失败: ${error.message}`);
    }
  }
  ...
  private buildSessionOptions(options?: ModelOptimizationOptions): any {
    ...
    return {
      enableCpuMemArena: mergedOptions.enableCPUMemArena,
      enableMemPattern: mergedOptions.enableMemoryPattern,
      executionMode: mergedOptions.executionMode,
      graphOptimizationLevel: mergedOptions.graphOptimizationLevel,
      logSeverityLevel: this.mapLogLevel(mergedOptions.logSeverityLevel),
      enableProfiling: mergedOptions.enableProfiling
    };
  }
