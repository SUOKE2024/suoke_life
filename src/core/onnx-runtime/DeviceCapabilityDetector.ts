...
import {
  ...
  NPUCapabilities,
  ExecutionProvider,
  { EdgeComputeConfig } from './types';
import { PROVIDER_PRIORITY, DEFAULT_CONFIGS, DEVICE_THRESHOLDS } from './constants';
...
export class DeviceCapabilityDetector {
  ...
  async detectCapabilities(): Promise<DeviceCapabilities> {
    ...
    try {
      ...
      console.log('设备能力检测完成:', this.capabilities);
      return this.capabilities;
  ...
  isModelCompatible(modelRequirements: ModelRequirements): CompatibilityResult {
    ...
    return {
      ...
      reason: issues.join(', '),
      recommendations,
      estimatedPerformance: this.estimateModelPerformance(modelRequirements)
    };
  }
  ...
  private async detectCPUCapabilities(): Promise<CPUCapabilities> {
    ...
    try {
      ...
      const capabilities: CPUCapabilities = {
      ...
      console.warn('CPU能力检测失败，使用默认值:', error);
  ...
  private async detectMemoryCapabilities(): Promise<MemoryCapabilities> {
    ...
    try {
      ...
      console.warn('内存能力检测失败，使用默认值:', error);
  ...
  private async detectGPUCapabilities(): Promise<GPUCapabilities | undefined> {
    ...
    try {
      ...
      const capabilities: GPUCapabilities = {
        vendor: await this.getGPUVendor(),
  ...
  private async detectNPUCapabilities(): Promise<NPUCapabilities | undefined> {
    ...
    try {
      ...
      const capabilities: NPUCapabilities = {
        vendor: await this.getNPUVendor(),
  ...
  private async getMemoryBandwidth(): Promise<number> {
    return 51200; // MB/s
  }
  ...
  private async getGPUVendor(): Promise<string> {
    if (Platform.OS === 'ios') {
      ...
      return 'Qualcomm'; // 或 'ARM Mali', 'PowerVR' 等
    }
  }
  ...
  private async getNPUModel(): Promise<string> {
  ...
  private async getNPUDriverVersion(): Promise<string> {
    return '1.0.0';
  }
  ...
  private async checkQNNSupport(): Promise<boolean> {
    return false; // Qualcomm Neural Network SDK
  }
  ...
  private getDefaultCapabilities(): DeviceCapabilities {
    return {
      cpu: this.getDefaultCPUCapabilities(),
      memory: this.getDefaultMemoryCapabilities(),
      supportedProviders: ['cpu'],
      recommendedConfig: DEFAULT_CONFIGS.EDGE_COMPUTE
    };
  }
