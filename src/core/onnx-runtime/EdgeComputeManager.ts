...
export class EdgeComputeManager extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.log('边缘计算管理器初始化完成');
    } catch (error) {
      throw new ONNXError({
        code: 'DEVICE_NOT_SUPPORTED',
        message: `边缘计算管理器初始化失败: ${error.message}`,
        details: error,
        timestamp: new Date()
      });
    }
  ...
  getTaskStatus(taskId: string): ComputeTaskStatus | null {
    ...
    if (activeTask) {
      return {
        ...
        estimatedCompletion: activeTask.estimatedCompletion
      };
    }
  ...
  private optimizeConfigForDevice(): void {
    ...
    console.log('已根据设备能力优化配置:', this.config);
  }
  ...
  private async executeTask(task: ComputeTask): Promise<void> {
    ...
    try {
      console.log(`开始执行任务: ${task.id}, 类型: ${task.type}`);
      ...
      console.error(`任务失败: ${task.id}, 错误: ${error.message}`);
  ...
  private checkThermalState(): void {
    ...
    if (newThermalState !== this.thermalState) {
      ...
      console.log(`热状态变化: ${oldState} -> ${newThermalState}`);
  ...
  private calculateAverageExecutionTime(tasks: ComputeTask[]): number {
    ...
    const totalTime = tasks.reduce((sum, task) => {
      if (task.startTime && task.completedAt) {
        return sum + (task.completedAt.getTime() - task.startTime.getTime());
      }
      return sum;
    }, 0);
