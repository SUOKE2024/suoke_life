...
export class InferenceCache {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      console.log('推理缓存初始化完成');
    } catch (error) {
      console.error('推理缓存初始化失败:', error);
      throw new ONNXError({
        code: 'CACHE_ERROR',
        message: `缓存初始化失败: ${error.message}`,
        details: error,
        timestamp: new Date()
      });
    }
  ...
  async setModel(modelId: string, session: InferenceSession): Promise<void> {
    ...
    try {
      ...
      console.error(`模型缓存失败: ${error.message}`);
    }
  }
  ...
  async getModel(modelId: string): Promise<InferenceSession | null> {
    ...
    try {
      ...
      console.warn(`从存储加载模型失败: ${error.message}`);
    }
  ...
  async setInference(cacheKey: string, result: InferenceResult): Promise<void> {
    ...
    try {
      const cachedResult: CachedInferenceResult = {
        result,
        timestamp: Date.now(),
        accessCount: 1
      };
      ...
      console.log(`推理结果已缓存: ${cacheKey}`);
    } catch (error) {
      console.error(`推理结果缓存失败: ${error.message}`);
    }
  }
  ...
  async getInference(cacheKey: string): Promise<InferenceResult | null> {
    ...
    try {
      ...
      console.warn(`从存储加载推理结果失败: ${error.message}`);
    }
  ...
  async clearModel(modelId: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`清除模型存储缓存失败: ${error.message}`);
    }
  }
  ...
  async clear(): Promise<void> {
    ...
    try {
      ...
      console.warn(`清除存储缓存失败: ${error.message}`);
    }
  ...
  getCacheStats(): CacheStats {
    ...
    return {
      ...
      hitRate,
      memoryUsage: (totalSize / this.config.maxCacheSize) * 100
    };
  }
    ...
    try {
      ...
      for (const key of inferenceKeys) {
        try {
          ...
          console.warn(`加载推理缓存失败 ${key}:`, error);
        }
      }
    } catch (error) {
      console.warn('加载推理缓存失败:', error);
    }
  }
  ...
  private async saveInferenceToStorage(cacheKey: string, cached: CachedInferenceResult): Promise<void> {
    ...
    try {
      ...
      console.warn('保存推理结果到存储失败:', error);
    }
  }
  ...
  private async loadInferenceFromStorage(cacheKey: string): Promise<CachedInferenceResult | null> {
    ...
    try {
      ...
      console.warn('从存储加载推理结果失败:', error);
      return null;
    }
  }
  ...
  private async removeInferenceFromStorage(cacheKey: string): Promise<void> {
    ...
    try {
      ...
      console.warn('从存储移除推理结果失败:', error);
    }
  }
  ...
  private async cleanupExpiredInferences(): Promise<void> {
    ...
    if (expiredKeys.length > 0) {
      console.log(`清理了 ${expiredKeys.length} 个过期的推理缓存`);
    }
  }
