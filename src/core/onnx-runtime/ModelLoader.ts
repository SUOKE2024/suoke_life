...
export class ModelLoader {
  ...
  async loadFromPath(modelPath: string): Promise<ONNXModel> {
    try {
      console.log(`正在从路径加载模型: ${modelPath}`);
      ...
      console.log(`模型加载成功: ${model.name} (${model.id})`);
      return model;
  ...
  async loadFromURL(
    ...
    try {
      console.log(`正在从URL下载模型: ${modelUrl}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      const downloadOptions = {
        ...
        begin: (res: any) => {
          ...
          console.log(`开始下载模型，大小: ${task.totalBytes} bytes`);
        },
        progress: (res: any) => {
  ...
  private async validateONNXHeader(modelPath: string): Promise<boolean> {
    try {
      ...
      console.warn('验证ONNX文件头失败:', error);
      return false;
    }
  }
}
