...
name: string,'weights: Record<ModalityType, number>;
...
/* 制 */
*/
class CrossModalAttention {private queryDim: numberprivate keyDim: number;
private valueDim: number;
constructor(queryDim: number, keyDim: number, valueDim: number) {this.queryDim = queryDimthis.keyDim = keyDim;
}
}
...
const weights = this.softmax(scores);
  ...
  private weightedSum(vectors: number[][], weights: number[]): number[] {const dim = vectors[0].lengthconst result = new Array(dim).fill(0);
...
strategyName: string = 'tcm_diagnosis'
  ): Promise<FusionResult> {cacheKey: this.generateCacheKey(embeddings, strategyName}    // 检查缓存
...
normalizedMagnitude: Math.min(vectorMagnitude / 10, 1);
totalWeight += weight;
}
