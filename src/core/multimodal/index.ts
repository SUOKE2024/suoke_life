 ...
 * 多模态RAG增强模块
 * 
 * 本模块提供了完整的多模态检索增强生成功能，专门针对中医诊断场景进行优化。
...
export type { 
  ...
  RAGResponse,
  VectorDatabase,
  { LanguageModel } from './MultimodalRAGService';
// 嵌入融合
export { 
  MultimodalEmbeddingFusion,
  { ModalityType } from './MultimodalEmbeddingFusion';
export type {
  Embedding,
  FusionStrategy,
  { FusionResult } from './MultimodalEmbeddingFusion';
// 多模态编码器
export { 
  MultimodalEncoder,
  TextEncoder,
  TongueEncoder,
  { PulseEncoder } from './MultimodalEncoder';
export type {
  EncoderConfig,
  TongueFeatures,
  { PulseFeatures } from './MultimodalEncoder';
/**
 * 创建默认的多模态RAG服务实例
 * 
 * @param vectorDB 向量数据库实例
...
export function createSymptomAnalysisStrategy(): FusionStrategy {
  return {
    ...
    weights: {
      [ModalityType.TEXT]: 0.6,       // 文本症状描述（主要）
...
export const MODALITY_TYPES = {
  ...
  TONGUE: ModalityType.TONGUE,
  PULSE: ModalityType.PULSE,
...
export const FUSION_STRATEGIES = {
  ...
  TCM_DIAGNOSIS_ENHANCED: 'tcm_diagnosis_enhanced',
