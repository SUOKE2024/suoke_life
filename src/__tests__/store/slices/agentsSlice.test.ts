...
import agentsSlice, {
import type { AgentsState, AgentType } from '../../../types';


/**
 * agentsSlice 测试用例
 * 测试智能体状态管理的Redux slice
 */
...
describe('agentsSlice', () => {
  ...
  describe('同步actions', () => {
    ...
    it('应该能够移除消息', () => {
      ...
      const state1 = store.getState().agents;
      const messageId = state1.conversations.xiaoai[0].id;
      ...
      const state2 = store.getState().agents;
  ...
  describe('异步actions', () => {
    ...
    it('应该能够发送消息给智能体', async () => {
      ...
      await store.dispatch(sendMessageToAgent(messageData));
  ...
  describe('选择器测试', () => {
    it('应该能够选择智能体状态', () => {
      const { selectAgents } = require('../../../store/slices/agentsSlice');
      const state = { agents: store.getState().agents };
      
      const agentsState = selectAgents(state);
    ...
    it('应该能够选择活跃智能体', () => {
      ...
      const activeAgent = selectActiveAgent(state);
      expect(activeAgent).toBe('xiaoke');
    });

    it('应该能够选择对话列表', () => {
      const { selectConversations } = require('../../../store/slices/agentsSlice');
      const state = { agents: store.getState().agents };
    ...
    it('应该能够选择加载状态', () => {
      const { selectAgentsLoading } = require('../../../store/slices/agentsSlice');
      const state = { agents: store.getState().agents };
      
      const loading = selectAgentsLoading(state);
      expect(loading).toBe(false);
    });

    it('应该能够选择错误状态', () => {
      const { selectAgentsError } = require('../../../store/slices/agentsSlice');
      const state = { agents: store.getState().agents };
      
      const error = selectAgentsError(state);
  ...
  describe('边界情况', () => {
    ...
    it('应该能够处理多种消息类型', () => {
      const messageTypes = ['text', 'image', 'audio', 'file'] as const;
