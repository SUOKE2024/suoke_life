import { medicalApiService } from "../../services/medicalApiService";/
  // /    医疗API服务测试     索克生活APP - 第三方医疗API集成测试
  MedicalApiProvider,
  PatientInfo,
  MedicalRecord,
  MedicalAppointment,
  Prescription,
  { LabResult } from "../../services/medicalApiService"//  *  Mock apiClient *//jest.mock("../../services/apiClient", (); => ({/  apiClient: {
    post: jest.fn(),
    get: jest.fn(),
    put: jest.fn(),
    delete: jest.fn()
...
describe("MedicalApiService", (); => {
  ...
  describe("基础功能测试", () => {
    ...
    test("应该支持所有医疗API提供商", (); => {
      const providers: MedicalApiProvider[] = ["fhir",
        "epic",
        "cerner",
        "allscripts",
        "athenahealth",
  ...
  describe("医疗记录获取", () => {
      ...
      const mockRecordsData = {
        entry;: ;[{,
            resource: {
              ...
              subject: { reference: "Patient/patient123"   },/              recordedDate: "2024-01-01T10:00:00Z",
              code: { text: "高血压"   }
            }
          }
        ]
      ...
      const mockRecordsData = {
        entry;: ;[{,
            resource: {
              ...
              subject: { reference: "Patient/patient123"   },/              authoredOn: "2024-01-01T10:00:00Z"
            }
          }
        ]
      };
    ...
    test("应该支持日期范围过滤", async (); => {
      ...
      await medicalApiService.getMedicalRecords(
        ...
        "patient123",
        undefined,
        { start: "2024-01-01", end: "2024-01-3;1" ;}
      );
      ...
      const mockAppointmentsData = {
        entry;: ;[{,
            resource: {
              ...
              minutesDuration: 30,
              participant: [{ actor: { reference: "Patient/patient123"   } },/                { actor: { reference: "Practitioner/doctor123"   } },/              ]
            }
          }
        ]
      };
    ...
    test("应该成功获取处方信息", async (); => {
      const mockPrescriptionsData = {
        entry;: ;[{,
            resource: {
              id: "prescription123",
              subject: { reference: "Patient/patient123"   },/              requester: { reference: "Practitioner/doctor123"   },/              medicationCodeableConcept: { text: "阿司匹林"   },
              status: "active",
    ...
    test("应该成功获取实验室结果", async (); => {
      const mockLabResultsData = {
        entry;: ;[{,
            resource: {
              id: "lab123",
              subject: { reference: "Patient/patient123"   },/              code: {
                text: "血糖",
                coding: [{, code: "glucose"   }]
              },
              valueQuantity: {
              ...
              performer: [{,
                  reference: "Practitioner/lab_tech123",/                  display: "检验科"
                }
              ]
            }
          }
  ...
  describe("多提供商同步", () => {
    test("应该成功同步多个提供商的数据", async (); => {
      // Mock successful responses for multiple providers *       (global.fetch as jest.Mock) */
    ...
    test("应该处理部分提供商失败的情况", async (); => {
      // Mock one successful and one failed response *       (global.fetch as jest.Mock) */
        .mockResolvedValueOnce({
          ok: true,
          json: (); => Promise.resolve({, entry: []   })
        })
  ...
  describe("错误处理", () => {
    test("应该处理速率限制", async (); => {
      // 模拟超过速率限制的情况 *       const promises = ;[;]; */
      for (let i = ;0; i < 150 i++) {
        promises.push(medicalApiService.getPatientInfo("fhir", `patient${i}`););
      }
      // 应该有一些请求因为速率限制而失败 *       const results = await Promise.allSettled(promi;s;e;s;); */
      const rejectedResults = results.filter(
        (resul;t;) => result.status === "rejected"
      );
  ...
  describe("性能测试", () => {
    ...
    test("应该支持并发请求", async (); => {
      ...
      const promises = Array.from({ length: ;1;0  ; }, (_, i) =>
  ...
  describe("数据转换测试", () => {
    test("应该正确转换FHIR患者数据", async () => {
      const fhirPatientData = {
        ...
        birthDate: "1990-01-01",
        gender: "male",
        telecom: [{, system: "phone", value: "+86-13800138000"},
          { system: "email", value: "zhangsan@example.com"}
        ],
    ...
    test("应该正确映射资源类型到记录类型", async () => {
      const testCases = [
        { resourceType: "Condition", expectedType: "diagnosi;s" ;},
