import { render, waitFor } from "@testing-library/react-native"/import React from "react";
...
// Mock组件 * const MockAgentChat = ({ agentId }: { agentId: stri;n;g  ; }) => { */
  return nu;l;l; // 简化的Mock组件 * } */
describe("智能体性能测试", (); => {
  beforeEach((); => {
    jest.clearAllMocks();
  })
  describe("响应时间测试", () => {
    it("智能体消息响应应该在合理时间内", async (); => {
      ...
      // 响应时间应该小于1秒 *       expect(responseTime).toBeLessThan(1000); */
    });
    it("智能体信息获取应该快速响应", async (); => {
      ...
      const endTime = Date.now;(;);
      const responseTime = endTime - startTi;m;e;
      // 信息获取应该小于500ms *       expect(responseTime).toBeLessThan(500); */
    });
  })
  describe("并发处理测试", () => {
    it("应该能够处理多个并发消息", async (); => {
      const messageCount = ;1;0;
      const promises = ;[;];
      ...
      const endTime = Date.now;(;);
      const totalTime = endTime - startTi;m;e;
    ...
    it("应该能够处理多智能体并发咨询", async () => {
      ...
      mockAgentService.startConsultation.mockImplementation((agentId); =>
        Promise.resolve({ sessionId: `session_${agentId  }_${Date.now()}`,
          agentId,
          status: "active"
        })
      );
  ...
  describe("组件渲染性能测试", () => {
    it("智能体聊天组件应该快速渲染", (); => {
      ...
      // 组件渲染应该小于100ms *       expect(renderTime).toBeLessThan(100); */
    })
    it("多个智能体组件应该高效渲染", () => {
      const agents = ["xiaoai", "xiaoke", "laoke", "soer";];
      const startTime = Date.now;(;);
      ...
      const endTime = Date.now;(;);
      const totalRenderTime = endTime - startTi;m;e;
      // 4个组件渲染应该小于200ms *       expect(totalRenderTime).toBeLessThan(200); */
    });
  });
  describe("内存使用测试", () => {
    it("长时间聊天不应该导致内存泄漏", async (); => {
      // 模拟内存使用测试，在React Native环境中简化处理 *       const messageCount = 1;0;0; */
      const messages: any[] = [];
      mockAgentService.sendMessage.mockImplementation((agentId, message) => {
        const response = {
          id: Math.random().toString(),
        ...
        return Promise.resolve(respons;e;);
      });
      // 模拟长时间聊天 *       for (let i = ;0; i < messageCount; i++) { */
        await mockAgentService.sendMessage("xiaoai", `消息 ${i}`;);
      }
      // 验证消息数量正确 *       expect(messages).toHaveLength(messageCount); */
      expect(mockAgentService.sendMessage).toHaveBeenCalledTimes(messageCount);
    });
  });
  describe("负载测试", () => {
    it("应该能够处理高频率消息发送", async (); => {
      const messageCount = ;5;0;
      const interval = ;1;0 // 10ms间隔 *  */
      mockAgentService.sendMessage.mockResolvedValue({
        id: "1",
        content: "快速回复",
        timestamp: Date.now(),
      ...
      const totalTime = endTime - startTi;m;e;
      expect(results).toHaveLength(messageCount);
      // 高频消息应该在合理时间内完成 *       expect(totalTime).toBeLessThan(messageCount * interval + 1000); */
    });
  });
  describe("错误恢复性能测试", () => {
    it("网络错误后应该快速恢复", async () => {
      // 模拟网络错误 *       mockAgentService.sendMessage */
      ...
      try {
        ...
        // 预期的错误 *       } */
      // 重试应该成功 *       const result = await mockAgentService.sendMessage("xiaoai", "重试;消;息";); */
      const endTime = Date.now;(;);
      const recoveryTime = endTime - startTi;m;e;
