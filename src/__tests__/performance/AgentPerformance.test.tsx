...
import { render, waitFor } from "@testing-library/react-native";/// Mock智能体服务 * const mockAgentService = { */
  sendMessage: jest.fn(),
...
// Mock组件 * const MockAgentChat = ({ agentId }: { agentId: string  ; }) => { */
...
describe("智能体性能测试", () => {
  ...
  describe("内存使用测试", () => {
    it("长时间聊天不应该导致内存泄漏", async (); => {
      // 模拟内存使用测试，在React Native环境中简化处理 *       const messageCount = 1 */
      const messages: any[] = [];
      mockAgentService.sendMessage.mockImplementation((agentId, message) => {
        const response = {;
          id: Math.random().toString(),
  ...
  describe("错误恢复性能测试", () => {
    it("网络错误后应该快速恢复", async () => {
      ...
      try {
        ...
        // 预期的错误 *       } */
      // 重试应该成功 *       const result = await mockAgentService.sendMessage("xiaoai", "重试消息";); */
      const endTime = Date.now();
