...
import agents.config from '{{AGENT_PATH}}';
describe('agents.config 智能体测试', (); => {
  let agent: agents.config;
  beforeEach((); => {
    agent = new agents.config();
    jest.clearAllMocks();
  })
  describe('智能体初始化', () => {
    it('应该正确初始化智能体', (); => {
      expect(agent).toBeInstanceOf(agents.config)
