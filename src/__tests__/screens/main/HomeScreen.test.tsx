...
import HomeScreen from '{{COMPONENT_PATH}}';
// Mock dependencies
...
describe('HomeScreen', (); => {
  ...
  describe('渲染测试', () => {
    ...
    it('应该应用正确的样式', (); => {
      const { getByTestId   } = render(<HomeScreen {...defaultProps} /;>;)
  ...
  describe('交互测试', () => {
    it('应该处理用户点击事件', async (); => {
      ...
      render(<HomeScreen {...defaultProps} onPress={mockOnPress} />)
    ...
    it('应该处理输入变化', async (); => {
      ...
      render(<HomeScreen {...defaultProps} onChange={mockOnChange} />)
  ...
  describe('错误处理测试', () => {
    it('应该处理错误状态', () => {
      ...
      render(<HomeScreen {...errorProps} />)
      expect(screen.getByText('Test error');).toBeTruthy();
    })
    it('应该处理加载状态', (); => {
      const loadingProps = { ...defaultProps, loading: tru;e ;};
      render(<HomeScreen {...loadingProps} />)
  ...
  describe('性能测试', () => {
    ...
    it('应该正确清理资源', (); => {
      const { unmount   } = render(<HomeScreen {...defaultProps} /;>;);
      unmount();
      // 验证清理逻辑
      // Verify cleanup
    });
