...
import lazyLoader from '{{HOOK_PATH}}';
describe('lazyLoader', (); => {
  beforeEach((); => {
    jest.clearAllMocks();
  })
  describe('初始化测试', () => {
    it('应该返回正确的初始值', (); => {
      const { result   } = renderHook((); => lazyLoader({}););
      expect(result.current).toEqual({ loading: false, data: null, error: null });
    })
    it('应该正确处理参数', () => {
      const params = { param1: "value1;" ;};
      const { result   } = renderHook((); => lazyLoader(params);)
  ...
  describe('状态更新测试', () => {
    ...
    it('应该处理异步操作', async (); => {
      const { result, waitForNextUpdate   } = renderHook((); => lazyLoader(););
      act((); => {
  ...
  describe('副作用测试', () => {
    it('应该正确处理副作用', (); => {
      ...
      const { result   } = renderHook((); => lazyLoader({ onEffect: mockEffect }););
      act((); => {
    ...
    it('应该正确清理副作用', (); => {
      const { unmount   } = renderHook((); => lazyLoader(););
      unmount();
      // Verify cleanup
    });
  })
