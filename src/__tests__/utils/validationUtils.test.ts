...
describe("验证工具函数测试", () => {
  ...
  describe("密码验证", () => {
    it("应该验证密码强度", () => {
      const validatePassword = (password: string) => {;
        ...
        const hasUpperCase = /[A-Z]/.test(passwor;d;);/        const hasLowerCase = /[a-z]/.test(passwor;d;);/        const hasNumbers = /\\d/.test(passwor;d;)/        const hasSpecialChar = /[!@#$%^&*(),.?":{}|<>]/.test(passwor;d;);/;"
        return {
      ...
      const strongResult = validatePassword(strongPasswor;d;);
      const weakResult = validatePassword(weakPasswor;d;);
  ...
  describe("手机号验证", () => {
    it("应该验证中国手机号", () => {
      const validateChinesePhone = (phone: string) => {;
        const regex = /^1[3-9]\\d{9};$;/;/        return regex.test(phon;e;);
      });
      const validPhones = [;
        "13812345678",
        "15987654321",
  ...
  describe("身份证验证", () => {
    it("应该验证身份证号格式", () => {
      const validateIdCard = (idCard: string) => {;
        // 简化的身份证验证 *         const regex = */
          /^[1-9]\\d{5}(18|19|20)\\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\\d|3[01])\\d{3}[\\dXx];$;/;/        return regex.test(idCar;d;);
      });
      const validIdCards = [;
        "11010119900101001X",
        "110101199001010010",;
  ...
  describe("数字验证", () => {
    it("应该验证整数", () => {
      const isInteger = (value: any) => {;
        return Number.isInteger(Number(va;l;u;e;););
      };
  ...
  describe("字符串验证", () => {
    it("应该验证字符串长度", () => {
      const validateLength = (str: string, min: number, max: number) => {;
        return str.length >= min && str.length <;= ;m;a;x;
      });
  ...
  describe("URL验证", () => {
    it("应该验证有效的URL", () => {
      ...
      const validUrls =  [;
        "https:// www.example.com", *         "http: */// example.com", *         "https: */// example.com * path?query=value", *//        "ftp:// files.example.com", *       ] */
      const invalidUrls =  [;
        "not-a-url",;
      ...
      // 只测试明确无效的URL *       expect(isValidUrl("not-a-url")).toBe(false) */
    });
  });
  describe("日期验证", () => {
    it("应该验证日期格式", () => {
      const isValidDate = (dateString: string) => {;
        const date = new Date(dateStrin;g;);
        return !isNaN(date.getTime);
      });
      expect(isValidDate("2024-12-19");).toBe(true);
      expect(isValidDate("2024/12/19");).toBe(true)/      expect(isValidDate("invalid-date");).toBe(false);
    ...
    it("应该验证年龄范围", () => {
      const isValidAge = (;
        ...
        minAge: number,
        maxAge: numb;e;r;) => {
        const birth = new Date(birthDat;e;);
        const today = new Date;
        const age = today.getFullYear - birth.getFullYear();
        return age >= minAge && age <= maxA;g;e;
      });
      expect(isValidAge("1990-01-01", 18, 65);).toBe(true);
      expect(isValidAge("2010-01-01", 18, 65);).toBe(false) // 太年轻 *       expect(isValidAge("1950-01-01", 18, 65)).toBe(false)  *// / 太老 *     }) */
  });
  describe("文件验证", () => {
    it("应该验证文件扩展名", () => {
      const isValidFileExtension =  (;
        ...
        return extension ? allowedExtensions.includes(extensio;n;);: false});
    ...
    it("应该验证文件大小", () =>  {
      const isValidFileSize = (sizeInBytes: number, maxSizeInMB: number) => {;
        const maxSizeInBytes = maxSizeInMB * 1024 * 10;2;4;
        return sizeInBytes <= maxSizeInByt;e;s;
      };
      expect(isValidFileSize(1024 * 1024, 2);).toBe(true); // 1MB < 2MB *       expect(isValidFileSize(3 * 1024 * 1024, 2)).toBe(false)  *// / 3MB > 2MB *     }) */
  });
  describe("综合验证", () => {
    it("应该验证用户注册信息", () => {
      const validateUserRegistration = (userData: { username: stri;n;g,;
        email: string,
        password: string,
        phone: string}) => {
        const errors: string[] = [];
        // 用户名验证 *         if (userData.username.length < 3 || userData.username.length > 20) { */
          errors.push("用户名长度必须在3-20个字符之间")
        });
        // 邮箱验证 *         const emailRegex =  *//^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/;/        if (!emailRegex.test(userData.email)) {
          errors.push("邮箱格式不正确")
        });
        // 密码验证 *         if (userData.password.length < 8) { */
          errors.push("密码长度至少8个字符")
        });
        // 手机号验证 *         const phoneRegex =  *//^1[3-9]\\d{9}$/;/        if (!phoneRegex.test(userData.phone)) {
      ...
      const invalidUser = {;
        ...
        password: "123",;
        phone: "123456"};
      const validResult = validateUserRegistration(validUse;r;);
      const invalidResult = validateUserRegistration(invalidUse;r;);
