...
const mockStore = configureStore({
  reducer: {
    agents: (state = {
      xiaoai: { status: "active", health: "healthy" },
      xiaoke: { status: "active", health: "healthy" },
      laoke: { status: "active", health: "healthy" },
      soer: { status: "active", health: "healthy" }
    }, action) => state,
  },
});
...
describe("智能体集成端到端测试", () => {
  ...
  describe("小艾智能体集成测试", () => {
    ...
    it("应该成功处理四诊协调请求", async () => {
      ...
      const diagnosisRequest = {
        userId: "user-123",
        sessionId: "session-456",
        diagnosisType: "looking" as const,
  ...
  describe("老克智能体集成测试", () => {
    it("应该成功处理知识检索请求", async () => {
      ...
      (agentApiService.laokeKnowledgeRetrieval as jest.Mock).mockResolvedValue(mockKnowledgeResponse);
  ...
  describe("索儿智能体集成测试", () => {
    it("应该成功处理生活方式管理请求", async () => {
      const mockLifestyleResponse = {
        ...
        timestamp: new Date(),
        requestId: "lifestyle-request-1"
      };

      (agentApiService.soerLifestyleManagement as jest.Mock).mockResolvedValue(mockLifestyleResponse);

      const lifestyleRequest = {
        userId: "user-123",
        dataType: "manual" as const,
        data: {
          dailySteps: 5000,
          sleepHours: 7,
          waterIntake: 1.5
  ...
  describe("错误处理集成测试", () => {
    ...
    it("应该正确处理超时错误", async () => {
      (agentApiService.getAgentStatus as jest.Mock).mockRejectedValue(
  ...
  describe("性能测试", () => {
    ...
    it("并发请求应该正常处理", async () => {
      const requests = [
        agentApiService.getAgentStatus(AgentType.XIAOAI),
        agentApiService.getAgentStatus(AgentType.XIAOKE),
        agentApiService.getAgentStatus(AgentType.LAOKE),
        agentApiService.getAgentStatus(AgentType.SOER)
      ];
