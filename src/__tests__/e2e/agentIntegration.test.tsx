...
const mockStore = configureStore({reducer: {agents: (;
      state = {xiaoai: { status: 'active', health: 'healthy' },xiaoke: { status: 'active', health: 'healthy' },laoke: { status: 'active', health: 'healthy' },soer: { status: 'active', health: 'healthy' };
      },action;
    ) => state;
  };
});
...
describe('智能体集成端到端测试', () => {
  ...
  describe('小艾智能体集成测试', () => {
    it('应该成功处理聊天请求', async () => {
      const mockChatResponse = {success: true,data: {id: 'response-1',agentType: AgentType.XIAOAI,content: '您好！我是小艾，很高兴为您服务。',responseType: 'text',timestamp: new Date(),processingTime: 200,confidence: 0.95;
        },timestamp: new Date(),requestId: 'chat-request-1';
      };

      (agentApiService.xiaoaiChat as jest.Mock).mockResolvedValue(mockChatResponse);

      const chatRequest = {message: '你好',messageType: MessageType.TEXT,userId: 'user-123',sessionId: 'session-456';
      };
    ...
    it('应该成功处理四诊协调请求', async () => {
      const mockDiagnosisResponse = {success: true,data: {sessionId: 'session-456',diagnosisType: 'looking',result: {faceColor: '红润',confidence: 0.88;
          };
      ...
      const diagnosisRequest = {userId: 'user-123',sessionId: 'session-456',diagnosisType: 'looking' as const,data: { imageData: 'base64-image-data' };
      };
  ...
  describe('小克智能体集成测试', () => {
    it('应该成功处理服务管理请求', async () => {
      const mockServiceResponse = {success: true,data: {serviceType: 'appointment_management',result: {appointmentId: 'apt-789',doctorName: '张医生',appointmentTime: '2024-01-15 14:00';
          };
      ...
      const serviceRequest = {userId: 'user-123',serviceType: 'appointment_management',parameters: {doctorId: 'doc-456',preferredTime: '2024-01-15 14:00';
        };
      };
  ...
  describe('老克智能体集成测试', () => {
    it('应该成功处理知识检索请求', async () => {
      const mockKnowledgeResponse = {success: true,data: {query: '中医养生',results: [;
            {title: '中医养生基础知识',content: '中医养生注重整体调理...',relevance: 0.92;
            };
          ];
      ...
      const knowledgeRequest = {userId: 'user-123',serviceType: 'knowledge_retrieval',parameters: {query: '中医养生',maxResults: 5;
        };
      };
  ...
  describe('索儿智能体集成测试', () => {
    it('应该成功处理生活方式管理请求', async () => {
      const mockLifestyleResponse = {
        success: true,
        data: {
          userId: 'user-123',recommendations: [;
            {type: 'diet',suggestion: '建议增加蔬菜摄入量',priority: 'high';
            },{type: 'exercise',suggestion: '每日步行30分钟',priority: 'medium';
            };
          ];
      ...
      const lifestyleRequest = {userId: 'user-123',dataType: 'manual' as const,data: {dailySteps: 5000,sleepHours: 7,waterIntake: 1.5;
        },timestamp: Date.now();
      };
  ...
  describe('错误处理集成测试', () => {
    it('应该正确处理API错误', async () => {
      const mockErrorResponse = {success: false,error: {code: 'NETWORK_ERROR',message: '网络连接失败';
        },timestamp: new Date(),requestId: 'error-request-1';
      };

      (agentApiService.xiaoaiChat as jest.Mock).mockResolvedValue(mockErrorResponse);

      const chatRequest = {message: '测试消息',messageType: MessageType.TEXT,userId: 'user-123',sessionId: 'session-456';
      };
