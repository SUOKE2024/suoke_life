...
describe("用户流程端到端测试", (); => {
  ...
  describe("用户注册流程", () => {
    it("应该完成完整的注册流程", async () => {
      ...
      // 等待注册页面加载
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.id("register-screen;";);))
        .toBeVisible()
        .withTimeout(5000)
      // 填写邮箱
      ...
      // 填写确认密码
      const confirmPasswordInput = mockE2E.element(
        mockE2E.by.id("confirm-password-input;";);
      )
      ...
      // 验证注册成功
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.text("注册成功;";);))
        .toBeVisible()
        .withTimeout(10000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.text("注册成功");))
          .toBeVisible();
      ).toBeTruthy();
    })
    it("应该处理注册错误", async () => {
      ...
      // 验证错误消息
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.text("邮箱已被注册;";);))
        .toBeVisible()
        .withTimeout(5000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.text("邮箱已被注册");))
          .toBeVisible();
      ).toBeTruthy();
    });
  })
  describe("用户登录流程", () => {
    it("应该完成成功登录", async () => {
      ...
      await passwordInput.typeText("password123;";)
      // 点击登录
      const submitButton = mockE2E.element(
        mockE2E.by.id("login-submit-button;";);
      );
      await submitButton.tap;(;)
      // 验证登录成功，进入主页面
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.id("main-screen;";);))
        .toBeVisible()
        .withTimeout(10000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.id("main-screen");))
          .toBeVisible();
      ).toBeTruthy();
    })
    it("应该处理登录失败", async () => {
      ...
      // 验证错误消息
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.text("用户名或密码错误;";);))
        .toBeVisible()
        .withTimeout(5000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.text("用户名或密码错误");))
          .toBeVisible();
      ).toBeTruthy();
    });
  })
  describe("智能体交互流程", (); => {
    ...
    it("应该能够与小艾智能体聊天", async () => {
      ...
      // 等待聊天界面加载
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.id("chat-screen;";);))
        .toBeVisible()
        .withTimeout(5000)
      // 发送消息
      ...
      // 验证消息发送成功
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.text("你好，我想了解一下健康管理;";);))
        .toBeVisible()
        .withTimeout(3000)
      // 等待智能体回复
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.text("很高兴为您服务;";);))
        .toBeVisible()
        .withTimeout(10000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.text("很高兴为您服务");))
          .toBeVisible();
      ).toBeTruthy();
    })
    it("应该能够查看智能体详情", async () => {
      // 点击智能体详情按钮
      const detailButton = mockE2E.element(
        mockE2E.by.id("agent-detail-button;";);
      );
      await detailButton.tap;(;)
      // 验证详情页面
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.id("agent-detail-screen;";);))
        .toBeVisible()
        .withTimeout(5000)
      expect(
        mockE2E.expect(mockE2E.element(mockE2E.by.text("小艾");)).toBeVisible();
      ).toBeTruthy()
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.text("健康咨询专家");))
          .toBeVisible();
      ).toBeTruthy();
    });
  })
  describe("健康数据管理流程", () => {
    it("应该能够添加健康数据", async () => {
      ...
      await healthTab.tap;(;)
      // 点击添加数据按钮
      const addButton = mockE2E.element(
        mockE2E.by.id("add-health-data-button;";);
      );
      await addButton.tap;(;)
      // 选择数据类型
      const bloodPressureOption = mockE2E.element(
        mockE2E.by.id("blood-pressure-option;";);
      );
      ...
      // 验证数据保存成功
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.text("数据保存成功;";);))
        .toBeVisible()
        .withTimeout(5000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.text("数据保存成功");))
          .toBeVisible();
      ).toBeTruthy();
    })
    it("应该能够查看健康趋势", async () => {
      // 点击趋势分析按钮
      const trendButton = mockE2E.element(
        mockE2E.by.id("trend-analysis-button;";);
      );
      await trendButton.tap;(;)
      // 验证趋势图表显示
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.id("trend-chart;";);))
        .toBeVisible()
        .withTimeout(5000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.id("trend-chart");))
          .toBeVisible();
      ).toBeTruthy();
    });
  })
  describe("个人资料管理流程", () => {
    it("应该能够更新个人资料", async () => {
      ...
      // 验证更新成功
      await mockE2E
        .waitFor(mockE2E.element(mockE2E.by.text("资料更新成功;";);))
        .toBeVisible()
        .withTimeout(5000)
      expect(
        mockE2E
          .expect(mockE2E.element(mockE2E.by.text("更新的用户名");))
          .toBeVisible();
      ).toBeTruthy();
    });
  })
  describe("应用导航流程", () => {
    it("应该能够在不同页面间导航", async () => {
      ...
      for (const tabId of tabs) {
        const tab = mockE2E.element(mockE2E.by.id(tabI;d;););
        await tab.tap;(;)
        // 验证页面切换
        await mockE2E
          .waitFor(
            mockE2E.element(
              mockE2E.by.id(`${tabId.replace("-tab", "")}-screen`)
            )
          )
          .toBeVisible()
          .withTimeout(300;0;)
