...
import { Challenge, ChallengeQuestion } from "../../types/maze";""/;"/g"/;
/* ; *//;/g/;
*//;,/g/;
View,;
Text,;
...
setScore(0);
setTimeLeft(challenge.timeLimit || 300); // 默认5分钟/;,/g/;
...
scoreAnim.setValue(0);
  ...
  /* 器 *//;/g/;
  *//;,/g/;
useEffect() => {const let = interval: NodeJS.Timeout;,}if (isTimerActive && timeLeft > 0) {interval = setInterval() => {}        setTimeLeft(prev => {);,}if (prev <= 1) {setIsTimerActive(false);,}handleTimeUp();
}
            return 0;}
          }
          return prev - 1;
  ...
  *//;,/g,/;
  ...
  /* 题 *//;/g/;
  *//;,/g/;
const handleNextQuestion = useCallback(() => {if (currentQuestionIndex < totalQuestions - 1) {setCurrentQuestionIndex(prev => prev + 1);}      // 更新进度动画/;,/g/;
Animated.timing(progressAnim, {);,}toValue: (currentQuestionIndex + 1) / totalQuestions,/;,/g,/;
  ...
  /* 题 *//;/g/;
  *//;,/g/;
const handlePreviousQuestion = useCallback(() => {if (currentQuestionIndex > 0) {setCurrentQuestionIndex(prev => prev - 1);}      // 更新进度动画/;,/g/;
Animated.timing(progressAnim, {);,}toValue: (currentQuestionIndex - 1) / totalQuestions,/;,/g,/;
  duration: 300,;
}
        const useNativeDriver = false;}
  ...
  /* 战 *//;/g/;
  *//;,/g/;
const handleSubmit = useCallback(() => {setIsTimerActive(false);}    // 计算分数/;,/g/;
...
const finalScore = Math.round(correctAnswers / totalQuestions) * 100);/;,/g/;
setScore(finalScore);
setShowResults(true);
    // 分数动画/;,/g/;
...
return `${minutes.toString().padStart(2, '0')}:${remainingSeconds.toString().padStart(2, '0')}`;````;```;
  ...
  /* 项 *//;/g/;
  *//;,/g/;
const renderAnswerOptions = useCallback(() => {if (!currentQuestion) return null;,}return currentQuestion.options.map(option, index) => {const isSelected = selectedAnswers[currentQuestionIndex] === index;);,}const isCorrect = showResults && index === parseInt(String(currentQuestion.correctAnswer));
...
const accuracy = Math.round(correctCount / totalQuestions) * 100);/;,/g/;
return (;);
      <View style={styles.resultsContainer}>;";"";
        <View style={styles.scoreDisplay}>;";"";
          <Icon name="emoji-events" size={48} color="#FFD54F"  />;"/;"/g"/;
          <Animated.Text style={[styles.scoreText, {opacity: scoreAnim.interpolate({inputRange: [0, 100],outputRange: [0, 1];)}  />/;/g/;
            }});
          }]}>;

          </Animated.Text>/;/g/;
          <Text style={styles.accuracyText}>;
            正确率: {accuracy}% ({correctCount}/{totalQuestions})/;/g/;
          </Text>/;/g/;
        </View>/;/g/;
        <View style={styles.resultDetails}>";"";
          <View style={styles.resultItem}>";"";
            <Icon name="timer" size={20} color="#81C784"  />"/;"/g"/;
            <Text style={styles.resultLabel}>用时</Text>/;/g/;
          ...
          <View style={styles.resultItem}>";"";
            ...
            <Text style={styles.resultLabel}>评级</Text>/;/g/;
            <Text style={styles.resultValue}>;
...
style={styles.reviewButton}
          ...
          >";"";
            ...
            <Text style={styles.reviewButtonText}>查看答案</Text>/;/g/;
          </TouchableOpacity>/;/g/;
          <TouchableOpacity;  />/;,/g/;
style={styles.completeButton}
          ...
          >";"";
            ...
            <Text style={styles.completeButtonText}>完成挑战</Text>/;/g/;
          </TouchableOpacity>/;/g/;
        </View>/;/g/;
      </View>;/;/g/;
    );
      ...
      <View style={styles.container}>;
        ...
        <View style={styles.header}>";"";
          <View style={styles.headerLeft}>";"";
            ...
            <Text style={styles.headerTitle}>健康挑战</Text>/;/g/;
          </View>/;/g/;
...
styles.progressFill,{width: progressAnim.interpolate({inputRange: [0, 1],outputRange: ["0%",100%'];)'}'';'';
            ...
            <Text style={styles.progressText}>;
              {currentQuestionIndex + 1} / {totalQuestions}/;/g/;
            </Text>/;/g/;
          </View>/;/g/;
        )}
        {// 内容区域}/;/g/;
          ...
          {showResults ? renderResults() : ();}}
            <>}
              ...
              <View style={styles.challengeInfo}>;
                <Text style={styles.challengeTitle}>{challenge.title}</Text>/;/g/;
                <Text style={styles.challengeDescription}>{challenge.description}</Text>/;/g/;
              </View>/;/g/;
              {// 当前问题}/;/g/;
              {currentQuestion  && <View style={styles.questionContainer}>;
                  <Text style={styles.questionText}>{currentQuestion.question}</Text>/;/g/;
                  {// 问题图片 - 暂时注释掉，因为类型定义中没有imageUrl属性}/;/g/;
                  {/* > *//;}                  )} */}/;/g/;
                  {// 答案选项}/;/g/;
                  ...
                  {showResults && currentQuestion.explanation  && <View style={styles.explanationContainer}>;
                      <Text style={styles.explanationTitle}>解释</Text>/;/g/;
