...
interface MazeStatsScreenProps {navigation: any,;}}
}
  const userId = string;}
}
  ...
  renderStatsCard: (title: string, value: string | number, icon: string, color: string) => (;);
    <View style={[styles.statsCard, { borderLeftColor: color ;}}]}>;
      <View style={styles.statsIcon}>;
        <Icon name={icon} size={24} color={color}  />;/;/g/;
      </View>;/;/g/;
      <View style={styles.statsContent}>;
        <Text style={styles.statsValue}>{value}</Text>;/;/g/;
        <Text style={styles.statsTitle}>{title}</Text>;/;/g/;
      </View>;/;/g/;
    </View>;/;/g/;
  );
  // 渲染成就徽章/;,/g/;
const renderAchievementBadge = (achievement: string) => (;)';'';
    <View key={achievement} style={styles.achievementBadge}>;';'';
      ...
      <Text style={styles.achievementText}>{achievement}</Text>;/;/g/;
    </View>;/;/g/;
  );
  // 渲染排行榜条目"/;,"/g,"/;
...
color={rankColor});
        ...
        <View style={styles.userInfo}>;
          ...
          <Text style={styles.mazeName}>{entry.mazeName}</Text>;/;/g/;
        </View>;/;/g/;
...
return (<SafeAreaView style={styles.container}>);
      <View style={styles.header}>)';'';
        ...
        <Text style={styles.title}>游戏统计</Text>/;/g/;
      ...
      >';'';
        {selectedTab === 'stats' && stats  && <View style={styles.statsSection}>';'';
            ...
            {(stats.level || stats.rank)  && <View style={styles.levelSection}>;
                <Text style={styles.sectionTitle}>等级信息</Text>/;/g/;
                <View style={styles.levelCard}>';'';
                  {stats.level  && <View style={styles.levelInfo}>';'';
                      <Icon name="trending-up" size={32} color={colors.primary}  />"/;"/g"/;
                      <View style={styles.levelText}>;
                        <Text style={styles.levelNumber}>等级 {stats.level}</Text>/;/g/;
                        <Text style={styles.levelDescription}>继续努力提升等级</Text>/;/g/;
                      </View>/;/g/;
                    </View>/;/g/;
                  )}";"";
                  {stats.rank  && <View style={styles.rankInfo}>";"";
                      ...
                      <View style={styles.rankText}>;
                        <Text style={styles.rankNumber}>排名 #{stats.rank}</Text>/;/g/;
                        <Text style={styles.rankDescription}>全球排名</Text>/;/g/;
                      </View>/;/g/;
                    </View>/;/g/;
                  )}
                </View>/;/g/;
            ...
            <View style={styles.favoriteSection}>;
              <Text style={styles.sectionTitle}>喜爱主题</Text>/;/g/;
...
name={themeConfig[stats.favoriteTheme].icon}
            ...
            {stats.achievements.length > 0  && <View style={styles.achievementsSection}>;
                <Text style={styles.sectionTitle}>获得成就</Text>/;/g/;
        ...
        {selectedTab === 'leaderboard'  && <View style={styles.leaderboardSection}>';'';
            <Text style={styles.sectionTitle}>全球排行榜</Text>;/;/g/;
            {leaderboard.length > 0 ? (;)}
              <View style={styles.leaderboardList}>;
                ...
                <Text style={styles.emptyText}>暂无排行榜数据</Text>;)/;/g/;
                <Text style={styles.emptySubtext}>完成更多迷宫来上榜吧</Text>;)/;/g/;
              </View>;)/;/g/;
            )};
          </View>;/;/g/;
        )};
