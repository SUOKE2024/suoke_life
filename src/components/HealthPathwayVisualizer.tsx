...
const PATHWAY = [
  { key: "inspection", label: "检测", desc: "健康数据采集与检测" },
  { key: "syndrome", label: "辨证", desc: "中医辨证分析" },
  { key: "regulation", label: "调理", desc: "个性化调理方案" },
  { key: "preservation", label: "养生", desc: "日常养生与预防" },
];

export const HealthPathwayVisualizer: React.FC<{
  currentStage: string;
  ...
  return (
    <View style={styles.container}>
      {PATHWAY.map((stage, idx) => (
        ...
        >
          <Text style={[styles.label, idx <= currentIdx && styles.activeLabel]}>
            {stage.label}
          </Text>
          <Text style={styles.desc}>{stage.desc}</Text>
          {idx < PATHWAY.length - 1 && <View style={styles.arrow} />}
        </TouchableOpacity>
      ))}
    </View>
  );
