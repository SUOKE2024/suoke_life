...
export const HealthPathwayVisualizer: React.FC<HealthPathwayVisualizerProps /    > void;}/;/g/;
/    }>  = ({ currentStage, onStagePress }) => {}/;,/g/;
...
onPress={() = accessibilityLabel="操作按钮"  /> onStagePress?.(stage.key)}/            >"/;"/g"/;
          <Text style={[styles.label, idx <= currentIdx && styles.activeLabel]}  />/                {stage.label}/;/g/;
          </Text>/          <Text style={styles.desc}>{stage.desc}</Text>/          {idx < PATHWAY.length - 1 && <View style={styles.arrow}>}/        </TouchableOpacity>/          ))}/;/g/;
    </View>/      );/;/g/;
}";,"";
