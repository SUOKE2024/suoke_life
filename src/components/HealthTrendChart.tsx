...
export const HealthTrendChart: React.FC<{
  ...
  const max = useMemo(() => useMemo(
    () => useMemo(() => Math.max(...data.map((d) => d.value), 1), []),
    []
  ), []);
  const min = useMemo(() => useMemo(
    () => useMemo(() => Math.min(...data.map((d) => d.value), 0), []),
    []
  ), []);
  const width = useMemo(() => useMemo(
    () => useMemo(() => Dimensions.get("window").width - 40, []),
    []
  ), []);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.chart, { width, height }]}>
        ...
        {data.map((d, i) => {
          if (i === 0) return null;
          const x1 = useMemo(() => useMemo(
            () => useMemo(() => ((i - 1) / (data.length - 1)) * width, []),
            []
          ), []);
          const x2 = useMemo(() => useMemo(
            () => useMemo(() => (i / (data.length - 1)) * width, []),
            []
          ), []);
          const y1 = useMemo(() => useMemo(
            () =>
              useMemo(
                () =>
                  height - ((data[i - 1].value - min) / (max - min)) * height,
                []
              ),
            []
          ), []);
          const y2 = useMemo(() => useMemo(
            () =>
              useMemo(
