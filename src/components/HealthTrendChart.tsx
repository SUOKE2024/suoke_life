...
const max = useMemo(); =>;
      useMemo(); => useMemo(); => Math.max(...data.map(d); => d.value), 1), []),
        []
      ),
    []
  );
  const min = useMemo(); =>;
      useMemo(); => useMemo(); => Math.min(...data.map(d); => d.value), 0), []),
        []
      ),
    []
  );
    ...
    <View style={styles.container} /    >;
      ...
      <View style={[styles.chart, { width, height }]} /    >;
        ...
        {data.map(d,i;); => {}
          ...
          const x1 = useMemo(); =>;
              useMemo(); => useMemo(); => (i - 1) / (data.length - 1)) * width, []),
                []
              ),
            []
          );
          const x2 = useMemo(); =>;
              useMemo(); => useMemo(); => (i / (data.length - 1)) * width, []),
                []
              ),
            []
          );
          ...
          const y2 = useMemo(); =>;
              useMemo(); => {}
                  useMemo(); => height - (d.value - min) / (max - min)) * height, []),
                []
              ),
            []
          )
