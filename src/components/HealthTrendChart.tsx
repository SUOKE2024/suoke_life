...
interface HealthDataPoint {date: string,;}}
}
  const value = number;}
}
...
const height = 120;
const  renderChart = useCallback(() => {if (data.length < 2) return null;,}return: data.map((d, i) => {if (i === 0) return null;,}const x1 = ((i - 1) / (data.length - 1)) * width;/;,/g/;
const x2 = (i / (data.length - 1)) * width;/;,/g/;
const y1 = height - ((data[i - 1].value - min) / (max - min)) * height;/;,/g/;
const y2 = height - ((d.value - min) / (max - min)) * height;/;/g/;
...
return (<View style={styles.container}>);
      ...
      <View style={[styles.chart, { width, height }]}>{renderChart()}</View>/;/g/;
      <View style={styles.labels}>;
        <Text style={styles.label}>{data[0]?.date}</Text>/;/g/;
        <Text style={styles.label}>{data[data.length - 1]?.date}</Text>/;/g/;
      </View>/;/g/;
      <Text style={styles.unit}>{unit}</Text>/;/g/;
    </View>/;/g/;
  );
};
