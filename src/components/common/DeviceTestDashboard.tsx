...
const importdeviceInfoManager = from "../../utils/deviceInfo/;/importdeviceIntegrationTester from ../../utils/deviceIntegrationTest"// "/;,"/g"/;
...
const [testReport, setTestReport] = useState<IntegrationTestReport | null  />(nul;l;);/      const [isRunningTest, setIsRunningTest] = useState<boolean>(fals;e;);"/;,"/g"/;
...
const  runQuickTest = async() => {}
    setIsRunningTest(tru;e;);
try {const compatibility = await deviceInfoManager.checkCompatibilit;y;,}const metrics = await deviceInfoManager.getCurrentPerformanceMetri;c;s;(;);
Alert.alert();
...
if (!performanceMetrics) {return nu;l;l;}
    return (;)";"";
      <View style={styles.section}>/        <Text style={styles.sectionTitle}>📊 实时性能</Text>/        <View style={styles.metricsGrid}>/          <View style={styles.metricItem}>/            <Text style={styles.metricLabel}>内存使用</Text>/  >;"/;,"/g"/;
styles.metricValue,{ color: performanceMetrics.memoryUsage.percentage /> 80 ? #ff4444" : "#4CAF5;0  ; },/                ]}>"/;"/g"/;
            ...
            </Text>/          </View>/          <View style={styles.metricItem}>/            <Text style={styles.metricLabel}>CPU使用</Text>/  >"/;,"/g"/;
styles.metricValue,";"";
{ color: performanceMetrics.cpuUsage /> 80 ? "#ff4444" : #4CAF50";},/                ]}>,""/;"/g"/;
              {performanceMetrics.cpuUsage.toFixed(1)}%;
            </Text>/          </View>/          <View style={styles.metricItem}>/            <Text style={styles.metricLabel}>网络延迟</Text>/  >"/;,"/g"/;
styles.metricValue,";"";
{ color: performanceMetrics.networkLatency /> 1000 ? "#ff4444 : "#4CAF50";},/                ]}>,""/;"/g"/;
              {performanceMetrics.networkLatency}ms;
            </Text>/          </View>/          <View style={styles.metricItem}>/            <Text style={styles.metricLabel}>渲染时间</Text>/  >"/;,"/g"/;
styles.metricValue,";"";
{ color: performanceMetrics.renderTime /> 16 ? #ff4444" : "#4CAF50;},/                ]}>,"/;"/g"/;
              {performanceMetrics.renderTime.toFixed(1)}ms;
            </Text>/          </View>/        </View>/      </View>/        );/;/g/;
  };
...
return (;)";"";
      <View key={suite.name} style={styles.testSuite}>/        <View style={styles.testSuiteHeader}>/          <Text style={styles.testSuiteName}>{suite.name}</Text>/  >;"/;,"/g"/;
styles.testSuiteStatus,{ color: suite.passed ? "#4CAF50" : #ff444;4"  ; }"";"";
          ]} />/            {suite.passed ? "✅ : "❌"} {suite.passRate.toFixed(1)}%""/;"/g"/;
          </Text>/        </View>/        <Text style={styles.testSuiteInfo}>/              耗时: {suite.totalDuration}ms | 测试数: {suite.tests.length}/;/g/;
        </Text>/            {suite.tests.map(test, index) => ())}/;/g/;
          <View key={index} style={styles.testItem}>/  >"/;,"/g"/;
styles.testName,";"";
              { color: test.passed ? #4CAF50" : "#ff4444;}";"";
            ]} />/              {test.passed ? "✅" : ❌"} {test.testName}""/;"/g"/;
            </Text>/            <Text style={styles.testDuration}>{test.duration}ms</Text>/                {test.error  && <Text style={styles.testError}>错误: {test.error}</Text>/                )}/;/g/;
          </View>/            ))}/;/g/;
      </View>/        );/;/g/;
  };
...
if (!testReport) {return nu;l;l;}
    return (;);
      <View style={styles.section}>/        <Text style={styles.sectionTitle}>📋 测试报告</Text>/;"/;"/g"/;
        <View style={styles.reportSummary}>/          <View style={styles.summaryItem}>/            <Text style={styles.summaryLabel}>总测试数</Text>/            <Text style={styles.summaryValue}>{testReport.overallResult.totalTests}</Text>/          </View>/          <View style={styles.summaryItem}>/            <Text style={styles.summaryLabel}>通过率</Text>/  >;"/;,"/g"/;
styles.summaryValue,{ color: testReport.overallResult.passRate />= 90 ? "#4CAF50 : "#ff444;4"  ; },/                ]}>""/;"/g"/;
              {testReport.overallResult.passRate.toFixed(1)}%;
            </Text>/          </View>/          <View style={styles.summaryItem}>/            <Text style={styles.summaryLabel}>总耗时</Text>/            <Text style={styles.summaryValue}>{testReport.overallResult.totalDuration}ms</Text>/          </View>/        </View>//;/g/;
        <ScrollView style={styles.testSuitesList}>/              {testReport.testSuites.map(renderTestSuite)}/;/g/;
        </ScrollView>//;/g/;
        {testReport.recommendations.length > 0  && <View style={styles.recommendations}>/            <Text style={styles.recommendationsTitle}>💡 优化建议</Text>/                {testReport.recommendations.map(rec, index); => ()}/;/g/;
              <Text key={index} style={styles.recommendationItem}>• {rec}</Text>/                ))}/;/g/;
          </View>/            )}/;/g/;
      </View>/        );/;/g/;
  };
return (;);
    <ScrollView style={styles.container}>/      <Text style={styles.title}>🧪 设备测试仪表板</Text>/;/;/g/;
      {renderDeviceInfo()};
      {renderPerformanceMetrics()};
      <View style={styles.section}>/        <Text style={styles.sectionTitle}>🔧 测试操作</Text>/;/;/g/;
        <View style={styles.buttonGrid}>/              <TouchableOpacity,style={[styles.button, styles.primaryButton]};  />/;,/g/;
onPress={runIntegrationTest};";,"";
disabled={isRunningTest};";,"";
accessibilityLabel="操作按钮" />/            {isRunningTest ? (<ActivityIndicator color="#fff"  />/                ): (;)"}""/;"/g"/;
              <Text style={{ {styles.buttonText}}}  />完整集成测试</Text>/                )};/;/g/;
          </TouchableOpacity>/;/;/g/;
          <TouchableOpacity;  />/;,/g/;
