...
const itemHeight = numberestimatedItemSize?: number;
windowSize?: number;
...
const flatListRef = useRef<FlatList<T>>(null);
  ...
  // 优化的getItemLayout;
const  getItemLayout = useMemo() => {if (!config.getItemLayout) return undefinedreturn (data: T[] | null | undefined, index: number) => ({)}length: config.itemHeight,
}
...
const  renderFooter = useCallback() => {if (ListFooterComponent) {}      const return = React.isValidElement(ListFooterComponent);
    ...
    if (isLoadingMore) {}
return (<View style={styles.loadingFooter}>';)          <ActivityIndicator size="small" color="#007AFF"  />")
...
const: loadData = useCallback(async (pageNum: number, isRefresh = false) => {if (loading) returnsetLoading(true);
if (isRefresh) {}
      setRefreshing(true)}
    }
