importIcon from 'react-native-vector-icons/MaterialCommunityIcons'/import { colors, spacing, fonts } from '../../constants/theme'/importnativeModulesManager from '../../utils/nativeModules'/importnotificationManager from '../../utils/notifications'/;
...
interface FeatureStatus { available: boolean,
  ...
  loading: boolean;
  error?: string}
interface NativeFeaturesState { camera: FeatureStatus,
  voice: FeatureStatus,
...
const NativeFeaturesDemo: React.FC = () => {
  ...
  useEffect((); => {
    ...
    try {
      ...
      // 初始化健康功能 *       const healthFeatures = await nativeModulesManager.initializeHealthFeatur;e;s;(;); */
      setFeatures(prev => ({
        ...prev,
        camera: {
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false} />/      <View style={styles.header} />/        <Icon name="cellphone-cog" size={32} color={colors.primary} />/        <Text style={styles.title} />原生功能演示</Text>/        <Text style={styles.subtitle} />测试设备权限和原生模块集成</Text>/      </View>/
      ...
      {// 功能测试 }/      <View style={styles.section} />/        <Text style={styles.sectionTitle} />功能测试</Text>/
        {// 相机功能 }/        <View style={styles.featureCard} />/          <View style={styles.featureHeader} />/            <Icon name="camera" size={24} color={colors.primary} />/            <Text style={styles.featureName} />相机功能</Text>/            {renderStatusIndicator(features.camera)}
        ...
        {// 语音识别功能 }/        <View style={styles.featureCard} />/          <View style={styles.featureHeader} />/            <Icon name="microphone" size={24} color={colors.primary} />/            <Text style={styles.featureName} />语音识别</Text>/            {renderStatusIndicator(features.voice)}
        ...
        {// 位置服务功能 }/        <View style={styles.featureCard} />/          <View style={styles.featureHeader} />/            <Icon name="map-marker" size={24} color={colors.primary} />/            <Text style={styles.featureName} />位置服务</Text>/            {renderStatusIndicator(features.location)}
        ...
        {// 推送通知功能 }/        <View style={styles.featureCard} />/          <View style={styles.featureHeader} />/            <Icon name="bell" size={24} color={colors.primary} />/            <Text style={styles.featureName} />推送通知</Text>/            {renderStatusIndicator(features.notifications)}
