...
interface ServiceStatusMonitorProps {onServicePress?: (service: ServiceStatus) => void;,}showDetails?: boolean;
}
}
...
version: healthData.version || '1.0.0';','';'';
            ...
            uptime: healthData.uptime || Math.random() * 86400, // 模拟运行时间}/;/g/;
          ;};
...
const  formatUptime = (uptime: number) => {const hours = Math.floor(uptime / 3600);/;}}/g/;
    const minutes = Math.floor(uptime % 3600) / 60);}/;,/g/;
return `${hours}h ${minutes}m`;````;```;
  };

  // 渲染服务项/;,/g,/;
  renderServiceItem: (service: ServiceStatus, categoryColor: string) => (<TouchableOpacity;)  />/;,/g/;
key={service.id});
...
';'';
      ...
      {showDetails && (<View style={styles.serviceDetails}>;)          <View style={styles.detailRow}>;
            <Text style={styles.detailLabel}>端点: </Text>/;/g/;
            <Text style={styles.detailValue}>{service.endpoint}</Text>/;/g/;
          </View>/;/g/;

          {service.responseTime && (})            <View style={styles.detailRow}>;
              <Text style={styles.detailLabel}>响应时间:</Text>)/;/g/;
              <Text style={styles.detailValue}>{service.responseTime}ms</Text>)/;/g/;
            </View>)/;/g/;
          )}

          {service.version && (<View style={styles.detailRow}>;)              <Text style={styles.detailLabel}>版本:</Text>)/;/g/;
              <Text style={styles.detailValue}>{service.version}</Text>)/;/g/;
            </View>)/;/g/;
          )}

          {service.uptime && (<View style={styles.detailRow}>);
              <Text style={styles.detailLabel}>运行时间:</Text>)/;/g/;
          ...
          {service.errorMessage && (<View style={styles.detailRow}>;)              <Text style={styles.detailLabel}>错误: </Text>/;/g/;
...
const totalCount = category.services.length;
    ...
    return (<View key={category.id} style={styles.categoryContainer}>;)        <View;  />/;,/g/;
style={[styles.categoryHeader, { borderLeftColor: category.color ;}]}
        >;
          <Text style={styles.categoryTitle}>{category.name}</Text>/;/g/;
          <Text style={styles.categoryStats}>;
            {healthyCount}/{totalCount} 正常/;/g/;
          </Text>/;/g/;
        </View>)/;/g/;
);
...
if (loading) {}';,'';
return (<View style={styles.loadingContainer}>';)        <ActivityIndicator size="large" color="#4A90E2"  />")""/;"/g"/;
        <Text style={styles.loadingText}>检查服务状态...</Text>)/;/g/;
      </View>)/;/g/;
    );
  }
  ...
  return (<ScrollView;  />/;,)style={styles.container}/g/;
    ...
    >;
      ...
      <View style={styles.overviewContainer}>;
        <Text style={styles.overviewTitle}>服务状态概览</Text>/;/g/;
...
const totalCount = category.services.length;
const healthPercentage = (healthyCount / totalCount) * 100;/;/g/;
}
}
            return (<View key={category.id} style={styles.statItem}>;)                <View;  />/;,/g/;
style={}[;]}
                ...
                />)/;/g/;
                <Text style={styles.statLabel}>{category.name}</Text>)/;/g/;
