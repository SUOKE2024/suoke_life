import React, { ComponentType, Profiler, ProfilerOnRenderCallback, useEffect, useRef, useState } from 'react';
...
export const PerformanceProfiler: React.FC<PerformanceProfilerProps> = ({
  ...
  const handleRender: ProfilerOnRenderCallback = (
    ...
    if (actualDuration > 16) {
      console.warn(`⚠️ Slow render detected in ${profileId}: ${actualDuration.toFixed(2)}ms`);
    }
  };
  ...
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      try {
        ...
        // 记录成功的网络请求
        PerformanceMonitor.recordNetworkRequest(url, method, startTime, endTime, 200);
        ...
        PerformanceMonitor.recordNetworkRequest(url, method, startTime, endTime, status);
  ...
  return (
    <div>
      <button onClick={runBenchmark} disabled={isRunning}>
        {isRunning ? `Running... (${timesRef.current.length}/${iterations})` : `Run Benchmark: ${name}`}
      </button>
      
      {results && (
        <div style={{ marginTop: 10, padding: 10, border: '1px solid #ccc', borderRadius: 4 }}>
