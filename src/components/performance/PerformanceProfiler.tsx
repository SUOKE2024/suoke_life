...
return {const renderCount = renderCountRef.currentrenderTime,
}
...
const descriptor = PropertyDescriptor;);
  ) {const originalMethod = descriptor.valuedescriptor.value = async function (...args: any[]) {const startTime = performance.now()try {result: await originalMethod.apply(this, args)const endTime = performance.now();
        // 记录成功的网络请求
...
endTime,);
        ...
        return result}
      } catch (error: any) {const endTime = performance.now();}        // 记录失败的网络请求
...
const name = stringiterations?: number;
...
const  runIteration = useCallback((iteration: number) => {if (iteration >= iterations) {}        // 计算结果
