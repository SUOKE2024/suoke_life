...
export const ONNXInferenceProvider: React.FC<ONNXInferenceProviderProps> = ({
  ...
  const destroy = useCallback(async () => {
    ...
    try {
      ...
      console.log('ONNX Runtime 已销毁');

    } catch (error) {
      console.error('销毁 ONNX Runtime 失败:', error);
    }
  }, []);
  ...
  const preloadModelsAsync = useCallback(async (modelPaths: string[]) => {
    for (const modelPath of modelPaths) {
      try {
        ...
        console.log(`预加载模型成功: ${modelPath}`);
      } catch (error) {
        console.warn(`预加载模型失败: ${modelPath}`, error);
      }
    }
  }, [loadModel]);
