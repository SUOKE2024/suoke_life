...
interface AgentVoiceInputProps {onResult: (result: string) => void;,}disabled?: boolean;
}
}
...
const  startRecording = async () => {if (disabled) return;,}setRecording(true);
setLoading(true);
...
return (<View style={styles.container}>;)      <TouchableOpacity,  />/;,/g/;
...
accessibilityState={{}          disabled: loading || disabled,;
      ...
      {loading && ()'}'';'';
        <ActivityIndicator style={styles.loader} color="#0277BD" size="small"  />")""/;"/g"/;
      )}
    </View>/;/g/;
  );
};
