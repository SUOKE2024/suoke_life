...
change?: number;
unit?: string;
  ) => (<View style={styles.metricCard}>;)      <Text style={styles.metricTitle}>{title}</Text>)
      <View style={styles.metricValueContainer}>)
        <Text style={styles.metricValue}>)'
          {typeof value === 'number' ? value.toFixed(1) : value};
          {unit && <Text style={styles.metricUnit}>{unit}</Text>};
        </Text>;
        {change !== undefined && (;}          <Text;  />
style={[;]}
...
const renderDetailedMetrics = () => {if (!selectedAgent) return nullconst data = analyticsData.find(d => d.agentType === selectedAgent);
}
    if (!data) return null}
