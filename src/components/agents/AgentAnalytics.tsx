...
const AgentAnalytics: React.FC<AgentAnalyticsProps> = ({ 
      ...
      const analyticsPromises = agents.map(async (agentType) => {
        ...
        const mockMetrics: AgentMetrics = {
          ...
          resources: {
            ...
            networkUsage: Math.random() * 60 + 15
          },
          sessions: {
            active: Math.floor(Math.random() * 50 + 10),
            total: Math.floor(Math.random() * 1000 + 500),
            averageDuration: Math.random() * 300 + 120
          }
        };

        const trends = {
          responseTimeChange: (Math.random() - 0.5) * 20,
          successRateChange: (Math.random() - 0.5) * 0.1,
          throughputChange: (Math.random() - 0.5) * 30
        };
  ...
  const renderMetricCard = (title: string, value: string | number, change?: number, unit?: string) => (
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        {analyticsData.map((data) => (
          <TouchableOpacity
            ...
            style={[
              styles.agentCard,
              selectedAgent === data.agentType && styles.selectedAgentCard
            ]}
          ...
          >
            ...
            <View style={styles.agentMetrics}>
              ...
              <Text style={styles.agentMetricText}>
                活跃会话: {data.metrics.sessions.active}
              </Text>
            </View>
    ...
    return (
      <View style={styles.detailsContainer}>
        ...
        <View style={styles.metricsGrid}>
          ...
          {renderMetricCard(
            "错误率",
            data.metrics.performance.errorRate * 100,
            undefined,
            "%"
          )}
        </View>
