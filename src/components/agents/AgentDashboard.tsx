...
const AgentDashboard: React.FC<AgentDashboardProps> = ({ 
  ...
  const handleQuickAction = (actionType: string, agentType: AgentType) => {
    Alert.alert(
      `${agentNames[agentType]}服务`,
      `即将启动${actionType}功能`,
      [
  ...
  const checkSystemHealth = async () => {
    try {
      const healthCheck = await agentApiService.healthCheck();
  ...
  const renderSystemStatus = () => {
    const statusConfig = {
      healthy: { color: "#34C759", text: "系统正常", icon: "✅" },
      warning: { color: "#FF9500", text: "系统警告", icon: "⚠️" },
      error: { color: "#FF3B30", text: "系统异常", icon: "❌" }
    };
