...
import { AgentType, AgentContext, AgentResponse } from '../../agents/types';
...
export const AgentInterface: React.FC<AgentInterfaceProps> = ({
  ...
  const addWelcomeMessage = useCallback(() => {
    const agentRole = AgentSystemUtils.getAgentRole(currentAgent);
  ...
  }, [inputText, isLoading, userId, sessionId, currentChannel, messages, currentAgent, onResponse, onError]);

  /**
   * 切换智能体
   */
  const switchAgent = useCallback((newAgentType: AgentType) => {
    setCurrentAgent(newAgentType);
  ...
  const renderMessage = useCallback((message: Message) => {
    ...
    const agentRole = message.agentType ? AgentSystemUtils.getAgentRole(message.agentType) : null;
   ...
   * 渲染智能体选择器
   */
  const renderAgentSelector = useCallback(() => {
    const agents = [AgentType.XIAOAI, AgentType.XIAOKE, AgentType.LAOKE, AgentType.SOER];
    
    return (
      <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.agentSelector}>
        {agents.map(agent => {
          const role = AgentSystemUtils.getAgentRole(agent);
          const isSelected = agent === currentAgent;
          
          return (
            <TouchableOpacity
              key={agent}
              style={[styles.agentButton, isSelected && styles.selectedAgentButton]}
              onPress={() => switchAgent(agent)}
            >
              <Text style={[styles.agentButtonText, isSelected && styles.selectedAgentButtonText]}>
                {role.name}
              </Text>
            </TouchableOpacity>
          );
