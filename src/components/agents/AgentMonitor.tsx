...
import { getAgentStatus, getAgentMetrics, AgentSystemUtils } from '../../agents';
 ...
 * 状态卡片组件
 */
const StatusCard: React.FC<StatusCardProps> = ({ agentType, status, metrics, onRestart }) => {
    ...
    <View style={styles.statusCard}>
      ...
      <View style={styles.metricsContainer}>
        <View style={styles.metricRow}>
          ...
          <Text style={styles.metricValue}>{(status.load * 100).toFixed(1)}%</Text>
        </View>
        <View style={styles.metricRow}>
        ...
        <View style={styles.metricRow}>
          ...
          <Text style={styles.metricValue}>{(status.errorRate * 100).toFixed(1)}%</Text>
        </View>
        {metrics && (
          <>
            <View style={styles.metricRow}>
            ...
            <View style={styles.metricRow}>
              ...
              <Text style={styles.metricValue}>{Math.floor(metrics.uptime / 60)}分钟</Text>
            </View>
          </>
        )}
      </View>
      ...
      <View style={styles.capabilitiesContainer}>
        ...
        <View style={styles.capabilitiesList}>
          ...
          {status.capabilities.length > 3 && (
            <Text style={styles.capabilityTag}>+{status.capabilities.length - 3}</Text>
          )}
        </View>
      </View>
...
export const AgentMonitor: React.FC<AgentMonitorProps> = ({
  ...
  const [agentStatuses, setAgentStatuses] = useState<Map<AgentType, AgentHealthStatus>>(new Map());
  const [agentMetrics, setAgentMetrics] = useState<Map<AgentType, AgentMetrics>>(new Map());
  ...
  const fetchAgentStatuses = useCallback(async () => {
    try {
      const statuses = await getAgentStatus() as Map<AgentType, AgentHealthStatus>;
  ...
  const generateSystemOverview = useCallback((
    ...
    const healthyAgents = Array.from(statuses.values()).filter(s => s.status === 'healthy').length;
    const totalTasks = Array.from(metrics.values()).reduce((sum, m) => sum + m.tasksProcessed, 0);
    const avgResponseTime = Array.from(statuses.values()).reduce((sum, s) => sum + s.responseTime, 0) / totalAgents;
    const avgSuccessRate = Array.from(metrics.values()).reduce((sum, m) => sum + m.successRate, 0) / totalAgents;
   ...
   * 重启智能体
   */
  const handleAgentRestart = useCallback((agentType: AgentType) => {
    Alert.alert(
      '重启智能体',
  ...
  const renderSystemOverview = useCallback(() => {
    ...
    const healthColor = systemOverview.systemHealth > 0.8 ? '#4CAF50' : 
                       systemOverview.systemHealth > 0.5 ? '#FF9800' : '#F44336';
      ...
      <View style={styles.overviewCard}>
        ...
        <View style={styles.overviewGrid}>
          <View style={styles.overviewItem}>
            <Text style={styles.overviewValue}>{systemOverview.healthyAgents}/{systemOverview.totalAgents}</Text>
            <Text style={styles.overviewLabel}>健康智能体</Text>
          </View>
          <View style={styles.overviewItem}>
            <Text style={styles.overviewValue}>{systemOverview.totalTasks}</Text>
            <Text style={styles.overviewLabel}>总处理任务</Text>
          </View>
          <View style={styles.overviewItem}>
            <Text style={styles.overviewValue}>{systemOverview.avgResponseTime}ms</Text>
            <Text style={styles.overviewLabel}>平均响应时间</Text>
          </View>
          <View style={styles.overviewItem}>
            <Text style={styles.overviewValue}>{systemOverview.avgSuccessRate}%</Text>
