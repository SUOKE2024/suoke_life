...
export const BenchmarkCreator: React.FC<BenchmarkCreatorProps> = ({
  ...
  const benchmarkTypes = [
    { id: 'tcm_diagnosis', name: '中医诊断评测', description: '评测中医诊断准确性' },
    { id: 'agent_collaboration', name: '智能体协作评测', description: '评测多智能体协作能力' },
    { id: 'privacy_security', name: '隐私安全评测', description: '评测数据隐私保护能力' },
    { id: 'performance_stress', name: '性能压力评测', description: '评测系统性能和稳定性' },
    { id: 'user_experience', name: '用户体验评测', description: '评测用户交互体验' },
    { id: 'custom', name: '自定义评测', description: '创建自定义评测任务' }
  ];
  ...
  const loadPlugins = async () => {
    try {
      const pluginList = await benchmarkService.listPlugins();
  ...
  // 处理基准测试类型选择
  const handleBenchmarkTypeSelect = (typeId: string) => {
    setBenchmarkId(typeId);
        ...
        {benchmarkTypes.map((type) => (
          ...
          >
            <Text style={[
              ...
              benchmarkId === type.id && styles.typeCardTitleSelected
            ]}>
              {type.name}
            </Text>
            <Text style={[
              styles.typeCardDescription,
              benchmarkId === type.id && styles.typeCardDescriptionSelected
            ]}>
              {type.description}
            </Text>
          </TouchableOpacity>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        ...
        {plugins.map((plugin) => (
          <TouchableOpacity
            ...
            style={[
              styles.pluginOption,
              selectedPlugin === plugin.name && styles.pluginOptionSelected
            ]}
            onPress={() => setSelectedPlugin(plugin.name)}
          >
            <Text style={[
              styles.pluginOptionText,
              selectedPlugin === plugin.name && styles.pluginOptionTextSelected
            ]}>
