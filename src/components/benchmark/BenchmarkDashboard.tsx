...
import { benchmarkService, benchmarkStreamingService } from "../../services";""/;,"/g"/;
import type { BenchmarkTask, HealthStatus, StreamEvent } from "../../services";""/;,"/g"/;
...
return () => {clearInterval(statusInterval);';,}benchmarkStreamingService.removeEventListener('*', handleStreamEvent);';'';
  ...
  // 获取状态颜色'/;,'/g'/;
const getStatusColor = (status: string) => {switch (status) {case 'completed': return '#4CAF50';';,}case 'running': ';,'';
return '#2196F3';';,'';
case 'failed': ';,'';
return '#F44336';';,'';
case 'pending': ';,'';
...
const renderHealthStatus = () => {if (!healthStatus) return null;}
    return (<View style={styles.healthStatusContainer}>;)        <Text style={styles.sectionTitle}>服务状态</Text>/;/g/;
        <View style={styles.healthStatusCard}>;
          <View style={styles.statusRow}>;
            <Text style={styles.statusLabel}>服务状态: </Text>/;/g/;
            <Text;  />/;,/g/;
style={[;]';}}'';
                styles.statusValue,'}'';'';
          ...
          <View style={styles.statusRow}>;
            <Text style={styles.statusLabel}>版本: </Text>/;/g/;
            <Text style={styles.statusValue}>{healthStatus.version}</Text>/;/g/;
          </View>)/;/g/;
          <View style={styles.statusRow}>);
            <Text style={styles.statusLabel}>运行时间: </Text>;)/;/g/;
            <Text style={styles.statusValue}>{Math.floor(healthStatus.uptime / 3600)}小时</Text>;/;/g/;
          </View>;/;/g/;
          <View style={styles.statusRow}>;
            <Text style={styles.statusLabel}>WebSocket: </Text>;/;/g/;
            <Text;'  />/;,'/g'/;
style={[;];'}'';
...
const renderTaskList = () => {if (tasks.length === 0) {return (;)}
        <View style={styles.emptyContainer}>;
          <Text style={styles.emptyText}>暂无基准测试任务</Text>;/;/g/;
        </View>;/;/g/;
      );
    }
    return (<View style={styles.taskListContainer}>);
        <Text style={styles.sectionTitle}>基准测试任务</Text>)/;/g/;
        {tasks.map(task => ())}
          <View key={task.task_id} style={styles.taskCard}>;
            <View style={styles.taskHeader}>;
              <Text style={styles.taskTitle}>{task.benchmark_id}</Text>/;/g/;
              <View style={[styles.statusBadge, { backgroundColor: getStatusColor(task.status) ;}}]}>;
                <Text style={styles.statusBadgeText}>{getStatusText(task.status)}</Text>/;/g/;
              </View>/;/g/;
            </View>/;/g/;
...
';'';
            ...
            {task.status === 'running' && (;)'}'';'';
              <View style={styles.progressContainer}>;
                <Text style={styles.progressText}>进度: {Math.round(task.progress * 100)}%</Text>;/;/g/;
                <View style={styles.progressBar}>;
                  <View style={[styles.progressFill, { width: `${task.progress * 100;}}%` }]}  />;```/`;`/g`/`;
                </View>;/;/g/;
              </View>;/;/g/;
            )};
            {task.error_message && <Text style={styles.errorText}>错误: {task.error_message}</Text>};/;/g/;
          </View>;/;/g/;
        ))};
      </View>;/;/g/;
    );
