...
export const BenchmarkReport: React.FC<BenchmarkReportProps> = ({
  ...
  // 生成报告
  const handleGenerateReport = async (format: 'html' | 'json') => {
    try {
      setGeneratingReport(prev => ({ ...prev, [format]: true }));
      
      const response = await dispatch(generateReport({ 
        taskId, 
        format 
      })).unwrap();
  ...
  // 分享报告
  const shareReport = async (url: string, format: string) => {
    try {
      await Share.share({
        message: `基准测试报告 (${format.toUpperCase()}): ${url}`,
        url: url,
        title: '基准测试报告',
      });
    } catch (error) {
