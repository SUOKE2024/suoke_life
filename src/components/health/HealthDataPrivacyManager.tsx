...
interface PrivacySettings {';,}data_sharing_enabled: boolean,';,'';
...
const  loadPrivacySettings = async () => {try {}      // 模拟从API加载隐私设置/;/g/;
}
      // 实际实现中会从后端获取用户的隐私设置}/;/g/;
    } catch (error) {}}
}
    }
  };
const  loadZKPProofs = async () => {try {}      // 模拟加载零知识证明信息/;,/g/;
...
color="#2196F3"";"";
      ...
      <View style={styles.recordDetails}>;
        ...
        {record.zkp_proof_id  && <View style={styles.detailRow}>;
            <Text style={styles.detailLabel}>零知识证明: </Text>/;/g/;
            <TouchableOpacity;  />/;,/g/;
style={styles.proofButton}
            ...
            >;
              <Text style={styles.proofButtonText}>查看证明</Text>/;/g/;
            </TouchableOpacity>/;/g/;
          </View>/;/g/;
        )}
      </View>/;/g/;
...
style={styles.actionButton}
        ...
        >";"";
          ...
          <Text style={styles.actionButtonText}>撤销访问</Text>/;/g/;
        </TouchableOpacity>/;/g/;
        <TouchableOpacity;  />/;,/g/;
style={[styles.actionButton, styles.primaryAction]}
        ...
        >";"";
          ...
          <Text style={[styles.actionButtonText, styles.primaryActionText]}>详情</Text>/;/g/;
        </TouchableOpacity>/;/g/;
      </View>/;/g/;
    </Animated.View>/;/g/;
  );
const  renderPrivacySettings = () => (<View style={styles.settingsContainer}>;)      <Text style={styles.sectionTitle}>隐私设置</Text>/;/g/;
      <View style={styles.settingItem}>;
        <View style={styles.settingInfo}>;
          <Text style={styles.settingLabel}>数据分享</Text>/;/g/;
          <Text style={styles.settingDescription}>允许与研究机构分享匿名数据</Text>/;/g/;
        </View>)/;/g/;
        <Switch;)"  />/;,"/g"/;
...
thumbColor={privacySettings.data_sharing_enabled ? '#FFFFFF' : '#FFFFFF'}';'';
      ...
      <View style={styles.settingItem}>;
        <View style={styles.settingInfo}>;
          <Text style={styles.settingLabel}>零知识证明验证</Text>/;/g/;
          <Text style={styles.settingDescription}>要求零知识证明验证数据访问</Text>/;/g/;
        </View>/;/g/;
        <Switch;'  />/;,'/g'/;
...
thumbColor={privacySettings.zkp_verification_required ? '#FFFFFF' : '#FFFFFF'}';'';
      ...
      <View style={styles.settingItem}>;
        <View style={styles.settingInfo}>;
          <Text style={styles.settingLabel}>区块链存储</Text>/;/g/;
          <Text style={styles.settingDescription}>将数据哈希存储到区块链</Text>/;/g/;
        </View>/;/g/;
        <Switch;'  />/;,'/g'/;
...
thumbColor={privacySettings.blockchain_storage_enabled ? '#FFFFFF' : '#FFFFFF'}';'';
      ...
      <View style={styles.settingItem}>;
        <View style={styles.settingInfo}>;
          <Text style={styles.settingLabel}>IPFS分布式存储</Text>/;/g/;
          <Text style={styles.settingDescription}>使用IPFS存储加密数据</Text>/;/g/;
        </View>/;/g/;
        <Switch;'  />/;,'/g'/;
      ...
      <View style={styles.modalOverlay}>;
        <View style={styles.modalContent}>;
          <View style={styles.modalHeader}>;
            <Text style={styles.modalTitle}>零知识证明详情</Text>/;/g/;
            <TouchableOpacity;  />/;,/g/;
                  ...
                  <View key={proof.proof_id} style={styles.proofDetails}>";"";
                    <View style={styles.proofHeader}>";"";
                      ...
                      <View style={styles.proofStatus}>;
                        <Text style={styles.proofTitle}>证明有效</Text>/;/g/;
                        <Text style={styles.proofId}>ID: {proof.proof_id;}</Text>/;/g/;
                      </View>/;/g/;
                    </View>/;/g/;
                    <Text style={styles.proofStatement}>{proof.statement}</Text>/;/g/;
                    <View style={styles.proofMetadata}>;
                      ...
                      <View style={styles.metadataRow}>;
                        <Text style={styles.metadataLabel}>公开输入数量: </Text>/;/g/;
                        <Text style={styles.metadataValue}>{proof.public_inputs_count}</Text>/;/g/;
                      </View>/;/g/;
                    </View>/;/g/;
                    <TouchableOpacity;  />/;,/g/;
style={styles.verifyButton}
                    ...
                    >';'';
                      ...
                      ) : (<>"}"";)                          <Ionicons name="checkmark-circle" size={20} color="#FFFFFF"  />")""/;"/g"/;
                          <Text style={styles.verifyButtonText}>重新验证</Text>)/;/g/;
                        < />)/;/g/;
                      )}
                    </TouchableOpacity>/;/g/;
                  </View>/;/g/;
...
return (<View style={styles.container}>";)      <LinearGradient;"  />/;,"/g"/;
colors={['#667eea', '#764ba2']}';,'';
style={styles.header}
      >;
        <Text style={styles.headerTitle}>健康数据隐私管理</Text>/;/g/;
          ...
          <View style={styles.loadingContainer}>";"";
            ...
            <Text style={styles.loadingText}>加载中...</Text>/;/g/;
              ...
              <View style={styles.statCard}>;
                <Text style={styles.statNumber}>{healthRecords.length}</Text>/;/g/;
              ...
              <View style={styles.statCard}>);
                ...
                <Text style={styles.statLabel}>ZKP保护</Text>/;/g/;
              </View>/;/g/;
              <View style={styles.statCard}>;
                <Text style={styles.statNumber}>;
                  {Math.round();}                    (healthRecords.filter(r => r.zkp_proof_id).length /)/;/g/;
}
                     Math.max(healthRecords.length, 1)) * 100;}
                  )}%;
                </Text>/;/g/;
                <Text style={styles.statLabel}>隐私保护率</Text>/;/g/;
              </View>/;/g/;
            </View>/;/g/;
            <Text style={styles.sectionTitle}>我的健康数据</Text>/;/g/;
...
transparent={true}
      ...
      >;
        <View style={styles.modalOverlay}>;
          <View style={styles.modalContent}>;
            <View style={styles.modalHeader}>;
              <Text style={styles.modalTitle}>隐私设置</Text>/;/g/;
              <TouchableOpacity;  />/;,/g/;
style={styles.closeButton}
