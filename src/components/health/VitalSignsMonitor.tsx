...
interface VitalSignsData {heartRate: number[],;}}
}
...
const getStatusColor = (status: 'normal' | 'warning' | 'danger'): string => {switch (status) {case 'normal': return '#4CAF50';';,}case 'warning': return '#FF9800';';,'';
...
onAdd: () => void;
        ...
        <Text style={styles.cardTitle}>{title}</Text>/;/g/;
      ...
      <View style={styles.cardContent}>;);
        ...
        <Text style={styles.vitalUnit}>{unit}</Text>;/;/g/;
      </View>;/;/g/;
...
const renderTrendChart = () => (;);
    <View style={styles.chartContainer}>;
      <Text style={styles.chartTitle}>生命体征趋势</Text>;/;/g/;
...
const  renderRecentData = () => (<View style={styles.recentDataContainer}>);
      <Text style={styles.sectionTitle}>最近记录</Text>)/;/g/;
      {vitalSigns.length === 0 ? ()}
...
return (<View style={styles.container}>;)      <View style={styles.header}>;
        <Text style={styles.title}>生命体征监控</Text>/;/g/;
...
const cardWidth = (width - 48) / 2; // 2列布局，考虑边距/;,/g/;
