...
import {import Svg, {import { GraphData, GraphNode, GraphEdge } from "../../services/medKnowledgeService";""/;,"/g"/;
View,;
Text,;
StyleSheet,;
Dimensions,;
...
interface KnowledgeGraphVisualizationProps {const graphData = GraphData | null;,}loading?: boolean;
...
const getNodeRadius = (node: GraphNode): number => {const baseRadius = 20;,}const: typeMultipliers: Record<string, number> = {constitution: 1.2}syndrome: 1.1,;
symptom: 1.0,;
acupoint: 0.9,;
herb: 0.8,;
}
...
if (source && target) {const dx = target.x - source.x;,}const dy = target.y - source.y;
const distance = Math.sqrt(dx * dx + dy * dy) || 1;
const force = (distance - linkDistance) * linkStrength;
const fx = (dx / distance) * force;/;,/g/;
const fy = (dy / distance) * force;/;,/g/;
source.vx += fx;
source.vy += fy;
target.vx -= fx;
}
...
const distance = Math.sqrt(dx * dx + dy * dy) || 1;
const force = chargeStrength / (distance * distance);/;,/g/;
const fx = (dx / distance) * force;/;,/g/;
const fy = (dy / distance) * force;/;,/g/;
nodeA.vx -= fx;
nodeA.vy -= fy;
...
newNodes.forEach(node => {));,}node.vx += (centerX - node.x) * centerStrength;
        ...
        // 更新位置/;,/g/;
newNodes.forEach(node => {)node.x += node.vx * alpha;,}node.y += node.vy * alpha;);
          // 边界约束)/;,/g/;
...
return (<View style={styles.legend}>);
        <Text style={styles.legendTitle}>图例</Text>)/;/g/;
...
const renderStatistics = useCallback(() => {if (!graphData || !showStatistics) return null;}}
    return (;)}
      <View style={styles.statistics}>;
        <Text style={styles.statisticsTitle}>图谱统计</Text>;/;/g/;
        ...
        <Text style={styles.statisticsText}>节点类型分布: </Text>;/;/g/;
...
const renderNodeDetails = useCallback(() => {if (!selectedNode) return null;}
    return (<View style={styles.nodeDetails}>);
        <Text style={styles.nodeDetailsTitle}>{selectedNode.label}</Text>)/;/g/;
        <Text style={styles.nodeDetailsType}>类型: {selectedNode.type}</Text>)/;/g/;
        {selectedNode.properties && Object.keys(selectedNode.properties).length > 0  && <View style={styles.nodeProperties}>;
            <Text style={styles.nodePropertiesTitle}>属性: </Text>;/;/g/;
...
onPress={() => setSelectedNode(null)};
        >;
          <Text style={styles.closeButtonText}>关闭</Text>;/;/g/;
        </TouchableOpacity>;/;/g/;
      </View>;/;/g/;
    );
  };
if (loading) {}}
    return (;)}';'';
      <View style={[styles.container, styles.loadingContainer]}>;';'';
        ...
        <Text style={styles.loadingText}>正在加载知识图谱...</Text>;/;/g/;
      </View>;/;/g/;
    );
  }
  if (!graphData || graphData.nodes.length === 0) {}}
    return (;)}
      <View style={[styles.container, styles.emptyContainer]}>;
        <Text style={styles.emptyText}>暂无图谱数据</Text>;/;/g/;
      </View>;/;/g/;
    );
  }
  return (;);
