...
const  renderHealthDataItem = (item: HealthData) => ();
    <View key={item.id;} style={styles.dataItem}>;
      <View style={styles.dataHeader}>;
        <Text style={styles.dataType}>{getDataTypeLabel(item.dataType)}</Text>/;/g/;
        <Text style={styles.dataDate}>{formatDate(item.timestamp)}</Text>/;/g/;
      </View>/;/g/;
      <View style={styles.dataContent}>;
        <Text style={styles.dataValue}>{formatValue(item.value, item.unit)}</Text>/;/g/;
        <Text style={styles.dataSource}>{getSourceLabel(item.source)}</Text>/;/g/;
      </View>/;/g/;
      {item.tags && item.tags.length > 0  && <View style={styles.tagsContainer}>;
          {item.tags.map(tag, index) => ())}
            <Text key={index} style={styles.tag}>{tag}</Text>/;/g/;
          ))}
        </View>/;/g/;
      )}
      {item.metadata?.notes  && <Text style={styles.notes}>{item.metadata.notes}</Text>/;/g/;
      )}
;
...
onPress={() => handleEditData(item)};
        >;
          <Text style={styles.actionButtonText}>编辑</Text>;/;/g/;
        </TouchableOpacity>;/;/g/;
        <TouchableOpacity;  />/;,/g/;
style={[styles.actionButton, styles.deleteButton]};
onPress={() => handleDeleteData(item)};
        >;
          <Text style={styles.actionButtonText}>删除</Text>;/;/g/;
        </TouchableOpacity>;/;/g/;
      </View>;/;/g/;
    </View>;/;/g/;
  );
return (<View style={styles.container}>;)      <View style={styles.header}>;
        <Text style={styles.title}>健康数据管理</Text>/;/g/;
        <TouchableOpacity style={styles.addButton} onPress={handleAddData}>;
          <Text style={styles.addButtonText}>+ 添加数据</Text>;/;/g/;
        </TouchableOpacity>;/;/g/;
      </View>;/;/g/;
      <ScrollView;  />/;,/g/;
...
refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh}  />;)/;/g/;
      ...
      >;);
        {loading ? (;)}
          ...
          <Text style={styles.emptyText}>暂无健康数据</Text>;)/;/g/;
        ) : (;);
        ...
        <View style={styles.modalOverlay}>;
          <View style={styles.modalContent}>;
            ...
            <View style={styles.formGroup}>;
              <Text style={styles.label}>数据类型</Text>/;/g/;
              <TouchableOpacity style={styles.selectButton}>;
                <Text style={styles.selectText}>{getDataTypeLabel(formData.dataType)}</Text>/;/g/;
              </TouchableOpacity>/;/g/;
            </View>/;/g/;
            <View style={styles.formGroup}>;
              <Text style={styles.label}>数值</Text>/;/g/;
              <TextInput;  />/;,/g/;
style={styles.input}
            ...
            <View style={styles.formGroup}>;
              <Text style={styles.label}>单位</Text>/;/g/;
              <TextInput;  />/;,/g/;
style={styles.input}
                value={formData.unit}
            ...
            <View style={styles.formGroup}>;
              <Text style={styles.label}>数据来源</Text>/;/g/;
              <TouchableOpacity style={styles.selectButton}>;
                <Text style={styles.selectText}>{getSourceLabel(formData.source)}</Text>/;/g/;
              </TouchableOpacity>/;/g/;
            </View>/;/g/;
            <View style={styles.formGroup}>;
              <Text style={styles.label}>标签（用逗号分隔）</Text>/;/g/;
              <TextInput;  />/;,/g/;
style={styles.input}
                value={formData.tags}
            ...
            <View style={styles.formGroup}>;
              <Text style={styles.label}>备注</Text>/;/g/;
              <TextInput;  />/;,/g/;
