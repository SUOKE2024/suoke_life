...
export const TCMDiagnosisPanel: React.FC<TCMDiagnosisPanelProps> = ({ userId }) => {
  ...
  const [formData, setFormData] = useState<DiagnosisFormData>({
    diagnosisType: TCMDiagnosisType.LOOK,
    observations: [],
    conclusion: '',
    recommendations: [],
    confidence: 80
  ...
  const getDiagnosisTypeLabel = (type: TCMDiagnosisType): string => {
    const labels: Record<TCMDiagnosisType, string> = {
      [TCMDiagnosisType.LOOK]: '望诊',
      [TCMDiagnosisType.LISTEN]: '闻诊',
      [TCMDiagnosisType.ASK]: '问诊',
      [TCMDiagnosisType.TOUCH]: '切诊',
      [TCMDiagnosisType.CALCULATE]: '算诊'
    };
    return labels[type];
  };

  const getDiagnosisTypeDescription = (type: TCMDiagnosisType): string => {
    const descriptions: Record<TCMDiagnosisType, string> = {
      [TCMDiagnosisType.LOOK]: '观察面色、舌象、形体等外在表现',
      [TCMDiagnosisType.LISTEN]: '听声音、嗅气味等感官诊断',
      [TCMDiagnosisType.ASK]: '询问症状、病史、生活习惯等',
      [TCMDiagnosisType.TOUCH]: '脉诊、触诊等手法检查',
      [TCMDiagnosisType.CALCULATE]: '综合分析、辨证论治'
    };
    return descriptions[type];
  };

  const getObservationCategories = (type: TCMDiagnosisType): string[] => {
    const categories: Record<TCMDiagnosisType, string[]> = {
      [TCMDiagnosisType.LOOK]: ['面色', '舌象', '形体', '神态', '皮肤'],
      [TCMDiagnosisType.LISTEN]: ['声音', '呼吸', '咳嗽', '气味'],
      [TCMDiagnosisType.ASK]: ['主诉', '现病史', '既往史', '家族史', '生活习惯'],
      [TCMDiagnosisType.TOUCH]: ['脉象', '腹诊', '经络', '穴位'],
      [TCMDiagnosisType.CALCULATE]: ['证候', '病机', '治法', '方药']
    };
    return categories[type] || [];
  };

  const handleAddDiagnosis = (type: TCMDiagnosisType) => {
    setSelectedDiagnosisType(type);
    setFormData({
      diagnosisType: type,
      observations: [],
      conclusion: '',
  ...
  const addObservation = () => {
    const newObservation: TCMObservation = {
      category: '',
      value: '',
      description: '',
      severity: 'mild',
  ...
  const updateObservation = (index: number, field: keyof TCMObservation, value: any) => {
  ...
  const renderDiagnosisTypeCard = (type: TCMDiagnosisType) => {
    const recentData = diagnosisData
          ...
          {(['mild', 'moderate', 'severe'] as const).map((severity) => (
            <TouchableOpacity
              ...
              style={[
                ...
                observation.severity === severity && styles.severityButtonActive
              ]}
              onPress={() => updateObservation(index, 'severity', severity)}
            >
              <Text style={[
                styles.severityButtonText,
                observation.severity === severity && styles.severityButtonTextActive
              ]}>
  ...
  const renderRecentDiagnosis = () => {
    const recentDiagnosis = diagnosisData
      .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
      .slice(0, 5);
