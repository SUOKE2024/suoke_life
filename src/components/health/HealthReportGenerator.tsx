...
export const HealthReportGenerator: React.FC<HealthReportGeneratorProps> = ({ userId }) => {
  ...
  const getReportTypeLabel = (type: string): string => {const labels: Record<string, string> = {comprehensive: '综合健康报告',vital_signs: '生命体征报告',tcm_analysis: '中医分析报告',trend_analysis: '趋势分析报告';
    };
  ...
  const renderGenerateButtons = () => (
    <View style={styles.generateSection}>
      ...
      <View style={styles.reportTypeGrid}>
        {[
          { type: 'comprehensive', label: '综合报告', description: '全面健康状况分析' },
          { type: 'vital_signs', label: '生命体征', description: '心率、血压等指标分析' },
          { type: 'tcm_analysis', label: '中医分析', description: '中医五诊综合分析' },
          { type: 'trend_analysis', label: '趋势分析', description: '健康数据变化趋势' }
        ].map((item) => (
          <View key={item.type} style={styles.reportTypeCard}>
            ...
            <View style={styles.periodButtons}>
              {[
                { period: 'week', label: '周报告' },
                { period: 'month', label: '月报告' },
                { period: 'quarter', label: '季报告' },
                { period: 'year', label: '年报告' }
              ].map((periodItem) => (
                <TouchableOpacity
  ...
  const getTrendLabel = (trend: string): string => {const labels: Record<string, string> = {increasing: '上升',decreasing: '下降',stable: '稳定';
    };
    return labels[trend] || trend;
  };

  const getTrendColor = (trend: string): string => {const colors: Record<string, string> = {increasing: '#f44336',decreasing: '#4CAF50',stable: '#666';
    };
...
const styles = StyleSheet.create({
  ...
  generatingModal: {backgroundColor: '#fff',borderRadius: 12,padding: 24,alignItems: 'center';
