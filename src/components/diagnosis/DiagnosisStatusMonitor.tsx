...
onStatusChange?: (status: FiveDiagnosisServiceStatus) => voidshowDetails?: boolean;
}
...
const fadeAnimation = React.useRef(new Animated.Value(1)).current;
  // 更新状态
const updateStatus = useCallback(async () => {try {setIsRefreshing(true);}      // 获取服务状态
...
useEffect() => {updateStatus()interval: setInterval(updateStatus, refreshInterval);
  ...
  // 状态变化时的动画效果
useEffect() => {if (status.isProcessing) {}      // 处理中的脉冲动画
Animated.loop();
...
if (healthyServices >= totalServices * 0.8) return 'warning';
  ...
  // 获取状态颜色'/,'/g'/;
const getStatusColor = (health: 'healthy' | 'warning' | 'error'): string => {switch (health) {case 'healthy': return '#28a745case 'warning': return '#ffc107';
}
...
return (<View style={styles.metricsContainer}>;)        <Text style={styles.sectionTitle}>性能指标</Text>
...
return (<View style={styles.container}>;)      {// 主状态指示器}
      <TouchableOpacity;  />
