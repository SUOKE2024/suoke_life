...
import {import { useBlockchainService } from "../../hooks/useBlockchainService";""/;,"/g"/;
import { ZKProof, VerifyWithZKPRequest } from "../../types/blockchain";""/;,"/g"/;
View,;
Text,;
StyleSheet,;
TouchableOpacity,;
...
return (<View style={styles.container}>;)      {// 头部统计}/;/g/;
      <View style={styles.statsContainer}>;
        <View style={styles.statItem}>;
          <Text style={styles.statValue}>{generatedProofs.length}</Text>/;/g/;
          <Text style={styles.statLabel}>生成的证明</Text>/;/g/;
        </View>)/;/g/;
        <View style={styles.statItem}>);
          <Text style={styles.statValue}>);
            {new Set(generatedProofs.map(p => p.circuitType)).size}
          </Text>/;/g/;
          <Text style={styles.statLabel}>电路类型</Text>/;/g/;
        </View>/;/g/;
        <View style={styles.statItem}>;
          <Text style={styles.statValue}>';'';
          ...
          <Text style={styles.statLabel}>最新证明</Text>/;/g/;
        </View>/;/g/;
      </View>/;/g/;
      {// 操作按钮}/;/g/;
...
style={styles.generateButton}
        ...
        >;
          <Text style={styles.generateButtonText}>+ 生成证明</Text>/;/g/;
        </TouchableOpacity>/;/g/;
        <TouchableOpacity;  />/;,/g/;
style={styles.verifyButton}
          onPress={() => setShowVerifyModal(true)}
        >;
          <Text style={styles.verifyButtonText}>验证证明</Text>/;/g/;
        </TouchableOpacity>/;/g/;
      </View>/;/g/;
      {// 错误显示}/;/g/;
      {error  && <View style={styles.errorContainer}>;
          ...
          <TouchableOpacity onPress={clearError} style={styles.errorCloseButton}>;
            <Text style={styles.errorCloseText}>×</Text>/;/g/;
          </TouchableOpacity>/;/g/;
        </View>/;/g/;
      )}
      {// 证明列表}/;/g/;
            ...
            <View key={proofRecord.id} style={styles.proofCard}>;
              ...
              <View style={styles.proofContent}>;
                <View style={styles.proofRow}>;
                  <Text style={styles.proofLabel}>电路类型: </Text>/;/g/;
                  <Text style={styles.proofValue}>{proofRecord.circuitType}</Text>/;/g/;
                </View>/;/g/;
                <View style={styles.proofRow}>;
                  <Text style={styles.proofLabel}>证明哈希: </Text>/;/g/;
                  <Text style={styles.proofValue}>{formatProofHash(proofRecord.proof.proof)}</Text>/;/g/;
                </View>/;/g/;
                <View style={styles.proofRow}>;
                  <Text style={styles.proofLabel}>验证密钥: </Text>/;/g/;
                  <Text style={styles.proofValue}>{proofRecord.proof.verificationKey.slice(0, 16)}...</Text>/;/g/;
                </View>/;/g/;
              </View>/;/g/;
            </View>/;/g/;
          ));
...
return (<Modal visible={visible} animationType="slide" presentationStyle="pageSheet">";)      <View style={styles.modalContainer}>;"";
        <View style={styles.modalHeader}>;
          <Text style={styles.modalTitle}>生成零知识证明</Text>/;/g/;
...
style={styles.textInput}
          ...
          <View style={styles.inputGroup}>);
            <Text style={styles.inputLabel}>私有输入 (JSON格式) *</Text>/;/g/;
            <TextInput;  />/;,/g/;
...
disabled={isLoading};
          >;";"";
            {isLoading ? (;)";}              <ActivityIndicator size="small" color="#FFFFFF"  />;"/;"/g"/;
              ...
              <Text style={styles.submitButtonText}>生成证明</Text>;)/;/g/;
            )};
          </TouchableOpacity>;/;/g/;
        </ScrollView>;/;/g/;
      </View>;/;/g/;
...
return (<Modal visible={visible} animationType="slide" presentationStyle="pageSheet">";)      <View style={styles.modalContainer}>;"";
        <View style={styles.modalHeader}>;
          <Text style={styles.modalTitle}>验证零知识证明</Text>/;/g/;
          <TouchableOpacity onPress={onClose}>;
            <Text style={styles.modalCloseText}>取消</Text>/;/g/;
          </TouchableOpacity>/;/g/;
        </View>/;/g/;
        <ScrollView style={styles.modalContent}>;
          <View style={styles.inputGroup}>;
            <Text style={styles.inputLabel}>验证者ID *</Text>/;/g/;
            <TextInput;  />/;,/g/;
style={styles.textInput}
              value={verifierId}
...
style={styles.textInput}
          ...
          <View style={styles.inputGroup}>);
            <Text style={styles.inputLabel}>证明数据 (数组格式) *</Text>/;/g/;
            <TextInput;  />/;,/g/;
style={[styles.textInput, styles.textArea]}
              value={proofData}
...
numberOfLines={6}
          ...
          <View style={styles.inputGroup}>;
            <Text style={styles.inputLabel}>公开输入 (数组格式) *</Text>/;/g/;
            <TextInput;  />/;,/g/;
style={[styles.textInput, styles.textArea]}
              value={publicInputs}
...
disabled={isLoading};
          >;";"";
            {isLoading ? (;)";}              <ActivityIndicator size="small" color="#FFFFFF"  />;"/;"/g"/;
              ...
              <Text style={styles.submitButtonText}>验证证明</Text>;)/;/g/;
            )};
          </TouchableOpacity>;/;/g/;
        </ScrollView>;/;/g/;
      </View>;/;/g/;
