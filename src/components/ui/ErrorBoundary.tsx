...
interface ErrorBoundaryProps {const children = ReactNode;,}fallback?: ReactNode;
...
render() {if (this.state.hasError) {}      // 如果提供了自定义fallback，使用它/;,/g/;
...
return (<View style={[styles.container, this.props.style]}>;)          <View style={styles.content}>;
            ...
            {this.props.showDetails && this.state.error && (})              <ScrollView style={styles.detailsContainer}>);
                <Text style={styles.detailsTitle}>错误详情：</Text>)/;/g/;
                <Text style={styles.errorText}>);
                  {this.state.error.toString()}
                </Text>/;/g/;

                {this.state.errorInfo && (<>})                    <Text style={styles.detailsTitle}>组件堆栈：</Text>/;/g/;
