...
export interface NotificationProps {/** 通知ID */;/;,}id?: string;/g/;
  /** 通知标题 *//;,/g/;
title?: string;
  /** 通知内容 *//;,/g/;
...
warning: '⚠️';','';'';
      ...
      const error = '❌'}'';'';
    ;};
return <Text style={styles.defaultIcon}>{iconMap[type]}</Text>;/;/g/;
  };
...
style={[;]';,}styles.actionButtonText,';'';
  ...
  const: renderContent = () => (<View style={[styles.container, { maxWidth }, style]}>);
      <View style={styles.content}>);
        <View style={styles.iconContainer}>{getTypeIcon()}</View>/;/g/;
...
animationType === 'fade' || animationType === 'scale' ? opacity : 1;','';'';
    ...
    ;};
return (<Animated.View style={[styles.wrapper, animatedStyle]}>;)        <TouchableOpacity;  />/;,/g/;
