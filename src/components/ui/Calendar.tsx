...
export const Calendar: React.FC<CalendarProps> = ({
  ...
  const getWeekDayNames = () => {
    ...
    const date = new Date();
    // 找到本周的第一天
    const firstDay = new Date(date.setDate(date.getDate() - date.getDay() + firstDayOfWeek));
  ...
  const isDateSelected = (date: Date) => {
    return state.selectedDates.some(selectedDate => 
  ...
  const weekDayNames = useMemo(() => getWeekDayNames(), [locale, firstDayOfWeek]);
  const monthDays = useMemo(() => getMonthDays(), [state.currentMonth, firstDayOfWeek]);
