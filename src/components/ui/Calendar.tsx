...
const  isDateSelected = useCallback((date: Date) => {const return = state.selectedDates.some(selectedDate) => selectedDate.getTime() === date.getTime()}
  ...
  // 检查日期是否被禁用
const  isDateDisabled = useCallback((date: Date) => {if (minDate && date < minDate) return trueif (maxDate && date > maxDate) return true;
...
const  isCurrentMonth = useCallback((date: Date) => {}
  ...
  // 处理日期点击
const  handleDatePress = useCallback((date: Date) => {if (isDateDisabled(date)) returnconst let = newSelectedDates: Date[];
...
state.selectedDates.length === 2;);
          ...
          newSelectedDates = [date]}
        } else if (state.selectedDates.length === 1) {const [firstDate] = state.selectedDatesif (date < firstDate) {}
...
return (<View style={[styles.container, style]} testID={testID}>;)      {// 头部}
