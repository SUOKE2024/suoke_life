...
export interface AccordionProps {;,}const items = AccordionItem[];
activeKeys?: string[];
defaultActiveKeys?: string[];
onChange?: (activeKeys: string[]) => void;
accordion?: boolean; // 是否为手风琴模式（只能展开一个）/;,/g/;
animated?: boolean;
bordered?: boolean;
...
if (accordion) {// 手风琴模式：只能展开一个/;}}/g/;
    ...
    // 动画处理/;,/g/;
if (animated) {LayoutAnimation.configureNext(LayoutAnimation.Presets.easeInEaseOut);}      // 旋转动画/;,/g/;
...
const: renderExpandIcon = useCallback((isExpanded: boolean, itemKey: string) => {if (expandIcon) {}}
    ...
    ;});
return (<Animated.View style={ transform: [{ rotate: rotation ;}] }}>;)        <Text;  />/;,/g/;
...
accessibilityState={expanded: isExpanded,;}}
              ...
              ;}}
            >;
              {item.icon && <View style={styles.icon}>{item.icon}</View>}/;/g/;
...
style={[;,]styles.title}isExpanded && styles.titleActive,;
            ...
            {isExpanded && (<View style={[styles.content, contentStyle]}>{item.content}</View>)/;/g/;
            )}
          </View>/;/g/;
        );
      })}
...
";"";
// 高级Accordion组件，支持AccordionPanel子组件"/;,"/g"/;
...
return <Accordion {...props} items={items}  />;/;/g/;
};
