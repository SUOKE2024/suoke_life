...
export const PerformanceMonitor: React.FC<PerformanceMonitorProps> = ({
  ...
  const monitorNetworkLatency = useCallback(async () => {
    ...
    try {
      ...
      console.warn('Network latency monitoring error:', error);
    }
  }, []);
  ...
  // 获取性能等级颜色
  const getPerformanceColor = useCallback((value: number, thresholds: number[]) => {
    if (value >= thresholds[0]) return '#e53e3e'; // 红色
    if (value >= thresholds[1]) return '#dd6b20'; // 橙色
    if (value >= thresholds[2]) return '#ecc94b'; // 黄色
    return '#38a169'; // 绿色
  }, []);

  // 渲染指标项
  const renderMetricItem = (label: string, value: number, unit: string, thresholds: number[]) => (
  ...
  const renderWarningItem = (warning: PerformanceWarning, index: number) => (
    <View key={index} style={[styles.warningItem, styles[`warning${warning.severity}`]]}>
    ...
    >
      <View style={styles.header}>
        ...
        <View style={styles.headerButtons}>
          ...
          >
            <Text style={styles.buttonText}>优化</Text>
          </TouchableOpacity>
          <TouchableOpacity
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.metricsSection}>
          ...
          {renderMetricItem('FPS', metrics.fps, '', [30, 45, 55])}
        ...
        {showDetailedMetrics && (
          <View style={styles.metricsSection}>
            <Text style={styles.sectionTitle}>详细指标</Text>
            {renderMetricItem('JS堆大小', metrics.jsHeapSize / 1024 / 1024, 'MB', [100, 50, 25])}
            {renderMetricItem('CPU使用率', metrics.cpuUsage, '%', [80, 60, 40])}
          </View>
        )}

        {/* 优化进度 */}
        <View style={styles.progressSection}>
          <Text style={styles.sectionTitle}>优化进度</Text>
        ...
        {warnings.length > 0 && (
          <View style={styles.warningsSection}>
            <Text style={styles.sectionTitle}>性能警告</Text>
