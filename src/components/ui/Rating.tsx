...
const: handlePress = useCallback((index: number, event: GestureResponderEvent) => {if (readonly) returnlet newRating = index + 1;
}
    if (allowHalf) {}
...
const  getRatingValue = useCallback((index: number) => {const currentValue = hoverValue !== null ? hoverValue : valueif (allowHalf) {if (currentValue >= index + 1) return 1; // 完全填充/if (currentValue >= index + 0.5) return 0.5; // 半填充
}
      return 0; // 空}
    } else {}
...
accessibilityLabel={}
      ...
      accessibilityValue={const min = 0max,
}
        const now = value)}
      ;}});
    >);
