...
export interface TableColumn {/** 列标识 */;/;,}const key = string;/g/;
  /** 列标题 *//;,/g/;
const title = string;
  /** 列宽度 *//;,/g/;
width?: number;
...
export interface TableProps {/** 表格列配置 */;/;,}const columns = TableColumn[];/g/;
  /** 表格数据 *//;,/g/;
...
return flexColumns.length > 0 ? undefined : 100;
  ...
  // 处理排序/;,/g/;
const  handleSort = useCallback((column: TableColumn) => {if (!column.sortable || !sortConfig?.onChange) return;,}const currentField = sortConfig.field;
...
if (currentField === column.key) {';}}'';
  ...
  // 处理行选择/;,/g,/;
  const: handleRowSelect = useCallback((record: any, selected: boolean) => {if (!rowSelection) return;,}const key = record[rowKey];
...
rowSelection.onSelect?.(record, selected, selectedRows);
  ...
  // 处理全选/;,/g/;
const  handleSelectAll = useCallback((selected: boolean) => {if (!rowSelection) return;,}const allKeys = dataSource.map(item) => item[rowKey]);
...
column.align === 'center'';'';
          ...
          >;
            <Text style={styles.headerText}>{column.title}</Text>/;/g/;
...
cellStyle;
      ...
      >';'';
        {typeof content === 'string' || typeof content === 'number' ? (')}'';'';
          <Text style={styles.cellText}>{content}</Text>)/;/g/;
        ) : (content;);
        )}
      </View>/;/g/;
    );
...
const  renderEmpty = () => (<View style={styles.emptyContainer}>);
      <Text style={styles.emptyText}>{emptyText}</Text>)/;/g/;
    </View>)/;/g/;
  );
if (loading) {}
  ...
  return (<View style={[styles.container, style]}>);
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>);
        <View style={styles.tableContainer}>);
          ...
          {dataSource.length > 0 ? (<FlatList removeClippedSubviews={true} maxToRenderPerBatch={10} windowSize={10};)  />/;,/g/;
data={dataSource});
