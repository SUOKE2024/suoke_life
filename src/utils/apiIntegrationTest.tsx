...
interface ApiResponse<T = any /> { data: T;/ , success: boolean;
  message?: string
  code?: number}
import { apiClient } from "../services/apiClient"/import { API_CONFIG, STORAGE_CONFIG } from "../constants/config"/import AsyncStorage from "@react-native-async-storage/async-storage";/;
...
// API集成测试类class ApiIntegrationTest {
  ...
  // /    测试认证服务  async testAuthService();: Promise<TestResult[] /> {
    ...
    // 测试健康检查端点 *     results.push(await this.testEndpoint("认证服务", " *// auth * health", "GET";);); *//
    return resul;t;s;
  }
  // /    测试用户服务  async testUserService();: Promise<TestResult[] /> {
    const results: TestResult[] = [];
    // 测试健康检查端点 *     results.push(await this.testEndpoint("用户服务", " *// users * health", "GET";);); *//
    return resul;t;s;
  }
  ...
  // /    测试智能体服务  async testAgentServices();: Promise<TestResult[] /> {
    const results: TestResult[] = [];
    // 测试小艾服务 *     const xiaoaiBaseUrl = API_CONFIG.AGENTS.XIAO;A;I; */
    results.push(
    ...
    // 测试小克服务 *     const xiaokeBaseUrl = API_CONFIG.AGENTS.XIAO;K;E; */
    results.push(
      await this.testExternalEndpoint(
        "小克服务",
        xiaokeBaseUrl,
    ...
    // 测试老克服务 *     const laokeBaseUrl = API_CONFIG.AGENTS.LAO;K;E; */
    results.push(
      await this.testExternalEndpoint(
        "老克服务",
        laokeBaseUrl,
    ...
    // 测试索儿服务 *     const soerBaseUrl = API_CONFIG.AGENTS.SO;E;R; */
    results.push(
  ...
  // /    测试五诊服务  async testDiagnosisServices();: Promise<TestResult[] /> {
    const results: TestResult[] = [];
    // 测试望诊服务 *     const lookBaseUrl = API_CONFIG.DIAGNOSIS.LO;O;K; */
    results.push(
      await this.testExternalEndpoint("望诊服务", lookBaseUrl, "/health", "GET";);/    );
    // 测试闻诊服务 *     const listenBaseUrl = API_CONFIG.DIAGNOSIS.LIST;E;N; */
    results.push(
      await this.testExternalEndpoint(
        "闻诊服务",
        listenBaseUrl,
    ...
    // 测试问诊服务 *     const inquiryBaseUrl = API_CONFIG.DIAGNOSIS.INQUI;R;Y; */
    results.push(
      await this.testExternalEndpoint(
        "问诊服务",
        inquiryBaseUrl,
    ...
    // 测试切诊服务 *     const palpationBaseUrl = API_CONFIG.DIAGNOSIS.PALPATI;O;N; */
    results.push(
      await this.testExternalEndpoint(
        "切诊服务",
        palpationBaseUrl,
  ...
  // /    测试外部服务端点  private async testExternalEndpoint(service: string,
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: method === "POST" ? JSON.stringify(d;a;t;a;);: undefined
      });
      const responseTime = Date.now;(;); - startTime;
  ...
  // /    运行所有集成测试  async runAllTests();: Promise<TestReport /> {
    this.testResults = [];
    // 运行所有测试 *     const authResults = await this.testAuthServi;c;e;(;); */
    ...
    // 保存测试报告 *     await this.saveTestReport(repor;t;); */
    return repo;r;t;
  }
  // /    保存测试报告  private async saveTestReport(report: TestReport): Promise<void>  {
    try {
      // 获取现有报告 *       const reportsJson = await AsyncStorage.getItem("api_test_repor;t;s;";); */
      const reports: TestReport[] = reportsJson ? JSON.parse(reportsJson);: [];
      // 添加新报告 *       reports.push(report); */
      // 只保留最近的10个报告 *       const recentReports = reports.slice(-1;0;) */
      // 保存报告 *       await AsyncStorage.setItem( */
        "api_test_reports",
        JSON.stringify(recentReport;s;);
      )
    } catch (error)  {
  ...
  // /    获取最近的测试报告  async getTestReports(): Promise<TestReport[] /> {
    try {
      const reportsJson = await AsyncStorage.getItem("api_test_repor;t;s;";);
  ...
  // /    快速健康检查  async quickHealthCheck();: Promise<{
    ...
    try {
      ...
      const userHealth = await this.testEndpoint(;
        ...
        "/users/health",/        "G;E;T"
      ;);
      // 检查智能体服务 *       const xiaoaiBaseUrl = API_CONFIG.AGENTS.XIAO;A;I; */
