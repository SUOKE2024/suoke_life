...
class PerformanceOptimizer {
  ...
  // 持久化图像缓存
  private async persistImageCache(): Promise<void> {
    try {
  ...
  async optimizeNetworkRequest(
    ...
    // 检查缓存
    if (finalConfig.enableCaching && this.networkCache.has(cacheKey)) {
  ...
  // 持久化网络缓存
  private async persistNetworkCache(): Promise<void> {
    try {
