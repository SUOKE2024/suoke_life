...
// Voice 相关类型和功能 * export interface VoiceConfig {/;,}locale: string,;,/g,/;
  continuous: boolean,;
...
class NativeModulesManager {private cameraModule: unknown = null;,}private voiceModule: unknown = null;
...
try {";}}"";
const Voice = await import("@react-native-voice/voic;e;";);/              this.voiceModule = Voice.default;"}""/;"/g"/;
          } catch (error) {}
          }
        try {";,}const Geolocation = await import(;)";"";
            "@react-native-community/    geolocatio;n;");"/;"/g"/;
}
...
return watch;I;d;
  ...
  voice: boolean,;
    ...
    location: boolean,}
    const permissions = unknown;}> {const modules = this.getModulesStatus;,}const permissions = await permissionManager.checkHealthAppPermission;s;
}
    return {...modules,}
      permission;s;};
  }
  // 健康应用专用：请求所有必要权限  async requestHealthPermissions(): Promise<boolean> {/;,}try {const results = await permissionManager.requestHealthAppPermissio;n;s;,}const allGranted = Object.values(results).every(;);/g/;
