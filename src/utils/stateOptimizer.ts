...
// 状态优化器类
export class StateOptimizer {private static instance: StateOptimizerprivate stateData = new Map<string, StatePerformanceData>();
...
const isUnnecessary = boolean;
  ): void {const existing = this.stateData.get(stateName)if (existing) {const newUpdateCount = existing.updateCount + 1const newAverageTime = (existing.averageUpdateTime * existing.updateCount + duration) / newUpdateCount;
...
averageUpdateTime: states.length > 0 ? states.reduce(sum, state) => sum + state.averageUpdateTime, 0) / states.length : 0;
