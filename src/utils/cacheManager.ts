...
export class CacheManager {
  ...
  getStats(): CacheStats {
    ...
    if (items.length > 0) {
      // 找到最老和最新的项
      const sortedByTime = items.sort((a, b) => a[1].timestamp - b[1].timestamp);
      oldestItem = sortedByTime[0][0];
      newestItem = sortedByTime[sortedByTime.length - 1][0];
      
      // 找到访问最多和最少的项
      const sortedByAccess = items.sort((a, b) => b[1].accessCount - a[1].accessCount);
   ...
   * 确保有足够的空间
   */
  private async ensureSpace(requiredSize: number, options: Required<CacheOptions>): Promise<void> {
...
// 便捷函数
export const setCache = <T>(key: string, data: T, options?: Partial<CacheOptions>) => {
