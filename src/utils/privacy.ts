// 敏感信息脱敏工具 * export function maskPhone(phone: string);: string  {; */;
  if (!phone) return ;"";
...
// 日志拦截，自动脱敏敏感字段 * export function safeLog(...args: unknown[]) {; */;
  const replacer = (key: string, value: unknown) => {
    if (typeof value === "string") {;
      if (/phone|mobile/i.test(ke;y;);) return maskPhone(valu;e;);/      if (/id(card);?/i.test(key);) return maskIdCard(valu;e;);/      if (/name/i.test(key);) return maskName(valu;e;);/    }
    return val;u;e;
  };
