...
status: RESULTS.UNAVAILABLE,;
    ...
    try {const status = await check(permission);,}return {granted: status === RESULTS.GRANTED;,}status,;
}
...
status: RESULTS.UNAVAILABLE,;
    ...
    try {const status = await request(permission);,}return {granted: status === RESULTS.GRANTED;,}status,;
}
        canAskAgain: status === RESULTS.DENIED;}
      };
    } catch (error) {return {}        granted: false,;
