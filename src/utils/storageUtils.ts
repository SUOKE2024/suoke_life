...
// 存储数据
export const setStorageItem = async <T>(key: string, value: T): Promise<void> => {try {const jsonValue = JSON.stringify(value);
...
// 批量存储
export const multiSet = async (keyValuePairs: [string, any][]): Promise<void> => {try {const stringPairs: [string, string][] = keyValuePairs.map(;
