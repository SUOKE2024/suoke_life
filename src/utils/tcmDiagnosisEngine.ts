...
export interface Symptom {
  id: string;
  name: string;
  category: 'inspection' | 'auscultation' | 'inquiry' | 'palpation';
...
class ConstitutionIdentificationEngine {
  ...
  // 体质辨识算法
  identifyConstitution(diagnosisData: FourDiagnosisData): {
    constitution: ConstitutionType;
    confidence: number;
    ...
    const characteristics = this.getConstitutionCharacteristics(primaryConstitution);
  ...
  private calculateConstitutionScores(data: FourDiagnosisData): Record<ConstitutionType, number> {
    ...
    data.inquiry.symptoms.forEach(symptom => {
      if (symptom.name.includes('乏力') || symptom.name.includes('气短')) {
        scores['气虚质'] += 10;
      } else if (symptom.name.includes('怕冷') || symptom.name.includes('手足冰凉')) {
        scores['阳虚质'] += 10;
      } else if (symptom.name.includes('口干') || symptom.name.includes('盗汗')) {
        scores['阴虚质'] += 10;
      } else if (symptom.name.includes('痰多') || symptom.name.includes('身重')) {
        scores['痰湿质'] += 10;
      }
    });
...
class SyndromePatternEngine {
  ...
  analyzeSyndromePatterns(
    diagnosisData: FourDiagnosisData,
    constitution: ConstitutionType
  ): SyndromePattern[] {
    ...
    // 基于体质调整
    const adjustedPatterns = this.adjustPatternsForConstitution(patterns, constitution);
    
    // 按置信度排序
    return adjustedPatterns.sort((a, b) => b.confidence - a.confidence).slice(0, 5);
  }

  private analyzeEightPrinciples(data: FourDiagnosisData): SyndromePattern[] {
    const patterns: SyndromePattern[] = [];
    
    // 表里辨证
    const hasExteriorSymptoms = data.inquiry.symptoms.some(s => 
      s.name.includes('发热') || s.name.includes('恶寒') || s.name.includes('头痛')
    );
    ...
    // 寒热辨证
    const coldSymptoms = data.inquiry.symptoms.filter(s => 
      s.name.includes('怕冷') || s.name.includes('手足冰凉') || s.name.includes('喜热饮')
    ).length;
    
    const heatSymptoms = data.inquiry.symptoms.filter(s => 
      s.name.includes('发热') || s.name.includes('口渴') || s.name.includes('喜冷饮')
    ).length;
  ...
  private analyzeOrganSyndromes(data: FourDiagnosisData): SyndromePattern[] {
    const patterns: SyndromePattern[] = [];
    
    // 心系证候
    const heartSymptoms = data.inquiry.symptoms.filter(s => 
      s.name.includes('心悸') || s.name.includes('失眠') || s.name.includes('健忘')
    );
    ...
    // 肝系证候
    const liverSymptoms = data.inquiry.symptoms.filter(s => 
      s.name.includes('胁痛') || s.name.includes('易怒') || s.name.includes('头晕')
    );
  ...
  private analyzeQiBloodSyndromes(data: FourDiagnosisData): SyndromePattern[] {
    const patterns: SyndromePattern[] = [];
    
    // 气虚证
    const qiDeficiencySymptoms = data.inquiry.symptoms.filter(s => 
      s.name.includes('乏力') || s.name.includes('气短') || s.name.includes('懒言')
    );
    ...
    // 血虚证
    const bloodDeficiencySymptoms = data.inquiry.symptoms.filter(s => 
      s.name.includes('头晕') || s.name.includes('心悸') || s.name.includes('失眠')
    );
    
    if (bloodDeficiencySymptoms.length > 0 && data.inspection.complexion.includes('萎黄')) {
      patterns.push({
        id: 'blood_deficiency',
        name: '血虚证',
        category: '气血辨证',
    ...
    const prescriptionDatabase: Record<string, Prescription> = {
      'qi_deficiency': {
        ...
        composition: [
          ...
          { herb: '白术', dosage: '9', unit: 'g', processing: '炒用', function: '健脾益气' },
        ...
        modifications: [
          {
            ...
            herbs_to_add: [
              ...
              { herb: '砂仁', dosage: '3', unit: 'g', processing: '后下', function: '化湿开胃' }
            ]
          }
        ],
        dosage: '每日1剂',
      ...
      'blood_deficiency': {
        ...
        category: '补益剂',
        composition: [
          { herb: '当归', dosage: '12', unit: 'g', processing: '生用', function: '补血活血' },
          { herb: '川芎', dosage: '6', unit: 'g', processing: '生用', function: '活血行气' },
          { herb: '白芍', dosage: '12', unit: 'g', processing: '生用', function: '养血敛阴' },
          { herb: '熟地黄', dosage: '15', unit: 'g', processing: '熟用', function: '滋阴补血' }
        ],
        functions: ['补血调血', '养血和血'],
...
export class TCMDiagnosisEngine {
  ...
  async performDiagnosis(
    ...
    try {
      // 1. 体质辨识
      const constitutionResult = this.constitutionEngine.identifyConstitution(diagnosisData);
      ...
      // 3. 治疗原则制定
      const treatmentPrinciples = this.generateTreatmentPrinciples(syndromePatterns);
  ...
  private generateLifestyleRecommendations(
    ...
    for (const pattern of patterns) {
      if (pattern.name.includes('肝气郁结')) {
        baseRecommendations.recommendations.push('保持心情舒畅', '避免情绪激动');
  ...
  private generateReasoning(
    constitutionResult: { constitution: ConstitutionType; confidence: number; characteristics: string[] },
    patterns: SyndromePattern[],
    principles: TreatmentPrinciple[]
  ): string {
    let reasoning = `基于四诊合参的综合分析：\n\n`;
  ...
  // 获取中医知识库信息
  getTCMKnowledge(category: 'constitution' | 'syndrome' | 'herb' | 'acupoint'): any {
    const knowledge = {
      constitution: {
        types: ['平和质', '气虚质', '阳虚质', '阴虚质', '痰湿质', '湿热质', '血瘀质', '气郁质', '特禀质'],
        description: '中医体质学说将人体体质分为九种基本类型'
      },
      syndrome: {
        categories: ['八纲辨证', '脏腑辨证', '气血辨证', '六经辨证', '卫气营血辨证'],
