...
class DeviceInfoManager {
  ...
  private async getDeviceCapabilities(): Promise<DeviceCapabilities> {
    try {
      return {
        camera: await DeviceInfo.hasSystemFeature('android.hardware.camera') || Platform.OS === 'ios',
        microphone: await DeviceInfo.hasSystemFeature('android.hardware.microphone') || Platform.OS === 'ios',
        location: await DeviceInfo.hasSystemFeature('android.hardware.location') || Platform.OS === 'ios',
        biometrics: await DeviceInfo.supportedAbis().then(() => true).catch(() => false),
        nfc: await DeviceInfo.hasSystemFeature('android.hardware.nfc') || false,
        bluetooth: await DeviceInfo.hasSystemFeature('android.hardware.bluetooth') || Platform.OS === 'ios',
      };
    } catch (error) {
      console.error('检测设备功能失败:', error);
      return {
        camera: false,
        microphone: false,
        location: false,
  ...
  private async getCpuUsage(): Promise<number> {
    // 在React Native中，CPU使用率需要通过原生模块获取
    // 这里提供一个模拟实现
   ...
   * 获取电池消耗率
   */
  private async getBatteryDrain(): Promise<number> {
    try {
      const currentLevel = await DeviceInfo.getBatteryLevel();
      // 计算电池消耗率 (需要历史数据)
  ...
  private async getRenderTime(): Promise<number> {
    // 实际实现需要集成性能监控工具
    return Math.random() * 16; // 模拟16ms以内的渲染时间
  }
  ...
  async checkCompatibility(): Promise<{
    ...
    if (Platform.OS === 'ios') {
      ...
      const androidVersion = parseInt(deviceSpecs.systemVersion);
      if (androidVersion < 21) {
        issues.push('Android版本过低，建议升级到Android 5.0 (API 21)或更高版本');
      }
    }
    ...
    // 检查存储空间
    const freeStorageGB = deviceSpecs.freeDiskStorage / (1024 * 1024 * 1024);
    if (freeStorageGB < 1) {
   ...
   * 生成设备报告
   */
  async generateDeviceReport(): Promise<string> {
    const deviceSpecs = await this.getDeviceSpecs();
...
- 内存: ${(deviceSpecs.totalMemory / (1024 * 1024 * 1024)).toFixed(2)}GB
- 存储: ${(deviceSpecs.freeDiskStorage / (1024 * 1024 * 1024)).toFixed(2)}GB 可用
- 屏幕: ${deviceSpecs.screenWidth}x${deviceSpecs.screenHeight} (${deviceSpecs.pixelRatio}x)
