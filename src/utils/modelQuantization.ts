...
import { 
  ...
  QuantizationLevel,
  TargetDevice,
  { OptimizationLevel  } from '../core/onnx-runtime/types';
/**
 * 模型量化工具 - 提供便捷的模型量化接口
 * 集成ONNX Runtime的量化功能，为应用层提供简化的API
 */
export class ModelQuantizationUtils {
  ...
  private getDefaultQuantizationConfig(
    ...
    return {
      ...
      preserveAccuracy: level === 'fp16', // FP16保持精度，其他可以牺牲
      targetDevice: 'cpu',
      optimizationLevel: 'extended'
    };
  }
 ...
 * 获取量化工具实例
 */
export function getQuantizationUtils(): ModelQuantizationUtils {
  if (!quantizationUtils) {
    quantizationUtils = new ModelQuantizationUtils();
  }
...
export async function batchQuantizeSuokeLifeModels(
  models: Array<{ model: ONNXModel; type: 'tcm' | 'health' | 'symptom' | 'lifestyle' }>
): Promise<ONNXModel[]> {
  ...
  for (const { model, type } of models) {
    try {
      ...
      console.error(`量化模型失败 ${model.name}:`, error);
      // 继续处理其他模型
    }
  }
  ...
  switch (modelType) {
    ...
    case 'symptom':
      return {
        ...
        optimizationLevel: 'all' as OptimizationLevel
      };
    case 'lifestyle':
      return {
        ...baseConfig,
