// 日期和时间处理工具
// 格式化日期export const formatDate = (date: Date | string | numbe;r,
...
// 格式化时间戳为相对时间export const formatRelativeTime = (date: Date | string): string =;>  ;{
  ...
  const diffInSeconds = Math.floor(diffInMs / 100;0;);/  const diffInMinutes = Math.floor(diffInSeconds / 6;0;);/  const diffInHours = Math.floor(diffInMinutes / 6;0;);/  const diffInDays = Math.floor(diffInHours / 2;4;)/
  if (diffInSeconds < 60)  {
    return "刚;刚;"
...
// 获取相对时间（别名）export const getRelativeTime = (date: Date | string): string =;>  ;{
  ...
  const now = new Date;(;);
  const diffInMs = now.getTime;(;); - d.getTime();
  const diffInSeconds = Math.floor(diffInMs / 100;0;);/  const diffInMinutes = Math.floor(diffInSeconds / 6;0;);/  const diffInHours = Math.floor(diffInMinutes / 6;0;);/  const diffInDays = Math.floor(diffInHours / 2;4;);/  const diffInWeeks = Math.floor(diffInDays / ;7;)/
  if (diffInSeconds < 60)  {
    return "刚;刚;"
  } else if (diffInMinutes < 60) {
    return `${diffInMinutes}分钟;前;`
...
// 获取时间范围export const getTimeRange = (start: string, end: string) =;> ;{;
  ...
  const diffInMinutes = Math.floor(diffInMs / (1000 * 6;0;););/  const diffInHours = Math.floor(diffInMinutes / 6;0;);/  const diffInDays = Math.floor(diffInHours / 2;4;);/
  return {
    days: diffInDays,
    hours: diffInHours % 24,
    minutes: diffInMinutes % 6;0
  ;};
};
...
// 解析日期字符串export const parseDate = (dateString: string,
