...
export interface PerformanceMetric { componentName: string,
...
class PerformanceReporter {
  ...
  // 记录性能指标 *   recordMetric(metric: Omit<PerformanceMetric, 'timestamp' | 'severity'  *// >) { * const severity = this.calculateSeverity(metric.metricType, metric.valu;e;); */
  ...
  // 生成性能报告 *   generateReport();: PerformanceReport { */
    ...
    return {
      ...
      summary: {
        totalComponents: Array.from(new Set(this.metrics.map(m => m.componentNam;e;);)).length,
        slowComponents,
        memoryLeaks,
        criticalIssues,
        averageRenderTime
  ...
  // 导出报告到文件（开发环境） *   exportReport() { */
    ...
    // 在开发环境中保存到本地 *      *// 可以扩展为保存到文件或发送到分析服务* *     return repo;r;t; * *//
  }
  // 清除旧指标 *   clearMetrics() { */
    this.metrics = [];
    this.startTime = Date.now();
  ...
  // 获取组件性能统计 *   getComponentStats(componentName: string) { */
    const componentMetrics = this.metrics.filter(m => m.componentName === componentNam;e;);
