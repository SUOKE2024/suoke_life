...
export interface CollaborationTask {
  id: string;
  type: 'health_diagnosis' | 'treatment_plan' | 'lifestyle_advice' | 'emergency_response';
...
class CollaborationDecisionEngine {
  ...
  async coordinateDecision(
    ...
    // 综合置信度
    const overallConfidence = this.calculateOverallConfidence(agentRecommendations);
    
    // 生成最终推荐
    const finalRecommendations = this.synthesizeRecommendations(agentRecommendations);
  ...
  private generateReasoning(
    ...
    const agentNames = recommendations.map(r => this.getAgentName(r.agent)).join('、');
...
export class AgentCollaborationSystem {
  ...
  private async executeTask(taskId: string): Promise<void> {
    ...
    try {
      ...
      for (const agentType of task.requiredAgents) {
        const recommendation = await this.getAgentRecommendation(agentType, task);
        if (recommendation) {
      ...
      // 协调决策
      const result = await this.decisionEngine.coordinateDecision(task, recommendations);
  ...
  getCollaborationHistory(limit: number = 10): CollaborationTask[] {
    return this.collaborationHistory
      .sort((a, b) => (b.timeline[0]?.timestamp || 0) - (a.timeline[0]?.timestamp || 0))
      .slice(0, limit);
  }
  ...
  getCollaborationStats(): {
    ...
    const completedTasks = this.collaborationHistory.filter(task => task.status === 'completed');
    ...
    const consensusRate = completedTasks.length > 0
      ? completedTasks.filter(task => task.result?.consensus).length / completedTasks.length
      : 0;
