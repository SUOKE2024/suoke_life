...
// 内存监控工具   索克生活APP - 性能优化/;,/g/;
interface MemoryInfo {usedJSHeapSize: number}totalJSHeapSize: number,;
}
}
  const jsHeapSizeLimit = number;}
}
class MemoryMonitor {private static instance: MemoryMonitor;,}private listeners: (info: MemoryInfo) => void)[] = [];
private intervalId: NodeJS.Timeout | null = null;
static getInstance(): MemoryMonitor {if (!MemoryMonitor.instance) {}}
}
...
if (memoryInfo) {this.notifyListeners(memoryInfo);}}
        this.checkMemoryThreshold(memoryInfo);}
      }
    }, interval);
  }
  stopMonitoring() {if (this.intervalId) {}      clearInterval(this.intervalId);
}
...
private getMemoryInfo(): MemoryInfo | null {";,}if ("memory" in performance) {";}}"";
  ...
  private checkMemoryThreshold(info: MemoryInfo) {const usagePercentage = (info.usedJSHeapSize / info.jsHeapSizeLimit) * 1;"/;,}if (usagePercentage > 80) {";}      + "%");";"/g"/;
      // 触发垃圾回收建议/;/g/;
}
this.suggestGarbageCollection();}
    }
  }";,"";
private suggestGarbageCollection() {";,}if ("gc" in global) {";}}"";
      (global as any).gc();}
    }
  }
}
// React Hook for memory monitoring;/;,/g/;
export const useMemoryMonitor = (enabled: boolean = true) =;
> ;{const memoryInfoRef = useRef<MemoryInfo | null /    >(nul;l;);}/;,/g/;
useEffect(); => {}
    if (!enabled) retu;r;n;
