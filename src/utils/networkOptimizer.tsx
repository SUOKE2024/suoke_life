...
export interface RequestResponse<T = any  /> {/;}/      data: T,status: number,headers: Record<string, string>;/;/g/;
}
  cached: boolean,}
...
private pendingRequests: Map<string, Promise<RequestResponse  /> /> = new Map();/      private batchQueue: BatchRequest[] = [];/;,/g/;
private batchTimer: unknown = null;
}
}
  private readonly batchDelay = 50;  private readonly maxBatchSize = 10  / 最大批量大小* ///}/;,/g/;
private constructor() {}
  ...
  ///        const requestKey = this.generateRequestKey(config;);/;,/g/;
...
this.pendingRequests.delete(requestKey);
  ...
  ///        config: RequestConfig): Promise<RequestResponse<T>>  {}/;,/g/;
...
while (attempt <= maxRetries) {try {}        const controller = new AbortController;
        ...
        const timeoutId = config.timeout;}
          ...
          : null;
const: fetchConfig: RequestInit = {method: config.method,";,}const headers = {";}}"";
            "Content-Type": "application/json",/                ...config.headers;"}""/;"/g"/;
          }
...
const await = Promise.allSettled(promise;s;);
  ...
  // 按域名分组请求  private groupRequestsByDomain(requests: BatchRequest[]);: Record<string, BatchRequest[]>  {}/;,/g/;
...
return group;e;d;
  ...
  // 处理单个域名的请求  private async processDomainRequests(domain: string,)/;,/g/;
const requests = BatchRequest[]);: Promise<void>  {const concurrency = 5;,}chunks: this.chunkArray(requests, concurrenc;y;);
}
...
const await = Promise.allSettled(promise;s;);
  ...
  // 生成请求键  private generateRequestKey(config: RequestConfig): string  {}/;,/g/;
...
return `$ {method}:${url}: ${bodyStr;};`;````;```;
  ...
  cached: await cacheManager.get<RequestResponse<T>>(;k;e;y;);/          if (cached) {return {...cached,}/;,/g/;
const cached = tru;e;}
      }
    } catch (error) {}
      }
  ...
  // 缓存响应  private async cacheResponse(key: string,)/;,/g/;
...
//   ;/;/g/;
//   ;/;,/g/;
i;g;): Promise<RequestResponse<T  /     >> => {}/;,/g/;
return networkOptimizer.request<T>(confi;g;);
};
export const batchRequest = <T = any  />(/      config: RequestConf;);/;,/g/;
