...
class DeviceInfoManager {
  ...
  ///    > {
    try {
      ...
      const deviceSpecs: DeviceSpecs = { deviceId: await DeviceInfo.getUniqueId();
...
const performanceMonitor = usePerformanceMonitor('deviceInfo', {trackRender: true,)
  ...
  // 检查设备兼容性  async checkCompatibility(): Promise<{ compatible: boolean,issues: string[],recommendations: string[];
    ...
    const memoryGB = deviceSpecs.totalMemory  / (1024  1024  1024;)/     if (memoryGB < 2) {


    }
    const freeStorageGB = deviceSpecs.freeDiskStorage  / (1024  1024  1024;)/     if (freeStorageGB < 1) {
