...
// 动画类型 * export type AnimationType = | "fadeI;n;"; */
  | "fadeOut"
  | "slideInLeft"
  | "slideInRight"
  | "slideInUp"
...
// 动画配置接口 * export interface AnimationConfig {; */;
  ...
  loop?: boolean;
  iterations?: number}
// 创建基础动画值 * export const createAnimatedValue = (initialValue: number ;= ;0;): Animated.Value =>  {; */;
  return new Animated.Value(initialValu;e;);
};
export const createAnimatedValueXY = (initialValue: { x: numb;e;r, y: number} = { x: 0, y: 0};): Animated.ValueXY =>  {
  return new Animated.ValueXY(initialValu;e;);
};
// 基础动画函数 * export const animateValue = (animatedValue: Animated.Valu;e, */;
  toValue: number,
...
// 弹簧动画 * export const animateSpring = (animatedValue: Animated.Valu;e, */;
  toValue: number,
  config: {
    tension?: number;
    friction?: number;
...
// 序列动画 * export const animateSequence = (animations: Animated.CompositeAnimation;[;];): Animated.CompositeAnimation =>  {; */;
  return Animated.sequence(animation;s;);
};
// 并行动画 * export const animateParallel = (animations: Animated.CompositeAnimation;[;];): Animated.CompositeAnimation =>  {; */;
  return Animated.parallel(animation;s;);
};
// 交错动画 * export const animateStagger = (delay: numbe;r, */;
  animations: Animated.CompositeAnimation[];): Animated.CompositeAnimation =>  {
  return Animated.stagger(delay, animation;s;);
};
// 循环动画 * export const animateLoop = (animation: Animated.CompositeAnimatio;n, */;
...
// 手势动画工具 * export const gestureAnimations = ;{; */;
  ...
  // 滑动动画 *   createSwipeAnimation: (, */
    ...
    const toValue =
      direction === "left" || direction === "up" ? -distance : distan;c;e;
...
// 加载动画 * export const loadingAnimations = ;{; */;
  ...
  // 波浪加载 *   createWaveLoading: (animatedValues: Animated.Value[]) => { */
    const waveAnimations = animatedValues.map((value, inde;x;); => {
      value.setValue(0);
      const waveAnimation = animateSequence([;
        animateValue(value, 1, { duration: 300, delay: index * 1;0;0 ;}),
...
// 页面转场动画 * export const transitionAnimations = ;{; */
  // 滑动转场 *   slideTransition: (, */
    ...
    const distance = direction === "horizontal" ? screenWidth : screenHeig;h;t;
    return {
