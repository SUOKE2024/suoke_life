...
export class ComponentOptimizer {
   ...
   * 结束组件渲染计时
   */
  endRender(componentName: string, propsChanged: boolean = false, stateChanged: boolean = false): void {
    ...
    // 更新组件性能数据
    this.updateComponentData(componentName, renderTime, propsChanged, stateChanged);
...
export const useOptimizedMemo = <T>(
  ...
  if (depsChanged || valueRef.current === undefined) {
    ...
    if (debugName && __DEV__) {
      console.log(`useOptimizedMemo: ${debugName} computed in ${(endTime - startTime).toFixed(2)}ms`);
    }
  }
