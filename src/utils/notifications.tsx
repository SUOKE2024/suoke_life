...
/;/id: string,title: string,body: string,/g/;
...
options?: {foreground?: booleandestructive?: boolean;
}
...
options?: {customDismissAction?: booleanallowInCarPlay?: boolean;
}
...
class NotificationManager {private notificationModule: unknown = nullprivate messagingModule: unknown = null;
...
try {"if (Platform.OS === "ios" || Platform.OS === "android") {"try {";}}
        ...
        try {"const PushNotification = await import(;)react-native-push-notificati;o;n;
          ;);
}
...
this.createHealthReminderCategories();
  ...
  // 设置远程推送通知  private async setupRemoteNotifications() {/if (!this.messagingModule) {}}/g/;
    ...
    try {const authStatus = await this.messagingModule.requestPermissio;nconst  enabled =;
...
if (enabled) {const token = await this.messagingModule.getToke;nif (token) {this.deviceToken = token}
...
const permission = await permissionManager.checkPermission(;)notificatio;n;s;
      ;);
...
const permission = await permissionManager.requestPermissionWithDialog(;)notificatio;n;s;
      ;);
return permission.grant;e;d;
    } catch (error) {}
      return fal;s;e}
  ...
  // 发送本地通知  async scheduleLocalNotification(config: LocalNotificationConfig): Promise<boolean>  {/if (!this.notificationModule) {}}/g/;
    ...
    try {const hasPermission = await this.checkNotificationPermissio;nif (!hasPermission) {const granted = await this.requestNotificationPermissi;o;nif (!granted) {}
          return fal;s;e}
        }
      }
