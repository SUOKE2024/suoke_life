...
export class NetworkOptimizer {
  ...
  async request<T = any>(config: RequestConfig): Promise<RequestResponse<T>> {
    ...
    if (config.dedupe !== false && this.pendingRequests.has(requestKey)) {
      return this.pendingRequests.get(requestKey)! as Promise<RequestResponse<T>>;
    }
   ...
   * 批量请求
   */
  async batchRequest<T = any>(config: RequestConfig): Promise<RequestResponse<T>> {
  ...
  private async executeRequest<T>(config: RequestConfig): Promise<RequestResponse<T>> {
    ...
    while (attempt <= maxRetries) {
      try {
        ...
        // 缓存GET请求的响应
        if (config.cache !== false && config.method === 'GET') {
          await this.cacheResponse(this.generateRequestKey(config), result, config.cacheTTL);
        }
   ...
   * 按域名分组请求
   */
  private groupRequestsByDomain(requests: BatchRequest[]): Record<string, BatchRequest[]> {
   ...
   * 处理单个域名的请求
   */
  private async processDomainRequests(domain: string, requests: BatchRequest[]): Promise<void> {
    // 限制并发数，避免过载
    const concurrency = 5;
   ...
   * 获取缓存的响应
   */
  private async getCachedResponse<T>(key: string): Promise<RequestResponse<T> | null> {
    try {
   ...
   * 缓存响应
   */
  private async cacheResponse(key: string, response: RequestResponse, ttl?: number): Promise<void> {
    try {
...
// 便捷函数
export const optimizedRequest = <T = any>(config: RequestConfig): Promise<RequestResponse<T>> => {
  return networkOptimizer.request<T>(config);
};

export const batchRequest = <T = any>(config: RequestConfig): Promise<RequestResponse<T>> => {
