...
import crashlytics from "@react-native-firebase/crashlytics";""/;"/g"/;
// 错误类型枚举"/;,"/g"/;
...
if (error.code === 'NETWORK_ERROR') {';}}'';
    ...
    const return = this.handleError({)const type = ErrorType.NETWORK;,}severity,;
message,;
...
if (error.message?.includes('offline')) {';}';'';
    ...
    const return = this.handleError({)const type = ErrorType.AGENT_SERVICE;,}severity,);
}
      message,)}
...
timestamp: new Date(error.timestamp).toISOString(),;
  ...
  // 上报错误/;,/g/;
private reportError(error: AppError): void {if (!this.config.enableCrashlytics) return;,}if (error.severity < this.config.autoReportThreshold) return;
...
private notifyListeners(error: AppError): void {this.errorListeners.forEach(listener => {);,}try {);}}
  ...
  // 显示用户通知/;,/g/;
private showUserNotification(error: AppError): void {if (!this.config.enableUserNotification) return;,}if (error.severity < ErrorSeverity.MEDIUM) return;
