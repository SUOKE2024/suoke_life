...
private failed = new Set<string>();
  ...
  private constructor() {}
  static getInstance(): StartupOptimizer {if (!StartupOptimizer.instance) {}}
...
if (nonCriticalTasks.length > 0) {}}
  ...
  private async executeBatch(tasks: StartupTask[]): Promise<void> {for (const task of tasks) {}      if (this.canExecuteTask(task)) {try {}};
const await = this.executeTask(task);}
        } catch (error) {}}
  ...
  private canExecuteTask(task: StartupTask): boolean {if (this.completed.has(task.name) || this.running.has(task.name)) {}}
      return false;}
    }
