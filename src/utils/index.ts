...
// export * as apiCache from './apiCache';
...
// export { default as healthDataProcessor } from './healthDataProcessor';
// export { default as healthDataValidator } from './healthDataValidator';
...
export const performanceMonitor = {
  ...
  measureAsync: async (name: string, category: string, fn: () => Promise<any>) => await fn(),
  recordMetric: (name: string, value: number, category: string, unit?: string) => {},
  getMetricHistory: (category: string, name: string) => [],
  generateReport: () => ({ id: 'test', timestamp: Date.now(), metrics: [], summary: {} }),
...
export const cacheManager = {
  ...
  set: async (type: string, key: string, value: any, options?: any) => true,
  clear: async (type: string) => true,
  setMultiple: async (type: string, items: any[]) => items.map(() => true),
  getMultiple: async (type: string, keys: string[]) => new Map(),
  getStats: (type: string) => ({ hits: 0, misses: 0 }),
  getOrSet: async (type: string, key: string, loader: () => Promise<any>) => await loader()
};
