...
export const SERVICE_CATEGORIES: CategoryConfig[] = [
  ...
  { key: 'subscription', label: '订阅', icon: 'calendar-check', color: '#3498DB' },
  { key: 'appointment', label: '预约', icon: 'calendar-clock', color: '#F39C12' },
...
// 生态服务配置
export const ECO_SERVICES: ServiceItem[] = [
  {
...
// 推荐服务（基于用户行为和评分）
export const RECOMMENDED_SERVICES = ALL_SERVICES
  .filter(service => service.rating && service.rating >= 4.7)
  .sort((a, b) => (b.rating || 0) - (a.rating || 0))
  .slice(0, 6);

// 热门服务（基于使用量）
export const POPULAR_SERVICES = ALL_SERVICES
  .filter(service => service.reviewCount && service.reviewCount >= 1000)
