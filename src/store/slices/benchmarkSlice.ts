...
const benchmarkSlice = createSlice({
  ...
  reducers: {
    // 设置当前任务
    setCurrentTask: (state, action: PayloadAction<BenchmarkTask | null>) => {
...
// 过滤后的任务列表
export const selectFilteredTasks = (state: { benchmark: BenchmarkState }) => {
  const { tasks, filters } = state.benchmark;
...
// 任务统计
export const selectTaskStats = (state: { benchmark: BenchmarkState }) => {
  const tasks = state.benchmark.tasks;
