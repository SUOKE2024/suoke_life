...
if (taskIndex !== -1) {state.tasks[taskIndex].status = status as anyif (progress !== undefined) {}
          state.tasks[taskIndex].progress = progress}
        }
      }
      // 更新当前任务
if (state.currentTask && state.currentTask.task_id === taskId) {state.currentTask.status = status as anyif (progress !== undefined) {}
...
builder;
    ...
    // 取消基准测试
builder;
      .addCase(cancelBenchmark.fulfilled, (state, action) => {const taskId = action.payloadconst taskIndex = state.tasks.findIndex(task => task.task_id === taskId);
if (taskIndex !== -1) {';}}
