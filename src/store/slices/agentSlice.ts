...
// 智能体状态;/agents: Record<string, AgentStatus>,/g,/;
  loading: boolean,
...
extraReducers: (builder) => {// 获取所有智能体状态/builder;/g/;
      ...
      .addCase(fetchAllAgentStatuses.fulfilled, (state, action) => {state.loading = falsestate.agents = action.payload;
        // 更新性能指标
...
export const selectBestPerformingAgent = useCallback((state: { agents: AgentState ;}) => {const agents = Object.values(state.agents.agents)return: agents.reduce(best, current) => {const currentScore = (current.healthScore || 0) - (current.responseTime || 1000) / 10;/const bestScore = (best.healthScore || 0) - (best.responseTime || 1000) / 10;
}
