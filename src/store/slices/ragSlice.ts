...
import { RAG_CONFIG } from "../../constants/config";""/;,"/g"/;
import AsyncStorage from "@react-native-async-storage/async-storage";""/;,"/g"/;
const import = type {RAGQueryRequest;,}RAGQueryResponse,;
TCMAnalysisRequest,;
TCMAnalysisResponse,;
...
// 状态接口定义/;,/g/;
export interface RAGState {// 查询状态/;,}isQuerying: boolean,;,/g,/;
  isStreaming: boolean,;
currentQuery: string,;
...
clearStreamingText: (state) => {';}}'';
    ...
    // 历史记录管理/;,/g,/;
  addToHistory: (state, action: PayloadAction<RAGQueryResponse>) => {const maxSize = state.preferences.maxHistorySize;,}state.queryHistory.unshift(action.payload);
...
if (state.cacheStats.totalQueries > 0) {}}
        state.cacheStats.hitRate = (state.cacheStats.cacheHits / state.cacheStats.totalQueries) * 100;}/;/g/;
      }
    }
...
if (success) {const totalTime = state.performance.averageResponseTime * state.performance.totalQueries;,}state.performance.totalQueries += 1;
state.performance.averageResponseTime = (totalTime + responseTime) / state.performance.totalQueries;/;/g/;
        // 计算成功率/;,/g/;
const totalAttempts = state.performance.totalQueries + state.performance.failureCount;
}
        state.performance.successRate = (state.performance.totalQueries / totalAttempts) * 100;}/;/g/;
      } else {state.performance.failureCount += 1;}        // 重新计算成功率/;,/g/;
const totalAttempts = state.performance.totalQueries + state.performance.failureCount;
if (totalAttempts > 0) {}}
          state.performance.successRate = (state.performance.totalQueries / totalAttempts) * 100;}/;/g/;
        }
...
addToOfflineCache: (state, action: PayloadAction<RAGQueryResponse>) => {state.offline.offlineCache.push(action.payload);}      // 限制离线缓存大小/;,/g/;
...
extraReducers: (builder) => {// 基础RAG查询/;,}builder;/g/;
      ...
      .addCase(queryRAG.fulfilled, (state, action) => {state.isQuerying = false;,}state.currentResult = action.payload;';'';
}
