...
import {medKnowledgeService;,}Constitution,;
Symptom,;
Acupoint,;
Herb,;
Syndrome,;
...
// 状态接口定义/;,/g/;
export interface MedKnowledgeState {// 数据状态/;,}constitutions: Constitution[],;,/g,/;
  symptoms: Symptom[],;
acupoints: Acupoint[],;
herbs: Herb[],;
syndromes: Syndrome[],;
...
selectSyndrome: (state, action: PayloadAction<Syndrome | null>) => {}}
    ...
    // 搜索历史管理/;,/g,/;
  addToSearchHistory: (state, action: PayloadAction<string>) => {const query = action.payload.trim();,}if (query && !state.searchHistory.includes(query)) {state.searchHistory.unshift(query);}        // 保持最多10条历史记录/;,/g/;
...
extraReducers: (builder) => {// 体质相关/;,}builder;/g/;
      ...
      .addCase(fetchConstitutions.fulfilled, (state, action) => {state.loading.constitutions = false;,}state.constitutions = action.payload;
}
      ...
      .addCase(fetchConstitutionById.fulfilled, (state, action) => {state.selectedConstitution = action.payload;}        // 更新列表中的对应项/;,/g/;
...
if (index !== -1) {}}
      ...
      .addCase(fetchSymptoms.fulfilled, (state, action) => {state.loading.symptoms = false;,}state.symptoms = action.payload;
}
      ...
      .addCase(fetchAcupoints.fulfilled, (state, action) => {state.loading.acupoints = false;,}state.acupoints = action.payload;
}
      ...
      .addCase(fetchHerbs.fulfilled, (state, action) => {state.loading.herbs = false;,}state.herbs = action.payload;
}
      ...
      .addCase(fetchSyndromes.fulfilled, (state, action) => {state.loading.syndromes = false;,}state.syndromes = action.payload;
}
      ...
      .addCase(searchKnowledge.fulfilled, (state, action) => {state.loading.search = false;,}state.searchResults = action.payload;
}
