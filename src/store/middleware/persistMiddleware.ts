...
// 持久化中间件 * export const persistMiddleware: Middleware =;(store) => (next) => (action: unknown) => {; */;
  ...
  try {
    switch (actionType) {
      ...
      case "auth/logout/fulfilled":/        // 清除认证信息 *         await AsyncStorage.multiRemove([ */
        ...
        ;];);
        break
      case "ui/setTheme":/        // 持久化主题设置 *         await AsyncStorage.setItem(STORAGE_CONFIG.KEYS.THEME, state.ui.them;e;); */
        break
      ...
      case "user/updateProfile/fulfilled":/        // 持久化用户资料（缓存） *         if (state.user.profile) { */
          await AsyncStorage.setItem(
            ...
            JSON.stringify(state.user.profil;e;);
          );
        }
        break
      default: break}
...
// 恢复持久化状态 * export async function restorePersistedState() {; */;
  try {
    const [theme, language, authToken, refreshToken, userId] = await AsyncStorage.multiGet([;
...
// 清除所有持久化数据 * export async function clearPersistedState() {; */;
  try {
    const keys = Object.values(STORAGE_CONFIG.KEY;S;);
