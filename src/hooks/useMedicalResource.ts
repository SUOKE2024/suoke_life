...
export const useMedicalResource = () => {const dispatch = useDispatch<AppDispatch>();
  ...
  const createAppointment = useCallback((appointmentData: Omit<Appointment, 'id' | 'status'>) => {return dispatch(bookAppointment(appointmentData));
  }, [dispatch]);
  ...
  const loadMoreResults = useCallback(() => {if (pagination.hasMore && !loading.search) {const nextPageQuery: SearchQuery = {...searchQuery,page: pagination.currentPage + 1;
      };
      return search(nextPageQuery);
    }
  }, [search, searchQuery, pagination, loading.search]);

  const refreshResults = useCallback(() => {const refreshQuery: SearchQuery = {...searchQuery,page: 1;
    };
  ...
  const quickBookAppointment = useCallback((
    resourceId: string,resourceName: string,userId: string,serviceType: string,scheduledTime: string,duration: number = 30,notes?: string;
