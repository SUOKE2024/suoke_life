...
export const useAgent = (initialAgent: AgentType = 'xiaoai'): UseAgentReturn => {
  ...
  const getAgentInfo = useCallback((agent: AgentType) => {
    const agentInfoMap = {
      xiaoai: { name: '小艾', role: '健康助手', status: 'active', avatar: '🤖' },
      xiaoke: { name: '小克', role: '数据分析师', status: 'active', avatar: '📊' },
      laoke: { name: '老克', role: '中医专家', status: 'active', avatar: '👨‍⚕️' },
      soer: { name: '索儿', role: '生活顾问', status: 'active', avatar: '🌟' },
    };
