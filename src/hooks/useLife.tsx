...
LifeHabit,";,"";
LifeGoal,";"";
  { LifeStats } from "../types/life/      SOER_SUGGESTIONS,";""/;,"/g"/;
HEALTH_METRICS,;
LIFE_PLANS,;
LIFE_HABITS,";,"";
LIFE_GOALS,";"";
  { LIFE_STATS } from ";../data/lifeData";/    export const useLife = () =;"/;"/g"/;
> ;{";}  // 性能监控"/;,"/g,"/;
  const: performanceMonitor = usePerformanceMonitor(useLife", {")";}}"";
    trackRender: true,}";,"";
trackMemory: true,warnThreshold: 50, // ms ;};);"/;,"/g"/;
const [suggestions, setSuggestions] = useState<LifeSuggestion[]  />(SOER_SUGGESTION;S;);/  const [healthMetrics, setHealthMetrics] = useState<HealthMetric[]  />(HEALTH_METRIC;S;);/  const [lifePlans, setLifePlans] = useState<LifePlan[]  />(LIFE_PLAN;S;);/  const [habits, setHabits] = useState<LifeHabit[]  />(LIFE_HABIT;S;);/  const [goals, setGoals] = useState<LifeGoal[]  />(LIFE_GOAL;S;);/  const [stats, setStats] = useState<LifeStats  />(LIFE_STAT;S;)/      const [activeTab, setActiveTab] = useState<"suggestions | "metrics" | plans">("suggestions;);""/;,"/g"/;
...
setStats(prev => ({));}      ...prev,);
      ...
      const completedSuggestions = prev.completedSuggestions + 1;)}
    }));
...
const iconMap: Record<string, string> = {up: trending-up";",";,}down: "trending-down,",";"";
}
      const stable = "trending-neutral"}"";"";
    ;}
...
category?: string;priority?: string;completed?: boolea;n;); => {}
    const return = suggestions.filter(suggestion => {});
...
const calculatedStats = useMemo() => {;,}const completedSuggestions = suggestions.filter(s => s.completed).leng;t;h;
...
const completedPlans = lifePlans.filter(p => p.progress >= 100).leng;t;h;
    ...
    averageProgress: lifePlans.reduce(sum,p;); => sum + p.progress, 0) / lifePlans.length;/        const activeHabits = habits.filter(h => h.streak > 0).leng;t;h;}/;,/g/;
return {completedSuggestions,totalSuggestions,completionRate: (completedSuggestions / totalSuggestions) * 100,/          activePlans,completedPlans,averageProgress,activeHabits,totalHabits: habits.lengt;h;};/;/g/;
  }, [suggestions, lifePlans, habits]);
const getTodaySuggestions = useCallback() => {;";,}return suggestions;";"";
      .filter(s => !s.completed && s.priority === "high");";"";
}
      .slice(0,3;);}
  }, [suggestions]);
const getRecommendedActions = useCallback() => {;";,}const actions = ;[;];";,"";
const urgentSuggestions = suggestions.filter(s => !s.completed && s.priority === high";) ";
if (urgentSuggestions.length > 0) {";,}actions.push({";,)type: "suggestion,",")"";})";"";
}
)"}";
action: () => setActiveTab(suggestions");});"";"";
    }
    const lowProgressPlans = lifePlans.filter(p => p.progress < 30;);
if (lowProgressPlans.length > 0) {";,}actions.push({";,)type: "plan,",")"";})";"";
}
)"}";
...
updateHealthMetric: useCallback(metricId: string, value: number;) => {}
    setHealthMetrics(prev => {});
prev.map(metric => {});
metric.id === metricId;
          ? {...metric,";,}value,";"";
}
              const trend = value > metric.value ? "up : value < metric.value ? "down" : stable"}"";"";
            ;}
          : metric;
      );
    );
  }, []);
return {suggestions}healthMetrics,;
lifePlans,;
habits,;
goals,;
}
