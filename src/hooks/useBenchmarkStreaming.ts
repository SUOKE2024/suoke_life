...
export const useBenchmarkStreaming = (options: UseBenchmarkStreamingOptions = {}) => {
  const {
    autoConnect = true,
    maxEvents = 100,
    eventTypes = ['benchmark_progress', 'benchmark_complete', 'benchmark_error', 'system_status']
  } = options;

  const [state, setState] = useState<BenchmarkStreamingState>({
    isConnected: false,
  ...
  const handleStreamEvent = useCallback((event: StreamEvent) => {
    setState(prevState => ({
      ...prevState,
      events: [event, ...prevState.events].slice(0, maxEvents),
      error: null
    }));
  }, [maxEvents]);
  ...
  // 启动流式基准测试
  const startStreamingBenchmark = useCallback((config: StreamConfig) => {
    if (state.isConnected) {
      benchmarkStreamingService.startStreamingBenchmark(config);
    } else {
  ...
  // 停止流式基准测试
  const stopStreamingBenchmark = useCallback((benchmarkId: string) => {
    if (state.isConnected) {
      benchmarkStreamingService.stopStreamingBenchmark(benchmarkId);
    }
  }, [state.isConnected]);
  ...
  // 获取最新事件
  const getLatestEvent = useCallback((eventType?: string) => {
    if (eventType) {
      return state.events.find(event => event.type === eventType) || null;
    }
  ...
  useEffect(() => {
    const checkConnectionState = () => {
      const currentState = benchmarkStreamingService.getConnectionState();
      setState(prevState => ({
        ...prevState,
  ...
  return {
    ...
    connect,
    disconnect,
    startStreamingBenchmark,
    stopStreamingBenchmark,
    clearEvents,
