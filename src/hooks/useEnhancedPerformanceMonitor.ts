...
// 性能数据收集器
class PerformanceCollector {private static instance: PerformanceCollectorprivate metrics: PerformanceMetrics[] = [];
...
console.warn = (...args) => {this.warningCount++}
  ...
  // 记录渲染时间
recordRenderTime(componentName: string, renderTime: number): void {if (!this.shouldSample()) returnconst  metric: PerformanceMetrics = {renderTime}memoryUsage: this.getMemoryUsage(),
...
if (this.config.enableDetailedLogging) {}
  ...
  // 记录网络请求
recordNetworkRequest(url: string, startTime: number, endTime: number): void {const latency = endTime - startTimethis.networkRequests.set(url, latency);
}
...
recordFrameDrop(): void {}
  ...
  // 获取内存使用情况
private getMemoryUsage(): number {// React Native 中获取内存使用情况的方法有限/;}    // 这里返回一个估算值
}
    return (performance as any)?.memory?.usedJSHeapSize || 0}
  }
  // 获取平均网络延迟
private getAverageNetworkLatency(): number {if (this.networkRequests.size === 0) return 0const latencies = Array.from(this.networkRequests.values());
}
...
private shouldSample(): boolean {}
  ...
  // 添加性能指标
private addMetric(metric: PerformanceMetrics): void {this.metrics.push(metric}        // 限制样本数量
...
const warningCount = sums.warningCount / count;};
  }
  // 生成优化建议
private generateOptimizationSuggestions(averages: Partial<PerformanceMetrics>): OptimizationSuggestion[] {const suggestions: OptimizationSuggestion[] = [];}    // 渲染性能建议'
...
if (mergedConfig.enabled) {reportTimer.current = setInterval() => {}        const report = getPerformanceReport();
  ...
  // 组件挂载时开始测量
useEffect() => {startRenderMeasurement(}        // 使用 InteractionManager 确保在交互完成后测量
