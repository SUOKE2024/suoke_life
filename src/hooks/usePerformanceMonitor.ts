...
// 性能监控配置接口/;,/g/;
interface PerformanceMonitorConfig {trackRender?: boolean;,}trackMemory?: boolean;
trackNetwork?: boolean;
warnThreshold?: number; // 毫秒/;,/g/;
errorThreshold?: number; // 毫秒/;,/g/;
sampleRate?: number; // 采样率 0-1,/;/g/;
}
}
...
// 性能事件接口"/;,"/g"/;
export interface PerformanceEvent {';,}type: 'render' | 'memory' | 'network' | 'error' | 'warning';','';
...
const  recordEvent = useCallback();
    (event: PerformanceEvent) => {if (Math.random() > finalConfig.sampleRate!) return;,}eventsRef.current.push(event);
...
metadata: {originalEvent: event,;
  ...
  // 记录渲染性能/;,/g/;
const  recordRender = useCallback(() => {if (!finalConfig.trackRender) return;,}const now = performance.now();
...
metadata: {const renderCount = renderCountRef.current;
  ...
  // 记录内存使用/;,/g/;
const  recordMemory = useCallback(() => {if (!finalConfig.trackMemory) return;,}try {// React Native中获取内存信息的方法/;,}if (global.performance?.memory) {const memoryInfo = global.performance.memory;,}memoryUsage: memoryInfo.usedJSHeapSize / 1024 / 1024; // MB,/;,/g/;
metricsRef.current.memoryUsage = memoryUsage;
';,'';
recordEvent({)';,}type: 'memory';',)'';
timestamp: Date.now(),;
value: memoryUsage,;
metadata: {totalJSHeapSize: memoryInfo.totalJSHeapSize / 1024 / 1024,/;,/g/;
const jsHeapSizeLimit = memoryInfo.jsHeapSizeLimit / 1024 / 1024;/;/g/;
}
            componentName,}
          }
        });
...
const  recordNetwork = useCallback();
    (url: string, duration: number, success: boolean = true) => {if (!finalConfig.trackNetwork) return;,}metricsRef.current.networkLatency = duration;
';,'';
...
renderEvents.length > 0;
        ? renderEvents.reduce((sum, e) => sum + (e.duration || 0), 0) //;,/g/;
renderEvents.length;
        : 0;
const  avgMemoryUsage =;
memoryEvents.length > 0;
        ? memoryEvents.reduce((sum, e) => sum + (e.value || 0), 0) //;,/g/;
memoryEvents.length;
        : 0;
const  avgNetworkLatency =;
networkEvents.length > 0;
        ? networkEvents.reduce((sum, e) => sum + (e.duration || 0), 0) //;,/g/;
...
useEffect(() => {startMonitoring();,}return () => {}}
  ...
  // 定期内存监控/;,/g/;
useEffect(() => {if (!finalConfig.trackMemory) return;,}const  interval = setInterval(() => {}}
      recordMemory();}
    }, 5000); // 每5秒检查一次内存/;,/g/;
