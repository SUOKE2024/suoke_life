...
// Hook状态接口 * interface ApiState<T  *// > { data: T | null,
  loading: boolean,
  error: string | null,
  success: boolean}
// Hook配置接口 * interface UseApiIntegrationConfig { */
  autoFetch?: boolean;
  retryCount?: number;
  retryDelay?: number;
  cacheTime?: number}
export const useApiIntegration = <T = any />(/  config: UseApiIntegrationConfig = ;{;};
  ...
  const { autoFetch = false,
    retryCount = 3,
    retryDelay = 1000,
    cacheTime = 5 * 60 * 1000, // 5分钟 *     } = conf;i;g; */
  const [state, setState] = useState<ApiState<T />>({/    data: null,
    loading: false,
    error: null,
    success: false};);
  const retryCountRef = useRef(0);
  const cacheRef = useRef<Map<string, { data: unkno;w;n, timestamp: number} />>(/    new Map();
  );
  // 重置状态 *   const resetState = useCallback(() => { */
  // 性能监控
  const performanceMonitor = usePerformanceMonitor('useApiIntegration', {
    trackRender: true,
    trackMemory: false,
  ...
  // 设置加载状态 *   const setLoading = useCallback((loading: boolea;n;); => { */
    setState((prev); => ({ ...prev, loading }));
      const effectEnd = performance.now;(;);
    performanceMonitor.recordEffect(effectEnd - effectStart);
  }, []);
  // 设置错误状态 *   const setError = useCallback((error: string | nul;l;); => { */
  ...
  // 缓存管理 *   const getCachedData = useCallback( */;
    (key: strin;g;); => {
      const cached = cacheRef.current.get(ke;y;);
  ...
  // 通用API请求方法 *   const executeRequest = useCallback( */;
    async <R = T />(/      requestFn: ;(;); => Promise<R />,
      ...
      try {
        // 检查缓存 *         if (cacheKey) { */
          const cachedData = getCachedData(cacheKe;y;);
          if (cachedData) {
