...
// 自定义RAG Hook
export const useRAGService = (): UseRAGServiceReturn => {
  ...
  // 中医症状分析
  const analyzeTCMSymptoms = useCallback(async (request: TCMAnalysisRequest) => {
    try {
  ...
  // 中药推荐
  const getHerbRecommendations = useCallback(async (request: HerbRecommendationRequest) => {
    try {
      await dispatch(recommendHerbs(request) as any);
    } catch (error) {
  ...
  // 多模态查询 - 图像
  const queryWithImage = useCallback(async (query: string, imageUri: string, userId: string) => {
    const request: RAGQueryRequest = {
      query,
      userId,
      taskType: 'diagnosis',
      multimodalData: [
        { type: 'image', data: imageUri, metadata: { format: 'jpeg' } }
      ]
    };
    await dispatch(queryRAG(request) as any);
  }, [dispatch]);
  ...
  // 多模态查询 - 音频
  const queryWithAudio = useCallback(async (query: string, audioUri: string, userId: string) => {
    const request: RAGQueryRequest = {
      query,
      userId,
      taskType: 'consultation',
      multimodalData: [
        { type: 'audio', data: audioUri, metadata: { format: 'wav' } }
      ]
    };
    await dispatch(queryRAG(request) as any);
  }, [dispatch]);
  ...
  // 多模态查询 - 传感器数据
  const queryWithSensorData = useCallback(async (query: string, sensorData: any, userId: string) => {
    const request: RAGQueryRequest = {
      query,
      userId,
      taskType: 'diagnosis',
      multimodalData: [
        { type: 'sensor', data: JSON.stringify(sensorData), metadata: { source: 'device' } }
      ]
    };
    await dispatch(queryRAG(request) as any);
  }, [dispatch]);
  ...
  const updateUserPreferences = useCallback((prefs: Partial<any>) => {
  ...
  const getCacheInfo = useCallback(() => {
    return ragService.getCacheStats();
  }, []);
  ...
  const syncPendingQueries = useCallback(async () => {
    if (!offlineStatus.isOffline && offlineStatus.pendingQueries.length > 0) {
      for (const pendingQuery of offlineStatus.pendingQueries) {
        try {
          await dispatch(queryRAG(pendingQuery) as any);
        } catch (error) {
  ...
  const searchHistory = useCallback((keyword: string) => {
  ...
  // 智能推荐
  const getSmartSuggestions = useCallback(async (context: string) => {
    try {
      // 基于上下文生成智能建议
  ...
  const getRelatedQueries = useCallback(async (query: string) => {
    try {
      // 获取相关查询建议
  ...
  // 健康评估
  const performHealthAssessment = useCallback(async (symptoms: string[], constitution: string) => {
    try {
      const request: TCMAnalysisRequest = {
        symptoms,
        constitutionType: constitution as any,
        userId: 'current-user'
      };
  ...
  const getPreventionAdvice = useCallback(async (riskFactors: string[]) => {
    try {
      const request: RAGQueryRequest = {
        query: `基于以下风险因素提供预防建议: ${riskFactors.join(', ')}`,
        userId: 'current-user',
        taskType: 'prevention',
        context: {
          riskFactors,
          requestType: 'prevention_advice'
        }
      };
