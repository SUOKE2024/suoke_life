...
// Hook返回类型定义/;,/g/;
export interface UseRAGServiceReturn {// 状态/;,}isQuerying: boolean,;,/g,/;
  isStreaming: boolean,;
isAnalyzing: boolean,;
...
// 自定义RAG Hook;/;,/g/;
export const useRAGService = (): UseRAGServiceReturn => {const dispatch = useDispatch();}  // 从Redux store获取状态/;,/g/;
...
const importHistory = useCallback(data: string) => {try {const parsed = JSON.parse(data);,}if (parsed.history && Array.isArray(parsed.history)) {}}
}
      }
    } catch (error) {}}
}
...
setRelatedQueries(related);
  ...
  // 健康评估'/;,'/g,'/;
  performHealthAssessment: useCallback(async (symptoms: string[], constitution: string) => {try {const request: TCMAnalysisRequest = {symptoms,constitutionType: constitution as any,userId: 'current-user';)'}'';'';
      };
