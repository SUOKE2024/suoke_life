...
onPress={() = accessibilityLabel="TODO: 添加无障碍标签" /    > !isDisabled && onTabPress(tab.id);}"/;,"/g"/;
activeOpacity={0.7}
        disabled={isDisabled}
      >;
        <View style={styles.tabContent} /    >/;/g/;
          <Text;  />/;,/g/;
...
;
        {isSelected && <View style={styles.activeIndicator} /    >};/;/g/;
      </    TouchableOpacity;>/;/g/;
    ;);
  };
const content = useMemo() => (;));
    <View style={[styles.container, style]} /    >/;/g/;
      {tabs.map(renderTab)}
    </    View>/;/g/;
  ), []);
if (scrollable) {return (;);}      <ScrollView;  />/;/g/;
}
horizontal;}
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={styles.scrollContainer} /    >;/;/g/;
        {content};
      </    ScrollView;>/;/g/;
    ;);
  }
