...
interface ChatMessageProps {const message = Message;,}onPress?: (message: Message) => void;
...
return (;);
    <View,style={ />/;}[;];";"/g"/;
      ...
      ]}} />/      {///          {!isUser && showAvatar  && <View style={styles.agentInfo}>/          <View style={[styles.agentAvatar, { backgroundColor: agent?.color + "2;0  ; }}]}  />/            <Text style={styles.agentAvatarText}>{agent?.avatar}</Text>/          </View>/          <Text style={styles.agentName}>{agent?.name}</Text>/        </View>/          )}""/;"/g"/;
...
styles.messageText,;
        ...
        {///            {showTimestamp  && <View style={styles.messageFooter}>/            <Text style={styles.timestamp}>/                  {formatTimestamp(message.timestamp)}/;/g/;
