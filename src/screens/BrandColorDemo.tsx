    ...
    <View style={styles.statusColors} />///        <Text variant="h6" style={{ ///  >;
        marginBottom: 12,color: theme.colors.onSurface,fontWeight: "bold}} />/////            状态色彩系统;"
          ...
          <View key={index} style={styles.statusItem} />///              <View style={[ ///  >
            ...
            ]} />/            <Text variant="caption" style={{ textAlign: "center", marginTop: 4}} />/////                  {status.icon} {status.name}
            </Text>/          </View>/////    ))}
      </View>/    </View>/////      );
};
const BrandColorDemo: React.FC  = () => {}
  ...
  return (;
    <ScrollView;
      ...
      {// 索克绿色系 }/      <Card style={styles.section} />/        <ColorGroup title="🌿 索克绿色系" colors={suokeGreenColors} />/      </Card>/////
      {// 索克橙色系 }/      <Card style={styles.section} />/        <ColorGroup title="🧡 索克橙色系" colors={suokeOrangeColors} />/      </Card>/////
      {// 中医五行色彩 }/      <Card style={styles.section} />/        <ColorGroup title="🏮 中医五行色彩" colors={tcmColors} />/      </Card>/////
      {// 按钮组件演示 }/      <Card style={styles.section} />///          <Text variant="h4" style={{ ///  >
          marginBottom: 20,
          color: theme.colors.primary,
          fontWeight: "bold"}} />/////              🎨 按钮组件演示
        </Text>/////
        <ButtonShowcase title="主要按钮 (索克绿)" buttons={primaryButtons} / accessibilityLabel="TODO: 添加无障碍标签" />/        <ButtonShowcase title="次要按钮 (索克橙)" buttons={secondaryButtons} / accessibilityLabel="TODO: 添加无障碍标签" />/        <ButtonShowcase title="轮廓按钮" buttons={outlineButtons} / accessibilityLabel="TODO: 添加无障碍标签" />/      </Card>/{// 状态色彩系统 }/      <Card style={styles.section} />/        <StatusColorDemo colors={statusColors} />/      </Card>/////
      {// 底部说明 }/      <Card style={Object.assign({}, styles.section, styles.footerCard)} />/        <Text variant="h4" style={styles.footerTitle} />/////              设计理念 💡
