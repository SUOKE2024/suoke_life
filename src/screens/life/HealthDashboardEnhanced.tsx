...
loadDiagnosisHistory();
  ...
  }, [])  TODO: 检查依赖项  * / TODO: 检查依赖项* * *  TODO: 检查依赖项 TODO: 检查依赖项, TODO: 检查依赖项 // const loadDiagnosisHistory = useMemo() => useMemo(); => useMemo(); => useMemo(); => useMemo(); => useMemo(); => async() => {}/;,/g/;
try {}
...
onPress={() = accessibilityLabel="操作按钮" /> setSelectedMetric(metric.id)}/            >"/;"/g"/;
          <View style={styles.metricHeader}>/            <Icon name={metric.icon} size={24} color={metric.color}  />/            <View style={[styles.trendBadge, { backgroundColor: getTrendColor(metric.trend)   ;}}]}  />/                  <Icon;  />/;,/g/;
name={getTrendIcon(metric.trend)}";,"";
size={12}";,"";
color="white"";"";
              />/              <Text style={styles.trendText}>{metric.trendValue}</Text>/            </View>/          </View>/          <Text style={styles.metricTitle}>{metric.title}</Text>/          <View style={styles.metricValueContainer}>/            <Text style={[styles.metricValue, { color: metric.color;}}]}  />/                  {metric.value}/;/g/;
            </Text>/            <Text style={styles.metricUnit}>{metric.unit}</Text>/          </View>/        </TouchableOpacity>/    ))}/;/g/;
    </View>/      ), []);/;,/g/;
const  renderChart = useCallback => {}
    if (!selectedMetricData) {return nu;l;l;}
    performanceMonitor.recordRender();
return (;);
      <View style={styles.chartContainer}>/        <Text style={styles.chartTitle}>{selectedMetricData.title}趋势</Text>/            <LineChart;  />/;,/g/;
data={datasets;: ;[;]{}                data: selectedMetricData.data,;
}
                color: () = /> selectedMetricData.color,/                    strokeWidth: 3;}/;/g/;
              }
];
            ];
          }}
...
const stroke = selectedMetricData.color;}
          ...
          bezier;
style={styles.chart}>/      </View>/        );/;/g/;
  };
  ///;/g/;
    <View style={styles.constitutionContainer}>/      <Text style={styles.sectionTitle}>中医体质分析</Text>/      <View style={styles.constitutionChart}>/            <PieChart;  />/;,/g/;
...
paddingLeft="15";
absolute;
        />/      </View>/      <View style={styles.constitutionLegend}>/            {constitutionData.map(item); => ()}/;/g/;
  ...
  //"/;"/g"/;
    <View style={styles.historyContainer}>/      <View style={styles.historyHeader}>/        <Text style={styles.sectionTitle}>五诊记录</Text>/        <TouchableOpacity onPress={() = accessibilityLabel="五诊记录"  /> Alert.alert("查看全部, "跳转到完整的诊断历史页面")}>/          <Text style={styles.viewAllText}>查看全部</Text>/        </TouchableOpacity>/      </View>/          {diagnosisSessions.slice(0, 3).map(session) => (")"}""/;"/g"/;
        <TouchableOpacity key={session.id} style={styles.historyItem} accessibilityLabel="五诊检查"  />/          <View style={styles.historyIcon}>/            <Icon name="stethoscope" size={20} color={colors.primary}  />/          </View>/          <View style={styles.historyContent}>/            <Text style={styles.historyTitle}>五诊检查</Text>/            <Text style={styles.historyDate}>/                  {new Date(session.startTime).toLocaleDateString()}"/;"/g"/;
            </Text>/          </View>/          <View style={[styles.historyStatus, { backgroundColor: session.status === completed" ? colors.success : colors.warning;}}]}  />/            <Text style={styles.historyStatusText}>/              {session.status === "completed ? "已完成" : 进行中"}""/;"/g"/;
            </Text>/          </View>/        </TouchableOpacity>/    ))}/;/g/;
    </View>/      ), []);/;,/g/;
...
showsVerticalScrollIndicator={false}
      >";"";
        ...
        {///            {selectedMetric !== overview" && renderChart()}""/;"/g"/;
        {///            {renderConstitutionAnalysis()}/;/g/;
        {///            {renderDiagnosisHistory()}/;/g/;
