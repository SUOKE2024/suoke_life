...
export const BenchmarkScreen: React.FC = () => {
  ...
  // 处理任务选择
  const handleTaskSelect = (task: BenchmarkTask) => {if (task.status === 'completed' && task.results) {setSelectedTaskId(task.task_id);
      setShowResultDetail(true);
    }
  };

  // 处理创建任务成功
  const handleTaskCreated = (taskId: string) => {console.log('基准测试任务已创建:', taskId);
...
const styles = StyleSheet.create({
  ...
  fab: {
    ...
    right: 24,
    width: 56,
    height: 56,borderRadius: 28,backgroundColor: '#2196F3',justifyContent: 'center',alignItems: 'center',elevation: 8,shadowColor: '#000',shadowOffset: { width: 0, height: 4 },shadowOpacity: 0.3,shadowRadius: 8;
