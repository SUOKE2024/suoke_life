...
import { MedicalResource } from "../../store/slices/medicalResourceSlice";""/;,"/g"/;
...
returnKeyType="search"";"";
        />"/;"/g"/;
        {searchText.length > 0  && <TouchableOpacity;"}""  />/;,"/g"/;
...
const renderSearchHistory = () => {if (!showSearchHistory || searchHistory.length === 0) return null;}}
    return (;)}
      <View style={styles.historyContainer}>;
        <Text style={styles.historyTitle}>搜索历史</Text>;/;/g/;
...
onPress={() => {setSearchText(item);,}setShowSearchHistory(false);
          ...
          >';'';
            ...
            <Text style={styles.historyText}>{item}</Text>/;/g/;
          </TouchableOpacity>/;/g/;
        ))}
      </View>/;/g/;
    );
    ...
    >;
      <View style={styles.resourceHeader}>;
        <View style={styles.resourceInfo}>;
          <Text style={styles.resourceName}>{item.name}</Text>/;/g/;
          <Text style={styles.resourceType}>{getTypeLabel(item.type)}</Text>/;/g/;
        </View>"/;"/g"/;
        <View style={styles.ratingContainer}>";"";
          <Icon name="star" size={16} color="#FFD700"  />"/;"/g"/;
          <Text style={styles.rating}>{item.rating.toFixed(1)}</Text>/;/g/;
        </View>/;/g/;
      </View>/;/g/;
      <Text style={styles.resourceAddress}>{item.location.address}</Text>/;/g/;
      <View style={styles.resourceServices}>;
        ...
        {item.services.length > 3  && <Text style={styles.moreServices}>+{item.services.length - 3}</Text>/;/g/;
        )}
      </View>/;/g/;
...
styles.availabilityText,{ color: item.availability.isOpen ? '#4CAF50' : '#FF9800' ;}};';'';
        ...
        {item.consultationFee && (;)}
          <Text style={styles.priceText}>¥{item.consultationFee}</Text>;/;/g/;
        )};
      </View>;/;/g/;
    </TouchableOpacity>;/;/g/;
  );
...
onPress={() => handleResourcePress(item)};
    >;
      <Text style={styles.nearbyName}>{item.name}</Text>;/;/g/;
      <Text style={styles.nearbyType}>{getTypeLabel(item.type)}</Text>;"/;"/g"/;
      <View style={styles.nearbyRating}>;";"";
        <Icon name="star" size={12} color="#FFD700"  />;"/;"/g"/;
        <Text style={styles.nearbyRatingText}>{item.rating.toFixed(1)}</Text>;/;/g/;
      </View>;/;/g/;
    </TouchableOpacity>;/;/g/;
  );";"";
  // 渲染搜索结果"/;,"/g"/;
const renderSearchResults = () => {if (loading.search) {return <LoadingSpinner message="搜索中..."  />;"}""/;"/g"/;
    }
    ...
    if (!hasResults) {}}
      return (;)}";"";
        <View style={styles.emptyContainer}>;";"";
          ...
          <Text style={styles.emptyText}>暂无搜索结果</Text>;/;/g/;
          <Text style={styles.emptySubtext}>尝试调整搜索关键词或筛选条件</Text>;/;/g/;
        </View>;/;/g/;
      );
    }
    return (;);
...
ListFooterComponent={pagination.hasMore ? <LoadingSpinner size="small"  /> : null;"}""/;"/g"/;
        };
    ...
    return (;)}";"";
      <View style={styles.healthWarning}>;";"";
        ...
        <TouchableOpacity onPress={healthCheck}>;
          <Text style={styles.retryText}>重试</Text>;/;/g/;
        </TouchableOpacity>;/;/g/;
      </View>;/;/g/;
    );
  };
...
refreshControl={<RefreshControl refreshing={isLoading} onRefresh={handleRefresh}  />;/;/g/;
        };
      >;
