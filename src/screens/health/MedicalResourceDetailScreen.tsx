...
const MedicalResourceDetailScreen: React.FC<MedicalResourceDetailScreenProps> = ({
  ...
  // 预约时间段
  const handleBookTimeSlot = useCallback((timeSlot: TimeSlot) => {
  ...
  // 渲染图片轮播
  const renderImages = () => {
    if (!selectedResource.images || selectedResource.images.length === 0) {
      return null;
    }
  ...
  const renderTimeSlots = () => {
    if (!selectedResource.availability.slots || selectedResource.availability.slots.length === 0) {
      return null;
    }
  ...
  const renderDescription = () => {
    if (!selectedResource.description) return null;
  ...
  // 渲染评价
  const renderReviews = () => {
    if (!selectedResource.reviews || selectedResource.reviews.length === 0) {
      return null;
    }
