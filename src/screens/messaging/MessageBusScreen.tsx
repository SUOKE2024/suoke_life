...
import {import { Topic } from "../../services/messageBusService";""/;,"/g"/;
...
if (!selectedTopic) {}}
          return (;)}
            <View style={styles.noTopicContainer}>;
              <Text style={styles.noTopicText}>请先选择一个主题</Text>;/;/g/;
              <TouchableOpacity;'  />/;,'/g'/;
style={styles.selectTopicButton};';,'';
onPress={() => setActiveTab('topics')};';'';
              >;
                <Text style={styles.selectTopicButtonText}>选择主题</Text>;/;/g/;
              </TouchableOpacity>;/;/g/;
            </View>;/;/g/;
          );
        }
...
return (<SafeAreaView style={styles.container}>';)      <StatusBar barStyle="dark-content" backgroundColor="#fff"  />"/;"/g"/;
      ...
      <View style={styles.header}>;
        <Text style={styles.headerTitle}>消息总线</Text>/;/g/;
        {selectedTopic  && <View style={styles.selectedTopicContainer}>;
            <Text style={styles.selectedTopicLabel}>当前主题:</Text>)/;/g/;
            <Text style={styles.selectedTopicName}>{selectedTopic.name}</Text>)/;/g/;
          </View>)/;/g/;
        )}
      </View>/;/g/;
      {// 标签页导航}/;/g/;
