...
const effectEnd = performance.now();
  ...
  // 弹簧反弹动画
const  demoSpringBounce = async () => {if (!enableAnimations) returnsetAnimationCount(prev => prev + 1);
if (enableHaptics) {';}}
...
await: animationManager.springBounce(scaleAnim, 1);
  ...
  // 弹性缩放动画
const  demoElasticScale = async () => {if (!enableAnimations) returnsetAnimationCount(prev => prev + 1);
if (enableHaptics) {';}}
...
await: animationManager.elasticScale(scaleAnim, 1.1, 1);
  ...
  // 涟漪效果
const  demoRippleEffect = async () => {if (!enableAnimations) returnsetAnimationCount(prev => prev + 1);
if (enableHaptics) {';}}
      const await = interactionEnhancer.triggerFeedback('button_press');'}
    }
  ...
  // 旋转动画
const  demoRotateAnimation = useCallback(() => {if (!enableAnimations) returnsetAnimationCount(prev => prev + 1);
Animated.timing(rotateAnim, {)      toValue: 1,)duration: 1000,);
  ...
  // 滑动动画
const  demoSlideAnimation = useCallback(() => {if (!enableAnimations) returnsetAnimationCount(prev => prev + 1);
...
Animated.timing(slideAnim, {)toValue: 0,)duration: 500,);
  ...
  // 淡入淡出动画
const  demoFadeAnimation = useCallback(() => {if (!enableAnimations) returnsetAnimationCount(prev => prev + 1);
