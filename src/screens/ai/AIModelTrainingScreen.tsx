...
import {
    ...
    TrainingData,
    TrainingStatus,
} from '../../services/ai/modelTrainingService';
      ...
      >
        ...
        <View style={styles.cardHeader}>
          <View style={styles.cardTitleContainer}>
            ...
            <Text style={styles.cardTitle}>{status.modelId.replace('_', ' ').toUpperCase()}</Text>
          </View>
          ...
          <View style={styles.progressSection}>
            <View style={styles.progressInfo}>
              <Text style={styles.progressText}>
                第 {status.currentEpoch} / {status.totalEpochs} 轮
              </Text>
        ...
        <View style={styles.metricsSection}>
          ...
          {performance && (
            <View style={styles.metricRow}>
              <View style={styles.metricItem}>
                <Text style={styles.metricLabel}>推理时间</Text>
                <Text style={styles.metricValue}>
        ...
        <View style={styles.cardActions}>
          {status.status === 'idle' || status.status === 'failed' ? (
            ...
            >
              ...
              <Text style={styles.actionButtonText}>开始训练</Text>
            </TouchableOpacity>
          ) : status.status === 'training' ? (
            <TouchableOpacity
            ...
            >
              ...
              <Text style={styles.actionButtonText}>停止训练</Text>
            </TouchableOpacity>
          ) : null}
...
const AIModelTrainingScreen: React.FC = () => {
  const performanceMonitor = usePerformanceMonitor('AIModelTrainingScreen', {
    trackRender: true;
    trackMemory: true;
    enableLogging: true;
  });
  ...
  const loadData = useCallback(async () => {
    try {
      const statuses = modelTrainingService.getAllTrainingStatus();
  ...
  const handleStartTraining = useCallback(async (modelId: string) => {
    try {
      // 生成模拟训练数据
      const mockData: TrainingData[] = Array.from({ length: 1000 ;}, (_, i) => ({
        id: `${modelId;}_data_${i}`,
      ...
      await modelTrainingService.startTraining(modelId);
  ...
  // 停止训练
  const handleStopTraining = useCallback(async (modelId: string) => {
    try {
      await modelTrainingService.stopTraining(modelId);
  ...
  useEffect(() => {
    ...
    return () => {
      modelTrainingService.off('statusUpdated', handleStatusUpdate);
  ...
  const getSelectedModelDetails = () => {
    ...
    const status = trainingStatuses.find(s => s.modelId === selectedModel);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.header}>
          <Text style={styles.headerTitle}>AI模型训练中心</Text>
        ...
        <View style={styles.statsContainer}>
          ...
          <View style={styles.statCard}>
            ...
            <Text style={styles.statLabel}>可用模型</Text>
          </View>
        </View>
        ...
        >
          {trainingStatuses.map((status) => {
            const performance = modelPerformances.find(p => p.modelId === status.modelId);
            return (
              <TrainingCard
                key={status.modelId}
                status={status}
          ...
          <View style={styles.modalContainer}>
            ...
            <ScrollView style={styles.modalContent}>
              {(() => {
                ...
                return (
                  <View>
                    <Text style={styles.modalSectionTitle}>训练状态</Text>
                    <View style={styles.modalDetailItem}>
                      <Text style={styles.modalDetailLabel}>当前状态:</Text>
                      <Text style={styles.modalDetailValue}>
                        {getStatusText(details.status.status)}
                      </Text>
                    </View>
                    <View style={styles.modalDetailItem}>
                      <Text style={styles.modalDetailLabel}>训练进度:</Text>
                      <Text style={styles.modalDetailValue}>
                        {details.status.progress.toFixed(1)}%
                      </Text>
                    </View>
                    <View style={styles.modalDetailItem}>
                      <Text style={styles.modalDetailLabel}>当前轮次:</Text>
                      <Text style={styles.modalDetailValue}>
                        {details.status.currentEpoch} / {details.status.totalEpochs}
                      </Text>
                    </View>
                    
                    {details.performance && (
                      <>
                        <Text style={styles.modalSectionTitle}>性能指标</Text>
                        <View style={styles.modalDetailItem}>
                          <Text style={styles.modalDetailLabel}>准确率:</Text>
                          <Text style={styles.modalDetailValue}>
                            {(details.performance.accuracy * 100).toFixed(2)}%
                          </Text>
                        </View>
                        <View style={styles.modalDetailItem}>
                          <Text style={styles.modalDetailLabel}>精确率:</Text>
                          <Text style={styles.modalDetailValue}>
                            {(details.performance.precision * 100).toFixed(2)}%
                          </Text>
                        </View>
                        <View style={styles.modalDetailItem}>
                          <Text style={styles.modalDetailLabel}>召回率:</Text>
                          <Text style={styles.modalDetailValue}>
                            {(details.performance.recall * 100).toFixed(2)}%
                          </Text>
                        </View>
                        <View style={styles.modalDetailItem}>
                          <Text style={styles.modalDetailLabel}>F1分数:</Text>
                          <Text style={styles.modalDetailValue}>
                            {details.performance.f1Score.toFixed(3)}
                          </Text>
                        </View>
                        <View style={styles.modalDetailItem}>
                          <Text style={styles.modalDetailLabel}>推理时间:</Text>
                          <Text style={styles.modalDetailValue}>
                            {details.performance.inferenceTime.toFixed(1)}ms
                          </Text>
                        </View>
                        <View style={styles.modalDetailItem}>
                          <Text style={styles.modalDetailLabel}>吞吐量:</Text>
                          <Text style={styles.modalDetailValue}>
                            {details.performance.throughput.toFixed(0)} req/s
                          </Text>
                        </View>
                      </>
                    )}
