...
const AgentManagementScreen: React.FC = () => {;
  ...
  const loadData = async () => {;
    try {;
      // 获取智能体状态 *       const statuses = await coordinationService.getAgentStat;u;s;(;); as AgentStatus[]; */
      setAgentStatuses(statuses);
      // 获取服务状态 *       const status = coordinationService.getServiceStatus;(;); */
      setServiceStatus(status);
      // 这里可以添加获取活跃任务的逻辑 *        *// const tasks = await coordinationService.getActiveTas;k;s;(;);* *        * */// setActiveTasks(tasks)* *     } catch (error) { * *//
