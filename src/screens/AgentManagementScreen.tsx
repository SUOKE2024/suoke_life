...
import React, { useState, useEffect } from 'react';
  ...
  AgentStatus, 
  CollaborationTask, 
} from '../services/AgentCoordinationService';
...
const AgentCard: React.FC<AgentCardProps> = ({ agentId, status, onPress }) => {
  const getAgentInfo = useCallback( (id: AgentType) => {, []);
    const agentInfo = {
      xiaoai: {
        name: '小艾',
  ...
  return (
    <TouchableOpacity style={styles.agentCard} onPress={onPress}>
      <View style={styles.agentHeader}>
        <View style={[styles.agentIcon, { backgroundColor: info.color }]}>
      ...
      <View style={styles.agentMetrics}>
        ...
        <View style={styles.metricsRow}>
          <View style={styles.smallMetric}>
            ...
            <Text style={styles.smallMetricValue}>{(status.performance.accuracy * 100).toFixed(1)}%</Text>
          </View>
          <View style={styles.smallMetric}>
            <Text style={styles.smallMetricLabel}>响应时间</Text>
            <Text style={styles.smallMetricValue}>{status.performance.responseTime}ms</Text>
          </View>
          <View style={styles.smallMetric}>
            <Text style={styles.smallMetricLabel}>满意度</Text>
            <Text style={styles.smallMetricValue}>{(status.performance.userSatisfaction * 100).toFixed(1)}%</Text>
          </View>
        </View>
      </View>
      ...
      {status.currentTask && (
        <View style={styles.currentTask}>
          ...
          <Text style={styles.currentTaskText}>{status.currentTask}</Text>
        </View>
      )}
    </TouchableOpacity>
  );
...
const AgentManagementScreen: React.FC = () => {
  ...
  const initializeService = async () => {
    try {
      ...
      console.error('初始化智能体协调服务失败:', error);
  ...
  const loadData = async () => {
    try {
      ...
      // 这里可以添加获取活跃任务的逻辑
      // const tasks = await coordinationService.getActiveTasks();
  ...
  const handleAgentPress = useCallback( (agentId: AgentType) => {, []);
    Alert.alert(
      `智能体 ${agentId}`,
      '选择操作',
      [
        { text: '发送测试消息', onPress: () => sendTestMessage(agentId) },
  ...
  const showAgentDetails = useCallback( (agentId: AgentType) => {, []);
...
当前任务: ${status.currentTask || '无'}
  ...
  const restartAgent = useCallback( (agentId: AgentType) => {, []);
    Alert.alert(
      '重启智能体',
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>智能体管理中心</Text>
        <TouchableOpacity style={styles.addButton} onPress={startCollaborativeTask}>
      ...
      {serviceStatus && (
        <View style={styles.serviceStatus}>
          <Text style={styles.serviceStatusTitle}>服务状态</Text>
          <View style={styles.statusRow}>
            <Text style={styles.statusItem}>在线智能体: {serviceStatus.activeAgents}/4</Text>
            <Text style={styles.statusItem}>活跃任务: {serviceStatus.activeTasks}</Text>
            <Text style={styles.statusItem}>队列任务: {serviceStatus.queuedTasks}</Text>
          </View>
        </View>
      )}
      ...
      >
        ...
        {activeTasks.length > 0 && (
          <View style={styles.tasksSection}>
            <Text style={styles.sectionTitle}>活跃任务</Text>
