...
';'';
// 导入服务'/;,'/g'/;
import { AdvancedAIModelIntegrationService } from "../../services/ai/AdvancedAIModelIntegrationService";""/;,"/g"/;
import { AdvancedDataAnalyticsService } from "../../services/analytics/AdvancedDataAnalyticsService";""/;,"/g"/;
import { ExpandedTCMKnowledgeService } from "../../services/knowledge/ExpandedTCMKnowledgeService";""/;,"/g"/;
import { UserPersonalizationEngine } from "../../services/personalization/UserPersonalizationEngine";""/;"/g"/;
';,'';
...
// 资源'/;,'/g'/;
interface Resource {';,}type: 'human' | 'technical' | 'financial' | 'data';','';
name: string,;
amount: number,;
unit: string,;
...
// 风险/;,/g/;
interface Risk {id: string,';,}description: string,';,'';
...
const  calculateOverallProgress = useCallback((goals: MediumTermGoal[]) => {totalProgress: goals.reduce((sum, goal) => sum + goal.progress, 0);}}
    setOverallProgress(Math.round(totalProgress / goals.length));}/;/g/;
  };
...
return (<LinearGradient colors={['#4F46E5', '#7C3AED']} style={styles.container}>';)      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>;'';
        ...
        <View style={styles.header}>;
          <Text style={styles.title}>中期目标管理</Text>/;/g/;
          ...
          <View style={styles.overallProgressCard}>;
            <Text style={styles.overallProgressTitle}>总体进度</Text>/;/g/;
...
style={}[;]}
            ...
            <Text style={styles.overallProgressText}>{overallProgress}%</Text>/;/g/;
          </View>/;/g/;
        </View>/;/g/;
);
        {/* 目标列表 */})/;/g/;
...
style={}[;]}
              ...
              <View style={styles.metricsContainer}>;
                <Text style={styles.metricsTitle}>关键指标</Text>/;/g/;
                {goal.keyMetrics.slice(0, 2).map((metric) => (<View key={metric.id} style={styles.metricItem}>;)                    <Text style={styles.metricName}>{metric.name}</Text>/;/g/;
...
color={milestone.completed ? '#10B981' : '#6B7280'}';'';
              ...
              <View style={styles.resourceRiskContainer}>;
                <View style={styles.resourceSection}>;
                  <Text style={styles.sectionTitle}>资源投入</Text>/;/g/;
                  <Text style={styles.resourceSummary}>;

                  </Text>/;/g/;
                </View>/;/g/;
                <View style={styles.riskSection}>;
                  <Text style={styles.sectionTitle}>风险评估</Text>/;/g/;
