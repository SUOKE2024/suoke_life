...
export const FiveDiagnosisAgentIntegrationScreen: React.FC = () => {
  ...
  const startDiagnosisProcess = async () => {
    ...
    try {
      ...
      // 重置诊断步骤状态
      setDiagnosisSteps(prev => prev.map(step => ({ ...step, status: 'pending', data: undefined, agentResponses: undefined })));
  ...
  const performFiveDiagnosisWithAgents = async () => {
    ...
    await performDiagnosisStep('looking', 0, async () => {
      ...
      // 小艾分析望诊结果
      const xiaoaiResponse = await getAgentResponse('xiaoai', 'looking', lookingData);
    ...
    await performDiagnosisStep('listening', 1, async () => {
      ...
      const xiaoaiResponse = await getAgentResponse('xiaoai', 'listening', listeningData);
    ...
    await performDiagnosisStep('inquiry', 2, async () => {
      ...
      const xiaoaiResponse = await getAgentResponse('xiaoai', 'inquiry', inquiryData);
      const laokeResponse = await getAgentResponse('laoke', 'inquiry', inquiryData);
    ...
    await performDiagnosisStep('palpation', 3, async () => {
      ...
      const xiaoaiResponse = await getAgentResponse('xiaoai', 'palpation', palpationData);
      const laokeResponse = await getAgentResponse('laoke', 'palpation', palpationData);
    ...
    await performDiagnosisStep('calculation', 4, async () => {
      // 执行五诊算法分析
      const diagnosisResult = await fiveDiagnosisService.performDiagnosis(diagnosisInput);
      setFinalDiagnosis(diagnosisResult);

      // 启动四大智能体协作分析
      const collaboration = await agentCoordinationService.performCollaborativeDiagnosis(
  ...
  const performDiagnosisStep = async (
    stepId: string, 
    stepIndex: number, 
    stepFunction: () => Promise<{ data: any; agentResponses: Record<string, string> }>
  ) => {
  ...
  const getAgentResponse = async (agentId: AgentType, diagnosisType: string, data: any): Promise<string> => {
    // 模拟智能体分析响应
    await new Promise(resolve => setTimeout(resolve, 1000));
    ...
    const responses = {
      ...
      xiaoke: {
        calculation: '已为患者匹配3位中医专家，推荐适合的有机食材和调理产品',
      },
      laoke: {
      ...
      soer: {
        calculation: '制定个性化健康管理方案：规律作息、适量运动、营养均衡',
      },
    };
...
const renderPatientModal = () => (
  ...
  const renderDiagnosisStep = useCallback( (step: DiagnosisStep, index: number) => {, []);
    ...
    const getStatusText = useCallback( (status: string) => {, []);
      switch (status) {
        case 'collecting': return '数据收集中';
        case 'analyzing': return '智能体分析中';
      ...
      <View key={step.id} style={[
        ...
        <View style={styles.stepHeader}>
          ...
          <View style={[styles.statusBadge, { backgroundColor: getStatusColor(step.status) }]}>
            <Text style={styles.statusText}>{getStatusText(step.status)}</Text>
          </View>
        </View>
        ...
        {(step.status === 'collecting' || step.status === 'analyzing') && (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>
              {step.status === 'collecting' ? '正在收集数据...' : '智能体分析中...'}
            </Text>
          </View>
        )}
        ...
        {step.data && (
          <View style={styles.stepData}>
            <Text style={styles.dataTitle}>收集数据:</Text>
        ...
        {step.agentResponses && (
          <View style={styles.agentResponses}>
            <Text style={styles.responsesTitle}>智能体分析:</Text>
            {Object.entries(step.agentResponses).map(([agentId, response]) => (
              <View key={agentId} style={styles.agentResponse}>
  ...
  const renderFinalResults = useCallback( () => {, []);
    if (!finalDiagnosis || !agentCollaboration) {return null;}

    return (
      <View style={styles.resultsContainer}>
        <Text style={styles.resultsTitle}>诊断结果与智能体协作</Text>
        
        <View style={styles.diagnosisResults}>
          <Text style={styles.sectionTitle}>五诊算法分析结果:</Text>
          <Text style={styles.resultText}>
            证候: {finalDiagnosis.syndrome || '气虚证'}
          </Text>
          <Text style={styles.resultText}>
            体质: {finalDiagnosis.constitution || '气虚质'}
          </Text>
          <Text style={styles.resultText}>
            置信度: {((finalDiagnosis.confidence || 0.85) * 100).toFixed(1)}%
          </Text>
        </View>

        <View style={styles.collaborationResults}>
          <Text style={styles.sectionTitle}>智能体协作建议:</Text>
          {Object.entries(agentCollaboration.responses).map(([agentId, response]) => (
            <View key={agentId} style={styles.collaborationItem}>
              <Text style={styles.agentName}>
                {agentId === 'xiaoai' ? '👩‍⚕️ 小艾' : 
                 agentId === 'xiaoke' ? '👨‍💼 小克' : 
                 agentId === 'laoke' ? '👴 老克' : '🤖 索儿'}:
              </Text>
              <Text style={styles.collaborationText}>{response}</Text>
            </View>
          ))}
        </View>

        <View style={styles.consensusResults}>
          <Text style={styles.sectionTitle}>协作共识:</Text>
          <Text style={styles.consensusText}>
            {agentCollaboration.consensus.summary || '四位专家一致认为患者为气虚证，建议采用补中益气的综合治疗方案'}
          </Text>
        </View>
      </View>
    );
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          <Text style={styles.title}>五诊算法 × 智能体协作</Text>
          <Text style={styles.subtitle}>中医五诊与AI智能体深度集成演示</Text>
        </View>

        {!isInitialized ? (
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color="#2196F3" />
            <Text style={styles.loadingText}>正在初始化服务...</Text>
          </View>
        ) : (
          <>
            <View style={styles.patientInfo}>
              ...
              {patient.name ? (
                <View style={styles.patientCard}>
                  <Text style={styles.patientName}>姓名: {patient.name}</Text>
                  <Text style={styles.patientDetail}>年龄: {patient.age}岁</Text>
                  <Text style={styles.patientDetail}>主诉: {patient.chiefComplaint}</Text>
                </View>
              ) : (
            ...
            <View style={styles.controlPanel}>
              ...
              >
                <Text style={styles.secondaryButtonText}>编辑患者信息</Text>
              </TouchableOpacity>
            </View>
