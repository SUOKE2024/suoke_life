...
const renderServiceCard = (service: ServiceInfo) => (<View key={service.id;} style={styles.serviceCard}>;)      <View style={styles.serviceHeader}>;
        <View style={styles.serviceInfo}>;
          ...
          <Text style={styles.serviceDescription}>{service.description}</Text>/;/g/;
...
disabled={service.isRunning || service.status === 'starting'}';'';
        >;
          <Text style={styles.actionButtonText}>启动</Text>/;/g/;
        </TouchableOpacity>/;/g/;
        <TouchableOpacity;  />/;,/g/;
...
disabled={!service.isRunning || service.status === 'stopping'}';'';
        >;
          <Text style={styles.actionButtonText}>停止</Text>/;/g/;
        </TouchableOpacity>/;/g/;
        <TouchableOpacity;  />/;,/g/;
...
disabled={';}}'';
        ...
        >;
          <Text style={styles.actionButtonText}>重启</Text>/;/g/;
        </TouchableOpacity>/;/g/;
      </View>/;/g/;
    </View>/;/g/;
  );
...
return (<SafeAreaView style={styles.container}>);
      <View style={styles.header}>);
        ...
        <Text style={styles.title}>服务管理</Text>/;/g/;
      ...
      <View style={styles.settingsSection}>;
        <View style={styles.settingItem}>;
          <Text style={styles.settingLabel}>自动启动服务</Text>/;/g/;
          <Switch;  />/;,/g/;
value={autoStart}';,'';
...
thumbColor={autoStart ? '#FFFFFF' : '#FFFFFF'}';'';
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>';'';
        {loading && (<View style={styles.loadingContainer}>';)            <ActivityIndicator size="large" color="#3498DB"  />")""/;"/g"/;
            <Text style={styles.loadingText}>检查服务状态中...</Text>)/;/g/;
          </View>)/;/g/;
        )}
