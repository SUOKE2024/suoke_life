...
return (<View style={styles.loadingContainer}>';)        <ActivityIndicator size="large" color="#007AFF"  />")
  ...
  return (<ScrollView style={styles.container}>;)      {/* 支付方式部分 */}
      <View style={styles.section}>);
        <Text style={styles.sectionTitle}>支付方式</Text>)
        <View style={styles.grid}>);
          {paymentMethods.map((method) => (<TouchableOpacity,)}  />
key={method});
...
onPress={() => handleCreatePayment(method)}
      ...
      <View style={styles.section}>;
        ...
        <View style={styles.grid}>;
          {logisticsProviders.slice(0, 6).map((provider) => (<TouchableOpacity,)}  />
key={provider});
style={styles.methodCard});
onPress={() => handleCreateShipment(provider)}
      ...
      <View style={styles.section}>;
        ...
        {recentShipments.map((shipment, index) => (<TouchableOpacity,)}  />
key={index});
