import EnvironmentManager from "../config/EnvironmentManager";/import MonitoringService from "../../services/monitoring/    MonitoringService";""/;,"/g"/;
export interface MonitoringStack {;}  // Prometheus配置  prometheus: PrometheusConfig;/;/g/;
  // Grafana配置  grafana: GrafanaConfig;/;/g/;
  // Jaeger配置  jaeger: JaegerConfig;/;/g/;
  // AlertManager配置  alertManager: AlertManagerConfig;/;/g/;
}
}
  // Node Exporter配置  nodeExporter: NodeExporterConfig;}/;/g/;
}
export interface PrometheusConfig {;}  // 是否启用  enabled: boolean;/;/g/;
  // 服务地址  endpoint: string;/;/g/;
  // 抓取间隔  scrapeInterval: string;/;/g/;
  // 数据保留时间  retention: string;/;/g/;
  // 存储配置  storage: {path: string,/;}}/g/;
}
  const size = string;}
}
  // 抓取目标  scrapeConfigs: ScrapeConfig[]/;/g/;
;}
export interface ScrapeConfig {;}  // 任务名称  jobName: string;/;/g/;
  // 抓取间隔  scrapeInterval: string;/;/g/;
  // 目标地址  staticConfigs: Array<{targets: string[];/;}}/g/;
}
    labels?: Record<string; string>;}
}>;
  // 指标路径  metricsPath: string;}/;,/g/;
export interface GrafanaConfig {;}  // 是否启用  enabled: boolean;/;/g/;
  // 服务地址  endpoint: string;/;/g/;
  // 管理员用户  adminUser: string;/;/g/;
  // 管理员密码  adminPassword: string;/;/g/;
  // 数据源配置  datasources: GrafanaDatasource[];/;/g/;
}
}
  // 仪表板配置  dashboards: GrafanaDashboard[];}/;/g/;
}
export interface GrafanaDatasource {";}  // 数据源名称  name: string;"/;"/g"/;
  ...
  // 访问模式  access: "proxy" | "direct";"}""/;"/g"/;
}
export interface GrafanaDashboard {;}  // 仪表板ID  id: string;/;/g/;
  // 仪表板标题  title: string;/;/g/;
  // 仪表板描述  description: string;/;/g/;
  // 仪表板JSON配置  json: unknown;/;/g/;
}
}
  // 文件夹  folder: string;}/;/g/;
}
export interface JaegerConfig {;}  // 是否启用  enabled: boolean;/;/g/;
  // 收集器端点  collectorEndpoint: string;/;/g/;
...
export interface AlertManagerConfig {;}  // 是否启用  enabled: boolean;/;/g/;
  // 服务端点  endpoint: string;/;/g/;
  // 告警路由  routes: AlertRoute[];/;/g/;
}
}
  // 接收器  receivers: AlertReceiver[];}/;/g/;
}
export interface AlertRoute {;}  // 匹配条件  match: Record<string, string>;/;/g/;
  // 接收器名称  receiver: string;/;/g/;
  // 分组等待时间  groupWait: string;/;/g/;
  // 分组间隔  groupInterval: string;/;/g/;
}
}
  // 重复间隔  repeatInterval: string;}/;/g/;
}
export interface AlertReceiver {;}  // 接收器名称  name: string;/;/g/;
  ///;/g/;
}
}
...
export interface NodeExporterConfig {;}  // 是否启用  enabled: boolean;/;/g/;
  // 监听端口  port: number;/;/g/;
}
}
  // 收集器  collectors: string[];}/;/g/;
}
export interface DeploymentStatus {";}  // 组件名称  component: string;"/;"/g"/;
  ...
  // 端点URL  endpoint?: string;}/;/g/;
}
export class MonitoringDeployment {private static instance: MonitoringDeployment;,}private envManager: EnvironmentManager;
...
retention: "30d";",";
storage: {,";}}"";
  path: "/prometheus/data",/              size: "10GB"}""/;"/g"/;
        ;}
...
webhookConfigs: [;]{,";}}"";
  const url = config.monitoring.alerting.webhookUrl ||"http: ///     sendResolved: true;")}""/;"/g"/;
        });
];
      ]);
    });
...
const throw = error;
  ...
  // 更新部署状态  private updateDeploymentStatus(component: string,)/;,/g/;
