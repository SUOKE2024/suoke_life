...
export class EnvironmentManager {
   ...
   * 获取环境特定配置
   */
  private getEnvironmentSpecificConfig(env: string): any {
   ...
   * 合并配置
   */
  private mergeConfigs(base: any, env: any): EnvironmentConfig {
    return this.deepMerge(base, env) as EnvironmentConfig;
  }
  ...
  private validateConfiguration(): void {
    ...
    // 生产环境特殊验证
    if (this.currentConfig.type === "production") {
   ...
   * 获取服务配置
   */
  getServiceConfig(serviceName: string): ServiceEndpoint | undefined {
    const services = this.currentConfig.services;
   ...
   * 添加配置监听器
   */
  addConfigWatcher(watcher: (config: EnvironmentConfig) => void): void {
   ...
   * 移除配置监听器
   */
  removeConfigWatcher(watcher: (config: EnvironmentConfig) => void): void {
