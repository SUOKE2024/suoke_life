...
class PaymentService {
  ...
  private activeOrders: Map<string, PaymentOrder> = new Map();
  ...
  async createPaymentOrder(
    orderData: Omit<PaymentOrder, "id" | "status" | "createdAt" | "updatedAt">
    ...
    try {
      ...
      const order: PaymentOrder = {
        ...
        expiresAt:
          orderData.expiresAt ||
          new Date(Date.now() + 30 * 60 * 1000).toISOString(), // 默认30分钟过期
      };
  ...
  async initiatePayment(
    ...
    try {
      ...
      switch (provider) {
        case "alipay":
          return await this.initiateAlipayPayment(order, additionalData);
        case "wechat":
          return await this.initiateWechatPayment(order, additionalData);
        case "stripe":
          return await this.initiateStripePayment(order, additionalData);
        case "paypal":
          return await this.initiatePaypalPayment(order, additionalData);
        case "apple_pay":
          return await this.initiateApplePayPayment(order, additionalData);
        default:
  ...
  async handlePaymentCallback(
    ...
    try {
      switch (provider) {
        ...
        case "stripe":
          return await this.handleStripeCallback(callbackData);
        case "paypal":
          return await this.handlePaypalCallback(callbackData);
        default:
   ...
   * 申请退款
   */
  async requestRefund(refundRequest: RefundRequest): Promise<{
    success: boolean;
    refundId: string;
    amount: number;
    ...
    try {
      const order = this.activeOrders.get(refundRequest.orderId);
      if (!order) {
        throw new Error("Order not found");
      }
  ...
  > {
    ...
    this.configs.forEach((config, provider) => {
      if (config.supportedCurrencies.includes(currency)) {
        const estimatedFees =
          (amount * config.fees.percentage) / 100 + config.fees.fixed;
        const estimatedTotal = amount + estimatedFees;
  ...
  async validatePaymentSecurity(
    ...
    try {
      const response = await apiClient.post(
        "/api/v1/payments/security/validate",
        {
          orderId,
          ...securityData,
        }
      ...
      console.error("Failed to validate payment security:", error);
      throw error;
    }
  }
   ...
   * 支付提供商特定实现
   */
  private async initiateAlipayPayment(
    order: PaymentOrder,
    additionalData?: any
  ) {
    // 支付宝支付实现
    ...
    return {
      ...
      deepLink: `alipays://platformapi/startapp?appId=${config.apiKey}&orderStr=encoded_payment_string`,
    };
  }
  ...
  private async initiateWechatPayment(
    ...
    return {
      ...
      qrCode: "data:image/png;base64,wechat_qr_code_data",
    };
  }
  ...
  private async initiateStripePayment(
    ...
    // 这里应该调用Stripe API创建PaymentIntent
    return {
  ...
  private async initiatePaypalPayment(
    ...
    // 这里应该调用PayPal API
    return {
      paymentUrl: `https://www.paypal.com/checkoutnow?token=paypal_token`,
      paymentToken: "paypal_payment_token",
    };
  }
   ...
   * 回调处理方法
   */
  private async handleAlipayCallback(
    callbackData: any
  ): Promise<PaymentResult> {
    // 验证支付宝回调签名
    // 更新订单状态
    ...
    return {
      ...
      currency: "CNY",
      status:
        callbackData.trade_status === "TRADE_SUCCESS" ? "completed" : "failed",
      provider: "alipay",
  ...
  private async handleWechatCallback(
    ...
    return {
      ...
      amount: callbackData.total_fee / 100, // 微信支付金额单位是分
      currency: "CNY",
  ...
  private async handleStripeCallback(
    callbackData: any
  ): Promise<PaymentResult> {
    // Stripe回调处理
    return {
      success: callbackData.status === "succeeded",
      orderId: callbackData.metadata.orderId,
      transactionId: callbackData.id,
      amount: callbackData.amount / 100, // Stripe金额单位是分
      currency: callbackData.currency.toUpperCase(),
      status: callbackData.status === "succeeded" ? "completed" : "failed",
      provider: "stripe",
  ...
  private async handlePaypalCallback(
    callbackData: any
  ): Promise<PaymentResult> {
    // PayPal回调处理
    return {
      success: callbackData.payment_status === "Completed",
      orderId: callbackData.custom,
      transactionId: callbackData.txn_id,
      amount: parseFloat(callbackData.mc_gross),
      currency: callbackData.mc_currency,
      status:
        callbackData.payment_status === "Completed" ? "completed" : "failed",
      provider: "paypal",
      paymentMethod: "digital_wallet",
      timestamp: new Date().toISOString(),
    };
