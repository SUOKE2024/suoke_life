...
export class AgentApiService {
   ...
   * 索儿情感支持API
   */
  public async soerEmotionalSupport(request: ServiceRequest): Promise<ApiResponse<any>> {
    const endpoint = `${AGENT_CONFIGS[AgentType.SOER].apiEndpoint}/emotional-support`;
    return this.makeRequest(endpoint, {
      method: 'POST',
      body: JSON.stringify({
        ...request,
   ...
   * 索儿个性化养生计划API
   */
  public async soerWellnessPlan(request: ServiceRequest): Promise<ApiResponse<any>> {
    const endpoint = `${AGENT_CONFIGS[AgentType.SOER].apiEndpoint}/wellness-plan`;
    return this.makeRequest(endpoint, {
      method: 'POST',
      body: JSON.stringify({
        ...request,
   ...
   * 智能体协作API - 多智能体协同工作
   */
  public async agentCollaboration(request: {
    ...
    const primaryConfig = AGENT_CONFIGS[request.primaryAgent];
    const endpoint = `${primaryConfig.apiEndpoint}/collaboration`;
    
    return this.makeRequest(endpoint, {
      method: 'POST',
      body: JSON.stringify({
        ...request,
        collaborationMode: 'distributed_autonomous_collaboration'
      })
    });
  }
  ...
  public async getAgentStatus(agentType: AgentType): Promise<ApiResponse<any>> {
    ...
    const endpoint = `${config.apiEndpoint}/status`;
   ...
   * 获取智能体健康状态
   */
  public async getAgentHealth(agentType: AgentType): Promise<ApiResponse<any>> {
    const config = AGENT_CONFIGS[agentType];
    const endpoint = `${config.apiEndpoint}/health`;
  ...
  public async batchRequest(requests: Array<{
    ...
    try {
      const promises = requests.map(async (req) => {
        const config = AGENT_CONFIGS[req.agentType];
