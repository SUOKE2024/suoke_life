...
this.isOnline = window.navigator.onLine;
  ...
  // 设置在线状态
private setOnlineStatus(isOnline: boolean): void {const wasOffline = !this.isOnlinethis.isOnline = isOnline;
...
if (now - cacheTime > cacheItem.ttl) {this.cache.delete(key)this.saveCacheData(); // 异步保存
}
      return null}
    }
...
pendingOperations: this.operationQueue.length,
  ...
  // 添加同步状态监听器
addSyncListener(listener: (status: SyncStatus) => void): () => void {this.syncListeners.push(listener}        // 返回取消监听的函数
...
return {const totalItems = this.cache.sizetotalSize,
}
