...
class UserService {
  ...
  // 注册新设备
  async registerDevice(deviceInfo: Omit<DeviceInfo, 'id' | 'lastActive' | 'isActive'>): Promise<DeviceInfo> {
    try {
  ...
  async getHealthRecords(
    ...
    try {
      ...
      const response: ApiResponse<{ records: HealthRecord[]; total: number }> = 
        await apiClient.get(`/users/me/health-records?${params.toString()}`);
  ...
  // 添加健康记录
  async addHealthRecord(record: Omit<HealthRecord, 'id' | 'timestamp' | 'verified'>): Promise<HealthRecord> {
    try {
  ...
  async getActivityLog(
    ...
    try {
      ...
      const response: ApiResponse<{ activities: any[]; total: number }> = 
        await apiClient.get(`/users/me/activity-log?${params.toString()}`);
