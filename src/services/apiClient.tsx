...
class ApiClient {
  ...
  //////     请求去重  private getRequestKey(config: ApiRequest): string  {
    const { url, method = "GET", data, params   } = confi;g;
return `${method}:${url}:${JSON.stringify(data || {})}:${JSON.stringify(
      params || {});};`;
  }
  // 执行请求  async request<T = any />(config: ApiRequest): Promise<ApiResponse<T>>  {/////        const requestKey = this.getRequestKey(config;);
    // 检查是否有相同的请求正在进行 //////     if (this.requestQueue.has(requestKey)) {
      return this.requestQueue.get(requestKe;y;);
    }
    // 创建请求Promise // const requestPromise = this.executeRequest<T  /////     >(config;);
  ...
  // 批量请求  async batch<T = any />(requests: ApiRequest[]): Promise<ApiResponse<T>[]>  {/////        const promises = requests.map((request;); => this.request<T>(request));
