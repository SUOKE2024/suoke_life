...
n";  * / Practice Fusion医疗系统* ///     ""/;"/g"/;
// 医疗数据类型 * export interface MedicalRecord {/;,}id: string,;,/g,/;
  patientId: string,";,"";
providerId: string,";,"";
...
metadata?: {clinician?: string;,}facility?: string;
department?: string;
}
}
    confidence?: number;}
...
// 患者信息 * export interface PatientInfo {/;,}id: string,";,"/g,"/;
  name: string,";,"";
...
// 医疗预约信息 * export interface MedicalAppointment {/;,}id: string,;,/g,/;
  patientId: string,;
providerId: string,";,"";
clinicianId: string,";,"";
...
notes?: string;
telehealth?: boolean}
// 处方信息 * export interface Prescription {/;,}id: string,;,/g,/;
  patientId: string,clinicianId: string,medication: {name: string;,}genericName?: string;
dosage: string,;
frequency: string,;
duration: string,;
}
...
// 实验室结果 * export interface LabResult {/;,}id: string,;,/g,/;
  patientId: string,;
testName: string,testCode: string,result: {value: string | number;,}unit?: string;";,"";
...
notes?: string}
// API配置 * interface ApiConfig {/;,}baseUrl: string,;,/g,/;
  apiKey: string,;
version: string,;
timeout: number,;
...
///;,/g/;
private configs: Map<MedicalApiProvider, ApiConfig  /> = new Map()/  private rateLimiters: Map<MedicalApiProvider, any  /> = new Map();//;,/g/;
...
timeout: 30000,);
  ...
  // 设置速率限制器  private setupRateLimiters(): void {}/;,/g/;
...
try {const controller = new AbortController;,}timeoutId: setTimeout(); => controller.abort(), config.timeout);";,"";
...
if (!response.ok)  {}}
  ...
  // 获取患者信息  async getPatientInfo(provider: MedicalApiProvider,)/;,/g/;
...
return this.transformPatientData(response, provide;r;);
  ...
  // 获取医疗记录  async getMedicalRecords(provider: MedicalApiProvider,)/;,/g/;
const patientId = string;
recordType?: string;
dateRange?: { start: string; end: string;}): Promise<MedicalRecord[] /    >  {/;}}/g/;
    try {}";,"";
let endpoint = `/Patient/${patientId} ;``"/`;,`/g`/`;
switch (recordType) {case "diagnosis": endpoint += "Condition";";,}break;";,"";
case "prescription": ";,"";
...
return this.transformMedicalRecords(response, provide;r;);
  ...
  // 获取预约信息  async getAppointments(provider: MedicalApiProvider,)/;,/g/;
const patientId = string;
status?: string;
...
fhirAppointm;e;n;t;);
  ...
  // 获取处方信息  async getPrescriptions(provider: MedicalApiProvider,)/;,/g/;
const patientId = string;
status?: string;
...
return this.transformPrescriptions(response, provide;r;);
  ...
  // 获取实验室结果  async getLabResults(provider: MedicalApiProvider,)/;,/g/;
const patientId = string;
testType?: string;
  ): Promise<LabResult[] /    >  {/;}}/g/;
    try {}
...
return resul;t;s;
  ...
  // 数据转换方法  private transformPatientData(data: unknown,)/;,/g/;
...
patientId: entry.resource.participant;?.find(p: unknown) => p.actor?.reference?.includes("Patient"))";"";
  ...
  clinicianId: entry.resource.participant;?.find(p: unknown) => p.actor?.reference?.includes("Practitioner"))";"";
          ?.actor?.reference?.split("/")[1] || ",/          appointmentType: this.mapAppointmentType(")"/;,"/g"/;
...
clinicianId: entry.resource.performer?.[0]?.reference?.split("/")[1] || ",/      labFacility: entry.resource.performer?.[0]?.display || ""/;"/g"/;
    ;}))";"";
  }";,"";
