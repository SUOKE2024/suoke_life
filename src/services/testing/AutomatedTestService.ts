...
export class AutomatedTestService {
  ...
  async runTestSuite(suiteId: string): Promise<TestRun> {
    ...
    try {
      ...
      if (suite.config.parallel) {
        ...
        await this.runTestsSequentially(suite, testRun);
      }
  ...
  private async runTestsInParallel(
    ...
    const { testCases, config } = suite;
    const concurrency = Math.min(config.maxConcurrency, testCases.length);
  ...
  private async runTestsSequentially(
    ...
    const sortedTests = this.sortTestsByDependencies(suite.testCases);

    for (const testCase of sortedTests) {
      await this.runSingleTest(testCase, testRun);
    }
  }
   ...
   * 按依赖关系排序测试用例
   */
  private sortTestsByDependencies(testCases: TestCase[]): TestCase[] {
