import {BPartnerType,} fromonst BPartnerService = as IBPartnerService};
} from "../../types/business"
...
const preferredTime = Date);
  ): {const success = booleanbookingId?: string;
}
...
return {const success = truebookingId,
}
}
    };
  }
...
if (firstSlot) {'const [hours, minutes] = firstSlot.start.split(':').map(Number);
  ...
  // 计算服务价格
private calculateServicePrice(service: IBPartnerService, userId: string): number {let price = service.pricing.basePrice;}    // 根据用户历史使用量计算折扣
