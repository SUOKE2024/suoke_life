...
export class SubscriptionService {
    ...
    ;},
    {
      ...
      features: [
        ...
        ;},
        {
          ...
          description: '24/7健康状态实时监控';
          category: 'health';
          enabled: true
        ;}
      ],
      limits: {
        aiConsultations: -1, // 无限制
  ...
  calculatePersonalizedPricing(
    ...
    const adjustments = this.calculatePriceAdjustments(factors, userProfile);
  ...
  // 分析定价因子
  private analyzePricingFactors(userProfile: any): PricingFactor[] {
    const factors: PricingFactor[] = [];

    // 使用历史分析
    if (userProfile.usageHistory) {
      const usageScore = this.calculateUsageScore(userProfile.usageHistory);
      factors.push({
        factor: 'usage_history';
        value: usageScore;
        impact: usageScore > 0.7 ? -0.1 : usageScore < 0.3 ? 0.1 : 0
      ;});
    }

    // 健康风险评估
    if (userProfile.healthRisk) {
      factors.push({
        factor: 'health_risk';
        value: userProfile.healthRisk;
        impact: userProfile.healthRisk > 0.6 ? 0.15 : -0.05
      ;});
    }

    // 用户参与度
    if (userProfile.engagement) {
      factors.push({
        factor: 'engagement';
        value: userProfile.engagement;
        impact: userProfile.engagement > 0.8 ? -0.15 : 0
      ;});
    }

    // 地理位置
    if (userProfile.location) {
      const locationImpact = this.getLocationPricingImpact(userProfile.location);
      factors.push({
        factor: 'location';
        value: locationImpact.value;
        impact: locationImpact.impact
      ;});
    }

    // 年龄因子
    if (userProfile.age) {
      const ageImpact = this.getAgePricingImpact(userProfile.age);
      factors.push({
        factor: 'age';
  ...
  private calculatePriceAdjustments(
    ...
    factors.forEach(factor => {
      if (Math.abs(factor.impact) > 0.05) {
        const adjustmentValue = Math.abs(factor.impact * 50); // 最大调整50元
        adjustments.push({
          type: factor.impact > 0 ? 'premium' : 'discount';
          value: adjustmentValue;
          reason: this.getAdjustmentReason(factor);
          weight: Math.abs(factor.impact)
        ;});
      }
    });

    // 忠诚度折扣
    if (userProfile.loyaltyLevel && userProfile.loyaltyLevel > 0.5) {
      adjustments.push({
        type: 'loyalty';
        value: userProfile.loyaltyLevel * 20;
  ...
  // 计算使用评分
  private calculateUsageScore(usageHistory: any): number {
    // 基于历史使用数据计算评分
    const totalSessions = usageHistory.totalSessions || 0;
  ...
  // 获取年龄定价影响
  private getAgePricingImpact(age: number): number {
    if (age < 25) return -0.1; // 年轻用户折扣
    if (age > 60) return 0.1;  // 老年用户可能需要更多服务
    return 0;
  }
  ...
  // 升级订阅
  upgradeSubscription(userId: string, currentTier: string, targetTier: string): {
    success: boolean;
    message: string;
  ...
  checkFeatureAccess(userId: string, featureId: string): {
    ...
    if (feature.usage) {
      const currentUsage = this.getCurrentUsage(userId, featureId);
      const remainingUsage = feature.usage.limit - currentUsage;
  ...
  // 获取用户订阅信息（示例）
  private getUserSubscription(userId: string): SubscriptionTier {
    // 这里应该从数据库获取
    return this.subscriptionTiers[1]; // 默认返回高级版
  }

  // 获取当前使用量（示例）
  private getCurrentUsage(userId: string, featureId: string): number {
    // 这里应该从数据库获取
    return 0;
  }
} 