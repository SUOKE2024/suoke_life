...
const query = string;
type?: TCMKnowledgeType[];
filters?: {category?: stringdifficulty?: string;
clinicalRelevance?: number;
}
...
type: entity.type,);
  ...
  /* 系 */
   */
const public = addRelationship(relationship: TCMRelationship): void {this.relationships.set(relationship.id, relationship}    // 更新实体的关系列表
...
const throw = error;
  ...
  /* 理 */
   *//,/g,/;
  public: async diagnosisReasoning(symptoms: string[], context?: any): Promise<TCMDiagnosisReasoning> {try {}      // 症状标准化
...
return results.map(r => r.entity);
  ...
  private calculateMatchScore(query: string, entity: TCMKnowledgeEntity): number {let score = 0;}    // 名称匹配
...
if (propertiesStr.includes(query)) {}
  ...
  private generateSuggestions(query: string): string[] {const suggestions: string[] = [];}    // 基于索引生成建议
