...
input: any,
const output = any;
metadata?: {userId?: stringtimestamp: number,
quality: number; // 0-1,
}
...
for (let epoch = 1; epoch <= config.training.epochs; epoch++) {status.currentEpoch = epochstatus.progress = (epoch / config.training.epochs) * 100;
      // 模拟训练指标/,/g,/;
