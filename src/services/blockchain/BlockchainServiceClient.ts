...
export class BlockchainServiceClient {
  ...
  async storeHealthData(request: StoreHealthDataRequest): Promise<StoreHealthDataResponse> {
    try {
      const response = await this.makeRequest('/api/v1/blockchain/health-data', {method: 'POST',body: {user_id: request.userId,data_type: request.dataType,data_hash: Array.from(request.dataHash),encrypted_data: Array.from(request.encryptedData),metadata: request.metadata,timestamp: request.timestamp;
        };
      });

      return {transactionId: response.transaction_id,blockHash: response.block_hash,success: response.success,message: response.message;
      };
    } catch (error) {
  ...
  async verifyHealthData(request: VerifyHealthDataRequest): Promise<VerifyHealthDataResponse> {
    try {
      const response = await this.makeRequest('/api/v1/blockchain/verify', {method: 'POST',body: {transaction_id: request.transactionId,data_hash: Array.from(request.dataHash);
        };
      });

      return {valid: response.valid,message: response.message,verificationTimestamp: response.verification_timestamp;
      };
    } catch (error) {
  ...
  async verifyWithZKP(request: VerifyWithZKPRequest): Promise<VerifyWithZKPResponse> {
    try {
      const response = await this.makeRequest('/api/v1/blockchain/verify-zkp', {method: 'POST',body: {user_id: request.userId,verifier_id: request.verifierId,data_type: request.dataType,proof: Array.from(request.proof),public_inputs: Array.from(request.publicInputs);
        };
      });

      return {valid: response.valid,message: response.message,verificationDetails: response.verification_details;
      };
    } catch (error) {
  ...
  async authorizeAccess(request: AuthorizeAccessRequest): Promise<AuthorizeAccessResponse> {
    try {
      const response = await this.makeRequest('/api/v1/blockchain/authorize', {method: 'POST',body: {user_id: request.userId,authorized_id: request.authorizedId,data_types: request.dataTypes,expiration_time: request.expirationTime,access_policies: request.accessPolicies;
        };
      });

      return {authorizationId: response.authorization_id,success: response.success,message: response.message;
      };
    } catch (error) {
  ...
  async revokeAccess(request: RevokeAccessRequest): Promise<RevokeAccessResponse> {
    try {
      const response = await this.makeRequest('/api/v1/blockchain/revoke', {method: 'POST',body: {authorization_id: request.authorizationId,user_id: request.userId,revocation_reason: request.revocationReason;
        };
      });

      return {success: response.success,message: response.message,revocationTimestamp: response.revocation_timestamp;
      };
    } catch (error) {
  ...
  async generateZKProof(
    ...
    try {
      const response = await this.makeRequest('/api/v1/blockchain/generate-proof', {method: 'POST',body: {user_id: userId,data_type: dataType,private_inputs: privateInputs,circuit_type: circuitType;
        };
      });

      return {proof: new Uint8Array(response.proof),publicInputs: new Uint8Array(response.public_inputs),verificationKey: response.verification_key,circuitType: response.circuit_type;
      };
    } catch (error) {
  ...
  async batchStoreHealthData(requests: StoreHealthDataRequest[]): Promise<StoreHealthDataResponse[]> {
    try {
      const response = await this.makeRequest('/api/v1/blockchain/batch-store', {method: 'POST',body: {requests: requests.map(req => ({user_id: req.userId,data_type: req.dataType,data_hash: Array.from(req.dataHash),encrypted_data: Array.from(req.encryptedData),metadata: req.metadata,timestamp: req.timestamp;
          }));
        };
      });
   ...
   * 将HTTP状态码映射到错误码
   */
  private mapHttpStatusToErrorCode(status: number): BlockchainErrorCode {
    switch (status) {
      case 400:
        return BlockchainErrorCode.INVALID_REQUEST;
      case 401:
      case 403:
        return BlockchainErrorCode.PERMISSION_DENIED;
      case 404:
        return BlockchainErrorCode.DATA_NOT_FOUND;
      case 500:
      case 502:
      case 503:
        return BlockchainErrorCode.BLOCKCHAIN_ERROR;
      default:
  ...
  private isRetryableError(error: any): boolean {
    if (error instanceof BlockchainError) {
      return error.code === BlockchainErrorCode.NETWORK_ERROR ||;
             error.code === BlockchainErrorCode.BLOCKCHAIN_ERROR;
    }
