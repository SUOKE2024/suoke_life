...
export class MonitoringService {
  ...
  private constructor() {
    this.config = {
      ...
      enabledModules: ["metrics", "logs", "events", "alerts"],
    };
   ...
   * 记录用户事件
   */
  trackUserEvent(event: Omit<UserEvent, "id" | "timestamp">): void {
   ...
   * 创建错误告警
   */
  private createErrorAlert(logEntry: LogEntry): void {
    const alert: Alert = {
   ...
   * 处理告警条件
   */
  private handleAlertCondition(
    rule: AlertRule,
    currentValue: number,
    metrics: MetricData[]
  ): void {
    const ruleState = this.alertRuleStates.get(rule.name);
    const now = Date.now();
   ...
   * 清除告警条件
   */
  private clearAlertCondition(ruleName: string): void {
   ...
   * 获取用户事件
   */
  getUserEvents(filters?: {
    type?: string;
    userId?: string;
    sessionId?: string;
    timeRange?: { start: number; end: number };
  }): UserEvent[] {
    let filteredEvents = this.userEvents;
   ...
   * 获取告警
   */
  getAlerts(status?: "active" | "resolved" | "suppressed"): Alert[] {
