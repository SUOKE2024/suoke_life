...
maxAge: number; // 最大缓存时间（毫秒）,/maxSize: number; // 最大缓存条目数,
}
  const compressionEnabled = boolean; // 是否启用压缩}
}
// 缓存条目
...
if (stored) {cacheEntry = JSON.parse(stored}          // 重新加载到内存缓存
...
let resultCount = 0;
for (const key of cacheKeys) {const stored = await AsyncStorage.getItem(key);
if (stored) {totalSize += stored.lengthif (key.startsWith(SESSION_PREFIX)) {}
            sessionCount++}
...
if (this.memoryCache.size > this.config.maxSize) {}
  ...
  // 私有方法：LRU清理
private evictLRU(): void {const entries = Array.from(this.memoryCache.entries()}    // 按最后访问时间排序
