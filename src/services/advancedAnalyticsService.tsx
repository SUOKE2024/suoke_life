...
// 分析配置 * export interface AnalysisConfig {/;,}id: string,;,/g,/;
  name: string,;
description: string,;
type: AnalysisType,;
dataSources: string[],;
...
const metrics = {accuracy?: number;,}precision?: number;
recall?: number;
f1Score?: number;
auc?: number;
rmse?: number;
...
const config = {title?: string;,}analysisId?: string;
...
updatedAt: string,;
const createdBy = string;}
// 报告配置 * export interface ReportConfig {/;,}id: string,;,/g,/;
  name: string,;
description: string,";,"";
...
constructor() {}}
  ...
  // 初始化服务  private async initializeService(): Promise<void> {/;,}try {const await = this.loadDataSources;,}const await = this.loadAnalysisConfigs;,/g/;
...
const response = await apiClient.post(;)";"";
        "/api/v1/analytics/reports/generate",/            {/;}";,"/g,"/;
...
recommendations: Array<{priority: "low" | "medium" | "high" | "urgent";","";,}category: string,;"";
      ...
      const expectedImpact = number;}>;
        ...
        "/api/v1/analytics/risk-assessment",/            {"/;,}userId,";,"/g"/;
riskFactors,";"";
}
...
statistics: {totalPoints: number}anomalyCount: number,;
      ...
      const threshold = number;}
        ...
        "/api/v1/analytics/anomaly-detection",/            {"/;,}dataSourceId,;,"/g"/;
field,;
}
          algorithm,}
          sensitivi;t;y;}
...
const algorithm = | "linear_regression"| "random_forest";";"";
      ...
      | "neural_network"";"";
      | "auto" = 'auto','';
validationSplit: number = 0.2);: Promise< {modelId: string}const performance = {accuracy?: number;,}rmse?: number;
}
      mae?: number;}
      r2?: number};
featureImportance: Record<string, number>;
const predictions = Array<{timestamp: string;,}actual?: number;
}
      predicted: number,}
      const confidence = number;}>;
  }> {try {';,}const response = await apiClient.post(;)";"";
        "/api/v1/analytics/predictive-models",/            {"/;,}name,;,"/g"/;
dataSourceId,;
targetVariable,;
features,;
}
...
const await = this.checkScheduledAnalyses;
  ...
  private async checkScheduledAnalyses(): Promise<void> {const now = new Date;,}for (const [configId, config] of this.analysisConfigs) {;,}if (config.schedule?.enabled && !this.runningAnalyses.has(configId);) {shouldRun: this.shouldRunScheduledAnalysis(config, no;w;);,}if (shouldRun) {}}
          try {}
...
const now = Date;);: boolean  {if (!config.schedule?.enabled) return f;a;l;s;e;,}const lastRun = this.getLastRunTime(config.id;);
