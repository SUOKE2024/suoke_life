...
interface ApiResponse<T = any  /> {/;,}data: T;/    , success: boolean;/;/g/;
  ...
  message?: string;}";,"";
code?: number}";,"";
const importAsyncStorage = from "@react-native-async-storage/async-storage";/import { EventEmitter } from "../../utils/eventEmitter"; 索克生活 - GraphQL客户端   完整的GraphQL API支持，包含查询、变更、订阅功能"/;"/g"/;
//;"/;,"/g"/;
n"; /    ""/;"/g"/;
// GraphQL变量类型 * export interface GraphQLVariables {/;}/;/;/g/;
}
}
  [key: string]: unknown;}
}
...
/    ;/;/g/;
  ...
  ttl?: number;  key?: string  / 自定义缓存键* // enabled?: boolean  * / 是否启用缓存* *}/;/g/;
} * //;/g/;
...
// 缓存项接口 * interface CacheItem {/;,}data: unknown,;,/g,/;
  timestamp: number,;
ttl: number,;
}
}
...
this.headers = { "Content-Type": "application/json", ...defaultHeaders };/        this.initializeCache();"/;,"/g"/;
...
if (cachedData) {}}
  ...
  // 设置认证令牌  setAuthToken(token: string): void  {"}""/;,"/g"/;
this.headers["Authorization"] = `Bearer ${token;}`````;```;
  ...
  // 设置自定义头部  setHeaders(headers: Record<string, string>): void  {}/;,/g/;
...
const throw = error;
  ...
  // 创建GraphQL订阅  subscribe(subscription: string,)/;,/g/;
variables?: GraphQLVariables;
...
wsEndpoint: this.endpoint.replace(/^http/, "ws;";) + "/graphql"// const ws = new WebSocket(wsEndpoint, "graphql-ws;";);"/;,"/g"/;
ws.onopen = () => {}
      ws.send()";,"";
JSON.stringify({)")"";}}"";
...
this.subscriptions.delete(subscriptionId);
  ...
  // 执行HTTP请求  private async executeRequest<T>(request: GraphQLRequest,)/;,/g/;
config?: RequestConfig;
  ): Promise<GraphQLResponse<T>>  {const timeout = config?.timeout || this.defaultTimeo;u;t;,}const retries = config?.retries || this.defaultRetri;e;s;
...
if (result.errors && result.errors.length > 0) {const throw = new Error(;);}}
  ...
  // 生成缓存键  private generateCacheKey(request: GraphQLRequest,)/;,/g/;
customKey?: string;
  );: string  {if (customKey) return custo;m;K;e;y;,}const queryHash = this.hashString(request.quer;y;);
...
return `gql_${queryHash}_${variablesHash;};`;````;```;
  ...
  // 生成订阅ID  private generateSubscriptionId(subscription: string,)/;,/g/;
variables?: GraphQLVariables;
