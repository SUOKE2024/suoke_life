...
// 认证令牌管理
class TokenManager {private accessToken: string | null = nullprivate refreshToken: string | null = null;
private tokenExpiry: number | null = null;
setTokens(accessToken: string, refreshToken: string, expiresIn: number) {this.accessToken = accessTokenthis.refreshToken = refreshToken;
...
getRefreshToken(): string | null {'if (!this.refreshToken && typeof localStorage !== 'undefined') {';}}
  ...
  clearTokens() {this.accessToken = nullthis.refreshToken = null;
...
if (data) {}
  ...
  // 准备请求头'
private async prepareHeaders(config: RequestConfig = {;}): Promise<Record<string, string>> {'const: headers: Record<string, string> = {';}}'Content-Type': "application/json",X-Client-Version': "1.0.0",X-Request-ID': this.generateRequestId(),'}''/;'/g'/;
      ...config.headers;};
    // 添加认证头'
...
retries: number = GATEWAY_PERFORMANCE_CONFIG.RETRY_ATTEMPTS;
  ): Promise<T> {const let = lastError: Errorfor (let attempt = 1; attempt <= retries; attempt++) {try {}
        return await operation()}
      } catch (error) {lastError = error as Error;}                // 如果是认证错误，尝试刷新令牌
...
const throw = error;
  ...
  async: post<T = any>(service: string, endpoint: string = ', data?: any; config: RequestConfig = {;}): Promise<ApiResponse<T>> {''url: this.buildUrl(service, endpoint);'';
    // 检查熔断器'
if (GATEWAY_FEATURES.ENABLE_CIRCUIT_BREAKER && !this.circuitBreaker.canExecute()) {// 如果启用离线模式，将操作添加到队列/if (GATEWAY_FEATURES.ENABLE_OFFLINE) {'const await = addOfflineOperation({',)const type = 'CREATEservice,,'/g'/;
endpoint,);
data,)
}
          maxRetries: 3,)'}
...
return result;
    ...
    } catch (error) {// 如果请求失败且启用离线模式，将操作添加到队列/if (GATEWAY_FEATURES.ENABLE_OFFLINE && !config.skipAuth) {'const await = addOfflineOperation({',)const type = 'CREATEservice,,'/g'/;
endpoint,);
data,)
}
          maxRetries: 3,)'}
