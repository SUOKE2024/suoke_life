...
duration,);
    ...
    } catch (error: any) {const duration = Date.now() - startTimethis.testResults.push({)        testName}const passed = false;);
duration,);
}
  ...
  /* 告 */
  */
private generateTestReport(totalDuration: number): IntegrationTestReport {const totalTests = this.testResults.lengthconst passedTests = this.testResults.filter(r => r.passed).length;
