import { FiveDiagnosisEngine } from "../algorithms/FiveDiagnosisEngine";""/;,"/g"/;
import { AlgorithmConfig } from "../algorithms/config/AlgorithmConfig";""/;,"/g"/;
import { usePerformanceMonitor } from "../hooks/usePerformanceMonitor";""/;,"/g"/;
import { apiClient } from "./apiClient";""/;"/g"/;

// 五诊数据接口/;,/g/;
export interface FiveDiagnosisInput {;,}const userId = string;
sessionId?: string;
lookingData?: {tongueImage?: string;,}faceImage?: string;
bodyImage?: string;
}
}
    metadata?: Record<string; any>;}
  };
listeningData?: {voiceRecording?: string;,}breathingPattern?: number[];
coughSound?: string;
}
    metadata?: Record<string; any>;}
  };
...
calculationData?: {const birthDate = string;,}birthTime?: string;
location?: string;
currentTime?: string;
}
    metadata?: Record<string; any>;}
...
const diagnosticResults = {looking?: any;,}listening?: any;
inquiry?: any;
palpation?: any;
}
    calculation?: any;}
...
inquiry: {,";,}baseUrl: "http://localhost:8001";",""/;"/g"/;
}
    const timeout = 30000;}
  },";,"";
...
constructor() {this.config = new AlgorithmConfig();}}
  ...
  // 初始化五诊服务/;,/g/;
const async = initialize(): Promise<void> {try {}      // 等待算法引擎初始化完成/;,/g/;
...
checkReady();
  ...
  // 加载配置/;,/g/;
private async loadConfiguration(): Promise<void> {try {}      // 加载算法配置/;,/g/;
...
sessionId: string,);
  ...
  const diagnosticResults = any;)}
  }): Promise<FiveDiagnosisResult> {try {}      // 使用算法引擎进行综合分析/;,/g/;
...
resultReliability: fusionResult.confidence || 0.8,;
  ...
  // 计算数据质量/;,/g/;
private calculateDataQuality(diagnosticResults: any): number {let totalQuality = 0;,}let count = 0;
';,'';
...
return count > 0 ? totalQuality / count : 0.5;/;/g/;
  }
...
private calculateCompleteness(diagnosticResults: any): number {const totalMethods = 5; // 五诊方法/;,}const completedMethods = Object.keys(diagnosticResults).filter(key => diagnosticResults[key] && !diagnosticResults[key].error;);/g/;
    ...
    return completedMethods / totalMethods;}/;/g/;
  }
...
this.performanceMetrics.responseTimes.reduce(a, b) => a + b, 0) //;,/g/;
this.performanceMetrics.responseTimes.length;
this.performanceMetrics.successRate =;
this.performanceMetrics.successfulSessions / this.performanceMetrics.totalSessions;/;/g/;
  }
...
return {const service = fiveDiagnosisService;,}performanceMonitor,;
