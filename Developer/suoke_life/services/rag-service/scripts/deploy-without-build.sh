...
pull_image() {
    ...
    if [ $? -ne 0 ]; then
        log "镜像拉取失败，检查镜像是否存在于仓库中" "ERROR"
        log "如果镜像不存在，请先构建并推送镜像" "ERROR"
        exit 1
    fi
...
# 更新docker-compose文件中的镜像地址
update_compose_file() {
    log "更新docker-compose.prod.yml中的镜像地址..."
    
    # 创建临时文件
    execute "cp docker-compose.prod.yml docker-compose.prod.yml.bak"
    
    # 使用sed更新镜像地址
    # macOS的sed命令需要使用''作为空字符串
