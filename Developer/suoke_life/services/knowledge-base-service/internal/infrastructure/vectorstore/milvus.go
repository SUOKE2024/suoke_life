...
// 让MilvusClient实现VectorStore接口
var _ VectorStore = (*MilvusClient)(nil)

// Ping 检查Milvus服务是否可用
func (m *MilvusClient) Ping(ctx context.Context) error {
	// 检查是否可以获取集合信息来验证连接
	_, err := m.client.DescribeCollection(ctx, m.collectionName)
	if err != nil {
		return fmt.Errorf("Milvus服务不可用: %w", err)
	}
	return nil
}
