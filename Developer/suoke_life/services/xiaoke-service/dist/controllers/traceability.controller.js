...
const traceability_service_1 = __importDefault(require("../services/traceability/traceability.service"));
...
class TraceabilityController {
     ...
     * 根据产品ID获取溯源信息
     */
    async getTraceabilityByProductId(req, res) {
        try {
            // 追踪请求指标
            metrics_1.httpRequestsTotal.inc({
                method: req.method,
    ...
    async verifyBlockchainRecord(req, res) {
        try {
            ...
            logger_1.logger.error('验证溯源信息失败:', error);
            res.status(500).json({
                success: false,
                error: '验证溯源信息失败',
                message: error instanceof Error ? error.message : '未知错误'
     ...
     * 获取溯源统计信息
     */
    async getTraceabilityStats(req, res) {
        try {
            // 追踪请求指标
            metrics_1.httpRequestsTotal.inc({
                method: req.method,
...
exports.default = new TraceabilityController(traceability_service_1.default);
