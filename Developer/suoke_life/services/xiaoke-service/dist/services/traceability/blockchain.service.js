...
class BlockchainService {
     ...
     */
    async uploadToBlockchain(traceabilityId) {
        try {
            // 获取溯源信息
            const traceability = await traceability_model_1.TraceabilityModel.findOne({ traceabilityId }).lean();
            if (!traceability) {
                return {
                    success: false,
                    message: '溯源信息不存在'
            ...
            if (process.env.BLOCKCHAIN_SIMULATION === 'true') {
                ...
                // 发送实际请求
                response = await axios_1.default.post(`${this.blockchainApiUrl}/transactions`, requestData, {
                    headers: {
            ...
            if (!response.data.success) {
                logger_1.logger.error('区块链上传失败:', response.data.message);
                return {
                    success: false,
            ...
            // 更新溯源记录中的区块链信息
            await traceability_model_1.TraceabilityModel.findOneAndUpdate({ traceabilityId }, {
                blockchainRecord,
    ...
    async verifyBlockchainRecord(transactionId) {
        try {
            ...
            if (cachedResult) {
                logger_1.logger.debug(`从缓存获取区块链验证结果: ${transactionId}`);
                return cachedResult;
            }
            // 从数据库获取区块链记录
            ...
            if (process.env.BLOCKCHAIN_SIMULATION === 'true') {
                ...
                // 从实际区块链获取数据
                const response = await axios_1.default.get(`${this.blockchainApiUrl}/transactions/${transactionId}`, {
                    headers: {
                        'Authorization': `Bearer ${this.blockchainApiKey}`
                    }
                });
            ...
            // 如果验证失败，更新验证状态
            if (!isVerified) {
                await traceability_model_1.TraceabilityModel.findOneAndUpdate({ 'blockchainRecord.transactionId': transactionId }, { verificationStatus: 'failed' });
            }
            // 准备验证结果
            const result = {
                verified: isVerified,
            ...
            // 更新缓存
            await (0, cache_1.setCache)(cacheKey, result, BLOCKCHAIN_CACHE_TTL);
            return result;
        }
        catch (error) {
     ...
     * @param limit 处理数量限制
     */
    async batchUploadToBlockchain(limit = 10) {
        try {
            // 查找所有没有区块链记录的溯源信息
            const traceabilities = await traceability_model_1.TraceabilityModel.find({
            ...
            logger_1.logger.error('批量上链失败:', error);
            return {
                success: false,
                processed: 0,
                succeeded: 0,
     ...
     * @param dataHash 数据哈希
     */
    simulateBlockchainResponse(dataHash) {
        // 生成模拟的交易ID
        const transactionId = 'tx_' + crypto_1.default.randomBytes(16).toString('hex');
        // 生成模拟的区块号
     ...
     */
    simulateBlockchainData(transactionId, traceability) {
        return {
            success: true,
            data: {
                transactionId,
