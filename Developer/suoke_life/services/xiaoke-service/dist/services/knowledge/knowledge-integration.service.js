...
exports.KnowledgeIntegrationServiceImpl = void 0;
/**
 * 知识整合服务实现
 */
 ...
 * 知识整合服务实现类
 */
class KnowledgeIntegrationServiceImpl {
    constructor(knowledgeBaseService, knowledgeGraphService) {
        this.logger = new logger_1.Logger('KnowledgeIntegrationService');
        this.isInitialized = false;
        this.cache = new Map();
        /**
         * 缓存过期时间(毫秒)
         */
        this.CACHE_TTL = 2 * 60 * 60 * 1000; // 2小时
        /**
         * 知识增强缓存键前缀
         */
        this.ENRICHMENT_CACHE_PREFIX = 'enrich_';
        /**
         * 农产品健康知识搜索缓存键前缀
         */
        this.AG_HEALTH_SEARCH_PREFIX = 'ag_health_search_';
        /**
         * 产品健康知识缓存键前缀
         */
        this.PRODUCT_HEALTH_PREFIX = 'product_health_';
        /**
         * 节气农产品知识缓存键前缀
         */
        this.SOLAR_TERM_PREFIX = 'solar_term_';
     ...
     * @param items 知识条目
     */
    extractHealthBenefits(concepts, items) {
        // 从概念中找出所有健康益处
        ...
        const benefitsFromItems = items
            .filter(item => item.categories?.includes('健康益处') ||
     ...
     * @param concepts 关联概念节点
     */
    extractConstitutionFit(concepts) {
        // 找出所有体质相关概念
        const constitutionNodes = concepts.filter(node => node.type === 'Constitution' || node.type === 'ConstitutionFit');
        const suitable = [];
        const unsuitable = [];
        // 分析体质适宜性
     ...
     * @param concepts 关联概念节点
     */
    extractSeasonalInfo(concepts) {
        // 找出所有季节和节气相关概念
        const seasonalNodes = concepts.filter(node => node.type === 'SolarTerm' || node.type === 'Season');
        const solarTerms = [];
        const bestSeasons = [];
        // 分析季节和节气信息
    ...
    async enrichProductKnowledge(productId) {
        ...
        try {
            ...
            throw new Error(`产品知识增强失败: ${error.message}`);
        }
    }
    /**
     * 查询农产品与健康知识的关联
     */
    async searchAgricultureHealthKnowledge(query, options) {
        this.checkServiceReady();
        // 生成缓存键
        ...
        try {
            ...
            throw new Error(`农产品健康知识搜索失败: ${error.message}`);
        }
    }
    /**
     * 获取产品相关健康知识
     */
    async getProductHealthKnowledge(productId) {
        this.checkServiceReady();
        // 生成缓存键
        ...
        try {
            ...
            throw new Error(`获取产品健康知识失败: ${error.message}`);
        }
    }
    /**
     * 获取节气相关农产品知识
     */
    async getSolarTermAgricultureKnowledge(solarTerm) {
        this.checkServiceReady();
        // 生成缓存键
      ...
      `;
            ...
            const products = productResult.map((item) => item.productName);
            // 获取节气相关知识
            ...
            throw new Error(`获取节气农产品知识失败: ${error.message}`);
        }
    }
}
exports.KnowledgeIntegrationServiceImpl = KnowledgeIntegrationServiceImpl;
