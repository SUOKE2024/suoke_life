...
export async function setupVault(): Promise<void> {
  try {
    ...
    // 检查密钥挂载路径是否存在
    if (fs.existsSync(SECRETS_MOUNT_PATH)) {
      logger.info(`Vault secrets mount path exists at ${SECRETS_MOUNT_PATH}`);
      
      // 列出可用的密钥
      const files = fs.readdirSync(SECRETS_MOUNT_PATH);
...
export async function getSecretFromVault(secretName: string): Promise<string | null> {
  try {
    ...
    // 检查密钥文件路径
    const secretPath = path.join(SECRETS_MOUNT_PATH, secretName);
...
export function listSecrets(): string[] {
  try {
    if (!process.env.USE_VAULT || !fs.existsSync(SECRETS_MOUNT_PATH)) {
      return [];
    }

    return fs.readdirSync(SECRETS_MOUNT_PATH);
  } catch (error) {
