 ...
 * 负责提供产品溯源数据可视化和分析功能
 */
export class TraceabilityVisualizationService {
  /**
   * 获取完整溯源链数据（用于产品生命周期可视化）
   * @param traceabilityId 溯源ID
   */
  async getTraceabilityChain(traceabilityId: string): Promise<any> {
    try {
      // 尝试从缓存获取
      ...
      if (cachedData) {
        logger.debug(`从缓存获取溯源链数据: ${traceabilityId}`);
        return cachedData;
      }
      ...
      // 更新缓存
      await setCache(cacheKey, chainData, VISUALIZATION_CACHE_TTL);
   ...
   */
  async getSupplyChainGeoDistribution(productCategory?: string): Promise<any> {
    try {
      // 尝试从缓存获取
      ...
      if (cachedData) {
        logger.debug(`从缓存获取供应链地理分布数据: ${productCategory || 'all'}`);
        return cachedData;
      }
      ...
      // 从数据库聚合数据
      const originData = await TraceabilityModel.aggregate([
        { $match: query },
        {
          $group: {
            _id: {
      ...
      // 获取生产阶段位置数据
      const productionStageData = await TraceabilityModel.aggregate([
        { $match: query },
      ...
      if (originData.some(item => item.coordinates)) {
        const connectionsData = await TraceabilityModel.aggregate([
          { $match: query },
          { $unwind: '$productionStages' },
          {
            $project: {
      ...
      // 更新缓存
      await setCache(cacheKey, geoData, VISUALIZATION_CACHE_TTL);
   ...
   */
  async getTraceabilityAnalytics(startDate?: string, endDate?: string): Promise<any> {
    try {
      // 构建查询条件
      const query: any = {};
      ...
      if (cachedData) {
        logger.debug(`从缓存获取溯源数据分析: ${dateRange}`);
        return cachedData;
      }

      // 分类统计
      const categoryCounts = await TraceabilityModel.aggregate([
        { $match: query },
        {
          $group: {
            _id: '$productCategory',
      ...
      // 验证状态统计
      const verificationStatusCounts = await TraceabilityModel.aggregate([
        { $match: query },
        {
          $group: {
      ...
      // 扫描量趋势（按天）
      const scanTrends = await TraceabilityModel.aggregate([
        { $match: query },
        {
          $group: {
            _id: {
      ...
      // 农场类型分布
      const farmingTypeCounts = await TraceabilityModel.aggregate([
        { $match: query },
        {
          $group: {
      ...
      // 产品生产周期分析
      const productionDurationAnalysis = await TraceabilityModel.aggregate([
        { $match: query },
        { $unwind: '$productionStages' },
        {
          $group: {
      ...
      const analyticsData = {
        summary: {
          total: await TraceabilityModel.countDocuments(query),
      ...
      // 更新缓存
      await setCache(cacheKey, analyticsData, VISUALIZATION_CACHE_TTL);
   ...
   */
  async getQualityMonitoringData(productId?: string): Promise<any> {
    try {
      // 构建查询条件
      const query: any = {};
      if (productId) {
      ...
      // 从数据库获取质量测试数据
      const qualityData = await TraceabilityModel.aggregate([
        { $match: query },
        { $unwind: '$productionStages' },
...
export default new TraceabilityVisualizationService(); 