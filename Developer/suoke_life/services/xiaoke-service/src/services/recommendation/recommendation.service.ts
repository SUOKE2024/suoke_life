...
import { FarmActivityModel } from '../../models/activity.model';
import { UserProfileModel } from '../../models/user-profile.model';
import { container } from '../../core/container';
import { KnowledgeGraphService } from '../../services/knowledge-graph/knowledge-graph.service';
import { KnowledgeIntegrationService } from '../../services/knowledge-integration/knowledge-integration.service';
...
export class RecommendationService {
  ...
  private readonly cacheService = getCache;
   ...
   * @param limit 限制返回数量
   */
  async getPersonalizedProductRecommendations(userId: string, limit: number = 10): Promise<any[]> {
    try {
      // 尝试从缓存获取
      ...
      if (cachedData) {
        logger.debug(`从缓存获取个性化产品推荐: ${userId}`);
        return cachedData;
      }
      ...
      // 获取符合条件的产品
      let recommendedProducts = await ProductModel.find(query)
        .sort({ rating: -1 })
      ...
      // 更新缓存
      await setCache(cacheKey, recommendedProducts, RECOMMENDATION_CACHE_TTL);
  ...
  async getPopularProducts(limit: number = 10): Promise<any[]> {
    try {
      ...
      // 更新缓存
      await setCache(cacheKey, popularProducts, RECOMMENDATION_CACHE_TTL);
   ...
   * @param limit 限制返回数量
   */
  async getSeasonalProductRecommendations(limit: number = 10): Promise<any> {
    try {
      // 尝试从缓存获取
      ...
      if (cachedData) {
        logger.debug('从缓存获取节气产品推荐');
        return cachedData;
      }

      // 获取当前节气
      const currentSolarTerm = await seasonalService.getCurrentSolarTerm();
      
      if (!currentSolarTerm) {
        logger.warn('当前节气信息不可用');
        return {
          solarTerm: null,
      ...
      // 获取符合条件的产品
      let seasonalProducts = await ProductModel.find(query)
        .sort({ rating: -1 })
        .limit(limit)
        .lean();
      ...
      if (seasonalProducts.length < limit) {
        ...
        for (const product of popularProducts) {
          if (!existingIds.has(product._id.toString())) {
            ...
            if (seasonalProducts.length >= limit) break;
          }
        }
      }
      ...
      // 更新缓存
      await setCache(cacheKey, result, RECOMMENDATION_CACHE_TTL);
   ...
   * @param limit 限制返回数量
   */
  async getPersonalizedActivityRecommendations(userId: string, limit: number = 5): Promise<any[]> {
    try {
      // 尝试从缓存获取
      ...
      if (cachedData) {
        logger.debug(`从缓存获取个性化活动推荐: ${userId}`);
        return cachedData;
      }
      ...
      // 获取符合条件的活动
      let recommendedActivities = await FarmActivityModel.find(query)
        .sort({ 
      ...
      // 更新缓存
      await setCache(cacheKey, recommendedActivities, RECOMMENDATION_CACHE_TTL);
   ...
   * @param limit 限制返回数量
   */
  async getPopularActivities(limit: number = 5): Promise<any[]> {
    try {
      // 尝试从缓存获取
      ...
      // 获取热门活动
      const popularActivities = await FarmActivityModel.find({
        status: 'active',
        startDate: { $gt: new Date() } // 未开始的活动
      })
      .sort({ 
      ...
      // 更新缓存
      await setCache(cacheKey, activitiesWithDays, RECOMMENDATION_CACHE_TTL);
   ...
   * @param limit 限制返回数量
   */
  async getSeasonalActivityRecommendations(limit: number = 5): Promise<any> {
    try {
      // 尝试从缓存获取
      ...
      // 获取符合条件的活动
      let seasonalActivities = await FarmActivityModel.find(query)
        .sort({ 
          startDate: 1, // 优先推荐即将开始的活动
          popularity: -1 
        })
      ...
      if (seasonalActivities.length < limit) {
        ...
        for (const activity of popularActivities) {
          if (!existingIds.has(activity._id.toString())) {
            ...
            if (seasonalActivities.length >= limit) break;
          }
        }
      }
      ...
      seasonalActivities = seasonalActivities.map(activity => {
        ...
        if (currentSolarTerm.activities && activity.activityType) {
          const matchedActivity = currentSolarTerm.activities.find(a => 
            a.includes(activity.activityType));
   ...
   */
  private calculateDaysToStart(startDate: Date): number {
    const now = new Date();
   ...
   * @returns 产品推荐列表
   */
  async getHealthOrientedProductRecommendations(
    userProfile: any,
    healthConcepts: string[],
    limit = 10
  ): Promise<any[]> {
    // 缓存键
    ...
    try {
      ...
      const knowledgeIntegrationService = container.resolve('knowledgeIntegrationService');
      ...
      for (const concept of healthConcepts) {
        try {
          ...
          if (conceptNodes.length > 0) {
            ...
            // 收集产品ID
            const productIds = result.map((item: any) => item.productId);
   ...
   * @returns 节气相关产品推荐
   */
  async getSolarTermProductRecommendations(
    solarTerm: string,
    userProfile: any,
    limit = 10
  ): Promise<any[]> {
