...
if [ -z "$REGISTRY_USERNAME" ] || [ -z "$REGISTRY_PASSWORD" ] || [ -z "$REGISTRY_URL" ]; then
  echo -e "${RED}错误: 缺少必要的阿里云容器镜像仓库配置${NC}"
  echo -e "请在 .env 文件中设置以下变量:"
  echo -e "  REGISTRY_USERNAME - 阿里云容器镜像仓库用户名"
  echo -e "  REGISTRY_PASSWORD - 阿里云容器镜像仓库密码"
  echo -e "  REGISTRY_URL - 阿里云容器镜像仓库地址"
  exit 1
fi
...
if [ "$1" = "--interactive" ] || [ "$1" = "-i" ]; then
  ...
  if [ -z "$REGISTRY_PASSWORD" ]; then
    echo -e "${RED}错误: 环境变量中缺少 REGISTRY_PASSWORD${NC}"
    exit 1
  fi
...
if [ -z "$REGISTRY_NAMESPACE" ]; then
  ...
  echo -e "${YELLOW}注意: 未设置 REGISTRY_NAMESPACE，使用默认值: $REGISTRY_NAMESPACE${NC}"
fi

# 获取镜像名称
if [ -z "$IMAGE_NAME" ]; then
  IMAGE_NAME="user-service"
  echo -e "${YELLOW}注意: 未设置 IMAGE_NAME，使用默认值: $IMAGE_NAME${NC}"
fi
