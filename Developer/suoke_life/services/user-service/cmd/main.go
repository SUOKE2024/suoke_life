...
import (
	...
	"github.com/suoke-life/user-service/internal/database"
	"github.com/suoke-life/user-service/internal/server"
)
...
func main() {
	...
	// 初始化日志
	log := logger.NewLogger("user-service", "info", "json")
	log.Info("用户服务启动中...")
	...
	// 创建服务器
	srv, err := server.NewServer(cfg, log, dbManager.GetDB())
	if err != nil {
		log.Error("创建服务器失败", "error", err)
		os.Exit(1)
	}
