...
pip3 install black mypy pylint pytest pytest-cov locust
...
if python3 -m pylint --disable=all --enable=E,F,W src tests; then
...
if python3 -m pytest --cov=src tests/; then
...
print_header "运行功能测试"
if [ -f ".env" ]; then
  if python3 scripts/verify_deployment.py --env=dev --verbose --retries=1; then
