...
class TestRetryAsync:
    ...
    async def test_backoff_delay(self):
        ...
        assert 0.05 <= delay1 <= 0.30  # 第一次延迟约0.1秒（考虑抖动和系统延迟）
        assert 0.15 <= delay2 <= 0.40  # 第二次延迟约0.2秒（2倍退避，考虑抖动和系统延迟）
    ...
    async def test_max_delay_limit(self):
        ...
        assert delay1 <= 0.35  # 应该被限制在max_delay附近（考虑抖动）
        assert delay2 <= 0.35
    ...
    async def test_logging_on_final_failure(self):
        ...
        with patch('blockchain_service.utils.retry.logger') as mock_logger:
            with pytest.raises(ValueError):
                await final_failure_function()
            
            # 验证错误日志被调用
            mock_logger.error.assert_called_once()
            error_call = mock_logger.error.call_args
            assert "重试 2 次后仍然失败" in error_call[0][0]
...
class TestRetryIntegration:
    ...
    async def test_nested_retry_decorators(self):
        ...
        assert inner_calls == 3  # inner_function被调用3次：第一次失败，第二次成功，然后在第二次outer调用时再次成功
...
class TestRetryEdgeCases:
    ...
    async def test_zero_max_attempts(self):
        ...
        async def zero_attempts_function():
            nonlocal call_count
            call_count += 1
            return "success"  # 改为返回成功，因为max_attempts=0时函数不会被调用
        
        # max_attempts=0时，range(0)不会执行，函数不会被调用，会到达最后的if语句
        # 但是last_exception为None，所以不会抛出异常
        result = await zero_attempts_function()
        
        # 函数不会被调用
        assert call_count == 1
        assert result is None  # 函数没有被调用，返回None
    ...
    async def test_large_backoff_factor(self):
        ...
        assert delay1 <= 0.15  # 应该被max_delay限制（考虑系统延迟）
        assert delay2 <= 0.20  # 应该被max_delay限制（考虑系统延迟）
