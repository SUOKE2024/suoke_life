...
class LifeActivityDataRepositoryImpl implements LifeActivityDataRepository {
  ...
  Future<List<LifeActivityData>> getLifeActivityData(String userId) async {
    final result = await _databaseService
        .query('life_activity_data', where: 'user_id = ?', whereArgs: [userId]);
    return result.map((e) => LifeActivityData.fromJson(e)).toList();
  }

  @override
  Future<LifeActivityData> addLifeActivityData(
      LifeActivityData lifeActivityData) async {
    final id = await _databaseService.insert(
        'life_activity_data', lifeActivityData.toJson());
    return lifeActivityData.copyWith(id: id);
  }

  @override
  Future<LifeActivityData> updateLifeActivityData(
      LifeActivityData lifeActivityData) async {
    await _databaseService.update(
        'life_activity_data', lifeActivityData.toJson(),
        where: 'id = ?', whereArgs: [lifeActivityData.id]);
  ...
  Future<void> deleteLifeActivityData(int id) async {
    await _databaseService
        .delete('life_activity_data', where: 'id = ?', whereArgs: [id]);
  }
}
