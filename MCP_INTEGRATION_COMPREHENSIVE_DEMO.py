...
class MCPIntegrationDemo:
    ...
    async def _integrate_device_data(self, device_data: Dict[str, Any]) -> Dict[str, Any]:
        """整合设备数据"""
        # 模拟数据整合逻辑
        ...
        return {
            ...
            "unified_metrics": {
                ...
                "avg_heart_rate": sum(all_heart_rates) / len(all_heart_rates) if all_heart_rates else 0,
    ...
    async def _demo_agent_collaboration(self):
        ...
        # 启动智能体协作
        collaboration_results = await self._simulate_agent_collaboration(health_scenario)
        
        print(f"\n🎯 协作分析结果:")
        for agent, result in collaboration_results.items():
            print(f"  • {agent}: {result['analysis']} (置信度: {result['confidence']:.1%})")
    ...
    async def _simulate_agent_collaboration(self, scenario: Dict[str, Any]) -> Dict[str, Any]:
        """模拟智能体协作"""
    ...
    async def _generate_consensus(self, agent_results: Dict[str, Any]) -> Dict[str, Any]:
        """生成共识决策"""
    ...
    async def _demo_proactive_health_monitoring(self):
        ...
        for i, event in enumerate(risk_events, 1):
            ...
            print(f"     风险等级: {event['risk_level']} | 置信度: {event['confidence']:.1%}")
        ...
        for i, intervention in enumerate(interventions, 1):
            ...
            print(f"     类型: {intervention['type']} | 优先级: {intervention['priority']}")
    ...
    async def _generate_interventions(self, risk_events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """生成干预建议"""
        await asyncio.sleep(1)
