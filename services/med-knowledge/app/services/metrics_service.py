...
class MetricsService:
    ...
    def record_graph_query(self, query_type: str, status: str, duration: float):
        """记录图查询指标"""
        self.graph_queries_total.labels(
            query_type=query_type,
            status=status
        ).inc()
        
        self.graph_query_duration.labels(
            query_type=query_type
        ).observe(duration)
    
    def record_knowledge_request(self, entity_type: str, operation: str):
        """记录知识请求指标"""
        self.knowledge_requests_total.labels(
...
def monitor_performance(metrics_service: Optional[MetricsService] = None):
    """性能监控装饰器"""
    def decorator(func):
        @wraps(func)
