...
class KnowledgeService:
    ...
    async def get_constitution_by_id(self, constitution_id: str) -> Optional[Constitution]:
        """根据ID获取体质信息"""
        cache_key = CacheKeys.format_key(CacheKeys.CONSTITUTION_DETAIL, constitution_id=constitution_id)
    ...
    async def get_recommendations_by_constitution(
        ...
        """根据体质获取推荐"""
        cache_key = CacheKeys.format_key(CacheKeys.CONSTITUTION_RECOMMENDATIONS, constitution_id=constitution_id)
        if types:
        ...
        # 从数据库获取
        try:
            response = await self.repository.get_recommendations_by_constitution(constitution_id, types)
            ...
            if self.metrics_service:
                self.metrics_service.record_knowledge_request("recommendation", "by_constitution_error")
    ...
    async def get_biomarkers(
        ...
        try:
            ...
            logger.error(f"获取生物标志物列表失败 limit={limit} offset={offset} category={category}: {e}")
            if self.metrics_service:
    ...
    async def get_biomarkers_by_constitution(
        ...
        cache_key = CacheKeys.format_key(
            CacheKeys.BIOMARKER_BY_CONSTITUTION, 
