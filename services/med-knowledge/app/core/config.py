...
class ServiceDependencies(BaseSettings):
    """服务依赖配置"""
    rag: Optional[ServiceSettings] = None
    xiaoai: Optional[ServiceSettings] = None
    auth: Optional[ServiceSettings] = None
...
def get_settings() -> Settings:
    ...
    if os.environ.get("JWT_SECRET"):
        env_overrides.setdefault("security", {}).setdefault("jwt", {})["secret"] = os.environ.get("JWT_SECRET")
    
    # Redis配置
    if os.environ.get("REDIS_HOST"):
        env_overrides.setdefault("cache", {}).setdefault("redis", {})["host"] = os.environ.get("REDIS_HOST")
    if os.environ.get("REDIS_PASSWORD"):
        env_overrides.setdefault("cache", {}).setdefault("redis", {})["password"] = os.environ.get("REDIS_PASSWORD")
...
def get_config_summary() -> Dict[str, Any]:
    ...
    summary = {
        ...
        "database": {
            "uri": settings.database.uri.replace(settings.database.password, "***") if settings.database.password else settings.database.uri,
