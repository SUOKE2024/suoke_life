...
class SearchRequest(PaginationRequest):
    ...
    def validate_entity_type(cls, v):
        if v is not None:
            allowed_types = [
                'constitution', 'symptom', 'acupoint', 'herb', 'syndrome',
                'biomarker', 'western_disease', 'prevention_evidence',
                'integrated_treatment', 'lifestyle_intervention'
            ]
...
class AcupointListRequest(PaginationRequest):
    """穴位列表请求模型"""
    meridian: Optional[str] = Field(None, max_length=50, description="经络过滤")
...
class RecommendationRequest(BaseModel):
    ...
    types: Optional[List[str]] = Field(None, description="推荐类型列表")
...
class PreventionEvidenceListRequest(PaginationRequest):
    """预防医学证据列表请求模型"""
    category: Optional[str] = Field(None, max_length=50, description="分类过滤")
    evidence_level: Optional[str] = Field(None, max_length=20, description="证据等级过滤")
...
class IntegratedTreatmentListRequest(PaginationRequest):
    """中西医结合治疗列表请求模型"""
    target_condition: Optional[str] = Field(None, max_length=100, description="目标疾病过滤")
...
class GraphVisualizationRequest(BaseModel):
    """知识图谱可视化请求模型"""
    entity_type: Optional[str] = Field(None, description="实体类型过滤")
    entity_id: Optional[str] = Field(None, description="中心实体ID")
...
class RelationshipAnalysisRequest(BaseModel):
    ...
    def validate_relationship_types(cls, v):
        if v is not None:
            allowed_types = [
                'BELONGS_TO', 'TREATS', 'CAUSES', 'PREVENTS', 'RELATED_TO',
                'INDICATES', 'CONTRAINDICATED', 'SYNERGISTIC', 'ANTAGONISTIC'
            ]
            for t in v:
                if t not in allowed_types:
