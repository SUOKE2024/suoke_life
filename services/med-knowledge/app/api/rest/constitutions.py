...
from app.models.requests import ConstitutionListRequest, RecommendationRequest
from app.models.entities import Constitution, ConstitutionListResponse, RecommendationListResponse
...
@router.get("/{constitution_id}/recommendations", response_model=RecommendationListResponse, summary="获取体质推荐")
async def get_constitution_recommendations(
    constitution_id: str,
    types: Optional[str] = Query(None, description="推荐类型，多个用逗号分隔"),
    knowledge_service: KnowledgeService = Depends(get_knowledge_service)
):
    """
    获取特定体质的个性化推荐
...
@router.get("/{constitution_id}/biomarkers", response_model=RecommendationListResponse, summary="获取体质相关生物标志物")
...
@router.get("/{constitution_id}/lifestyle-interventions", response_model=RecommendationListResponse, summary="获取体质生活方式干预")
async def get_constitution_lifestyle_interventions(
    constitution_id: str,
    limit: int = Query(20, ge=1, le=100, description="每页数量"),
    offset: int = Query(0, ge=0, description="偏移量"),
    category: Optional[str] = Query(None, description="干预分类过滤"),
    knowledge_service: KnowledgeService = Depends(get_knowledge_service)
):
    """
    获取适合特定体质的生活方式干预建议
