...
async def search_knowledge(
    ...
    try:
        # 验证搜索参数
        request = SearchRequest(query=q, entity_type=entity_type, limit=limit, offset=offset)
...
async def get_search_suggestions(
    ...
    try:
        ...
        search_result = await knowledge_service.search_knowledge(
            query=q.strip(), entity_type=None, limit=limit, offset=0
        )
        ...
        for item in search_result.data:
            suggestions.append(
                {"text": item.get("name", ""), "type": item.get("type", ""), "count": 1}
            )
