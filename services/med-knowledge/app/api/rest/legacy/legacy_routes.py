...
async def get_stats(service: KnowledgeService = Depends(get_knowledge_service)):
...
async def get_acupoints(
    ...
    meridian: str | None = Query(None, description="按经络筛选"),
...
async def get_recommendations(
    ...
    types: list[str] | None = Query(None, description="推荐类型筛选"),
    service: KnowledgeService = Depends(get_knowledge_service),
):
...
async def get_prevention_evidence(
    ...
    offset: int = Query(0, ge=0),
    category: str | None = Query(None, description="按分类筛选"),
    evidence_level: str | None = Query(None, description="按证据级别筛选"),
    service: KnowledgeService = Depends(get_knowledge_service),
):
...
async def get_integrated_treatments(
    limit: int = Query(10, ge=1, le=100),
    offset: int = Query(0, ge=0),
    target_condition: str | None = Query(None, description="按目标状况筛选"),
    service: KnowledgeService = Depends(get_knowledge_service),
):
...
async def get_lifestyle_interventions_by_constitution(
    ...
    return await service.get_lifestyle_interventions_by_constitution(
...
async def get_graph_visualization(
    ...
    node_types: list[str] | None = Query(None, description="节点类型过滤"),
    relationship_types: list[str] | None = Query(None, description="关系类型过滤"),
    graph_service: KnowledgeGraphService = Depends(get_knowledge_graph_service),
):
    """获取知识图谱可视化数据"""
    return await graph_service.get_graph_visualization_data(limit, node_types, relationship_types)
...
async def find_paths(
    ...
    """查找两个节点之间的路径"""
    return await graph_service.find_path_between_nodes(start_node_id, end_node_id, max_depth)
...
async def get_node_relationships(
    ...
    """获取节点的关系"""
    return await graph_service.get_node_relationships(node_id, direction, relationship_types, limit)
...
async def get_knowledge_subgraph(
    ...
    """获取以特定实体为中心的知识子图"""
    return await graph_service.get_knowledge_subgraph(entity_type, entity_id, depth, max_nodes)
...
async def get_entity_neighbors(
    entity_type: str = Path(..., description="实体类型"),
    entity_id: str = Path(..., description="实体ID"),
    neighbor_types: list[str] | None = Query(None, description="邻居类型过滤"),
    graph_service: KnowledgeGraphService = Depends(get_knowledge_graph_service),
):
    """获取实体相邻节点"""
    return await graph_service.get_entity_neighbors(entity_type, entity_id, neighbor_types)
...
async def get_related_entities(
    ...
    relationship_type: str | None = Query(None, description="关系类型"),
    limit: int = Query(20, description="限制数量"),
    graph_service: KnowledgeGraphService = Depends(get_knowledge_graph_service),
):
    """获取相关实体"""
