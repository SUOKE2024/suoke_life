...
class RateLimiter:
    ...
    async def __call__(self, request_id: str = None) -> bool:
        """检查是否超过限流"""
        import time
        ...
        # 清理过期记录
        self.requests = {
            req_id: timestamps for req_id, timestamps in self.requests.items()
        ...
        # 过滤当前窗口内的请求
        self.requests[request_id] = [
            t for t in self.requests[request_id] if t > window_start
        ]
