...
class OptimizationExecutor:
    ...
    async def _improve_test_coverage(self):
        ...
        try:
            # 运行功能测试套件
            result = subprocess.run(
                ["python", "functional_test_suite.py"], capture_output=True, text=True
            )
        ...
        self.results.update(
            {
                ...
                "summary": {
                    "short_term_completed": len(
                        [
                            k
                            for k, v in self.results["short_term_optimizations"].items()
                            if v.get("status") == "completed"
                        ]
                    ),
                    "medium_term_planned": len(
                        self.results["medium_term_optimizations"]
                    ),
