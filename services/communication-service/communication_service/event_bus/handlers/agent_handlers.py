...
class AgentEventHandlers:
    ...
    async def handle_diagnosis_started(self, event: Event) -> None:
        ...
        try:
            ...
            logger.info("诊断流程启动成功", 
                       session_id=session_id,
                       user_id=user_id)
            
        except Exception as e:
            logger.error("处理诊断开始事件失败", 
                        event_id=event.id,
                        error=str(e))
    ...
    async def handle_xiaoai_look_completed(self, event: Event) -> None:
        ...
        try:
            ...
            logger.info("小艾望诊完成，启动小克闻诊", 
                       session_id=session_id)
            
        except Exception as e:
            logger.error("处理小艾望诊完成事件失败", 
                        event_id=event.id,
                        error=str(e))
    ...
    async def handle_xiaoke_listen_completed(self, event: Event) -> None:
        ...
        try:
            ...
            logger.info("小克闻诊完成，启动老克问诊", 
                       session_id=session_id)
            
        except Exception as e:
            logger.error("处理小克闻诊完成事件失败", 
                        event_id=event.id,
                        error=str(e))
    ...
    async def handle_laoke_inquiry_completed(self, event: Event) -> None:
        ...
        try:
            ...
            logger.info("老克问诊完成，启动索儿切诊", 
                       session_id=session_id)
            
        except Exception as e:
            logger.error("处理老克问诊完成事件失败", 
                        event_id=event.id,
                        error=str(e))
    ...
    async def handle_soer_palpation_completed(self, event: Event) -> None:
        ...
        try:
            ...
            if all(session['results'].values()):
                ...
                logger.info("四诊完成，启动辨证论治", 
                           session_id=session_id)
            else:
                logger.warning("四诊结果不完整", 
                              session_id=session_id,
                              results=session['results'])
            
        except Exception as e:
            logger.error("处理索儿切诊完成事件失败", 
                        event_id=event.id,
                        error=str(e))
    ...
    async def handle_agent_failure(self, agent_name: str, session_id: str, error: str) -> None:
        """处理智能体失败"""
        try:
            # 更新智能体状态
            ...
            logger.error("智能体执行失败", 
                        agent=agent_name,
                        session_id=session_id,
                        error=error)
            
        except Exception as e:
            logger.error("处理智能体失败事件失败", 
                        agent=agent_name,
                        error=str(e))
...
class AgentCollaborationOrchestrator:
    ...
    async def start_collaborative_diagnosis(self, user_id: str, user_data: Dict[str, Any]) -> str:
        """启动协同诊断"""
