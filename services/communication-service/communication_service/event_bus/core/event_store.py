...
class EventStore:
    ...
    def __init__(self, database_url: Optional[str] = None):
        """初始化事件存储"""
        self.database_url = (
            database_url or "postgresql://suoke:suoke123@localhost:5432/suoke_db"
        )
    ...
    async def save_event(self, event: Event) -> None:
        ...
        try:
            ...
            logger.error(
                "事件保存失败", event_id=event.id, event_type=event.type, error=str(e)
            )
            raise
    ...
    async def save_snapshot(
        self, aggregate_id: str, aggregate_type: str, version: int, data: Dict[str, Any]
    ) -> None:
        """保存聚合快照"""
        try:
            async with self.pool.acquire() as conn:
            ...
            logger.error("快照保存失败", aggregate_id=aggregate_id, error=str(e))
            raise
    ...
    async def get_latest_snapshot(self, aggregate_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("快照查询失败", aggregate_id=aggregate_id, error=str(e))
            raise
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                result = await conn.execute(
                    "DELETE FROM events WHERE timestamp < $1", cutoff_date
                )
