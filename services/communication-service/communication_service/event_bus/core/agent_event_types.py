...
class XiaoaiEvents:
    ...
    FOUR_DIAGNOSIS_COORDINATION_COMPLETED = (
        "xiaoai.coordination.four_diagnosis_completed"
    )
...
class XiaokeEvents:
    ...
    PERSONALIZED_RECOMMENDATION_GENERATED = (
        "xiaoke.subscription.recommendation_generated"
    )
...
class AgentCollaborationEvents:
    ...
    MULTI_AGENT_CONSULTATION_COMPLETED = (
        "collaboration.multi_agent_consultation_completed"
    )
    ...
    LIFESTYLE_MEDICAL_SERVICE_COORDINATED = (
        "collaboration.lifestyle_medical_coordinated"
    )

    # 用户体验协同事件
    SEAMLESS_USER_EXPERIENCE_ORCHESTRATED = (
        "collaboration.seamless_experience_orchestrated"
    )
    ...
    HOLISTIC_USER_UNDERSTANDING_ACHIEVED = (
        "collaboration.holistic_understanding_achieved"
    )
...
COLLABORATION_SCENARIOS = {
    ...
    "health_education_journey": {
        ...
        "workflow": ["laoke.知识内容推荐", "xiaoai.互动式学习", "soer.实践指导跟踪"],
    },
    "emergency_health_support": {
        "description": "紧急健康支持场景",
        "participating_agents": ["xiaoai", "xiaoke", "soer"],
        "workflow": ["soer.异常状态检测", "xiaoai.紧急评估处理", "xiaoke.医疗资源调度"],
    },
}
