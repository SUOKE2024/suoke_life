...
class SuokeEventBus:
    ...
    def __init__(
        self, redis_url: Optional[str] = None, service_name: Optional[str] = None
    ):
        """初始化事件总线"""
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(
                "事件总线Redis连接失败", error=str(e), service=self.service_name
            )
            raise
    ...
    async def publish(
        ...
        try:
            ...
            await self.redis_client.publish(
                "suoke.events.all", json.dumps(event.to_dict())
            )
    ...
    async def subscribe(
        ...
        logger.info("事件订阅成功", event_type=event_type, service=self.service_name)
    ...
    async def subscribe_pattern(
        ...
        logger.info("事件模式订阅成功", pattern=pattern, service=self.service_name)
    ...
    async def _subscribe_to_channel(self, event_type: str) -> None:
        ...
        try:
            ...
            logger.info(
                "开始监听事件频道", event_type=event_type, service=self.service_name
            )
...
async def initialize_event_bus(
    redis_url: Optional[str] = None, service_name: Optional[str] = None
) -> SuokeEventBus:
    """初始化全局事件总线"""
    global _global_event_bus
