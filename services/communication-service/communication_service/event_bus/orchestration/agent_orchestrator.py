...
class AgentOrchestrator:
    ...
    async def _generate_tasks(
        self, scenario: str, session_id: str, context: Dict[str, Any]
    ) -> List[AgentTask]:
        """生成任务列表"""
        tasks = []
        ...
        try:
            ...
            while len(completed_tasks) < len(session.tasks):
                ...
                await asyncio.gather(
                    *[self._execute_task(task, session_id) for task in ready_tasks]
                )
        ...
        try:
            ...
            if task.status != "completed":
                ...
                if task.retry_count < task.max_retries:
                    ...
                    task.status = "pending"
                    self.logger.warning(
                        f"任务超时，准备重试: {task.agent_id}.{task.task_type}"
                    )
                else:
                    task.status = "failed"
            ...
            task.error = str(e)
            self.logger.error(
                f"执行任务异常: {task.agent_id}.{task.task_type}, {str(e)}"
            )

    async def _generate_collaboration_result(
        ...
        result = {
            ...
            "execution_summary": {
                ...
                "completed_tasks": len(
                    [t for t in session.tasks if t.status == "completed"]
                ),
    ...
    async def get_session_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "progress": {
                ...
                "running_tasks": len(
                    [t for t in session.tasks if t.status == "running"]
                ),
                "failed_tasks": len([t for t in session.tasks if t.status == "failed"]),
            },
    ...
    async def cleanup_completed_sessions(self, max_age_hours: int = 24):
        ...
        for session_id, session in self.active_sessions.items():
            if (
                session.state
                in [CollaborationState.COMPLETED, CollaborationState.FAILED]
...
class AgentCollaborationManager:
    ...
    async def initialize(self):
        ...
        await self.event_bus.subscribe(
            AgentCollaborationEvents.AGENT_HANDOFF_INITIATED, self._handle_agent_handoff
        )
