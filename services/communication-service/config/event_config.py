...
class DatabaseConfig:
    """数据库配置"""
    url: str = field(default_factory=lambda: os.getenv('DATABASE_URL', 'postgresql://suoke:suoke123@localhost:5432/suoke_db'))
    min_pool_size: int = field(default_factory=lambda: int(os.getenv('DB_MIN_POOL_SIZE', '5')))
...
class EventBusConfig:
    """事件总线配置"""
    service_name: str = field(default_factory=lambda: os.getenv('SERVICE_NAME', 'suoke-event-bus'))
...
class CacheConfig:
    """缓存配置"""
    default_ttl: int = field(default_factory=lambda: int(os.getenv('CACHE_DEFAULT_TTL', '3600')))  # 1小时
    max_memory_usage: str = field(default_factory=lambda: os.getenv('CACHE_MAX_MEMORY', '512mb'))
    eviction_policy: str = field(default_factory=lambda: os.getenv('CACHE_EVICTION_POLICY', 'allkeys-lru'))
    
    # 不同数据类型的缓存策略
    cache_strategies: Dict[str, Dict[str, Any]] = field(default_factory=lambda: {
...
class AgentConfig:
    ...
    retry_delay: int = field(default_factory=lambda: int(os.getenv('AGENT_RETRY_DELAY', '5')))  # 秒
...
class HealthDataConfig:
    """健康数据配置"""
    # 数据质量阈值
    quality_thresholds: Dict[str, Dict[str, float]] = field(default_factory=lambda: {
    ...
    # 异常检测规则
    anomaly_rules: Dict[str, Any] = field(default_factory=lambda: {
        'heart_rate_high': 100,
        'heart_rate_low': 60,
...
class SecurityConfig:
    ...
    # 访问控制
    allowed_sources: List[str] = field(default_factory=lambda: os.getenv('ALLOWED_SOURCES', '').split(',') if os.getenv('ALLOWED_SOURCES') else [])
