...
async def publish_event(request: EventPublishRequest) -> EventPublishResponse:
    ...
    try:
        ...
        return EventPublishResponse(
            success=True, event_id=event_id, message="事件发布成功"
        )
...
async def start_diagnosis(request: DiagnosisStartRequest) -> DiagnosisStartResponse:
    ...
    try:
        ...
        return DiagnosisStartResponse(
            success=True, session_id=session_id, message="诊断流程启动成功"
        )
...
async def update_health_data(
    request: HealthDataUpdateRequest,
) -> HealthDataUpdateResponse:
    """更新健康数据"""
    try:
        event_bus = get_event_bus()
        ...
        return HealthDataUpdateResponse(success=True, message="健康数据更新成功")
...
async def get_health_trends(
    user_id: str, data_type: str, period: str = "week"
) -> Dict[str, Any]:
    """获取健康趋势"""
    try:
...
async def submit_user_question(
    user_id: str, question: str, context: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """提交用户问题"""
    try:
        event_bus = get_event_bus()
...
async def submit_user_feedback(
    user_id: str, feedback_type: str, content: str, rating: Optional[int] = None
) -> Dict[str, Any]:
    """提交用户反馈"""
    try:
        event_bus = get_event_bus()
