...
async def start_collaboration(
    ...
    try:
        ...
        if request.scenario not in COLLABORATION_SCENARIOS:
            raise HTTPException(
                status_code=400, detail=f"不支持的协同场景: {request.scenario}"
            )

        # 启动协同
        session_id = await manager.start_collaboration(
            scenario=request.scenario, user_id=request.user_id, context=request.context
        )
...
async def start_health_diagnosis(
    ...
    try:
        ...
        scenario = (
            ...
            else "comprehensive_health_diagnosis"
        )
        ...
        session_id = await manager.start_collaboration(
            scenario=scenario, user_id=request.user_id, context=context
        )
...
async def get_agents_status(
    ...
    try:
        ...
        return {
            ...
            "active_agents": len(
                [s for s in status_data.values() if s.get("status") == "active"]
            ),
        }
...
async def test_mock_collaboration(
    ...
    try:
        context = {
            ...
            "user_info": {"age": 30, "gender": "female", "occupation": "office_worker"},
        }

        session_id = await manager.start_collaboration(
            scenario=scenario, user_id=user_id, context=context
        )
