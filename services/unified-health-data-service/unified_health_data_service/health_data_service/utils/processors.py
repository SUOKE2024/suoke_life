...
class HealthDataProcessor:
    ...
    async def process(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 数据质量评估
            processed_data["quality_score"] = await self._assess_data_quality(
                processed_data
            )
        ...
        for key in ["recorded_at", "created_at", "updated_at"]:
            if key in data and data[key]:
                if isinstance(data[key], str):
                    try:
                        normalized_data[key] = datetime.fromisoformat(
    ...
    async def _calculate_trend(
        ...
        if len(values) >= 2:
            ...
            change_percent = (
                ((second_mean - first_mean) / first_mean) * 100
                if first_mean != 0
                else 0
            )
        ...
        analysis = self._generate_trend_analysis(
            trend_direction, change_percent, mean_value
        )
    ...
    def _generate_trend_analysis(
        self, direction: str, change_percent: float, mean_value: float
    ) -> str:
        """生成趋势分析文本"""
    ...
    async def _score_diagnostics(self, diagnostics: List[Dict[str, Any]]) -> float:
        ...
        if any(
            word in result for word in ["异常", "问题", "疾病", "abnormal", "disease"]
        ):
    ...
    async def _vital_signs_recommendations(
    ...
    async def _anomaly_recommendations(
