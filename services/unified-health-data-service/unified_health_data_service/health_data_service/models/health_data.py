...
class UpdateHealthDataRequest(BaseRequest):
    ...
    processed_data: Optional[dict[str, Any]] = Field(
        default=None, description="处理后数据"
    )
    quality_score: Optional[float] = Field(
        default=None, ge=0, le=1, description="数据质量评分"
    )
    confidence_score: Optional[float] = Field(
        default=None, ge=0, le=1, description="置信度评分"
    )
...
class CreateVitalSignsRequest(BaseRequest):
    ...
    heart_rate: Optional[int] = Field(
        default=None, ge=30, le=220, description="心率(bpm)"
    )
    blood_pressure_systolic: Optional[int] = Field(
        default=None, ge=70, le=250, description="收缩压(mmHg)"
    )
    blood_pressure_diastolic: Optional[int] = Field(
        default=None, ge=40, le=150, description="舒张压(mmHg)"
    )
    body_temperature: Optional[float] = Field(
        default=None, ge=35.0, le=42.0, description="体温(°C)"
    )
    respiratory_rate: Optional[int] = Field(
        default=None, ge=8, le=40, description="呼吸频率(次 / 分)"
    )
    oxygen_saturation: Optional[float] = Field(
        default=None, ge=70.0, le=100.0, description="血氧饱和度(%)"
    )
    weight: Optional[float] = Field(
        default=None, ge=20.0, le=300.0, description="体重(kg)"
    )
    height: Optional[float] = Field(
        default=None, ge=100.0, le=250.0, description="身高(cm)"
    )
    device_id: Optional[str] = Field(default=None, description="设备ID")
