...
class SamsungHealthConnector(BaseDeviceConnector):
    ...
    async def _get_steps_data(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.api_base_url}/steps", headers=headers, params=params
            ) as response:
                if response.status == 200:
...
class GarminConnector(BaseDeviceConnector):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.api_base_url}/userprofile-service/userprofile",
                    headers=headers,
                ) as response:
                    if response.status == 200:
                        self.is_connected = True
    ...
    async def _get_hrv_data(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.api_base_url}/hrv-service/hrv", headers=headers, params=params
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    return [
...
class OuraConnector(BaseDeviceConnector):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.api_base_url}/usercollection/personal_info", headers=headers
                ) as response:
                    if response.status == 200:
                        self.is_connected = True
    ...
    async def _get_readiness_scores(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.api_base_url}/usercollection/daily_readiness",
                headers=headers,
                params=params,
            ) as response:
                if response.status == 200:
...
class WHOOPConnector(BaseDeviceConnector):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{self.api_base_url}/user/profile/basic", headers=headers
                ) as response:
                    if response.status == 200:
                        self.is_connected = True
    ...
    async def _get_strain_data(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{self.api_base_url}/activity/strain", headers=headers, params=params
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    return [
...
class ExtendedDeviceManager:
    ...
    async def register_device(
        self, device_type: ExtendedDeviceType, config: Dict[str, Any]
    ) -> bool:
        """注册设备"""
        try:
    ...
    async def sync_all_devices(
        self, start_time: datetime, end_time: datetime
    ) -> Dict[str, List[ExtendedDeviceData]]:
        """同步所有设备数据"""
        ...
        for device_type, connector in self.connectors.items():
            task = asyncio.create_task(
                self._sync_device_with_metrics(
                    device_type, connector, start_time, end_time
                )
            )
        ...
        logger.info(
            f"设备同步完成: 成功{successful_syncs}个, 失败{failed_syncs}个, 耗时{sync_duration:.2f}秒"
        )

        return all_data

    async def _sync_device_with_metrics(
        self,
        device_type: str,
        connector: BaseDeviceConnector,
        start_time: datetime,
        end_time: datetime,
    ) -> List[ExtendedDeviceData]:
        """带性能指标的设备同步"""
            ...
            for item in data:
                if hasattr(item, "processing_time"):
                    item.processing_time = (
                        sync_duration / len(data) if data else sync_duration
                    )

            logger.info(
                f"设备 {device_type} 同步完成: {len(data)}条数据, 耗时{sync_duration:.2f}秒"
            )
            return data
    ...
    def _calculate_data_quality(
        self, all_data: Dict[str, List[ExtendedDeviceData]]
    ) -> float:
        """计算数据质量分数"""
        if not all_data:
            return 0.0
        ...
        return {
            ...
            "device_status": {
                device_type: {
                    ...
                    "last_sync": (
                        connector.last_sync_time.isoformat()
                        if hasattr(connector, "last_sync_time")
                        and connector.last_sync_time
                        else None
                    ),
                }
                for device_type, connector in self.connectors.items()
            ...
            "supported_devices": [
                device_type.value for device_type in ExtendedDeviceType
            ],
    ...
    async def optimize_sync_performance(self) -> Dict[str, Any]:
        ...
        # 检查失败率
        total_syncs = (
            self.sync_performance_metrics["successful_syncs"]
            + self.sync_performance_metrics["failed_syncs"]
        )
        if (
            total_syncs > 0
            and self.sync_performance_metrics["failed_syncs"] / total_syncs > 0.1
        ):
            optimizations.append(
                {
