...
class AppleHealthConnector(BaseDeviceConnector):
    ...
    async def get_device_info(self, device_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_metrics": [
                metric.value
                for metric in ExtendedHealthMetrics
                if metric.value
                in [
                    "heart_rate",
                    "steps",
    ...
    async def get_device_info(self, device_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_metrics": [
                ...
                for metric in ExtendedHealthMetrics
                if metric.value
                in ["heart_rate", "steps", "sleep_duration", "blood_oxygen", "calories"]
            ],
        }
        ...
        try:
            mock_data = [
                DeviceData(
                    ...
                    raw_data={
                        "stress_category": "low",
                        "measurement_method": "hrv_analysis",
                    },
                    metric_type=ExtendedHealthMetrics.STRESS_LEVEL,
        ...
        try:
            mock_data = [
                DeviceData(
                    ...
                    quality_score=0.98,
                    raw_data={
                        "contributors": {"hrv": 85, "resting_hr": 90, "temperature": 80}
                    },
...
class DeviceConnectorManager:
    ...
    async def get_cross_device_data(
        self, user_id: str, start_time: datetime, end_time: datetime
    ) -> List[DeviceData]:
        """获取跨设备数据"""
        ...
        for device_type, connector in self.connectors.items():
            task = asyncio.create_task(
                self._get_device_data_with_metrics(
                    connector, user_id, start_time, end_time
                )
            )
            tasks.append(task)
        ...
        logger.info(
            f"跨设备数据同步完成: {len(deduplicated_data)}条数据, 耗时{sync_duration:.2f}秒"
        )
        return deduplicated_data

    async def _get_device_data_with_metrics(
        self,
        ...
        for device_type, connector in self.connectors.items():
            try:
                info = await connector.get_device_info(
                    connector.config.get("device_id", "unknown")
                )
...
class UnifiedDataFormatter:
    ...
    def to_standard_format(device_data: List[DeviceData]) -> Dict[str, Any]:
        ...
        formatted_data = {
            ...
            "quality_summary": {
                "avg_quality_score": (
                    sum(data.quality_score for data in device_data) / len(device_data)
                    if device_data
                    else 0
                ),
                "avg_confidence_level": (
                    sum(data.confidence_level or 0 for data in device_data)
                    / len(device_data)
                    if device_data
                    else 0
                ),
                "total_processing_time": sum(
                    data.processing_time or 0 for data in device_data
                ),
            },
        }
