...
from ..unified_health_data_service.health_data_service.models.health_data import (
    DiagnosticDataCreate,
    HealthDataCreate,
    TCMDataCreate,
    VitalSignsCreate,
)
from ..unified_health_data_service.health_data_service.services.health_data_service import (
    HealthDataService,
)
...
async def create_health_data(
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            raise HTTPException(
                status_code=400, detail=f"健康数据创建失败: {result.get('error')}"
            )
...
async def get_health_data_by_id(
    ...
    try:
        # 这里需要实现根据ID查询的方法
        # 暂时返回模拟数据
        result = {"id": data_id, "message": "根据ID查询功能待实现"}

        return APIResponse(success=True, message="查询成功", data=result)
...
async def create_vital_signs(
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            raise HTTPException(
                status_code=400, detail=f"生命体征数据创建失败: {result.get('error')}"
            )
...
async def get_latest_vital_signs(
    ...
    try:
        ...
        if result:
            return APIResponse(
                success=True, message="获取最新生命体征成功", data=result
            )
        else:
            return APIResponse(success=False, message="未找到生命体征数据", data=None)
...
async def create_diagnostic_data(
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            raise HTTPException(
                status_code=400, detail=f"诊断数据创建失败: {result.get('error')}"
            )
...
async def create_tcm_data(
    data: TCMDataCreate, service: HealthDataService = Depends(get_health_data_service)
) -> APIResponse:
    """创建中医数据"""
    try:
        # 转换为健康数据格式
        ...
        if result.get("status") == "success":
            ...
            raise HTTPException(
                status_code=400, detail=f"中医数据创建失败: {result.get('error')}"
            )
...
async def get_health_trends(
    ...
    try:
        result = await service.get_health_trends(
            user_id=user_id, data_type=data_type, days=days
        )

        return APIResponse(success=True, message="趋势分析获取成功", data=result)
...
async def generate_health_report(
    ...
    try:
        ...
        return APIResponse(success=True, message="健康报告生成成功", data=result)
    ...
    try:
        ...
        for record in results:
            ...
            if created_at:
                ...
                if (
                    not statistics["latest_update"]
                    or created_at > statistics["latest_update"]
                ):
                    statistics["latest_update"] = created_at.isoformat()

        return APIResponse(success=True, message="统计信息获取成功", data=statistics)
...
async def health_check(
    ...
    try:
        ...
        return APIResponse(success=True, message="服务状态正常", data=status)
