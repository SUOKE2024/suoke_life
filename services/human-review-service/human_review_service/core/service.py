...
class HumanReviewService:
    ...
    async def submit_review(
        ...
        # 生成任务ID
        task_id = f"review_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid4().hex[:8]}"
        
        # 风险评估
        risk_score = self.risk_engine.assess_risk(
            content_data=review_data.content,
            content_type=review_data.review_type.value
        )
        ...
        expires_at = review_data.expires_at or (
            datetime.utcnow() + timedelta(seconds=settings.review.default_review_timeout)
        )
        ...
        await self._add_history_record(
            ...
            actor_id="system",
            actor_type="system",
            details={"risk_score": risk_score, "assessed_priority": assessed_priority.value}
        )
    ...
    async def complete_review(
        ...
        if db_task.assigned_to != reviewer_id:
            raise ValueError(f"Task {task_id} is not assigned to reviewer {reviewer_id}")
        
        # 计算实际审核时间
        actual_duration = None
        if db_task.started_at:
            actual_duration = int((datetime.utcnow() - db_task.started_at).total_seconds())
        ...
        # 更新审核员统计
        await self._update_reviewer_stats(reviewer_id, decision.decision, actual_duration, session)
    ...
    async def assign_reviewer(
        ...
        # 获取任务
        result = await session.execute(
            session.query(ReviewTaskDB).filter(ReviewTaskDB.task_id == task_id)
        )
        db_task = result.scalar_one_or_none()
        ...
        if reviewer.current_task_count >= reviewer.max_concurrent_tasks:
            raise ValueError(f"Reviewer {reviewer_id} has reached maximum concurrent tasks")
        ...
        await session.execute(
            session.query(ReviewerDB)
            .filter(ReviewerDB.reviewer_id == reviewer_id)
            .update({"current_task_count": ReviewerDB.current_task_count + 1})
        )
    ...
    async def cancel_review_task(
        ...
        # 检查任务状态是否可以取消
        if db_task.status in [ReviewStatus.APPROVED, ReviewStatus.REJECTED, ReviewStatus.CANCELLED]:
            raise ValueError(f"Task {task_id} cannot be cancelled in current state: {db_task.status}")
        ...
        if assigned_reviewer:
            await session.execute(
                session.query(ReviewerDB)
                .filter(ReviewerDB.reviewer_id == assigned_reviewer)
                .update({"current_task_count": func.greatest(0, ReviewerDB.current_task_count - 1)})
            )
    ...
    async def get_pending_tasks(
        ...
        result = await session.execute(
            session.query(ReviewTaskDB)
            .filter(ReviewTaskDB.status.in_([ReviewStatus.PENDING, ReviewStatus.ASSIGNED]))
            .order_by(
    ...
    async def get_reviewer_tasks(
        ...
        Returns:
            任务列表
        """
        query = session.query(ReviewTaskDB).filter(
    ...
    async def get_reviewer(
        ...
        """获取审核员信息"""
        result = await session.execute(
            session.query(ReviewerDB).filter(ReviewerDB.reviewer_id == reviewer_id)
        )
    ...
    async def update_reviewer(
        ...
        db_reviewer.updated_at = datetime.utcnow()
        await session.commit()
    ...
    async def list_reviewers(
        ...
        """获取审核员列表"""
        query = session.query(ReviewerDB)
        
        if filters:
            if "status" in filters:
                query = query.filter(ReviewerDB.status == filters["status"])
            if "specialty" in filters:
                query = query.filter(ReviewerDB.specialties.contains(filters["specialty"]))
    ...
    async def list_review_tasks(
        ...
        """获取审核任务列表"""
        query = session.query(ReviewTaskDB)
    ...
    async def activate_reviewer(self, reviewer_id: str, session: AsyncSession) -> Optional[Reviewer]:
        """激活审核员"""
        result = await session.execute(
            session.query(ReviewerDB).filter(ReviewerDB.reviewer_id == reviewer_id)
        )
    ...
    async def deactivate_reviewer(self, reviewer_id: str, session: AsyncSession) -> Optional[Reviewer]:
        """停用审核员"""
        result = await session.execute(
            session.query(ReviewerDB).filter(ReviewerDB.reviewer_id == reviewer_id)
        )
    ...
    async def get_reviewer_workload(self, reviewer_id: str, session: AsyncSession) -> Optional[Dict[str, Any]]:
        """获取审核员工作负载"""
        result = await session.execute(
            session.query(ReviewerDB).filter(ReviewerDB.reviewer_id == reviewer_id)
        )
        ...
        current_tasks_result = await session.execute(
            session.query(func.count(ReviewTaskDB.id))
        ...
        return {
            ...
            "utilization": current_tasks / db_reviewer.max_concurrent_tasks if db_reviewer.max_concurrent_tasks > 0 else 0,
            "is_available": db_reviewer.is_available,
            "status": db_reviewer.status.value
        }
    
    async def get_reviewer_performance(self, reviewer_id: str, days: int, session: AsyncSession) -> Optional[Dict[str, Any]]:
        """获取审核员绩效统计"""
        result = await session.execute(
            session.query(ReviewerDB).filter(ReviewerDB.reviewer_id == reviewer_id)
        )
        ...
        tasks_result = await session.execute(
            session.query(
                ...
                func.count(ReviewTaskDB.id).filter(
                    ReviewTaskDB.status == ReviewStatus.APPROVED
                ).label("approved_reviews"),
                func.count(ReviewTaskDB.id).filter(
                    ReviewTaskDB.status == ReviewStatus.REJECTED
        ...
        return {
            ...
            "approval_rate": (stats.approved_reviews / stats.total_reviews * 100) if stats.total_reviews > 0 else 0,
    ...
    async def delete_reviewer(self, reviewer_id: str, session: AsyncSession) -> bool:
        ...
        if active_tasks > 0:
            raise ValueError(f"Cannot delete reviewer {reviewer_id}: has {active_tasks} active tasks")
    ...
    async def get_real_time_metrics(self, session: AsyncSession) -> Dict[str, Any]:
        ...
        available_reviewers_result = await session.execute(
            session.query(func.count(ReviewerDB.id))
            .filter(ReviewerDB.status == ReviewerStatus.ACTIVE)
            .filter(ReviewerDB.is_available == True)
        )
    ...
    async def get_hourly_trends(self, hours: int, session: AsyncSession) -> List[Dict[str, Any]]:
        """获取小时趋势"""
        # 简化实现，返回模拟数据
        trends = []
    ...
    async def get_daily_trends(self, days: int, session: AsyncSession) -> List[Dict[str, Any]]:
        """获取日趋势"""
        # 简化实现，返回模拟数据
        trends = []
        for i in range(days):
    ...
    async def get_reviewer_performance_ranking(self, days: int, limit: int, session: AsyncSession) -> List[Dict[str, Any]]:
        """获取审核员绩效排行"""
        # 简化实现，返回模拟数据
        ranking = []
    ...
    async def get_quality_metrics(self, days: int, session: AsyncSession) -> Dict[str, Any]:
        """获取质量指标"""
        # 简化实现，返回模拟数据
        return {
    ...
    async def get_system_alerts(self, severity: Optional[str], limit: int, session: AsyncSession) -> List[Dict[str, Any]]:
        """获取系统告警"""
        # 简化实现，返回空列表
        return []
    ...
    async def _auto_approve_task(self, task: ReviewTaskDB, session: AsyncSession) -> None:
        """自动通过任务"""
    ...
    async def _auto_assign_reviewer(self, task: ReviewTaskDB, session: AsyncSession) -> None:
        """自动分配审核员"""
        try:
            ...
            logger.warning(
                "Failed to auto-assign reviewer",
    ...
    async def _update_reviewer_stats(
        ...
        if duration and duration > 0:
            total_time = reviewer.average_review_time * (reviewer.total_reviews - 1) + duration
    ...
    async def _get_review_statistics(self, session: AsyncSession) -> ReviewStatistics:
        ...
        reviewer_result = await session.execute(
            session.query(
                ...
                func.count(ReviewerDB.id).filter(
    ...
    async def _get_recent_completions(
        ...
        result = await session.execute(
            session.query(ReviewTaskDB)
            .filter(ReviewTaskDB.status.in_([
                ReviewStatus.APPROVED,
                ReviewStatus.REJECTED,
                ReviewStatus.NEEDS_REVISION
            ]))
    ...
    async def _calculate_current_load(self, session: AsyncSession) -> float:
        ...
        return min(100.0, (current_tasks / max_capacity) * 100.0) if max_capacity > 0 else 0.0
