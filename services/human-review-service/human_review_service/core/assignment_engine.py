...
class AssignmentEngine:
    ...
    async def find_best_reviewer(
        ...
        try:
            ...
            logger.error(
                "Failed to find best reviewer", task_type=task_type.value, error=str(e)
            )
            return None
    ...
    async def _calculate_assignment_score(
        ...
        # 5. 复杂度匹配 (0-5分)
        complexity_score_match = self._calculate_complexity_score(
            reviewer, complexity_score
        )
    ...
    def _calculate_specialty_score(
        self, reviewer: ReviewerDB, task_type: ReviewType
    ) -> float:
        """计算专业匹配度分数"""
        required_specialties = self.specialty_mapping.get(task_type.value, [])
    ...
    def _calculate_priority_score(
        self, reviewer: ReviewerDB, priority: ReviewPriority
    ) -> float:
        """计算优先级匹配分数"""
        # 高优先级任务优先分配给经验丰富的审核员
    ...
    def _calculate_complexity_score(
        self, reviewer: ReviewerDB, complexity_score: float
    ) -> float:
        """计算复杂度匹配分数"""
        # 根据审核员的经验水平匹配复杂度
        reviewer_experience = getattr(reviewer, "experience_level", "intermediate")
