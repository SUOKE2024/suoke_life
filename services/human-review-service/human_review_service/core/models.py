...
class ReviewTaskDB(Base):
    ...
    priority = Column(SQLEnum(ReviewPriority), nullable=False, default=ReviewPriority.NORMAL)
    ...
    # 审核相关
    assigned_to = Column(String(100), ForeignKey("reviewers.reviewer_id"), nullable=True)
...
class ReviewHistoryDB(Base):
    ...
    id = Column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    task_id = Column(PGUUID(as_uuid=True), ForeignKey("review_tasks.id"), nullable=False)
    ...
    actor_type = Column(String(50), nullable=False)  # 操作者类型（reviewer/system/admin）
