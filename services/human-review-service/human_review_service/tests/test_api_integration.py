...
class TestReviewTaskAPI:
    ...
    def test_complete_task(self, test_client):
        ...
        # 分配任务
        assign_response = test_client.post(f"/api/v1/tasks/{task_id}/assign/{reviewer_id}")
        ...
        response = test_client.post(f"/api/v1/tasks/{task_id}/complete", json=decision_data)
        assert response.status_code == 200
