...
class TestDatabaseIntegration:
    ...
    async def test_review_task_crud_operations(self, db_session: AsyncSession):
        ...
        found_task.assigned_at = datetime.now(timezone.utc)
        await db_session.commit()
    ...
    async def test_transaction_rollback(self, db_session: AsyncSession):
        ...
        try:
            ...
            # 验证回滚后数据库状态
            result = await db_session.execute(
                select(ReviewerDB).where(
                    ReviewerDB.reviewer_id == reviewer_data["reviewer_id"]
                )
            )
            found_reviewer = result.scalar_one_or_none()
    ...
    async def test_bulk_operations(self, db_session: AsyncSession):
        ...
        result = await db_session.execute(
            select(func.count())
            .select_from(ReviewerDB)
            .where(ReviewerDB.name.like("批量测试医生%"))
        )
        count = result.scalar()
        assert count == 5
