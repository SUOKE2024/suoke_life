...
class TestBasicFunctionality:
    ...
    def test_risk_assessment_engine(self):
        ...
        risk_score = service.risk_engine.assess_risk(
            content_data=low_risk_content, content_type="diagnosis"
        )
        ...
        high_risk_score = service.risk_engine.assess_risk(
            content_data=high_risk_content, content_type="emergency"
        )
    ...
    def test_review_type_mapping(self):
        ...
        # 测试专业领域映射
        medical_specialties = service.assignment_engine.specialty_mapping.get(
            "medical_diagnosis", []
        )
        assert len(medical_specialties) > 0
        assert "中医诊断" in medical_specialties or "西医诊断" in medical_specialties

        medication_specialties = service.assignment_engine.specialty_mapping.get(
            "medication_review", []
        )
