...
from ..core.models import (
    ReviewerCreate, ReviewerUpdate, ReviewerStatus,
    ReviewTaskCreate, ReviewTaskUpdate, ReviewStatus, ReviewType, ReviewPriority
)
...
class TestHumanReviewService:
    ...
    async def test_assign_review_task(self, db_session):
        ...
        # 分配任务
        assigned_task = await service.assign_review_task(task.task_id, reviewer.reviewer_id)
    ...
    async def test_list_reviewers(self, db_session):
        ...
        # 测试按专业领域过滤
        tcm_reviewers = await service.list_reviewers(
            filters={"specialty": "中医诊断"}
        )
