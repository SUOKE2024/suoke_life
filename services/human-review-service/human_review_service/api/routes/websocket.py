...
class ConnectionManager:
    ...
    async def connect(
        self, websocket: WebSocket, user_id: str = None, reviewer_id: str = None
    ):
        """
        接受WebSocket连接
        ...
        while True:
            ...
            try:
                ...
                if message_type == "ping":
                    ...
                    await manager.send_personal_message(
                        json.dumps(
                            {"type": "subscription_confirmed", "event_type": event_type}
                        ),
                        websocket,
                    )
        ...
        while True:
            ...
            try:
                ...
                if message_type == "status_update":
                    ...
                    logger.warning(
                        "Unknown reviewer message type", message_type=message_type
                    )

            except json.JSONDecodeError:
                logger.error(
                    "Invalid JSON message from reviewer",
                    reviewer_id=reviewer_id,
                    data=data,
                )
        ...
        while True:
            ...
            try:
                ...
                if message_type == "ping":
                    ...
                    task_id = message.get("task_id")
                    logger.info(
                        "User subscribed to task", user_id=user_id, task_id=task_id
                    )

                    await manager.send_personal_message(
                        json.dumps(
                            {"type": "task_subscription_confirmed", "task_id": task_id}
                        ),
                        websocket,
                    )
                    ...
                    logger.warning(
                        "Unknown user message type", message_type=message_type
                    )

            except json.JSONDecodeError:
                logger.error(
                    "Invalid JSON message from user", user_id=user_id, data=data
                )
...
async def notify_task_status_change(task_id: str, status: str, user_id: str = None):
    ...
    """
    message = json.dumps(
        {"type": "task_status_changed", "task_id": task_id, "status": status}
    )
...
async def notify_task_assigned(task_id: str, reviewer_id: str):
    ...
    """
    message = json.dumps(
        {"type": "task_assigned", "task_id": task_id, "reviewer_id": reviewer_id}
    )
