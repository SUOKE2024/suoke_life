...
async def get_reviewer(
    reviewer_id: str, session: AsyncSession = Depends(get_session_dependency)
) -> Reviewer:
    """
    获取审核员信息
...
async def activate_reviewer(
    ...
    try:
        ...
        logger.error(
            "Failed to activate reviewer", reviewer_id=reviewer_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def deactivate_reviewer(
    ...
    try:
        ...
        logger.error(
            "Failed to deactivate reviewer", reviewer_id=reviewer_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_reviewer_workload(
    ...
    try:
        ...
        logger.error(
            "Failed to get reviewer workload", reviewer_id=reviewer_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_reviewer_performance(
    ...
    try:
        ...
        logger.error(
            "Failed to get reviewer performance", reviewer_id=reviewer_id, error=str(e)
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
