...
async def submit_review(
    ...
    try:
        ...
        task = await service.submit_review(review_data, session)
...
async def get_review(
    ...
    try:
        service = HumanReviewService()
        task = await service.get_review_task(task_id, session)
...
async def update_review(
    ...
    try:
        service = HumanReviewService()
        task = await service.update_review_task(task_id, update_data, session)
...
async def assign_review(
    ...
    try:
        service = HumanReviewService()
        task = await service.assign_reviewer(task_id, reviewer_id, session)
...
async def complete_review(
    ...
    try:
        ...
        # 首先获取任务以获得审核员ID
        existing_task = await service.get_review_task(task_id, session)
        ...
        task = await service.complete_review(task_id, existing_task.assigned_to, decision, session)
        ...
        logger.error(
            "Failed to complete review task",
            task_id=task_id,
            error=str(e)
        )
        raise HTTPException(
...
async def cancel_review(
    ...
    try:
        service = HumanReviewService()
        success = await service.cancel_review_task(task_id, session)
