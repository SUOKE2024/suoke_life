...
def create_token_pair(user_id: str, username: str, scopes: list[str] = None) -> Token:
    ...
    token_data = {"sub": user_id, "username": username, "scopes": scopes}
...
def generate_api_key(user_id: str, platform_id: str) -> str:
    ...
    try:
        api_key = jwt.encode(data, settings.secret_key, algorithm=settings.algorithm)
        return api_key
    except Exception as e:
...
def verify_api_key(api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if payload.get("type") != "api_key":
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="无效的API密钥类型"
            )
...
def encrypt_sensitive_data(data: str) -> str:
    ...
    try:
        encrypted = jwt.encode(
            {"data": data}, settings.secret_key, algorithm=settings.algorithm
        )
        return encrypted
    except Exception as e:
...
def decrypt_sensitive_data(encrypted_data: str) -> str:
    ...
    try:
        payload = jwt.decode(
            encrypted_data, settings.secret_key, algorithms=[settings.algorithm]
        )
