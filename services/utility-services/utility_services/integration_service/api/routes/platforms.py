...
async def get_platform(
    ...
    try:
        ...
        if not platform:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="平台不存在"
            )
...
async def create_platform(
    ...
    try:
        ...
        if existing_platform:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="平台名称已存在"
            )
...
@router.get(
    " / {platform_id} / configs",
    response_model=List[PlatformConfigResponse],
    summary="获取平台配置",
)
    ...
    try:
        ...
        return [
            PlatformConfigResponse(
                ...
                config_value=(
                    config.config_value if not config.is_encrypted else " * * * "
                ),
...
async def create_platform_config(
    ...
    try:
        ...
        logger.info(
            f"用户 {current_user.username} 为平台 {platform_id} 创建配置 {config_data.config_key}"
        )
