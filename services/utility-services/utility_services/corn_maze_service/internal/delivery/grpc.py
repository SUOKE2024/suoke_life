...
class CornMazeServicer(corn_maze_pb2_grpc.CornMazeServiceServicer):
    ...
    async def CreateMaze(
        ...
        """创建迷宫"""
        logger.info(
            "gRPC CreateMaze called: user_id = %s, type = %s",
            request.user_id,
            request.maze_type,
        )
    ...
    async def GetMaze(
        self, request: corn_maze_pb2.GetMazeRequest, context: grpc.aio.ServicerContext
    ) -> corn_maze_pb2.MazeResponse:
        """获取迷宫"""
    ...
    async def MoveInMaze(
        self, request: corn_maze_pb2.MoveRequest, context: grpc.aio.ServicerContext
    ) -> corn_maze_pb2.MoveResponse:
        """在迷宫中移动"""
        logger.info(
            "gRPC MoveInMaze called: maze_id = %s, direction = %d",
            request.maze_id,
            request.direction,
        )
    ...
    async def GetUserProgress(
        ...
        """获取用户进度"""
        logger.info(
            "gRPC GetUserProgress called: user_id = %s, maze_id = %s",
            request.user_id,
            request.maze_id,
        )
...
async def create_grpc_server(settings=None) -> aio.Server:
    ...
    # 添加服务
    corn_maze_pb2_grpc.add_CornMazeServiceServicer_to_server(CornMazeServicer(), server)
