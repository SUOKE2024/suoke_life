...
class MazeNode(BaseModel):
    ...
    connections: list[tuple[int, int]] = Field(
        default_factory=list, description="连接的节点坐标"
    )
...
class Maze(BaseModel):
    ...
    description: str | None = Field(
        default=None, description="迷宫描述", max_length=500
    )
    ...
    average_completion_time: float | None = Field(
        default=None, description="平均完成时间(秒)"
    )

    @field_validator("nodes")
    @classmethod
    def validate_nodes_size(
        cls, v: list[list[MazeNode]], info: Any
    ) -> list[list[MazeNode]]:
        """验证节点矩阵大小"""
    ...
    def validate_position(cls, v: tuple[int, int], info: Any) -> tuple[int, int]:
        ...
        if not (0 <= x < size and 0 <= y < size):
            raise ValueError(
                f"Position must be within maze bounds (0, 0) to ({size - 1}, {size - 1})"
            )
        return v
...
class MazeProgress(BaseModel):
    ...
    status: ProgressStatus = Field(
        default=ProgressStatus.NOT_STARTED, description="进度状态"
    )
    current_position: tuple[int, int] = Field(description="当前位置")
    visited_nodes: list[tuple[int, int]] = Field(
        default_factory=list, description="已访问节点"
    )
    ...
    def calculate_score(self) -> int:
        ...
        self.score = max(
            0, base_score + knowledge_bonus + achievement_bonus - hint_penalty
        )
        return self.score
