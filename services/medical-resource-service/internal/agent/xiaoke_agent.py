...
class XiaokeAgent:
    ...
    async def analyze_constitution(
        ...
        try:
            ...
            # 5. 确定主要和次要体质类型
            primary_type, secondary_type = self._determine_constitution_types(final_scores)
    ...
    def _analyze_symptoms(self, symptoms: List[str]) -> Dict[str, float]:
        """基于症状分析体质"""
        scores = {const.value: 0.0 for const in ConstitutionType if const != ConstitutionType.CONSTITUTION_TYPE_UNSPECIFIED}
    ...
    def _combine_scores(
        ...
        final_scores = {}
        all_constitutions = set(symptom_scores.keys()) | set(health_scores.keys()) | set(lifestyle_scores.keys())
    ...
    def _determine_constitution_types(
        ...
        if len(sorted_scores) > 1:
            ...
            if secondary_score > 0.3 and (primary_score - secondary_score) < 0.2:
                ...
                secondary_type = ConstitutionType[secondary_name.upper().replace("质", "")]
    ...
    def _generate_analysis_summary(
        ...
        """生成分析总结"""
        summary = f"根据症状分析和健康数据评估，您的主要体质类型为{primary_type.value}。"
    ...
    async def _generate_constitution_recommendations(
        ...
        # 基于次要体质的建议
        if secondary_type:
            secondary_recommendations = self._get_constitution_recommendations(secondary_type)
            # 避免重复建议
    ...
    def _get_constitution_recommendations(self, constitution_type: ConstitutionType) -> List[str]:
        """获取特定体质的调理建议"""
        recommendations_map = {
    ...
    async def _generate_optimization_suggestions(
        ...
        for resource in resources:
            ...
            # 当前利用率
            current_utilization = historical_data.get(resource_id, {}).get("utilization", 0.0)
            ...
            if current_utilization < 0.6 and predicted_demand > current_utilization:
                ...
                suggestions.append({
                    ...
                    "parameters": {
                        "current_utilization": current_utilization,
                        "predicted_demand": predicted_demand,
                        "adjustment_direction": "increase" if predicted_demand > current_utilization else "decrease"
                    }
                })
        
        return suggestions
    
    def _calculate_expected_improvement(self, suggestions: List[Dict[str, Any]]) -> float:
        """计算预期改进"""
        if not suggestions:
            return 0.0
        
        total_impact = sum(suggestion.get("impact_score", 0.0) for suggestion in suggestions)
    ...
    def _doctor_matches_symptoms(self, doctor: Doctor, symptoms: List[str]) -> bool:
        ...
        for symptom in symptoms:
            if symptom in symptom_keywords:
                ...
                if any(keyword in specialty for specialty in doctor_specialties for keyword in keywords):
                    return True
