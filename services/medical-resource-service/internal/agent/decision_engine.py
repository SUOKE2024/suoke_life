...
from ..domain.models import (
    ConstitutionType, ResourceType, UrgencyLevel,
    Resource, Doctor, Appointment, Recommendation
)
...
class DecisionEngine:
    ...
    def _initialize_constitution_weights(self) -> Dict[ConstitutionType, Dict[str, float]]:
        """初始化体质-资源权重矩阵"""
        weights = {
    ...
    def _initialize_seasonal_factors(self) -> Dict[int, Dict[str, float]]:
        ...
        factors = {
            # 春季 (3-5月)
            3: {"liver_care": 1.2, "detox": 1.1, "allergy_prevention": 1.3, "exercise": 1.1},
            4: {"liver_care": 1.3, "detox": 1.2, "allergy_prevention": 1.4, "exercise": 1.2},
            5: {"liver_care": 1.1, "detox": 1.0, "allergy_prevention": 1.2, "exercise": 1.1},
            
            # 夏季 (6-8月)
            6: {"heart_care": 1.2, "cooling": 1.3, "hydration": 1.2, "heat_prevention": 1.3},
            7: {"heart_care": 1.3, "cooling": 1.4, "hydration": 1.3, "heat_prevention": 1.4},
            8: {"heart_care": 1.2, "cooling": 1.3, "hydration": 1.2, "heat_prevention": 1.3},
            
            # 秋季 (9-11月)
            9: {"lung_care": 1.2, "moistening": 1.2, "immune_boost": 1.1, "dryness_prevention": 1.2},
            10: {"lung_care": 1.3, "moistening": 1.3, "immune_boost": 1.2, "dryness_prevention": 1.3},
            11: {"lung_care": 1.1, "moistening": 1.1, "immune_boost": 1.3, "dryness_prevention": 1.1},
            
            # 冬季 (12-2月)
            12: {"kidney_care": 1.2, "warming": 1.3, "energy_conservation": 1.2, "cold_prevention": 1.3},
            1: {"kidney_care": 1.3, "warming": 1.4, "energy_conservation": 1.3, "cold_prevention": 1.4},
            2: {"kidney_care": 1.2, "warming": 1.3, "energy_conservation": 1.2, "cold_prevention": 1.3}
        }
        return factors
    ...
    async def make_decision(
        ...
        try:
            decision_id = f"{decision_type.value}_{context.user_id}_{int(start_time.timestamp())}"
            ...
            # 确定置信度等级
            result.confidence_level = self._determine_confidence_level(result.confidence_score)
            ...
            logger.info(f"决策完成: {decision_id}, 置信度: {result.confidence_score:.3f}")
            return result
            
        except Exception as e:
            logger.error(f"决策执行失败: {e}")
            # 返回默认决策结果
            return self._create_default_decision_result(decision_type, context, strategy)
    ...
    async def _rule_based_decision(
        ...
        if not applicable_rules:
            return self._create_default_decision_result(decision_type, context, DecisionStrategy.RULE_BASED)
    ...
    async def _hybrid_decision(
        ...
        return DecisionResult(
            ...
            alternative_options=rule_result.alternative_options + ml_result.alternative_options,
    ...
    async def _multi_criteria_decision(
        ...
        if not candidates:
            return self._create_default_decision_result(decision_type, context, DecisionStrategy.MULTI_CRITERIA)
        ...
        return DecisionResult(
            ...
            expected_outcome=self._predict_outcome(best_candidate["candidate"], context),
    ...
    async def _fuzzy_logic_decision(
        ...
        fuzzy_sets = {
            "urgency": {
                "low": self._triangular_membership(context.urgency_level.value, 0, 0, 0.3),
                "medium": self._triangular_membership(context.urgency_level.value, 0.2, 0.5, 0.8),
                "high": self._triangular_membership(context.urgency_level.value, 0.7, 1.0, 1.0)
            },
            "symptom_severity": {
        ...
        if not rule_activations:
            return self._create_default_decision_result(decision_type, context, DecisionStrategy.FUZZY_LOGIC)
        
        # 选择最高激活度的规则
        best_rule = max(rule_activations, key=lambda x: x["weighted_confidence"])
        
        # 生成推荐
        recommendation = await self._execute_fuzzy_action(best_rule["rule"]["action"], context)
    ...
    def _evaluate_rule(self, rule: DecisionRule, context: DecisionContext) -> float:
        ...
        try:
            ...
            if "symptoms" in rule.condition:
                symptom_match = any(symptom in context.symptoms for symptom in ["乏力", "怕冷", "口干"])
                if symptom_match:
                    base_score *= 1.1
    ...
    async def _execute_rule_action(self, rule: DecisionRule, context: DecisionContext) -> Dict[str, Any]:
        """执行规则动作"""
        action = rule.action
    ...
    async def _get_constitution_treatment(self, constitution_type: ConstitutionType) -> Dict[str, Any]:
        """获取体质治疗方案"""
        treatments = {
            ConstitutionType.QI_DEFICIENCY: {
        ...
        return treatments.get(constitution_type, {
            "type": "general_care",
            "methods": ["综合调理"],
            "lifestyle": ["健康生活方式"],
            "diet": ["均衡饮食"]
    ...
    def _extract_features(self, context: DecisionContext) -> Dict[str, Any]:
        """提取决策特征"""
        features = {
            "constitution_encoded": self._encode_constitution(context.constitution_type),
            ...
            "resource_availability_score": self._calculate_resource_availability_score(context.resource_availability)
        }
    ...
    def _calculate_resource_availability_score(self, resource_availability: Dict[str, Any]) -> float:
        """计算资源可用性分数"""
    ...
    async def _ml_resource_allocation(self, features: Dict[str, Any], context: DecisionContext) -> Dict[str, Any]:
        """机器学习资源分配"""
        # 简化的ML逻辑
        constitution_weights = self.constitution_resource_weights.get(context.constitution_type, {})
    ...
    async def _ml_treatment_recommendation(self, features: Dict[str, Any], context: DecisionContext) -> Dict[str, Any]:
        """机器学习治疗推荐"""
        # 基于症状和体质的治疗推荐
        treatments = []
        
        # 体质相关治疗
        constitution_treatment = await self._get_constitution_treatment(context.constitution_type)
        treatments.append({
    ...
    async def _generate_candidates(self, decision_type: DecisionType, context: DecisionContext) -> List[Dict[str, Any]]:
        """生成候选方案"""
        candidates = []
    ...
    def _evaluate_criterion(self, candidate: Dict[str, Any], criterion: str, context: DecisionContext) -> float:
        """评估候选方案的准则得分"""
    ...
    def _evaluate_effectiveness(self, candidate: Dict[str, Any], context: DecisionContext) -> float:
        """评估有效性"""
        # 简化的有效性评估
        base_score = 0.7
    ...
    def _evaluate_safety(self, candidate: Dict[str, Any], context: DecisionContext) -> float:
        """评估安全性"""
        # 中医治疗通常安全性较高
    ...
    def _evaluate_cost(self, candidate: Dict[str, Any], context: DecisionContext) -> float:
        """评估成本"""
        # 简化的成本评估
    ...
    def _evaluate_accessibility(self, candidate: Dict[str, Any], context: DecisionContext) -> float:
        """评估可及性"""
        # 基于资源可用性评估
        return self._calculate_resource_availability_score(context.resource_availability)
    
    def _evaluate_user_preference(self, candidate: Dict[str, Any], context: DecisionContext) -> float:
        """评估用户偏好"""
        # 基于用户偏好设置
        ...
        if preferences.get("prefer_tcm", False) and "tcm" in str(candidate).lower():
            return 0.9
        elif preferences.get("prefer_western", False) and "western" in str(candidate).lower():
            return 0.9
        
        return 0.6
    
    async def _execute_fuzzy_action(self, action: str, context: DecisionContext) -> Dict[str, Any]:
        """执行模糊动作"""
    ...
    def _predict_outcome(self, recommendation: Dict[str, Any], context: DecisionContext) -> Dict[str, Any]:
        """预测结果"""
        return {
    ...
    def _assess_risks(self, recommendation: Dict[str, Any], context: DecisionContext) -> Dict[str, Any]:
        """评估风险"""
        return {
            "risk_level": "low",
    ...
    def _update_decision_stats(self, decision_type: DecisionType, result: DecisionResult):
        """更新决策统计信息"""
        ...
        stats["avg_confidence"] = (
            (stats["avg_confidence"] * (stats["total_decisions"] - 1) + result.confidence_score) /
            stats["total_decisions"]
        )
        stats["avg_execution_time"] = (
            (stats["avg_execution_time"] * (stats["total_decisions"] - 1) + result.execution_time_ms) /
            stats["total_decisions"]
        )
    ...
    async def _update_rule_success_rates(self, feedback: DecisionFeedback):
        ...
        for decision_type, rules in self.decision_rules.items():
            for rule in rules:
                if rule.last_used and (datetime.now() - rule.last_used).total_seconds() < 3600:
                    # 更新成功率
                    rule.usage_count += 1
                    if success:
                        rule.success_rate = (
                            (rule.success_rate * (rule.usage_count - 1) + 1.0) / rule.usage_count
                        )
                    else:
                        rule.success_rate = (
                            rule.success_rate * (rule.usage_count - 1) / rule.usage_count
                        )
    ...
    def get_decision_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_confidence": np.mean([d.confidence_score for d in self.decision_history]) if self.decision_history else 0.0,
            "avg_execution_time": np.mean([d.execution_time_ms for d in self.decision_history]) if self.decision_history else 0.0
        }
                ...
                for rule in rules:
                    if rule.usage_count >= 10:  # 有足够的使用数据
                        ...
                        if rule.success_rate > 0.8:
                            rule.confidence_weight = min(rule.confidence_weight * 1.1, 1.0)
                        elif rule.success_rate < 0.5:
                            rule.confidence_weight = max(rule.confidence_weight * 0.9, 0.1)
