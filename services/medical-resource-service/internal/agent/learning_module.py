...
class LearningModule:
                ...
                if model_dir.exists():
                    ...
                    if version_dirs:
                        ...
                        if model_file.exists():
                            ...
                            logger.info(f"加载模型: {model_type.value} v{latest_version.name}")
    ...
    def _initialize_encoders(self):
        ...
        # 症状编码器
        common_symptoms = [
            "头痛", "发热", "咳嗽", "乏力", "失眠", "食欲不振", "腹痛", "腹泻",
            "便秘", "心悸", "胸闷", "气短", "眩晕", "耳鸣", "视力模糊", "关节痛",
            "肌肉酸痛", "皮疹", "瘙痒", "水肿", "尿频", "尿急", "月经不调", "痛经"
        ]
    ...
    async def train_model(
        ...
        try:
            ...
            if hyperparameter_tuning:
                model = self._tune_hyperparameters(model, algorithm_type, X_train, y_train)
            ...
            metrics = self._calculate_metrics(
                model_type, algorithm_type, y_val, y_pred, y_pred_proba,
                len(features), len(feature_names), start_time
            )
            ...
            # 保存模型
            version = await self._save_model(model_type, model, scaler, {
            ...
            logger.info(f"模型训练完成: {model_type.value}, 准确率: {metrics.accuracy:.4f}")
    ...
    def _encode_environmental_features(self, environmental: Dict[str, Any]) -> List[float]:
        """编码环境特征"""
        features = []
    ...
    def _prepare_recommendation_data(
        ...
        for data in training_data:
            ...
            constitution_encoded = self.encoders["constitution"].transform([constitution])[0]
        ...
        feature_names = (
            ...
            ["prefer_tcm", "prefer_modern", "price_sensitivity", "distance_sensitivity"]
        )
    ...
    def _tune_hyperparameters(self, model, algorithm_type: AlgorithmType, X_train, y_train):
        """超参数调优"""
        param_grids = {
    ...
    async def predict(
        ...
        try:
            ...
            logger.debug(f"预测完成: {model_type.value}, 结果: {prediction}, 置信度: {confidence}")
    ...
    async def online_learning_update(
        ...
        try:
            ...
            logger.info(f"在线学习更新完成: {model_type.value}, 新数据量: {len(features)}")
            
        except Exception as e:
            logger.error(f"在线学习更新失败: {e}")
    
    async def _batch_retrain(self, model_type: ModelType, new_data: List[Dict[str, Any]]):
        """批量重训练"""
        try:
            # 获取历史数据
        ...
        while True:
            try:
                ...
                for model_type, model_data in self.models.items():
                    ...
                    if current_metrics and current_metrics.accuracy < self.auto_retrain_threshold:
    ...
    def get_learning_statistics(self) -> Dict[str, Any]:
        ...
        for model_type, model_data in self.models.items():
            stats["current_models"][model_type.value] = {
                ...
                "algorithm": model_data.get("metadata", {}).get("algorithm_type", "unknown"),
        ...
        if self.learning_metrics:
            avg_accuracy = sum(m.accuracy for m in self.learning_metrics.values()) / len(self.learning_metrics)
    ...
    async def get_model_insights(self, model_type: ModelType) -> Dict[str, Any]:
        ...
        try:
            ...
            # 特征重要性
            performance_records = [p for p in self.performance_history if p.model_name.startswith(model_type.value)]
            if performance_records:
                latest_performance = max(performance_records, key=lambda x: x.last_updated)
            ...
            if model_type in self.learning_metrics:
                ...
                if metrics.precision < 0.7:
                    recommendations.append("模型精确率较低，建议调整分类阈值或使用集成方法")
                
                if metrics.recall < 0.7:
                    recommendations.append("模型召回率较低，建议平衡数据集或调整损失函数")
                
                if metrics.training_time > 300:  # 5分钟
                    recommendations.append("训练时间较长，考虑特征选择或使用更简单的算法")
