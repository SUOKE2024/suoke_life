...
from ...domain.enums.season_enums import SeasonalHealthFocus, SeasonType
from ...domain.models.food_models import FoodCombination, FoodItem, NutritionalInfo
...
class FoodTherapyService:
    ...
    def __init__(
        self, food_repository: FoodRepository, therapy_repository: TherapyRepository
    ):
    ...
    async def recommend_food_therapy(
        ...
        try:
            logger.info(
                f"为用户 {user_id} 推荐食疗方案，体质：{constitution_type.value}"
            )
    ...
    def _generate_daily_meals(
        self, foods: List[FoodItem], preferences: Dict[str, Any] = None
    ) -> Dict[str, List[str]]:
        """生成每日餐食安排"""
    ...
    def _generate_expected_benefits(
        ...
        constitution_benefits = {
            ...
            ConstitutionType.SPECIAL_CONSTITUTION: [
                "调节过敏",
                "增强适应性",
                "改善体质",
            ],
    ...
    def _generate_therapy_precautions(
        self, constitution_type: ConstitutionType, foods: List[FoodItem]
    ) -> List[str]:
        """生成食疗注意事项"""
        precautions = [
            "请在专业医师指导下进行食疗",
    ...
    def _calculate_confidence_score(
        ...
        if required_benefits:
            ...
            coverage = len(set(required_benefits) & covered_benefits) / len(
                required_benefits
            )
            score += 40 * coverage
    ...
    def _generate_follow_up_schedule(self, duration_days: int) -> List[Dict[str, Any]]:
        ...
        # 第一周随访
        schedule.append(
            {"day": 7, "type": "初期评估", "content": "检查适应性，调整方案"}
        )

        # 第二周随访
        if duration_days >= 14:
            schedule.append(
                {"day": 14, "type": "中期评估", "content": "评估效果，优化配方"}
            )

        # 第四周随访
        if duration_days >= 28:
            schedule.append(
                {"day": 28, "type": "阶段评估", "content": "全面评估，制定后续计划"}
            )
    ...
    async def update_therapy_progress(
        self, recommendation_id: str, progress_data: Dict[str, Any]
    ) -> bool:
        """更新食疗进展"""
