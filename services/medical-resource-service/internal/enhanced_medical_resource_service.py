...
class EnhancedMedicalResourceService:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.circuit_breaker_configs = {
            "resource_matching": CircuitBreakerConfig(
                failure_threshold=5, recovery_timeout=30.0, timeout=10.0
            ),
            "scheduling": CircuitBreakerConfig(
                failure_threshold=3, recovery_timeout=20.0, timeout=5.0
            ),
        }
    ...
    def _start_background_tasks(self):
        ...
        # 调度器
        self.background_tasks.append(asyncio.create_task(self._scheduler()))
    ...
    async def find_resources(
        ...
        cache_key = self._generate_cache_key(
            "matching",
            resource_type.value,
            specialty.value if specialty else "",
            str(location),
            str(preferred_time),
            str(symptoms),
        )
        ...
        try:
            ...
            # 计算总分
            total_score = (
                np.mean([m.match_score for m in sorted_matches])
                if sorted_matches
                else 0.0
            )
    ...
    async def _evaluate_doctor_match(
        self, doctor: Doctor, request: ResourceRequest
    ) -> ResourceMatch:
        """评估医生匹配度"""
        scores = {}
        reasons = []
        ...
        if request.budget_range:
            ...
            if min_budget <= doctor.consultation_fee <= max_budget:
                ...
                scores["cost"] = max(
                    0, 1.0 - (doctor.consultation_fee - max_budget) / max_budget
                )
        else:
            scores["cost"] = 0.8
        ...
        # 获取最近可用时间
        available_time = await self._get_next_available_time(
            doctor, request.preferred_time
        )
    ...
    async def _check_doctor_availability(
        self, doctor: Doctor, preferred_time: Optional[datetime]
    ) -> float:
        """检查医生可用性"""
        ...
        time_window = timedelta(
            hours=self.enhanced_config["matching"]["time_window_hours"]
        )
    ...
    async def _match_symptoms_to_skills(
        self, symptoms: List[str], skills: List[str]
    ) -> float:
        """匹配症状与技能"""
    ...
    async def _evaluate_hospital_match(
        self, hospital: Hospital, request: ResourceRequest
    ) -> ResourceMatch:
        """评估医院匹配度"""
        scores = {}
        reasons = []
    ...
    async def _evaluate_equipment_match(
        self, equipment: Equipment, request: ResourceRequest
    ) -> ResourceMatch:
        """评估设备匹配度"""
        scores = {}
        reasons = []
    ...
    async def _check_equipment_availability(
        self, equipment: Equipment, preferred_time: Optional[datetime]
    ) -> float:
        """检查设备可用性"""
        if not preferred_time:
            return 0.8
    ...
    async def _evaluate_medicine_match(
        self, medicine: Medicine, request: ResourceRequest
    ) -> ResourceMatch:
        """评估药品匹配度"""
        scores = {}
        reasons = []
    ...
    async def _rank_matches(
        self, matches: List[ResourceMatch], request: ResourceRequest
    ) -> List[ResourceMatch]:
        """排序匹配结果"""
        # 按匹配分数排序
    ...
    async def _generate_resource_recommendations(
        ...
        if request.budget_range:
            avg_cost = np.mean(
                [m.estimated_cost for m in matches if m.estimated_cost > 0]
            )
            min_budget, max_budget = request.budget_range
            if avg_cost > max_budget:
                recommendations.append(
                    "平均费用超出预算，建议调整预算或选择更经济的选项"
                )
            ...
            try:
                ...
                for doctor in self.doctors.values():
                    ...
                    doctor.available_slots = [
                        slot for slot in doctor.available_slots if slot.end_time > now
                    ]
                ...
                for equipment in self.equipment.values():
                    ...
                    equipment.booking_slots = [
                        slot for slot in equipment.booking_slots if slot.end_time > now
                    ]
    ...
    async def _set_to_cache(
        self, key: str, value: Any, ttl_type: str = "match_results"
    ):
        """设置缓存"""
        if not self.enhanced_config["caching"]["enabled"]:
            return
