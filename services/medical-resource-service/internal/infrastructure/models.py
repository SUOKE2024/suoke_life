...
class MedicalResource(Base):
    ...
    quality_level = Column(
        Enum(QualityLevel), default=QualityLevel.AVERAGE, comment="质量等级"
    )
...
class DoctorResource(Base):
    ...
    id = Column(
        UUID(as_uuid=True), ForeignKey("medical_resources.id"), primary_key=True
    )
...
class Appointment(Base):
    ...
    resource_id = Column(
        UUID(as_uuid=True), ForeignKey("medical_resources.id"), nullable=False
    )
    ...
    # 状态信息
    status = Column(
        Enum(AppointmentStatus), default=AppointmentStatus.PENDING, comment="预约状态"
    )
    ...
    quality_assessment = relationship(
        "QualityAssessment", back_populates="appointment", uselist=False
    )
