...
class PostgreSQLConnection(DatabaseConnection):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"PostgreSQL连接池创建成功: {self.config.host}:{self.config.port}"
            )
            return True
...
class MongoDBConnection(DatabaseConnection):
    ...
    async def execute_query(
        self, collection: str, operation: str, *args, **kwargs
    ) -> Any:
        """执行MongoDB操作"""
...
class DatabasePoolManager:
    ...
    async def get_metrics(self) -> Dict[str, ConnectionMetrics]:
        """获取所有连接的指标"""
        return {
            name: connection.metrics for name, connection in self.connections.items()
        }
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        for name, connection in self.connections.items():
            status[name] = {
                ...
                "last_error": (
                    str(connection.last_error) if connection.last_error else None
                ),
            }
