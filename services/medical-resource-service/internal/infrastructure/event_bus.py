...
class EventBus:
    ...
    async def stop_workers(self):
        ...
        # 等待任务完成
        await asyncio.gather(*self.worker_tasks, self.retry_task, return_exceptions=True)
                ...
                if self.event_store:
                    ...
                    for event in failed_events:
                        ...
                        if event.retry_count < event.max_retries:
                            ...
                            # 添加到队列
                            await self.event_queue.put(event)
                            logger.info(f"事件重试: {event.id} (第{event.retry_count}次)")
...
async def init_event_bus(config: Dict[str, Any], event_store: Optional[EventStore] = None) -> EventBus:
    """初始化全局事件总线"""
    global _event_bus
