...
class MedicalResourceCoordinator:
    ...
    async def _initialize_services(self):
        ...
        try:
            ...
            # 初始化小克智能体
            self.xiaoke_agent = XiaokeAgent(
                self.config.get("xiaoke_agent", {})
            )
    ...
    async def create_comprehensive_health_plan(
        ...
        """创建综合健康方案"""
        try:
            plan_id = f"health_plan_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{user_id}"
            
            # 获取用户体质类型
            constitution_type = health_assessment.get("constitution_type")
            if not constitution_type:
                constitution_type = await self._assess_constitution(user_id, health_assessment)
    ...
    async def _handle_consultation_request(self, request: ServiceRequest) -> Dict[str, Any]:
        """处理咨询请求"""
        try:
            ...
            if constitution_type:
                ...
                consultation_result["resource_recommendations"] = resource_recommendations
    ...
    async def _handle_appointment_request(self, request: ServiceRequest) -> Dict[str, Any]:
        """处理预约请求"""
        try:
            user_id = request.parameters.get("user_id")
            ...
            # 创建预约请求
            appointment_request_id = await self.scheduling_service.create_appointment_request(
                user_id=user_id,
            ...
            # 如果预约成功，获取详细信息
            if appointment_id:
                appointment_details = await self.scheduling_service.get_appointment_details(
                    appointment_id
                )
    ...
    async def _handle_treatment_plan_request(self, request: ServiceRequest) -> Dict[str, Any]:
        """处理治疗方案请求"""
        try:
            user_id = request.parameters.get("user_id")
            symptoms = request.parameters.get("symptoms", [])
            ...
            # 推荐诊断检查
            diagnostic_recommendations = await self.medical_service.recommend_diagnostics(
    ...
    async def _handle_resource_search_request(self, request: ServiceRequest) -> Dict[str, Any]:
        """处理资源搜索请求"""
        try:
    ...
    async def _handle_quality_assessment_request(self, request: ServiceRequest) -> Dict[str, Any]:
        """处理质量评估请求"""
        try:
            resource_id = request.parameters.get("resource_id")
            ...
            if resource_id:
                assessment = await self.quality_service.assess_resource_quality(resource_id)
            elif service_id:
                assessment = await self.quality_service.assess_service_quality(service_id)
            else:
    ...
    async def _handle_health_monitoring_request(self, request: ServiceRequest) -> Dict[str, Any]:
        """处理健康监测请求"""
        try:
            user_id = request.parameters.get("user_id")
            ...
            if analysis_result.get("anomalies"):
                resource_recommendations = await self._recommend_resources_for_anomalies(
