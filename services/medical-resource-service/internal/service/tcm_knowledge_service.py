...
class TCMKnowledgeService:
    ...
    def _initialize_common_herbs(self):
        ...
        self.herbs["甘草"] = TCMHerb(
            ...
            properties=[TCMProperty.SWEET, TCMProperty.NEUTRAL],
            meridians=[MeridianType.HEART, MeridianType.LUNG, MeridianType.SPLEEN, MeridianType.STOMACH],
        ...
        self.herbs["柴胡"] = TCMHerb(
            ...
            meridians=[MeridianType.LIVER, MeridianType.GALLBLADDER, MeridianType.TRIPLE_HEATER],
    ...
    async def search_knowledge(
        ...
        try:
            ...
            if not category or category == TCMCategory.FORMULA:
                for formula_name, formula in self.formulas.items():
                    if self._match_query(query, formula.name, *formula.alias_names, *formula.indications):
                        if not constitution or constitution in formula.related_constitutions:
                            results.append({
                                "type": "formula",
                                ...
                                "relevance_score": self._calculate_relevance_score(query, formula)
                            })
            
            # 搜索中药
            if not category or category == TCMCategory.HERB:
                for herb_name, herb in self.herbs.items():
                    if self._match_query(query, herb.name, *herb.alias_names, *herb.functions):
                        results.append({
                            "type": "herb",
                            "id": herb.herb_id,
                            "name": herb.name,
                            ...
                            "relevance_score": self._calculate_relevance_score(query, herb)
                        })
            
            # 搜索穴位
            if not category or category == TCMCategory.ACUPOINT:
                for acupoint_name, acupoint in self.acupoints.items():
                    if self._match_query(query, acupoint.name, acupoint.code, *acupoint.indications):
                        results.append({
                            "type": "acupoint",
                            ...
                            "relevance_score": self._calculate_relevance_score(query, acupoint)
                        })
            
            # 搜索证候
            if not category or category == TCMCategory.SYNDROME:
                for syndrome_name, syndrome in self.syndromes.items():
                    if self._match_query(query, syndrome.name, *syndrome.clinical_manifestations):
                        if not constitution or constitution in syndrome.related_constitutions:
                            results.append({
                                "type": "syndrome",
                                ...
                                "relevance_score": self._calculate_relevance_score(query, syndrome)
                            })
        ...
        for formula in self.formulas.values():
            if formula.formula_id == formula_id:
                return {
                    ...
                    "related_constitutions": [c.value for c in formula.related_constitutions],
    ...
    async def recommend_treatment(
        ...
        try:
            ...
            # 推荐方剂
            constitution_formulas = self.constitution_formula_index.get(constitution_type, [])
            for formula_name in constitution_formulas:
                formula = self.formulas[formula_name]
                match_score = self._calculate_symptom_match(symptoms, formula.indications)
                if match_score > 0.3:
            ...
            # 生成生活建议
            recommendations["lifestyle_advice"] = self._generate_lifestyle_advice(constitution_type)
            
            # 生成治疗原则
            if syndrome and syndrome in self.syndromes:
                syndrome_obj = self.syndromes[syndrome]
                recommendations["treatment_principles"] = syndrome_obj.treatment_principles
    ...
    def _calculate_symptom_match(self, symptoms: List[str], indications: List[str]) -> float:
        """计算症状匹配度"""
    ...
    def _generate_lifestyle_advice(self, constitution_type: ConstitutionType) -> List[str]:
        """生成生活建议"""
        advice_map = {
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "herb_categories": list(set(
                herb.source.split("科")[0] + "科" if "科" in herb.source else "其他"
                for herb in self.herbs.values()
            )),
            "meridian_coverage": list(set(
                meridian.value for acupoint in self.acupoints.values()
                for meridian in [acupoint.meridian]
            )),
            "syndrome_categories": list(set(
                syndrome.category for syndrome in self.syndromes.values()
            ))
        } 