...
class QualityControlService:
    ...
    async def record_quality_metric(
        ...
        """记录质量指标"""
        try:
            metric_id = f"metric_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{resource_id}"
    ...
    async def collect_user_feedback(
        ...
        try:
            ...
            # 分类标签
            categories = await self._categorize_feedback(comment, rating) if comment else []
    ...
    async def evaluate_resource_quality(
        self,
        resource_id: str,
        evaluation_period: timedelta = None
    ) -> Dict[str, Any]:
        """评估资源质量"""
        try:
            ...
            # 服务质量分数
            if results:
                quality_scores["service_quality"] = statistics.mean([r.quality_score for r in results])
            ...
            # 可用性分数
            availability_metrics = [m for m in metrics if m.metric_type == QualityMetricType.RESOURCE_AVAILABILITY]
            if availability_metrics:
                quality_scores["availability"] = statistics.mean([m.value for m in availability_metrics])
            
            # 响应时间分数
            response_metrics = [m for m in metrics if m.metric_type == QualityMetricType.RESPONSE_TIME]
            if response_metrics:
    ...
    async def generate_quality_report(
        ...
        try:
            ...
            for resource_id in target_resources:
                all_metrics.extend(self._get_metrics_in_period(resource_id, start_time, end_time))
                all_feedback.extend(self._get_feedback_in_period(resource_id, start_time, end_time))
                all_results.extend(self._get_results_in_period(resource_id, start_time, end_time))
            
            # 计算整体指标
            overall_metrics = {}
            
            if all_results:
                overall_metrics["average_service_quality"] = statistics.mean([r.quality_score for r in all_results])
                overall_metrics["service_success_rate"] = sum(1 for r in all_results if r.success) / len(all_results) * 100
            ...
            # 识别问题
            issues = await self._identify_quality_issues(all_metrics, all_feedback, all_results)
            
            # 生成建议
            recommendations = await self._generate_report_recommendations(overall_metrics, issues)
    ...
    async def get_quality_dashboard(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for resource_id in self.quality_metrics.keys():
                recent_metrics.extend(self._get_metrics_in_period(resource_id, cutoff_time, datetime.now()))
                recent_feedback.extend(self._get_feedback_in_period(resource_id, cutoff_time, datetime.now()))
                recent_results.extend(self._get_results_in_period(resource_id, cutoff_time, datetime.now()))
            
            # 活跃预警
            active_alerts = [alert for alert in self.quality_alerts if not alert.is_resolved]
            
            # 进行中的改进计划
            active_plans = [plan for plan in self.improvement_plans.values() 
            ...
            dashboard = {
                "overview": {
                    ...
                    "average_quality_score": self.quality_stats["average_quality_score"],
                ...
                "improvement_success_rate": self.quality_stats["improvement_success_rate"],
                "alerts": [
                    {
    ...
    def _get_results_in_period(
        ...
        """获取时间段内的服务结果"""
        results = [r for r in self.service_results.values() if r.resource_id == resource_id]
    ...
    def _update_quality_trends(self, resource_id: str, metric_type: QualityMetricType, value: float):
        """更新质量趋势"""
        ...
        if len(self.quality_stats["quality_trends"][trend_key]) > 100:
            self.quality_stats["quality_trends"][trend_key] = \
                self.quality_stats["quality_trends"][trend_key][-100:]
    ...
    async def _generate_report_recommendations(
        ...
        # 基于整体表现生成建议
        if "average_service_quality" in metrics and metrics["average_service_quality"] < 80:
    ...
    async def _get_top_quality_issues(self) -> List[Dict[str, Any]]:
        ...
        # 统计预警频率
        alert_counts = defaultdict(int)
        recent_alerts = [a for a in self.quality_alerts 
        ...
        # 转换为问题列表
        for metric_type, count in sorted(alert_counts.items(), key=lambda x: x[1], reverse=True)[:5]:
            issues.append({
