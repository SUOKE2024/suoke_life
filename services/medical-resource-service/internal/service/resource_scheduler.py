...
from ..domain.models import (
    ...
    Resource, Doctor, MedicalInstitution, Equipment, Appointment
)
...
class ResourceScheduler:
    ...
    async def update_resource_status(self, resource_id: str, status: ResourceStatus) -> bool:
        """更新资源状态"""
        try:
    ...
    async def submit_scheduling_request(self, request: SchedulingRequest) -> str:
        ...
        try:
            ...
            self.metrics.constitution_distribution[request.constitution_type] = \
                self.metrics.constitution_distribution.get(request.constitution_type, 0) + 1
    ...
    def _calculate_priority(self, request: SchedulingRequest) -> int:
        ...
        # 截止时间
        if request.deadline:
            time_to_deadline = (request.deadline - datetime.now()).total_seconds() / 3600
    ...
    async def _round_robin_scheduling(self) -> List[SchedulingResult]:
        ...
        results = []
        available_resources = [
            rid for rid, status in self.resource_status.items()
        ...
        while self.scheduling_queue:
            ...
            if available_resources:
                # 轮转分配资源
                resource_id = available_resources[resource_index % len(available_resources)]
        ...
        while self.scheduling_queue:
            ...
            if best_resource:
                ...
                alternatives = await self._generate_alternative_options(
                    request,
                    exclude_resource=best_resource.resource_id
                )
                ...
                # 更新资源负载
                await self._update_resource_load(best_resource.resource_id, request.duration_minutes)
    ...
    async def _find_best_resource_for_constitution(
        ...
        constitution_weights = self.constitution_resource_weights.get(constitution_type, {})
    ...
    async def _find_least_loaded_resource(
        ...
        for resource_id, resource in self.resource_pool.items():
            ...
            if hasattr(resource, 'specialties') and required_specialties:
                if not any(spec in resource.specialties for spec in required_specialties):
                    continue
    ...
    async def _calculate_optimal_time(
        ...
        """计算最优调度时间"""
        # 简化实现：返回首选时间或最早可用时间
        earliest_time = await self._calculate_earliest_available_time(resource, duration_minutes)
    ...
    async def _generate_alternative_options(
        ...
        for resource_id, resource in self.resource_pool.items():
            ...
            alternative = {
                ...
                "estimated_wait": (available_time - datetime.now()).total_seconds() / 60
            }
    ...
    async def _update_metrics(self, results: List[SchedulingResult]):
        ...
        for result in results:
            if result.scheduled_time:
                # 假设请求创建时间作为等待开始时间
                wait_time = (result.scheduled_time - result.created_at).total_seconds() / 60
        ...
        if self.metrics.total_requests > 0:
            success_rate = self.metrics.successful_allocations / self.metrics.total_requests
        ...
        if self.metrics.resource_utilization:
            ...
            self.metrics.optimization_score = max(0, 1 - variance)  # 方差越小，优化分数越高
    ...
    async def _simulate_strategy(self, strategy: SchedulingStrategy) -> Dict[str, float]:
        """模拟调度策略效果"""
        # 简化的模拟实现
        ...
        base_scores = {
            ...
            SchedulingStrategy.PRIORITY: {"success_rate": 0.8, "optimization_score": 0.7},
            SchedulingStrategy.SHORTEST_JOB: {"success_rate": 0.75, "optimization_score": 0.65},
            SchedulingStrategy.ROUND_ROBIN: {"success_rate": 0.72, "optimization_score": 0.8},
            SchedulingStrategy.CONSTITUTION_BASED: {"success_rate": 0.85, "optimization_score": 0.9},
            SchedulingStrategy.LOAD_BALANCING: {"success_rate": 0.78, "optimization_score": 0.85}
        }
        
        return base_scores.get(strategy, {"success_rate": 0.5, "optimization_score": 0.5})
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "resource_types": list(set(
                resource.resource_type.value for resource in self.resource_pool.values()
            )),
            "scheduling_strategies": [strategy.value for strategy in SchedulingStrategy],
