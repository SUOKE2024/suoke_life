...
class FoodAgricultureService:
    ...
    def _initialize_constitution_food_mapping(self) -> Dict[ConstitutionType, Dict[str, List[str]]]:
        """初始化体质-食物映射"""
        mapping = {
    ...
    def _initialize_seasonal_recommendations(self) -> Dict[SeasonType, Dict[str, List[str]]]:
        """初始化季节性食材推荐"""
        recommendations = {
            SeasonType.SPRING: {
                "vegetables": ["韭菜", "菠菜", "芹菜", "豆苗", "春笋", "荠菜", "马兰头"],
            ...
            SeasonType.SUMMER: {
                "vegetables": ["冬瓜", "苦瓜", "丝瓜", "黄瓜", "茄子", "番茄", "绿豆芽"],
        ...
        try:
            ...
            for plan in self.therapy_plans.values():
                if plan.target_constitution == constitution_type:
                    symptom_match = any(symptom in plan.target_symptoms for symptom in symptoms)
                    if symptom_match or not symptoms:
                        matching_plans.append(plan)
            
            if matching_plans:
                # 选择最匹配的方案
                best_plan = max(matching_plans, key=lambda p: len(set(p.target_symptoms) & set(symptoms)))
    ...
    async def get_seasonal_ingredients(
        ...
        try:
            ...
            for product in self.agricultural_products.values():
                if season in [SeasonType.SPRING, SeasonType.SUMMER] and product.planting_date.month in [3, 4, 5, 6]:
                ...
                elif season in [SeasonType.AUTUMN, SeasonType.WINTER] and product.harvest_date and product.harvest_date.month in [9, 10, 11, 12]:
                    seasonal_products.append({
                        "name": product.name,
                        "variety": product.variety,
                        "region": product.growing_region,
    ...
    async def analyze_nutrition(
        ...
        try:
            # 计算每日热量需求
            daily_calories = self._calculate_daily_calorie_needs(age, gender, activity_level)
            
            # 计算宏量营养素比例
            macro_ratios = self._calculate_macro_nutrient_ratios(constitution_type)
            
            # 计算微量营养素需求
            micro_needs = self._calculate_micro_nutrient_needs(age, gender, constitution_type)
    ...
    async def get_planting_guidance(
        ...
        try:
            ...
            if not matching_guidance:
                return {"error": f"未找到 {crop_name} 在 {region or '当前地区'} {season.value} 季节的种植指导"}
        ...
        try:
            ...
            for product in self.agricultural_products.values():
                ...
                results.append({
                    ...
                    "nutritional_highlights": self._get_nutritional_highlights(product.nutritional_analysis),
                    "safety_info": {
    ...
    def _calculate_daily_calorie_needs(
        ...
        if gender.lower() == "male":
            bmr = 88.362 + (13.397 * 70) + (4.799 * 170) - (5.677 * age)  # 假设体重70kg，身高170cm
        else:
            bmr = 447.593 + (9.247 * 60) + (3.098 * 160) - (4.330 * age)  # 假设体重60kg，身高160cm
    ...
    def _calculate_macro_nutrient_ratios(self, constitution_type: ConstitutionType) -> Dict[str, float]:
        """计算宏量营养素比例"""
        # 基于体质的营养素比例调整
    ...
    def _get_recommended_foods_for_nutrition(
        ...
        # 过滤过敏食物
        if allergies:
            recommended = [food for food in recommended if not any(allergen in food for allergen in allergies)]
    ...
    def _get_meal_timing_suggestions(self, constitution_type: ConstitutionType) -> Dict[str, str]:
        """获取用餐时间建议"""
        base_timing = {
            "breakfast": "7:00-8:00",
            "lunch": "12:00-13:00",
    ...
    def _calculate_hydration_needs(self, age: int = None, activity_level: str = "moderate") -> float:
        """计算水分需求（升/天）"""
    ...
    def _get_nutritional_highlights(self, nutritional_info: NutritionalInfo) -> List[str]:
        """获取营养亮点"""
        highlights = []
        ...
        if nutritional_info.antioxidants:
            highlights.append(f"富含抗氧化物质: {', '.join(nutritional_info.antioxidants[:3])}")
