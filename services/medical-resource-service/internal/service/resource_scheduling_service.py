...
from .resource_management_service import (
    ResourceManagementService, ResourceCategory, AvailabilityStatus,
    QualityGrade, DoctorLevel, InstitutionType, EquipmentType
)
...
class ResourceSchedulingService:
    ...
    def __init__(self, config: Dict[str, Any], resource_service: ResourceManagementService):
        self.config = config
        ...
        try:
            ...
            for doctor_id, doctor in self.resource_service.tcm_doctors.items():
                current_appointments = [
                    apt for apt in self.scheduled_appointments.values()
                    if apt.resource_id == doctor_id and apt.status == AppointmentStatus.CONFIRMED
                ]
                
                current_load = len(current_appointments)
                max_capacity = 20  # 假设每天最多20个预约
                utilization_rate = current_load / max_capacity if max_capacity > 0 else 0
                
                # 计算平均等待时间
                wait_times = []
                for apt in current_appointments:
                    wait_time = (apt.scheduled_time - apt.created_at).total_seconds() / 3600
                ...
                load = ResourceLoad(
                    ...
                    queue_length=len([req for req in self.pending_requests 
                                    if req.preferred_resource_category == ResourceCategory.TCM_DOCTOR]),
            ...
            # 更新设备负载
            for equipment_id, equipment in self.resource_service.medical_equipment.items():
                utilization_rate = equipment.usage_statistics.get("utilization_rate", 0.5)
        ...
        try:
            ...
            for appointment in self.scheduled_appointments.values():
                if appointment.status == AppointmentStatus.CONFIRMED:
                    ...
                    end_time = appointment.scheduled_time + timedelta(minutes=appointment.duration_minutes)
    ...
    async def _redistribute_load(self, high_load_resources: List[str], low_load_resources: List[str]):
        """重新分配负载"""
        try:
            # 查找可以重新分配的预约
            ...
            for appointment in self.scheduled_appointments.values():
                if (appointment.resource_id in high_load_resources and
                    appointment.status == AppointmentStatus.CONFIRMED and
                    appointment.scheduled_time > datetime.now() + timedelta(hours=24)):
            ...
            for appointment in redistributable_appointments[:5]:  # 限制重新分配数量
                for low_load_resource_id in low_load_resources:
                    ...
                    if self._is_resource_compatible(appointment, low_load_resource_id):
                        # 尝试重新调度
                        success = await self._reschedule_appointment(appointment, low_load_resource_id)
                        if success:
                            logger.info(f"成功重新分配预约 {appointment.appointment_id} 到资源 {low_load_resource_id}")
                            break
            
        except Exception as e:
            logger.error(f"重新分配负载失败: {e}")
    
    def _is_resource_compatible(self, appointment: ScheduledAppointment, resource_id: str) -> bool:
        """检查资源是否兼容"""
        try:
            # 检查资源类型
    ...
    async def _reschedule_appointment(self, appointment: ScheduledAppointment, new_resource_id: str) -> bool:
        """重新调度预约"""
        try:
            # 检查新资源的可用性
    ...
    async def submit_appointment_request(self, request: AppointmentRequest) -> Dict[str, Any]:
        """提交预约请求"""
        try:
            # 添加到待处理队列
    ...
    def _calculate_priority_score(self, request: AppointmentRequest) -> float:
        ...
        try:
            ...
            # 等待时间权重
            wait_time_hours = (datetime.now() - request.created_at).total_seconds() / 3600
            wait_score = min(wait_time_hours / 24.0, 1.0)  # 最多24小时满分
            score += wait_score * 0.3
            
            # 体质匹配权重
            constitution_score = self._get_constitution_priority(request.constitution_type)
    ...
    def _assess_symptom_severity(self, symptoms: List[str]) -> float:
        ...
        severe_count = sum(1 for symptom in symptoms if any(s in symptom for s in severe_symptoms))
        moderate_count = sum(1 for symptom in symptoms if any(s in symptom for s in moderate_symptoms))
    ...
    async def _schedule_request(self, request: AppointmentRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            # 创建预约
            appointment = await self._create_appointment(request, best_resource, best_time_slot)
    ...
    async def _select_best_resource(self, request: AppointmentRequest, resources: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """选择最佳资源"""
        try:
            if not resources:
                return None
    ...
    async def _multi_criteria_selection(self, request: AppointmentRequest, resources: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """多准则决策选择"""
        try:
            best_resource = None
            best_score = -1
            ...
            for resource in resources:
                ...
                elif request.preferred_resource_category == ResourceCategory.MEDICAL_EQUIPMENT:
                ...
                elif request.preferred_resource_category == ResourceCategory.MODERN_MEDICAL_INSTITUTION:
    ...
    async def _load_balanced_selection(self, request: AppointmentRequest, resources: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """负载均衡选择"""
        try:
    ...
    async def _constitution_optimized_selection(self, request: AppointmentRequest, resources: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """体质优化选择"""
        try:
            constitution_weights = self.constitution_weights.get(request.constitution_type, {})
    ...
    async def _priority_based_selection(self, request: AppointmentRequest, resources: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """基于优先级选择"""
        try:
            # 根据紧急程度选择资源
            if request.urgency_level == UrgencyLevel.URGENT:
                ...
                # 非紧急情况选择最佳匹配
                return max(resources, key=lambda r: r.get("match_score", 0)) if resources else None
            
        except Exception as e:
            logger.error(f"基于优先级选择失败: {e}")
            return None
    
    async def _weighted_round_robin_selection(self, request: AppointmentRequest, resources: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """加权轮询选择"""
        try:
            # 简化的加权轮询实现
            weights = []
    ...
    async def _constitution_aware_selection(self, request: AppointmentRequest, resources: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """体质感知选择"""
        try:
            # 结合体质匹配和负载均衡
            best_resource = None
    ...
    async def _find_best_time_slot(self, request: AppointmentRequest, resource: Dict[str, Any]) -> Optional[datetime]:
        """查找最佳时间段"""
        try:
            resource_id = resource["resource_id"]
            ...
            for slot in preferred_slots:
                ...
                # 估算持续时间
                duration_minutes = self._estimate_duration(request.preferred_resource_category)
            ...
            # 如果偏好时间都不可用，查找最近可用时间
            return await self._find_next_available_time(resource_id, request.preferred_resource_category)
    ...
    def _generate_default_time_slots(self) -> List[Dict[str, str]]:
        ...
        for day in range(7):
            ...
            # 上午时段
            morning_slot = {
                "start_time": date.replace(hour=9, minute=0, second=0, microsecond=0).isoformat(),
                "end_time": date.replace(hour=12, minute=0, second=0, microsecond=0).isoformat()
            }
            slots.append(morning_slot)
            
            # 下午时段
            afternoon_slot = {
                "start_time": date.replace(hour=14, minute=0, second=0, microsecond=0).isoformat(),
                "end_time": date.replace(hour=17, minute=0, second=0, microsecond=0).isoformat()
            }
    ...
    async def _find_next_available_time(self, resource_id: str, resource_category: ResourceCategory) -> Optional[datetime]:
        """查找下一个可用时间"""
        try:
            # 从明天开始查找
    ...
    async def _create_appointment(self, request: AppointmentRequest, resource: Dict[str, Any], scheduled_time: datetime) -> Optional[ScheduledAppointment]:
        """创建预约"""
        try:
        ...
        try:
            ...
            stats = {
                "scheduling_metrics": {
                    ...
                    "success_rate": (
                        self.scheduling_metrics.successful_schedules / self.scheduling_metrics.total_requests
                        if self.scheduling_metrics.total_requests > 0 else 0
                    ),
                ...
                "load_balancing": {
                    ...
                    "high_load_resources": len([
                        load for load in self.resource_loads.values()
                        if load.utilization_rate > 0.8
                    ]),
        ...
        try:
            ...
            for appointment in self.appointment_history[-100:]:  # 最近100个预约
                if appointment.status != AppointmentStatus.CANCELLED:
                    # 计算等待时间（从请求到调度的时间）
                    wait_time = (appointment.created_at - appointment.created_at).total_seconds() / 3600
                    wait_times.append(wait_time)
            ...
            if match_scores:
                self.scheduling_metrics.average_match_score = sum(match_scores) / len(match_scores)
    ...
    async def optimize_scheduling_strategy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 策略调整建议
            high_load_count = stats.get("load_balancing", {}).get("high_load_resources", 0)
