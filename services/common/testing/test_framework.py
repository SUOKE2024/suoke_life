...
class TestCase:
    ...
    async def run(self) -> TestResult:
        """运行测试用例"""
        result = TestResult(
            test_name=self.name, status=TestStatus.PENDING, start_time=datetime.now()
        )
...
class TestRunner:
    ...
    async def run_all(
        ...
        if parallel_suites:
            ...
            for (suite_name, _), suite_results in zip(tasks, results, strict=False):
...
class TestFramework:
    ...
    def __init__(self, name: str = "default"):
        ...
        self.global_setup: Callable | None = None
        self.global_teardown: Callable | None = None
...
def get_test_framework(name: str) -> TestFramework | None:
    """获取测试框架"""
