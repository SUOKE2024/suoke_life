...
def api_security(
    security_schemes: List[Dict[str, List[str]]]
):
    """
    API安全装饰器
...
def header_param(name: str, description: str = "", required: bool = False, **kwargs):
    """请求头参数装饰器"""
    return api_parameter(name, ParameterLocation.HEADER, description, required, **kwargs)


# 常用响应装饰器
def success_response(description: str = "成功", schema: Dict[str, Any] = None, **kwargs):
    """成功响应装饰器"""
    return api_response(200, description, schema=schema, **kwargs)


def created_response(description: str = "创建成功", schema: Dict[str, Any] = None, **kwargs):
    """创建成功响应装饰器"""
