...
class Message:
    ...
    def serialize(self) -> bytes:
        ...
        if self.format == MessageFormat.JSON:
            if isinstance(self.value, (dict, list)):
                ...
                return json.dumps({"value": self.value}, ensure_ascii=False).encode('utf-8')
    ...
    @classmethod
    def deserialize(cls, data: bytes, format: MessageFormat = MessageFormat.JSON) -> Any:
        """反序列化消息"""
...
class MessageQueue(ABC):
    ...
    async def start_consuming(self, handler: MessageHandler, topics: Union[str, List[str]]):
        """
        开始消费循环
