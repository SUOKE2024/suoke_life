...
from .message_queue import (
    MessageQueue, Message, MessageHandler,
    ProducerConfig, ConsumerConfig, MessageFormat
)
...
class RabbitMQMessageQueue(MessageQueue):
    ...
    async def create_topic(self, topic: str, partitions: int = 1, replication_factor: int = 1):
    ...
    async def send(self, message: Message) -> bool:
        ...
        try:
            ...
            rabbit_message = RabbitMessage(
                body=body,
                content_type="application/json" if message.format == MessageFormat.JSON else "text/plain",
    ...
    async def subscribe(self, topics: Union[str, List[str]], handler: MessageHandler):
        """订阅主题"""
        if not self.channel:
            raise RuntimeError("未连接到RabbitMQ")
        ...
        for topic in topics:
            try:
                ...
                # 创建队列
                queue_name = f"{self.consumer_config.group_id}.{topic}" if self.consumer_config else topic
                    ...
                    async for rabbit_message in queue:
                        ...
                        message = Message(
                            ...
                            value=Message.deserialize(
                                rabbit_message.body,
                                MessageFormat.JSON if rabbit_message.content_type == "application/json" else MessageFormat.TEXT
                            ),
                            headers=dict(rabbit_message.headers) if rabbit_message.headers else {},
    ...
    async def _consume_queue(self, queue: aio_pika.Queue, topic: str, handler: MessageHandler):
        """消费队列消息"""
        try:
            async for rabbit_message in queue:
                try:
