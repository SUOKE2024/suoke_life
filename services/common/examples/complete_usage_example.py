...
async def main():
    ...
    try:
        ...
        health_data = {
            ...
            'fields': {'heart_rate': 75, 'blood_pressure_sys': 120, 'blood_pressure_dia': 80},
        ...
        await metrics.record_histogram('request_duration_seconds', 0.123, {'endpoint': '/health'})
        ...
        logger.info(f"健康检查详情: {json.dumps(health_result, indent=2, ensure_ascii=False)}")
        ...
        # 生成API文档
        api_spec = await openapi_gen.generate_spec({
            'title': '索克生活健康管理平台API',
            'version': '1.0.0',
        ...
        # 1) 数据加密存储
        encrypted_health_data = await encryption.encrypt(json.dumps(user_health_data))
        ...
        # 5) 记录评估指标
        await metrics.increment_counter('health_assessments_total', {
            'user_type': 'patient',
