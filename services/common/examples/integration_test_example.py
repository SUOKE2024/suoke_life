...
async def test_load_balancer():
    ...
    from services.common.load_balancer import LoadBalancer, LoadBalancingStrategy, ServiceEndpoint
    from services.common.load_balancer.health_aware_balancer import HealthAwareLoadBalancer, AdaptiveLoadBalancer
...
async def test_health_checker():
    ...
    logger.info(f"CPU检查结果: {json.dumps({k: v.to_dict() for k, v in cpu_result.items()}, indent=2, ensure_ascii=False)}")
...
async def test_health_monitor():
    ...
    from services.common.health import HealthMonitor, AlertLevel
    ...
    async def alert_handler(alert):
        logger.warning(f"🚨 收到告警: [{alert.level.value}] {alert.component} - {alert.message}")
...
async def test_health_aggregator():
    ...
    from services.common.health import HealthAggregator, HealthStatus, AlertLevel
    ...
    logger.info(f"整体健康状态: {json.dumps(overall_health, indent=2, ensure_ascii=False)}")
    
    # 获取特定服务健康状态
    service_health = await aggregator.get_service_health('xiaoke-service')
    logger.info(f"小克服务健康状态: {json.dumps(service_health, indent=2, ensure_ascii=False)}")
    
    # 获取关键问题
    critical_issues = await aggregator.get_critical_issues()
    logger.info(f"关键问题: {json.dumps(critical_issues, indent=2, ensure_ascii=False)}")
    ...
    logger.info(f"不健康的服务: {json.dumps(unhealthy_services, indent=2, ensure_ascii=False)}")
...
async def test_integrated_scenario():
    ...
    from services.common.health import HealthChecker, HealthMonitor, HealthAggregator
    ...
    for i in range(20):
        ...
        endpoint = await load_balancer.select_endpoint(
            'xiaoai-service',
            LoadBalancingStrategy.HEALTH_AWARE if hasattr(LoadBalancingStrategy, 'HEALTH_AWARE') else LoadBalancingStrategy.ROUND_ROBIN,
        ...
        if endpoint:
            ...
            await health_aggregator.update_service_health('xiaoai-service', health_result)
