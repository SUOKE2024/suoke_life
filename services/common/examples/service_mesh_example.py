...
from ..service_mesh.mesh_manager import (
    ServiceMeshManager, MeshType, TrafficPolicyType, SecurityPolicyType
)
from ..service_mesh.istio_client import IstioClient, VirtualService, DestinationRule, Gateway
from ..service_mesh.linkerd_client import LinkerdClient, TrafficSplit, ServiceProfile
from ..service_mesh.envoy_config import EnvoyConfigManager, ClusterConfig, ListenerConfig

# 导入API文档生成组件
from ..api_docs.openapi_generator import OpenAPIGenerator, APIEndpoint, APIParameter, APIResponse
from ..api_docs.doc_decorators import api_doc, get_api, post_api, query_param, success_response
...
# 导入测试框架
from ..testing.test_framework import get_test_framework, TestCase, TestType
...
async def demo_service_mesh():
    ...
    await mesh_manager.configure_security_policy(
        service_name="xiaoai-service",
        policy_type=SecurityPolicyType.MTLS_STRICT
    )
...
async def demo_envoy_config():
    ...
    for service_name, hosts in health_services:
        cluster = envoy_manager.create_health_service_config(
    ...
    # 创建金丝雀路由配置
    canary_route = envoy_manager.create_canary_route_config(
        service_name="xiaoai",
    ...
    envoy_manager.save_config(
        file_path=str(config_dir / "suoke_envoy_config.yaml"),
        format="yaml"
    )
...
async def demo_api_docs():
    ...
    # 启动Swagger UI服务器
    swagger_server = SwaggerUIServer(
