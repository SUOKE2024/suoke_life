    ...
    def register_service(self, 
        ...
        try:
            ...
            if health_check_url:
                ...
                # 默认TCP检查
                check = consul.Check.tcp(f"{address}:{port}", interval="10s", timeout="3s")
            ...
            if success:
                ...
                logger.info(f"Service registered: {service_name} ({service_id}) at {address}:{port}")
...
def get_consul_client(consul_host: str = "localhost", consul_port: int = 8500) -> ConsulServiceRegistry:
    """获取Consul客户端单例"""
...
if __name__ == "__main__":
    ...
    for instance in instances:
        print(f"Found service: {instance.service_name} at {instance.address}:{instance.port}")
