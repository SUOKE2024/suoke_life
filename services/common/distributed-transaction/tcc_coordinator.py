...
class TCCCoordinator:
    ...
    def add_operation(self, transaction: TCCTransaction, operation: TCCOperation):
        ...
        self._record_event(transaction, "operation_added", {
    ...
    async def execute_transaction(
        self, 
        transaction_id: str,
        operations: Optional[List[TCCOperation]] = None
    ) -> bool:
        """
        执行TCC事务
        ...
        try:
            ...
            self._record_event(transaction, "transaction_confirmed", {
    ...
    async def _try_phase(self, transaction: TCCTransaction) -> bool:
        ...
        for operation in transaction.operations:
            ...
            if not success:
                self._record_event(transaction, "try_phase_failed", {
        ...
        self._record_event(transaction, "try_phase_completed", {
    ...
    async def _try_operation(self, operation: TCCOperation) -> bool:
        ...
        for attempt in range(operation.retry_count):
            ...
            try:
                logger.debug(f"Try操作: {operation.resource_name} (尝试 {attempt + 1}/{operation.retry_count})")
                
                # 执行Try
                reservation_id = await asyncio.wait_for(
                    resource.try_resource(operation.request),
                ...
                operation.error = TimeoutError(f"Try操作 {operation.resource_name} 超时")
    ...
    async def _confirm_phase(self, transaction: TCCTransaction) -> bool:
        ...
        if all_confirmed:
            ...
            self._record_event(transaction, "confirm_phase_completed", {
                "confirmed_count": len(transaction.confirmed_operations)
            })
            return True
        else:
            self._record_event(transaction, "confirm_phase_failed", {
                "results": str(results)
            })
            return False
    ...
    async def _confirm_operation(self, operation: TCCOperation) -> bool:
        ...
        try:
            logger.debug(f"Confirm操作: {operation.resource_name}, 预留ID: {operation.reservation_id}")
    ...
    async def _cancel_phase(self, transaction: TCCTransaction) -> bool:
        ...
        if cancel_tasks:
            ...
            self._record_event(transaction, "cancel_phase_completed", {
                "cancelled_count": cancelled_count,
                "total_count": len(cancel_tasks)
            })
    ...
    async def _cancel_operation(self, operation: TCCOperation) -> bool:
        ...
        try:
            logger.debug(f"Cancel操作: {operation.resource_name}, 预留ID: {operation.reservation_id}")
    ...
    def _record_event(self, transaction: TCCTransaction, event_type: str, data: Dict[str, Any]):
        """记录事件"""
        event = {
            "timestamp": time.time(),
            "type": event_type,
