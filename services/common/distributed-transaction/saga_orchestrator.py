...
class SagaOrchestrator:
        ...
        for attempt in range(step.retry_count + 1):
            try:
                ...
                result = await asyncio.wait_for(
                    client.execute_action(step.action, input_data),
                ...
                logger.error(f"Step failed: {step.step_id} (attempt {attempt + 1}), error: {e}")
                if attempt < step.retry_count:
    ...
    async def _compensate_step(self, saga: SagaTransaction, step: SagaStep):
        ...
        try:
            ...
            client = self.service_clients.get(step.service_name)
            if not client:
                logger.error(f"Service client not found for compensation: {step.service_name}")
                return
    ...
    async def retry_saga(self, saga_id: str) -> bool:
        ...
        try:
            saga = await self.get_saga_status(saga_id)
            if not saga or saga.status not in [SagaStatus.FAILED, SagaStatus.COMPENSATED]:
                return False
            ...
            for key in keys:
                ...
                if saga_data.get("status") in ["completed", "failed", "compensated"]:
                    ...
                    if completed_at:
                        ...
                        if datetime.utcnow() - completed_time > timedelta(hours=older_than_hours):
...
class HealthDiagnosisSaga:
    ...
    async def create_diagnosis_saga(self, user_id: str, diagnosis_data: Dict[str, Any]) -> str:
        """创建诊断Saga"""
        steps = [
            SagaStep(
