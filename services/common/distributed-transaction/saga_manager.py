...
class SagaManager:
    ...
    async def execute(self) -> bool:
        ...
        try:
            ...
            self._record_event("saga_completed", {
    ...
    async def _execute_step(self, step: SagaStep) -> bool:
        ...
        for attempt in range(step.retry_count):
            ...
            try:
                logger.info(f"执行步骤: {step.name} (尝试 {attempt + 1}/{step.retry_count})")
                ...
                self._record_event("step_completed", {
                    "step_name": step.name,
        ...
        self._record_event("step_failed", {
            "step_name": step.name,
    ...
    async def _compensate(self):
        ...
        self._record_event("compensation_started", {
        ...
        for step_name in reversed(self.completed_steps):
            ...
            try:
                ...
                self._record_event("compensation_completed", {
                    "step_name": step.name
                })
                
            except Exception as e:
                logger.error(f"补偿步骤 {step.name} 失败: {e}")
                self._record_event("compensation_failed", {
                    "step_name": step.name,
                    "error": str(e)
                })
        ...
        self._record_event("saga_compensated", {
...
class SagaOrchestrator:
    ...
    async def get_saga(self, saga_id: str) -> Optional[SagaManager]:
        """获取Saga实例"""
