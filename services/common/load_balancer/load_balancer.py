...
class LoadBalancer:
    ...
    async def select_endpoint(
        ...
        if selected_endpoint:
            ...
            logger.debug(
                f"选择端点: {selected_endpoint.address} (策略: {strategy.value})"
            )
    ...
    async def update_endpoint_health(
        self, service_name: str, endpoint_address: str, healthy: bool
    ):
        """更新端点健康状态"""
        if service_name in self.endpoints:
    ...
    async def get_all_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "available_strategies": [
                strategy.value for strategy in self.algorithms
            ],
        }
    ...
    async def health_check(self) -> dict[str, Any]:
        ...
        for _service_name, endpoints in self.endpoints.items():
