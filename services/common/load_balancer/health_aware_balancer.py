...
class HealthAwareLoadBalancer(LoadBalancingAlgorithm):
    ...
    async def select_endpoint(
        ...
        logger.debug(f"选择端点 {selected_endpoint.address}，得分: {scored_endpoints[0][1]:.3f}")
    ...
    async def get_endpoint_rankings(self, endpoints: List[ServiceEndpoint]) -> List[Dict[str, Any]]:
        """获取端点排名信息"""
        rankings = []
...
class AdaptiveLoadBalancer(LoadBalancingAlgorithm):
    ...
    async def _evaluate_strategies(self):
        ...
        for strategy_name, performance in self.strategy_performance.items():
            ...
            # 计算平均响应时间
            avg_response_time = performance['total_response_time'] / performance['request_count']
