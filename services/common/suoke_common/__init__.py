...
class SuokeCommonComponents:
    ...
    async def initialize(self, config: Optional[Dict[str, Any]] = None):
        ...
        logging.info(
            f"索克生活通用组件初始化完成，已初始化 {len(self._initialized_components)} 个组件"
        )
    ...
    async def _init_health_components(self):
        ...
        # 初始化健康聚合器
        await self._initialized_components["health_aggregator"].initialize(
            health_config
        )
    ...
    async def _init_observability_components(self):
        ...
        await self._initialized_components["metrics_collector"].initialize(
            metrics_config
        )
    ...
    async def _init_messaging_components(self):
        ...
        if "rabbitmq" in messaging_config:
            ...
            await self._initialized_components["rabbitmq_client"].initialize(
                rabbitmq_config
            )
