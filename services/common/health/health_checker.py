...
class HealthChecker:
    ...
    async def _register_default_checks(self):
        ...
        if 'database' in self.config:
            ...
            if 'postgresql' in db_config:
                self.register_check("database_postgresql", 
                                  lambda: self._check_postgresql(db_config['postgresql']))
            if 'mongodb' in db_config:
                self.register_check("database_mongodb",
                                  lambda: self._check_mongodb(db_config['mongodb']))
            if 'redis' in db_config:
                self.register_check("database_redis",
                                  lambda: self._check_redis(db_config['redis']))
        
        # HTTP服务检查
        if 'services' in self.config:
            for service_name, service_config in self.config['services'].items():
                self.register_check(f"service_{service_name}",
    ...
    async def get_overall_status(self, check_results: Dict[str, HealthCheckResult]) -> HealthStatus:
        """获取整体健康状态"""
