...
class HealthMonitor:
    ...
    async def _analyze_component_health(
        ...
        if status == 'unhealthy':
            ...
            # 检查连续失败次数
            if consecutive_failures[component_name] >= self.alert_thresholds['consecutive_failures']:
                await self._create_alert(
                    AlertLevel.ERROR,
                    component_name,
        ...
        if duration_ms > self.alert_thresholds['response_time_threshold'] * 1000:
            await self._create_alert(
                ...
                f'组件响应时间过长: {duration_ms:.1f}ms',
                {
                    ...
                    'threshold_ms': self.alert_thresholds['response_time_threshold'] * 1000
                }
            )
    ...
    async def _resolve_alert(self, component: str):
        ...
        for alert in resolved_alerts:
            logger.info(f"解决告警 [{alert.level.value}] {alert.component}: {alert.message}")
    ...
    def _calculate_health_trend(self) -> str:
        ...
        if len(self.health_history) >= 20:
            ...
            previous_healthy = len([h for h in previous_checks if h.get('status') == 'healthy'])
    ...
    async def get_health_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'monitoring_uptime': time.time() - self.health_history[0].get('timestamp', time.time()) if self.health_history else 0
        }
