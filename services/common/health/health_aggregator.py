...
from .health_monitor import HealthAlert, AlertLevel
...
class HealthAggregator:
    ...
    async def get_overall_health(self) -> Dict[str, Any]:
        ...
        stale_services = [
            service.service_name for service in self.services.values()
    ...
    def _analyze_dependencies(self) -> Dict[str, Any]:
        ...
        for service_name, dependencies in self.dependencies.items():
            ...
            # 分析依赖影响
            unhealthy_deps = [s for s in dependency_statuses if s == HealthStatus.UNHEALTHY]
            degraded_deps = [s for s in dependency_statuses if s == HealthStatus.DEGRADED]
            ...
            dependency_health[service_name] = {
                ...
                'potentially_affected': len(unhealthy_deps) > 0 or len(degraded_deps) > 0
            }
    ...
    async def get_service_health(self, service_name: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            **service.to_dict(),
            ...
            'is_critical': service_name in self.aggregation_rules.get('critical_services', [])
        }
    
    async def get_services_by_status(self, status: HealthStatus) -> List[Dict[str, Any]]:
        """获取指定状态的服务列表"""
        services = []
    ...
    async def get_critical_issues(self) -> Dict[str, Any]:
        ...
        for service in self.services.values():
            ...
            # 关键服务故障
            if service_name in critical_services and service.status == HealthStatus.UNHEALTHY:
            ...
            # 高告警服务
            critical_alerts = [a for a in service.alerts if a.level == AlertLevel.CRITICAL]
