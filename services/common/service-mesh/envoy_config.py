        ...
        config = {
            ...
            "load_assignment": {
                ...
                "endpoints": [
                    {
                        "lb_endpoints": [
                            {"endpoint": host} for host in self.hosts
                        ]
                    }
                ]
            }
...
class RouteConfig:
    ...
    def add_weighted_route(
        ...
        prefix: str,
        clusters: List[Dict[str, Any]]  # [{"name": "cluster1", "weight": 80}, {"name": "cluster2", "weight": 20}]
    ):
        """添加加权路由"""
        route = {
            "match": {"prefix": prefix},
...
class EnvoyConfigManager:
    ...
    def generate_config(self) -> Dict[str, Any]:
        ...
        config = {
            "static_resources": {
                "listeners": [
                    listener.to_envoy_config()
                    for listener in self.listeners.values()
                ],
                "clusters": [
                    cluster.to_envoy_config()
    ...
    def create_canary_route_config(
        ...
        route.add_virtual_host(
            name=f"{service_name}-vh",
            domains=[f"{service_name}.local", "*"],
            routes=[]
        )
