...
from .istio_client import IstioClient, VirtualService, DestinationRule, Gateway
from .linkerd_client import LinkerdClient, TrafficSplit, ServiceProfile
...
class ServiceMeshManager:
    ...
    async def _create_linkerd_canary(self, canary: CanaryDeployment) -> bool:
        ...
        traffic_split.add_backend(
            service=f"{canary.service_name}-stable",
            weight=100 - canary.canary_weight
        )
        
        traffic_split.add_backend(
            service=f"{canary.service_name}-canary",
            weight=canary.canary_weight
        )
