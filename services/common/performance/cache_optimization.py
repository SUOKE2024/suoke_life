...
# Prometheus 指标
cache_hits = Counter('cache_hits_total', 'Total cache hits', ['cache_level', 'cache_name'])
cache_misses = Counter('cache_misses_total', 'Total cache misses', ['cache_name'])
cache_operations = Histogram('cache_operation_duration_seconds', 'Cache operation duration', ['operation', 'cache_name'])
cache_size = Gauge('cache_size_items', 'Number of items in cache', ['cache_level', 'cache_name'])
...
class MultiLevelCache:
    ...
    def __init__(
        ...
        l1_size: int = 1000, 
        l2_ttl: int = 300,
        l2_client: Optional[aioredis.Redis] = None
    ):
