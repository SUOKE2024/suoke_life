...
class TestPerformance:
    ...
    @pytest.mark.asyncio
    async def test_single_session_performance(
        self, orchestrator, performance_patient_info, performance_diagnosis_inputs
    ):
        """测试单个会话的性能"""
        # 记录开始时间
        ...
        session_id = await orchestrator.create_diagnosis_session(
            patient_info=performance_patient_info, enabled_diagnoses=enabled_diagnoses
        )
        ...
        filtered_inputs = [
            inp
            for inp in performance_diagnosis_inputs
    ...
    async def test_parallel_vs_sequential_performance(
        ...
        sequential_patient_info = PatientInfo(
            patient_id=str(uuid.uuid4()), name="顺序测试患者", age=30, gender="女"
        )
