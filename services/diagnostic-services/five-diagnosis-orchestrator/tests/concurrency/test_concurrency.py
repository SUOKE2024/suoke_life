...
class TestConcurrency:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_diagnosis_execution(self, orchestrator, concurrency_diagnosis_inputs):
        """测试并发诊断执行"""
        concurrent_count = 8
        enabled_diagnoses = [DiagnosisType.INQUIRY, DiagnosisType.LOOK]
        ...
        for session_id in session_ids:
            filtered_inputs = [
                inp for inp in concurrency_diagnosis_inputs 
    ...
    @pytest.mark.asyncio
    async def test_mixed_concurrent_operations(self, orchestrator, concurrency_diagnosis_inputs):
        """测试混合并发操作"""
        ...
        for session_id in new_session_ids:
            for _ in range(3):  # 每个会话查询3次
                task = asyncio.create_task(
                    orchestrator.get_session_status(session_id)
                )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_session_cancellation(self, orchestrator, concurrency_diagnosis_inputs):
        """测试并发会话取消"""
        session_count = 6
        ...
        for i in range(cancel_count):
            task = asyncio.create_task(
                orchestrator.cancel_session(session_ids[i])
            )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_result_retrieval(self, orchestrator, concurrency_diagnosis_inputs):
        """测试并发结果获取"""
        session_count = 5
        enabled_diagnoses = [DiagnosisType.INQUIRY, DiagnosisType.LOOK]
        ...
        for session_id in session_ids:
            for _ in range(3):  # 每个会话获取3次结果
                task = asyncio.create_task(
                    orchestrator.get_session_result(session_id)
                )
    ...
    @pytest.mark.asyncio
    async def test_race_condition_protection(self, orchestrator, concurrency_diagnosis_inputs):
        """测试竞态条件保护"""
        patient_info = PatientInfo(
        ...
        serious_exceptions = [
            r for r in results 
            if isinstance(r, Exception) and not isinstance(r, (ValueError, RuntimeError))
        ]
        assert len(serious_exceptions)==0, f"发现严重异常: {serious_exceptions}"
    
    @pytest.mark.asyncio
    async def test_resource_contention(self, orchestrator, concurrency_diagnosis_inputs):
        """测试资源竞争"""
    ...
    @pytest.mark.asyncio
    async def test_deadlock_prevention(self, orchestrator, concurrency_diagnosis_inputs):
        """测试死锁预防"""
        session_count = 8
        enabled_diagnoses = [DiagnosisType.INQUIRY, DiagnosisType.LOOK, DiagnosisType.CALCULATION]
    ...
    async def test_memory_consistency(self, orchestrator, concurrency_diagnosis_inputs):
        ...
        valid_transitions = {
            ...
            SessionStatus.RUNNING.value: [SessionStatus.COMPLETED.value, SessionStatus.FAILED.value],
        ...
        for session_id, statuses in session_status_map.items():
            ...
            for i in range(len(statuses) - 1):
                ...
                # 状态应该保持不变或者按照有效转换进行
                assert (current_status==next_status or 
                        next_status in valid_transitions.get(current_status, []))
