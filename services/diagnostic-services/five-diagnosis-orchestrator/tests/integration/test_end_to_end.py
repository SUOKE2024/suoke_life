...
class TestEndToEndDiagnosis:
    ...
    @pytest.mark.asyncio
    async def test_complete_diagnosis_workflow(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试完整诊断工作流程"""
        # 1. 创建诊断会话
        ...
        # 3. 启动诊断
        filtered_inputs = [
            inp for inp in sample_diagnosis_inputs 
        ...
        # 8. 验证建议
        assert len(result.treatment_recommendations) > 0 or len(result.lifestyle_recommendations) > 0
    
    @pytest.mark.asyncio
    async def test_parallel_execution_mode(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试并行执行模式"""
    ...
    @pytest.mark.asyncio
    async def test_sequential_execution_mode(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试顺序执行模式"""
    ...
    @pytest.mark.asyncio
    async def test_priority_based_execution(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试基于优先级的执行"""
    ...
    @pytest.mark.asyncio
    async def test_adaptive_execution_mode(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试自适应执行模式"""
    ...
    @pytest.mark.asyncio
    async def test_session_cancellation(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试会话取消"""
        # 创建会话
    ...
    @pytest.mark.asyncio
    async def test_multiple_concurrent_sessions(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试多个并发会话"""
        session_count = 3
        sessions = []
        ...
        for session_id in sessions:
            for _ in range(30):
                status = await orchestrator.get_session_status(session_id)
                if status["status"] in [SessionStatus.COMPLETED.value, SessionStatus.FAILED.value]:
                    break
                await asyncio.sleep(1)
    ...
    @pytest.mark.asyncio
    async def test_system_metrics(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试系统指标"""
        # 执行一个完整的诊断流程
        enabled_diagnoses = [DiagnosisType.INQUIRY, DiagnosisType.LOOK]
        session_id = await orchestrator.create_diagnosis_session(
    ...
    @pytest.mark.asyncio
    async def test_diagnosis_quality_assessment(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试诊断质量评估"""
        # 创建高质量的诊断会话（启用所有诊断类型）
    ...
    @pytest.mark.asyncio
    async def test_recommendation_generation(self, orchestrator, sample_patient_info, sample_diagnosis_inputs):
        """测试建议生成"""
        # 创建会话
