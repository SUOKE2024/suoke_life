...
from ..models.diagnosis_models import (
    FusedDiagnosisResult, PatientInfo, DiagnosisRecommendation
)
...
class DiagnosisDecisionEngine:
    ...
    async def generate_recommendations(
        ...
        try:
            ...
            # 过滤和排序建议
            filtered_recommendations = await self._filter_and_rank_recommendations(recommendations, context)
            
            # 按类型分组
            grouped_recommendations = self._group_recommendations_by_type(filtered_recommendations)
            
            # 转换为简单格式
            simple_recommendations = {}
            for rec_type, recs in grouped_recommendations.items():
                simple_recommendations[rec_type.value] = [rec.description for rec in recs]
    ...
    async def _generate_treatment_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成治疗建议"""
        recommendations = []
        ...
        if fused_result.primary_syndrome:
            ...
            for i, treatment in enumerate(syndrome_treatments[:3]):  # 取前3个治疗建议
                ...
                # 添加注意事项
                rec.precautions = await self.tcm_knowledge.get_treatment_precautions(treatment)
                rec.contraindications = await self.tcm_knowledge.get_treatment_contraindications(
    ...
    async def _generate_lifestyle_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成生活方式建议"""
        recommendations = []
        fused_result = context.fused_result
    ...
    async def _generate_diet_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成饮食建议"""
        recommendations = []
        fused_result = context.fused_result
        ...
        # 基于证型生成饮食建议
        if fused_result.primary_syndrome:
            diet_advice = await self.tcm_knowledge.get_syndrome_diet(fused_result.primary_syndrome)
        ...
        if fused_result.constitution_type:
            ...
            if constitution_diet:
                rec = RecommendationItem(
                    ...
                    implementation_steps=await self._get_constitution_diet_steps(constitution_diet),
                    duration="长期坚持",
                    confidence=0.8
                )
                recommendations.append(rec)
        ...
        # 基于季节生成饮食建议
        seasonal_diet = await self.tcm_knowledge.get_seasonal_diet(context.current_season)
        if seasonal_diet:
            rec = RecommendationItem(
                id="diet_seasonal",
                type=RecommendationType.DIET,
    ...
    async def _generate_exercise_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成运动建议"""
        recommendations = []
        fused_result = context.fused_result
    ...
    async def _generate_emotional_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成情志调节建议"""
        recommendations = []
        fused_result = context.fused_result
        ...
        if fused_result.primary_syndrome:
            ...
            if emotional_advice:
                rec = RecommendationItem(
                    ...
                    implementation_steps=await self._get_emotional_steps(emotional_advice),
                    duration="每日练习",
                    frequency="每次15-30分钟",
                    confidence=fused_result.overall_confidence * 0.7
                )
        ...
        # 基于体质生成情志调节建议
        if fused_result.constitution_type:
            constitution_emotional = await self.tcm_knowledge.get_constitution_emotional_care(
                fused_result.constitution_type
            )
            ...
            if constitution_emotional:
                rec = RecommendationItem(
                    ...
                    implementation_steps=await self._get_constitution_emotional_steps(constitution_emotional),
                    duration="长期坚持",
                    confidence=0.7
                )
                recommendations.append(rec)
    ...
    async def _generate_follow_up_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成随访建议"""
        recommendations = []
        fused_result = context.fused_result
        ...
        for risk_factor in fused_result.risk_factors:
            ...
            if risk_follow_up:
                rec = RecommendationItem(
                    ...
                    implementation_steps=await self._get_risk_monitoring_steps(risk_factor),
                    confidence=0.8
                )
                recommendations.append(rec)
    ...
    async def _generate_prevention_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成预防建议"""
        recommendations = []
        fused_result = context.fused_result
        patient_info = context.patient_info
        ...
        # 基于体质生成预防建议
        if fused_result.constitution_type:
            constitution_prevention = await self.tcm_knowledge.get_constitution_prevention(
                fused_result.constitution_type
            )
            ...
            if constitution_prevention:
                rec = RecommendationItem(
                    ...
                    implementation_steps=await self._get_constitution_prevention_steps(constitution_prevention),
                    duration="长期坚持",
                    confidence=0.7
                )
                recommendations.append(rec)
    ...
    async def _generate_emergency_recommendations(self, context: DecisionContext) -> List[RecommendationItem]:
        """生成紧急建议"""
        recommendations = []
        fused_result = context.fused_result
    ...
    async def _check_safety(self, recommendation: RecommendationItem, context: DecisionContext) -> bool:
        """检查建议安全性"""
        # 检查过敏史
        ...
        for medication in context.current_medications:
            if await self.tcm_knowledge.has_drug_interaction(recommendation.description, medication):
                return False
    ...
    async def _get_monitoring_indicators(self, fused_result: FusedDiagnosisResult) -> List[str]:
        """获取监测指标"""
        indicators = ["症状变化", "精神状态", "睡眠质量", "食欲情况"]
        
        if fused_result.primary_syndrome:
            syndrome_indicators = await self.tcm_knowledge.get_syndrome_monitoring_indicators(
                fused_result.primary_syndrome
            )
