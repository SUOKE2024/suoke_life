...
from ..models.diagnosis_models import (
    DiagnosisType, DiagnosisSession, DiagnosisResult, 
    FusedDiagnosisResult, PatientInfo, SessionStatus,
    DiagnosisInput, DiagnosisEvent
)
from ..config.settings import get_settings
from .fusion_engine import DiagnosisFusionEngine
from .decision_engine import DiagnosisDecisionEngine
from ..utils.exceptions import (
    OrchestrationError, ServiceUnavailableError, 
...
class OrchestrationConfig:
    ...
    diagnosis_priorities: Dict[DiagnosisType, int] = field(default_factory=lambda: {
...
class FiveDiagnosisOrchestrator:
    ...
    async def create_diagnosis_session(
        ...
        if len(enabled_diagnoses) < self.config.require_minimum_diagnoses:
            raise ValidationError(f"至少需要启用{self.config.require_minimum_diagnoses}种诊断类型")
        ...
        await self._publish_event(
            ...
            {"patient_id": patient_info.patient_id, "enabled_diagnoses": [d.value for d in enabled_diagnoses]}
        )
        
        logger.info(f"创建诊断会话: {session_id}, 启用诊断: {[d.value for d in enabled_diagnoses]}")
        return session_id
    
    async def start_diagnosis(
        self, 
        session_id: str, 
        diagnosis_inputs: List[DiagnosisInput]
    ) -> None:
        """启动诊断流程"""
        async with self.session_locks.get(session_id, asyncio.Lock()):
            ...
            try:
                ...
                session.total_duration = (session.completed_at - session.started_at).total_seconds()
                ...
                # 发布事件
                await self._publish_event(session_id, "diagnosis_completed", {
                    "duration": session.total_duration,
                    "results_count": len(session.diagnosis_results)
                })
                
                logger.info(f"诊断完成: {session_id}, 耗时: {session.total_duration:.2f}秒")
                ...
                await self._publish_event(session_id, "diagnosis_failed", {"error": str(e)})
    ...
    async def _execute_priority_based_diagnosis(
        ...
        for diagnosis_input in diagnosis_inputs:
            if diagnosis_input.diagnosis_type in session.enabled_diagnoses:
                priority = self.config.diagnosis_priorities.get(diagnosis_input.diagnosis_type, 999)
    ...
    async def _execute_single_diagnosis(
        ...
        try:
            # 获取服务实例
            service_client = await self.service_registry.get_service_client(diagnosis_type.value)
            ...
            # 发布事件
            await self._publish_event(
                session.session_id, "diagnosis_completed",
                {"diagnosis_type": diagnosis_type.value, "processing_time": processing_time}
            )
            
            logger.info(f"诊断完成: {diagnosis_type.value}, 耗时: {processing_time:.2f}秒")
    ...
    async def _execute_diagnosis_fusion(self, session: DiagnosisSession) -> None:
        ...
        try:
            ...
            # 发布事件
            await self._publish_event(
                session.session_id, "fusion_completed",
    ...
    async def _execute_decision_making(self, session: DiagnosisSession) -> None:
        ...
        try:
            ...
            recommendations = await self.decision_engine.generate_recommendations(
                session.fused_result,
                session.patient_info
            )
            
            # 更新融合结果中的建议
            session.fused_result.treatment_recommendations = recommendations.get("treatment", [])
            session.fused_result.lifestyle_recommendations = recommendations.get("lifestyle", [])
            session.fused_result.follow_up_recommendations = recommendations.get("follow_up", [])
            
            # 发布事件
            await self._publish_event(
                session.session_id, "decision_completed",
    ...
    async def get_session_result(self, session_id: str) -> Optional[FusedDiagnosisResult]:
        """获取会话结果"""
        session = self.active_sessions.get(session_id)
        if not session:
            raise ValidationError(f"会话不存在: {session_id}")
    ...
    async def get_system_metrics(self) -> Dict[str, Any]:
        ...
        for service_type in DiagnosisType:
            self.metrics["service_availability"][service_type.value] = service_type.value in available_services
        ...
        return {
            ...
            "active_sessions": len([s for s in self.active_sessions.values() if s.status==SessionStatus.RUNNING]),
    ...
    async def _register_event_handlers(self) -> None:
        ...
        await self.event_bus.subscribe("diagnosis_started", self._handle_diagnosis_started)
        await self.event_bus.subscribe("diagnosis_completed", self._handle_diagnosis_completed)
        await self.event_bus.subscribe("fusion_completed", self._handle_fusion_completed)
        await self.event_bus.subscribe("session_completed", self._handle_session_completed)
    ...
    async def _publish_event(self, session_id: str, event_type: str, data: Dict[str, Any]) -> None:
        """发布事件"""
    ...
    async def cleanup_completed_sessions(self, max_age_hours: int = 24) -> int:
        ...
        for session_id, session in self.active_sessions.items():
            if (session.status in [SessionStatus.COMPLETED, SessionStatus.FAILED, SessionStatus.CANCELLED] and
