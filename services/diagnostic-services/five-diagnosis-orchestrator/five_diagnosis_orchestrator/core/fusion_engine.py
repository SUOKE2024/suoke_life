...
from ..models.diagnosis_models import (
    DiagnosisType, DiagnosisResult, FusedDiagnosisResult, 
    PatientInfo, DiagnosisRecommendation
)
...
class FusionConfig:
    ...
    # 诊断类型权重
    diagnosis_weights: Dict[DiagnosisType, float] = field(default_factory=lambda: {
...
class DiagnosisFusionEngine:
    ...
    async def fuse_diagnosis_results(
        ...
        try:
            ...
            if self.config.strategy==FusionStrategy.WEIGHTED_AVERAGE:
                fused_features = await self._weighted_average_fusion(features, diagnosis_results)
            elif self.config.strategy==FusionStrategy.BAYESIAN_FUSION:
                fused_features = await self._bayesian_fusion(features, diagnosis_results)
            elif self.config.strategy==FusionStrategy.ENSEMBLE_LEARNING:
                fused_features = await self._ensemble_learning_fusion(features, diagnosis_results)
            elif self.config.strategy==FusionStrategy.EXPERT_SYSTEM:
                fused_features = await self._expert_system_fusion(features, diagnosis_results)
            else:  # HYBRID
                fused_features = await self._hybrid_fusion(features, diagnosis_results)
            
            # 计算置信度和一致性
            overall_confidence = await self._calculate_overall_confidence(fused_features, diagnosis_results)
            consistency_score = await self._calculate_consistency_score(diagnosis_results)
            completeness_score = await self._calculate_completeness_score(diagnosis_results)
            
            # 分析证型和体质
            syndrome_analysis = await self._analyze_syndromes(fused_features)
            constitution_analysis = await self._analyze_constitution(fused_features, patient_info)
            
            # 评估健康状态和风险
            health_status = await self._assess_health_status(fused_features, syndrome_analysis)
            risk_factors = await self._identify_risk_factors(fused_features, patient_info)
            ...
            # 更新统计
            self._update_fusion_stats(overall_confidence, consistency_score, len(conflicts))
            
            logger.info(f"诊断融合完成: {session_id}, 置信度: {overall_confidence:.3f}, 一致性: {consistency_score:.3f}")
            return fused_result
    ...
    async def _extract_features(self, diagnosis_results: Dict[DiagnosisType, DiagnosisResult]) -> Dict[str, Any]:
        """提取特征"""
        features = {
            "syndromes": {},
            "constitution": {},
        ...
        for diagnosis_type, result in diagnosis_results.items():
            if result.features:
                ...
                if diagnosis_type==DiagnosisType.INQUIRY:
                    ...
                    features["constitution"].update(result.features.get("constitution", {}))
    ...
    async def _detect_conflicts(self, features: Dict[str, Any]) -> List[ConflictInfo]:
        ...
        # 检测证型冲突
        syndrome_conflicts = await self._detect_syndrome_conflicts(features.get("syndromes", {}))
        conflicts.extend(syndrome_conflicts)
        
        # 检测体质冲突
        constitution_conflicts = await self._detect_constitution_conflicts(features.get("constitution", {}))
        conflicts.extend(constitution_conflicts)
        
        return conflicts
    
    async def _detect_syndrome_conflicts(self, syndromes: Dict[str, Any]) -> List[ConflictInfo]:
        """检测证型冲突"""
        conflicts = []
        ...
        for syndrome1, confidence1 in syndromes.items():
            for syndrome2, confidence2 in syndromes.items():
                if syndrome1!=syndrome2:
                    if await self.tcm_knowledge.are_syndromes_conflicting(syndrome1, syndrome2):
    ...
    async def _detect_constitution_conflicts(self, constitutions: Dict[str, Any]) -> List[ConflictInfo]:
        """检测体质冲突"""
        conflicts = []
        
        # 体质类型通常是互斥的，检测多个高置信度体质
        high_confidence_constitutions = [
            (const, conf) for const, conf in constitutions.items() 
            if conf > 0.7
        ]
    ...
    async def _resolve_conflicts(self, features: Dict[str, Any], conflicts: List[ConflictInfo]) -> Dict[str, Any]:
        """解决冲突"""
        ...
        for conflict in conflicts:
            if conflict.resolution_method=="tcm_knowledge":
                # 使用中医知识库解决冲突
                resolved_value, confidence = await self._resolve_with_tcm_knowledge(conflict)
                ...
                # 默认使用加权平均
                resolved_value, confidence = await self._resolve_with_weighted_average(conflict)
    ...
    async def _resolve_with_tcm_knowledge(self, conflict: ConflictInfo) -> Tuple[str, float]:
        """使用中医知识库解决冲突"""
        # 查询中医知识库，找到最合适的证型
        values = [val for val, _, _ in conflict.conflicting_values]
        confidences = [conf for _, conf, _ in conflict.conflicting_values]
        
        best_syndrome = await self.tcm_knowledge.resolve_syndrome_conflict(values, confidences)
        best_confidence = max(confidences)
        
        return best_syndrome, best_confidence
    
    async def _resolve_with_weighted_average(self, conflict: ConflictInfo) -> Tuple[str, float]:
        """使用加权平均解决冲突"""
        # 简单选择置信度最高的值
        return max(
            [(val, conf) for val, conf, _ in conflict.conflicting_values],
                ...
                if isinstance(feature_value, (int, float)):
                    ...
                    for diagnosis_type, result in diagnosis_results.items():
                        if feature_name in result.features.get(feature_category, {}):
                            diagnosis_weight = self.config.diagnosis_weights.get(diagnosis_type, 1.0)
                            weight = diagnosis_weight * result.confidence * category_weight
    ...
    async def _bayesian_fusion(
        ...
        for feature_category, feature_data in features.items():
            ...
            for feature_name, feature_value in feature_data.items():
                ...
                bayesian_features[feature_name] = feature_value if isinstance(feature_value, str) else posterior
    ...
    async def _expert_system_fusion(
        ...
        # 应用专家规则
        rules_output = await self.tcm_knowledge.apply_expert_rules(features, diagnosis_results)
    ...
    async def _hybrid_fusion(
        ...
        if self.ml_models.has_fusion_model():
            ml_fusion = await self._ensemble_learning_fusion(features, diagnosis_results)
            # 融合三种结果
            final_fusion = await self._combine_fusion_results([base_fusion, expert_fusion, ml_fusion])
        else:
            # 融合两种结果
            final_fusion = await self._combine_fusion_results([base_fusion, expert_fusion])
        
        return final_fusion
    
    async def _combine_fusion_results(self, fusion_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        """合并多个融合结果"""
            ...
            for feature_name in all_features:
                ...
                if feature_values:
                    if all(isinstance(v, (int, float)) for v in feature_values):
                        # 数值特征：取平均值
                        category_features[feature_name] = sum(feature_values) / len(feature_values)
                    else:
                        # 分类特征：取第一个非空值
                        category_features[feature_name] = next(v for v in feature_values if v)
    ...
    async def _calculate_consistency_score(self, diagnosis_results: Dict[DiagnosisType, DiagnosisResult]) -> float:
        """计算一致性分数"""
    ...
    async def _calculate_completeness_score(self, diagnosis_results: Dict[DiagnosisType, DiagnosisResult]) -> float:
        """计算完整性分数"""
    ...
    async def _analyze_syndromes(self, fused_features: Dict[str, Any]) -> Dict[str, Any]:
        """分析证型"""
    ...
    async def _analyze_constitution(self, fused_features: Dict[str, Any], patient_info: PatientInfo) -> Dict[str, Any]:
        """分析体质"""
    ...
    async def _assess_health_status(self, fused_features: Dict[str, Any], syndrome_analysis: Dict[str, Any]) -> str:
        """评估健康状态"""
        # 基于证型和症状评估健康状态
        primary_syndrome = syndrome_analysis.get("primary", "")
    ...
    async def _identify_risk_factors(self, fused_features: Dict[str, Any], patient_info: PatientInfo) -> List[str]:
        """识别风险因素"""
        risk_factors = []
        ...
        # 基于年龄和性别识别风险
        age_risks = await self.tcm_knowledge.get_age_related_risks(patient_info.age, patient_info.gender)
    ...
    async def _generate_recommendations(
        ...
        primary_syndrome = syndrome_analysis.get("primary", "")
        if primary_syndrome:
            treatment_recs = await self.tcm_knowledge.get_syndrome_treatments(primary_syndrome)
            recommendations["treatment"].extend(treatment_recs)
        
        # 基于体质生成生活方式建议
        constitution_type = constitution_analysis.get("type", "")
        if constitution_type:
            lifestyle_recs = await self.tcm_knowledge.get_constitution_lifestyle(constitution_type)
        ...
        if high_risk_symptoms:
            follow_up_recs = await self.tcm_knowledge.get_follow_up_recommendations(high_risk_symptoms)
            recommendations["follow_up"].extend(follow_up_recs)
        
        return recommendations
    
    async def _prepare_ml_input(self, features: Dict[str, Any], diagnosis_results: Dict[DiagnosisType, DiagnosisResult]) -> np.ndarray:
        """准备机器学习输入数据"""
        # 将特征转换为数值向量
        feature_vector = []
        ...
        for category in ["syndromes", "constitution", "symptoms", "pulse"]:
            ...
            if category_features:
                # 取前5个最高置信度的特征
                sorted_features = sorted(category_features.items(), key=lambda x: x[1], reverse=True)[:5]
                for _, confidence in sorted_features:
                    feature_vector.append(confidence if isinstance(confidence, (int, float)) else 0.0)
                # 填充到固定长度
    ...
    def _update_fusion_stats(self, confidence: float, consistency: float, conflicts_count: int) -> None:
        """更新融合统计"""
        ...
        self.fusion_stats["average_confidence"] = (
            (self.fusion_stats["average_confidence"] * (total - 1) + confidence) / total
        )
        self.fusion_stats["average_consistency"] = (
            (self.fusion_stats["average_consistency"] * (total - 1) + consistency) / total
        )
