...
class FusedDiagnosisResult:
    ...
    individual_results: Dict[DiagnosisType, DiagnosisResult] = field(
    ...
    def is_high_quality(self) -> bool:
        ...
        return (
            ...
            and self.consistency_score >= 0.6
...
class DiagnosisSession:
    ...
    # 诊断结果
    diagnosis_results: Dict[DiagnosisType, DiagnosisResult] = field(
        default_factory=dict
    )
...
class DiagnosisEvent:
    ...
    session_id: str = ""
    event_type: str = (
        ""  # session_created, diagnosis_started, diagnosis_completed, etc.
    )
...
class FusedDiagnosisResultModel(BaseModel):
    ...
    treatment_recommendations: List[str] = Field(
        default_factory=list, description="治疗建议"
    )
    lifestyle_recommendations: List[str] = Field(
        default_factory=list, description="生活方式建议"
    )
    follow_up_recommendations: List[str] = Field(
        default_factory=list, description="随访建议"
    )
    syndrome_analysis: Dict[str, Any] = Field(
        default_factory=dict, description="证型分析"
    )
    constitution_analysis: Dict[str, Any] = Field(
        default_factory=dict, description="体质分析"
    )
    symptom_analysis: Dict[str, Any] = Field(
        default_factory=dict, description="症状分析"
    )
...
class StartDiagnosisRequest(BaseModel):
    ...
    session_id: str = Field(..., description="会话ID")
    diagnosis_inputs: List[DiagnosisInputModel] = Field(
        ..., min_items=1, description="诊断输入"
    )
