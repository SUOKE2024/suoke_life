...
class ResourceManager:
        ...
        async with self._metrics_lock:
            try:
                ...
                try:
                    ...
                    metrics.file_descriptors = self.process.num_fds() if hasattr(self.process, 'num_fds') else 0
                except (psutil.AccessDenied, psutil.NoSuchProcess):
                    metrics.thread_count = 0
                    metrics.file_descriptors = 0
                
                # 应用指标
                metrics.connection_pool_size = sum(len(pool) if hasattr(pool, '__len__') else 0 
    ...
    async def get_resource_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alerts": {
                "total": len(alerts),
                "critical": len([a for a in alerts if a.status==ResourceStatus.CRITICAL]),
                "warning": len([a for a in alerts if a.status==ResourceStatus.WARNING])
            },
            "timestamp": metrics.timestamp.isoformat()
        }
    
    async def set_threshold(self, resource_type: ResourceType, threshold: ResourceThreshold) -> None:
        """设置资源阈值"""
    ...
    def decorator(func):
        async def wrapper(*args,**kwargs):
            ...
            try:
                ...
                # 计算资源使用变化
                memory_diff = after_metrics.memory_used_mb - before_metrics.memory_used_mb
