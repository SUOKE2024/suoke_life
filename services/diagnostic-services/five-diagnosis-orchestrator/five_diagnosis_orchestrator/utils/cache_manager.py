...
class CacheManager:
    ...
    def __init__(self, config: CacheConfig):
        ...
        if config.strategy == CacheStrategy.LRU:
            ...
            self.memory_cache = TTLCache(
                maxsize=config.max_size, ttl=config.ttl_seconds
            )
        else:
            self.memory_cache = LRUCache(maxsize=config.max_size)
    ...
    async def get(
        self, key: str, namespace: str = "", levels: List[CacheLevel] = None
    ) -> Optional[Any]:
        """获取缓存值"""
    ...
    async def get_cache_keys(self, namespace: str = "", limit: int = 100) -> List[str]:
        ...
        if namespace:
            memory_keys = [
                k for k in memory_keys if k.startswith(f"{self.key_prefix}{namespace}:")
            ]
...
async def cache_set(
    key: str, value: Any, namespace: str = "", ttl_seconds: Optional[int] = None
) -> None:
    """设置缓存值的便捷函数"""
    cache_manager = await get_cache_manager()
