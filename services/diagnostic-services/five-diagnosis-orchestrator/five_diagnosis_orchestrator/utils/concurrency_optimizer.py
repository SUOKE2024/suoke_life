...
class ConcurrencyOptimizer:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.thread_executor = ThreadPoolExecutor(
                max_workers=self.thread_pool_size, thread_name_prefix="diagnosis_thread"
            )
            ...
            logger.info(
                f"并发优化器初始化完成 - 线程池: {self.thread_pool_size}, 进程池: {self.process_pool_size}"
            )
    ...
    async def submit_task(
        ...
        await self.task_queue.put(
            (priority, time.time(), task_id, coro_or_func, config)
        )
    ...
    async def _worker_loop(self) -> None:
        ...
        while not self._shutdown:
            try:
                # 获取任务
                try:
                    priority, submit_time, task_id, coro_or_func, config = (
            ...
            if task_result.duration:
                ...
                if total_completed == 1:
                    ...
                    self.metrics.average_execution_time = (
                        current_avg * (total_completed - 1) + task_result.duration
                    ) / total_completed
    ...
    async def _execute_async_task(
        ...
        async with self.task_semaphore:
            ...
            self.metrics.peak_concurrency = max(
                self.metrics.peak_concurrency, self.metrics.active_tasks
            )
    ...
    async def _check_adaptive_scaling(self) -> None:
        ...
        if queue_load > 0.8 and active_load < 0.6:
            ...
            if new_limit > self.task_semaphore._value:
                logger.info(
                    f"自适应增加并发限制: {self.max_async_tasks} -> {new_limit}"
                )
            ...
            if new_limit < self.max_async_tasks:
                logger.info(
                    f"自适应减少并发限制: {self.max_async_tasks} -> {new_limit}"
                )
                self.max_async_tasks = new_limit
...
async def wait_for_concurrent_task(
...
def concurrent_task(task_id: Optional[str] = None, config: Optional[TaskConfig] = None):
    """并发任务装饰器"""
