...
from ..models.audio_models import AudioAnalysisRequest, AudioAnalysisResponse, VoiceFeatures
from ..models.tcm_models import TCMDiagnosis
from ..utils.performance import performance_monitor, async_timer
...
class AudioAnalyzer:
    ...
    async def analyze_audio(
        ...
        request: Optional[AudioAnalysisRequest] = None,
    ...
    def _validate_audio(self, audio_array: np.ndarray, sample_rate: int) -> None:
        ...
        if duration < self.config.min_duration:
            raise ValueError(f"音频时长过短: {duration:.2f}s < {self.config.min_duration}s")
        
        if duration > self.config.max_duration:
            raise ValueError(f"音频时长过长: {duration:.2f}s > {self.config.max_duration}s")
    ...
    async def _detect_voice_activity(self, audio: np.ndarray) -> List[Tuple[float, float]]:
        """语音活动检测"""
        loop = asyncio.get_event_loop()
        
        return await loop.run_in_executor(
            self.executor, self._vad_segments, audio
        )
    
    def _vad_segments(self, audio: np.ndarray) -> List[Tuple[float, float]]:
        """VAD 分段检测"""
        # 转换为 16kHz, 16-bit PCM
        audio_16k = librosa.resample(audio, orig_sr=self.config.sample_rate, target_sr=16000)
    ...
    async def _extract_features_parallel(self, audio: np.ndarray) -> VoiceFeatures:
        ...
        tasks = [
            ...
            loop.run_in_executor(self.executor, self._extract_voice_quality_features, audio),
        ]
    ...
    def _generate_cache_key(
        self, 
        audio_data: Union[np.ndarray, bytes, str, Path],
        request: Optional[AudioAnalysisRequest],
    ) -> str:
        """生成缓存键"""
        import hashlib
        ...
        if isinstance(audio_data, (str, Path)):
            ...
            key_data = str(hash(audio_data.tobytes() if hasattr(audio_data, 'tobytes') else str(audio_data)))
