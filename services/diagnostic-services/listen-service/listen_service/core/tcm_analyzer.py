...
from ..models.tcm_models import TCMDiagnosis, ConstitutionType, EmotionState, OrganSoundMapping
...
class TCMFeatureExtractor:
    ...
    def __init__(
        ...
        feature_weights: Optional[TCMFeatureWeights] = None,
        enable_ml: bool = True,
    ) -> None:
        """
        初始化中医特征分析器
    ...
    async def analyze_tcm_features(
        ...
        try:
            ...
            constitution_result, emotion_result, organ_result = await asyncio.gather(*tasks)
    ...
    async def _analyze_constitution(self, features: VoiceFeatures) -> Dict[str, Any]:
        """分析体质类型"""
        return await self.constitution_analyzer.analyze(features, self.ml_models.get("constitution"))
    
    async def _analyze_emotion(self, features: VoiceFeatures) -> Dict[str, Any]:
        """分析情绪状态"""
        return await self.emotion_analyzer.analyze(features, self.ml_models.get("emotion"))
    ...
    def _update_stats(self, processing_time: float) -> None:
        ...
        total_time = (
            self.analysis_stats["average_time"] * (self.analysis_stats["total_analyses"] - 1) +
            processing_time
        )
        self.analysis_stats["average_time"] = total_time / self.analysis_stats["total_analyses"]
...
class ConstitutionAnalyzer:
    ...
    async def analyze(
        ...
        # 传统理论分析
        traditional_result = self._traditional_constitution_analysis(constitution_features)
        ...
        if ml_result:
            # 结合传统和ML结果
            confidence = (traditional_result["confidence"] + ml_result["confidence"]) / 2
            constitution_type = ml_result["type"] if ml_result["confidence"] > traditional_result["confidence"] else traditional_result["type"]
        else:
    ...
    def _extract_constitution_features(self, features: VoiceFeatures) -> np.ndarray:
        ...
        if features.spectral_features:
            spectral_centroids = features.spectral_features.get("spectral_centroids", np.array([]))
    ...
    def _traditional_constitution_analysis(self, features: np.ndarray) -> Dict[str, Any]:
        """传统体质分析"""
        # 简化的传统分析逻辑
        if len(features) < 5:
    ...
    def _ml_constitution_analysis(self, features: np.ndarray, model: Any) -> Dict[str, Any]:
        """机器学习体质分析"""
        try:
            # 这里应该使用训练好的模型
            # 暂时返回模拟结果
...
class EmotionAnalyzer:
    ...
    async def analyze(
        ...
        if ml_result:
            confidence = (traditional_result["confidence"] + ml_result["confidence"]) / 2
            emotion_state = ml_result["state"] if ml_result["confidence"] > traditional_result["confidence"] else traditional_result["state"]
        else:
            confidence = traditional_result["confidence"]
