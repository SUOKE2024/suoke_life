...
class ConstitutionAnalysis(BaseModel):
    ...
    primary_type: ConstitutionType = Field(
        ...,
        description="主要体质类型"
    )
    
    secondary_type: Optional[ConstitutionType] = Field(
        default=None,
        description="次要体质类型（兼夹体质）"
    )
...
class EmotionAnalysis(BaseModel):
    ...
    primary_emotion: EmotionState = Field(
        ...,
        description="主要情绪状态"
    )
    ...
    stability: confloat(ge=0.0, le=1.0) = Field(
        default=0.5,
        description="情绪稳定性"
    )
...
class OrganSoundMapping(BaseModel):
    ...
    health_score: confloat(ge=0.0, le=1.0) = Field(
        ...,
        description="健康评分"
    )
...
class OrganAnalysis(BaseModel):
    ...
    organ_mappings: List[OrganSoundMapping] = Field(
        default_factory=list,
        description="脏腑声音对应分析"
    )
    
    overall_health: confloat(ge=0.0, le=1.0) = Field(
        ...,
        description="整体健康评分"
    )
...
class TCMDiagnosis(BaseModel):
    ...
    organ_analysis: Dict[str, Any] = Field(
        default_factory=dict,
        description="脏腑功能分析结果"
    )
    
    organ_details: Optional[OrganAnalysis] = Field(
        default=None,
        description="详细脏腑分析"
    )
...
class TCMAnalysisRequest(BaseModel):
    ...
    # 分析选项
    enable_constitution_analysis: bool = Field(
        default=True,
        description="启用体质分析"
    )
    
    enable_emotion_analysis: bool = Field(
        default=True,
        description="启用情绪分析"
    )
    
    enable_organ_analysis: bool = Field(
        default=True,
        description="启用脏腑分析"
    )
    ...
    analysis_method: str = Field(
        default="hybrid",
        description="分析方法（traditional/ml_enhanced/hybrid）"
    )
    ...
    user_region: Optional[str] = Field(
        default=None,
        description="用户地区（方言影响）"
    )
...
class TCMAnalysisResponse(BaseModel):
    ...
    diagnosis: Optional[TCMDiagnosis] = Field(
        default=None,
        description="中医诊断结果"
    )
