...
class ListenServiceGRPCServer:
    ...
    async def AnalyzeAudio(self, request, context) -> AudioAnalysisResponse:
        ...
        try:
            ...
            if not request.audio_data:
                await context.abort(
                    grpc.StatusCode.INVALID_ARGUMENT, "音频数据不能为空"
                )
            ...
            metadata = AudioMetadata(
                ...
                duration=len(request.audio_data)
                / (
                    getattr(request, "sample_rate", 16000)
                    * getattr(request, "channels", 1)
                    * 2
                ),  # 假设16位
            ...
            await context.abort(grpc.StatusCode.INTERNAL, f"音频分析失败: {e!s}")
    ...
    async def AnalyzeTCM(self, request, context) -> TCMAnalysisResponse:
        ...
        try:
            ...
            await context.abort(grpc.StatusCode.INTERNAL, f"中医分析失败: {e!s}")
    ...
    async def GetServiceHealth(self, request, context):
        ...
        try:
            ...
            await context.abort(grpc.StatusCode.INTERNAL, f"健康检查失败: {e!s}")
    ...
    async def GetServiceStats(self, request, context):
        ...
        try:
            ...
            await context.abort(grpc.StatusCode.INTERNAL, f"获取统计信息失败: {e!s}")
    ...
    def _convert_to_grpc_response(self, analysis_result: AudioAnalysisResponse) -> Any:
        ...
        return {
            ...
            "success": analysis_result.success,
            "voice_features": analysis_result.voice_features.model_dump()
            if analysis_result.voice_features
            else None,
            "processing_time": analysis_result.processing_time,
            "error_message": analysis_result.error_message,
            "timestamp": analysis_result.timestamp.isoformat()
            if analysis_result.timestamp
            else None,
        }
    ...
    def _convert_to_grpc_tcm_response(self, tcm_response: TCMAnalysisResponse) -> Any:
        ...
        return {
            ...
            "diagnosis": tcm_response.diagnosis.model_dump()
            if tcm_response.diagnosis
            else None,
...
async def create_grpc_server(
    host: str = "0.0.0.0", port: int = 50051, **kwargs
