...
class TCMAnalysisRequest(BaseModel):
    ...
    user_age: int | None = Field(default=None, description="用户年龄")
    user_gender: str | None = Field(default=None, description="用户性别")
    user_region: str | None = Field(default=None, description="用户地区")
...
def create_rest_app(
    ...
    tcm_analyzer: TCMFeatureExtractor | None = None,
...
"健康检查失败")
    ...
    async def get_stats(user_id: str | None = Depends(verify_token)):
        """获取服务统计信息"""
        try:
            uptime = time.time() - app.state.start_time
            avg_processing_time = (
                app.state.stats["total_processing_time"]
                / app.state.stats["successful_requests"]
                if app.state.stats["successful_requests"] > 0
                else 0.0
            )
  ...
  @limiter.limit("100/minute")  # 每分钟100次请求
    ...
    async def analyze_audio(
        ...
        try:
            ...
            return {
                "request_id": request_id,
                "success": True,
                "voice_features": result.voice_features.model_dump()
    ...
    async def analyze_tcm(
        ...
        try:
            ...
            logger.info(
                "开始中医诊断分析", request_id=request_id, filename=file.filename
            )
...
us_code=500, detail=f"清空缓存失败: {e!s}")
    ...
    async def get_performance_metrics(user_id: str | None = Depends(verify_token)):
        """获取性能指标"""
        if not user_id:
            raise HTTPException(status_code=401, detail="需要认证")
    ...
    try:
        ...
        result = {
            ...
            "audio_analysis": {
                ...
                "voice_features": audio_result.voice_features.model_dump()
                if audio_result.voice_features
                else None,
        ...
        raise Exception(f"处理文件 {file.filename} 失败: {e!s}")
