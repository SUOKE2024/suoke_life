...
class TCMAnalysisSettings(BaseSettings):
    ...
    organ_types: List[str] = Field(
        default=["心", "肝", "脾", "肺", "肾", "胆", "胃", "小肠", "大肠", "膀胱", "三焦"],
        description="脏腑类型"
    )
    ...
    # 模型路径
    constitution_model_path: Optional[str] = Field(default=None, description="体质分析模型路径")
    emotion_model_path: Optional[str] = Field(default=None, description="情绪分析模型路径")
...
class DatabaseSettings(BaseSettings):
    ...
    mongodb_username: Optional[str] = Field(default=None, description="MongoDB 用户名")
    mongodb_password: Optional[str] = Field(default=None, description="MongoDB 密码")
...
class ServerSettings(BaseSettings):
    ...
    max_message_length: int = Field(default=100 * 1024 * 1024, description="最大消息长度")
...
class LoggingSettings(BaseSettings):
    ...
    # 文件配置
    file_path: str = Field(default="logs/listen_service.log", description="日志文件路径")
