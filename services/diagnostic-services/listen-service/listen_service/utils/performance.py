...
class PerformanceMonitor:
    ...
    def record(self, function_name: str, execution_time: float, is_error: bool = False) -> None:
        """记录性能数据"""
        with self._lock:
            self.metrics[function_name].update(execution_time, is_error)
    
    def get_metrics(self, function_name: Optional[str] = None) -> Union[Dict[str, Dict[str, Any]], Dict[str, Any]]:
        """获取性能指标"""
        with self._lock:
            if function_name:
                ...
                if metrics:
                    return {
                        ...
                        "error_rate": metrics.error_count / metrics.total_calls if metrics.total_calls > 0 else 0.0,
                    }
                return {}
...
class PerformanceProfiler:
    ...
    def _analyze_profile_data(self) -> Dict[str, Any]:
        ...
        # 生成分析报告
        analysis = {
            "profile_duration": self.profile_data[-1]["timestamp"] - self.profile_data[0]["timestamp"],
        ...
        for func_name, trend_data in function_trends.items():
            ...
            call_rate = (end_metrics["total_calls"] - start_metrics["total_calls"]) / analysis["profile_duration"]
            avg_time_change = end_metrics["average_time"] - start_metrics["average_time"]
