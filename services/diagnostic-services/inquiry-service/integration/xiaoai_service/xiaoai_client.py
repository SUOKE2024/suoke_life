...
class XiaoaiServiceClient:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        """
        self.config = config
        self.integration_config = config.get("integration", {}).get(
            "xiaoai_service", {}
        )
        ...
        # 检查是否为测试模式
        self.is_test_mode = os.environ.get("TEST_MODE", "false").lower() in [
            "true",
            "1",
            "yes",
        ]
        ...
        logger.info(
            f"小艾服务客户端初始化完成: {self.service_address}, mock模式: {self.use_mock}"
        )
    ...
    def setup_channel(self) -> None:
        ...
        options = [
            (
                ...
                self.integration_config.get("max_message_size", 10485760),
            ),
            (
            ...
            (
                ...
                self.integration_config.get("keep_alive_time", 60000),
            ),
            (
                "grpc.keepalive_timeout_ms",
                self.integration_config.get("keep_alive_timeout", 20000),
            ),
        ]
        ...
        if self.use_tls and self.cert_path:
            ...
            self._channel = grpc.aio.secure_channel(
                self.service_address, creds, options=options
            )
        else:
            # 不安全连接
            self._channel = grpc.aio.insecure_channel(
                self.service_address, options=options
            )
    ...
    @circuit_breaker
    async def get_inquiry_analysis(
        self, session_id: str, symptoms: list[dict], tcm_patterns: list[dict]
    ) -> dict:
        """
        获取问诊分析结果
        ...
        """
        if self.use_mock:
            return await self._mock_get_inquiry_analysis(
                session_id, symptoms, tcm_patterns
            )
        ...
        try:
            ...
            logger.info(
                f"小艾服务调用成功 (GetInquiryAnalysis): session_id={session_id}, elapsed_time={elapsed_time:.3f}s"
            )
            ...
            logger.error(
                f"小艾服务调用失败 (GetInquiryAnalysis): status={status_code.name}, "
            ...
            elapsed_time = time.time() - start_time
            logger.exception(
                f"小艾服务调用发生未知异常 (GetInquiryAnalysis): error={e!s}, "
    ...
    async def sync_inquiry_status(
        self, session_id: str, status: str, metadata: dict[str, Any]
    ) -> bool:
        """
        同步问诊状态到小艾服务
        ...
        try:
            ...
            elapsed_time = time.time() - start_time
            logger.info(
                f"小艾服务调用成功 (SyncInquiryStatus): session_id={session_id}, "
            ...
            logger.error(
                f"小艾服务调用失败 (SyncInquiryStatus): status={status_code.name}, "
                f"details={e.details()}, elapsed_time={elapsed_time:.3f}s"
            )
            ...
            elapsed_time = time.time() - start_time
            logger.exception(
                f"小艾服务调用发生未知异常 (SyncInquiryStatus): error={e!s}, "
                f"elapsed_time={elapsed_time:.3f}s"
            )
    ...
    async def _mock_get_inquiry_analysis(
        self, session_id: str, symptoms: list[dict], tcm_patterns: list[dict]
    ) -> dict:
        """
        模拟获取问诊分析结果
    ...
    async def _mock_sync_inquiry_status(
        self, session_id: str, status: str, metadata: dict[str, Any]
    ) -> bool:
        """
        模拟同步问诊状态
