...
class MetricsCollector:
    ...
    def __init__(self, host: str = '0.0.0.0', port: int = 8080, path: str = '/metrics', config: Dict[str, Any] = None):
        """
        初始化度量收集器
    ...
    def _init_metrics(self):
        ...
        self.request_counter = Counter(
            'inquiry_service_requests_total',
            '问诊服务请求总数',
            ['method', 'status']
        )
        ...
        self.active_sessions = Gauge(
            'inquiry_service_active_sessions',
            '问诊服务当前活跃会话数'
        )
        ...
        self.llm_tokens_input = Counter(
            'inquiry_service_llm_tokens_input_total',
            'LLM输入Token总数'
        )
        
        self.llm_tokens_output = Counter(
            'inquiry_service_llm_tokens_output_total',
            'LLM输出Token总数'
        )
        
        # 症状提取指标
        self.symptom_extraction_count = Counter(
            'inquiry_service_symptom_extraction_total',
            '症状提取次数'
        )
        ...
        self.symptom_extraction_accuracy = Gauge(
            'inquiry_service_symptom_extraction_accuracy',
            '症状提取准确率'
        )
        
        # TCM证型指标
        self.tcm_pattern_match_count = Counter(
            'inquiry_service_tcm_pattern_match_total',
            'TCM证型匹配次数'
        )
        ...
        self.health_risk_assessment_count = Counter(
            'inquiry_service_health_risk_assessment_total',
            '健康风险评估次数'
        )
        ...
        self.system_memory_usage = Gauge(
            'inquiry_service_system_memory_usage_bytes',
            '问诊服务内存使用量（字节）'
        )
        
        self.system_cpu_usage = Gauge(
            'inquiry_service_system_cpu_usage_percent',
            '问诊服务CPU使用率（百分比）'
        )
    ...
    def start(self):
        ...
        try:
            ...
            logger.error(f"启动度量收集器失败: {str(e)}")
    ...
    def record_llm_response(self, response_time: float, input_tokens: int, output_tokens: int):
        """
        记录LLM响应指标
