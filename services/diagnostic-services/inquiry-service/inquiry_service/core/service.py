...
class InquiryResponse:
    ...
    syndrome_analysis: dict[str, Any] | None = None
...
class EnhancedInquiryServiceV2(BaseService):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        # 核心组件
        self.flow_manager: IntelligentFlowManager | None = None
        self.knowledge_graph: TCMKnowledgeGraph | None = None
        self.health_monitor: HealthMonitor | None = None

        # 提取器组件
        self.symptom_extractor: SymptomExtractor | None = None
        self.context_analyzer: ContextAnalyzer | None = None
        self.severity_analyzer: SeverityAnalyzer | None = None
    ...
    async def start_inquiry_session(
        self, patient_id: str, initial_data: dict[str, Any] | None = None
    ) -> InquiryContext:
        """
        开始问诊会话
        ...
        try:
            ...
            if (
                ...
                >= self.service_config["max_concurrent_sessions"]
            ):
            ...
            context = await self.flow_manager.start_inquiry_session(
                session_id=session_id, patient_id=patient_id, initial_data=initial_data
            )
    ...
    async def process_inquiry_message(self, request: InquiryRequest) -> InquiryResponse:
        """
        处理问诊消息
    ...
    async def _analyze_context(
        self, message: str, context: InquiryContext
    ) -> dict[str, Any]:
        """分析上下文"""
        ...
        try:
            context_info = await self.context_analyzer.analyze_context(
                text=message, existing_symptoms=context.symptom_profile
            )
            return context_info
        except Exception as e:
    ...
    async def _analyze_severity(self, symptoms: list[dict[str, Any]]) -> dict[str, Any]:
        """分析严重程度"""
    ...
    async def _generate_response(
        ...
        next_questions = await self.flow_manager.generate_next_questions(
            session_id=context.session_id, max_questions=3
        )
        ...
        if (
            self.knowledge_graph and len(context.symptom_profile) >= 3
        ):  # 至少3个症状才进行分析
            syndrome_mappings = await self.knowledge_graph.map_symptoms_to_syndromes(
                symptoms=symptoms, patient_context=context.metadata
            )
    ...
    async def _generate_response_text(
        self, context: InquiryContext, decision: FlowDecision
    ) -> str:
        """生成响应文本"""
        # 基于决策类型生成不同的响应
        if decision.decision_type.value == "emergency_protocol":
            ...
            if next_stage:
                return stage_messages.get(next_stage.value, "让我们继续问诊。")
    ...
    async def _perform_real_time_analysis(
        self, context: InquiryContext, response: InquiryResponse
    ):
        """执行实时分析"""
        try:
            # 检查是否需要自动诊断
            if (
                response.syndrome_analysis
                and response.syndrome_analysis.get("confidence", 0)
    ...
    def _update_real_time_stats(
        ...
        if response.formula_recommendations:
            self.stats["formula_recommendations"] += len(
                response.formula_recommendations
            )

    @timer("inquiry_service_v2.generate_diagnosis")
    @counter("inquiry_service_v2.diagnoses_generated")
    async def generate_diagnosis(self, session_id: str) -> DiagnosisResult:
        """
        生成诊断结果
        ...
        try:
            ...
            # 提取主诉
            chief_complaint = context.collected_data.get(
                "chief_complaint", "未明确主诉"
            )
            ...
            if self.knowledge_graph and symptom_summary:
                ...
                # 证型映射
                syndrome_mappings = (
                    await self.knowledge_graph.map_symptoms_to_syndromes(
                        symptoms=symptoms_for_mapping, patient_context=context.metadata
                    )
                )
                ...
                if syndrome_mappings:
                    formula_recommendations = (
                        await self.knowledge_graph.recommend_formulas(
            ...
            diagnosis = DiagnosisResult(
                ...
                metadata={
                    "session_duration": (
                        datetime.now() - context.stage_history[0][1]
                    ).total_seconds(),
    ...
    async def _generate_health_assessment(
        self, context: InquiryContext, syndrome_mappings: list[SyndromeMapping]
    ) -> dict[str, Any]:
        """生成健康评估"""
        assessment = {
        ...
        if syndrome_mappings:
            ...
            if primary_syndrome.confidence >= 0.8:
                ...
                assessment["key_concerns"].append(
                    f"识别出明确证型：{primary_syndrome.syndrome_name}"
                )
            else:
    ...
    async def _generate_recommendations(
        ...
        if syndrome_mappings:
            ...
            recommendations.append(
                f"根据症状分析，您可能存在{primary_syndrome.syndrome_name}的情况"
            )
    ...
    async def get_service_health(self) -> dict[str, Any]:
        ...
        health_info = {
            ...
            "stats": self.stats,
            "uptime_seconds": (
                datetime.now() - self.stats["service_uptime"]
            ).total_seconds(),
        }
