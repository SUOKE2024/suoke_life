...
class SessionRepository:
    ...
    async def update_session_status(self, session_id: str, status: str) -> bool:
        ...
        try:
            ...
            if self.db_type == 'memory':
                if session_id not in self.sessions:
                    logger.warning(f"更新会话状态失败: 未找到会话 {session_id} (内存模式)")
                    return False
    ...
    async def count_sessions_by_user_id(self, user_id: str) -> int:
        ...
        try:
            ...
            logger.error(f"计算用户会话数量失败: {str(e)}")
            return 0
    ...
    async def get_active_sessions(self, limit: int = 100) -> List[Dict]:
        ...
        try:
            ...
            cursor = self.sessions_collection.find({
                'status': 'active',
                'last_interaction': {'$gt': active_threshold}
            }).sort('last_interaction', pymongo.DESCENDING).limit(limit)
