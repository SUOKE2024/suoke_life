...
class UserRepository:
    ...
    async def update_user_health_profile(self, user_id: str, health_profile: Dict) -> bool:
        """
        更新用户健康档案
        ...
        try:
            ...
            logger.error(f"更新用户健康档案失败: {str(e)}")
            return False
    ...
    async def add_symptoms_to_history(self, user_id: str, symptoms: List[Dict]) -> bool:
        ...
        try:
            ...
            if result.matched_count > 0:
                logger.info(f"已添加症状到用户历史: {user_id}, 症状数量: {len(symptoms)}")
                return True
            else:
            ...
            logger.error(f"添加症状到用户历史失败: {str(e)}")
            return False
            
    async def add_tcm_patterns_to_history(self, user_id: str, patterns: List[Dict]) -> bool:
        """
        添加中医证型到用户历史记录
        ...
        try:
            ...
            if result.matched_count > 0:
                logger.info(f"已添加证型到用户历史: {user_id}, 证型数量: {len(patterns)}")
                return True
            else:
            ...
            logger.error(f"添加证型到用户历史失败: {str(e)}")
            return False
            
    async def get_user_symptom_history(self, user_id: str, limit: int = 50) -> List[Dict]:
        """
        获取用户症状历史
        ...
        try:
            ...
            logger.error(f"获取用户症状历史失败: {str(e)}")
            return []
            
    async def get_user_tcm_pattern_history(self, user_id: str, limit: int = 20) -> List[Dict]:
        """
        获取用户证型历史
        ...
        try:
            user = await self.users_collection.find_one(
                {'id': user_id},
                {'tcm_pattern_history': {'$slice': -limit}}
            )
            ...
            logger.error(f"获取用户证型历史失败: {str(e)}")
            return []
            
    async def update_user_constitution(self, user_id: str, constitution_type: str) -> bool:
        """
        更新用户体质类型
        ...
        try:
            ...
            logger.error(f"更新用户体质类型失败: {str(e)}")
            return False
