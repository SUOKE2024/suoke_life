...
from internal.model.tcm_models import DetailedTCMPattern, SymptomTCMMapping, TCMDiagnosisRule
...
class PatternMapper:
    ...
    def __init__(self, config: Dict = None):
        ...
        # 规则库文件路径
        rules_path = config.get('tcm_rules_path', os.path.join('config', 'tcm_rules.json'))
        mapping_path = config.get('symptom_mapping_path', os.path.join('config', 'symptom_mapping.json'))
        patterns_path = config.get('tcm_patterns_path', os.path.join('config', 'tcm_patterns.json'))
        ...
        self.logger.info(f"证型映射器初始化完成，加载了 {len(self.diagnosis_rules)} 条辨证规则")
        ...
        try:
            ...
            for rule_data in rules_data:
                ...
                rule = TCMDiagnosisRule(
                    ...
                    minimum_supporting_score=rule_data.get('minimum_supporting_score', 0.0),
            ...
            self.logger.warning(f"加载中医辨证规则失败: {str(e)}，将使用默认值")
            return {}
    ...
    def _load_symptom_mappings(self, path: str) -> Dict[str, SymptomTCMMapping]:
        ...
        try:
            ...
            self.logger.warning(f"加载症状映射失败: {str(e)}，将使用默认值")
            return {}
    ...
    def _load_tcm_patterns(self, path: str) -> Dict[str, DetailedTCMPattern]:
        ...
        try:
            ...
            self.logger.warning(f"加载中医证型失败: {str(e)}，将使用默认值")
            return {}
    
    def map_symptoms_to_patterns(self, symptoms: List[Symptom]) -> TCMPatternMappingResult:
        """
        将症状映射到中医证型
        ...
        # 合并两种方法的结果
        combined_scores = self._combine_pattern_scores(pattern_scores, symptom_pattern_scores)
        ...
        self.logger.info(f"完成证型映射，找到 {len(tcm_patterns)} 个相关证型，主要证型: {primary_pattern.name if primary_pattern else 'None'}")
        return result
    ...
    def _apply_diagnosis_rules(self, symptoms: List[Symptom]) -> Dict[str, float]:
        ...
        for rule_id, rule in self.diagnosis_rules.items():
            ...
            total_score = 0.6 * required_ratio + 0.4 * min(1.0, supporting_score / max(1.0, rule.minimum_supporting_score * 2))
            
            # 更新证型得分
            if pattern_name in pattern_scores:
                pattern_scores[pattern_name] = max(pattern_scores[pattern_name], total_score)
            else:
                pattern_scores[pattern_name] = total_score
        
        return pattern_scores
    
    def _calculate_symptom_pattern_scores(self, symptoms: List[Symptom]) -> Dict[str, float]:
        """
        基于症状-证型关联计算证型得分
        ...
        for symptom in symptoms:
            ...
            if symptom_name in self.symptom_mappings:
                ...
                for pattern_name, weight in mapping.pattern_associations.items():
                    pattern_scores[pattern_name] = pattern_scores.get(pattern_name, 0.0) + weight * symptom.confidence
    ...
    def _combine_pattern_scores(self, rule_scores: Dict[str, float], symptom_scores: Dict[str, float]) -> Dict[str, float]:
        """
        合并规则匹配和症状关联的证型得分
    ...
    def _generate_tcm_patterns(self, pattern_scores: Dict[str, float]) -> List[TCMPattern]:
        """
        根据证型得分生成证型列表
        ...
        for pattern_name, score in sorted_patterns:
            ...
            if found:
                ...
                self.logger.warning(f"证型 '{pattern_name}' 在证型库中未找到详细信息，已跳过")
        
        return tcm_patterns
    
    def _generate_pattern_analysis(self, symptoms: List[Symptom], patterns: List[TCMPattern]) -> str:
        """
        生成证型分析报告
        ...
        if primary_pattern.recommendations:
            analysis += "\n\n治疗调养建议："
            for i, rec in enumerate(primary_pattern.recommendations[:5]):  # 最多显示5条建议
                analysis += f"\n{i+1}. {rec}"
        
        return analysis
    
    def _calculate_overall_confidence(self, patterns: List[TCMPattern], symptoms: List[Symptom]) -> float:
        """
        计算整体置信度
        ...
        # 基于症状置信度平均值
        symptom_confidence_avg = sum(s.confidence for s in symptoms) / max(1, len(symptoms))
        
        # 综合计算
        overall_confidence = 0.5 * primary_score + 0.3 * symptom_factor + 0.2 * symptom_confidence_avg
