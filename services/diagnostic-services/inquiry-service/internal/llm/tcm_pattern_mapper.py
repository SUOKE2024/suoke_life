...
class TCMPatternMapper:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.constitution_types_path = self.tcm_config.get(
            "constitution_types_path", ""
        )
    ...
    def _load_patterns_db(self)-> dict:
        ...
        try:
            if self.patterns_db_path and os.path.exists(self.patterns_db_path):
                with open(self.patterns_db_path, encoding = "utf - 8") as file:
                    return json.load(file)
            else:
                logger.warning("证型数据库文件不存在，将使用内置的基本证型数据")
                return self._get_default_patterns_db()
        except Exception as e:
            logger.error(f"加载证型数据库失败: {e!s}")
            return self._get_default_patterns_db()
    ...
    def _load_constitution_types(self)-> dict:
        """加载体质类型数据"""
        try:
            if self.constitution_types_path and os.path.exists(
                self.constitution_types_path
            ):
                with open(self.constitution_types_path, encoding = "utf - 8") as file:
                    return json.load(file)
            else:
                logger.warning("体质类型数据文件不存在，将使用内置的基本体质数据")
                return self._get_default_constitution_types()
        except Exception as e:
            logger.error(f"加载体质类型数据失败: {e!s}")
            return self._get_default_constitution_types()
        ...
        try:
            ...
            for pattern in all_patterns:
                score = self._calculate_pattern_match(
                    pattern, symptom_names, body_locations, temporal_factors
                )
                    ...
                    pattern_scores.append(
                        {
                            ...
                            "match_score": score,
                            "related_symptoms": self._get_related_symptoms(
                                pattern, symptom_names
                            ),
            ...
            # 检查证型组合
            combination_patterns = self._check_pattern_combinations(
            ...
            if not primary_patterns and user_constitution:
                # 根据体质推断可能的证型
                constitution_patterns = self._get_patterns_from_constitution(
                    user_constitution
                )
            ...
            interpretation = self._generate_interpretation(
                primary_patterns, secondary_patterns, symptoms
            )

            # 计算总体置信度
            if primary_patterns:
                confidence_score = sum(
                    [p["match_score"] for p in primary_patterns]
            ...
            logger.info(
                f"映射到 {len(primary_patterns)} 个主证和 {len(secondary_patterns)} 个次证"
            )

            return result

        except Exception as e:
            logger.error(f"映射中医证型失败: {e!s}")
            return {
                "primary_patterns": [],
                "secondary_patterns": [],
    ...
    def _calculate_pattern_match(
        ...
        # 在症状中查找舌脉特征
        has_tongue_match = any(
            tongue
            for tongue in tongue_symptoms
            if any(tongue in s for s in symptom_names)
        )
        has_pulse_match = any(
            pulse for pulse in pulse_symptoms if any(pulse in s for s in symptom_names)
        )
    ...
    def _get_related_symptoms(
        self, pattern: dict, symptom_names: list[str]
    )-> list[str]:
        """获取与证型相关的症状"""
        core_symptoms = pattern.get("core_symptoms", [])
        common_symptoms = pattern.get("common_symptoms", [])
    ...
    def _check_pattern_combinations(
        self, primary_patterns: list[dict], secondary_patterns: list[dict]
    )-> list[dict]:
        """检查证型组合"""
    ...
    def _get_patterns_from_constitution(self, constitution_type: str)-> list[dict]:
        """根据体质类型获取相关的证型"""
        constitution_pattern_relations = self.constitution_types.get(
            "constitution_pattern_relations", {}
        )

        # 获取与该体质类型相关的证型名称
        related_pattern_names = constitution_pattern_relations.get(
            constitution_type, []
        )
    ...
    def _generate_interpretation(
        ...
        if primary_patterns:
            ...
            interpretation + = (
                f"{main_pattern['pattern_name']}是指{main_pattern['description']}。"
            )
