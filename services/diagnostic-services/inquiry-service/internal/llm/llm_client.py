...
class LLMClient:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"LLM客户端初始化完成，模型类型: {self.model_type}, 使用Mock模式: {self.use_mock_mode}")
...
记住，你不是在替代医生诊断或治疗，而是帮助用户理解自己的健康状况，并在必要时建议就医。"""
        ...
        try:
            ...
            if self.model_type == "llama3":
                ...
                try:
                    ...
                    logger.error(f"加载ChatGLM模型失败: {str(e)}")
                    self.local_inference = False
    ...
    def _build_prompt(self, history: List[Dict], user_profile: Dict) -> str:
        ...
        if user_profile:
            ...
            if 'constitution_type' in user_profile:
                ...
                constitution = constitution_map.get(user_profile.get('constitution_type'), '未知体质')
            ...
            if 'lifestyle' in user_profile:
                ...
                if 'emotional_tendency' in lifestyle:
                    emotions = lifestyle.get('emotional_tendency', {}).get('dominant_emotions', [])
                    if emotions:
        ...
        try:
            ...
            if self.model_type == "llama3":
                ...
                try:
                    ...
                    logger.error(f"ChatGLM模型推理失败: {str(e)}")
                    return self._get_fallback_response(prompt)
    ...
    async def _generate_remote(self, prompt: str) -> str:
        ...
        try:
            ...
            if "ClientError" in str(type(e)):
                ...
                logger.error(f"远程API调用未知错误: {str(e)}")
            return self._get_fallback_response(prompt)
    ...
    def _get_fallback_response(self, prompt: str) -> str:
        ...
        if "头痛" in prompt:
            ...
            response_data = {
                ...
                "follow_up_questions": ["您的头痛是持续性的还是间歇性的？", "头痛部位在哪里？", "是否伴有眩晕、耳鸣等症状？"]
            }
        elif "失眠" in prompt:
            ...
            response_data = {
                ...
                "follow_up_questions": ["您的失眠主要表现为难以入睡、易醒，还是早醒？", "是否有心烦、口干等伴随症状？"]
            }
        elif "胃痛" in prompt:
            ...
            response_data = {
                ...
                "response_type": "RECOMMENDATION",
                "follow_up_questions": ["您的胃痛是持续性还是间歇性的？", "是否与进食有关？", "是否伴有反酸、嗳气等症状？"]
            }
        elif "腰痛" in prompt:
            ...
            response_data = {
                ...
                "response_type": "RECOMMENDATION",
                "follow_up_questions": ["您的腰痛是刺痛、酸痛还是跳痛？", "疼痛是持续性的还是活动后加重？"]
            }
        else:
            ...
            response_data = {
                ...
                "follow_up_questions": ["您目前有哪些不适症状？", "这些症状持续了多长时间？", "您的日常生活习惯如何？"]
            }
    ...
    def _parse_response(self, raw_response: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"解析LLM回复失败: {str(e)}")
            return {
                'response_text': raw_response,
                'response_type': 'TEXT',
                'detected_symptoms': [],
    ...
    def _extract_symptoms_from_text(self, text: str) -> List[str]:
        ...
        # 常见症状关键词
        symptom_keywords = [
            '疲劳', '乏力', '头痛', '头晕', '失眠', '心悸', '胸闷',
            '咳嗽', '咳痰', '气短', '胃痛', '腹痛', '腹泻', '便秘',
            '口干', '口苦', '口臭', '恶心', '呕吐', '食欲不振',
            '潮热', '盗汗', '手脚冰凉', '浮肿', '尿频', '尿急',
            '腰酸', '腰痛', '关节疼痛', '肌肉酸痛', '麻木', '刺痛',
            '瘙痒', '皮疹', '多梦', '健忘', '焦虑', '抑郁', '烦躁',
            '痰多', '舌苔厚', '舌苔白', '舌苔黄', '舌质红', '舌质淡'
        ]
    ...
    def _extract_follow_up_questions(self, text: str) -> List[str]:
        ...
        for line in lines:
            ...
            elif '您' in line and ('吗' in line or '呢' in line) and len(line) > 5:
                questions.append(line)
    ...
    def _determine_response_type(self, text: str) -> str:
        ...
        # 检查是否是症状确认
        symptom_confirmation_keywords = ['您是否有', '您是不是有', '您提到的', '您所说的症状']
