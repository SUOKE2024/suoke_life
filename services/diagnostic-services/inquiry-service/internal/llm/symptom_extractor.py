...
class SymptomExtractor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.enable_negation_detection = self.extractor_config.get('enable_negation_detection', True)
        self.max_symptoms_per_text = self.extractor_config.get('max_symptoms_per_text', 30)
        
        # 症状映射与分类
        self.symptoms_mapping_path = config.get('tcm_knowledge', {}).get('symptoms_mapping_path', '')
        ...
        # 否定词
        self.negation_words = [
            '没有', '不', '不存在', '不会', '不曾', '不再', '无', '未',
            '否认', '拒绝', '排除', '不太', '不怎么', '从不', '从来不',
            '不敢说', '不确定', '不一定', '尚未', '不能确定', '并非'
        ]
    ...
    def _load_symptoms_map(self) -> Dict:
        """加载症状映射表"""
        try:
            if self.symptoms_mapping_path and os.path.exists(self.symptoms_mapping_path):
                with open(self.symptoms_mapping_path, 'r', encoding='utf-8') as file:
                    data = json.load(file)
    ...
    def _get_default_symptoms_map(self) -> Dict:
        ...
        return {
            "symptom_categories": {
                ...
                "abdomen": ["腹痛", "腹胀", "恶心", "呕吐", "腹泻", "便秘", "消化不良", "嗳气"],
            ...
            "tcm_mapping": {
                "气虚症状": ["疲劳", "乏力", "气短", "懒言", "自汗", "动则汗出", "语声低弱", "气短懒言"],
                "血虚症状": ["面色苍白", "唇甲色淡", "头晕", "眼花", "失眠", "心悸", "月经量少"],
                "阴虚症状": ["口干", "咽干", "五心烦热", "潮热", "盗汗", "失眠", "颧红", "手足心热"],
                "阳虚症状": ["怕冷", "手脚冰凉", "面色苍白", "喜温", "腰膝酸软", "神疲"],
                "痰湿症状": ["痰多", "胸闷", "恶心", "呕吐", "腹胀", "苔腻", "头重", "肢体沉重"],
                "湿热症状": ["口苦", "口臭", "小便黄", "大便粘滞不爽", "苔黄腻", "身重"],
                "气滞症状": ["胸胁胀痛", "情志不畅", "嗳气", "打嗝", "痛处不固定", "善太息"],
                "血瘀症状": ["疼痛固定", "刺痛", "肌肤甲错", "舌质紫暗", "口唇紫暗", "瘀斑"]
            },
            "severity": {
                "mild": ["有点", "轻微", "些许", "一点点", "偶尔", "不太", "有时"],
                "moderate": ["明显", "较为", "比较", "常常", "时常", "经常"],
                "severe": ["严重", "剧烈", "难以忍受", "极度", "非常", "特别", "很", "十分"]
            },
            "duration": {
    ...
    def _load_symptom_keywords(self) -> List[str]:
        ...
        # 从分类中提取
        for category, symptoms in self.symptoms_map.get('symptom_categories', {}).items():
    ...
    def _load_model(self):
        ...
        try:
            ...
            logger.error(f"加载症状提取模型失败: {str(e)}")
            self.model = None
    ...
    async def extract_symptoms(self, text: str) -> Dict:
        ...
        try:
            ...
            if self.model is not None:
                symptoms, body_locations, temporal_factors, confidence = await self._extract_with_model(text)
            else:
                # 否则使用规则和关键词匹配
                symptoms, body_locations, temporal_factors, confidence = self._extract_with_rules(text)
    ...
    async def _extract_with_model(self, text: str) -> Tuple[List[Dict], List[Dict], List[Dict], float]:
        """使用NER模型提取症状"""
        try:
            import torch
            ...
            token_predictions = [self.model.config.id2label[p.item()] for p in predictions[0]]
            ...
            for token, label in zip(tokens, token_predictions):
            ...
            logger.error(f"使用模型提取症状失败: {str(e)}")
            # 回退到规则匹配
            return self._extract_with_rules(text)
            
    def _extract_with_rules(self, text: str) -> Tuple[List[Dict], List[Dict], List[Dict], float]:
        """使用规则和关键词匹配提取症状"""
        symptoms = []
        body_locations = []
        temporal_factors = []
        ...
        time_patterns = {
            ...
            'seasonal': ['春天', '夏天', '秋天', '冬天', '春季', '夏季', '秋季', '冬季'],
            'durational': ['持续', '一直', '长期', '短期', '几天', '几周', '几个月', '几年'],
            'frequency': ['经常', '偶尔', '有时', '时常', '总是', '频繁', '很少', '从不'],
            'triggers': ['吃完', '吃后', '饭后', '运动后', '劳累', '熬夜', '情绪', '受凉']
        }
                ...
                if keyword in text:
                    ...
                    for symptom in symptoms:
                        ...
                        for sentence in sentences:
                            if keyword in sentence and symptom['symptom_name'] in sentence:
        ...
        for sentence in sentences:
            if symptom in sentence:
                ...
                for neg_word in self.negation_words:
                    ...
                    if neg_pos != -1 and neg_pos < symptom_pos and symptom_pos - neg_pos < 15:
                        return True
        return False
    ...
    def _associate_symptoms_with_locations(self, symptoms: List[Dict], body_locations: List[Dict]):
        """关联症状和身体部位"""
        # 简单关联：基于同一句话中出现
        # 这部分可以根据实际需求优化
