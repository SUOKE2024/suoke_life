...
class HealthRiskAssessor:
    ...
    def _load_risk_rules(self) -> Dict[str, Any]:
        ...
        # 尝试从文件加载自定义规则
        rules_file = os.path.join(self.config.get('data', {}).get('base_dir', './data'), 'risk_rules.json')
        if os.path.exists(rules_file):
            try:
                ...
                logger.error(f"加载自定义风险评估规则失败: {str(e)}")
    ...
    def _analyze_symptom_risks(self, symptoms: List[Dict]) -> List[Dict]:
        ...
        symptom_severities = {
            s.get('symptom_name', ''): s.get('severity', 'MODERATE')
            for s in symptoms
        }
        ...
        for factor in risk_factors:
            if factor.get('risk_score', 0) > 0.5:
                risks.append({
                    ...
                    "severity": self._get_severity_level(factor.get('risk_score', 0)),
                    "timeframe": "long_term",
    ...
    def _analyze_constitution_risks(
        ...
        # 获取体质相关疾病风险
        constitution_risks = self.constitution_disease_mapping.get(constitution_type, [])
        ...
        for risk in all_risks:
            ...
            if risk_name in merged_risks:
                ...
                merged_risks[risk_name]['probability'] = max(
                    merged_risks[risk_name]['probability'],
                    risk['probability']
                )
                # 合并贡献因素
    ...
    def _generate_prevention_strategies(
        ...
        if constitution_type != 'BALANCED':
            strategies.append(self._get_constitution_prevention_strategy(constitution_type))
            
        # 去重并按有效性排序
        unique_strategies = {s['strategy_name']: s for s in strategies}.values()
        return sorted(unique_strategies, key=lambda x: x['effectiveness_score'], reverse=True)
    ...
    def _get_constitution_prevention_strategy(self, constitution_type: str) -> Dict:
        ...
        return strategies.get(constitution_type, {
            "strategy_name": "体质调理",
    ...
    def _calculate_overall_risk_score(
        self,
        immediate_risks: List[Dict],
        long_term_risks: List[Dict]
    ) -> float:
        """计算总体风险评分"""
        ...
        immediate_avg = 0.0
        if immediate_risks:
            immediate_avg = sum(r['probability'] for r in immediate_risks) / len(immediate_risks)
            
        long_term_avg = 0.0
        if long_term_risks:
            long_term_avg = sum(r['probability'] for r in long_term_risks) / len(long_term_risks)
            
        overall_score = immediate_avg * immediate_weight + long_term_avg * long_term_weight
