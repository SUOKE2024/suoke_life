...
class ConversationProcessor:
    ...
    def __init__(self, config: Dict, llm_client: LLMClient, knowledge_connector: KnowledgeConnector):
        """
        初始化会话处理器
        ...
        self.system_prompt = self._load_system_prompt(config.get("system_prompt_path", ""))
    ...
    async def process_message(
        ...
        # 限制历史长度
        if len(session['history']) > self.max_history_length * 2:  # 因为每次对话有用户和助手两条消息
        ...
        try:
            ...
            error_response = f"很抱歉，我现在无法处理您的问题。错误：{str(e)}"
    ...
    async def reset_session(self, session_id: str) -> Dict:
        ...
        self._active_sessions[session_id]["previous_session_count"] = session.get("previous_session_count", 0) + 1
    ...
    def _get_or_create_session(self, session_id: str, user_id: str, metadata: Dict = None) -> Dict:
        """获取或创建会话"""
...
""".strip()
        ...
        try:
            ...
            try:
                ...
                if "symptoms" in result and isinstance(result["symptoms"], list):
                    ...
                    new_symptoms = [s for s in result["symptoms"] if s and s not in existing_symptoms]
                    
                    if new_symptoms:
                        session["symptoms"] = list(existing_symptoms.union(new_symptoms))
            ...
            if symptom_analyses or syndrome_patterns:
                ...
                if symptom_analyses:
                    ...
                    for analysis in symptom_analyses:
                        ...
                        context_parts.append(f"- 症状 [{analysis['name']}] 可能的病因: {causes}")
                ...
                if syndrome_patterns:
                    ...
                    for pattern in syndrome_patterns[:2]:  # 限制显示的证型数量
                        context_parts.append(f"- {pattern['title']}: {pattern['content']}")
    ...
    async def _generate_response(self, session: Dict, message: str, enhanced_context: str) -> str:
        """生成回复"""
        # 构建完整的消息历史
        ...
        try:
            ...
            return f"抱歉，我现在无法回答您的问题。技术原因：{str(e)}"
...
""".strip()
        ...
        try:
            ...
            try:
                ...
                if "syndrome_patterns" in data and isinstance(data["syndrome_patterns"], list):
                    # 更新会话中的辨证结果
                    session["syndrome_patterns"] = data["syndrome_patterns"]
                    session["diagnosis_confidence"] = data.get("overall_confidence", 0.0)
    ...
    async def _enrich_syndrome_patterns(self, session: Dict) -> None:
        ...
        for pattern in session["syndrome_patterns"]:
            try:
                # 获取证型详细信息
                if "name" in pattern:
                    info = await self.knowledge_connector.get_syndrome_information(pattern["name"])
                    
                    # 添加详细信息但保留原始置信度
                    pattern["details"] = info
            except Exception as e:
                logger.error(f"获取证型详细信息失败: {str(e)}")
