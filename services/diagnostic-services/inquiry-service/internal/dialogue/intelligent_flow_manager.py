...
from ..common.utils import calculate_confidence, normalize_text
...
class IntelligentFlowManager(BaseService):
    ...
    async def generate_next_questions(
        ...
        try:
            ...
            logger.debug(f"生成问题完成: {session_id}, 数量: {len(formatted_questions)}")
    ...
    async def _analyze_stage_completion(
        ...
        # 计算已回答问题数
        stage_questions = [
            q_id for q_id, template in self.question_templates.items()
        ...
        # 计算置信度
        avg_confidence = np.mean(list(context.confidence_scores.values())) if context.confidence_scores else 0.0
    ...
    async def _evaluate_condition(
        ...
        if condition == "session_started":
            ...
            return any(keyword in str(context.collected_data.get('chief_complaint', ''))
                      for keyword in self.flow_config['emergency_keywords'])
            
        elif condition == "chief_complaint_clear":
            return (stage_completion['information_adequacy'] > 0.7 and
                   'chief_complaint' in context.collected_data)
            
        elif condition == "symptoms_well_characterized":
            return (stage_completion['completion_rate'] > 0.6 and
                   stage_completion['average_confidence'] > 0.7)
            ...
            return len(context.risk_factors) > 0 or stage_completion['completion_rate'] > 0.5
    ...
    async def _calculate_information_adequacy(
        self,
        context: InquiryContext
    ) -> float:
        """计算信息充分度"""
    ...
    async def get_session_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_confidence': np.mean(list(context.confidence_scores.values())) if context.confidence_scores else 0.0
        }
