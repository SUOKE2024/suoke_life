...
class DialogueManager:
    ...
    def __init__(
        ...
        # 会话配置
        self.session_timeout = config.get('dialogue', {}).get('session_timeout_minutes', 60) * 60
        self.max_history = config.get('dialogue', {}).get('max_session_history', 50)
        self.default_language = config.get('dialogue', {}).get('default_language', 'zh-CN')
        self.default_follow_up_count = config.get('dialogue', {}).get('default_follow_up_count', 3)
        
        # 活跃会话缓存
        self.active_sessions: Dict[str, Dict] = {}
        
        logger.info("对话管理器初始化完成，支持 %d 个并发会话", 
    ...
    async def start_session(
        ...
        # 生成欢迎信息和建议问题
        welcome_message = await self._generate_welcome_message(user, session_type, language)
        suggested_questions = await self._generate_suggested_questions(user, session_type, language)
        
        logger.info(f"用户 {user_id} 开始新会话 {session_id}，类型: {session_type}")
        
        return session_id, welcome_message, suggested_questions

    async def _generate_welcome_message(self, user: Dict, session_type: str, language: str) -> str:
        """生成会话欢迎信息"""
        ...
        try:
            ...
            if session_type == "general":
                ...
                if last_session:
                    last_time = datetime.fromtimestamp(last_session.get('created_at', 0))
                ...
                welcome_template = f"您好，{user_name}，{time_of_day}好！有什么可以帮助您的吗？"
                
            return welcome_template
            
        except Exception as e:
            logger.error(f"生成欢迎信息失败: {str(e)}")
    ...
    async def _get_last_session(self, user_id: str) -> Optional[Dict]:
        """获取用户的上一次会话"""
    ...
    async def _generate_suggested_questions(self, user: Dict, session_type: str, language: str) -> List[str]:
        """生成建议问题列表"""
        try:
            ...
            if session_type == "general":
                ...
                if last_session and 'summary' in last_session:
                    ...
                    recommendations = last_session.get('summary', {}).get('recommendations', [])
                    
                    if symptoms:
                        symptom_names = [s.get('name', 'symptoms') for s in symptoms[:2]]
                        suggested_questions.append(f"我上次提到的{'/'.join(symptom_names)}情况有好转吗？")
                    
                    if recommendations:
                        rec_types = [r.get('type', 'recommendation') for r in recommendations[:2]]
                        suggested_questions.append(f"我按照建议{'/'.join(rec_types)}执行了，效果如何？")
    ...
    async def _get_session(self, session_id: str) -> Optional[Dict]:
        ...
        if session_id in self.active_sessions:
            ...
            if time.time() - session['last_interaction'] > self.session_timeout:
                ...
                # 更新会话状态为过期
                await self.session_repository.update_session_status(session_id, 'expired')
                return None
            return session
    ...
    async def _generate_llm_response(self, session: Dict) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'response_type': 'TEXT',
                'detected_symptoms': [],
                'follow_up_questions': ['您能换个方式描述您的问题吗？', '您可以尝试问一些其他问题。']
            }
    ...
    async def _generate_session_summary(self, session: Dict) -> Dict:
        ...
        try:
            ...
            # 映射到中医证型
            tcm_patterns = await self._map_to_tcm_patterns(detected_symptoms, session['user_id'])
    ...
    async def _map_to_tcm_patterns(self, symptoms: List[Dict], user_id: str) -> List[Dict]:
        """将症状映射到中医证型"""
        # 这里需要实现更复杂的中医证型映射逻辑
        # 为简化示例，我们返回一些简单的示例证型
        if not symptoms:
        ...
        # 获取用户体质信息
        user = await self.user_repository.get_user_by_id(user_id)
        constitution = user.get('health_profile', {}).get('constitution_type', 'BALANCED')
        ...
        # 检查是否存在疲劳、疲惫等气虚症状
        if any(s in symptom_names for s in ['疲劳', '乏力', '气短', '少气懒言', '疲惫']):
            patterns.append({
                'pattern_name': '气虚证',
                'category': '虚证',
                'match_score': 0.85,
                'related_symptoms': [s for s in symptom_names if s in ['疲劳', '乏力', '气短', '少气懒言', '疲惫']],
                'description': '气虚证是指人体内气的不足所导致的一系列症状'
            })
            
        # 检查是否存在痰湿症状
        if any(s in symptom_names for s in ['痰多', '浮肿', '肥胖', '胸闷', '恶心', '胃胀']):
            patterns.append({
                'pattern_name': '痰湿证',
                'category': '实证',
                'match_score': 0.8,
                'related_symptoms': [s for s in symptom_names if s in ['痰多', '浮肿', '肥胖', '胸闷', '恶心', '胃胀']],
                'description': '痰湿证是指体内水液代谢异常，聚湿成痰所导致的一系列症状'
            })
            
        # 检查是否存在阴虚症状
        if any(s in symptom_names for s in ['口干', '咽干', '潮热', '盗汗', '失眠', '五心烦热']):
            patterns.append({
                'pattern_name': '阴虚证',
                'category': '虚证',
                'match_score': 0.75,
                'related_symptoms': [s for s in symptom_names if s in ['口干', '咽干', '潮热', '盗汗', '失眠', '五心烦热']],
            ...
            for pattern in tcm_patterns:
                if pattern['pattern_name'] == '气虚证':
                    recommendations.append({
                        ...
                        'suggested_timeframe': int(time.time()) + 7 * 24 * 3600  # 一周后
                    })
                    recommendations.append({
