...
def calculate_confidence(factors: List[float], weights: Optional[List[float]] = None) -> float:
    """计算置信度"""
    if not factors:
        return 0.0
...
def parse_time_expression(expression: str) -> Optional[timedelta]:
    """解析时间表达式"""
    patterns = {
...
async def batch_process_async(items: List[Any], batch_size: int, processor: Callable) -> List[Any]:
    """异步批量处理"""
    results = []
