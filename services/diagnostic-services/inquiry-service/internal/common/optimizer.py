...
class ParallelProcessor:
    ...
    async def process_parallel(self, items: List[Any], processor: Callable, chunk_size: int = None) -> List[Any]:
        """并行处理项目列表"""
        if not items:
            return []
        ...
        for chunk in chunks:
            if self.use_processes:
                task = loop.run_in_executor(self.executor, self._process_chunk, processor, chunk)
            else:
                task = loop.run_in_executor(self.executor, self._process_chunk_sync, processor, chunk)
            tasks.append(task)
        ...
        for result in results:
            if isinstance(result, Exception):
                self.logger.error(f"并行处理出错: {str(result)}")
                raise result
    ...
    def _process_chunk_sync(self, processor: Callable, chunk: List[Any]) -> List[Any]:
        ...
        if asyncio.iscoroutinefunction(processor):
            ...
            try:
                return loop.run_until_complete(self._process_chunk_async(processor, chunk))
            finally:
                loop.close()
        else:
            return [processor(item) for item in chunk]
    
    async def _process_chunk_async(self, processor: Callable, chunk: List[Any]) -> List[Any]:
        """异步处理数据块"""
