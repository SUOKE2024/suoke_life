...
class CacheConfig:
    ...
    memory: dict[str, Any] = field(
        default_factory=lambda: {"max_size": 1000, "default_ttl": 3600}
    )
...
class DatabaseConfig:
    ...
    sqlite: dict[str, Any] = field(default_factory=lambda: {"path": "data/inquiry.db"})
...
class ConfigManager:
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                self._logger.warning(
                    f"配置文件不存在: {self.config_path}，使用默认配置"
                )
                self._config_data = {}
        except Exception as e:
            self._logger.error(f"加载配置文件失败: {e!s}")
            raise ConfigurationError(f"加载配置文件失败: {e!s}")
    ...
    def _get_config_section(
        self, section: str, default: dict | None = None
    ) -> dict[str, Any]:
        """获取配置段"""
    ...
    def _create_cache_config(self) -> CacheConfig:
        ...
        return CacheConfig(
            ...
            memory=config.get("memory", {"max_size": 1000, "default_ttl": 3600}),
            redis=config.get(
                "redis",
                {
                    "url": "redis://localhost:6379/0",
    ...
    def _create_health_risk_assessment_config(self) -> HealthRiskAssessmentConfig:
        ...
        return HealthRiskAssessmentConfig(
            ...
            enable_body_constitution_analysis=config.get(
                "enable_body_constitution_analysis", True
            ),
    ...
    def _create_security_config(self) -> SecurityConfig:
        ...
        return SecurityConfig(
            ...
            rate_limit_requests_per_minute=config.get(
    ...
    def save_config(self, path: str | None = None) -> None:
        ...
        try:
            ...
            self._logger.error(f"保存配置失败: {e!s}")
            raise ConfigurationError(f"保存配置失败: {e!s}")
    ...
    def validate_config(self) -> List[str]:
        ...
        if not (0.0 <= self.health_risk_assessment.min_confidence <= 1.0):
            errors.append(
                f"无效的风险评估最小置信度: {self.health_risk_assessment.min_confidence}"
            )
