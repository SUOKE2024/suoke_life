...
class MetricsCollector:
    ...
    def _setup_prometheus_metrics(self) -> None:
        ...
        self.prom_tcm_mappings_total = Counter(
            'tcm_mappings_total',
            'Total number of TCM pattern mappings',
            ['status']
        )
        ...
        self.prom_tcm_mapping_duration = Histogram(
            'tcm_mapping_duration_seconds',
            'TCM pattern mapping duration in seconds'
        )
        ...
        self.prom_active_sessions = Gauge(
            'inquiry_active_sessions',
            'Number of active inquiry sessions'
        )
        ...
        self.prom_memory_usage = Gauge(
            'inquiry_memory_usage_bytes',
            'Memory usage in bytes'
        )
        
        # 信息
        self.prom_service_info = Info(
            'inquiry_service_info',
            'Service information'
        )
    ...
    def start_prometheus_server(self) -> None:
        ...
        try:
            ...
            self._logger.error(f"启动Prometheus服务器失败: {str(e)}")
    ...
    def record_duration(self, operation: str, duration: float, is_error: bool = False, 
        ...
        with self._lock:
            ...
            if self.enable_prometheus:
                ...
                elif operation == 'symptom_extraction' and hasattr(self, 'prom_symptom_extraction_duration'):
                    self.prom_symptom_extraction_duration.observe(duration)
                elif operation == 'tcm_mapping' and hasattr(self, 'prom_tcm_mapping_duration'):
                    self.prom_tcm_mapping_duration.observe(duration)
                elif operation == 'risk_assessment' and hasattr(self, 'prom_risk_assessment_duration'):
    ...
    def get_performance_stats(self, operation: Optional[str] = None) -> Dict[str, Dict[str, Any]]:
        """获取性能统计信息"""
        with self._lock:
            if operation:
            ...
            return {
                op: stats.to_dict() 
                for op, stats in self._performance_stats.items()
            }
...
def timer(metrics: MetricsCollector, operation: str, labels: Optional[Dict[str, str]] = None):
    """计时装饰器"""
...
def counter(metrics: MetricsCollector, name: str, labels: Optional[Dict[str, str]] = None):
    """计数装饰器"""
    def decorator(func: Callable) -> Callable:
        async def async_wrapper(*args, **kwargs):
            try:
                result = await func(*args, **kwargs)
                metrics.increment_counter(name, 1, {**(labels or {}), "status": "success"})
                return result
            except Exception as e:
                metrics.increment_counter(name, 1, {**(labels or {}), "status": "error"})
                raise
