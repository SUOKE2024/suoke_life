...
class HealthMonitor(BaseService):
    ...
    async def start_monitoring(self) -> None:
        ...
        try:
            ...
            if self.system_metrics_enabled:
                self._system_monitoring_task = asyncio.create_task(self._system_monitoring_loop())
        ...
        for attempt in range(check.retries + 1):
            try:
                ...
                if attempt < check.retries:
                    self.logger.warning(f"{error_msg}, retrying ({attempt + 1}/{check.retries})")
                    continue
                else:
                    result = HealthResult(
                        name=check.name,
    ...
    async def record_metric(self, metric: MetricValue) -> None:
        ...
        if metric.metric_type == MetricType.COUNTER:
            await self.metrics_collector.increment(metric.name, metric.value, metric.tags)
        elif metric.metric_type == MetricType.GAUGE:
            await self.metrics_collector.gauge(metric.name, metric.value, metric.tags)
        elif metric.metric_type == MetricType.HISTOGRAM:
            await self.metrics_collector.histogram(metric.name, metric.value, metric.tags)
        ...
        for rule in self.threshold_rules:
            if rule.metric_name in self.metrics:
                ...
                if recent_metrics:
                    ...
                    # 检查阈值
                    if self._evaluate_threshold(latest_value, rule.operator, rule.threshold):
                        # 检查是否持续超过阈值
    ...
    def _check_duration_threshold(self, metrics: List[MetricValue], rule: ThresholdRule) -> bool:
        """检查持续时间阈值"""
        if len(metrics) < 2:
            return False
    ...
    async def _trigger_threshold_alert(self, rule: ThresholdRule, value: float) -> None:
        ...
        message = rule.message_template.format(value=value) if rule.message_template else f"{rule.metric_name} {rule.operator} {rule.threshold}"
        ...
        alert = Alert(
            ...
            details={"value": value, "threshold": rule.threshold, "operator": rule.operator}
        )
    ...
    async def _check_health_alerts(self, result: HealthResult) -> None:
        ...
        if result.status in [HealthStatus.CRITICAL, HealthStatus.DOWN]:
            ...
            if alert_id not in self.alerts or self.alerts[alert_id].resolved:
                severity = AlertSeverity.CRITICAL if result.status == HealthStatus.DOWN else AlertSeverity.WARNING
    ...
    async def _update_statistics(self) -> None:
        ...
        await self.metrics_collector.gauge("health_monitor_active_alerts", self.stats["active_alerts"])
        await self.metrics_collector.gauge("health_monitor_total_checks", self.stats["total_checks"])
        
        # 计算成功率
        if self.stats["total_checks"] > 0:
            success_rate = self.stats["successful_checks"] / self.stats["total_checks"]
            await self.metrics_collector.gauge("health_monitor_success_rate", success_rate)
