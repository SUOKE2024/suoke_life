...
class InquiryServiceServicer(pb2_grpc.InquiryServiceServicer):
    ...
    async def StartInquirySession(self, request, context):
        ...
        try:
            ...
            logger.error(f"开始问诊会话失败: {e!s}")
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(f"开始问诊会话失败: {e!s}")
    ...
    async def InteractWithUser(self, request, context):
        ...
        try:
            ...
            logger.info(
                f"用户交互: session_id={session_id}, message={user_message[:50]}..."
            )
            ...
            # 会话不存在或已过期
            logger.error(f"用户交互失败(会话无效): {e!s}")
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details(f"会话不存在或已过期: {e!s}")
            yield pb2.InteractionResponse()
        except Exception as e:
            # 其他错误
            logger.error(f"用户交互失败: {e!s}")
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(f"用户交互失败: {e!s}")
            yield pb2.InteractionResponse()
    ...
    async def EndInquirySession(self, request, context):
        ...
        try:
            ...
            # 会话不存在或已过期
            logger.error(f"结束问诊会话失败(会话无效): {e!s}")
            context.set_code(grpc.StatusCode.NOT_FOUND)
            context.set_details(f"会话不存在或已过期: {e!s}")
            return pb2.InquirySummary()
        except Exception as e:
            # 其他错误
            logger.error(f"结束问诊会话失败: {e!s}")
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(f"结束问诊会话失败: {e!s}")
            return pb2.InquirySummary()
    ...
    async def AnalyzeMedicalHistory(self, request, context):
        ...
        try:
            ...
            user_id = request.user_id
            medical_records = [
                MessageToDict(record) for record in request.medical_records
            ]
            ...
            logger.info(
                f"分析病史: user_id={user_id}, records_count={len(medical_records)}"
            )

            return response

        except Exception as e:
            logger.error(f"分析病史失败: {e!s}")
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(f"分析病史失败: {e!s}")
    ...
    async def ExtractSymptoms(self, request, context):
        ...
        try:
            ...
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(f"提取症状失败: {e!s}")
    ...
    async def MapToTCMPatterns(self, request, context):
        ...
        try:
            ...
            body_locations = [
                MessageToDict(location) for location in request.body_locations
            ]
            temporal_factors = [
                MessageToDict(factor) for factor in request.temporal_factors
            ]
            ...
            logger.info(
                f"中医证型匹配: primary_patterns_count={len(primary_patterns)}, secondary_patterns_count={len(secondary_patterns)}"
            )

            return response

        except Exception as e:
            logger.error(f"中医证型匹配失败: {e!s}")
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(f"中医证型匹配失败: {e!s}")
        ...
        try:
            ...
            for insight in insights:
                insight.supporting_evidence.extend(
                    ["多次问诊记录分析", "症状频率统计", "时间序列对比"]
                )
                response.insights.append(insight)

            logger.info(
                f"批量分析健康数据: session_count={len(session_ids)}, analysis_type={analysis_type}"
            )

            return response

        except Exception as e:
            logger.error(f"批量分析健康数据失败: {e!s}")
            context.set_code(grpc.StatusCode.INTERNAL)
            context.set_details(f"批量分析健康数据失败: {e!s}")
    ...
    async def AssessHealthRisks(self, request, context):
        ...
        try:
            ...
            user_id = request.user_id
            current_symptoms = [
                MessageToDict(symptom) for symptom in request.current_symptoms
            ]
