...
class KnowledgeConnector:
    ...
    def __init__(self, config: Dict):
        ...
        self.knowledge_service_url = f"{self.knowledge_service_host}:{self.knowledge_service_port}"
        ...
        logger.info(f"医学知识库连接器初始化完成，服务地址: {self.knowledge_service_url}")
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.error(f"连接医学知识库服务失败: {str(e)}")
            return False
    ...
    async def search_knowledge(
        ...
        try:
            ...
            logger.error(f"搜索医学知识失败: {str(e)}, query={query}")
            return []
    
    async def _fallback_http_search(self, query: str, knowledge_type: str, context: Dict = None) -> List[Dict]:
        """备用HTTP接口搜索"""
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(f"HTTP备用接口错误: {response.status_code}, {response.text}")
                return []
        except Exception as e:
            logger.error(f"HTTP备用接口调用失败: {str(e)}")
            return []
    ...
    async def get_syndrome_information(self, syndrome_name: str) -> Dict:
        ...
        try:
            ...
            response = await self.stub.GetSyndrome(request, timeout=self.timeout_ms/1000)
            return self._convert_syndrome(response)
            
        except grpc.RpcError as e:
            status_code = e.code()
            status_details = e.details()
            logger.error(f"获取证型信息失败: {status_code}, {status_details}, syndrome={syndrome_name}")
            ...
            logger.error(f"获取证型信息失败: {str(e)}, syndrome={syndrome_name}")
    ...
    async def get_herb_information(self, herb_name: str) -> Dict:
        ...
        try:
            ...
            status_code = e.code()
            status_details = e.details()
            logger.error(f"获取中药材信息失败: {status_code}, {status_details}, herb={herb_name}")
            ...
            logger.error(f"获取中药材信息失败: {str(e)}, herb={herb_name}")
    ...
    async def get_symptom_analysis(self, symptom: str) -> Dict:
        ...
        try:
            ...
            response = await self.stub.AnalyzeSymptom(request, timeout=self.timeout_ms/1000)
            return self._convert_symptom_analysis(response)
            
        except grpc.RpcError as e:
            status_code = e.code()
            status_details = e.details()
            logger.error(f"获取症状分析失败: {status_code}, {status_details}, symptom={symptom}")
            
            # 处理找不到数据的情况
            if status_code == grpc.StatusCode.NOT_FOUND:
                return {"name": symptom, "tcm_analysis": {"common_causes": ["数据不足"], "differentiation": "未找到相关症状分析"}}
            
            raise
        except Exception as e:
            logger.error(f"获取症状分析失败: {str(e)}, symptom={symptom}")
