...
class TCMKnowledgeGraph(BaseService):
    ...
    def _load_symptom_entities(self):
        """加载症状实体"""
        symptoms = [
            {
                'id': 'symptom_headache',
    ...
    def _load_syndrome_entities(self):
        """加载证型实体"""
        syndromes = [
            {
    ...
    def _load_formula_entities(self):
        """加载方剂实体"""
        formulas = [
            {
                'id': 'formula_sijunzi',
    ...
    async def map_symptoms_to_syndromes(
        ...
        try:
            ...
            for syndrome in syndrome_entities:
                ...
                if mapping.match_score >= self.inference_config['syndrome_match_threshold']:
    ...
    async def _calculate_context_bonus(
        ...
        if syndrome.name in ['气虚证', '阳虚证'] and age > 60:
            bonus += 0.1
        elif syndrome.name in ['肝气郁结证'] and 20 <= age <= 50:
            bonus += 0.1
    ...
    async def _find_related_formulas(self, syndrome_id: str) -> List[Tuple[str, float]]:
        ...
        for relation_id in relation_ids:
            ...
            if (relation.relation_type == RelationType.SYNDROME_TREATS and
                relation.source_id == syndrome_id):
    ...
    async def _calculate_patient_adaptation(
        ...
        constitution = patient_context.get('constitution', '')
        if constitution:
            if '虚' in constitution and '补' in formula_props.get('functions', []):
                adaptation += 0.15
            elif '热' in constitution and '清' in formula_props.get('functions', []):
                adaptation += 0.15
            ...
            if rec.formula_id not in seen_formulas:
                ...
                for existing_rec in deduplicated:
                    if existing_rec.formula_id == rec.formula_id:
                        ...
                        # 取较高的推荐分数
                        if rec.recommendation_score > existing_rec.recommendation_score:
    ...
    async def _recursive_relation_query(
        ...
        for relation_id in relation_ids:
            ...
            if depth > 1:
                next_entity_id = (
                    relation.target_id if relation.source_id == entity_id
    ...
    async def get_knowledge_stats(self) -> Dict[str, Any]:
        ...
        for entity_type in TCMEntityType:
            entity_stats[entity_type.value] = len(
