...
class TracingManager:
    ...
    def _setup_tracing(self) -> None:
        ...
        try:
            ...
            resource = Resource.create({
                ...
                ResourceAttributes.SERVICE_VERSION: self.config.get("service_version", "unknown"),
                ResourceAttributes.DEPLOYMENT_ENVIRONMENT: os.environ.get("SERVICE_ENV", "development"),
            })
            ...
            logger.info(f"分布式追踪已初始化，服务名: {self.service_name}, 端点: {self.otlp_endpoint}")
        except Exception as e:
            logger.error(f"初始化追踪时出错: {str(e)}")
            self.enabled = False
    ...
    def trace_function(self, name: Optional[str] = None, attributes: Optional[Dict[str, Any]] = None) -> Callable[[F], F]:
        """
        函数追踪装饰器
