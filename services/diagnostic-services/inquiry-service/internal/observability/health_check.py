...
class HealthChecker:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.checks: Dict[str, Callable[[], Awaitable[Tuple[bool, Optional[str], Dict[str, Any]]]]] = {}
    ...
    async def _check_memory(self) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        """检查内存使用情况"""
        try:
            import psutil
    ...
    async def _check_cpu(self) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        """检查CPU使用情况"""
        try:
            import psutil
            # 获取CPU使用率
            ...
            return False, f"CPU检查失败: {str(e)}", {}
    
    async def _check_disk(self) -> Tuple[bool, Optional[str], Dict[str, Any]]:
        """检查磁盘使用情况"""
        try:
            import psutil
            # 获取服务目录所在磁盘使用情况
    ...
    def register_check(self, name: str, check_func: Callable[[], Awaitable[Tuple[bool, Optional[str], Dict[str, Any]]]]) -> None:
        """
        注册健康检查函数
    ...
    async def check_health(self) -> Dict[str, ComponentHealth]:
        ...
        for name, check_func in self.checks.items():
            ...
            try:
                ...
                logger.error(f"执行 {name} 健康检查时出错: {str(e)}")
                component.status = HealthStatus.WARNING
                component.description = f"检查执行出错: {str(e)}"
                component.consecutive_failures += 1
    ...
    async def _run_periodic_checks(self) -> None:
        ...
        try:
                ...
                if status != HealthStatus.SERVING:
                    ...
                    for name, component in self.components.items():
                        if component.status != HealthStatus.SERVING:
                            logger.warning(f"组件 {name} 状态: {component.status.value}, 原因: {component.description}")
            ...
            logger.error(f"健康检查任务出错: {str(e)}", exc_info=True)
            self.is_running = False 