...
class DiagnosticAssessment:
    ...
    primary_diagnosis: Optional[DiagnosisResult]
...
class DiagnosticReasoningEngine(BaseService):
        ...
        return [
            {
                ...
                "condition": lambda symptoms: any(s.name == "胸痛" and s.severity >= 7 for s in symptoms),
            ...
            },
            {
                ...
                "condition": lambda symptoms: (
                    any(s.name == "头痛" and s.severity >= 8 for s in symptoms) and
                    any(s.name == "发热" for s in symptoms)
                ),
            ...
            },
            {
                ...
                "condition": lambda symptoms: any(s.name == "呼吸困难" and s.severity >= 6 for s in symptoms),
                "action": "immediate_medical_attention",
                "priority": 10
            }
        ]
    ...
    async def perform_diagnostic_assessment(
        ...
        try:
            ...
            # 确定主要诊断
            primary_diagnosis = await self._determine_primary_diagnosis(differential_diagnoses)
            ...
            recommendations = await self._generate_recommendations(
                processed_symptoms, differential_diagnoses, overall_risk_level, patient_context
            )
    ...
    async def _calculate_differential_diagnoses(
        ...
        for disease_id, disease in self.disease_database.items():
            ...
            if probability >= self.reasoning_config['min_probability_threshold']:
                # 确定置信度
                confidence = await self._determine_confidence_level(probability, disease, symptoms)
                
                # 识别支持和缺失的症状
                supporting_symptoms = [s for s in symptom_names if s in disease.typical_symptoms]
                missing_symptoms = [s for s in disease.typical_symptoms if s not in symptom_names]
                
                # 评估风险等级
                risk_level = await self._assess_disease_risk(disease, symptoms, patient_context)
    ...
    async def _calculate_simple_probability(
        ...
        if disease.required_symptoms:
            required_present = all(req in symptom_names for req in disease.required_symptoms)
            if not required_present:
                return 0.0
        
        # 检查排除症状
        if disease.exclusion_symptoms:
            exclusion_present = any(excl in symptom_names for excl in disease.exclusion_symptoms)
            if exclusion_present:
                return 0.0
        
        # 计算匹配度
        matching_symptoms = len([s for s in symptom_names if s in disease.typical_symptoms])
    ...
    async def _generate_disease_reasoning(
        ...
        # 概率说明
        reasoning_parts.append(f"基于症状分析，{disease.name}的可能性为{probability:.1%}")
        
        # 支持症状
        if supporting_symptoms:
            reasoning_parts.append(f"支持该诊断的症状包括：{', '.join(supporting_symptoms)}")
    ...
    async def _determine_primary_diagnosis(
        ...
        for diagnosis in differential_diagnoses:
            if (diagnosis.probability >= self.reasoning_config['confidence_threshold'] and
    ...
    async def _assess_overall_risk(
        ...
        has_emergency = any(s.name in emergency_symptoms and s.severity >= 7 for s in symptoms)
        ...
        if differential_diagnoses:
            max_risk = max([d.risk_level for d in differential_diagnoses], 
    ...
    async def _calculate_overall_confidence(
        ...
        overall_confidence = primary_confidence * diagnosis_count_factor * symptom_completeness
    ...
    async def _generate_reasoning_summary(
        ...
        if len(differential_diagnoses) > 1:
            ...
            summary_parts.append(f"需要考虑的其他可能诊断包括：{', '.join(other_diagnoses)}")
    ...
    @cached(ttl=600)
    async def get_assessment_by_id(self, assessment_id: str) -> Optional[DiagnosticAssessment]:
        """根据ID获取评估结果"""
    ...
    async def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            'success_rate': (
                self.stats['successful_diagnoses'] / max(self.stats['total_assessments'], 1)
            ),
            'high_confidence_rate': (
                self.stats['high_confidence_diagnoses'] / max(self.stats['successful_diagnoses'], 1)
            ),
            'emergency_detection_rate': (
                self.stats['emergency_cases_detected'] / max(self.stats['total_assessments'], 1)
            )
        } 