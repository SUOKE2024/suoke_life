...
class MultimodalProcessor(BaseService):
    ...
    async def _process_voice_input(self, input_data: InputData) -> ProcessingResult:
        ...
        try:
            ...
            # 提取文本特征
            text_features = await self._extract_text_features(voice_analysis.transcribed_text)
    ...
    async def _detect_medical_terms(self, text: str) -> bool:
        """检测医学术语"""
        medical_terms = [
            '头痛', '发热', '咳嗽', '乏力', '恶心', '呕吐', '腹痛', '腹泻',
            '胸痛', '心悸', '气短', '失眠', '头晕', '耳鸣', '便秘'
        ]
    ...
    async def _analyze_image(self, image_data: bytes) -> ImageAnalysis:
        ...
        try:
            ...
            # 生成描述
            description = await self._generate_image_description(image, detected_objects)
    ...
    @cached(ttl=300)
    async def get_processing_status(self, input_id: str) -> Optional[ProcessingResult]:
        """获取处理状态"""
    ...
    async def get_processing_history(
        ...
        results.sort(key=lambda x: x.metadata.get('timestamp', datetime.now()), reverse=True)
