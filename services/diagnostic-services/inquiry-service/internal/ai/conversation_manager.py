...
class ConversationManager(BaseService):
    ...
    async def start_conversation(
        ...
        try:
            ...
            # 个性化设置
            if patient_profile:
                context.preferred_style = await self._determine_preferred_style(patient_profile)
    ...
    async def process_message(
        ...
        try:
            ...
            # 更新对话状态
            await self._update_conversation_state(context, user_message, detected_emotion)
            
            # 选择对话策略
            strategy = await self._select_conversation_strategy(context, detected_emotion)
    ...
    async def _assess_urgency(self, message: str) -> int:
        """评估紧急程度"""
        urgent_keywords = [
            "急", "紧急", "严重", "剧烈", "无法忍受", "马上", "立即",
            "呼吸困难", "胸痛", "大出血", "昏迷", "休克", "抽搐"
        ]
    ...
    async def _update_conversation_state(
        ...
        elif current_state == ConversationState.EMPATHY:
            context.current_state = ConversationState.INFORMATION_GATHERING
    ...
    async def _generate_response(
        ...
        try:
            # 确定回复风格
            response_style = await self._determine_response_style(context, emotion, strategy)
    ...
    async def _generate_main_response(
        ...
        # 获取模板
        templates = self.response_templates.get(state.value, {}).get(response_style.value, [])
        ...
        if templates:
            ...
            if "{question}" in template:
                question = await self._generate_contextual_question(context, user_message)
    ...
    def _update_response_stats(self, processing_time: float):
        ...
        total_responses = sum(len(conv.conversation_history) for conv in self.active_conversations.values())
    ...
    async def get_conversation_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_interaction': context.conversation_history[-1].timestamp.isoformat() if context.conversation_history else None
        }
        ...
        return {
            **self.stats,
            ...
            'emotion_detection_rate': (
                self.stats['emotion_detections'] / max(self.stats['total_conversations'], 1)
            ),
            'personalization_rate': (
                self.stats['personalized_responses'] / max(self.stats['total_conversations'], 1)
            )
        } 