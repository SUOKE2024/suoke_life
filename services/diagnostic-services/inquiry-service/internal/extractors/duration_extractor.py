...
class DurationExtractor(BaseService):
    ...
    async def extract_duration(self, text: str, symptom: str) -> int:
        ...
        try:
            ...
            self.logger.error(f"持续时间提取失败: {e!s}")
            return 0

    async def extract_detailed_duration(
        ...
        try:
            ...
            self.logger.error(f"详细持续时间提取失败: {e!s}")
    ...
    def _extract_exact_duration(self, context: str) -> float | None:
        """提取精确的数字时间"""
    ...
    def _extract_chinese_duration(self, context: str) -> float | None:
        """提取中文数字时间"""
    ...
    def _extract_fuzzy_duration(self, context: str) -> float | None:
        """提取模糊时间"""
    ...
    def _extract_range_duration(self, context: str) -> float | None:
        """提取范围时间"""
        # 匹配 "3到5天" 或 "3-5天" 格式
        patterns = [
            r"(\d+)\s*到\s*(\d+)\s*(天|日|周|星期|月|年)",
    ...
    async def compare_durations(
        ...
        try:
            ...
            self.logger.error(f"持续时间比较失败: {e!s}")
    ...
    async def extract_temporal_patterns(self, text: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"时间模式提取失败: {e!s}")
            return {}
