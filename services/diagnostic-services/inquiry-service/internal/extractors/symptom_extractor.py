...
from ..common.metrics import timer, counter
...
class OptimizedSymptomExtractor(BaseExtractor):
    ...
    async def extract(self, text: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            # 文本预处理
            cleaned_text = sanitize_text(text, self.config.get('max_text_length', 10000))
            ...
            raise ProcessingError(f"症状提取失败: {str(e)}", "extract_symptoms", e)
    ...
    async def _extract_body_locations(self, text: str) -> List[Dict[str, Any]]:
        ...
        # 身体部位关键词
        location_keywords = [
            "头部", "头", "颈部", "脖子", "胸部", "胸", "腹部", "肚子", "腰部", "腰",
            "背部", "背", "手", "手臂", "腿", "脚", "眼睛", "耳朵", "鼻子", "嘴"
        ]
    ...
    def _calculate_symptom_confidence(self, symptom: str, text: str, 
                                    severity: str, duration: int, 
    ...
    def _calculate_overall_confidence(self, symptoms: List[SymptomInfo], 
                                    body_locations: List[Dict], 
                                    temporal_factors: List[Dict]) -> float:
        """计算整体置信度"""
        factors = []
        ...
        # 身体部位置信度
        if body_locations:
            avg_location_confidence = sum(loc['confidence'] for loc in body_locations) / len(body_locations)
            factors.append(avg_location_confidence)
        
        # 时间因素置信度
        if temporal_factors:
            avg_temporal_confidence = sum(tf['confidence'] for tf in temporal_factors) / len(temporal_factors)
    ...
    def _load_symptom_keywords(self) -> List[str]:
        ...
        keywords = [
            ...
            "头痛", "头晕", "头胀", "偏头痛", "头重",
            # 胸部症状
            "胸闷", "胸痛", "心悸", "气短", "憋气", "咳嗽", "咳痰",
            # 腹部症状
            "腹痛", "腹胀", "恶心", "呕吐", "腹泻", "便秘", "消化不良",
            # 四肢症状
            "关节痛", "肌肉酸痛", "肢体麻木", "腿软", "手脚冰凉",
            # 感官症状
            "视力下降", "听力下降", "耳鸣", "嗅觉减退",
            # 睡眠症状
            "失眠", "多梦", "易醒", "嗜睡",
    ...
    async def extract_with_knowledge_base(self, text: str, knowledge_base) -> Dict[str, Any]:
        """结合知识库的增强提取"""
        # 基础提取
