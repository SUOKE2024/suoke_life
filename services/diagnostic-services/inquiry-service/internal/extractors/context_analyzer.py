...
class SymptomContextAnalyzer(BaseService):
    ...
    async def analyze_context(self, text: str, symptom: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'triggers': [], 'relievers': [], 'aggravators': [], 'accompanying': [], 'confidence': 0.0}
    ...
    async def _analyze_aggravators(self, context: str) -> List[Dict[str, Any]]:
        ...
        for category, keywords in self.aggravating_keywords.items():
            for keyword in keywords:
                if keyword in context:
                    confidence = self._calculate_aggravating_confidence(context, keyword)
                    if confidence > 0.3:
                        aggravators.append({
                            'factor': keyword,
                            'category': category,
    ...
    async def _analyze_accompanying_symptoms(self, context: str) -> List[Dict[str, Any]]:
        """分析伴随症状"""
        accompanying = []
        ...
        for category, keywords in self.accompanying_keywords.items():
            for keyword in keywords:
                if keyword in context:
                    confidence = self._calculate_accompanying_confidence(context, keyword)
                    if confidence > 0.4:
                        accompanying.append({
                            'symptom': keyword,
                            'category': category,
    ...
    def _calculate_trigger_confidence(self, context: str, keyword: str) -> float:
        ...
        # 检查诱发模式
        trigger_indicators = ['因为', '由于', '当', '在', '每当', '一旦', '遇到', '碰到']
    ...
    def _extract_pattern_relievers(self, context: str) -> List[Dict[str, Any]]:
        ...
        for pattern in self.context_patterns['relief']:
            ...
            for match in matches:
                if len(match.groups()) >= 2:
                    factor = match.group(2) if len(match.groups()) > 1 else match.group(1)
                    relievers.append({
                        'factor': factor,
                        'category': 'pattern_detected',
                        'confidence': 0.7,
    ...
    def _deduplicate_factors(self, factors: List[Dict[str, Any]], key: str = 'factor') -> List[Dict[str, Any]]:
        """去除重复的因素"""
        seen = set()
        unique_factors = []
