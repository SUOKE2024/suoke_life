...
class SeverityAnalyzer(BaseService):
    ...
    async def _do_initialize(self)-> None:
        ...
        self.impact_words = {
            ...
            "medium_impact": ["有些影响", "稍有影响", "轻微影响", "不太方便"],
            "low_impact": ["基本不影响", "影响不大", "还能", "勉强可以"],
        }
        ...
        try:
            ...
            for severity, data in self.severity_keywords.items():
                score = self._calculate_severity_score(
                    context, data["keywords"], data["score"]
                )
            ...
            # 选择得分最高的严重程度
            if not severity_scores or all(
                score == 0 for score in severity_scores.values()
            ):
                return "unknown"
            ...
            self.logger.error(f"严重程度分析失败: {e!s}")
            return "unknown"

    async def analyze_detailed_severity(
        self, text: str, symptom: str
    )-> dict[str, Any]:
        """详细的严重程度分析"""
        try:
            cleaned_text = sanitize_text(text)
            ...
            self.logger.error(f"详细严重程度分析失败: {e!s}")
    ...
    def _calculate_severity_score(
        self, context: str, keywords: list[str], base_score: float
    ) -> float:
        """计算严重程度得分"""
        score = 0.0
        matched_keywords = 0
    ...
    def _apply_modifier_weights(
        self, context: str, severity_scores: dict[str, float]
    )-> dict[str, float]:
        """应用修饰词加权"""
    ...
    def _analyze_frequency(self, context: str)-> dict[str, Any] | None:
        """分析频率"""
        for freq_level, freq_words in self.frequency_words.items():
            for word in freq_words:
                if word in context:
    ...
    def _analyze_impact(self, context: str)-> dict[str, Any] | None:
        """分析影响程度"""
        for impact_level, impact_words in self.impact_words.items():
            for word in impact_words:
                if word in context:
    ...
    async def compare_severities(
        self, text1: str, symptom1: str, text2: str, symptom2: str
    )-> dict[str, Any]:
        """比较两个症状的严重程度"""
        try:
            ...
            self.logger.error(f"严重程度比较失败: {e!s}")
