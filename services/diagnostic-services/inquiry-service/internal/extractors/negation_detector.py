...
class NegationDetector(BaseService):
    ...
    async def is_negated(self, text: str, target_word: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"否定检测失败: {str(e)}")
            return False
    ...
    async def get_negation_scope(self, text: str, negation_pos: int) -> Tuple[int, int]:
        ...
        try:
            ...
            self.logger.error(f"否定范围计算失败: {str(e)}")
    ...
    async def _is_position_negated(self, text: str, position: int, target_word: str) -> bool:
        """检查特定位置的词是否被否定"""
        # 获取否定检查范围
    ...
    def _calculate_negation_confidence(self, negation_word: str, target_word: str, text: str) -> float:
        """计算否定的置信度"""
    ...
    def _deduplicate_negations(self, negations: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """去除重复的否定检测结果"""
        seen = set()
        unique_negations = []
    ...
    async def analyze_negation_context(self, text: str, negation_word: str, target_word: str) -> Dict[str, Any]:
        """分析否定的上下文信息"""
        try:
            # 查找否定词和目标词的位置
            neg_pos = text.find(negation_word)
            ...
            self.logger.error(f"否定上下文分析失败: {str(e)}")
            return {}
