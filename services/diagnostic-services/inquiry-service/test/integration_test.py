    ...
    try:
        ...
        for msg in test_messages:
            ...
            interaction_request = pb2.InteractionRequest(
                session_id=session_id, user_message=msg, timestamp=0
            )
        ...
        extraction_request = pb2.SymptomsExtractionRequest(
            text_content=symptom_text, user_id="test_user_001", language="zh-CN"
        )
        ...
        for symptom in symptom_response.symptoms:
            print(
                f"  - {symptom.symptom_name} (严重程度: {symptom.severity}, 置信度: {symptom.confidence:.2f})"
            )
        ...
        if symptom_response.symptoms:
            ...
            for pattern in pattern_response.primary_patterns:
                print(
                    f"  - {pattern.pattern_name} ({pattern.category}, 匹配度: {pattern.match_score:.2f})"
                )
        ...
        health_profile = pb2.HealthProfile(
            user_id="test_user_001", constitution_type=pb2.HealthProfile.QI_DEFICIENCY
        )
        ...
        if risk_response.immediate_risks:
            ...
            for risk in risk_response.immediate_risks[:3]:
                print(
                    f"    - {risk.risk_name}: {risk.probability:.2f} ({risk.severity})"
                )
        ...
        end_request = pb2.EndSessionRequest(
            session_id=session_id, feedback="测试完成，服务运行正常"
        )
...
async def test_batch_analysis():
    ...
    try:
        ...
        for insight in batch_response.insights:
            print(
                f"    - {insight.insight_type}: {insight.description} (置信度: {insight.confidence:.2f})"
            )
...
async def test_medical_history_analysis():
    ...
    try:
        ...
        history_request = pb2.MedicalHistoryRequest(
            ...
            additional_info={"age": "52", "gender": "female", "occupation": "教师"},
        )
