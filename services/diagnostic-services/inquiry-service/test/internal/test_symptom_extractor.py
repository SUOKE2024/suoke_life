...
class TestSymptomExtractor:
    ...
    @pytest.mark.asyncio
    @patch("internal.llm.llm_client.LLMClient.generate")
    async def test_extract_symptoms_with_empty_text(self, mock_generate, symptom_extractor):
        """测试空文本情况"""
        # 模拟LLM响应
        mock_generate.return_value = json.dumps({"symptoms": [], "body_locations": [], "temporal_factors": [], "confidence_score": 0.0})
    ...
    @pytest.mark.asyncio
    @patch("internal.llm.llm_client.LLMClient.generate")
    async def test_extract_symptoms_with_llm_error(self, mock_generate, symptom_extractor):
        """测试LLM错误情况"""
        # 模拟LLM错误
