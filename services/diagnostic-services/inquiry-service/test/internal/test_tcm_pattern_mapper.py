...
from internal.model.dialogue_models import Symptom, SymptomSeverity, SymptomDuration
...
class TestPatternMapper(unittest.TestCase):
    ...
    def test_combine_pattern_scores(self):
        ...
        # 调用待测试方法
        combined_scores = self.pattern_mapper._combine_pattern_scores(rule_scores, symptom_scores)
    ...
    def test_map_symptoms_to_patterns(self):
        ...
        symptoms = [
            ...
            Symptom(
                name="食欲不振",
                description="没有胃口，不想吃东西",
                confidence=0.9
            ),
            Symptom(
                name="易怒",
                description="容易发脾气，情绪波动大",
                confidence=0.7
            )
        ]
