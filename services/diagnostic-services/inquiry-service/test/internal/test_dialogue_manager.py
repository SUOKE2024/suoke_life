...
class TestDialogueManager:
        ...
        async def get_session(session_id):
            ...
            return {
                ...
                "messages": [
                    ...
                    {"role": "assistant", "content": "您好，我是您的中医问诊助手，请问有什么可以帮助您的?"}
                ]
            }
        ...
        async def generate(prompt, system_prompt=None, history=None, temperature=0.7):
            if "问诊开始" in str(prompt):
                ...
                return json.dumps({
                    ...
                    "follow_up_questions": ["疼痛性质是怎样的？", "是否伴有其他症状？"]
                })
            else:
                return json.dumps({
    ...
    @pytest.fixture
    def dialogue_manager(self, config, mock_session_repo, mock_user_repo, mock_llm_client):
        """创建对话管理器实例"""
    ...
    @pytest.mark.asyncio
    async def test_start_session(self, dialogue_manager, mock_session_repo, mock_llm_client):
        """测试开始会话"""
        # 执行测试
    ...
    @pytest.mark.asyncio
    async def test_interact_with_valid_session(self, dialogue_manager, mock_session_repo, mock_llm_client):
        """测试有效会话的交互"""
        # 执行测试
    ...
    @pytest.mark.asyncio
    async def test_interact_with_invalid_session(self, dialogue_manager, mock_session_repo):
        """测试无效会话的交互"""
        # 执行测试
