...
# 测试规则匹配
@pytest.mark.asyncio
async def test_match_by_rules(test_config, test_symptoms, test_tongue_features, test_pulse_features):
    mapper = pattern_mapper.PatternMapper(test_config)
    
    # 私有方法测试，直接调用
    matched_patterns = mapper._match_by_rules(test_symptoms, test_tongue_features, test_pulse_features)
...
# 测试完整映射流程
@pytest.mark.asyncio
async def test_map_symptoms_to_patterns(test_config, test_symptoms, test_tongue_features, test_pulse_features):
    mapper = pattern_mapper.PatternMapper(test_config)
