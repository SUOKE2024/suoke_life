...
from services.diagnostic_services.inquiry_service.internal.enhanced_inquiry_service import (
    EnhancedInquiryService, InquirySession, InquiryResult, InquiryQuestion
)
...
# 全局服务实例
inquiry_service: Optional[EnhancedInquiryService] = None
            ...
            if data["type"] == "answer":
                ...
                if next_question:
                    ...
                    await manager.send_message(session_id, {
                        "type": "result",
                        "data": {
                            "syndrome": result.syndrome_analysis.get("primary", "未明确"),
...
async def get_metrics(
    ...
    try:
        ...
        # 会话指标
        metrics.append(f'# HELP inquiry_total_sessions Total number of inquiry sessions')
        metrics.append(f'# TYPE inquiry_total_sessions counter')
        metrics.append(f'inquiry_total_sessions {stats["total_sessions"]}')
        
        metrics.append(f'# HELP inquiry_completed_sessions Number of completed sessions')
        metrics.append(f'# TYPE inquiry_completed_sessions counter')
        metrics.append(f'inquiry_completed_sessions {stats["completed_sessions"]}')
        
        metrics.append(f'# HELP inquiry_active_sessions Number of active sessions')
        metrics.append(f'# TYPE inquiry_active_sessions gauge')
        metrics.append(f'inquiry_active_sessions {stats["active_sessions"]}')
        
        # 缓存指标
        metrics.append(f'# HELP inquiry_cache_hit_rate Cache hit rate')
        metrics.append(f'# TYPE inquiry_cache_hit_rate gauge')
        metrics.append(f'inquiry_cache_hit_rate {stats["cache_hit_rate"]}')
        
        # 性能指标
        metrics.append(f'# HELP inquiry_average_processing_time_ms Average processing time')
        metrics.append(f'# TYPE inquiry_average_processing_time_ms gauge')
        metrics.append(f'inquiry_average_processing_time_ms {stats["average_processing_time_ms"]}')
        
        metrics.append(f'# HELP inquiry_average_questions_per_session Average questions per session')
        metrics.append(f'# TYPE inquiry_average_questions_per_session gauge')
        metrics.append(f'inquiry_average_questions_per_session {stats["average_questions_per_session"]}')
