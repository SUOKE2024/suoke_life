...
from internal.observability.errors import handle_exceptions
...
def setup_logging(config: Dict):
    ...
    log_level = getattr(logging, log_level_env or log_config.get('level', 'INFO').upper())
...
def apply_env_to_config(config: Dict) -> Dict:
    ...
    if 'METRICS_ENABLED' in os.environ:
        config['metrics']['enabled'] = os.environ.get('METRICS_ENABLED', 'false').lower() in ['true', '1', 'yes']
    ...
    if 'MOCK_EXTERNAL_SERVICES' in os.environ:
        mock_services = os.environ.get('MOCK_EXTERNAL_SERVICES', 'false').lower() in ['true', '1', 'yes']
        if 'integration' in config:
            for service_name in ['xiaoai_service', 'med_knowledge', 'listen_service', 'look_service']:
    ...
    if 'USE_MOCK_MODE' in os.environ or 'MOCK_EXTERNAL_SERVICES' in os.environ:
        mock_enabled = os.environ.get('USE_MOCK_MODE', 'false').lower() in ['true', '1', 'yes'] or \
                       os.environ.get('MOCK_EXTERNAL_SERVICES', 'false').lower() in ['true', '1', 'yes']
...
async def run_server():
    ...
    try:
        ...
        if metrics_config.get('enabled', False):
            ...
            logger.info(f"度量收集器已启动: http://{metrics_host}:{metrics_port}{metrics_path}")
        
        # 初始化gRPC服务器
        server = grpc.aio.server(
            futures.ThreadPoolExecutor(max_workers=max_workers),
            options=[
                ('grpc.max_receive_message_length', config.get('grpc', {}).get('max_message_size', 10485760)),
                ('grpc.max_send_message_length', config.get('grpc', {}).get('max_message_size', 10485760)),
                ('grpc.keepalive_time_ms', config.get('grpc', {}).get('keep_alive_time', 60000)),
                ('grpc.keepalive_timeout_ms', config.get('grpc', {}).get('keep_alive_timeout', 20000)),
        ...
        if server_config.get('enable_reflection', True):
            try:
                ...
                try:
                    ...
                    logger.warning(f"服务反射导入错误: {str(e)}")
                    SERVICE_NAMES = (reflection.SERVICE_NAME,)
                
                reflection.enable_server_reflection(SERVICE_NAMES, server)
                logger.info("已启用服务器反射")
            except Exception as e:
                logger.warning(f"启用服务器反射失败: {str(e)}，继续启动服务")
...
def serve():
    ...
    if sys.version_info >= (3, 11) and hasattr(asyncio, 'WindowsSelectorEventLoopPolicy'):
