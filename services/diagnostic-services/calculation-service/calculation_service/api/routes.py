...
class CalculationRequest(BaseModel):
    ...
    analysis_date: str | None = Field(None, description="分析日期，格式：YYYY-MM-DD")
...
async def comprehensive_calculation(request: CalculationRequest):
    ...
    try:
        ...
        return {"success": True, "data": result, "message": "综合算诊分析完成"}
...
async def ziwu_liuzhu_analysis(
    current_time: str | None = Query(
        None, description="当前时间，格式：YYYY-MM-DD HH:MM"
    ),
):
    """
    子午流注分析
    ...
    try:
        ...
        return {"success": True, "data": result, "message": "子午流注分析完成"}
...
async def constitution_analysis(personal_info: PersonalInfo):
    ...
    try:
        ...
        return {"success": True, "data": result, "message": "八字体质分析完成"}
...
async def bagua_analysis(personal_info: PersonalInfo):
    ...
    try:
        ...
        return {"success": True, "data": result, "message": "八卦配属分析完成"}
...
async def current_wuyun_liuqi_analysis(
    analysis_date: str | None = Query(None, description="分析日期，格式：YYYY-MM-DD"),
):
    """
    当前运气分析
    ...
    try:
        ...
        return {"success": True, "data": result, "message": "当前运气分析完成"}
...
async def yearly_wuyun_liuqi_prediction(year: int):
    ...
    try:
        ...
        return {"success": True, "data": result, "message": f"{year}年运气预测完成"}
...
async def personalized_health_advice(
    ...
    analysis_date: str | None = Query(None, description="分析日期"),
):
    """
    个性化健康建议
    ...
    try:
        ...
        # 执行综合分析
        result = comprehensive_calc.comprehensive_analysis(
            birth_info=birth_info, analysis_date=dt
        )
