...
class Settings(BaseSettings):
    ...
    MONGODB_URL: str = Field(
        default="mongodb://localhost:27017/calculation_db", description="MongoDB连接URL"
    )

    # 安全配置
    SECRET_KEY: str = Field(
        default="suoke-calculation-service-secret-key-2024", description="密钥"
    )
    ...
    ASTRONOMICAL_DATA_PATH: str = Field(
        default="./data/astronomical", description="天文数据路径"
    )
    ...
    USER_SERVICE_URL: str = Field(
        default="http://user-service:8001", description="用户服务URL"
    )
    HEALTH_DATA_SERVICE_URL: str = Field(
        default="http://health-data-service:8002", description="健康数据服务URL"
    )
