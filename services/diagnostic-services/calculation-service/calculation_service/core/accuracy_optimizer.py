...
class AccuracyOptimizer:
    ...
    def optimize_diagnosis(
        ...
        feature_importance = self._analyze_feature_importance(
            normalized_features, fused_results
        )

        # 5. 生成优化建议
        recommendations = self._generate_recommendations(
            fused_results, feature_importance
        )
    ...
    def _fuse_algorithm_results(
        ...
        primary_constitution = (
            sorted_constitutions[0][0] if sorted_constitutions else "平和质"
        )
    ...
    def _generate_recommendations(
        ...
        recommendations.extend(
            constitution_recommendations.get(primary_constitution, [])
        )
    ...
    def _calculate_accuracy_score(
        ...
        feature_coverage = len(
            [f for f in feature_importance.values() if f > 0.05]
