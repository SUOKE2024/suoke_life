...
def validate_date_range(
    input_date: date, min_year: int = 1900, max_year: int = 2100
) -> None:
    """
    验证日期范围
    ...
    if input_date.year < min_year or input_date.year > max_year:
        raise HTTPException(
            status_code=400, detail=f"日期年份必须在{min_year} - {max_year}之间"
        )
...
def validate_patient_info(patient_info: dict[str, Any]) -> None:
    ...
    for field in required_fields:
        if field not in patient_info:
            raise HTTPException(
                status_code=400, detail=f"患者信息缺少必需字段: {field}"
            )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail="出生日期格式无效，请使用YYYY - MM - DD格式"
        )
    ...
    if "birth_time" in patient_info and patient_info["birth_time"]:
        ...
        if not validate_time_format(birth_time):
            raise HTTPException(
                status_code=400, detail="出生时间格式无效，请使用HH:MM格式"
            )
    ...
    if "birth_location" in patient_info and patient_info["birth_location"]:
        ...
        if not validate_location_format(location):
            raise HTTPException(status_code=400, detail="出生地点信息格式无效")
...
def validate_symptoms(symptoms: list) -> None:
    ...
    if not isinstance(symptoms, list):
        raise HTTPException(status_code=400, detail="症状必须是列表格式")

    if len(symptoms) == 0:
        raise HTTPException(status_code=400, detail="症状列表不能为空")

    if len(symptoms) > 10:
        raise HTTPException(status_code=400, detail="症状数量不能超过10个")

    for symptom in symptoms:
        if not isinstance(symptom, str) or len(symptom.strip()) == 0:
            raise HTTPException(status_code=400, detail="每个症状必须是非空字符串")

        if len(symptom) > 50:
            raise HTTPException(status_code=400, detail="单个症状描述不能超过50个字符")
...
def validate_treatment_type(treatment_type: str) -> None:
    ...
    if treatment_type not in valid_types:
        raise HTTPException(
            status_code=400, detail=f"治疗类型必须是以下之一: {', '.join(valid_types)}"
        )
...
def validate_condition(condition: str) -> None:
    ...
    if not isinstance(condition, str) or len(condition.strip()) == 0:
        raise HTTPException(status_code=400, detail="病症名称不能为空")

    if len(condition) > 100:
        raise HTTPException(status_code=400, detail="病症名称不能超过100个字符")

    # 检查是否包含特殊字符
    invalid_chars = ["<", ">", "&", '"', "'", "\\", " / ", ";"]
    if any(char in condition for char in invalid_chars):
        raise HTTPException(status_code=400, detail="病症名称不能包含特殊字符")
