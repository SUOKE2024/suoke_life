...
class BaziCalculator:
    ...
    def calculate_bazi(self, birth_date: date, birth_time: Optional[str] = None) -> Dict:
        """
        计算生辰八字
        ...
        # 计算时柱
        hour_ganzhi = self._get_hour_ganzhi(birth_time, day_ganzhi) if birth_time else None
        
        # 计算五行
        wuxing = self._calculate_wuxing(year_ganzhi, month_ganzhi, day_ganzhi, hour_ganzhi)
    ...
    def _calculate_wuxing(self, year_ganzhi: str, month_ganzhi: str, 
                         day_ganzhi: str, hour_ganzhi: Optional[str]) -> Dict:
        """计算五行分布"""
    ...
    def _analyze_wuxing_balance(self, wuxing: Dict) -> Dict:
        ...
        # 计算各元素比例
        proportions = {
            element: count / total for element, count in wuxing.items()
