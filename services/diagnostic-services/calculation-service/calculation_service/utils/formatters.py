...
def format_analysis_result(result: dict[str, Any]) -> dict[str, Any]:
    ...
    if "analysis_metadata" in result and "individual_analyses" in result:
        ...
        if "ziwu" in individual_analyses and "error" not in individual_analyses["ziwu"]:
            formatted_result["详细分析"]["子午流注"] = _format_ziwu_result(
                individual_analyses["ziwu"]
            )

        if (
            "constitution" in individual_analyses
            and "error" not in individual_analyses["constitution"]
        ):
            formatted_result["详细分析"]["体质分析"] = _format_constitution_result(
                individual_analyses["constitution"]
            )

        if (
            "bagua" in individual_analyses
            and "error" not in individual_analyses["bagua"]
        ):
            formatted_result["详细分析"]["八卦分析"] = _format_bagua_result(
                individual_analyses["bagua"]
            )

        if (
            "wuyun_liuqi" in individual_analyses
            and "error" not in individual_analyses["wuyun_liuqi"]
        ):
            formatted_result["详细分析"]["运气分析"] = _format_wuyun_result(
                individual_analyses["wuyun_liuqi"]
            )
