...
class ZiwuCalculator:
    ...
    def get_current_shichen(self, current_time: datetime | None = None) -> dict:
        """
        获取当前时辰信息
    ...
    def get_optimal_treatment_time(self, target_organ: str) -> dict:
        ...
        # 添加详细说明
        treatment_info["说明"] = (
            f"{target_organ}的最佳治疗时间是{treatment_info['最佳']}，此时{target_organ}经气最旺盛"
        )

        return treatment_info

    def analyze_daily_schedule(self, target_date: datetime | None = None) -> list[dict]:
        """
        分析一天的子午流注时间表
    ...
    def calculate_optimal_time(self, target_date) -> dict:
        ...
        for shichen, info in self.shichen_zangfu.items():
            meridian_flow.append(
                {"time": shichen, "organ": info["脏腑"], "element": info["五行"]}
            )
