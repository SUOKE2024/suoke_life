...
class ConstitutionAnalysis:
    ...
    secondary_constitution: ConstitutionType | None
...
class MeridianFlowCalculator:
    ...
    def calculate_current_flow(self, current_time: datetime = None) -> MeridianFlow:
        ...
        for time_name, (
            ...
            organ,
            element,
        ) in self.meridian_schedule.items():
...
class ConstitutionCalculator:
    ...
    def calculate_constitution(self, birth_info: BirthInfo) -> ConstitutionAnalysis:
        ...
        month_stem, month_branch = self._get_month_stem_branch(
            birth_info.year, birth_info.month
        )
        day_stem, day_branch = self._get_day_stem_branch(
        ...
        # 计算体质得分
        constitution_scores = self._calculate_constitution_scores(
            element_scores, birth_info
        )
        ...
        sorted_constitutions = sorted(
            constitution_scores.items(), key=lambda x: x[1], reverse=True
        )
        secondary_constitution = (
            sorted_constitutions[1][0]
            if len(sorted_constitutions) > 1 and sorted_constitutions[1][1] > 0.3
            else None
        )

        # 获取特征描述
        characteristics = self.constitution_characteristics.get(
    ...
    def _analyze_five_elements(
        self, stems: list[str], branches: list[str]
    ) -> dict[Element, float]:
        """分析五行强弱"""
        # 天干五行
        stem_elements = {
    ...
    def _calculate_constitution_scores(
        self, element_scores: dict[Element, float], birth_info: BirthInfo
    ...
    def _generate_lifestyle_recommendations(
        self, constitution: ConstitutionType
    ) -> list[str]:
        """生成生活方式建议"""
        recommendations = {
            ConstitutionType.QI_DEFICIENCY: [
    ...
    def _calculate_constitution_confidence(
        self, scores: dict[ConstitutionType, float]
    ) -> float:
        """计算体质分析置信度"""
        if not scores:
            return 0.5
...
class EnhancedCalculationService:
    ...
    async def comprehensive_calculation(
        self, birth_info: BirthInfo, current_time: datetime = None
        ...
        try:
            ...
            # 子午流注分析
            meridian_flow = self.meridian_calculator.calculate_current_flow(
                current_time
            )

            # 体质分析
            constitution_analysis = self.constitution_calculator.calculate_constitution(
                birth_info
            )

            # 五运六气分析（简化实现）
            five_elements_analysis = self._analyze_five_elements_simplified(
                birth_info, current_time
            )
            ...
            logger.info(
                f"综合算诊完成，主要体质: {constitution_analysis.primary_constitution}"
            )
            return result
    ...
    def _analyze_five_elements_simplified(
        self, birth_info: BirthInfo, current_time: datetime
        ...
        seasonal_influence = (
            f"当前{seasonal_element.value}旺季，宜调养{seasonal_element.value}脏"
        )
    ...
    def _generate_overall_assessment(
        ...
        assessment_parts.append(
            f"您的主要体质为{constitution.primary_constitution.value}"
        )

        if constitution.secondary_constitution:
            assessment_parts.append(
                f"兼有{constitution.secondary_constitution.value}倾向"
            )
        ...
        # 当前时辰评估
        assessment_parts.append(
            f"当前{meridian_flow.time_period}，{meridian_flow.primary_organ.value}经当令"
        )
