...
class TestCalculationService:
    ...
    @pytest.fixture
    def sample_birth_info(self) -> dict[str, Any]:
        """示例出生信息"""
        return {
            "year": 1990,
            "month": 5,
    ...
    @pytest.fixture
    def comprehensive_calculator(self) -> ComprehensiveCalculator:
        """综合算诊计算器"""
    ...
    def test_birth_info_validation(self, sample_birth_info):
        """测试出生信息验证"""
        # 正常情况
        assert validate_birth_info(sample_birth_info)
    ...
    def test_bagua_analysis(self, sample_birth_info):
        ...
        benming_gua = calculator.calculate_benming_gua(
            sample_birth_info["year"], sample_birth_info["gender"]
        )
    ...
    async def test_comprehensive_analysis(
        self, sample_birth_info, comprehensive_calculator
    ):
        """测试综合算诊分析"""
        analysis_options = {
            "include_ziwu": True,
    ...
    def test_api_comprehensive_analysis(self, client, sample_birth_info):
        ...
        response = client.post(
            " / api / v1 / calculation / comprehensive", json=request_data
        )
        assert response.status_code == 200
    ...
    def test_api_ziwu_analysis(self, client):
        """测试API子午流注分析接口"""
        request_data = {"analysis_time": datetime.now().strftime("%Y - %m - %d %H:%M")}
    ...
    def test_api_constitution_analysis(self, client, sample_birth_info):
        """测试API体质分析接口"""
        response = client.post(
            " / api / v1 / calculation / constitution", json=sample_birth_info
        )
        assert response.status_code == 200
    ...
    def test_api_bagua_analysis(self, client, sample_birth_info):
        """测试API八卦分析接口"""
        response = client.post(
            " / api / v1 / calculation / bagua", json=sample_birth_info
        )
        assert response.status_code == 200
    ...
    def test_error_handling(self, client):
        ...
        response = client.post(
            " / api / v1 / calculation / constitution", json=invalid_birth_info
        )
