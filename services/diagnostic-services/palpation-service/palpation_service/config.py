...
class PredictionConfig(BaseSettings):
    ...
    enabled: bool = Field(default=True)
    model_path: str = Field(default="models/prediction")
    confidence_threshold: float = Field(default=0.7)
    
    model_config = {"env_prefix": "PREDICTION_"}


class ReportConfig(BaseSettings):
    """报告配置"""
    
    template_path: str = Field(default="templates/reports")
...
class CoordinatorConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "COORDINATOR_"}
...
class VisualizationConfig(BaseSettings):
    ...
    model_config = {"env_prefix": "VISUALIZATION_"}
