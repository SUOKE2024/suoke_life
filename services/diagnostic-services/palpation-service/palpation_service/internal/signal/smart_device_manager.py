...
class DeviceHealth:
    ...
    temperature: float | None = None  # 摄氏度
...
class SmartDeviceManager:
    ...
    async def register_device(str):
        ...
        try:
            ...
            self.device_health[device_id] = DeviceHealth(
                status = DeviceStatus.DISCONNECTED, last_heartbeat = datetime.now()
            )
    ...
    async def _disconnect_device(bool):
        """内部断开设备方法"""
        if device_id not in self.devices:
            return False
    ...
    async def calibrate_device(SensorCalibrationData | None):
        """
        校准设备
    ...
    async def start_session(
        self, session_id: str, device_preference: str | None = None
    ) - > str | None:
        """
        开始数据采集会话
        ...
        async with self.device_locks[device_id]:
            try:
                # 检查设备状态
                if self.device_health[device_id].status not in [
                    DeviceStatus.READY,
                    DeviceStatus.CONNECTED,
                ]:
                    logger.error(
                        f"设备状态不可用: {device_id}, {self.device_health[device_id].status}"
                    )
                    return None
    ...
    async def get_device_status(dict[str, Any] | None):
        """
        获取设备状态
        ...
        return {
            ...
            "last_calibration": (
                metrics.last_calibration.isoformat() if metrics.last_calibration else None
            ),
        }

    async def get_all_devices_status(dict[str, dict[str, Any]]):
        """获取所有设备状态"""
        status = {}
    ...
    async def _select_best_device(str | None):
        """选择最佳设备"""
        available_devices = []
    ...
    async def _auto_discovery_task(None):
        """自动发现设备任务"""
        while self.running:
            try:
                if self.auto_discovery:
    ...
    async def _session_processor(None):
        """会话处理器"""
        while self.running:
            try:
                # 处理会话队列中的任务
            ...
            if metrics.total_sessions > 0:
                ...
                if success_rate < (1 - self.max_error_rate):
                    await self._trigger_event(
                        "device_error", device_id, f"成功率过低: {success_rate:.2%}"
                    )
