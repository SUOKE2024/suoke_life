...
class AbdominalAnalyzer:
    ...
    def analyze_abdominal_regions(
        self, regions_data: list[AbdominalRegionData]
    ) -> list[AbdominalFinding]:
        """
        分析腹部各区域数据
    ...
    def _analyze_tenderness(self, region_data: AbdominalRegionData) -> AbdominalFinding | None:
        """分析压痛"""
        severity_map = {
        ...
        potential_causes = self._infer_tenderness_causes(
            region_data.region_type, region_data.tenderness_level
        )
    ...
    def _analyze_tension(self, region_data: AbdominalRegionData) -> AbdominalFinding | None:
        """分析腹部紧张度"""
        severity_map = {TensionLevel.RELAXED: 0.2, TensionLevel.TENSE: 0.6, TensionLevel.RIGID: 0.9}
        ...
        tcm_interpretation = self._get_tcm_interpretation_for_tension(
            region_data.region_type, region_data.tension_level
        )
    ...
    def _analyze_mass(self, region_data: AbdominalRegionData) -> AbdominalFinding | None:
        """分析腹部肿块"""
        # 根据区域推断可能的肿块性质
    ...
    def _analyze_pulsation(self, region_data: AbdominalRegionData) -> AbdominalFinding | None:
        """分析腹部搏动"""
        # 不同区域的搏动有不同意义
    ...
    def _analyze_gurgling(self, region_data: AbdominalRegionData) -> AbdominalFinding | None:
        """分析肠鸣音"""
    ...
    def _analyze_region_correlations(
        self, regions_data: list[AbdominalRegionData]
    ) -> list[AbdominalFinding]:
        """分析各区域之间的关联"""
        findings = []
    ...
    def _check_asymmetry(
        self, left_regions: list[AbdominalRegionData], right_regions: list[AbdominalRegionData]
    ) -> bool:
        """检查左右是否不对称"""
        ...
        right_tenderness = sum(
            1 for r in right_regions if r.tenderness_level != TendernessLevel.NONE
        )
        ...
        return (
            abs(left_tenderness - right_tenderness) >= 2 or abs(left_tension - right_tension) >= 2
        )

    def _infer_tenderness_causes(
        self, region: AbdominalRegion, level: TendernessLevel
    ) -> list[str]:
        """根据区域和压痛程度推断可能原因"""
        causes_map = {
    ...
    def _get_tcm_interpretation_for_tenderness(
        self, region: AbdominalRegion, level: TendernessLevel
    ) -> str:
        """获取压痛的中医解释"""
    ...
    def _get_tcm_interpretation_for_tension(
        self, region: AbdominalRegion, level: TensionLevel
    ) -> str:
        """获取腹部紧张度的中医解释"""
    ...
    def generate_summary(self, findings: list[AbdominalFinding]) -> str:
        ...
        summary_parts.append(
            f"腹诊检查发现最显著的问题是{most_severe.region_name}{most_severe.finding_type}"
        )
