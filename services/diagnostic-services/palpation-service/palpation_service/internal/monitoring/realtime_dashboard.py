...
class MetricsCollector:
    ...
    def collect_all_metrics(list[MetricData]):
        """收集所有指标"""
        all_metrics = []
    ...
    def get_metric(MetricData | None):
        """获取指定指标"""
        if labels is None:
            labels = {}
...
class RealtimeDashboard:
    ...
    def _collect_system_metrics(list[MetricData]):
        """收集系统指标"""
        metrics = []
        current_time = datetime.now()
    ...
    def _collect_application_metrics(list[MetricData]):
        """收集应用指标"""
        metrics = []
        current_time = datetime.now()
    ...
    def _register_default_alert_rules(None):
        """注册默认告警规则"""
        # CPU使用率告警
    ...
    def _register_alert_handlers(None):
        """注册告警处理器"""
