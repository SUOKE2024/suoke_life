...
class TCMPatternMapper:
    ...
    def _init_pattern_database(dict[str, dict[str, Any]]):
        """初始化证型数据库"""
        return {
            # 表证
            "wind_cold_exterior": {
    ...
    def _init_symptom_mapping(dict[str, list[str]]):
        """初始化症状 - 证型映射"""
        return {
            # 脉象映射
    ...
    def _init_five_elements(dict[str, dict[str, Any]]):
        """初始化五行关系"""
        return {
            "木": {
                "organ": "肝胆",
    ...
    def _apply_five_elements_theory(list[TCMPattern]):
        """应用五行理论调整证型"""
        # 如果只有一个证型，不需要调整
        if len(patterns) < = 1:
            return patterns
    ...
    def get_treatment_principles(list[str]):
        """
        根据证型获取治疗原则
