...
class IntelligentCoordinator:
    ...
    async def _submit_task(dict[str, Any]):
        """提交分析任务"""
        # 检查系统负载
    ...
    async def _task_processor(None):
        """任务处理器"""
        while self.running:
            try:
                # 获取任务
    ...
    async def _process_pulse_task(dict[str, Any]):
        """处理脉搏分析任务"""
    ...
    def _analyze_pulse_data(dict[str, Any]):
        """分析脉搏数据（在线程池中执行）"""
        # 提取特征
    ...
    async def _process_comprehensive_task(dict[str, Any]):
        """处理综合分析任务"""
    ...
    async def _check_resource_availability(bool):
        """检查资源可用性"""
        # 检查CPU使用率
    ...
    async def _check_system_health(None):
        """检查系统健康状态"""
        # 检查子系统状态
        if self.device_manager:
    ...
    async def _complete_active_tasks(None):
        """完成所有活跃任务"""
        timeout = 30  # 30秒超时
        start_time = time.time()
    ...
    async def _process_abdominal_task(dict[str, Any]):
        """处理腹诊分析任务"""
    ...
    async def _process_skin_task(dict[str, Any]):
        """处理皮肤触诊分析任务"""
    ...
    async def _performance_optimizer(None):
        """性能优化器"""
        while self.running:
            try:
    ...
    async def _cleanup_manager(None):
        """清理管理器"""
        while self.running:
            try:
    ...
    async def stop_palpation_session(dict[str, Any]):
        """
        停止触诊会话
