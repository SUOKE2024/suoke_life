...
class IntelligentReportGenerator:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.age_groups = self.personalization.get(
            "age_groups", {"young": (0, 30), "middle": (30, 60), "senior": (60, 120)}
        )
    ...
    def _load_chinese_templates(dict[str, str]):
        """加载中文模板"""
        return {
    ...
    def _load_english_templates(dict[str, str]):
        """加载英文模板"""
        return {
    ...
    async def _generate_report_sections(
        ...
        # 1. 基本信息章节
        basic_section = await self._generate_basic_info_section(user_profile, language)
        sections.append(basic_section)

        # 2. 脉象分析章节
        if "comprehensive_features" in analysis_results:
            pulse_section = await self._generate_pulse_analysis_section(analysis_results, language)
        ...
        if report_type in [ReportType.DETAILED, ReportType.PROFESSIONAL]:
            trend_section = await self._generate_trend_analysis_section(analysis_results, language)
            if trend_section:
                sections.append(trend_section)

        # 6. 预测分析章节（专业报告）
        if report_type == ReportType.PROFESSIONAL and "prediction" in analysis_results:
            prediction_section = await self._generate_prediction_section(analysis_results, language)
            sections.append(prediction_section)

        return sections

    async def _generate_basic_info_section(
        self, user_profile: dict[str, Any], language: Language
    ) - > ReportSection:
        """生成基本信息章节"""
        ...
        return ReportSection(title = title, content = content.strip(), importance = 1)

    async def _generate_pulse_analysis_section(
        self, analysis_results: dict[str, Any], language: Language
    ) - > ReportSection:
        """生成脉象分析章节"""
        ...
        return ReportSection(title = title, content = content, charts = charts, importance = 5)

    async def _generate_pulse_analysis_text_zh(str):
        """生成中文脉象分析文本"""
    ...
    async def _generate_pulse_analysis_text_en(str):
        """生成英文脉象分析文本"""
        heart_rate = features.get("heart_rate", 70)
        hrv = features.get("heart_rate_variability", 0.5)
        rhythm = features.get("rhythm_regularity", 0.8)
        ...
        if hrv > 0.4:
            ...
            content + = (
                "Heart rate variability is low, recommend attention to cardiovascular health."
            )
    ...
    async def _generate_constitution_section(
        ...
        return ReportSection(title = title, content = content, charts = charts, importance = 4)

    async def _generate_constitution_text_zh(str):
        """生成中文体质分析文本"""
    ...
    async def _generate_constitution_text_en(str):
        """生成英文体质分析文本"""
        constitution_descriptions = {
    ...
    async def _generate_health_assessment_text_zh(str):
        """生成中文健康评估文本"""
        overall_score = health_assessment.get("overall_score", 0.7)
    ...
    async def _generate_health_assessment_text_en(str):
        """生成英文健康评估文本"""
        overall_score = health_assessment.get("overall_score", 0.7)
        risk_factors = health_assessment.get("risk_factors", [])
    ...
    async def _generate_trend_analysis_section(
        ...
        return ReportSection(title = title, content = content, importance = 3)
    ...
    async def _generate_prediction_section(
        ...
        return ReportSection(title = title, content = content, importance = 4)

    async def _create_pulse_features_chart(
        self, features: dict[str, Any], language: Language
    ) - > str | None:
        """创建脉象特征图表"""
        try:
            # 提取关键特征
            feature_names = (
                ["心率", "心率变异性", "节律规整度", "脉搏强度"]
                if language == Language.ZH_CN
                else ["Heart Rate", "HRV", "Rhythm", "Pulse Strength"]
            )
            ...
            ax.plot(
                ...
                "o - ",
                linewidth = 2,
                label = "当前值" if language == Language.ZH_CN else "Current",
            )
    ...
    async def _create_constitution_chart(
        self, constitution_type: str, language: Language
    ) - > str | None:
        """创建体质分析图表"""
        try:
            # 体质类型和对应的特征值（模拟数据）
            ...
            feature_names = (
                ["气血", "阴阳", "脏腑", "经络", "体质"]
                if language == Language.ZH_CN
                else ["Qi - Blood", "Yin - Yang", "Organs", "Meridians", "Constitution"]
            )
            ...
            # 设置标题和标签
            title = (
                f"{constitution_type}体质特征"
                if language == Language.ZH_CN
                else f"{constitution_type} Constitution Features"
            )
            ...
            # 添加数值标签
            for bar, value in zip(bars, features, strict = False):
    ...
    async def _create_health_assessment_chart(
        self, health_assessment: dict[str, Any], language: Language
    ) - > str | None:
        """创建健康评估图表"""
        try:
            overall_score = health_assessment.get("overall_score", 0.7)
    ...
    async def _generate_summary(
        ...
        if age < 30:
            age_advice = (
                "年轻人应注重预防保健。"
                if language == Language.ZH_CN
                else "Young people should focus on preventive care."
            )
        elif age < 60:
            age_advice = (
                "中年人应定期体检。"
                if language == Language.ZH_CN
                else "Middle - aged people should have regular check - ups."
            )
        else:
            age_advice = (
                "老年人应加强健康监测。"
                if language == Language.ZH_CN
                else "Elderly people should strengthen health monitoring."
            )

        return f"{base_summary} {age_advice}"

    async def _generate_recommendations(
        self, analysis_results: dict[str, Any], user_profile: dict[str, Any], language: Language
    ) - > list[str]:
        """生成个性化建议"""
        recommendations = []
        templates = self.language_templates[language]
    ...
    def _get_constitution_advice(list[str]):
        """获取体质相关建议"""
        if language == Language.ZH_CN:
            advice_map = {
                "qi_deficiency": [
            ...
            advice_map = {
                "qi_deficiency": [
                    "Eat qi - tonifying foods like yam and red dates",
                ...
                "yin_deficiency": [
                    "Eat yin - nourishing foods like white fungus and lily",
                    "Avoid spicy foods",
                    "Maintain emotional balance",
                ],
                "phlegm_dampness": [
                    "Light diet, less greasy food",
                    "Moderate exercise to promote metabolism",
                    "Keep environment dry",
                ],
                "damp_heat": [
                    "Heat - clearing and dampness - draining diet",
                    "Avoid spicy and greasy foods",
                    "Moderate exercise to sweat",
                ],
            }

        return advice_map.get(constitution_type, [])

    def _get_age_specific_advice(list[str]):
        """获取年龄相关建议"""
        if language == Language.ZH_CN:
            if age < 30:
    ...
    async def _calculate_follow_up_date(
        self, overall_score: float, risk_level: str
    ) - > datetime | None:
        """计算随访日期"""
        now = datetime.now()
    ...
    async def export_report(
        self, report: HealthReport, format_type: ReportFormat = ReportFormat.HTML
    ) - > str:
        """
        导出报告
    ...
    async def _export_html_report(str):
        """导出HTML格式报告"""
        # 生成HTML内容
    ...
    async def _generate_html_content(str):
        """生成HTML内容"""
        # 简单的HTML模板
        html_template = """
        <!DOCTYPE html>
    ...
    async def _export_json_report(str):
        """导出JSON格式报告"""
        # 转换为字典
        report_dict = {
    ...
    async def _export_markdown_report(str):
        """导出Markdown格式报告"""
        md_content = f"""# 健康报告
