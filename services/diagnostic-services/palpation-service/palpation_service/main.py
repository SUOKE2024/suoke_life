...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler("logs/palpation_service.log")],
)
...
class PalpationService:
    ...
    async def initialize(self):
        ...
        try:
            ...
            # 加载配置文件
            config_files = ["config/palpation.yaml", "config/devices.yaml", "config/ai.yaml"]
            ...
            self.report_generator = IntelligentReportGenerator(report_config, self.visualizer)
        ...
        async def health_check():
            ...
            try:
                ...
                status = {
                    ...
                    "fusion_engine": (
                        self.fusion_engine.get_status() if self.fusion_engine else "stopped"
                    ),
                    "cache_manager": (
                        self.cache_manager.get_status() if self.cache_manager else "stopped"
                    ),
        ...
        async def analyze_palpation(request: AnalysisRequest):
            ...
            try:
                ...
                analysis_result = await self.coordinator.analyze_comprehensive_palpation(
                    session_id=request.session_id,
                    data_list=[
                        {"modality": item.modality, "data": item.data, "timestamp": item.timestamp}
...
async def main():
    """主函数"""
    # 创建必要的目录
    directories = ["logs", "data", "config", "models", "cache", "reports", "monitoring"]
