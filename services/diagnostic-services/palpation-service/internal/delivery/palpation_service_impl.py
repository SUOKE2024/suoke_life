...
class PalpationServiceImpl(pb2_grpc.PalpationServiceServicer):
    ...
    def StartPulseSession(
        self, 
        request: pb2.StartPulseSessionRequest, 
        ...
        try:
            ...
            sampling_config_response = pb2.SamplingConfig(
                ...
                required_positions=["CUN_LEFT", "GUAN_LEFT", "CHI_LEFT", 
                                    "CUN_RIGHT", "GUAN_RIGHT", "CHI_RIGHT"],
                minimum_pressure=device_config.get('minimum_pressure', 0),
                maximum_pressure=device_config.get('maximum_pressure', 100)
            )
            
            # 记录指标
            self.metrics.record_counter('pulse_sessions_started', 1, {'device_model': request.device_info.model})
            ...
            logger.exception(f"开始脉诊会话失败: {str(e)}")
            self.metrics.record_counter('pulse_sessions_errors', 1, {'error_type': 'start_session'})
            
            return pb2.StartPulseSessionResponse(
                session_id="",
                success=False,
                error_message=f"Failed to start pulse session: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def RecordPulseData(
        self, 
        request_iterator: Iterator[pb2.PulseDataPacket], 
        context: grpc.ServicerContext
        ...
        try:
            ...
            for packet in request_iterator:
                ...
                if packet.session_id != session_id:
                    logger.warning(f"数据包会话ID不匹配: 预期 {session_id}, 实际 {packet.session_id}")
                    continue
            ...
            logger.exception(f"记录脉诊数据失败: {str(e)}")
            ...
            return pb2.RecordPulseDataResponse(
                ...
                packets_received=packets_received,
                success=False,
                error_message=f"Failed to record pulse data: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def ExtractPulseFeatures(
        self, 
        request: pb2.ExtractPulseFeaturesRequest, 
        context: grpc.ServicerContext
        ...
        try:
            ...
            logger.exception(f"提取脉象特征失败: {str(e)}")
            self.metrics.record_counter('pulse_sessions_errors', 1, {'error_type': 'extract_features'})
            
            return pb2.ExtractPulseFeaturesResponse(
                session_id=request.session_id,
                success=False,
                error_message=f"Failed to extract pulse features: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def AnalyzePulse(
        self, 
        request: pb2.AnalyzePulseRequest, 
        context: grpc.ServicerContext
        ...
        try:
            ...
            analysis_options = {
                'use_tcm_model': request.options.use_tcm_model if hasattr(request, 'options') and request.options else True,
                'use_western_model': request.options.use_western_model if hasattr(request, 'options') and request.options else False,
                'analysis_depth': request.options.analysis_depth if hasattr(request, 'options') and request.options else 'standard',
                'specific_conditions': list(request.options.specific_conditions) if hasattr(request, 'options') and request.options else []
            }
            ...
            logger.exception(f"分析脉象失败: {str(e)}")
            self.metrics.record_counter('pulse_sessions_errors', 1, {'error_type': 'analyze_pulse'})
            
            return pb2.AnalyzePulseResponse(
                session_id=request.session_id,
                success=False,
                error_message=f"Failed to analyze pulse: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def AnalyzeAbdominalPalpation(
        ...
        try:
            ...
            logger.exception(f"分析腹诊数据失败: {str(e)}")
            self.metrics.record_counter('abdominal_analysis_errors', 1)
            
            return pb2.AbdominalPalpationResponse(
                success=False,
                error_message=f"Failed to analyze abdominal palpation data: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def AnalyzeSkinPalpation(
        ...
        try:
            ...
            logger.exception(f"分析皮肤触诊数据失败: {str(e)}")
            self.metrics.record_counter('skin_analysis_errors', 1)
            
            return pb2.SkinPalpationResponse(
                success=False,
                error_message=f"Failed to analyze skin palpation data: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def HealthCheck(self, request, context):
        ...
        if request.level >= pb2.HealthCheckRequest.HealthCheckLevel.BASIC:
            ...
            try:
                ...
                db_component.details = f"数据库连接失败: {str(e)}"
                db_component.response_time_ms = int((time.time() - db_check_start) * 1000)
        ...
        if request.level == pb2.HealthCheckRequest.HealthCheckLevel.FULL:
            ...
            for integration_name in integration_names:
                ...
                try:
                    ...
                    if integration_status:
                        ...
                        integration_component.status = pb2.HealthCheckResponse.ServiceStatus.SERVICE_UNKNOWN
                        integration_component.details = f"{integration_name} 服务状态未知"
                        
                    integration_component.response_time_ms = int((time.time() - integration_check_start) * 1000)
                except Exception as e:
                    logger.warning(f"{integration_name} 健康检查失败: {e}")
                    integration_component = response.components.add()
                    integration_component.component_name = integration_name
                    integration_component.status = pb2.HealthCheckResponse.ServiceStatus.NOT_SERVING
                    integration_component.details = f"{integration_name} 连接失败: {str(e)}"
                    integration_component.response_time_ms = int((time.time() - integration_check_start) * 1000)
            ...
            try:
                ...
                model_component.details = f"模型加载失败: {str(e)}"
                model_component.response_time_ms = int((time.time() - model_check_start) * 1000)
