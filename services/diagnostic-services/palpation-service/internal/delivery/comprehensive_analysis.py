...
class ComprehensiveAnalysisHandler:
    ...
    def handle_comprehensive_analysis(
        ...
        try:
            ...
            # 映射到中医证型
            tcm_patterns = self._map_to_tcm_patterns(
                pulse_analysis, abdominal_data, skin_data
            )
            ...
            summary = self._generate_comprehensive_summary(
                overview, tcm_patterns, health_alerts
            )
            ...
            # 记录指标
            self.metrics.record_counter('comprehensive_analyses', 1, {
                'included_abdominal': str(include_abdominal),
                'included_skin': str(include_skin)
            })
            ...
            logger.exception(f"生成综合切诊分析失败: {str(e)}")
            self.metrics.record_counter('analysis_errors', 1, {'error_type': 'comprehensive_analysis'})
            
            return pb2.ComprehensiveAnalysisResponse(
                analysis_id="",
                success=False,
                error_message=f"Failed to generate comprehensive palpation analysis: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def _map_to_tcm_patterns(self, pulse_analysis, abdominal_data, skin_data):
        ...
        if pulse_analysis and 'tcm_patterns' in pulse_analysis:
            for pattern in pulse_analysis['tcm_patterns']:
                all_patterns.append(pb2.TCMPattern(
    ...
    def _generate_health_alerts(self, pulse_analysis, abdominal_data, skin_data):
        ...
        if pulse_analysis and 'health_alerts' in pulse_analysis:
            for alert in pulse_analysis['health_alerts']:
                alerts.append(pb2.HealthAlert(
                    alert_type=alert.get('type', ''),
                    description=alert.get('description', ''),
                    severity=alert.get('severity', 0.0),
                    recommendation=alert.get('recommendation', ''),
                    requires_immediate_attention=alert.get('requires_immediate_attention', False)
                ))
