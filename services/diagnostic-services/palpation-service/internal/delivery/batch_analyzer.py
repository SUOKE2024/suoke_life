...
class BatchAnalysisHandler:
        ...
        try:
            ...
            if not session_ids:
                ...
                if not session_ids:
                    ...
                    return pb2.BatchAnalysisResponse(
                        success=False,
                        error_message="No sessions found in the specified timeframe"
                    )
            ...
            if not analysis_summaries:
                ...
                return pb2.BatchAnalysisResponse(
                    success=False,
                    error_message="No valid analysis results found"
                )
            ...
            # 记录指标
            self.metrics.record_counter('batch_analyses', 1, {
                'user_id': user_id,
                'session_count': len(analysis_summaries)
            })
            ...
            logger.exception(f"批量分析失败: {str(e)}")
            self.metrics.record_counter('analysis_errors', 1, {'error_type': 'batch_analysis'})
            
            return pb2.BatchAnalysisResponse(
                success=False,
                error_message=f"Failed to perform batch analysis: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def handle_compare_pulse_sessions(
        self, 
        request: pb2.ComparePulseSessionsRequest, 
        context: grpc.ServicerContext
        ...
        try:
            ...
            # 比较证型变化
            baseline_patterns = [p.get('name') for p in baseline_analysis.get('tcm_patterns', []) 
                                if isinstance(p, dict) and p.get('confidence', 0) > 0.6]
            
            comparison_patterns = [p.get('name') for p in comparison_analysis.get('tcm_patterns', []) 
                                  if isinstance(p, dict) and p.get('confidence', 0) > 0.6]
            ...
            comparison_summary = self._generate_comparison_summary(
                changed_features, new_patterns, resolved_patterns, persisting_patterns,
            ...
            logger.exception(f"比较脉诊会话失败: {str(e)}")
            self.metrics.record_counter('analysis_errors', 1, {'error_type': 'compare_sessions'})
            
            return pb2.ComparePulseSessionsResponse(
                success=False,
                error_message=f"Failed to compare pulse sessions: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
    ...
    def handle_generate_report(
        self, 
        request: pb2.GeneratePalpationReportRequest, 
        context: grpc.ServicerContext
        ...
        try:
            ...
            if not analysis_result:
                ...
                return pb2.GeneratePalpationReportResponse(
                    success=False,
                    error_message=f"Analysis result not found: {analysis_id}"
                )
            ...
            # 记录指标
            self.metrics.record_counter('reports_generated', 1, {
                'format': pb2.ReportFormat.Name(report_format)
            })
            ...
            logger.exception(f"生成切诊报告失败: {str(e)}")
            self.metrics.record_counter('report_errors', 1, {'error_type': 'generate_report'})
            
            return pb2.GeneratePalpationReportResponse(
                success=False,
                error_message=f"Failed to generate palpation report: {str(e)}"
            )
        finally:
            # 记录延迟
            end_time = time.time()
        ...
        for feature in comparison_features:
            if isinstance(feature, dict):
                ...
                if key in baseline_dict:
                    ...
                    # 计算变化百分比
                    if baseline_value != 0:
                        change_percentage = (current_value - baseline_value) / abs(baseline_value) * 100
                    else:
                        change_percentage = 0
    ...
    def _generate_trend_analysis(self, analysis_summaries):
        ...
        bad_pulse_types = {'RAPID', 'FAINT', 'INTERMITTENT', 'SCATTERED', 
                           '数脉', '微脉', '代脉', '散脉'}  # 异常脉象
        
        first_bad_count = sum(1 for t in first_analysis.dominant_pulse_types if t in bad_pulse_types)
        last_bad_count = sum(1 for t in last_analysis.dominant_pulse_types if t in bad_pulse_types)
        
        first_good_count = sum(1 for t in first_analysis.dominant_pulse_types if t in good_pulse_types)
        last_good_count = sum(1 for t in last_analysis.dominant_pulse_types if t in good_pulse_types)
        ...
        if changed_features:
            ...
            if significant_changes:
                ...
                for feature in significant_changes[:3]:
                    ...
                    change_desc.append(f"{feature.feature_name}{direction}{abs(int(feature.change_percentage))}%")
