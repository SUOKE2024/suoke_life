...
class AdvancedVisualizer:
    ...
    async def create_pulse_waveform_chart(
        ...
        try:
            ...
            if config.interaction_mode == InteractionMode.INTERACTIVE:
                ...
                # 检测峰值
                peaks, _ = signal.find_peaks(pulse_data, height=np.mean(pulse_data), distance=sampling_rate//3)
                if len(peaks) > 0:
                    fig.add_trace(go.Scatter(
                        x=time_axis[peaks],
                ...
                peaks, _ = signal.find_peaks(pulse_data, height=np.mean(pulse_data), distance=sampling_rate//3)
                if len(peaks) > 0:
                    ax.scatter(time_axis[peaks], np.array(pulse_data)[peaks], 
                             color='#F18F01', s=50, zorder=5, label='心跳峰值')
    ...
    async def create_frequency_spectrum_chart(
        ...
        try:
            ...
            if config.interaction_mode == InteractionMode.INTERACTIVE:
                ...
                peak_indices, _ = signal.find_peaks(magnitude, height=np.max(magnitude)*0.3)
                if len(peak_indices) > 0:
                    ax.scatter(frequencies[peak_indices], magnitude[peak_indices],
                             color='#F18F01', s=100, zorder=5, marker='D', label='主要频率')
    ...
    async def create_constitution_radar_chart(
        ...
        try:
            ...
            # 准备数据
            categories = [constitution_names.get(k, k) for k in constitution_scores.keys()]
            ...
            if config.interaction_mode == InteractionMode.INTERACTIVE:
                ...
                # 绘制雷达图
                ax.plot(angles_plot, values_plot, 'o-', linewidth=3, color='#2E86AB', label='体质评分')
        ...
        try:
            ...
            if config.interaction_mode == InteractionMode.INTERACTIVE:
                ...
                if 'overall_health' in df.columns:
                    fig.add_trace(
                        go.Scatter(x=df['timestamp'], y=df['overall_health'],
                                 mode='lines+markers', name='健康评分',
                                 line=dict(color='#2E86AB', width=3)),
                        row=1, col=1
                    )
                ...
                if 'systolic_bp' in df.columns and 'diastolic_bp' in df.columns:
                    fig.add_trace(
                        go.Scatter(x=df['timestamp'], y=df['systolic_bp'],
                                 mode='lines+markers', name='收缩压',
                                 line=dict(color='#F18F01', width=2)),
                        row=2, col=1
                    )
                    fig.add_trace(
                        go.Scatter(x=df['timestamp'], y=df['diastolic_bp'],
                ...
                if 'constitution_stability' in df.columns:
                    fig.add_trace(
                        go.Bar(x=df['timestamp'], y=df['constitution_stability'],
                               name='体质稳定性', marker_color='#6A994E'),
                        row=2, col=2
                    )
                ...
                # 整体健康评分
                if 'overall_health' in df.columns:
                    axes[0, 0].plot(df['timestamp'], df['overall_health'], 
                ...
                # 心率变化
                if 'heart_rate' in df.columns:
                    axes[0, 1].plot(df['timestamp'], df['heart_rate'],
                ...
                # 血压趋势
                if 'systolic_bp' in df.columns and 'diastolic_bp' in df.columns:
                    axes[1, 0].plot(df['timestamp'], df['systolic_bp'],
                                   color='#F18F01', linewidth=2, marker='^', markersize=3, label='收缩压')
                    axes[1, 0].plot(df['timestamp'], df['diastolic_bp'],
                                   color='#C73E1D', linewidth=2, marker='v', markersize=3, label='舒张压')
                ...
                # 体质稳定性
                if 'constitution_stability' in df.columns:
                    axes[1, 1].bar(df['timestamp'], df['constitution_stability'],
        ...
        try:
            ...
            if config.interaction_mode == InteractionMode.INTERACTIVE:
                ...
                for i, (risk_type, score) in enumerate(risk_scores.items()):
                    ...
                    # 添加分数文本
                    ax.text(0, -0.3, f'{score:.1%}', ha='center', va='center', 
    ...
    async def create_comprehensive_dashboard(
        ...
        try:
            ...
            if pulse_data:
                ...
                fig.add_trace(
                    go.Scatter(x=time_axis, y=pulse_data, mode='lines',
                             name='脉搏', line=dict(color='#A23B72')),
                    row=1, col=2
                )
            ...
            if constitution_scores:
                ...
                fig.add_trace(
                    go.Scatterpolar(r=values, theta=categories, fill='toself',
                                  name='体质', line=dict(color='#F18F01')),
                    row=1, col=3
                )
            ...
            if trend_data:
                ...
                fig.add_trace(
                    go.Scatter(x=dates, y=trend_data, mode='lines+markers',
                             name='趋势', line=dict(color='#6A994E')),
                    row=2, col=2
                )
            ...
            if spectrum_data:
                ...
                fig.add_trace(
                    go.Scatter(x=frequencies, y=magnitudes, mode='lines',
                             fill='tonexty', name='频谱', line=dict(color='#9B59B6')),
                    row=2, col=3
                )
            ...
            fig.add_trace(
                go.Heatmap(z=correlation_matrix, colorscale='RdBu', zmid=0),
                row=3, col=1
            )
            ...
            if prediction_data:
                future_dates = [datetime.now() + timedelta(days=i) for i in range(len(prediction_data))]
                fig.add_trace(
                    go.Scatter(x=future_dates, y=prediction_data, mode='lines+markers',
                             name='预测', line=dict(color='#E74C3C', dash='dash')),
                    row=3, col=2
                )
