...
class PulseAnalyzer:
    ...
    def analyze_pulse(
        ...
        try:
            ...
            logger.exception(f"脉象分析失败: {str(e)}")
            raise
    ...
    def _identify_pulse_types(self, features: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for pulse_type in identified_types:
            ...
            if type_name not in merged_types or pulse_type['confidence'] > merged_types[type_name]['confidence']:
    ...
    def _assess_organ_conditions(
        ...
        for feature in feature_list:
            ...
            if position in self.organ_mapping:
                ...
                position_assessments[position]['features'][feature.get('name')] = feature.get('value')
