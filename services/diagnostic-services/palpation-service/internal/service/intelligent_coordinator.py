...
from internal.model.pulse_models import PulseSession, PulseDataPacket
...
class IntelligentCoordinator:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 核心组件
        self.pulse_processor: Optional[EnhancedPulseProcessor] = None
        self.device_manager: Optional[SmartDeviceManager] = None
        self.abdominal_analyzer: Optional[AbdominalAnalyzer] = None
        self.skin_analyzer: Optional[SkinAnalyzer] = None
        self.tcm_analyzer: Optional[AITCMAnalyzer] = None
    ...
    async def start_palpation_session(
        ...
        try:
            ...
            async with self.session_locks[session_id]:
                ...
                # 分配设备
                device_preference = session_config.get('device_preference') if session_config else None
    ...
    def _analyze_pulse_data(self, pulse_data: PulseDataPacket) -> Dict[str, Any]:
        ...
        # 中医分析
        tcm_patterns = asyncio.run(
            self.tcm_analyzer.analyze_pulse_pattern(features)
        )
    ...
    async def _perform_comprehensive_analysis(
        ...
        health_assessment = await self.tcm_analyzer.generate_health_assessment(
            [self.tcm_analyzer._create_ai_pattern(pattern['pattern_type'], pattern['confidence']) 
             for pattern in all_patterns[:3]],  # 取前3个主要证型
            constitution,
    ...
    async def _check_system_health(self):
        ...
        if self.device_manager:
            ...
            unhealthy_devices = [
                device_id for device_id, status in devices_status.items()
    ...
    async def _collect_metrics(self):
        ...
        if hasattr(self, '_last_metrics_time'):
            ...
            completed_diff = self.metrics.completed_sessions - getattr(self, '_last_completed_count', 0)
    ...
    async def _process_abdominal_task(self, task: AnalysisTask) -> Dict[str, Any]:
        ...
        result = await loop.run_in_executor(
            self.executor,
            self.abdominal_analyzer.analyze_abdominal_palpation,
            abdominal_data
        )
    ...
    async def _process_skin_task(self, task: AnalysisTask) -> Dict[str, Any]:
        ...
        result = await loop.run_in_executor(
            self.executor,
            self.skin_analyzer.analyze_skin_palpation,
            skin_data
        )
