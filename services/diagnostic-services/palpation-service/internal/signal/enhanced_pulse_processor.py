...
class EnhancedPulseProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"增强版脉诊处理器初始化完成 - 采样率: {self.sampling_rate}Hz, 缓冲区: {self.buffer_size}")
    ...
    async def process_realtime_data(self, data_point: float, timestamp: float) -> Optional[Dict]:
        """
        实时处理单个数据点
    ...
    def extract_features_optimized(self, pulse_data: np.ndarray) -> Dict[str, Any]:
        ...
        # 时域特征
        futures.append(
            self.executor.submit(self._extract_time_domain_features_fast, pulse_data)
        )
        ...
        # 小波域特征
        futures.append(
            self.executor.submit(self._extract_wavelet_features_fast, pulse_data)
        )
        
        # 非线性特征
        futures.append(
            self.executor.submit(self._extract_nonlinear_features, pulse_data)
        )
    ...
    def _extract_frequency_domain_features_fast(self, pulse_data: np.ndarray) -> Dict[str, float]:
        ...
        # 谱质心和带宽
        features['spectral_centroid'] = np.sum(positive_freqs * psd) / np.sum(psd) if np.sum(psd) > 0 else 0
        features['spectral_bandwidth'] = np.sqrt(
            np.sum(((positive_freqs - features['spectral_centroid']) ** 2) * psd) / np.sum(psd)
        ) if np.sum(psd) > 0 else 0
    ...
    def _extract_wavelet_features_fast(self, pulse_data: np.ndarray) -> Dict[str, float]:
        ...
        try:
            ...
            for i, coeff in enumerate(coeffs):
                ...
                features[f'wavelet_energy_level_{i}'] = level_energy / total_energy if total_energy > 0 else 0
            ...
            if detail_coeffs:
                ...
                features['wavelet_detail_energy'] = np.sum(all_details**2) / total_energy if total_energy > 0 else 0
    ...
    def _extract_waveform_features(self, pulse_data: np.ndarray, peaks: np.ndarray) -> Dict[str, float]:
        """提取波形特征"""
        features = {}
