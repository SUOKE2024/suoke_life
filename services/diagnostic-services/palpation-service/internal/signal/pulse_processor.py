...
class PulseProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"初始化脉诊信号处理器，采样率: {self.sampling_rate}Hz, 窗口大小: {self.window_size}")
    ...
    def _remove_baseline_wander(self, data: np.ndarray) -> np.ndarray:
        """
        去除基线漂移
    ...
    def segment_pulse_waves(self, data: np.ndarray) -> List[np.ndarray]:
        """
        将连续的脉搏信号分割为单个脉搏波
    ...
    def extract_time_domain_features(self, pulse_wave: np.ndarray) -> Dict[str, float]:
        """
        提取时域特征
        ...
        # 峰值检测
        peaks, peak_props = signal.find_peaks(pulse_wave, prominence=0.1, width=0.05*self.sampling_rate)
        ...
        if len(peaks) >= 1:
            ...
            if len(pulse_wave) - main_peak > 0:
                features['falling_slope'] = (pulse_wave[main_peak] - pulse_wave[-1]) / (len(pulse_wave) - main_peak)
            else:
    ...
    def extract_frequency_domain_features(self, pulse_wave: np.ndarray) -> Dict[str, float]:
        """
        提取频域特征
        ...
        if len(fft_result) > 0:
            ...
            features['high_frequency_ratio'] = high_freq_power / total_power if total_power > 0 else 0
    ...
    def extract_wavelet_features(self, pulse_wave: np.ndarray) -> Dict[str, float]:
        """
        使用小波变换提取特征
        ...
        try:
            ...
            for i, coef in enumerate(coeffs):
                if i == 0:
                    features[f'wavelet_approximation_energy'] = np.sum(coef**2)
                else:
            ...
            logger.error(f"小波变换提取特征失败: {str(e)}")
    ...
    def extract_features(self, pulse_data: np.ndarray, position: str) -> Dict[str, Any]:
        """
        从脉搏数据中提取完整的特征集
    ...
    def _assess_signal_quality(self, full_signal: np.ndarray, pulse_waves: List[np.ndarray]) -> Dict[str, Any]:
        """
        评估信号质量
    ...
    def _estimate_noise(self, signal_data: np.ndarray) -> np.ndarray:
        """估计信号中的噪声分量，使用高通滤波"""
        nyquist_freq = 0.5 * self.sampling_rate
    ...
    def analyze_pulse_rhythm(self, pulse_data: np.ndarray) -> Dict[str, Any]:
        """
        分析脉搏节律
        ...
        for i in range(len(intervals) - 1):
            if intervals[i] < 0.7 * np.median(intervals) and intervals[i+1] > 1.3 * np.median(intervals):
                # 短间隔后跟长间隔，可能是早搏
    ...
    def classify_pulse_type(self, features: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        基于提取的特征，对脉象类型进行分类
        使用简化的规则引擎进行初步分类
    ...
    def check_model_loaded(self):
        ...
        try:
            ...
            raise Exception(f"脉搏分析模型不可用: {str(e)}")
