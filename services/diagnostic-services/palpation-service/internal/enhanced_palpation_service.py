...
class EnhancedPalpationService:
    ...
    async def analyze_pulse(
        ...
        try:
            # 预处理脉象数据
            processed_data = await self._preprocess_pulse_data(
                pulse_data, sample_rate
            )
            
            # 评估数据质量
            quality_score = await self._assess_pulse_quality(
    ...
    async def _extract_time_domain_features(
        ...
        features.append(PulseFeature(
            feature_type="heart_rate_variability",
            value=hrv,
            confidence=0.85
        ))
        ...
        features.append(PulseFeature(
            feature_type="pulse_amplitude",
            value=amplitude_stats,
            confidence=0.95
        ))
    ...
    async def _extract_frequency_domain_features(
        ...
        features.append(PulseFeature(
            feature_type="power_spectrum",
            value=power_spectrum,
            confidence=0.85
        ))
        ...
        features.append(PulseFeature(
            feature_type="dominant_frequency",
            value=dominant_freq,
            confidence=0.80
        ))
