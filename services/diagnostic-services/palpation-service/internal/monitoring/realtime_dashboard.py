...
class RealtimeDashboard:
    ...
    async def _cleanup_old_data(self):
        ...
        try:
            ...
            # 清理旧系统信息
            cursor.execute('DELETE FROM system_info WHERE timestamp < ?', (cutoff_date.isoformat(),))
            
            # 清理旧用户活动
            cursor.execute('DELETE FROM user_activities WHERE timestamp < ?', (cutoff_date.isoformat(),))
