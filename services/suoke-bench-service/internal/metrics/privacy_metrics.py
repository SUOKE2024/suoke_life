...
class PrivacyTestResult:
    ...
    leakage_type: Optional[str]  # 泄露类型
    leakage_severity: Optional[float]  # 泄露严重程度
    mitigation_success: Optional[bool]  # 缓解措施是否成功
...
class PrivacyVerificationMetric(BaseMetric):
    """隐私验证评测指标。"""
    
    def __init__(self, threshold: float = 0.85):
        super().__init__(threshold)
        self.name = "privacy_verification"
        self.description = "评估隐私保护和零知识证明的有效性"
    
    def calculate(self,
                 privacy_cases: List[PrivacyTestCase],
                 privacy_results: List[PrivacyTestResult],
                 zkp_cases: List[ZKPTestCase],
                 zkp_results: List[ZKPTestResult]) -> MetricResult:
        """计算隐私验证的评测指标。"""
        
        # 计算隐私保护指标
        privacy_score = self._calculate_privacy_score(
            privacy_cases, privacy_results
        )
        
        # 计算零知识证明指标
        zkp_score = self._calculate_zkp_score(
            zkp_cases, zkp_results
        )
        
        # 计算性能指标
        performance = self._calculate_performance_metrics(
            privacy_results, zkp_results
        )
    ...
    def _calculate_privacy_score(self,
                               cases: List[PrivacyTestCase],
                               results: List[PrivacyTestResult]) -> Dict[str, float]:
        """计算隐私保护指标。"""
        ...
        # 计算泄露检测率
        leakage_detection_rate = sum(1 for r in results if r.leakage_detected) / len(results)
        
        # 计算泄露严重程度
        severity_scores = [r.leakage_severity for r in results if r.leakage_severity is not None]
        avg_severity = np.mean(severity_scores) if severity_scores else 0.0
        
        # 计算缓解成功率
        mitigation_results = [r.mitigation_success for r in results if r.mitigation_success is not None]
        mitigation_rate = sum(1 for m in mitigation_results if m) / len(mitigation_results) if mitigation_results else 0.0
        
        # 计算隐私等级覆盖度
        privacy_levels = set(case.privacy_level for case in cases)
    ...
    def _calculate_zkp_score(self,
                           cases: List[ZKPTestCase],
                           results: List[ZKPTestResult]) -> Dict[str, float]:
        """计算零知识证明指标。"""
    ...
    def _calculate_performance_metrics(self,
                                    privacy_results: List[PrivacyTestResult],
                                    zkp_results: List[ZKPTestResult]) -> Dict[str, float]:
        """计算性能指标。"""
