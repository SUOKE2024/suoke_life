...
class Metric:
    ...
    def __init__(self, name: str, threshold: float = 0.0, unit: str = "", higher_is_better: bool = True):
        """
        初始化指标
...
class AccuracyMetric(Metric):
    ...
    def compute(self, predictions: List[Any], references: List[Any]) -> float:
        ...
        if len(predictions) != len(references):
            logger.warning(f"预测值与参考值数量不一致: {len(predictions)} vs {len(references)}")
            return 0.0
...
class F1Metric(Metric):
    ...
    def __init__(self, threshold: float = 0.75, unit: str = "", average: str = "weighted"):
        """
        初始化F1分数指标
...
class AgreementRateMetric(Metric):
    ...
    def compute(self, predictions: List[Dict[str, Any]], expert_opinions: List[Dict[str, Any]]) -> float:
        """
        计算辨证一致率
        ...
        if len(predictions) != len(expert_opinions):
            logger.warning(f"预测值与专家意见数量不一致: {len(predictions)} vs {len(expert_opinions)}")
            return 0.0
        ...
        for pred, expert in zip(predictions, expert_opinions):
            # 判断主诊断是否一致
...
class ROUGEMetric(Metric):
    ...
    def __init__(self, threshold: float = 0.6, unit: str = "", rouge_type: str = "rouge-l"):
        """
        初始化ROUGE评分指标
        ...
        Args:
            ...
            rouge_type: ROUGE类型
        """
        super().__init__(f"rouge_{rouge_type.split('-')[1]}", threshold, unit, higher_is_better=True)
...
class LatencyMetric(Metric):
    ...
    def __init__(self, threshold: float = 500.0, unit: str = "ms", percentile: int = 95):
        """
        初始化延迟指标
        ...
        Args:
            ...
            percentile: 百分位数
        """
        super().__init__(f"latency_p{percentile}", threshold, unit, higher_is_better=False)
...
class MetricRegistry:
    ...
    def compute(self, name: str, predictions: List[Any], references: List[Any]) -> Optional[float]:
        """
        计算指标值
