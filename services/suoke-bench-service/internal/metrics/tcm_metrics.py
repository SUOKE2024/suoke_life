...
class TongueRecognitionMetric(BaseMetric):
    """舌象识别评测指标。"""
    
    def __init__(self, threshold: float = 0.85):
        super().__init__(threshold)
        self.name = "tongue_recognition"
        self.description = "评估舌象识别的准确性"
    
    def calculate(self, 
                 predictions: List[TongueFeature],
                 ground_truth: List[TongueFeature]) -> MetricResult:
        """计算舌象识别的准确性指标。"""
        
        # 计算各个特征的准确率
        color_acc = self._calculate_feature_accuracy(
            [p.color for p in predictions],
            [g.color for g in ground_truth]
        )
        
        coating_acc = self._calculate_feature_accuracy(
            [p.coating for p in predictions],
            [g.coating for g in ground_truth]
        )
        
        shape_acc = self._calculate_feature_accuracy(
            [p.shape for p in predictions],
            [g.shape for g in ground_truth]
        )
        
        moisture_acc = self._calculate_feature_accuracy(
            [p.moisture for p in predictions],
            [g.moisture for g in ground_truth]
        )
        
        # 计算裂纹识别的F1分数
        cracks_f1 = self._calculate_cracks_f1(
            [p.cracks for p in predictions],
            [g.cracks for g in ground_truth]
        )
        
        # 计算斑点识别的IoU
        spots_iou = self._calculate_spots_iou(
            [p.spots for p in predictions],
            [g.spots for g in ground_truth]
        )
    ...
    def _calculate_feature_accuracy(self,
                                  pred: List[str],
                                  true: List[str]) -> float:
        """计算分类特征的准确率。"""
        return accuracy_score(true, pred)
    
    def _calculate_cracks_f1(self,
                            pred: List[List[str]],
                            true: List[List[str]]) -> float:
        """计算裂纹识别的F1分数。"""
        f1_scores = []
    ...
    def _calculate_spots_iou(self,
                           pred: List[List[Dict[str, Any]]],
                           true: List[List[Dict[str, Any]]]) -> float:
        """计算斑点识别的IoU (Intersection over Union)。"""
        ious = []
        for p_spots, t_spots in zip(pred, true):
    ...
    def _calculate_spot_intersection(self,
                                   spot1: Dict[str, Any],
                                   spot2: Dict[str, Any]) -> float:
        """计算两个斑点区域的交集面积。"""
    ...
    def _calculate_spot_union(self,
                            spot1: Dict[str, Any],
                            spot2: Dict[str, Any]) -> float:
        """计算两个斑点区域的并集面积。"""
...
class FaceRecognitionMetric(BaseMetric):
    """面色识别评测指标。"""
    ...
    def calculate(self,
                 predictions: List[FaceFeature],
                 ground_truth: List[FaceFeature]) -> MetricResult:
        """计算面色识别的准确性指标。"""
        ...
        luster_acc = self._calculate_feature_accuracy(
            [p.luster for p in predictions],
            [g.luster for g in ground_truth]
        )
        
        expression_acc = self._calculate_feature_accuracy(
            [p.expression for p in predictions],
            [g.expression for g in ground_truth]
        )
        
        # 计算面部分区特征的准确率
        areas_acc = self._calculate_areas_accuracy(
            [p.areas for p in predictions],
            [g.areas for g in ground_truth]
        )
    ...
    def _calculate_areas_accuracy(self,
                                pred: List[Dict[str, str]],
                                true: List[Dict[str, str]]) -> float:
        """计算面部分区特征的准确率。"""
        accuracies = []
        for p_areas, t_areas in zip(pred, true):
            # 确保两个字典有相同的键
            ...
            # 计算每个分区的准确率
            area_matches = sum(1 for area in common_areas 
                             if p_areas[area] == t_areas[area])
            accuracies.append(area_matches / len(common_areas))
        
        return np.mean(accuracies)

class PulseRecognitionMetric(BaseMetric):
    """脉象识别评测指标。"""
    ...
    def calculate(self,
                 predictions: List[PulseFeature],
                 ground_truth: List[PulseFeature]) -> MetricResult:
        """计算脉象识别的准确性指标。"""
        
        # 计算脉率的均方根误差
        frequency_rmse = self._calculate_frequency_rmse(
            [p.frequency for p in predictions],
            [g.frequency for g in ground_truth]
        )
        
        # 计算其他特征的准确率
        strength_acc = self._calculate_feature_accuracy(
            [p.strength for p in predictions],
            [g.strength for g in ground_truth]
        )
        
        rhythm_acc = self._calculate_feature_accuracy(
            [p.rhythm for p in predictions],
            [g.rhythm for g in ground_truth]
        )
        
        width_acc = self._calculate_feature_accuracy(
            [p.width for p in predictions],
            [g.width for g in ground_truth]
        )
        
        depth_acc = self._calculate_feature_accuracy(
            [p.depth for p in predictions],
            [g.depth for g in ground_truth]
        )
        
        length_acc = self._calculate_feature_accuracy(
            [p.length for p in predictions],
            [g.length for g in ground_truth]
        )
    ...
    def _calculate_frequency_rmse(self,
                                pred: List[float],
                                true: List[float]) -> float:
        """计算脉率的均方根误差。"""
        return np.sqrt(np.mean((np.array(pred) - np.array(true)) ** 2))

class ConstitutionClassificationMetric(BaseMetric):
    """体质辨识评测指标。"""
    ...
    def calculate(self,
                 ...
                 confidences: Optional[List[float]] = None) -> MetricResult:
        """计算体质辨识的准确性指标。"""
    ...
    def _calculate_confidence_weighted_accuracy(self,
                                             pred: List[str],
                                             true: List[str],
                                             conf: List[float]) -> float:
        """计算置信度加权的准确率。"""
        correct = [1 if p == t else 0 for p, t in zip(pred, true)]
        weighted_correct = sum(c * w for c, w in zip(correct, conf))
