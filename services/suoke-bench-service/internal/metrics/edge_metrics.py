...
class EdgePerformanceMetric(BaseMetric):
    """端侧性能评测指标。"""
    
    def __init__(self, threshold: float = 0.85):
        super().__init__(threshold)
        self.name = "edge_performance"
        self.description = "评估端侧设备上的模型性能"
    
    def calculate(self,
                 device_info: DeviceInfo,
                 model_info: ModelInfo,
                 perf_metrics: List[PerformanceMetrics],
                 infer_metrics: List[InferenceMetrics]) -> MetricResult:
        """计算端侧性能的评测指标。"""
        
        # 计算资源使用指标
        resource_score = self._calculate_resource_score(
            device_info, perf_metrics
        )
        
        # 计算推理性能指标
        inference_score = self._calculate_inference_score(
            model_info, infer_metrics
        )
    ...
    def _calculate_resource_score(self,
                                device_info: DeviceInfo,
                                metrics: List[PerformanceMetrics]) -> Dict[str, float]:
        """计算资源使用指标。"""
    ...
    def _calculate_inference_score(self,
                                 model_info: ModelInfo,
                                 metrics: List[InferenceMetrics]) -> Dict[str, float]:
        """计算推理性能指标。"""
    ...
    def _calculate_stability_score(self,
                                 perf_metrics: List[PerformanceMetrics],
                                 infer_metrics: List[InferenceMetrics]) -> Dict[str, float]:
        """计算稳定性指标。"""
