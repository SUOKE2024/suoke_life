...
class MetricsCollector:
    ...
    def __init__(self, registry: CollectorRegistry | None = None):
        ...
        self.cached_models = Gauge(
            "suoke_cached_models", "Number of models in cache", registry=self.registry
        )
        ...
        self.service_info = Info(
            "suoke_service_info", "Service information", registry=self.registry
        )
    ...
    def record_benchmark_run(
        self, benchmark_id: str, model_id: str, status: str, duration: float
    ):
        """记录基准测试运行"""
        with self._lock:
            self.benchmark_runs_total.labels(
                benchmark_id=benchmark_id, model_id=model_id, status=status
            ).inc()

            if status == "SUCCESS":
                self.benchmark_duration_seconds.labels(
                    benchmark_id=benchmark_id, model_id=model_id
                ).observe(duration)

    def record_model_inference(
        self, model_id: str, model_version: str, task_type: str, duration: float
    ):
        """记录模型推理"""
        with self._lock:
            self.model_inference_duration_seconds.labels(
                model_id=model_id, model_version=model_version, task_type=task_type
            ).observe(duration)
    ...
    def record_benchmark_quality(
        self, benchmark_id: str, model_id: str, metrics: dict[str, float]
    ):
        """记录基准测试质量指标"""
        with self._lock:
    ...
    def update_system_metrics(
        self, cpu_percent: float, memory_bytes: int, disk_usage: dict[str, int]
    ):
        """更新系统指标"""
    ...
    def get_custom_metric_stats(
        ...
        with self._lock:
            ...
            recent_points = [
                point
                for point in self._custom_metrics[name]
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "cache_memory_mb": self.cache_memory_usage_bytes._value._value
                / 1024
                / 1024,
                "system_cpu_percent": self.system_cpu_usage._value._value,
                "system_memory_mb": self.system_memory_usage._value._value
                / 1024
                / 1024,
                "custom_metrics": {
...
) -> tuple[MetricsCollector, PerformanceMonitor]:
    """初始化监控系统"""
