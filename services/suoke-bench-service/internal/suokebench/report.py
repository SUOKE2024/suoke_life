...
class ReportGenerator:
    ...
    def _prepare_report_data(self, summary: Dict[str, Any], data_dir: Path) -> Dict[str, Any]:
        """
        准备报告数据
    ...
    def _calculate_metrics_summary(self, summary: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metrics_summary = {
            "overall": {
                ...
                "success_rate": summary["success"] / summary["tasks"] if summary["tasks"] > 0 else 0,
            },
            "metrics": {},
        }
        ...
        for task_id, result in summary["results"].items():
            ...
            for metric_name, metric_value in result.get("metrics", {}).items():
                if isinstance(metric_value, (int, float)):
                    ...
                    metrics_summary["metrics"][metric_name]["values"].append(metric_value)
                    
                    # 更新统计值
                    values = metrics_summary["metrics"][metric_name]["values"]
                    metrics_summary["metrics"][metric_name]["avg"] = sum(values) / len(values)
                    metrics_summary["metrics"][metric_name]["min"] = min(values)
                    metrics_summary["metrics"][metric_name]["max"] = max(values)
                    
        return metrics_summary
        
    def _generate_plots(self, summary: Dict[str, Any], metrics_summary: Dict[str, Any], data_dir: Path) -> Dict[str, str]:
        """
        生成图表
...
def main():
    ...
    parser.add_argument("--format", type=str, default="html", choices=["html", "pdf", "markdown", "json"], help="输出格式")
