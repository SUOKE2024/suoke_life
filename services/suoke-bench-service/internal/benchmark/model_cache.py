...
from internal.benchmark.model_interface import ModelInterface, LocalModel, RemoteAPIModel
...
class ModelCache:
    ...
    def __init__(
        ...
        logger.info(f"模型缓存初始化完成，最大内存: {max_memory_mb}MB，最大模型数: {max_models}")
    ...
    def get_model(
        ...
        with self._lock:
            ...
            logger.info(f"模型加载完成: {cache_key}, 耗时: {load_time:.2f}s, 内存: {memory_usage/1024/1024:.1f}MB")
