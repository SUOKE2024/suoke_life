...
class RequestResult:
    ...
    def is_success(self) -> bool:
        """请求是否成功"""
        return self.error is None and (self.status_code is None or 200 <= self.status_code < 400)
...
class TestProfile:
    ...
    def __init__(self,
        ...
        if peak_duration is None:
            self.peak_duration = max(0, duration - warmup_duration - ramp_up_duration - ramp_down_duration - cooldown_duration)
        else:
    ...
    def get_target_rps_at_time(self, elapsed_time: float) -> float:
        ...
        if phase == TestPhase.RAMP_DOWN:
            # 减压阶段线性减少到目标RPS的25%
            progress = (elapsed_time - self.warmup_duration - self.ramp_up_duration - self.peak_duration) / self.ramp_down_duration
            return self.target_rps * (1.0 - 0.75 * progress)
        
        # 冷却阶段线性减少到0
        progress = (elapsed_time - self.warmup_duration - self.ramp_up_duration - self.peak_duration - self.ramp_down_duration) / self.cooldown_duration
...
class EndpointSpec:
    ...
    def __init__(self,
                ...
                method: str = "GET",
                weight: int = 1,
                request_generator: Optional[Callable[[], Dict[str, Any]]] = None,
                metadata: Optional[Dict[str, Any]] = None):
        """
        初始化接口规格
...
class TestResult:
    ...
    def _analyze_results(self):
        ...
        if self.results:
            min_time = min(r.start_time for r in self.results)
    ...
    def _generate_plots(self, output_dir: str):
        ...
        # 计算滚动平均值
        df['rolling_avg'] = df[df['is_success']]['response_time'].rolling(window=50).mean()
        
        plt.scatter(df['timestamp'], df['response_time'], s=5, alpha=0.3, label='每个请求')
        plt.plot(df['timestamp'], df['rolling_avg'], color='red', linewidth=2, label='滚动平均值')
        plt.title('响应时间随时间变化')
        plt.xlabel('时间')
        plt.ylabel('响应时间 (ms)')
        plt.legend()
        ...
        if service_data:
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'response_time_by_service.png'), dpi=300)
        plt.close()
        ...
        if not endpoint_df.empty and len(endpoint_df) > 1:
            ...
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, 'success_rate_by_endpoint.png'), dpi=300)
            plt.close()
    ...
    def _generate_html_report(self, output_dir: str):
        ...
        # 确定成功率和P95响应时间的状态类
        success_rate_class = "success" if self.success_rate >= self.profile.success_threshold else "failure"
        p95_class = "success" if self.p95_response_time <= self.profile.p95_threshold else "warning"
        ...
        # 计算RPS达成率
        rps_achievement = (self.rps / self.profile.target_rps) * 100 if self.profile.target_rps > 0 else 0
    ...
    def is_successful(self) -> bool:
        ...
        Returns:
            bool: 测试是否成功
        """
        return (self.success_rate >= self.profile.success_threshold and
                self.p95_response_time <= self.profile.p95_threshold)
...
class PerformanceTester:
    ...
    async def _send_request(self, 
                          ...
                          phase: TestPhase,
                          semaphore: asyncio.Semaphore) -> RequestResult:
        """
        发送请求
...
class SuokeBenchmark:
    ...
    def create_user_service_test(self, 
                               base_url: str, 
                               target_rps: int = 100,
                               duration: int = 300,
                               token: Optional[str] = None) -> PerformanceTester:
        """
        创建用户服务测试
        ...
        endpoints = [
            ...
            EndpointSpec(
                ...
                method="GET",
                weight=30,
                request_generator=lambda: {"user_id": f"user_{random.randint(1, 1000)}"}
            ),
            EndpointSpec(
                service_name=ServiceType.USER_SERVICE,
    ...
    def create_four_diagnosis_test(self, 
                                 base_url: str, 
                                 target_rps: int = 50,
                                 duration: int = 300,
                                 token: Optional[str] = None) -> PerformanceTester:
    ...
    def create_api_gateway_test(self, 
                              base_url: str, 
                              target_rps: int = 200,
                              duration: int = 300,
                              token: Optional[str] = None) -> PerformanceTester:
        ...
        endpoints = [
            ...
            EndpointSpec(
                ...
                weight=12,
                request_generator=lambda: {
                    "q": random.choice(["舌诊", "脉诊", "面诊", "中医", "养生", "体质"]),
                    "limit": 10
                }
            ),
    ...
    async def run_comprehensive_benchmark(self, 
                                       base_url: str,
                                       token: Optional[str] = None) -> List[TestResult]:
        """
        运行全面基准测试
    ...
    def _create_comparison_charts(self, results: List[TestResult], output_dir: str):
        ...
        plt.bar(x, avg_times, width=bar_width, label='平均响应时间', color='blue', alpha=0.7)
        plt.bar([i + bar_width for i in x], p95_times, width=bar_width, label='P95响应时间', color='red', alpha=0.7)
        ...
        plt.bar(x, target_rps, width=bar_width, label='目标RPS', color='green', alpha=0.7)
        plt.bar([i + bar_width for i in x], actual_rps, width=bar_width, label='实际RPS', color='orange', alpha=0.7)
        ...
        plt.bar(x, success_rates, color=['green' if sr >= th else 'red' for sr, th in zip(success_rates, thresholds)])
    ...
    async def _generate_summary_html(self, results: List[TestResult], output_dir: str):
        ...
        highest_rps_test = next(result.test_name for result in results if result.rps == highest_rps)
        
        fastest_response = min(result.average_response_time for result in results)
        fastest_response_test = next(result.test_name for result in results if result.average_response_time == fastest_response)
        
        highest_success_rate = max(result.success_rate for result in results)
        highest_success_rate_test = next(result.test_name for result in results if result.success_rate == highest_success_rate)
        
        slowest_response = max(result.p95_response_time for result in results)
        slowest_response_test = next(result.test_name for result in results if result.p95_response_time == slowest_response)
        
        lowest_success_rate = min(result.success_rate for result in results)
        lowest_success_rate_test = next(result.test_name for result in results if result.success_rate == lowest_success_rate)
        
        rps_achievements = [(result.rps / result.profile.target_rps) * 100 if result.profile.target_rps > 0 else 0 for result in results]
        lowest_rps_achievement = min(rps_achievements)
        lowest_rps_achievement_test = results[rps_achievements.index(lowest_rps_achievement)].test_name
        
        # 生成测试行
        test_rows = ""
        for result in results:
            rps_achievement = (result.rps / result.profile.target_rps) * 100 if result.profile.target_rps > 0 else 0
