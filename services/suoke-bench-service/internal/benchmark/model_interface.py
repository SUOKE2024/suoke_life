...
class ModelInterface(ABC):
    ...
    @abstractmethod
    def predict(
        self, inputs: Any, parameters: dict[str, Any] | None = None
    ) -> ModelResponse:
        """
        模型预测接口
...
class LocalModel(ModelInterface):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"模型 {model_id}:{model_version} 加载完成，耗时 {load_time:.2f} 秒"
        )
    ...
    def _load_pytorch_model(self):
        ...
        try:
            ...
            raise RuntimeError("无法导入PyTorch，请确保已安装") from None
        except Exception as e:
            logger.error(f"PyTorch模型加载失败: {str(e)}")
            raise RuntimeError(f"PyTorch模型加载失败: {str(e)}") from e
    ...
    def _load_tensorflow_model(self):
        ...
        try:
            ...
            raise RuntimeError("无法导入TensorFlow，请确保已安装") from None
        except Exception as e:
            logger.error(f"TensorFlow模型加载失败: {str(e)}")
            raise RuntimeError(f"TensorFlow模型加载失败: {str(e)}") from e
    ...
    def _load_onnx_model(self):
        ...
        try:
            ...
            raise RuntimeError("无法导入ONNX Runtime，请确保已安装") from None
        except Exception as e:
            logger.error(f"ONNX模型加载失败: {str(e)}")
            raise RuntimeError(f"ONNX模型加载失败: {str(e)}") from e
...
class RemoteAPIModel(ModelInterface):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        logger.info(
            f"远程API模型 {model_id}:{model_version} 初始化完成，API地址: {api_url}"
        )
    ...
    def predict(
        ...
        try:
            ...
            raise RuntimeError(f"API响应解析失败: {str(e)}") from e
        except Exception as e:
            logger.error(f"调用API模型失败: {str(e)}")
            raise RuntimeError(f"调用API模型失败: {str(e)}") from e
    ...
    async def predict_async(
        ...
        try:
            ...
            raise RuntimeError(f"异步API请求失败: {str(e)}") from e
        except json.JSONDecodeError as e:
            logger.error(f"异步API响应解析失败: {str(e)}")
            raise RuntimeError(f"异步API响应解析失败: {str(e)}") from e
        except Exception as e:
...
class ModelRegistry:
    ...
    def get_model(self, model_id: str, model_version: str) -> ModelInterface | None:
        """
        获取模型
