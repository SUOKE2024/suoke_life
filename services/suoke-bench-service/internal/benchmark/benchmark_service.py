...
from internal.resilience.retry import retry, circuit_breaker, RetryConfig, CircuitBreakerConfig
...
class BenchmarkExecutor:
    ...
    def __init__(self, config: BenchConfig):
        ...
        # 线程池
        self.executor = ThreadPoolExecutor(max_workers=config.benchmark.max_concurrent_tasks)
        ...
        logger.info(f"基准测试执行器初始化完成，最大并发任务数: {config.benchmark.max_concurrent_tasks}")
    ...
    async def _execute_benchmark_async(self, task: BenchmarkTask):
        ...
        try:
            task.status = "running"
            self.metrics.update_active_benchmarks(len([t for t in self.active_tasks.values() if t.status == "running"]))
            ...
            logger.info(f"基准测试任务完成: {task.task_id}, 耗时: {execution_time:.2f}s")
    ...
    async def _run_benchmark(self, task: BenchmarkTask) -> Dict[str, Any]:
        ...
        for i in range(0, total_items, batch_size):
            ...
            if processed_items % 100 == 0:
                logger.info(f"任务 {task.task_id} 进度: {processed_items}/{total_items}")
        
        # 计算评估指标
        metrics = await self._calculate_metrics(task.benchmark_id, predictions, task.test_data)
    ...
    async def _calculate_metrics(
        ...
        successful_predictions = len([p for p in predictions if not p.metadata.get("error", False)])
        
        metrics["total_predictions"] = total_predictions
        metrics["successful_predictions"] = successful_predictions
        metrics["success_rate"] = successful_predictions / total_predictions if total_predictions > 0 else 0.0
        
        # 延迟指标
        latencies = [p.latency for p in predictions if not p.metadata.get("error", False)]
        if latencies:
            metrics["avg_latency"] = sum(latencies) / len(latencies)
            metrics["max_latency"] = max(latencies)
            metrics["min_latency"] = min(latencies)
            metrics["p95_latency"] = sorted(latencies)[int(len(latencies) * 0.95)] if len(latencies) > 20 else max(latencies)
        
        # 置信度指标
        confidences = [p.confidence for p in predictions if not p.metadata.get("error", False)]
        if confidences:
        ...
        if benchmark_id == "tcm_diagnosis":
            ...
            metrics.update(await self._calculate_health_plan_metrics(predictions, test_data))
        elif benchmark_id == "agent_collaboration":
            metrics.update(await self._calculate_collaboration_metrics(predictions, test_data))
    ...
    def get_task_result(self, task_id: str) -> Optional[BenchmarkResult]:
        """获取任务结果"""
    ...
    async def _generate_html_report(self, result: BenchmarkResult) -> str:
        ...
        successful_predictions = len([p for p in result.predictions if not p.metadata.get("error", False)])
        avg_latency = sum(p.latency for p in result.predictions) / total_predictions if total_predictions > 0 else 0
...
# 全局基准测试执行器
_global_executor: Optional[BenchmarkExecutor] = None
