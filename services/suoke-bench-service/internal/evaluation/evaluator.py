...
class BaseEvaluator:
    ...
    def __init__(self, metrics: List[str], config: Optional[Dict[str, Any]] = None):
        """
        初始化评估器
    ...
    def evaluate(self, predictions: List[Any], references: List[Any]) -> Dict[str, float]:
        ...
        for metric_name in self.metrics:
            try:
                # 使用指标注册表计算指标
                metric_value = METRIC_REGISTRY.compute(metric_name, predictions, references)
...
class TCMDiagnosisEvaluator(BaseEvaluator):
    ...
    def evaluate(self, predictions: List[Dict[str, Any]], expert_opinions: List[Dict[str, Any]]) -> Dict[str, float]:
        """
        评估中医辨证预测结果
        ...
        for pred, ref in zip(predictions, expert_opinions):
            # 提取主要特征
    ...
    def _evaluate_tcm_specific(
        ...
        # 计算症状匹配率
        symptom_match_rate = self._calculate_symptom_match_rate(predictions, expert_opinions)
        results["symptom_match_rate"] = symptom_match_rate
        
        # 计算治法一致性
        treatment_consistency = self._calculate_treatment_consistency(predictions, expert_opinions)
...
class EvaluatorFactory:
    ...
    @staticmethod
    def create(task_type: str, metrics: List[str], config: Optional[Dict[str, Any]] = None) -> BaseEvaluator:
        """
        创建评估器
        ...
        Returns:
            评估器实例
        """
        if task_type in ["TCM_DIAGNOSIS", "TONGUE_RECOGNITION", "FACE_RECOGNITION", "PULSE_RECOGNITION"]:
