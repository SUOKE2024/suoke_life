...
async function fetchDashboardStats() {
    try {
        ...
        // 更新统计卡片
        document.getElementById('completed-benchmarks').textContent = data.completed_benchmarks || 0;
        document.getElementById('running-benchmarks').textContent = data.running_benchmarks || 0;
        document.getElementById('available-models').textContent = data.available_models || 0;
...
async function fetchBenchmarks() {
    try {
        ...
        if (!response.ok) throw new Error('获取评测列表失败');
...
async function fetchRecentRuns() {
    try {
        ...
        if (!response.ok) throw new Error('获取最近运行记录失败');
        ...
        data.forEach(run => {
            ...
            tr.innerHTML = `
                ...
                <td><span class="status ${run.status.toLowerCase()}">${getStatusText(run.status)}</span></td>
                <td>${formatDate(run.created_at)}</td>
                <td>
                    <button class="btn btn-small primary" onclick="viewResult('${run.run_id}')">查看</button>
                </td>
            `;
...
async function fetchResultsList(page = 1, filter = 'all') {
    try {
        ...
        if (!response.ok) throw new Error('获取结果列表失败');
        ...
        results.forEach(result => {
            ...
            tr.innerHTML = `
                ...
                <td><span class="status ${result.status.toLowerCase()}">${getStatusText(result.status)}</span></td>
                <td>${formatDate(result.created_at)}</td>
                <td>${result.completed_at ? formatDate(result.completed_at) : '-'}</td>
                <td>
                    <button class="btn btn-small primary" onclick="viewResult('${result.run_id}')">详情</button>
                    <button class="btn btn-small" onclick="generateReport('${result.run_id}')">报告</button>
                </td>
            `;
...
function initBenchmarkForm() {
    ...
    benchmarkSelect.addEventListener('change', () => {
        ...
        if (!benchmark || !benchmark.parameters) return;
    ...
    benchmarks.forEach(benchmark => {
        ...
        card.innerHTML = `
            ...
            <div class="card-footer" style="margin-top: 1rem;">
                <button class="btn primary" onclick="selectBenchmark('${benchmark.id}')">选择</button>
            </div>
        `;
...
function viewReport(reportUrl) {
    ...
    reportContent.innerHTML = '<div class="report-placeholder"><div class="loading"></div> 加载中...</div>';
...
function selectBenchmark(benchmarkId) {
    ...
    // 滚动到表单
    document.querySelector('.run-benchmark-form').scrollIntoView({ behavior: 'smooth' });
}
