...
class ReportGenerator:
    ...
    def generate_report(
        ...
        """
        # 准备报告数据
        report_data = self._prepare_report_data(result, include_samples, include_metrics)
    ...
    def _prepare_report_data(
        ...
        report_data["passing_rate"] = passing_count / total_count if total_count > 0 else 0
    ...
    def _generate_summary(self, report_data: Dict[str, Any]) -> str:
        ...
        if metrics:
            ...
            for metric in metrics:
                ...
                if not best_metric or (metric["higher_is_better"] and normalized_value > best_metric["normalized_value"]) or (not metric["higher_is_better"] and normalized_value < best_metric["normalized_value"]):
                    best_metric = {
                ...
                if not worst_metric or (metric["higher_is_better"] and normalized_value < worst_metric["normalized_value"]) or (not metric["higher_is_better"] and normalized_value > worst_metric["normalized_value"]):
                    worst_metric = {
                        "name": metric["display_name"],
                        "value": metric["value"],
                        "unit": metric["unit"],
            ...
            best_metric_text = f"{best_metric['name']}（{best_metric['value']}{best_metric['unit']}）" if best_metric else "无"
            worst_metric_text = f"{worst_metric['name']}（{worst_metric['value']}{worst_metric['unit']}）" if worst_metric else "无"
        else:
            best_metric_text = "无"
            worst_metric_text = "无"
        ...
        if passing_rate < 0.7:
            ...
            summary += " 模型整体表现良好，但仍有提升空间，建议针对表现较差的指标进行优化。"
        else:
