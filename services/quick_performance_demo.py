...
async def quick_performance_demo():
    ...
    for i in range(5):
        ...
        logger.info(f"  📈 CPU: {metrics['cpu_usage']:.1f}%, 内存: {metrics['memory_usage']:.1f}%")
        await asyncio.sleep(0.5)
    
    # 收集服务指标
    logger.info("🔍 收集服务性能指标...")
    services = ["xiaoai-service", "xiaoke-service", "laoke-service", "soer-service", "api-gateway"]
    ...
    for service in services:
        ...
        logger.info(f"  🚀 {service}: {metrics['response_time']:.2f}s, {metrics['status']}")
        await asyncio.sleep(0.3)
    
    # 生成报告
    report_file = f"quick_performance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(report_file, 'w', encoding='utf-8') as f:
        json.dump(monitoring_data, f, ensure_ascii=False, indent=2)
    
    # 显示摘要
    avg_cpu = sum(m["cpu_usage"] for m in monitoring_data["system_metrics"]) / len(monitoring_data["system_metrics"])
    avg_memory = sum(m["memory_usage"] for m in monitoring_data["system_metrics"]) / len(monitoring_data["system_metrics"])
    avg_response = sum(m["response_time"] for m in monitoring_data["service_metrics"]) / len(monitoring_data["service_metrics"])
