...
async def test_rate_limiting():
    ...
    try:
        from pkg.utils.rate_limiter import RateLimiter, TokenBucket, SlidingWindowCounter
...
async def test_service_communication():
    ...
    try:
        from pkg.utils.service_client import (
            ServiceEndpoint, ServiceClient, ServiceRegistry,
...
async def test_alerting_system():
    ...
    try:
        from pkg.utils.alerting import (
            ...
            WebhookChannel, get_default_alert_rules
        )
...
async def test_metrics_collection():
    ...
    try:
        from pkg.utils.metrics import (
            maze_operations_total, response_time_histogram,
            active_mazes_gauge, record_maze_operation
        )
        ...
        print(f"  ✅ 指标记录完成:")
        print(f"     - 操作计数器: 已记录20个操作")
        print(f"     - 响应时间: 已记录3个样本")
        print(f"     - 活跃迷宫: 设置为25个")
