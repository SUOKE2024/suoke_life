    ...
    async def _execute_with_retry(self, operation, *args, **kwargs):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.warning(f"Redis操作失败 (尝试 {attempt + 1}/{self.max_retries}): {str(e)}")
...
class CacheManager:
    ...
    def __init__(self, use_redis: bool = None, redis_url: str = None, fallback_to_memory: bool = True):
        ...
        if self.use_redis and REDIS_AVAILABLE:
            try:
                ...
                logger.warning(f"Redis初始化失败: {str(e)}")
                if self.fallback_to_memory:
    ...
    async def delete_pattern(self, pattern: str) -> int:
        ...
        try:
            ...
            logger.error(f"删除主缓存模式失败: {str(e)}")
        ...
        if self.fallback_backend and hasattr(self.fallback_backend, 'delete_pattern'):
            try:
                ...
                logger.error(f"删除备用缓存模式失败: {str(e)}")
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"获取主缓存统计失败: {str(e)}")
            stats["primary"] = {}
        ...
        if self.fallback_backend:
            try:
                ...
                logger.error(f"获取备用缓存统计失败: {str(e)}")
                stats["fallback"] = {}
