...
class MetricsStore:
    ...
    def get_metrics_store(self) -> dict[str, Any]:
        """获取指标存储"""
        return self.metrics_store

    def get_performance_tracker(self) -> dict[str, Any]:
        """获取性能跟踪器"""
...
def maze_generation_time(func: Callable) -> Callable:
    """迷宫生成时间装饰器"""
    # 性能阈值常量
    SLOW_OPERATION_THRESHOLD = 5.0  # 超过5秒认为是慢操作
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            # 记录内存指标
            _metrics_store.get_metrics_store()["generation_times"].append({
                "maze_type": maze_type,
            ...
            if duration > SLOW_OPERATION_THRESHOLD:
                _metrics_store.get_performance_tracker()["slow_operations"].append({
...
def api_request_time(endpoint: str):
    """API请求时间装饰器"""
    # 性能阈值常量
    SLOW_REQUEST_THRESHOLD = 2.0  # 超过2秒认为是慢请求
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                _metrics_store.get_performance_tracker()["error_patterns"][error_pattern] += 1
                raise
                ...
                _metrics_store.get_metrics_store()["api_requests"][key].append({
                    "duration": duration,
                    "status": status,
                    "timestamp": datetime.now().isoformat()
                })
...
def record_maze_operation(operation: str, maze_type: str, _difficulty: int):
    """记录迷宫操作"""
    ...
    _metrics_store.get_metrics_store()["maze_operations"][key] += 1
...
def record_maze_error(operation: str, error_type: str):
    ...
    _metrics_store.get_metrics_store()["errors"][key] += 1
...
def record_cache_operation(operation: str, backend: str, status: str = 'success'):
    ...
    _metrics_store.get_metrics_store()["cache_operations"][key] += 1
...
def record_db_operation(operation: str, table: str, status: str = 'success'):
    ...
    _metrics_store.get_metrics_store()["db_operations"][key] += 1
...
def update_active_mazes_count(maze_type: str, count: int):
    ...
    # 内存指标
    _metrics_store.get_metrics_store()["system_stats"][f"active_mazes_{maze_type}"] = count
...
def update_active_users_count(count: int):
    ...
    # 内存指标
    _metrics_store.get_metrics_store()["system_stats"]["active_users"] = count
...
def update_cache_hit_rate(cache_type: str, hit_rate: float):
    ...
    # 内存指标
    _metrics_store.get_metrics_store()["system_stats"][f"cache_hit_rate_{cache_type}"] = hit_rate
...
def update_memory_usage(component: str, bytes_used: int):
    ...
    # 内存指标
    _metrics_store.get_metrics_store()["system_stats"][f"memory_usage_{component}"] = bytes_used

def update_concurrent_connections(count: int):
    """更新并发连接数"""
    concurrent_connections.set(count)
    _metrics_store.get_metrics_store()["system_stats"]["concurrent_connections"] = count

def update_queue_length(queue_type: str, length: int):
    """更新队列长度"""
    queue_length.labels(queue_type=queue_type).set(length)
    _metrics_store.get_metrics_store()["system_stats"][f"queue_length_{queue_type}"] = length
...
def get_metrics_summary() -> dict[str, Any]:
    ...
    summary = {
        ...
        "metrics_store": _metrics_store.get_metrics_store(),
        "performance_tracker": _metrics_store.get_performance_tracker()
    }

    # 计算生成时间统计
    if _metrics_store.get_metrics_store()["generation_times"]:
        successful_times = [item["duration"] for item in _metrics_store.get_metrics_store()["generation_times"] if item.get("success", True)]
        if successful_times:
    ...
    # 计算错误率
    total_operations = sum(_metrics_store.get_metrics_store()["maze_operations"].values())
    total_errors = sum(_metrics_store.get_metrics_store()["errors"].values())
    ...
    # 计算API性能统计
    api_stats = {}
    for endpoint, requests in _metrics_store.get_metrics_store()["api_requests"].items():
        if requests:
    ...
    # 慢操作统计
    summary["slow_operations_count"] = len(_metrics_store.get_performance_tracker()["slow_operations"])
    summary["top_error_patterns"] = dict(sorted(
        _metrics_store.get_performance_tracker()["error_patterns"].items(),
        key=lambda x: x[1],
        reverse=True
    )[:10])
...
def monitor_performance(component: str):
    """性能监控装饰器"""
    # 性能数据保留常量
    MAX_PERFORMANCE_RECORDS = 1000  # 保持最近1000条记录
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                _metrics_store.get_metrics_store()["performance_data"][component].append(perf_data)

                # 保持最近1000条记录
                if len(_metrics_store.get_metrics_store()["performance_data"][component]) > MAX_PERFORMANCE_RECORDS:
                    _metrics_store.get_metrics_store()["performance_data"][component] = _metrics_store.get_metrics_store()["performance_data"][component][-MAX_PERFORMANCE_RECORDS:]

                # 记录资源使用
                _metrics_store.get_performance_tracker()["resource_usage"].append({
                    "timestamp": datetime.now().isoformat(),
                    "memory_usage": end_memory,
                    "active_operations": len(_metrics_store.get_metrics_store()["performance_data"])
                })
...
def record_health_check(component: str, status: str, duration: float):
    ...
    _metrics_store.get_metrics_store()["system_stats"][key] = {
        "duration": duration,
        "timestamp": datetime.now().isoformat()
    }

# 业务指标
def record_user_action(action: str, _user_id: str, maze_type: str | None = None):
    """记录用户行为"""
