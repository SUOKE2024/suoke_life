...
async def test_maze_generator():
    ...
    try:
        ...
        logger.error(f"❌ 迷宫生成器测试失败: {str(e)}")
        raise
...
async def test_metrics():
    ...
    try:
        from pkg.utils.metrics import (
            record_maze_operation, record_maze_error, 
            update_active_mazes_count, get_metrics_summary,
            maze_generation_time, api_request_time,
            record_cache_operation, update_memory_usage
        )
