    ...
    if reports["optimization_execution"]:
        try:
            with open(reports["optimization_execution"], 'r', encoding='utf-8') as f:
                ...
                summary["execution_duration"] = opt_data.get("execution_duration_seconds", 0)
                summary["detailed_results"] = opt_data.get("summary", {})
        except Exception as e:
            print(f"  ⚠️ 读取优化执行报告失败: {e}")
    
    # 保存总结报告
    summary_file = f"OPTIMIZATION_SUMMARY_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(summary_file, 'w', encoding='utf-8') as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    
    print(f"\n📊 优化执行总结:")
    print(f"  执行状态: ✅ {summary['optimization_status']}")
    print(f"  短期优化: ✅ {summary['metrics']['short_term_optimizations_completed']}/4 完成")
    print(f"  中期规划: 📋 {summary['metrics']['medium_term_optimizations_planned']}/4 已制定")
