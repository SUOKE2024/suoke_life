...
class IntelligentRedundancyCleaner:
    ...
    def scan_services(self) -> Dict:
        ...
        for root, dirs, files in os.walk(self.services_root):
            # 跳过虚拟环境和缓存目录
            dirs[:] = [d for d in dirs if not d.startswith(('.venv', '__pycache__', '.pytest_cache', '.mypy_cache', '.ruff_cache', 'htmlcov', '.benchmarks'))]
    ...
    def _is_backup_file(self, filename: str) -> bool:
        """检查是否为备份文件"""
        backup_patterns = [
            '.backup', '.backup_advanced', '.backup_priority',
            '.bak', '.orig', '.old', '.tmp'
        ]
    ...
    def _is_placeholder_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            return any(pattern in content for pattern in placeholder_patterns) and len(content) < 200
    ...
    def _is_cache_file(self, filename: str) -> bool:
        """检查是否为缓存文件"""
        cache_patterns = [
            '.coverage', 'coverage.xml', '.pytest_cache',
    ...
    def generate_cleanup_report(self) -> str:
        ...
        for category, files in self.redundant_files.items():
            if files:
                report.append(f"### {self._get_category_name(category)} ({len(files)}个)")
