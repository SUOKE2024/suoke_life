...
class PerformanceMonitor:
        ...
        while self.monitoring_active:
            try:
                ...
                if len(self.metrics_data["system_metrics"]) > 100:
                    self.metrics_data["system_metrics"] = self.metrics_data[
                        "system_metrics"
                    ][-100:]
        ...
        while self.monitoring_active:
            try:
                ...
                if len(self.metrics_data["service_metrics"]) > 500:
                    self.metrics_data["service_metrics"] = self.metrics_data[
                        "service_metrics"
                    ][-500:]
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        recent_alerts = [
            a
            for a in self.metrics_data["alerts"]
...
async def main():
    ...
    try:
        ...
        for i in range(12):  # 运行2分钟
            ...
            print(
                f"📈 监控状态 - CPU: {summary['system_status']['cpu_usage']:.1f}%, "
        ...
        print(f"\n📊 性能摘要:")
        print(
            f"  系统状态: CPU {summary['system_status']['cpu_usage']:.1f}%, "
            f"内存 {summary['system_status']['memory_usage']:.1f}%"
        )
