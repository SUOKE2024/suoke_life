...
class TestScientificComputingManager:
    ...
    def test_manager_initialization(self):
        ...
        # 检查numpy是否可用（应该总是可用）
        assert manager.is_library_available('numpy') == True
    ...
    def test_library_availability_check(self):
        ...
        # 测试不存在的库
        assert manager.is_library_available('nonexistent_library') == False
...
class TestSignalProcessingEngine:
    ...
    def test_analyze_frequency_spectrum(self):
        """测试频谱分析"""
        result = self.engine.analyze_frequency_spectrum(self.test_signal, sample_rate=1000)
...
class TestScientificComputingService:
    ...
    def test_process_data_analyze(self):
        """测试数据分析处理"""
    ...
    def test_process_data_detect_anomalies(self):
        """测试异常检测处理"""
        result = self.service.process_data(self.test_data, 'detect_anomalies', threshold=2.0)
        
        assert result['status'] == 'success'
        assert 'results' in result
    
    def test_process_data_filter(self):
        """测试信号滤波处理"""
        result = self.service.process_data(self.test_data, 'filter', filter_type='lowpass', cutoff=0.1)
        
        assert result['status'] == 'success'
        assert 'filtered_signal' in result
    
    def test_process_data_spectrum(self):
        """测试频谱分析处理"""
...
class TestAdvancedFeatures:
    ...
    def test_complex_signal_processing(self):
        ...
        filter_result = self.service.process_data(
            self.complex_signal, 'filter',
            filter_type='lowpass', cutoff=0.1
        )
        assert filter_result['status'] == 'success'
        
        # 频谱分析
        spectrum_result = self.service.process_data(
            self.complex_signal, 'spectrum',
            sample_rate=100
        )
...
class TestIntegrationWithAccessibilityService:
    ...
    def test_accessibility_feature_support(self):
        """测试无障碍功能支持"""
        # 模拟盲人辅助中的图像特征数据
...
class TestComprehensiveScientificLibraries:
    ...
    def test_all_library_categories(self):
        """测试所有库类别"""
        ...
        for lib in ml_libs:
            if lib in available_libs:
                print(f"🤖 机器学习库 {lib}: {'可用' if available_libs[lib] else '不可用'}")
        
        # 计算机视觉库
        cv_libs = ['cv2', 'PIL', 'skimage', 'mediapipe']
        for lib in cv_libs:
            if lib in available_libs:
                print(f"👁️ 计算机视觉库 {lib}: {'可用' if available_libs[lib] else '不可用'}")
        
        # 音频处理库
        audio_libs = ['librosa', 'sounddevice', 'pydub']
        for lib in audio_libs:
            if lib in available_libs:
                print(f"🔊 音频处理库 {lib}: {'可用' if available_libs[lib] else '不可用'}")
        ...
        # 至少应该有基础的numpy可用
        assert available_libs.get('numpy', False) == True
