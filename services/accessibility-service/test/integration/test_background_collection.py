...
class TestBackgroundCollectionIntegration(unittest.TestCase):
    ...
    def test_configure_background_collection(self):
        """测试配置后台收集"""
        # 注册用户同意数据采集
        result = self.collection_service.register_user_consent(
            TEST_USER_ID,
            ["pulse", "sleep", "activity"],
            expiry_days=30
        )
        ...
        interval_result = self.collection_service.set_collection_interval(TEST_USER_ID, intervals)
        ...
        self.assertEqual(set(consent_status["data_types"]), {"pulse", "sleep", "activity"})
    ...
    def test_get_collection_status(self):
        ...
        self.assertEqual(set(status["collecting_types"]), {"pulse", "sleep", "activity"})

    def test_submit_collected_data(self):
        """测试提交收集的数据"""
        # 先注册用户同意
        self.collection_service.register_user_consent(
            TEST_USER_ID,
        ...
        # 获取最近的数据
        recent_pulse = self.collection_service.get_recent_data(TEST_USER_ID, "pulse", limit=1)
        recent_sleep = self.collection_service.get_recent_data(TEST_USER_ID, "sleep", limit=1)
    ...
    def test_e2e_background_collection_flow(self):
        """端到端测试后台数据收集流程"""
        # 步骤1: 注册用户同意
        ...
        for data_type in test_data.keys():
            recent_data = self.collection_service.get_recent_data(TEST_USER_ID, data_type, limit=1)
    ...
    def test_battery_optimization(self):
        """测试电池优化功能"""
        # 注册用户同意
        self.collection_service.register_user_consent(TEST_USER_ID, ["pulse", "activity"])
