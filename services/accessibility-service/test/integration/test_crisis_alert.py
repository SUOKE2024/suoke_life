...
class TestCrisisAlertIntegration(unittest.TestCase):
    ...
    def setup_test_user(self):
        ...
        # 设置紧急联系人
        self.crisis_service.set_emergency_contacts("test_user", [
            {
                "name": "紧急联系人",
                "relation": "家人",
                "phone": "13800138000",
    ...
    def test_service_integration(self):
        ...
        # 验证服务集成状态
        self.assertEqual(self.background_service.crisis_alert_service, self.crisis_service)
        self.assertEqual(self.crisis_service.background_collection_service, self.background_service)
    ...
    def test_danger_level_alert(self):
        """测试危险级别警报"""
        # 采集非常异常的脉搏数据
    ...
    def test_critical_alert_emergency_contacts(self):
        """测试严重警报时通知紧急联系人"""
    ...
    def test_battery_optimization(self):
        ...
        if hasattr(self.background_service, "_collection_worker"):
            ...
            with patch('time.sleep', side_effect=mock_time_sleep):
                ...
                # 验证电池优化 - 睡眠时间应该增加
                if self.sleep_times:
                    self.assertGreater(max(self.sleep_times), 10)  # 低电量时睡眠时间应大于默认的10秒
    
    def test_long_term_data_trend_alert(self):
        """测试长期数据趋势警报"""
        # 模拟一周的睡眠数据，逐渐变差
        for day in range(7):
            hours = 8.0 - (day * 0.5)  # 从8小时逐渐减少到5小时
            sleep_data = {"duration_hours": hours, "quality": "deteriorating", "day": day}
        ...
        # 验证警报生成
        alerts = [a for a in self.crisis_service.alert_history.get("test_user", []) 
