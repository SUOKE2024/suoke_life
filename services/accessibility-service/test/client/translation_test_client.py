...
class TranslationTestClient:
    ...
    async def test_speech_translation(
        self,
        audio_file: str,
        user_id: str,
        source_language: str,
        ...
        try:
            ...
            preferences = pb2.UserPreferences(
                voice_type="female", speech_rate=1.0, language=target_language
            )
        ...
        try:
            ...
            async def request_generator() -> AsyncIterator[pb2.SpeechTranslationChunk]:
                for i, chunk in enumerate(chunks):
                    ...
                    yield request
                    logger.info(
                        f"已发送块 {i+1}/{total_chunks}, 大小: {len(chunk)}字节"
                    )
            ...
            async for response in responses:
                ...
                if response.translated_audio:
                    output_file = (
                        f"translated_stream_{session_id}_{response.segment_id}.wav"
                    )
                    with open(output_file, "wb") as f:
                        f.write(response.translated_audio)
                    logger.info(f"  翻译音频已保存到: {output_file}")
            ...
            elapsed_time = time.time() - start_time
            logger.info(
                f"流式翻译请求完成，总耗时: {elapsed_time:.2f}秒，收到 {segment_count} 个结果"
            )
    ...
    async def test_supported_languages(
        self, user_id: str, include_dialects: bool = True
    ) -> None:
        """测试获取支持的语言API
        ...
        try:
            ...
            request = pb2.SupportedLanguagesRequest(
                user_id=user_id, include_dialects=include_dialects
            )
            ...
            for i, lang in enumerate(response.languages):
                logger.info(
                    f"  语言 #{i+1}: {lang.name} ({lang.code}), 支持语音: {lang.supports_speech}"
                )

            logger.info(f"支持的语言对数量: {len(response.language_pairs)}")
            for i, pair in enumerate(response.language_pairs[:5]):  # 只显示前5个
                logger.info(
                    f"  语言对 #{i+1}: {pair.source_name} → {pair.target_name}, 支持语音: {pair.supports_speech}"
                )
...
async def main():
    ...
    parser.add_argument(
        "--test",
        choices=["simple", "stream", "languages", "all"],
        default="all",
        help="测试类型",
    )
