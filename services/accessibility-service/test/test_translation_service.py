...
class TestTranslationService(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        with (
            patch(
                "transformers.MBartTokenizer.from_pretrained",
                return_value=self.mock_tokenizer,
            ),
            patch(
                "transformers.MBartForConditionalGeneration.from_pretrained",
        ...
        self.service.translation_models = {
            "general": {"tokenizer": self.mock_tokenizer, "model": self.mock_model}
        }
    ...
    def test_get_supported_language_pairs(self) -> None:
        """测试获取支持的语言对"""
    ...
    def test_translate_speech(self) -> None:
        ...
        # 模拟翻译文本的方法
        with patch.object(
            self.service, "translate_text", return_value="This is a test translation"
        ):
    ...
    def test_create_streaming_session(self) -> None:
        """测试创建流式翻译会话"""
        user_id = "test_user"
        source_lang = "zh_CN"
        target_lang = "en_XX"

        session_id = self.service.create_streaming_session(
        ...
        self.assertEqual(
            self.service.active_sessions[session_id]["source_language"], source_lang
        )
        self.assertEqual(
            self.service.active_sessions[session_id]["target_language"], target_lang
        )

    def test_process_streaming_chunk(self) -> None:
        """测试处理流式翻译数据块"""
        # 先创建一个会话
        user_id = "test_user"
        source_lang = "zh_CN"
        ...
        with patch.object(
            self.service, "_process_audio_buffer", return_value=mock_result
        ):
            # 处理一个最终的数据块
            result = self.service.process_streaming_chunk(
                session_id, b"audio_data", is_final=True
            )
            ...
            self.assertEqual(
                result["translated_audio"], mock_result["translated_audio"]
            )
