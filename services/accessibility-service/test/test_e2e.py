...
class TestCompleteUserScenarios:
    ...
    async def e2e_coordinator(self):
        ...
        container.register('model_manager', type(mock_model_manager), mock_model_manager)
        container.register('cache_manager', type(mock_cache_manager), mock_cache_manager)
        container.register('config_manager', type(mock_config_manager), mock_config_manager)
        ...
        from internal.service.implementations.sign_language_impl import SignLanguageServiceImpl
        from internal.service.implementations.screen_reading_impl import ScreenReadingServiceImpl
        from internal.service.implementations.content_conversion_impl import ContentConversionServiceImpl
        
        # 创建mock服务实例
        mock_blind_service = Mock(spec=BlindAssistanceServiceImpl)
        mock_blind_service.analyze_environment = AsyncMock(return_value={"obstacles": [], "safe_path": True})
        mock_blind_service.provide_navigation = AsyncMock(return_value={"direction": "forward", "distance": 10})
        
        mock_voice_service = Mock(spec=VoiceAssistanceServiceImpl)
        mock_voice_service.process_voice_command = AsyncMock(return_value={"command": "navigate", "confidence": 0.9})
        
        mock_sign_service = Mock(spec=SignLanguageServiceImpl)
        mock_sign_service.recognize_sign_language = AsyncMock(return_value={"language": "CSL", "semantic": {"sentence": "你好"}, "recognition": {"confidence": 0.9}})
        
        mock_screen_service = Mock(spec=ScreenReadingServiceImpl)
        mock_screen_service.read_screen = AsyncMock(return_value={"ui_elements": [], "reading_content": {"summary": "测试页面"}})
        
        mock_content_service = Mock(spec=ContentConversionServiceImpl)
        mock_content_service.convert_content = AsyncMock(return_value={"converted_content": {"adapted_content": "简化内容"}})
        mock_content_service.simplify_text = AsyncMock(return_value="简化文本")
        
        # 注册服务
        container.register('blind_assistance', BlindAssistanceServiceImpl, factory=lambda: mock_blind_service)
        container.register('voice_assistance', VoiceAssistanceServiceImpl, factory=lambda: mock_voice_service)
        container.register('sign_language', SignLanguageServiceImpl, factory=lambda: mock_sign_service)
        container.register('screen_reading', ScreenReadingServiceImpl, factory=lambda: mock_screen_service)
        container.register('content_conversion', ContentConversionServiceImpl, factory=lambda: mock_content_service)
    ...
    async def test_blind_user_navigation_scenario(self, e2e_coordinator):
        ...
        voice_result = await e2e_coordinator._voice_assistance_service.process_voice_command(
    ...
    async def test_deaf_user_communication_scenario(self, e2e_coordinator):
        ...
        sign_result = await e2e_coordinator._sign_language_service.recognize_sign_language(
        ...
        # 将识别的手语转换为简化文本
        simplified_text = await e2e_coordinator._content_conversion_service.simplify_text(
    ...
    async def test_elderly_user_assistance_scenario(self, e2e_coordinator):
        ...
        # 为老年人适配内容
        adapted_content = await e2e_coordinator._content_conversion_service.convert_content(
            complex_text,
            "content_adaptation",
            {
    ...
    async def test_multilingual_user_scenario(self, e2e_coordinator):
        ...
        mixed_content = "欢迎使用Suoke Life健康管理平台，我们提供AI-powered的个性化服务。"
        
        # 翻译为英文
        translation_result = await e2e_coordinator._content_conversion_service.convert_content(
            mixed_content,
        ...
        for lang in sign_languages:
            supported_languages = await e2e_coordinator._sign_language_service.get_supported_languages()
...
class TestAPICompatibility:
    ...
    async def legacy_service(self):
        ...
        # 模拟原有API方法
        mock_service.analyze_scene = AsyncMock(return_value={
            'scene_type': 'outdoor',
            'obstacles': [],
            'confidence': 0.85
        })
        
        mock_service.process_voice_command = AsyncMock(return_value={
    ...
    async def test_response_format_consistency(self, new_coordinator):
        ...
        # 场景分析API
        scene_result = await new_coordinator.analyze_scene(
            b"test_data", "user", {}, {}
        )
        ...
        # 语音处理API
        voice_result = await new_coordinator._voice_assistance_service.process_voice_command(
        ...
        # 手语识别API
        sign_result = await new_coordinator._sign_language_service.recognize_sign_language(
    ...
    async def test_error_handling_consistency(self, new_coordinator):
        ...
        with pytest.raises(Exception) as exc_info:
            await new_coordinator.analyze_scene(
                None, "user", {}, {}  # 无效的图像数据
            )
        ...
        # 3. 服务未初始化
        uninitialized_service = await new_coordinator._factory.create_blind_assistance_service()
        # 不调用initialize()
        
        with pytest.raises(Exception) as exc_info:
            await uninitialized_service.analyze_scene(
                b"test_data", "user", {}, {}
            )
