...
from internal.service.interfaces.haptic_feedback_interface import HapticDeviceType, HapticModality, HapticPattern
...
class TestBCIService:
    ...
    async def test_detect_bci_devices(self, bci_service):
        ...
        assert any(device["device_type"] == BCIDeviceType.EEG.value for device in devices)
        assert any(device["device_type"] == BCIDeviceType.FNIRS.value for device in devices)
    ...
    async def test_calibrate_bci_device(self, bci_service):
        ...
        result = await bci_service.calibrate_bci_device(user_id, device_id, calibration_config)
    ...
    async def test_start_signal_acquisition(self, bci_service):
        ...
        result = await bci_service.start_signal_acquisition(user_id, device_id, acquisition_config)
    ...
    async def test_process_brain_signals(self, bci_service):
        ...
        result = await bci_service.process_brain_signals(user_id, signal_data, processing_config)
    ...
    async def test_classify_mental_state(self, bci_service):
        ...
        result = await bci_service.classify_mental_state(user_id, signal_features, classification_config)
    ...
    async def test_detect_bci_commands(self, bci_service):
        ...
        result = await bci_service.detect_bci_commands(user_id, signal_data, detection_config)
...
class TestHapticFeedbackService:
    ...
    async def test_calibrate_haptic_device(self, haptic_service):
        ...
        # 先连接设备
        device_id = "haptic_gloves_001"
        await haptic_service.connect_haptic_device(device_id, HapticDeviceType.HAPTIC_GLOVES)
        ...
        result = await haptic_service.calibrate_haptic_device(user_id, device_id, calibration_config)
    ...
    async def test_create_haptic_pattern(self, haptic_service):
        ...
        result = await haptic_service.create_haptic_pattern(pattern_name, pattern_config)
    ...
    async def test_play_haptic_pattern(self, haptic_service):
        ...
        # 播放模式
        result = await haptic_service.play_haptic_pattern(user_id, device_id, pattern_name)
    ...
    async def test_create_spatial_haptic_map(self, haptic_service):
        ...
        result = await haptic_service.create_spatial_haptic_map(user_id, device_id, spatial_config)
    ...
    async def test_render_spatial_haptics(self, haptic_service):
        ...
        # 连接设备
        await haptic_service.connect_haptic_device(device_id, HapticDeviceType.HAPTIC_VEST)
        ...
        result = await haptic_service.render_spatial_haptics(user_id, device_id, spatial_data)
    ...
    async def test_create_haptic_language(self, haptic_service):
        ...
        result = await haptic_service.create_haptic_language(language_name, language_config)
    ...
    async def test_encode_message_to_haptic(self, haptic_service):
        ...
        # 先创建触觉语言
        language_name = "test_morse"
        await haptic_service.create_haptic_language(language_name, {
            "alphabet": {
        ...
        result = await haptic_service.encode_message_to_haptic(user_id, message, language_name)
...
class TestSpatialAudioService:
    ...
    async def test_create_spatial_scene(self, audio_service):
        ...
        result = await audio_service.create_spatial_scene(user_id, scene_name, scene_config)
    ...
    async def test_add_audio_source(self, audio_service):
        ...
        result = await audio_service.add_audio_source(user_id, scene_name, source_config)
    ...
    async def test_render_spatial_audio(self, audio_service):
        ...
        # 创建场景并添加音频源
        await audio_service.create_spatial_scene(user_id, scene_name, {})
        await audio_service.add_audio_source(user_id, scene_name, {
            "source_id": "source1",
            "position": [3, 0, 0]
        })
        ...
        result = await audio_service.render_spatial_audio(user_id, scene_name, rendering_config)
    ...
    async def test_simulate_room_acoustics(self, audio_service):
        ...
        result = await audio_service.simulate_room_acoustics(user_id, scene_name, room_config)
...
class TestIntegratedFrontierServices:
    ...
    async def test_multimodal_interaction(self, all_services):
        ...
        # 2. 连接触觉设备提供反馈
        await haptic_service.connect_haptic_device("haptic_001", HapticDeviceType.HAPTIC_GLOVES)
        ...
        bci_result = await bci_service.detect_bci_commands(user_id, signal_data, {
            "command_types": [BCICommand.SELECT.value]
        })
        
        # 5. 基于BCI命令触发触觉反馈
        if bci_result["success"] and bci_result["detected_commands"]:
            haptic_result = await haptic_service.send_haptic_signal(user_id, "haptic_001", {
                "modality": HapticModality.VIBRATION,
                "intensity": 0.7,
                "duration": 0.3
            })
        ...
        # 6. 同时提供空间音频反馈
        audio_result = await audio_service.provide_spatial_guidance(user_id, {
            "current_position": [0, 0, 0],
    ...
    async def test_accessibility_enhancement(self, all_services):
        ...
        # 为听力障碍用户创建触觉语言
        language_result = await haptic_service.create_haptic_language("braille_haptic", {
            "alphabet": {
                "A": [[1, 0], [0, 0], [0, 0]],
                "B": [[1, 0], [1, 0], [0, 0]]
            }
        ...
        # 为运动障碍用户提供BCI控制
        bci_connect_result = await bci_service.connect_bci_device("eeg_002", BCIDeviceType.EEG)
