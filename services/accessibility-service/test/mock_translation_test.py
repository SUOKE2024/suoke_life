...
class MockConfig:
    ...
    def __init__(self) -> None:
        self.translation = type(
            "obj", (object,), {"model_name": "mocked_model", "special_pairs": {}}
        )
...
class MockTranslationService:
    ...
    def translate_text(self, text, source_language, target_language):
        """模拟文本翻译功能"""
        translations = {
            "zh_CN-en_XX": {"你好": "Hello", "我需要帮助": "I need help"},
            "en_XX-zh_CN": {"Hello": "你好", "I need help": "我需要帮助"},
        }
    ...
    def get_supported_language_pairs(self) -> None:
        """模拟获取支持的语言对"""
        language_pairs = []
...
class TestMockTranslationService(unittest.TestCase):
    ...
    def test_streaming_session(self) -> None:
        """测试流式翻译会话"""
        # 创建会话
        session_id = self.service.create_streaming_session(
            "test_user", "zh_CN", "en_XX"
        )
        ...
        result = self.service.process_streaming_chunk(
            session_id, b"audio_chunk", is_final=True
        )
    ...
    def test_supported_language_pairs(self) -> None:
        """测试支持的语言对"""
        ...
        zh_to_en_pair = next(
            (
                p
                for p in pairs
                if p["source_code"] == "zh_CN" and p["target_code"] == "en_XX"
            ),
            None,
        )
