...
def run_integration_tests() -> None:
    """运行集成测试"""
    return run_command(
        "python3 -m pytest test/test_integration.py -v --tb=short", "集成测试"
    )
...
def run_performance_tests() -> None:
    """运行性能测试"""
    return run_command(
        "python3 -m pytest test/test_performance.py -v --tb=short -s", "性能测试"
    )
...
def run_e2e_tests() -> None:
    """运行端到端测试"""
    return run_command(
        "python3 -m pytest test/test_e2e.py -v --tb=short -s", "端到端测试"
    )


def run_all_tests() -> None:
    """运行所有测试"""
    return run_command("python3 -m pytest test/ -v --tb=short", "所有测试")
...
def generate_test_report() -> None:
    ...
    junit_report = run_command(
        "python3 -m pytest test/ --junitxml=test_results.xml", "生成JUnit XML报告"
    )
...
def main() -> None:
    ...
    parser.add_argument(
        "--function", "-fn", help="运行特定测试函数 (需要与 --file 一起使用)"
    )
