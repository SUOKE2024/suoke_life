...
# 导入被测试模块
from internal.platform.battery_bridge import (
    BatteryBridge,
    get_battery_info,
    get_battery_level,
    get_power_mode,
...
class TestBatteryBridge(unittest.TestCase):
    ...
    def test_battery_info_caching(self) -> None:
        """测试电池信息缓存机制"""
        # 模拟电池信息获取方法
        ...
        with patch.object(
            self.battery_bridge, platform_method, return_value=mock_battery_info
        ):
            # 首次调用应该触发平台特定方法
            ...
            with patch.object(
                self.battery_bridge, platform_method, return_value=new_mock_info
            ):
                # 由于缓存，应该返回旧值
    ...
    def test_battery_level_method(self) -> None:
        """测试获取电池电量方法"""
        # 模拟电池信息
        mock_battery_info = {"level": 60, "is_charging": True, "power_mode": "normal"}

        with patch.object(
            self.battery_bridge, "get_battery_info", return_value=mock_battery_info
        ):
    ...
    def test_is_charging_method(self) -> None:
        """测试获取充电状态方法"""
        # 模拟电池信息
    ...
    def test_get_power_mode_method(self) -> None:
        """测试获取电源模式方法"""
        # 模拟电池信息
        mock_battery_info = {
            "level": 30,
    ...
    def test_error_handling(self) -> None:
        ...
        with patch.object(
            self.battery_bridge, platform_method, side_effect=Exception(error_message)
        ):
            # 获取电池信息应该返回默认值
    ...
    def test_linux_battery_detection(self) -> None:
        """测试Linux平台电池检测"""
        # 模拟upower命令输出
        upower_output = """
        native-path:          BAT0
        ...
        with patch("subprocess.check_output", return_value=upower_output):
            with patch.object(
                self.battery_bridge, "_is_command_available", return_value=True
            ):
        ...
        with patch("os.path.exists", return_value=True):
            with patch("builtins.open", unittest.mock.mock_open(read_data="85")):
                with patch.object(
                    self.battery_bridge, "_is_command_available", return_value=False
                ):
                    info = self.battery_bridge._get_battery_linux()
                    self.assertEqual(info["level"], 85)

    @unittest.skipIf(platform.system() != "Darwin", "仅在macOS平台上运行")
    def test_macos_battery_detection(self) -> None:
        """测试macOS平台电池检测"""
        # 模拟pmset命令输出
        pmset_output = """
    ...
    def test_windows_battery_detection(self) -> None:
        """测试Windows平台电池检测"""
        # 模拟WMIC命令输出
        wmic_output = """
        BatteryStatus=2
    ...
    def test_android_battery_detection(self) -> None:
        """测试Android平台电池检测"""
        # 测试shell方法
    ...
    def test_global_functions(self) -> None:
        ...
        # 替换单例实例
        with patch(
            "internal.platform.battery_bridge.battery_bridge", mock_battery_bridge
        ):
            # 测试全局函数
