...
class TestPerformanceBenchmarks:
    ...
    async def test_concurrent_request_performance(self, performance_coordinator):
        ...
        for user_count in concurrent_users:
            ...
            print(
                f"  平均响应时间: {performance_results[user_count]['avg_response_time']:.2f}ms"
            )
            print(
                f"  内存使用: {performance_results[user_count]['memory_usage_mb']:.2f}MB"
            )
        ...
        for user_count, metrics in performance_results.items():
            ...
            ), f"{user_count}用户时成功率过低: {metrics['success_rate']:.2f}%"

            # QPS应该 > 50（根据实际情况调整）
            if user_count >= 50:
                assert (
                    metrics["qps"] > 50
                ), f"{user_count}用户时QPS过低: {metrics['qps']:.2f}"

            # 平均响应时间应该 < 500ms
            assert (
                metrics["avg_response_time"] < 500
            ), f"{user_count}用户时响应时间过长: {metrics['avg_response_time']:.2f}ms"
    ...
    async def test_memory_usage_under_load(self, performance_coordinator):
        ...
        for i in range(0, len(tasks), batch_size):
            ...
            print(
                f"批次 {i//batch_size + 1}: 内存使用 {current_memory:.2f}MB (+{memory_increase:.2f}MB)"
            )
    ...
    async def test_service_startup_performance(self):
        ...
        for service_name, service_class in services_to_test:
            ...
            assert (
                startup_time < 5000
            ), f"{service_name} 启动时间过长: {startup_time:.2f}ms"
...
class TestCachePerformance:
    ...
    async def test_cache_hit_ratio(self, cache_test_service):
        ...
        hit_ratio = (
            cache_manager.cache_hits / total_requests if total_requests > 0 else 0
        )
    ...
    async def test_cache_performance_impact(self, cache_test_service):
        ...
        cache_manager.get = AsyncMock(
            return_value={"cached": True, "data": "result"}
        )  # 强制缓存命中
        ...
        ), f"缓存性能提升不足: {performance_improvement:.2f}%"
...
class TestScalabilityPerformance:
    ...
    async def test_horizontal_scaling_simulation(self):
        ...
        for instance_count in instance_counts:
            ...
            print(f"  QPS: {qps:.2f}")
            print(
                f"  每实例效率: {performance_metrics[instance_count]['efficiency']:.2f} QPS/实例"
            )
        ...
        for i in range(1, len(instance_counts)):
            ...
            # QPS增长应该是正向的
            assert (
                current_qps > previous_qps
            ), f"QPS没有随实例数增加: {previous_qps} -> {current_qps}"
    ...
    async def test_resource_utilization_under_load(self):
        ...
        for load_level in load_levels:
            ...
            if cpu_samples and memory_samples:
                ...
                assert (
                    max_memory - baseline_memory < 500
                ), f"内存增长过多: {max_memory - baseline_memory:.2f}MB"
