...
class ScientificLibraryInstaller:
    ...
    def __init__(self) -> None:
        ...
        # 信号处理库
        self.signal_libraries = ["filterpy>=1.4.5", "pywavelets>=1.4.1"]
        ...
        # 性能优化库
        self.performance_libraries = ["numba>=0.58.0", "joblib>=1.3.0"]

        # 数据存储库
        self.storage_libraries = ["h5py>=3.10.0"]

        # 开发工具
        self.dev_libraries = ["jupyter>=1.0.0", "ipython>=8.17.0", "notebook>=7.0.0"]
    ...
    def install_libraries(self, library_groups: list[str]) -> dict[str, bool]:
        ...
        for group in library_groups:
            if group == "core":
                results.update(
                    self._install_library_group("核心科学计算库", self.core_libraries)
                )
            elif group == "ml":
                results.update(
                    self._install_library_group("机器学习库", self.ml_libraries)
                )
            elif group == "cv":
                results.update(
                    self._install_library_group("计算机视觉库", self.cv_libraries)
                )
            elif group == "audio":
                results.update(
                    self._install_library_group("音频处理库", self.audio_libraries)
                )
            elif group == "signal":
                results.update(
                    self._install_library_group("信号处理库", self.signal_libraries)
                )
            elif group == "geo":
                results.update(
                    self._install_library_group("地理信息库", self.geo_libraries)
                )
            elif group == "performance":
                results.update(
                    self._install_library_group(
                        "性能优化库", self.performance_libraries
                    )
                )
            elif group == "storage":
                results.update(
                    self._install_library_group("数据存储库", self.storage_libraries)
                )
            elif group == "dev":
                results.update(
                    self._install_library_group("开发工具", self.dev_libraries)
                )
            else:
    ...
    def _install_library_group(
        self, group_name: str, libraries: list[str]
    ) -> dict[str, bool]:
        """安装一组库"""
...
def main() -> None:
    ...
    parser.add_argument("--core-only", action="store_true", help="只安装核心库")
    parser.add_argument(
        "--no-system-deps", action="store_true", help="跳过系统依赖安装"
    )
    parser.add_argument(
        "--groups",
        nargs="+",
