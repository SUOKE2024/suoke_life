...
class APIDocGenerator:
    ...
    def generate_openapi_spec(self) -> None:
        """生成OpenAPI 3.0规范"""
        spec = {
            "openapi": "3.0.3",
            "info": {
            ...
            "servers": [
                {"url": self.service_info["base_url"], "description": "生产环境"},
                {
        ...
        # 保存OpenAPI规范
        with open(
            os.path.join(self.output_dir, "openapi.yaml"), "w", encoding="utf-8"
        ) as f:
            yaml.dump(spec, f, default_flow_style=False, allow_unicode=True)

        with open(
            os.path.join(self.output_dir, "openapi.json"), "w", encoding="utf-8"
        ) as f:
    ...
    def generate_markdown_docs(self) -> None:
        ...
        for service_id, service_name in services:
            ...
            with open(
                os.path.join(self.output_dir, f"{service_id}.md"), "w", encoding="utf-8"
            ) as f:
                f.write(doc)
...
"""
    ...
    def generate_postman_collection(self) -> None:
        """生成Postman集合"""
        collection = {
            "info": {
        ...
        with open(
            os.path.join(self.output_dir, "postman_collection.json"),
            "w",
            encoding="utf-8",
        ) as f:
    ...
    def generate_sdk_examples(self) -> None:
        """生成SDK示例"""
        # 创建SDK目录
        ...
        with open(
            os.path.join(sdk_dir, "python_example.py"), "w", encoding="utf-8"
        ) as f:
        ...
        with open(
            os.path.join(sdk_dir, "javascript_example.js"), "w", encoding="utf-8"
        ) as f:
            f.write(js_example)
