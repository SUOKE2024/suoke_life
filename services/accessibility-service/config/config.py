...
class Config:
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            logger.warning(f"无法加载配置文件: {e!s}，将使用默认配置")
    ...
    def _init_section_properties(self):
        ...
        # 设置版本属性
        self._version = self.config_data.get("version") or self._service.get(
            "version", "0.2.0"
        )

        # 新增服务配置节
        self._edge_computing = ConfigSection(self.config_data.get("edge_computing", {}))
        self._background_collection = ConfigSection(
            self.config_data.get("background_collection", {})
        )
        self._agent_coordination = ConfigSection(
            self.config_data.get("agent_coordination", {})
        )
    ...
    def _set_default_config(self) -> None:
        ...
        self.config_data = {
            ...
            "features": {
                ...
                "screen_reading": {"enabled": True, "element_detection_threshold": 0.6},
                "voice_assistance": {
                    "enabled": True,
                    "supported_dialects": [
                        "mandarin",
            ...
            "background_collection": {
                "enabled": False,
                "collection_types": {},
                "consent": {"require_explicit": True, "default_expiry_days": 365},
                "battery_optimization": {"low_power_mode": {"enabled": True}},
            },
            "agent_coordination": {"event_bus": {"type": "memory"}},
            "observability": {
                "metrics": {"provider": "memory", "interval_seconds": 60},
                "tracing": {
                    "provider": "memory",
                    "service_name": "accessibility-service",
                },
                "logging": {"provider": "stdout", "structured": True},
            },
            "security": {
                "encryption": {"data_at_rest": {"algorithm": "AES-256-GCM"}},
                "access_control": {
                    "authorization": {"rbac_enabled": False, "default_deny": False}
                },
            },
            "resilience": {"backup": {"enabled": False}},
        }
