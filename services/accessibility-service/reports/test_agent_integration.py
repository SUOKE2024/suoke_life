...
class AgentAccessibilityIntegrationTest:
    ...
    async def test_xiaoai_accessibility_features(self) -> Dict[str, Any]:
        ...
        try:
            ...
            diagnosis_result = await self.xiaoai_service.coordinate_four_diagnoses_accessible(
                diagnosis_request, 
            ...
            multimodal_result = await self.xiaoai_service.process_multimodal_input_accessible(
                multimodal_request,
                self.config['test_user_id'],
                {'format': 'simplified'}
            )
        ...
        if test_results['tests']:
            success_count = sum(1 for test in test_results['tests'].values() if test['success'])
            test_results['success_rate'] = success_count / len(test_results['tests'])
            test_results['overall_success'] = test_results['success_rate'] > 0.5
        
        logger.info(f"小艾智能体测试完成，成功率: {test_results.get('success_rate', 0):.2%}")
        return test_results
    ...
    async def test_xiaoke_accessibility_features(self) -> Dict[str, Any]:
        ...
        try:
            ...
            resource_result = await self.xiaoke_service.schedule_medical_resources_accessible(
                resource_request,
                self.config['test_user_id'],
                {'format': 'audio'}
            )
            ...
            product_result = await self.xiaoke_service.customize_agricultural_products_accessible(
                product_request,
                self.config['test_user_id'],
                {'format': 'simplified'}
            )
        ...
        logger.info(f"小克智能体测试完成，成功率: {test_results.get('success_rate', 0):.2%}")
        return test_results
    ...
    async def test_laoke_accessibility_features(self) -> Dict[str, Any]:
        ...
        logger.info(f"老克智能体测试完成，成功率: {test_results.get('success_rate', 0):.2%}")
        return test_results
    ...
    async def test_soer_accessibility_features(self) -> Dict[str, Any]:
        ...
        logger.info(f"索儿智能体测试完成，成功率: {test_results.get('success_rate', 0):.2%}")
        return test_results
    ...
    async def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        successful_tests = sum(
            sum(1 for test in result.get('tests', {}).values() if test.get('success', False))
...
def print_test_results(results: Dict[str, Any]):
    ...
    for result in results.get('test_results', []):
        ...
        print(f"{service_name}: {'✅' if success else '❌'} (成功率: {success_rate:.2%})")
