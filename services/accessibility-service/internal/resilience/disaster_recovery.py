...
class BackupScheduler:
        ...
        for job_name, job_config in self.backup_schedule.items():
            ...
            if self._should_run_backup(job_name, job_config, now):
                ...
                try:
                    ...
                    logger.error(f"执行备份任务 {job_name} 失败: {str(e)}")
    ...
    def _should_run_backup(self, job_name: str, job_config: Dict[str, Any], 
        ...
        if frequency == "hourly":
            ...
            is_new_month = (current_time.month > last_run.month or
                         (current_time.month == last_run.month and current_time.year > last_run.year))
            time_passed = (current_time.hour > hour or 
                         (current_time.hour == hour and current_time.minute >= minute))
    ...
    def _run_backup(self, job_name: str, backup_type: str) -> str:
        ...
        # 创建备份ID
        backup_id = f"backup_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}_{job_name}"
        ...
        try:
            ...
            logger.error(f"备份任务 {job_name} 失败: {str(e)}")
    ...
    def _perform_incremental_backup(self, backup_dir: str, metadata: Dict[str, Any]) -> int:
        ...
        if not last_backup:
            logger.warning(f"找不到上次备份，执行完整备份")
    ...
    def _perform_differential_backup(self, backup_dir: str, metadata: Dict[str, Any]) -> int:
        ...
        """
        # 查找上次成功的完整备份
        last_full_backup = self._find_last_backup(metadata["job_name"], backup_type="full")
        if not last_full_backup:
            logger.warning(f"找不到上次完整备份，执行完整备份")
            return self._perform_full_backup(backup_dir, metadata)
        ...
        # 获取上次备份时间
        last_backup_time = datetime.datetime.fromisoformat(last_full_backup["completed_at"])
        last_backup_timestamp = last_backup_time.timestamp()
    ...
    def _copy_directory(self, src_dir: str, dst_dir: str, metadata: Dict[str, Any], 
                      last_modified_after: float = 0) -> int:
        """递归复制目录内容
        ...
        try:
            ...
            for item in os.listdir(src_dir):
                ...
                if os.path.isdir(src_path):
                    file_count += self._copy_directory(src_path, dst_path, metadata, last_modified_after)
                # 处理文件
                else:
                    # 检查文件修改时间
            ...
            logger.error(f"复制目录失败 {src_dir} -> {dst_dir}: {str(e)}")
            return file_count
    ...
    def _find_last_backup(self, job_name: str, backup_type: str = None) -> Dict[str, Any]:
        """查找上次成功的备份
            ...
            for filename in os.listdir(self.backup_path):
                if filename.endswith("_metadata.json"):
                    ...
                    try:
                        ...
                        logger.error(f"读取备份元数据失败 {metadata_path}: {str(e)}")
        except Exception as e:
    ...
    def restore_backup(self, backup_id: str, target_dir: str = None) -> bool:
        ...
        # 对于增量和差异备份，需要先恢复父备份
        if metadata["type"] in ["incremental", "differential"] and "parent_backup" in metadata:
        ...
        for file_info in metadata.get("files", []):
            try:
                ...
                logger.error(f"恢复文件 {rel_path} 失败: {str(e)}")
                success = False
