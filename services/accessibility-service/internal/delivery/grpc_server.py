...
class AccessibilityServicer(pb2_grpc.AccessibilityServiceServicer):
    ...
    def _log_response(self, method_name: str, user_id: str, start_time: float) -> None:
        ...
        logger.info(
            f"{method_name}请求处理完成: 用户={user_id}, 耗时={elapsed_time:.2f}秒"
        )

    def _handle_error(self, method_name: str, user_id: str, error: Exception) -> None:
        """处理并记录错误"""
        logger.error(
            f"{method_name}请求处理失败: 用户={user_id}, 错误={error!s}",
            exc_info=True,
        )
        return grpc.StatusCode.INTERNAL, f"服务内部错误: {error!s}"
    ...
    def BlindAssistance(self, request, context):
        ...
        try:
            ...
            preferences = MessageToDict(
                request.preferences, preserving_proto_field_name=True
            )
            ...
            status_code, error_message = self._handle_error(
                "BlindAssistance", user_id, e
            )
    ...
    def SignLanguageRecognition(self, request, context):
        ...
        try:
            ...
            status_code, error_message = self._handle_error(
                "SignLanguageRecognition", user_id, e
            )
            context.set_code(status_code)
            context.set_details(error_message)
    ...
    def ManageSettings(self, request, context):
        ...
        try:
            ...
            result = self.service.manage_settings(
                user_id=user_id, preferences=preferences, action=request.action
            )
            ...
            if current_preferences:
                response.current_preferences.font_size = current_preferences.get(
                    "font_size", ""
                )
                response.current_preferences.high_contrast = current_preferences.get(
                    "high_contrast", False
                )
                response.current_preferences.voice_type = current_preferences.get(
                    "voice_type", ""
                )
                response.current_preferences.speech_rate = current_preferences.get(
                    "speech_rate", 1.0
                )
                response.current_preferences.language = current_preferences.get(
                    "language", ""
                )
                response.current_preferences.dialect = current_preferences.get(
                    "dialect", ""
                )
                response.current_preferences.screen_reader = current_preferences.get(
                    "screen_reader", False
                )
                response.current_preferences.sign_language = current_preferences.get(
                    "sign_language", False
                )
