...
class AccessibilityServicer(pb2_grpc.AccessibilityServiceServicer):
    ...
    async def StreamingSpeechTranslation(self, request_iterator, context):
        """流式语音翻译API"""
        async for result in self.translation_handler.streaming_speech_translation(
    ...
    async def CreateTranslationSession(self, request, context):
        """创建翻译会话API"""
        return await self.translation_handler.create_translation_session(
            request, context
        )
...
def register_servicer(
    ...
    service: OptimizedAccessibilityService,
    config: dict[str, Any],
