...
class TranslationHandler:
    ...
    async def speech_translation(self, request: pb2.SpeechTranslationRequest, context: grpc.aio.ServicerContext) -> pb2.SpeechTranslationResponse:
        ...
        """
        logger.info(f"接收到语音翻译请求: 用户={request.user_id}, 源语言={request.source_language}, 目标语言={request.target_language}")
        ...
        try:
            ...
            logger.info(f"语音翻译请求处理完成: 用户={request.user_id}, 耗时={(time.time() - start_time):.2f}秒")
            return response

        except Exception as e:
            logger.error(f"语音翻译处理异常: {str(e)}", exc_info=True)
            # 设置gRPC错误状态
            await context.set_code(grpc.StatusCode.INTERNAL)
            await context.set_details(f"翻译服务内部错误: {str(e)}")
    ...
    async def create_translation_session(self, request: pb2.CreateSessionRequest, context: grpc.aio.ServicerContext) -> pb2.CreateSessionResponse:
        """创建翻译会话
        ...
        """
        logger.info(f"接收到创建翻译会话请求: 用户={request.user_id}, 源语言={request.source_language}, 目标语言={request.target_language}")
        ...
        try:
            ...
            logger.info(f"创建翻译会话请求处理完成: 用户={request.user_id}, 会话ID={response.session_id}")
            return response

        except Exception as e:
            logger.error(f"创建翻译会话异常: {str(e)}", exc_info=True)
            # 设置gRPC错误状态
            await context.set_code(grpc.StatusCode.INTERNAL)
            await context.set_details(f"会话创建服务内部错误: {str(e)}")
    ...
    async def get_session_status(self, request: pb2.SessionStatusRequest, context: grpc.aio.ServicerContext) -> pb2.SessionStatusResponse:
        """获取会话状态
        ...
        """
        logger.info(f"接收到获取会话状态请求: 会话ID={request.session_id}, 用户={request.user_id}")
        ...
        try:
            ...
            # 构造基本响应
            response = pb2.SessionStatusResponse(
            ...
            logger.error(f"获取会话状态异常: {str(e)}", exc_info=True)
            # 设置gRPC错误状态
            await context.set_code(grpc.StatusCode.INTERNAL)
            await context.set_details(f"会话状态服务内部错误: {str(e)}")
            return pb2.SessionStatusResponse(
                session_id=request.session_id,
                error=str(e)
            )

    async def streaming_speech_translation(self, request_iterator: Iterator[pb2.SpeechTranslationChunk], context: grpc.aio.ServicerContext) -> Iterator[pb2.SpeechTranslationResult]:
        """流式语音翻译处理
        ...
        try:
            ...
            async for request in request_iterator:
                ...
                if session_id is None:
                    ...
                    logger.info(f"开始处理流式翻译请求: 会话ID={session_id}, 用户ID={user_id}")

                    # 首个请求应包含配置信息
                    if request.HasField("config"):
                        config = request.config
                        logger.info(f"流式翻译配置: 源语言={config.source_language}, 目标语言={config.target_language}")
                ...
                if not session_id:
                    ...
                    # 创建会话
                    session_creation_result = await self.create_session_from_config(user_id, config)
                    if not session_creation_result.get("success", False):
                        error_result = pb2.SpeechTranslationResult(
                            error=session_creation_result.get("message", "session_creation_failed"),
                            is_final=True
                        )
                        yield error_result
                        return
            ...
            logger.error(f"流式翻译处理异常: {str(e)}", exc_info=True)
            # 返回错误响应
            error_result = pb2.SpeechTranslationResult(
                error=str(e),
                is_final=True
    ...
    async def create_session_from_config(self, user_id: str, config: pb2.TranslationConfig) -> Dict[str, Any]:
        """根据配置创建会话
    ...
    async def get_supported_languages(self, request: pb2.SupportedLanguagesRequest, context: grpc.aio.ServicerContext) -> pb2.SupportedLanguagesResponse:
        """获取支持的语言和方言列表
        ...
        """
        logger.info(f"接收到获取支持语言请求: 用户={request.user_id}, 包含方言={request.include_dialects}")

        try:
            # 调用服务处理
            result = self.service.get_supported_languages(
                request.user_id,
                request.include_dialects
            )
            ...
            logger.info(f"获取支持语言请求处理完成: 语言数量={len(response.languages)}, 语言对数量={len(response.language_pairs)}")
            return response

        except Exception as e:
            logger.error(f"获取支持语言异常: {str(e)}", exc_info=True)
            # 设置gRPC错误状态
            await context.set_code(grpc.StatusCode.INTERNAL)
            await context.set_details(f"语言支持服务内部错误: {str(e)}")
