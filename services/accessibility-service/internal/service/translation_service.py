...
import numpy as np
import torch
from transformers import MBartForConditionalGeneration, MBartTokenizer, pipeline, AutoModelForSeq2SeqLM, AutoTokenizer
...
class TranslationService:
    ...
    def _load_translation_models(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 通用翻译模型 (多语言)
            model_name = self.config.translation.model_name if hasattr(self.config, 'translation') and hasattr(self.config.translation, 'model_name') else "facebook/mbart-large-50-many-to-many-mmt"
            ...
            # 特殊语言对的专用模型 (如中英翻译)
            # 这里可以根据需要加载其他专用模型
            special_pairs = self.config.translation.special_pairs if hasattr(self.config, 'translation') and hasattr(self.config.translation, 'special_pairs') else {}
            
            for pair, model_info in special_pairs.items():
                src, tgt = pair.split("-")
                logger.info(f"加载专用翻译模型: {src}-{tgt}, 模型: {model_info['model_name']}")
                
                special_tokenizer = AutoTokenizer.from_pretrained(model_info["model_name"])
                special_model = AutoModelForSeq2SeqLM.from_pretrained(model_info["model_name"])
            ...
            logger.error(f"加载翻译模型失败: {str(e)}", exc_info=True)
            # 加载失败时至少提供一个占位模型
            models["fallback"] = {
                "tokenizer": None,
                "model": None
    ...
    def _check_speech_support(self, language_code: str) -> bool:
        ...
        supported_speech_languages = [
            "en_XX", "zh_CN", "ja_XX", "ko_KR", "fr_XX", 
            "de_DE", "es_XX", "ru_RU", "ar_AR", "hi_IN"
        ]
        return language_code in supported_speech_languages
    
    def _get_model_for_language_pair(self, source_lang: str, target_lang: str) -> Dict[str, Any]:
        """根据语言对获取合适的翻译模型
    ...
    def translate_speech(self, audio_data: bytes, source_language: str, target_language: str,
        ...
        try:
            ...
            if source_dialect and self.dialect_service:
                # 使用方言服务进行语音识别
                transcription_result = self.dialect_service.transcribe_with_dialect(audio_data, source_dialect)
                recognized_text = transcription_result.get("text", "")
                source_confidence = transcription_result.get("confidence", 0.0)
                logger.info(f"方言识别结果: {recognized_text[:30]}... (置信度: {source_confidence:.2f})")
            else:
                # 这里应该使用通用ASR模型
                # 由于这是示例代码，我们使用占位实现
            ...
            # 2. 文本翻译
            translated_text = self.translate_text(recognized_text, source_language, target_language)
            ...
            if target_dialect and self.dialect_service:
                # 使用方言服务进行语音合成
                voice_type = preferences.get("voice_type", "default") if preferences else "default"
    ...
    def translate_text(self, text: str, source_language: str, target_language: str) -> str:
        ...
        # 获取适合的翻译模型
        translation_model = self._get_model_for_language_pair(source_language, target_language)
    ...
    def create_streaming_session(self, user_id: str, source_language: str, target_language: str,
                               source_dialect: Optional[str] = None, target_dialect: Optional[str] = None,
        ...
        logger.info(f"创建流式翻译会话: {session_id}, 用户: {user_id}, 语言: {source_language} → {target_language}")
        return session_id
    
    def process_streaming_chunk(self, session_id: str, audio_chunk: bytes, is_final: bool = False) -> Optional[Dict[str, Any]]:
        """处理流式翻译的音频数据块
