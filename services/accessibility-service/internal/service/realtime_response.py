...
class LoadBalancer:
    ...
    def remove_processor(self, processor_id: str):
        """移除处理器"""
        self.processors = [p for p in self.processors if p["processor"].processor_id != processor_id]
        logger.info(f"移除处理器: {processor_id}")
    
    def select_processor(self, event: Event) -> Optional[EventProcessor]:
        """选择处理器"""
...
class RealtimeResponseSystem:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 处理器池
        self.processors = {}
        self.default_processor_count = config.get("realtime_response", {}).get("processor_count", 4)
    ...
    def _register_default_handlers(self, processor: EventProcessor):
        ...
        processor.register_handler(EventType.ACCESSIBILITY_REQUEST, handle_accessibility_request)
    ...
    async def submit_event(self, event: Event) -> str:
        ...
        if self.event_queue.put(event):
            ...
            logger.info(f"事件已提交: {event.event_id} (优先级: {event.priority.value})")
            return event.event_id
        else:
    ...
    async def get_response(self, event_id: str, timeout: float = 30.0) -> Optional[Response]:
        """获取事件响应"""
        start_time = time.time()
    ...
    async def _processing_loop(self):
        ...
        while self.is_running:
            try:
                ...
                # 创建处理任务
                task = asyncio.create_task(self._process_event_with_processor(event, processor))
    ...
    async def _process_event_with_processor(self, event: Event, processor: EventProcessor):
        """使用指定处理器处理事件"""
        try:
            start_time = time.time()
            ...
            logger.debug(f"事件处理完成: {event.event_id} (耗时: {processing_time:.3f}s)")
    ...
    def register_custom_handler(self, event_type: EventType, handler: Callable, processor_id: Optional[str] = None):
        """注册自定义事件处理器"""
        if processor_id:
    ...
    def add_middleware(self, middleware: Callable, processor_id: Optional[str] = None):
        """添加中间件"""
        if processor_id:
            if processor_id in self.processors:
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        recent_times = [r.processing_time for r in recent_responses if r.status == ResponseStatus.COMPLETED]
        ...
        metrics = {
            ...
            "success_rate": (self.stats["events_processed"] - self.stats["events_failed"]) / max(1, self.stats["events_processed"]),
...
# 便捷函数
async def create_event(event_type: EventType, data: Dict[str, Any], 
...
async def rate_limiting_middleware(event: Event) -> Optional[Event]:
    """限流中间件"""
    # 简化的限流逻辑
...
async def authentication_middleware(event: Event) -> Optional[Event]:
    """认证中间件"""
    # 检查事件是否包含认证信息
