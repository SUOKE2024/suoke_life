...
class ModulesConfig:
    ...
    blind_assistance: ModuleConfig = field(
    ...
    sign_language: ModuleConfig = field(
        default_factory=lambda: ModuleConfig(enabled=True, device="cpu")
    )

    voice_assistance: ModuleConfig = field(
        default_factory=lambda: ModuleConfig(enabled=True, device="cpu")
    )

    screen_reading: ModuleConfig = field(
        default_factory=lambda: ModuleConfig(enabled=True, device="cpu")
    )

    content_conversion: ModuleConfig = field(
        default_factory=lambda: ModuleConfig(
            enabled=True, model_path="google/flan-t5-base", device="cpu"
        )
    )

    translation: ModuleConfig = field(
        default_factory=lambda: ModuleConfig(
            enabled=True,
...
class ConfigManager:
    ...
    def _set_config_value(self, config_path: tuple, value: str):
        ...
        try:
            ...
            logger.warning(f"设置配置值失败 {config_path}: {e!s}")
    ...
    def get_module_config(self, module_name: str) -> ModuleConfig | None:
        """获取模块配置"""
    ...
    def validate_config(self) -> dict[str, Any]:
        ...
        try:
            ...
            validation_result["errors"].append(f"配置验证失败: {e!s}")
