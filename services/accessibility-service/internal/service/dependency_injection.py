        ...
        try:
            ...
            for param_name, param in sig.parameters.items():
                ...
                # 检查是否有默认值
                    ...
                    logger.warning(
                        f"无法解析依赖: {param_name} in {service_def.implementation}"
                    )

            return service_def.implementation(**kwargs)

        except Exception as e:
            logger.error(f"创建服务实例失败: {service_def.implementation}, 错误: {e!s}")
            raise

    def _find_service_by_type(self, service_type: type) -> str | None:
        """根据类型查找服务名称"""
        ...
        for service_name, service_def in self._services.items():
            if service_def.singleton:
                try:
                    ...
                    logger.error(f"初始化服务失败: {service_name}, 错误: {e!s}")
                    raise
    ...
    async def shutdown(self) -> None:
        ...
        for service_name, instance in self._instances.items():
            try:
                ...
                logger.error(f"清理服务失败: {service_name}, 错误: {e!s}")
    ...
    def get_service_info(self) -> dict[str, dict[str, Any]]:
        ...
        for name, service_def in self._services.items():
            info[name] = {
                ...
                "implementation": (
                    service_def.implementation.__name__
...
async def setup_container(config: dict[str, Any]) -> DIContainer:
    ...
    # 注册业务服务
    container.register(
        "blind_assistance", IBlindAssistanceService, BlindAssistanceServiceImpl
    )
    container.register("sign_language", ISignLanguageService, SignLanguageServiceImpl)
    container.register(
        "screen_reading", IScreenReadingService, ScreenReadingServiceImpl
    )
    container.register(
        "voice_assistance", IVoiceAssistanceService, VoiceAssistanceServiceImpl
    )
    container.register(
        "content_conversion", IContentConversionService, ContentConversionServiceImpl
    )
