...
class EdgeComputingService:
    ...
    def _load_tflite_model(self, model_path: str) -> Any:
        ...
        return {
            ...
            "metadata": {"version": "1.0", "type": "tflite"},
        }
    ...
    def prepare_offline_bundle(
        self, user_id: str, features: list[str]
    ) -> dict[str, Any]:
        """准备用户离线包，包含轻量级模型和配置
        ...
        try:
            ...
            logger.error(f"准备离线包失败: {e!s}")
            return {"error": str(e)}

    def _get_user_required_models(
        self, user_id: str, features: list[str]
    ) -> dict[str, Any]:
        """根据用户需求和功能获取所需的轻量级模型
        ...
        # 过滤用户请求的功能
        enabled_features = [
            f for f in features if f in self.config.edge_computing.offline_mode.features
        ]
    ...
    def _get_personalized_settings(self, user_id: str) -> dict[str, Any]:
        ...
        """
        # 在实际实现中，应该从用户配置服务获取
        return {"speech_rate": 1.0, "voice_type": "default", "text_size": "medium"}
    ...
    def check_device_compatibility(self, device_info: dict[str, Any]) -> dict[str, Any]:
        ...
        # 检查GPU/NPU
        if device_info.get("gpu_supported", False) or device_info.get(
            "npu_supported", False
        ):
