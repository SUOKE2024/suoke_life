...
class EmotionRecognitionModel:
    ...
    def recognize_emotion(
        self, sensor_data: dict[str, Any], behavioral_data: dict[str, Any]
    ) -> AIModelResult:
        """识别用户情感状态"""
        try:
            # 提取特征
    ...
    def _extract_emotion_features(
        ...
        if "microphone" in sensor_data:
            ...
            features["voice_energy"] = np.mean(
                [d.get("values", [0])[0] for d in mic_data]
            )
            features["voice_variability"] = np.std(
                [d.get("values", [0])[0] for d in mic_data]
            )
        ...
        if "accelerometer" in sensor_data:
            ...
            movement_intensity = np.mean(
                [np.linalg.norm(d.get("values", [0, 0, 0])) for d in acc_data]
            )
            features["movement_intensity"] = movement_intensity
            features["movement_variability"] = np.std(
                [np.linalg.norm(d.get("values", [0, 0, 0])) for d in acc_data]
            )
        ...
        if "light" in sensor_data:
            ...
            features["ambient_light"] = np.mean(
                [d.get("values", [0])[0] for d in light_data]
            )
...
class HealthRiskAssessmentModel:
    ...
    def _assess_metabolic_risk(self, user_context: UserContext) -> HealthAlert | None:
        """评估代谢风险"""
        sensor_data = user_context.sensor_context
    ...
    def _assess_sleep_risk(self, user_context: UserContext) -> HealthAlert | None:
        """评估睡眠风险"""
        temporal_data = user_context.temporal_context
...
class AICoordinator:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.ai_results_history = defaultdict(
            deque
        )  # user_id -> deque of AIModelResult
    ...
    async def _coordination_loop(self) -> None:
        ...
        while self.running:
            try:
                ...
                logger.error(f"AI协调循环错误: {e!s}")
                await asyncio.sleep(5)

    async def _update_user_contexts(self) -> None:
        """更新用户上下文信息"""
        try:
            # 这里应该从其他模块获取用户数据
            ...
            for user_id, context in self.user_contexts.items():
                ...
                if context.sensor_context:
                    emotion_result = self.emotion_model.recognize_emotion(
                        context.sensor_context, context.historical_patterns
                    )
    ...
    def _is_duplicate_alert(self, user_id: str, new_alert: HealthAlert) -> bool:
        ...
        for existing_alert in existing_alerts:
            if (
                ...
                and existing_alert.severity == new_alert.severity
                and time.time() - existing_alert.timestamp < 1800
            ):  # 30分钟内
                return True
        ...
        if emotional_state:
            ...
            if dominant_emotion[0] in ["stressed", "anxious"]:
                recommendations.extend(
                    ["尝试深呼吸练习或冥想", "考虑短暂休息或散步", "听一些舒缓的音乐"]
                )
            elif dominant_emotion[0] == "tired":
                recommendations.extend(
                    ["确保充足的睡眠", "适当补充水分", "考虑小憩片刻"]
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "average_predictions_per_minute": self.stats["total_predictions"]
            / max(uptime / 60, 1),
        }
