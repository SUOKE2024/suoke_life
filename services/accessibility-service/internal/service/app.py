...
class AccessibilityApp:
    ...
    def __init__(self, app_config: Optional[Any] = None) -> None:
        """
        初始化应用
    ...
    def _init_accessibility_service(self) -> Optional[OptimizedAccessibilityService]:
        """
        初始化无障碍服务
    ...
    def start(self) -> None:
        ...
        try:
            if (
                ...
                and hasattr(self.config, "background_collection")
                and hasattr(self.config.background_collection, "enabled")
                and self.config.background_collection.enabled
            ):
                logger.info("启动后台数据采集服务")
                self.background_collection.start()
        except AttributeError as e:
            logger.warning(f"未能启动后台数据采集服务: {e!s}")

        # 启动危机报警服务
        try:
            if (
                self.crisis_alert
                and hasattr(self.config, "crisis_alert")
                and hasattr(self.config.crisis_alert, "enabled")
                and self.config.crisis_alert.enabled
            ):
                logger.info("启动危机报警服务")
                self.crisis_alert.start()
        except AttributeError as e:
            logger.warning(f"未能启动危机报警服务: {e!s}")

        # 记录应用启动指标
        if self.monitoring_service and hasattr(
            self.monitoring_service, "metrics_client"
        ):
