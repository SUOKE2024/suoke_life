        ...
        for component, metrics_deque in self.metrics_history.items():
            ...
            for metric in recent_metrics:
                component_summary["recent_metrics"].append({
                    ...
                    "status": "critical" if metric.value >= metric.threshold_critical else
                             "warning" if metric.value >= metric.threshold_warning else "normal"
                })
...
class FailurePredictor:
    ...
    def train_models(self, monitor: SystemMonitor):
        ...
        for component, metrics_deque in monitor.metrics_history.items():
            ...
            try:
                ...
                logger.error(f"训练组件 {component.value} 的模型失败: {str(e)}")
            ...
            try:
                ...
                if failure_probability > 0.7:
                    ...
                    if trend < 0:  # 分数下降趋势（更可能故障）
                        hours_to_failure = max(1, abs(latest_score / trend) * 0.5)  # 转换为小时
                ...
                # 分析贡献因素
                contributing_factors = self._analyze_contributing_factors(recent_metrics)
                
                # 生成推荐操作
                recommended_actions = self._generate_recommendations(component, failure_probability, contributing_factors)
                ...
                logger.error(f"预测组件 {component.value} 故障失败: {str(e)}")
    ...
    def _generate_recommendations(self, component: ComponentType, failure_probability: float, 
...
class MaintenanceScheduler:
    ...
    def schedule_from_prediction(self, prediction: FailurePrediction) -> str:
        ...
        task = MaintenanceTask(
            ...
            dependencies=[],
            auto_executable=prediction.failure_probability < 0.8  # 高风险任务需要人工确认
        )
        
        return self.schedule_task(task)
    
    def _estimate_duration(self, component: ComponentType, task_type: MaintenanceType) -> float:
        """估算维护时长（分钟）"""
        base_durations = {
    ...
    async def execute_next_task(self) -> Optional[MaintenanceTask]:
        """执行下一个维护任务"""
    ...
    def _check_dependencies(self, task: MaintenanceTask) -> bool:
        ...
        for dep_id in task.dependencies:
            # 检查依赖任务是否已完成
            if not any(t.task_id == dep_id and t.status == "completed" for t in self.completed_tasks):
                return False
        return True
...
class PredictiveMaintenance:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 配置参数
        self.prediction_interval = config.get("predictive_maintenance", {}).get("prediction_interval", 300.0)  # 5分钟
        self.auto_maintenance = config.get("predictive_maintenance", {}).get("auto_maintenance", True)
