    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            try:
                ...
                # 检查是否需要忽略此异常
                if ignore_exceptions and any(isinstance(e, exc_type) for exc_type in ignore_exceptions):
                    raise
                
                # 检查是否需要处理此异常
                if handle_exceptions and not any(isinstance(e, exc_type) for exc_type in handle_exceptions):
                    raise
                
                # 记录错误
                logger.error(f"错误处理: {op_name} 发生异常: {type(e).__name__}: {str(e)}")
                
                # 使用错误处理器处理
                if _error_handler:
                    error_info = _error_handler._classifier.classify(e, {
                        'operation': op_name,
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            if _error_handler:
                ...
                return await _error_handler._retry_with_config(func, config, *args, **kwargs)
            else:
                # 简单重试逻辑
                last_exception = None
        ...
        def sync_wrapper(*args, **kwargs):
            if _error_handler:
                ...
                return loop.run_until_complete(
                    _error_handler._retry_with_config(async_func, config, *args, **kwargs)
                )
            else:
                # 简单重试逻辑
                import time
