...
def _generate_cache_key(func: Callable, args: tuple, kwargs: dict, 
                       key_prefix: str = None, include_args: bool = True) -> str:
    """
    生成缓存键
    ...
    try:
        ...
        logger.warning(f"生成缓存键失败: {str(e)}, 使用基础键")
        return base_key
...
def cache_result(ttl: int = 3600,
    ...
    def decorator(func: Callable):
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if _cache_manager:
                try:
                    cached_result = await _cache_manager.get(cache_key, level=cache_level)
            ...
            if condition:
                try:
                    ...
                    logger.warning(f"缓存条件检查失败: {str(e)}")
                    should_cache = True
            
            # 缓存结果
            if should_cache and _cache_manager:
                try:
                    await _cache_manager.set(cache_key, result, ttl=ttl, level=cache_level)
            ...
            if should_cache and _cache_manager:
                try:
                    ...
                    loop.run_until_complete(
                        _cache_manager.set(cache_key, result, ttl=ttl, level=cache_level)
                    )
                    logger.debug(f"缓存设置: {cache_key}")
                except Exception as e:
                    logger.warning(f"缓存设置失败: {str(e)}")
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if _cache_manager:
                try:
                    if key_pattern:
                        # 使用模式删除
                        await _cache_manager.delete_pattern(key_pattern, level=cache_level)
                    else:
                        # 生成具体的缓存键
                        cache_key = _generate_cache_key(func, args, kwargs, key_prefix, include_args)
            ...
            if _cache_manager:
                try:
                    ...
                    if key_pattern:
                        # 使用模式删除
                        loop.run_until_complete(
                            _cache_manager.delete_pattern(key_pattern, level=cache_level)
                        )
                    else:
                        # 生成具体的缓存键
                        cache_key = _generate_cache_key(func, args, kwargs, key_prefix, include_args)
...
def cache_warm_up(cache_keys: list,
                 cache_level: str = "memory"):
    """
    缓存预热装饰器
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if _cache_manager:
                for cache_key in cache_keys:
                    try:
                        # 检查缓存是否存在
                        cached_value = await _cache_manager.get(cache_key, level=cache_level)
                        if cached_value is None:
                            logger.debug(f"缓存预热: {cache_key} 不存在，跳过")
                        else:
                            logger.debug(f"缓存预热: {cache_key} 已存在")
                    except Exception as e:
                        logger.warning(f"缓存预热失败: {cache_key}, 错误: {str(e)}")
