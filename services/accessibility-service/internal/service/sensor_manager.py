...
class SensorHandler:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.error(f"传感器 {self.sensor_type.value} 启动失败: {str(e)}")
    ...
    async def stop(self) -> bool:
        ...
        try:
            ...
            logger.error(f"传感器 {self.sensor_type.value} 停止失败: {str(e)}")
            return False
    ...
    async def read(self) -> Optional[SensorReading]:
        ...
        try:
            ...
            logger.error(f"传感器 {self.sensor_type.value} 读取失败: {str(e)}")
            self.error_count += 1
            if self.error_count > 5:
                self.status = SensorStatus.ERROR
            return None
    
    async def _read_raw_data(self) -> Optional[List[float]]:
        """读取原始数据 - 子类需要实现"""
...
class StepCounterHandler(SensorHandler):
    ...
    async def _read_raw_data(self) -> Optional[List[float]]:
        ...
        # 模拟步数增加
        if random.random() < 0.3:  # 30%概率增加步数
            self.total_steps += random.randint(1, 3)
...
class SensorFusionEngine:
    ...
    def fuse_data(self, sensor_data: Dict[SensorType, List[SensorReading]]) -> Dict[str, Any]:
        """融合多传感器数据"""
        fused_results = {}
        ...
        for fusion_type, algorithm in self.fusion_algorithms.items():
            try:
                ...
                logger.error(f"传感器融合失败 {fusion_type}: {str(e)}")
        
        return fused_results
    
    def _fuse_orientation(self, sensor_data: Dict[SensorType, List[SensorReading]]) -> Optional[Dict[str, Any]]:
        """融合方向相关的传感器数据"""
    ...
    def _fuse_motion(self, sensor_data: Dict[SensorType, List[SensorReading]]) -> Optional[Dict[str, Any]]:
        """融合运动相关的传感器数据"""
        accel_data = sensor_data.get(SensorType.ACCELEROMETER, [])
        gyro_data = sensor_data.get(SensorType.GYROSCOPE, [])
    ...
    def _fuse_environment(self, sensor_data: Dict[SensorType, List[SensorReading]]) -> Optional[Dict[str, Any]]:
        """融合环境相关的传感器数据"""
    ...
    def _fuse_location(self, sensor_data: Dict[SensorType, List[SensorReading]]) -> Optional[Dict[str, Any]]:
        """融合位置相关的传感器数据"""
...
class SensorManager:
        ...
        for sensor_name, sensor_config in sensor_configs.items():
            try:
                ...
                if sensor_type in handler_classes and config_obj.enabled:
                    ...
                    self.sensor_handlers[sensor_type] = handler_class(sensor_type, config_obj)
                ...
                logger.error(f"传感器 {sensor_name} 初始化失败: {str(e)}")
    ...
    async def _process_reading(self, sensor_type: SensorType, reading: SensorReading):
        ...
        try:
            ...
            logger.error(f"处理传感器读数失败 {sensor_type.value}: {str(e)}")
            self.stats["errors_per_sensor"][sensor_type.value] += 1
    
    def _apply_calibration(self, reading: SensorReading, calibration: Dict[str, float]) -> SensorReading:
        """应用传感器校准"""
        calibrated_values = []
        ...
        if sensor_type in self.callbacks:
            for callback in self.callbacks[sensor_type]:
                try:
                    ...
                    logger.error(f"传感器回调执行失败: {str(e)}")
    
    async def _perform_sensor_fusion(self):
        """执行传感器融合"""
        try:
            # 收集最近的传感器数据
            recent_data = {}
            ...
            if len(recent_data) > 1:
                ...
                if fusion_result:
                    ...
                    # 触发融合结果回调
                    self._trigger_callbacks(SensorType.ORIENTATION, fusion_result)  # 使用特殊类型表示融合结果
                    
        except Exception as e:
            logger.error(f"传感器融合失败: {str(e)}")
    ...
    def get_readings(self, sensor_type: SensorType, count: int = 10) -> List[SensorReading]:
        """获取指定数量的传感器读数"""
        if sensor_type in self.sensor_data:
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_sensors": len([h for h in self.sensor_handlers.values() if h.status == SensorStatus.ACTIVE]),
...
class MockSensorHandler:
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            logger.error(f"模拟传感器 {self.sensor_type.value} 启动失败: {str(e)}")
            self.status = SensorStatus.ERROR
            return False
    ...
    async def stop(self) -> bool:
        ...
        try:
            ...
            logger.error(f"模拟传感器 {self.sensor_type.value} 停止失败: {str(e)}")
            return False
    ...
    async def read(self) -> Optional[SensorReading]:
        ...
        try:
            ...
            logger.error(f"模拟传感器 {self.sensor_type.value} 读取失败: {str(e)}")
            self.error_count += 1
            if self.error_count > 5:
                self.status = SensorStatus.ERROR
            return None
    
    def _generate_mock_data(self) -> Optional[List[float]]:
        """生成模拟传感器数据"""
        import random
