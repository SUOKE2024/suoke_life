...
from ..interfaces import ICacheManager, IModelManager, IVoiceAssistanceService
...
class VoiceAssistanceServiceImpl(IVoiceAssistanceService):
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.error(f"语音辅助服务初始化失败: {e!s}")
            raise
    ...
    async def _load_models(self):
        ...
        try:
            # 加载语音识别模型
            asr_model_config = self.model_config.get(
                "asr",
                {
            ...
            # 加载自然语言处理模型
            nlp_model_config = self.model_config.get(
                "nlp",
                {
            ...
            # 加载文本转语音模型
            tts_model_config = self.model_config.get(
                "tts",
                {
    ...
    async def process_voice_command(
        ...
        async with self._semaphore:
            ...
            try:
                ...
                logger.debug(
                    f"语音命令处理完成: 用户 {user_id}, 意图 {intent_result.get('intent', 'unknown')}"
                )
                return result

            except Exception as e:
                self._error_count += 1
                logger.error(f"语音命令处理失败: 用户 {user_id}, 错误: {e!s}")
                raise
    ...
    async def text_to_speech(
        ...
        async with self._semaphore:
            ...
            try:
                ...
                logger.debug(
                    f"文本转语音完成: 文本长度 {len(text)}, 音频大小 {len(processed_audio)} bytes"
                )
    ...
    async def _speech_to_text(
        self, audio_data: bytes, language: str, dialect: str
    ) -> dict:
        """
        语音转文本
    ...
    async def _analyze_intent(self, text: str, context: str, language: str) -> dict:
        ...
        try:
            ...
            logger.error(f"意图分析失败: {e!s}")
            raise

    async def _generate_response(
        self, intent_result: dict, context: str, language: str, user_id: str
    ) -> dict:
        """
        生成响应
        ...
        try:
            ...
            if intent == "find_facility":
                ...
                response["text_response"] = (
                    f"正在为您查找最近的{'无障碍' if accessibility else ''}{facility_type}"
                )
                response["data"] = {
                ...
                response["suggestions"] = ["找设施", "导航", "帮助"]

            return response

        except Exception as e:
            logger.error(f"生成响应失败: {e!s}")
            raise
    ...
    async def _preprocess_text(self, text: str, language: str) -> str:
        ...
        try:
            ...
            logger.error(f"文本预处理失败: {e!s}")
            raise

    async def _text_to_speech_with_model(
        self, text: str, language: str, voice_preferences: dict
    ) -> bytes:
        """
        使用TTS模型转换文本
        ...
        try:
            ...
            logger.error(f"TTS模型推理失败: {e!s}")
            raise

    async def _postprocess_audio(
        self, audio_data: bytes, voice_preferences: dict
    ) -> bytes:
        """
        音频后处理
