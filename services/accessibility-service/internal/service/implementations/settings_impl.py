...
from ..interfaces import ICacheManager, ISettingsService
...
class SettingsServiceImpl(ISettingsService):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.error(f"设置管理服务初始化失败: {e!s}")
            raise
        ...
        async with self._semaphore:
            ...
            try:
                ...
                response = {
                    ...
                    "source": (
                        "storage"
                        if preferences != self._default_preferences
                        else "default"
                    ),
                }
                ...
                self._error_count += 1
                logger.error(f"获取用户偏好失败: 用户 {user_id}, 错误: {e!s}")
                raise
    ...
    async def update_user_preferences(self, user_id: str, preferences: dict) -> dict:
        ...
        async with self._semaphore:
            ...
            try:
                ...
                # 保存到存储
                await self._save_user_preferences_to_storage(
                    user_id, merged_preferences
                )

                # 更新缓存
                cache_key = f"user_preferences:{user_id}"
                await self.cache_manager.set(
                    cache_key, merged_preferences, self.cache_ttl
                )
                ...
                self._error_count += 1
                logger.error(f"更新用户偏好失败: 用户 {user_id}, 错误: {e!s}")
                raise

    async def _load_user_preferences_from_storage(self, user_id: str) -> dict | None:
        """从存储加载用户偏好"""
        try:
            # 模拟从数据库加载
            ...
            logger.error(f"从存储加载用户偏好失败: 用户 {user_id}, 错误: {e!s}")
            return None
    ...
    async def _save_user_preferences_to_storage(self, user_id: str, preferences: dict):
        ...
        try:
            ...
            logger.error(f"保存用户偏好到存储失败: 用户 {user_id}, 错误: {e!s}")
            raise
        ...
        try:
            ...
            if "accessibility" in preferences:
                validated["accessibility"] = (
                    await self._validate_accessibility_preferences(
            ...
            if "ui" in preferences:
                validated["ui"] = await self._validate_ui_preferences(preferences["ui"])
            ...
            if "notifications" in preferences:
                validated["notifications"] = (
                    await self._validate_notification_preferences(
            ...
            logger.error(f"验证偏好设置失败: {e!s}")
            raise ValueError(f"偏好设置格式无效: {e!s}")
    ...
    async def _validate_accessibility_preferences(self, accessibility: dict) -> dict:
        ...
        if "voice_assistance" in accessibility:
            ...
            validated["voice_assistance"] = {
                ...
                "voice_speed": max(
                    0.5, min(2.0, float(voice_settings.get("voice_speed", 1.0)))
                ),
                "voice_volume": max(
                    0.0, min(1.0, float(voice_settings.get("voice_volume", 0.8)))
                ),
    ...
    async def _record_preferences_change(
        self, user_id: str, old_prefs: dict, new_prefs: dict
    ):
        """记录偏好设置变更历史"""
        try:
            ...
            logger.error(f"记录偏好设置变更失败: 用户 {user_id}, 错误: {e!s}")
        ...
        def compare_dicts(old_dict, new_dict, path=""):
            for key, new_value in new_dict.items():
                ...
                if key not in old_dict:
                    changes.append(
                        {"type": "added", "path": current_path, "new_value": new_value}
                    )
    ...
    async def reset_user_preferences(self, user_id: str) -> dict:
        ...
        try:
            ...
            logger.error(f"重置用户偏好失败: 用户 {user_id}, 错误: {e!s}")
            raise
