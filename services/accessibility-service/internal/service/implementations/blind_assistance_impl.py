...
from ..interfaces import IBlindAssistanceService, IModelManager, ICacheManager
...
class BlindAssistanceServiceImpl(IBlindAssistanceService):
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.error(f"导盲服务初始化失败: {str(e)}")
            raise
    ...
    async def _load_models(self):
        ...
        try:
            # 加载场景分析模型
            scene_model_config = self.model_config.get('scene_analysis', {
            ...
            # 加载障碍物检测模型
            obstacle_model_config = self.model_config.get('obstacle_detection', {
    ...
    async def analyze_scene(self, image_data: bytes, user_id: str, 
        ...
        async with self._semaphore:
            ...
            try:
                ...
                logger.debug(f"场景分析完成: 用户 {user_id}, 置信度 {result['confidence']}")
                return result
                
            except Exception as e:
                self._error_count += 1
                logger.error(f"场景分析失败: 用户 {user_id}, 错误: {str(e)}")
                raise
    ...
    async def _analyze_scene_with_model(self, processed_image: Any, 
        ...
        try:
            ...
            logger.error(f"场景分析模型推理失败: {str(e)}")
            raise
    
    async def _generate_navigation_advice(self, scene_result: Dict, 
                                        location: Dict, preferences: Dict) -> Dict:
        """
        生成导航建议
        ...
        try:
            ...
            if scene_type == 'indoor':
                ...
                if doors:
                    door = doors[0]
                    advice['secondary_advice'].append(f"前方有门，位置在图像的 {door['bbox']} 区域")
                
                # 检查障碍物
                obstacles = [obj for obj in objects if obj['name'] in ['chair', 'table']]
                if obstacles:
            ...
            logger.error(f"生成导航建议失败: {str(e)}")
            raise
    
    async def _detect_obstacles_with_model(self, processed_image: Any, 
        ...
        try:
            ...
            logger.error(f"障碍物检测模型推理失败: {str(e)}")
            raise
    ...
    async def _postprocess_obstacles(self, obstacles: List[Dict]) -> List[Dict]:
        ...
        try:
            ...
            for obstacle in obstacles:
                ...
                # 添加时间戳
                processed_obstacle['detected_at'] = datetime.now(timezone.utc).isoformat()
            ...
            logger.error(f"障碍物结果后处理失败: {str(e)}")
            raise
