...
class BackgroundCollectionService:
    ...
    def __init__(self, config: Any):
        ...
        self.config = config
        self.enabled = (
            config.background_collection.enabled
            if hasattr(config, "background_collection")
            else False
        )
        ...
        # 电池管理相关
        self.battery_threshold_low = (
            config.background_collection.battery_threshold_low
            if hasattr(config, "background_collection")
            and hasattr(config.background_collection, "battery_threshold_low")
            else 20
        )
        self.battery_threshold_critical = (
            config.background_collection.battery_threshold_critical
            if hasattr(config, "background_collection")
            and hasattr(config.background_collection, "battery_threshold_critical")
            else 10
        )
        self.interval_multiplier_low = (
            config.background_collection.interval_multiplier_low
            if hasattr(config, "background_collection")
            and hasattr(config.background_collection, "interval_multiplier_low")
            else 2
        )
        self.interval_multiplier_critical = (
            config.background_collection.interval_multiplier_critical
            if hasattr(config, "background_collection")
            and hasattr(config.background_collection, "interval_multiplier_critical")
            else 4
        )
    ...
    def setup_encryption(self) -> None:
        ...
        if (
            ...
            and hasattr(self.config.security, "encryption")
        ):
            try:
                # 实际应用中应从安全存储获取密钥
                self.encryption_key = hashlib.sha256(
                    self.config.security.encryption.get(
                        "seed", "accessibility-service"
                    ).encode()
                ).digest()
                logger.info("数据加密已设置")
            except Exception as e:
                logger.error(f"设置数据加密失败: {e!s}")
    ...
    def register_user_consent(
        self, user_id: str, data_types: list[str], expiry_days: int = 365
    ) -> dict[str, Any]:
        """
        注册用户对特定数据类型的采集同意
        ...
        try:
            ...
            # 如果有新的同意类型，启动对应的采集线程
            if (
                user_id not in self.collection_threads
                or not self.collection_threads[user_id].is_alive()
            ):
            ...
            logger.error(f"注册用户同意失败: {e!s}")
            return {"success": False, "message": f"注册失败: {e!s}"}

    def revoke_user_consent(
        self, user_id: str, data_types: list[str] | None = None
    ) -> dict[str, Any]:
        """
        撤销用户对特定数据类型的采集同意
        ...
        try:
            if data_types is None:
                # 撤销所有同意
                if (
                    user_id in self.collection_threads
                    and self.collection_threads[user_id].is_alive()
                ):
                ...
                if user_id in self.user_consents:
                    ...
                    logger.info(
                        f"已撤销用户 {user_id} 对数据类型 {data_types} 的采集同意"
                    )
                    return {
                        "success": True,
                        "message": f"已撤销指定数据类型的采集同意: {data_types}",
                    }

                return {"success": False, "message": "未找到用户同意记录"}

        except Exception as e:
            logger.error(f"撤销用户同意失败: {e!s}")
            return {"success": False, "message": f"撤销失败: {e!s}"}
    ...
    def get_user_consent_status(self, user_id: str) -> dict[str, Any]:
        ...
        """
        if user_id not in self.user_consents:
            return {"has_consent": False, "message": "用户未授权数据采集"}
    ...
    def set_collection_interval(
        self, user_id: str, intervals: dict[str, int]
    ) -> dict[str, Any]:
        """
        设置特定用户的数据采集间隔
        ...
        try:
            ...
            for data_type, interval in intervals.items():
                if data_type in self.collectors:
                    self.collection_intervals[user_id][data_type] = max(
                        interval, 10
                    )  # 最小间隔10秒
            ...
            logger.error(f"设置采集间隔失败: {e!s}")
            return {"success": False, "message": f"设置失败: {e!s}"}
            ...
            with self.cache_lock:
                if user_id in self.data_cache:
                    for data_type in self.data_cache[user_id]:
                        result["data_points"][data_type] = len(
    ...
    def start(self) -> None:
        ...
        logger.info(
            f"后台数据采集服务已启动，当前活动用户数: {len(self.collection_threads)}"
        )
        ...
        while not self.stopping and user_id in self.user_consents:
            try:
                ...
                if battery_level <= self.battery_threshold_critical:
                    ...
                    logger.warning(
                        f"电池电量严重不足 ({battery_level}%)，大幅降低采集频率"
                    )
                elif battery_level <= self.battery_threshold_low:
                ...
                for data_type in data_types:
                    ...
                    if (
                        data_type not in last_collection
                        or (current_time - last_collection[data_type])
                        >= adjusted_interval
                    ):
                        # 执行数据采集
                        ...
                        if collector:
                            try:
                                ...
                                logger.error(
                                    f"用户 {user_id} 的 {data_type} 数据采集失败: {e!s}"
                                )
                ...
                if (
                    user_id not in self.last_sync_time
                    or (current_time - self.last_sync_time.get(user_id, 0))
                    >= sync_interval
                ):
                    self._sync_user_data(user_id)
                ...
                logger.error(f"用户 {user_id} 的数据采集线程出错: {e!s}")
    ...
    def _cache_data(self, user_id: str, data_type: str, data: Any):
        ...
        with self.cache_lock:
            ...
            # 添加时间戳
            data_entry = {"timestamp": datetime.now().isoformat(), "data": data}

            # 加密数据（如果已设置加密）
            if self.privacy_service and self.encryption_key:
                try:
                    data_str = json.dumps(data_entry)
                    encrypted_data = self.privacy_service.encrypt_data(
                        data_str, self.encryption_key
                    )
            ...
            if len(self.data_cache[user_id][data_type]) > max_cache_size:
                self.data_cache[user_id][data_type] = self.data_cache[user_id][
                    data_type
                ][-max_cache_size:]
            ...
            if hasattr(self, "crisis_alert_service") and self.crisis_alert_service:
                try:
                    # 传递数据给危机报警服务进行分析
                    self.crisis_alert_service._process_collected_data(
                        user_id, data_type, data
                    )
                except Exception as e:
                    logger.error(f"传递数据到危机报警服务失败: {e!s}")
    ...
    def _sync_user_data(self, user_id: str):
        ...
        try:
            ...
            logger.error(f"同步用户 {user_id} 的数据失败: {e!s}")
        ...
        try:
            ...
            if system == "Darwin":  # macOS
                try:
                    ...
                    logger.warning(f"通过pmset获取电池信息失败: {e!s}")
                ...
                try:
                    ...
                    # 读取当前和最大能量值
                    with (
                        open(energy_now_path) as f_now,
                        open(energy_full_path) as f_full,
                    ):
                    ...
                    logger.warning(f"从系统文件读取电池信息失败: {e!s}")
                    ...
                    try:
                        ...
                        result = subprocess.run(
                            ["acpi", "-b"], capture_output=True, text=True
                        )
                        if result.returncode == 0:
                            output = result.stdout
                        ...
                        logger.warning(f"通过acpi获取电池信息失败: {e!s}")
                ...
                try:
                    ...
                    logger.warning(f"通过WMI获取电池信息失败: {e!s}")
                    ...
                    try:
                        ...
                        logger.warning(f"通过PowerShell获取电池信息失败: {e!s}")
            ...
            if battery_level is None:
                try:
                    ...
                    logger.warning(f"通过平台桥接获取电池信息失败: {e!s}")
        ...
        try:
            ...
            with self.cache_lock:
                if user_id in self.data_cache:
                    # 检查最近的睡眠数据
                    if (
                        "sleep" in self.data_cache[user_id]
                        and self.data_cache[user_id]["sleep"]
                    ):
                    ...
                    # 检查最近的活动数据
                    if (
                        "activity" in self.data_cache[user_id]
                        and self.data_cache[user_id]["activity"]
                    ):
                        recent_activity = self.data_cache[user_id]["activity"][-1]
                        if (
                            isinstance(recent_activity, dict)
                            and "data" in recent_activity
                        ):
    ...
    def _collect_pulse_data(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            "pulse_rate": 75
            + hash(f"{user_id}:{time.time()}") % 10,  # 模拟心率在75-84之间
            "rhythm": "regular",
            "strength": "moderate",
        }
    ...
    def _collect_sleep_data(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "duration": (
                0
                if not is_night
                else (current_hour - 22 if current_hour >= 22 else current_hour + 8)
            ),
        }
    ...
    def _collect_activity_data(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            "step_count": (
                hash(f"{user_id}:{time.time()}") % 100 if is_active_time else 0
            ),
    ...
    def _collect_voice_data(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            "speech_detected": hash(f"{user_id}:{time.time()}") % 10 > 7
            and is_speaking_time,  # 70%几率检测到说话
            "voice_features": {
                "pitch": 220 + hash(f"{user_id}") % 40,  # 基频
                "energy": 0.6 if is_speaking_time else 0.2,
            },
            "duration": (
                hash(f"{user_id}:{time.time()}") % 60 if is_speaking_time else 0
            ),  # 说话持续时间0-60秒
        }

    def get_recent_data(
        self, user_id: str, data_type: str, limit: int = 10
    ) -> dict[str, Any]:
        """
        获取用户最近的特定类型数据
        ...
        Returns:
            包含最近数据的字典
        """
        result = {"success": False, "message": "未找到数据", "data": []}

        try:
            with self.cache_lock:
                if (
                    user_id in self.data_cache
                    and data_type in self.data_cache[user_id]
                    and self.data_cache[user_id][data_type]
                ):
                    ...
                    for item in recent_data:
                        if (
                            isinstance(item, str)
                            and self.privacy_service
                            and self.encryption_key
                        ):
                            try:
                                decrypted_item = self.privacy_service.decrypt_data(
                                    item, self.encryption_key
                                )
            ...
            logger.error(f"获取用户 {user_id} 的 {data_type} 数据失败: {e!s}")
            result["message"] = f"获取数据失败: {e!s}"
            return result
    ...
    def clear_user_data(
        ...
        try:
            ...
            logger.error(f"清除用户 {user_id} 数据失败: {e!s}")
            return {"success": False, "message": f"清除数据失败: {e!s}"}
    ...
    def export_user_data(
        ...
        try:
            with self.cache_lock:
                ...
                logger.info(
                    f"已导出用户 {user_id} 的数据类型: {list(export_data.keys())}"
                )
                return {
                    "success": True,
                    "message": "成功导出数据",
            ...
            logger.error(f"导出用户 {user_id} 数据失败: {e!s}")
            return {"success": False, "message": f"导出数据失败: {e!s}"}
