...
class AccessibilityServiceFactory(SingletonServiceFactory):
        ...
        try:
            ...
            if config_manager:
                ...
                self._service_configs['BlindAssistanceServiceImpl'] = {
                    'enabled': config_manager.get('services.blind_assistance.enabled', True),
                    'model_config': config_manager.get('services.blind_assistance.model', {}),
                    'cache_ttl': config_manager.get('services.blind_assistance.cache_ttl', 3600),
                    'max_concurrent_requests': config_manager.get('services.blind_assistance.max_concurrent', 10)
                }
                
                # 手语识别服务配置
                self._service_configs['SignLanguageServiceImpl'] = {
                    'enabled': config_manager.get('services.sign_language.enabled', True),
                    'model_config': config_manager.get('services.sign_language.model', {}),
                    'cache_ttl': config_manager.get('services.sign_language.cache_ttl', 1800),
                    'max_concurrent_requests': config_manager.get('services.sign_language.max_concurrent', 5)
                }
                
                # 屏幕阅读服务配置
                self._service_configs['ScreenReadingServiceImpl'] = {
                    'enabled': config_manager.get('services.screen_reading.enabled', True),
                    'voice_config': config_manager.get('services.screen_reading.voice', {}),
                    'cache_ttl': config_manager.get('services.screen_reading.cache_ttl', 600),
                    'max_concurrent_requests': config_manager.get('services.screen_reading.max_concurrent', 15)
                }
                
                # 语音辅助服务配置
                self._service_configs['VoiceAssistanceServiceImpl'] = {
                    'enabled': config_manager.get('services.voice_assistance.enabled', True),
                    'model_config': config_manager.get('services.voice_assistance.model', {}),
                    'cache_ttl': config_manager.get('services.voice_assistance.cache_ttl', 1200),
                    'max_concurrent_requests': config_manager.get('services.voice_assistance.max_concurrent', 8)
                }
                
                # 内容转换服务配置
                self._service_configs['ContentConversionServiceImpl'] = {
                    'enabled': config_manager.get('services.content_conversion.enabled', True),
                    'conversion_config': config_manager.get('services.content_conversion.config', {}),
                    'cache_ttl': config_manager.get('services.content_conversion.cache_ttl', 7200),
                    'max_concurrent_requests': config_manager.get('services.content_conversion.max_concurrent', 12)
                }
            ...
            for service_name in core_services:
                if self._service_configs.get(service_name, {}).get('enabled', True):
                    try:
                        ...
                        if service_class:
                            await self.create_service(service_class, self._service_configs[service_name])
                            logger.info(f"预创建核心服务: {service_name}")
                    except Exception as e:
                        logger.warning(f"预创建服务失败: {service_name}, 错误: {str(e)}")
                        
        except Exception as e:
            logger.error(f"预创建核心服务失败: {str(e)}")
    ...
    async def _get_service_class(self, service_name: str):
        ...
        try:
            ...
            logger.error(f"导入服务类失败: {service_name}, 错误: {str(e)}")
            return None
    ...
    async def reload_service_config(self, service_name: str):
        ...
        try:
            ...
            logger.error(f"重新加载服务配置失败: {service_name}, 错误: {str(e)}")
            raise 