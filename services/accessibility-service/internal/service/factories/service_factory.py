...
class ServiceFactory(ABC):
    ...
    async def create_service(
        self, service_type: type[T], config: dict[str, Any] = None
    ) -> T:
        """
        创建服务实例
        ...
        try:
            ...
            if mapped_service_name:
                try:
                    ...
                    if service:
                        ...
                        logger.debug(
                            f"从容器获取服务: {service_name} -> {mapped_service_name}"
                        )
                        return service
                except ValueError:
                    # 服务未注册，继续创建新实例
            ...
            logger.error(f"创建服务失败: {service_name}, 错误: {e!s}")
            raise
        ...
        if hasattr(service_type, "__annotations__"):
            for param_name, param_type in service_type.__annotations__.items():
                ...
                try:
                    ...
                    logger.warning(
                        f"解析依赖失败: {param_name} -> {param_type}, 错误: {e!s}"
                    )

        return dependencies

    def _get_mapped_service_name(self, service_name: str) -> str | None:
        """
        获取映射的服务名称
        ...
        if service_name in self._service_cache:
            ...
            if hasattr(service, "cleanup") and callable(service.cleanup):
                try:
                    ...
                    logger.warning(f"服务清理失败: {service_name}, 错误: {e!s}")
...
class PrototypeServiceFactory(ServiceFactory):
    ...
    async def create_service(
        ...
        try:
            ...
            logger.error(f"创建原型服务失败: {service_type.__name__}, 错误: {e!s}")
            raise
