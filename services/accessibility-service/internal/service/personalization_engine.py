...
class PreferenceLearner:
    ...
    def learn_from_interaction(self, user_id: str, interaction_data: Dict[str, Any]) -> List[UserPreference]:
        """从用户交互中学习偏好"""
        try:
            # 记录交互历史
            ...
            for algorithm_name, algorithm_func in self.learning_algorithms.items():
                try:
                    ...
                    logger.error(f"学习算法 {algorithm_name} 执行失败: {str(e)}")
    ...
    def _frequency_based_learning(self, user_id: str, interaction_data: Dict[str, Any]) -> List[UserPreference]:
        """基于频率的学习"""
        preferences = []
        ...
        if action_type:
            ...
            if pattern_key not in self.preference_patterns[user_id]:
                self.preference_patterns[user_id][pattern_key] = {"count": 0, "total_time": 0}
            
            self.preference_patterns[user_id][pattern_key]["count"] += 1
            self.preference_patterns[user_id][pattern_key]["total_time"] += interaction_data.get("duration", 0)
            ...
            if self.preference_patterns[user_id][pattern_key]["count"] >= 5:
                preference = UserPreference(
                    ...
                    confidence=min(1.0, self.preference_patterns[user_id][pattern_key]["count"] / 20.0),
    ...
    def _pattern_recognition_learning(self, user_id: str, interaction_data: Dict[str, Any]) -> List[UserPreference]:
        """模式识别学习"""
        preferences = []
    ...
    def _collaborative_filtering_learning(self, user_id: str, interaction_data: Dict[str, Any]) -> List[UserPreference]:
        """协同过滤学习"""
        preferences = []
    ...
    def _reinforcement_learning(self, user_id: str, interaction_data: Dict[str, Any]) -> List[UserPreference]:
        """强化学习"""
        preferences = []
    ...
    def _get_user_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        # 计算用户特征
        action_types = [i.get("action_type") for i in interactions if i.get("action_type")]
        most_common_actions = {}
        ...
        return {
            ...
            "activity_hours": [datetime.fromtimestamp(i["timestamp"]).hour for i in interactions],
            "avg_session_duration": np.mean([i.get("duration", 0) for i in interactions])
        }
    
    def _find_similar_users(self, user_id: str, user_profile: Dict[str, Any]) -> List[str]:
        """找到相似用户"""
        # 简化实现：返回一些模拟的相似用户
        # 在实际实现中，这里会有复杂的相似度计算
...
class AdaptationEngine:
    ...
    def _initialize_adaptation_rules(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "assistance_level": {
                "trigger_conditions": ["user_expertise_increase", "successful_completions"],
    ...
    def evaluate_adaptation_needs(self, user_id: str, user_context: Dict[str, Any], 
                                 user_preferences: List[UserPreference]) -> List[AdaptationAction]:
        """评估适应需求"""
        try:
            adaptation_actions = []
            
            for rule_name, rule in self.adaptation_rules.items():
                if self._check_trigger_conditions(rule["trigger_conditions"], user_context):
            ...
            logger.error(f"评估适应需求失败: {str(e)}")
            return []
    
    def _check_trigger_conditions(self, conditions: List[str], user_context: Dict[str, Any]) -> bool:
        """检查触发条件"""
        for condition in conditions:
            if condition == "low_light_environment":
                light_level = user_context.get("sensor_data", {}).get("light", {}).get("lux", 500)
                if light_level < 100:
                    return True
    ...
    def _create_adaptation_action(self, user_id: str, rule_name: str, rule: Dict[str, Any],
                                user_context: Dict[str, Any], 
                                user_preferences: List[UserPreference]) -> Optional[AdaptationAction]:
        """创建适应动作"""
        try:
            # 计算期望收益
            expected_benefit = self._calculate_expected_benefit(rule_name, user_context, user_preferences)
            ...
            logger.error(f"创建适应动作失败: {str(e)}")
            return None
    
    def _calculate_expected_benefit(self, rule_name: str, user_context: Dict[str, Any],
                                  user_preferences: List[UserPreference]) -> float:
        """计算期望收益"""
        base_benefit = 0.5
    ...
    def _calculate_adaptation_confidence(self, rule_name: str, user_context: Dict[str, Any]) -> float:
        """计算适应置信度"""
        base_confidence = 0.6
...
class PersonalizationEngine:
    ...
    async def process_user_interaction(self, user_id: str, interaction_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            # 学习用户偏好
            learned_preferences = self.preference_learner.learn_from_interaction(user_id, interaction_data)
    ...
    def _update_user_preference(self, user_id: str, new_preference: UserPreference):
        ...
        if existing_pref:
            ...
            existing_pref.confidence = (existing_pref.confidence + new_preference.confidence) / 2
    ...
    def _calculate_personalization_level(self, user_id: str) -> float:
        ...
        # 基于偏好类型多样性
        unique_types = len(set(p.preference_type for p in preferences))
    ...
    def record_user_feedback(self, user_id: str, adaptation_id: str, feedback: str, rating: float):
        """记录用户反馈"""
        try:
            # 找到对应的适应动作
            ...
            self.stats["user_satisfaction"][user_id] = (current_satisfaction + rating) / 2
            
            logger.info(f"用户 {user_id} 对适应 {adaptation_id} 的反馈: {feedback} (评分: {rating})")
    ...
    def import_user_preferences(self, user_id: str, import_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(f"导入用户偏好失败: {str(e)}")
            return False
    
    def get_stats(self) -> Dict[str, Any]:
        """获取统计信息"""
        total_users = len(self.user_preferences)
        avg_satisfaction = np.mean(list(self.stats["user_satisfaction"].values())) if self.stats["user_satisfaction"] else 0.0
