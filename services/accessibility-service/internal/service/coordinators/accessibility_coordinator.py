...
from ..decorators import error_handler, performance_monitor, trace
...
class AccessibilityServiceCoordinator:
    ...
    def __init__(self, service_factory: AccessibilityServiceFactory):
        ...
        # 服务实例缓存
        self._blind_assistance_service: IBlindAssistanceService | None = None
        self._sign_language_service: ISignLanguageService | None = None
        self._screen_reading_service: IScreenReadingService | None = None
        self._voice_assistance_service: IVoiceAssistanceService | None = None
        self._content_conversion_service: IContentConversionService | None = None
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.error(f"无障碍服务协调器初始化失败: {e!s}")
            raise
    ...
    async def _preload_services(self):
        ...
        try:
            # 预加载导盲服务
            self._blind_assistance_service = (
                await self.service_factory.create_blind_assistance_service()
            )

            # 预加载语音辅助服务
            self._voice_assistance_service = (
                await self.service_factory.create_voice_assistance_service()
            )

            logger.info("核心服务预加载完成")

        except Exception as e:
            logger.warning(f"核心服务预加载失败: {e!s}")
    ...
    async def recognize_sign_language(
        ...
        try:
            ...
            if not self._sign_language_service:
                self._sign_language_service = (
                    await self.service_factory.create_sign_language_service()
                )
    ...
    async def read_screen(
        ...
        try:
            ...
            if not self._screen_reading_service:
                self._screen_reading_service = (
                    await self.service_factory.create_screen_reading_service()
                )
    ...
    async def convert_content(
        ...
        try:
            ...
            if not self._content_conversion_service:
                self._content_conversion_service = (
                    await self.service_factory.create_content_conversion_service()
                )
    ...
    async def _coordinate_voice_response(self, voice_result: dict, user_id: str):
        ...
        try:
            ...
            logger.warning(f"语音响应协调失败: {e!s}")

    async def _coordinate_screen_reading_voice(
        self, screen_result: dict, preferences: dict
    ):
        """
        协调屏幕阅读的语音输出
        ...
        try:
            ...
            logger.warning(f"屏幕阅读语音协调失败: {e!s}")
    ...
    async def comprehensive_assistance(self, request_data: dict, user_id: str) -> dict:
        ...
        try:
            ...
            if request_type == "scene_analysis_with_voice":
                ...
                # 语音播报
                advice_text = scene_result.get("navigation_advice", {}).get(
                    "primary_advice", ""
                )
                if advice_text:
                    voice_preferences = preferences.get("voice", {})
    ...
    async def reload_service(self, service_name: str):
        ...
        try:
            ...
            # 重新加载服务配置
            await self.service_factory.reload_service_config(
                f"{service_name.title().replace('_', '')}ServiceImpl"
            )

            logger.info(f"服务重新加载完成: {service_name}")

        except Exception as e:
            logger.error(f"服务重新加载失败: {service_name}, 错误: {e!s}")
            raise
    ...
    async def cleanup(self):
        ...
        try:
            ...
            logger.error(f"无障碍服务协调器清理失败: {e!s}")
            raise
