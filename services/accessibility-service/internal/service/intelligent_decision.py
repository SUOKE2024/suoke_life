...
class BehaviorLearningEngine:
    ...
    def learn_from_data(
        ...
        try:
            ...
            self._update_behavior_patterns(
                user_id, activity, time_features, sensor_features, location_features
            )

        except Exception as e:
            logger.error(f"行为学习失败: {e!s}")
    ...
    def _extract_sensor_features(
        ...
        for sensor_type, data in sensor_data.items():
            if isinstance(data, list) and data:
                # 计算统计特征
                values = [
                    d.get("values", [0])[0] if isinstance(d, dict) else d for d in data
                ]
    ...
    def _update_behavior_patterns(
        ...
        for pattern in self.user_patterns[user_id]:
            if pattern.pattern_type == activity and self._is_similar_time_pattern(
                pattern.time_patterns, time_features
            ):
        ...
        if existing_pattern:
            ...
            self._update_existing_pattern(
                existing_pattern, time_features, sensor_features, location_features
            )
        else:
            # 创建新模式
    ...
    def _is_similar_time_pattern(
        self, existing_patterns: list[dict[str, Any]], new_features: dict[str, Any]
    ) -> bool:
        """判断时间模式是否相似"""
    ...
    def _update_existing_pattern(
        ...
        for sensor_type, features in sensor_features.items():
            ...
            if len(pattern.sensor_patterns[sensor_type]) > 400:  # 保持最近400个特征
                pattern.sensor_patterns[sensor_type] = pattern.sensor_patterns[
                    sensor_type
                ][-400:]
    ...
    def predict_next_activity(
        ...
        for pattern in self.user_patterns[user_id]:
            score = self._calculate_pattern_match_score(
                pattern, current_hour, current_context
            )
            if score > best_score:
                best_score = score
                best_match = pattern
    ...
    def _calculate_pattern_match_score(
        self, pattern: UserBehaviorPattern, current_hour: int, context: dict[str, Any]
    ) -> float:
        """计算模式匹配分数"""
        score = 0.0
...
class RecommendationEngine:
    ...
    def _initialize_recommendation_rules(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            "low_light_environment": {
                "condition": lambda ctx: ctx.sensor_data.get("light", {}).get(
                    "lux", 1000
                )
                < 50,
                "recommendation": {
            ...
            "high_noise_environment": {
                "condition": lambda ctx: ctx.sensor_data.get("microphone", {}).get(
                    "volume_db", 0
                )
                > 70,
                "recommendation": {
            ...
            "motion_detected": {
                "condition": lambda ctx: ctx.sensor_data.get("accelerometer", {}).get(
                    "is_moving", False
                ),
                "recommendation": {
            ...
            "outdoor_location": {
                "condition": lambda ctx: ctx.location_data.get("location_type")
                == "outdoor",
                "recommendation": {
            ...
            "work_hours": {
                "condition": lambda ctx: 9
                <= datetime.fromtimestamp(ctx.timestamp).hour
                <= 17,
                "recommendation": {
    ...
    def generate_recommendations(self, context: DecisionContext) -> list[Decision]:
        ...
        try:
            ...
            # 去重和排序
            recommendations = self._deduplicate_and_rank_recommendations(
                recommendations
            )

        except Exception as e:
            logger.error(f"生成推荐失败: {e!s}")
    ...
    def _apply_recommendation_rules(self, context: DecisionContext) -> list[Decision]:
        ...
        for rule_name, rule in self.recommendation_rules.items():
            try:
                ...
                logger.error(f"规则 {rule_name} 执行失败: {e!s}")
    ...
    def _generate_pattern_based_recommendations(
        ...
        elif current_hour >= 22 or current_hour <= 6:  # 夜晚
            decision = Decision(
    ...
    def _deduplicate_and_rank_recommendations(
        self, recommendations: list[Decision]
    ) -> list[Decision]:
        """去重和排序推荐"""
        # 按动作类型去重
        seen_actions = set()
    ...
    def record_feedback(
        self, user_id: str, decision_id: str, feedback: str, rating: int
    ):
        """记录用户反馈"""
        feedback_entry = {
...
class IntelligentDecisionEngine:
    ...
    async def make_decision(self, context: DecisionContext) -> list[Decision]:
        ...
        try:
            ...
            # 过滤和优化决策
            final_decisions = self._filter_and_optimize_decisions(
                all_decisions, context
            )
            ...
            logger.info(
                f"为用户 {context.user_id} 生成了 {len(final_decisions)} 个决策"
            )
            return final_decisions

        except Exception as e:
            logger.error(f"智能决策失败: {e!s}")
            return []
    ...
    def _generate_automation_decisions(
        self, context: DecisionContext
    ) -> list[Decision]:
        """生成自动化决策"""
    ...
    def _filter_and_optimize_decisions(
        self, decisions: list[Decision], context: DecisionContext
    ) -> list[Decision]:
        """过滤和优化决策"""
        if not decisions:
            return []
        ...
        valid_decisions = [
            d for d in decisions if d.expires_at is None or d.expires_at > current_time
        ]
    ...
    def record_decision_feedback(
        ...
        self.recommendation_engine.record_feedback(
            user_id, decision_id, feedback, rating
        )
