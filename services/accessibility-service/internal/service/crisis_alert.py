...
class CrisisAlertService:
    ...
    def __init__(self, config: Any):
        ...
        self.config = config
        self.enabled = (
            config.crisis_alert.enabled if hasattr(config, "crisis_alert") else False
        )
    ...
    def _init_default_alert_handlers(self) -> None:
        """初始化默认的警报处理器"""
        self.alert_handlers = {
    ...
    def set_user_thresholds(
        self, user_id: str, thresholds: dict[str, Any]
    ) -> dict[str, Any]:
        """
        设置用户的警报阈值
    ...
    def get_user_thresholds(self, user_id: str) -> dict[str, Any]:
        ...
        return {"success": True, "thresholds": self.alert_thresholds[user_id]}

    def set_emergency_contacts(
        self, user_id: str, contacts: list[dict[str, Any]]
    ) -> dict[str, Any]:
        """
        设置用户的紧急联系人
    ...
    def get_emergency_contacts(self, user_id: str) -> dict[str, Any]:
        ...
        return {"success": True, "contacts": self.alert_contacts[user_id]}
    ...
    def get_alert_history(self, user_id: str, limit: int = 20) -> dict[str, Any]:
        ...
        alerts = sorted(
            self.alert_history[user_id], key=lambda x: x["timestamp"], reverse=True
        )[:limit]
    ...
    def start(self) -> None:
        ...
        self.alert_processor_thread = threading.Thread(
            target=self._alert_processor_worker, daemon=True, name="alert-processor"
        )
    ...
    def stop(self) -> None:
        ...
        if hasattr(self.background_collection_service, "_original_cache_data"):
            self.background_collection_service._cache_data = (
        ...
        while not self.stopping:
            try:
                ...
                if self.background_collection_service:
                    collection_status = (
                        self.background_collection_service.get_collection_status(
                            user_id
                        )
                    )
                ...
                logger.error(f"用户 {user_id} 的警报监控出错: {e!s}")
    ...
    def _check_user_data(self, user_id: str, data_cache: dict[str, list[dict]]):
        ...
        """
        # 从数据采集服务获取最新缓存
        if self.background_collection_service and hasattr(
            self.background_collection_service, "data_cache"
        ):
            ...
            for data_type, data_list in user_cache.items():
                ...
                if data_list and len(data_list) > 0:
                    ...
                    self._process_collected_data(
                        user_id, data_type, latest_data.get("data", {})
                    )

    def _process_collected_data(
        self, user_id: str, data_type: str, data: dict[str, Any]
    ):
        """
        处理采集的数据，分析是否需要触发警报
        ...
        try:
            # 获取用户的警报阈值
            if (
                user_id not in self.alert_thresholds
                or data_type not in self.alert_thresholds.get(user_id, {})
            ):
                logger.debug(
                    f"用户 {user_id} 未设置 {data_type} 数据的警报阈值，使用默认阈值"
                )
                thresholds = {}
            else:
            ...
            if alert_info:
                logger.info(
                    f"数据分析触发警报: 用户={user_id}, 类型={data_type}, 级别={alert_info.get('level', 'info')}"
                )
                # 添加一些基本信息
            ...
            logger.error(f"处理 {data_type} 数据出错: {e!s}")
    ...
    def _handle_info_alert(self, user_id: str, alert_info: dict[str, Any]):
        """处理信息级别警报"""
        logger.info(
            f"信息级别警报: 用户={user_id}, 内容={alert_info.get('message', '')}"
        )

        # 记录指标
        if self.monitoring_service:
            self.monitoring_service.metrics_client.counter(
                "crisis_alerts", 1, {"level": "info"}
            )
    ...
    def _handle_warning_alert(self, user_id: str, alert_info: dict[str, Any]):
        """处理警告级别警报"""
        logger.warning(
            f"警告级别警报: 用户={user_id}, 内容={alert_info.get('message', '')}"
        )

        # 记录指标
        if self.monitoring_service:
            self.monitoring_service.metrics_client.counter(
                "crisis_alerts", 1, {"level": "warning"}
            )
    ...
    def _handle_danger_alert(self, user_id: str, alert_info: dict[str, Any]):
        """处理危险级别警报"""
        logger.error(
            f"危险级别警报: 用户={user_id}, 内容={alert_info.get('message', '')}"
        )

        # 记录指标
        if self.monitoring_service:
            self.monitoring_service.metrics_client.counter(
                "crisis_alerts", 1, {"level": "danger"}
            )
    ...
    def _handle_critical_alert(self, user_id: str, alert_info: dict[str, Any]):
        """处理严重级别警报"""
        logger.critical(
            f"严重级别警报: 用户={user_id}, 内容={alert_info.get('message', '')}"
        )

        # 记录指标
        if self.monitoring_service:
            self.monitoring_service.metrics_client.counter(
                "crisis_alerts", 1, {"level": "critical"}
            )
    ...
    def _notify_emergency_contacts(self, user_id: str, alert_info: dict[str, Any]):
        ...
        for contact in self.alert_contacts[user_id]:
            ...
            if self._should_notify_contact(alert_level, notify_level):
                ...
                logger.info(
                    f"已通知紧急联系人: {contact.get('name')} ({contact.get('relation')})"
                )
    ...
    def _should_notify_contact(self, alert_level: str, notify_level: str) -> bool:
        ...
        Returns:
            是否应该通知
        """
        level_order = {"info": 0, "warning": 1, "danger": 2, "critical": 3}
    ...
    # 数据分析器
    def _analyze_pulse_data(
        self, user_id: str, data: dict[str, Any], thresholds: dict[str, Any]
    ) -> dict[str, Any] | None:
        """
        分析脉搏数据
        ...
        # 判断警报级别
        if (
            pulse_rate <= default_thresholds["min_critical"]
            or pulse_rate >= default_thresholds["max_critical"]
        ):
            return {
                "level": "critical",
            ...
            pulse_rate <= default_thresholds["min_danger"]
            or pulse_rate >= default_thresholds["max_danger"]
        ):
            return {
                "level": "danger",
            ...
            pulse_rate <= default_thresholds["min_warning"]
            or pulse_rate >= default_thresholds["max_warning"]
        ):
            return {
                "level": "warning",
            ...
            pulse_rate < default_thresholds["min_normal"]
            or pulse_rate > default_thresholds["max_normal"]
        ):
            return {
                "level": "warning",
    ...
    def _analyze_activity_data(
        ...
        if (
            activity_type == "fall"
            and data.get("fall_probability", 0) >= default_thresholds["fall_detection"]
        ):
            return {
                "level": "critical",
                ...
                "data_values": {
                    ...
                    "fall_probability": data.get("fall_probability"),
                },
            }
        ...
        if (
            ...
            and duration >= default_thresholds["no_movement_duration"]
        ):
            return {
                "level": "warning",
        ...
        # 过长高强度活动检测
        if (
            activity_type in ["running", "exercise"]
            and intensity > 0.7
            and duration >= default_thresholds["high_intensity_duration"]
        ):
            return {
                "level": "info",
    ...
    def _analyze_voice_data(
        ...
        if (
            "clarity_change" in voice_features
            and abs(voice_features["clarity_change"])
            >= default_thresholds["speech_clarity_change"]
        ):
            change_direction = (
                "下降" if voice_features["clarity_change"] < 0 else "上升"
            )
            return {
                "level": "info",
    ...
    def integrate_with_agents(self, agent_coordination_service):
        ...
        if hasattr(agent_coordination_service, "event_bus"):
            agent_coordination_service.event_bus.subscribe(
                "agent.health_recommendation", self._handle_agent_recommendation
            )
    ...
    def request_agent_assistance(self, user_id: str, alert_info: dict[str, Any]):
        ...
        response = self.agent_coordination.request_assistance(
            ...
            context={
                ...
                "alert_history": self.get_alert_history(user_id, limit=5).get(
                    "alerts", []
                ),
            ...
            priority=(
                "high"
                if alert_info.get("level") in ["danger", "critical"]
                else "normal"
            ),
        )
