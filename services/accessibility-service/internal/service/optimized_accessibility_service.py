...
class OptimizedAccessibilityService:
    ...
    def _init_modules(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"已启用 {len(enabled_modules)} 个模块: {', '.join(enabled_modules)}"
            )

        except Exception as e:
            logger.error(f"模块初始化失败: {e!s}")
            raise

    async def blind_assistance(
        ...
        if not self.config.modules.blind_assistance.enabled:
            return {"success": False, "error": "导盲服务模块未启用", "user_id": user_id}
        ...
        try:
            ...
            logger.error(f"导盲服务处理失败: {e!s}")
            return {
                "success": False,
                "error": f"导盲服务处理失败: {e!s}",
                "user_id": user_id,
            }
    ...
    async def speech_translation(
        ...
        if not self.config.modules.translation.enabled:
            return {"success": False, "error": "翻译服务模块未启用", "user_id": user_id}
        ...
        try:
            ...
            self._stats["requests_failed"] += 1
            logger.error(f"语音翻译处理失败: {e!s}")
            return {
                "success": False,
                "error": f"语音翻译处理失败: {e!s}",
                "user_id": user_id,
            }
    ...
    def get_service_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "modules": {
                "blind_assistance": {
                    "enabled": self.config.modules.blind_assistance.enabled
                },
                "sign_language": {"enabled": self.config.modules.sign_language.enabled},
                "voice_assistance": {
                    "enabled": self.config.modules.voice_assistance.enabled
                },
                "screen_reading": {
                    "enabled": self.config.modules.screen_reading.enabled
                },
                "content_conversion": {
                    "enabled": self.config.modules.content_conversion.enabled
                },
    ...
    def reload_config(self) -> None:
        ...
        try:
            ...
            logger.error(f"配置重新加载失败: {e!s}")
            raise

    def get_supported_languages(self) -> dict[str, Any]:
        """获取支持的语言列表"""
        if not self.config.modules.translation.enabled:
            return {"success": False, "error": "翻译服务模块未启用"}
        ...
        try:
            ...
            logger.error(f"获取支持语言失败: {e!s}")
            return {"success": False, "error": f"获取支持语言失败: {e!s}"}
