...
class ErrorClassifier:
    ...
    def _setup_default_rules(self):
        ...
        self._rules = [
            ...
            # 数据库错误
            {
                'exceptions': ['psycopg2.OperationalError', 'sqlalchemy.exc.OperationalError'],
    ...
    def add_rule(self, exceptions: List[Union[Type[Exception], str]], 
...
class RetryStrategy:
    ...
    def exponential_backoff(attempt: int, base_delay: float, max_delay: float, 
                          exponential_base: float, jitter: bool) -> float:
        """指数退避"""
    ...
    @staticmethod
    def linear_backoff(attempt: int, base_delay: float, max_delay: float, 
                      **kwargs) -> float:
        """线性退避"""
...
class ErrorHandler:
    ...
    async def _retry_with_config(self, func: Callable, config: RetryConfig, 
                               *args, **kwargs):
        """使用配置重试"""
        last_exception = None
        ...
        for attempt in range(config.max_attempts):
            try:
                ...
                if attempt < config.max_attempts - 1:
                    ...
                    logger.warning(f"重试 {func.__name__} (第{attempt + 1}次), "
                                 f"延迟 {delay:.2f}s, 错误: {str(e)}")
    ...
    def _should_retry(self, exception: Exception, config: RetryConfig, 
                     attempt: int) -> bool:
        """判断是否应该重试"""
        # 检查是否达到最大重试次数
        if attempt >= config.max_attempts - 1:
    ...
    async def _handle_final_error(self, function_name: str, error_info: ErrorInfo):
        """处理最终错误"""
        logger.error(f"函数 {function_name} 最终失败: "
                    f"类别={error_info.category.value}, "
                    f"严重程度={error_info.severity.value}, "
                    f"错误={str(error_info.exception)}")
