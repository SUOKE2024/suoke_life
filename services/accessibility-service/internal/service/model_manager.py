...
class ModelManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self._max_memory_usage = config.get('model_manager', {}).get('max_memory_mb', 4096) * 1024 * 1024
        self._cleanup_interval = config.get('model_manager', {}).get('cleanup_interval_seconds', 300)
        self._model_ttl = config.get('model_manager', {}).get('model_ttl_seconds', 1800)
        
        # 启动清理任务
        self._start_cleanup_task()
        
        logger.info(f"模型管理器初始化完成，最大内存使用: {self._max_memory_usage // 1024 // 1024}MB")
        ...
        async def cleanup_loop():
            while True:
                try:
                    ...
                    logger.error(f"模型清理任务异常: {str(e)}")
        
        self._cleanup_task = asyncio.create_task(cleanup_loop())
    
    async def load_model(self, model_name: str, model_config: Dict[str, Any] = None) -> Any:
        """
        加载模型（懒加载）
        ...
        async with self._loading_locks[model_name]:
            ...
            if model_name in self._models:
                ...
                if model_info.status == ModelStatus.LOADED:
                    ...
                    if model_info.status == ModelStatus.LOADED:
                        ...
                        raise RuntimeError(f"模型加载失败: {model_name}, 错误: {model_info.error_message}")
            ...
            try:
                ...
                logger.info(f"模型加载完成: {model_name}, 耗时: {load_time:.2f}s, 内存: {memory_usage // 1024 // 1024}MB")
    ...
    async def unload_model(self, model_name: str) -> bool:
        ...
        async with self._lock:
            ...
            try:
                ...
                logger.error(f"模型卸载失败: {model_name}, 错误: {str(e)}")
                model_info.status = ModelStatus.ERROR
                model_info.error_message = str(e)
                return False
    ...
    async def get_model_status(self, model_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'memory_usage_mb': model_info.memory_usage // 1024 // 1024 if model_info.memory_usage else None,
    ...
    async def _load_transformers_model(self, model_name: str, config: Dict[str, Any]) -> Any:
        """加载Transformers模型"""
        try:
            ...
            if config.get('has_processor', False):
                processor = await loop.run_in_executor(
                    None,
                    lambda: AutoProcessor.from_pretrained(model_path)
                )
            else:
                processor = None
            ...
            model = await loop.run_in_executor(
                None,
                lambda: AutoModel.from_pretrained(model_path)
            )
    ...
    async def _load_opencv_model(self, model_name: str, config: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            net = await loop.run_in_executor(
                None,
                lambda: cv2.dnn.readNet(model_path)
            )
    ...
    async def _load_mediapipe_model(self, model_name: str, config: Dict[str, Any]) -> Any:
        """加载MediaPipe模型"""
        try:
    ...
    async def _check_memory_usage(self):
        ...
        try:
            ...
            if current_memory > self._max_memory_usage:
                logger.warning(f"内存使用超限: {current_memory // 1024 // 1024}MB > {self._max_memory_usage // 1024 // 1024}MB")
                await self._cleanup_unused_models()
        except Exception as e:
        ...
        async with self._lock:
            for model_name, model_info in self._models.items():
                if (model_info.status == ModelStatus.LOADED and 
                    ...
                    current_time - model_info.last_used > self._model_ttl):
