        ...
        try:
            ...
            if self.computing_manager.is_library_available("scipy"):
                ...
                if len(data.flatten()) > 3:
                    shapiro_stat, shapiro_p = scipy_stats.shapiro(
                        data.flatten()[:5000]
                    )  # 限制样本数量
            ...
            if self.computing_manager.is_library_available("sklearn"):
                ...
                result["advanced_detection"] = {
                    ...
                    "elliptic_envelope": {
                        ...
                        "outlier_indices": np.where(outliers_elliptic == -1)[
                            0
                        ].tolist(),
                    },
                }
...
class MachineLearningEngine:
    ...
    def cluster_data(self, X: np.ndarray, n_clusters: int = 3) -> dict[str, Any]:
        ...
        try:
            if not self.computing_manager.is_library_available("sklearn"):
                # 简单的K-means实现
                centroids = X[np.secrets.choice(X.shape[0], n_clusters, replace=False)]
...
class VisualizationEngine:
    ...
    def create_plot_data(
        self, data: np.ndarray, plot_type: str = "line"
    ) -> dict[str, Any]:
        """创建绘图数据"""
        try:
            if plot_type == "line":
                ...
                plot_data = {"x": x.tolist(), "y": y.tolist(), "type": "line"}
                ...
                if self.computing_manager.is_library_available("numpy"):
                    ...
                    for value in data:
                        bin_idx = min(
                            19, int((value - min_val) / (max_val - min_val) * 20)
                        )
                        hist[bin_idx] += 1
                ...
                if data.ndim >= 2:
                    ...
                    x = np.arange(len(data))
                    plot_data = {"x": x.tolist(), "y": data.tolist(), "type": "scatter"}
...
class ScientificComputingService:
    ...
    def get_service_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "coverage_percentage": sum(available_libs.values())
            / len(available_libs)
            * 100,
            "timestamp": datetime.now(UTC).isoformat(),
        }
    ...
    def process_data(
        self, data: list[float], operation: str, **kwargs
    ) -> dict[str, Any]:
        """处理数据的统一接口"""
        try:
            # 转换为numpy数组
            ...
            if operation == "analyze":
                ...
                return self.signal_processing.filter_signal(
                    np_data, filter_type, cutoff
                )
            elif operation == "spectrum":
                sample_rate = kwargs.get("sample_rate", 1.0)
                return self.signal_processing.analyze_frequency_spectrum(
                    np_data, sample_rate
                )
            elif operation == "cluster":
                n_clusters = kwargs.get("n_clusters", 3)
                return self.machine_learning.cluster_data(
                    np_data.reshape(-1, 1), n_clusters
                )
            elif operation == "plot":
