...
class ResourceMonitor:
    ...
    async def _collect_process_metrics(self):
        ...
        try:
            ...
            logger.error(f"收集进程指标失败: {str(e)}")
    
    async def _collect_python_metrics(self):
        """收集Python指标"""
        try:
            # 垃圾回收统计
            ...
            for i, stats in enumerate(gc_stats):
                ...
                self.metrics.set_gauge(f"python_gc_generation_{i}_uncollectable", stats['uncollectable'])
            ...
            logger.error(f"收集Python指标失败: {str(e)}")
...
class AlertManager:
    ...
    def add_alert_rule(self, name: str, metric_name: str, condition: str, 
                      threshold: float, duration: int = 60, severity: str = "warning"):
        """添加告警规则"""
        rule = {
            'name': name,
    ...
    async def _check_alerts(self):
        ...
        for rule in self._alert_rules:
            ...
            condition_met = self._evaluate_condition(metric.value, rule['condition'], rule['threshold'])
            ...
            try:
                ...
                try:
                    result = await asyncio.wait_for(
                        self._execute_check(health_check),
                ...
                logger.error(f"健康检查任务异常: {health_check.name}, 错误: {str(e)}")
                await asyncio.sleep(health_check.interval)
