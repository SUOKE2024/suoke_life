...
class EnvironmentClassifier:
    ...
    def classify_environment(
        self, reading: EnvironmentReading, historical_data: list[EnvironmentReading]
    ) -> EnvironmentType:
        """分类环境类型"""
        try:
            scores = {}
    ...
    def _calculate_environment_score(
        ...
        rules: dict[str, Any],
        historical_data: list[EnvironmentReading],
    ) -> float:
        """计算环境类型匹配分数"""
        score = 0.0
        ...
        if temp_range[0] <= reading.temperature <= temp_range[1]:
            ...
            # 计算偏离程度
            deviation = min(
                abs(reading.temperature - temp_range[0]),
        ...
        if humidity_range[0] <= reading.humidity <= humidity_range[1]:
            ...
            deviation = min(
                abs(reading.humidity - humidity_range[0]),
        ...
        if noise_range[0] <= reading.noise_level <= noise_range[1]:
            ...
            deviation = min(
                abs(reading.noise_level - noise_range[0]),
...
class WeatherAnalyzer:
    ...
    def analyze_weather(
        ...
        try:
            # 只对户外环境进行天气分析
            if reading.metadata.get("environment_type") not in [
                "outdoor_urban",
                "outdoor_nature",
            ]:
            ...
            logger.error(f"天气分析失败: {e!s}")
            return WeatherCondition.UNKNOWN

    def _calculate_weather_score(
        self, reading: EnvironmentReading, pattern: dict[str, Any]
    ) -> float:
        """计算天气模式匹配分数"""
        score = 0.0
        factors = 0
        ...
        if "humidity_range" in pattern:
            ...
            if h_min <= reading.humidity <= h_max:
                ...
                deviation = min(
                    abs(reading.humidity - h_min), abs(reading.humidity - h_max)
                )
        ...
        if "pressure_range" in pattern:
            ...
            if p_min <= reading.pressure <= p_max:
                ...
                deviation = min(
                    abs(reading.pressure - p_min), abs(reading.pressure - p_max)
                )
                score += max(0, 1.0 - deviation / 20.0)
            factors += 1
        ...
        if "temperature_max" in pattern:
            if reading.temperature <= pattern["temperature_max"]:
                ...
                score += max(
                    0, 1.0 - (reading.temperature - pattern["temperature_max"]) / 10.0
                )
            factors += 1
...
class SafetyAnalyzer:
    ...
    def _initialize_safety_thresholds(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "humidity": {"very_dry": 20, "very_humid": 90},
            "noise": {"hearing_damage": 85, "very_loud": 80, "loud": 70},
            "air_quality": {"unhealthy": 150, "very_unhealthy": 200, "hazardous": 300},
            "uv_index": {"high": 6, "very_high": 8, "extreme": 11},
            "wind_speed": {"strong": 10, "very_strong": 15, "dangerous": 25},
        }

    def calculate_safety_score(
        self, reading: EnvironmentReading
        ...
        try:
            ...
            # 温度安全性
            temp_safety, temp_alerts = self._check_temperature_safety(
                reading.temperature
            )
            ...
            # 湿度安全性
            humidity_safety, humidity_alerts = self._check_humidity_safety(
                reading.humidity
            )
            ...
            # 空气质量安全性
            if reading.air_quality is not None:
                air_safety, air_alerts = self._check_air_quality_safety(
                    reading.air_quality
                )
...
class AccessibilityAnalyzer:
    ...
    def calculate_accessibility_score(
        self, reading: EnvironmentReading, environment_type: EnvironmentType
    ) -> tuple[float, list[str]]:
        """计算无障碍性评分"""
        try:
            ...
            accessibility_scores.append(
                visual_score
                * self.accessibility_factors["visual_accessibility"]["weight"]
            )
            recommendations.extend(visual_recs)

            # 听觉无障碍性
            auditory_score, auditory_recs = self._analyze_auditory_accessibility(
                reading
            )
            accessibility_scores.append(
                auditory_score
                * self.accessibility_factors["auditory_accessibility"]["weight"]
            )
            recommendations.extend(auditory_recs)

            # 行动无障碍性
            mobility_score, mobility_recs = self._analyze_mobility_accessibility(
                reading, environment_type
            )
            accessibility_scores.append(
                mobility_score
                * self.accessibility_factors["mobility_accessibility"]["weight"]
            )
            ...
            logger.error(f"无障碍性分析失败: {e!s}")
            return 0.5, ["无障碍性分析异常"]

    def _analyze_visual_accessibility(
        self, reading: EnvironmentReading
    ) -> tuple[float, list[str]]:
        """分析视觉无障碍性"""
        ...
        if light < factors["min_light"]:
            ...
            if score < 0.8:
                recommendations.append(
                    f"光线过强({light:.0f} lux)，建议调节照明或使用遮光"
                )
        else:
            score = 0.4
    ...
    def _analyze_auditory_accessibility(
        self, reading: EnvironmentReading
    ) -> tuple[float, list[str]]:
        """分析听觉无障碍性"""
        ...
        if noise <= factors["optimal_noise"]:
            ...
            if score < 0.8:
                recommendations.append(
                    f"背景噪音较高({noise:.0f} dB)，建议使用降噪设备"
                )
        else:
            score = 0.4
            recommendations.append(
                f"噪音过大({noise:.0f} dB)，严重影响听觉辅助设备效果"
            )

        return score, recommendations

    def _analyze_mobility_accessibility(
        self, reading: EnvironmentReading, environment_type: EnvironmentType
    ) -> tuple[float, list[str]]:
        """分析行动无障碍性"""
        ...
        if temp_range[0] <= reading.temperature <= temp_range[1]:
            ...
            if temp_score < 0.7:
                recommendations.append(
                    f"温度不适({reading.temperature:.1f}°C)，可能影响行动能力"
                )
        ...
        if humidity_range[0] <= reading.humidity <= humidity_range[1]:
            ...
            if humidity_score < 0.7:
                recommendations.append(
                    f"湿度不适({reading.humidity:.0f}%)，可能影响舒适度"
                )
        ...
        if environment_type in [
            ...
            EnvironmentType.OUTDOOR_NATURE,
        ]:
            if reading.wind_speed is not None:
                if reading.wind_speed <= factors["wind_limit"]:
                    ...
                    wind_score = max(
                        0.2, 1.0 - (reading.wind_speed - factors["wind_limit"]) / 10.0
                    )
                    if wind_score < 0.7:
                        recommendations.append(
                            f"风速过大({reading.wind_speed:.1f} m/s)，可能影响行走稳定性"
                        )
...
class EnvironmentalIntelligenceEngine:
    ...
    def analyze_environment(
        self, user_id: str, reading: EnvironmentReading
    ) -> EnvironmentAnalysis:
        """
        分析环境状况
        ...
        try:
            ...
            # 环境类型分类
            environment_type = self.environment_classifier.classify_environment(
                reading, historical_data
            )

            # 天气分析
            weather_condition = self.weather_analyzer.analyze_weather(
                reading, historical_data
            )
            ...
            # 安全性分析
            safety_score, safety_alerts = self.safety_analyzer.calculate_safety_score(
                reading
            )

            # 无障碍性分析
            accessibility_score, accessibility_recs = (
                self.accessibility_analyzer.calculate_accessibility_score(
                    reading, environment_type
                )
            )

            # 确定整体环境状况
            condition = self._determine_environment_condition(
                comfort_score, safety_score, accessibility_score
            )
            ...
            confidence = self._calculate_analysis_confidence(
                historical_data, environment_type
            )
    ...
    def _determine_environment_condition(
        self, comfort_score: float, safety_score: float, accessibility_score: float
        ...
        try:
            ...
            trends = {
                "temperature_trend": self._calculate_trend(
                    [r.temperature for r in recent_data]
                ),
                "humidity_trend": self._calculate_trend(
                    [r.humidity for r in recent_data]
                ),
                "light_trend": self._calculate_trend(
                    [r.light_level for r in recent_data]
                ),
                "noise_trend": self._calculate_trend(
                    [r.noise_level for r in recent_data]
                ),
    ...
    def predict_environment_change(
        self, user_id: str, hours_ahead: int = 1
    ) -> dict[str, Any]:
        """预测环境变化"""
        try:
            historical_data = list(self.environment_history[user_id])
