...
class PerformanceCollector:
    ...
    def _add_metric(self, name: str, value: float, tags: Dict[str, str] = None, unit: str = ""):
        """添加指标"""
...
class Tracer:
    ...
    def start_span(self, operation_name: str, kind: SpanKind = SpanKind.INTERNAL,
                   parent_context: TraceContext = None) -> Span:
        """开始一个新的Span"""
        # 获取当前上下文或创建新的
    ...
    def finish_span(self, span: Span, status: SpanStatus = SpanStatus.OK):
        ...
        for processor in self._span_processors:
            try:
                ...
                logger.error(f"Span处理器异常: {str(e)}")
...
class PerformanceMonitor:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.service_name = config.get('service', {}).get('name', 'accessibility-service')
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            self._collector.record_gauge("system.cpu.usage", cpu_percent, {"unit": "percent"})
            
            # 内存使用情况
            memory = psutil.virtual_memory()
            self._collector.record_gauge("system.memory.usage", memory.percent, {"unit": "percent"})
            self._collector.record_gauge("system.memory.available", memory.available, {"unit": "bytes"})
            
            # 进程信息
            process = psutil.Process()
            self._collector.record_gauge("process.cpu.usage", process.cpu_percent(), {"unit": "percent"})
            self._collector.record_gauge("process.memory.rss", process.memory_info().rss, {"unit": "bytes"})
            self._collector.record_gauge("process.threads", process.num_threads(), {"unit": "count"})
        ...
        def decorator(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                try:
                    ...
                    span.log(f"异常: {str(e)}", level="error")
