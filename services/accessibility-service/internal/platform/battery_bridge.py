...
class BatteryBridge:
    ...
    def get_battery_info(self, force_refresh: bool = False) -> dict[str, Any]:
        ...
        if (
            force_refresh
            or not self.battery_cache
            or (current_time - self.last_update) > self.cache_ttl
        ):
            try:
    ...
    def _get_battery_android(self) -> dict[str, Any]:
        ...
        try:
            ...
            intent = context.registerReceiver(
                None,
                jnius.autoclass("android.content.IntentFilter")(
                    jnius.autoclass("android.content.Intent").ACTION_BATTERY_CHANGED
                ),
            )

            level = intent.getIntExtra(
                jnius.autoclass("android.os.BatteryManager").EXTRA_LEVEL, -1
            )
            scale = intent.getIntExtra(
                jnius.autoclass("android.os.BatteryManager").EXTRA_SCALE, -1
            )
            ...
            status = intent.getIntExtra(
                jnius.autoclass("android.os.BatteryManager").EXTRA_STATUS, -1
            )
            is_charging = (
                status
                == jnius.autoclass("android.os.BatteryManager").BATTERY_STATUS_CHARGING
                or status
                == jnius.autoclass("android.os.BatteryManager").BATTERY_STATUS_FULL
            )

            # 获取电源模式
            power_manager = context.getSystemService(
                jnius.autoclass("android.content.Context").POWER_SERVICE
            )
            ...
            logger.warning(f"无法导入Android特定库: {e!s}，尝试使用命令行方法")
            return self._get_battery_android_shell()
        except Exception as e:
            logger.warning(f"Android API方法失败: {e!s}，尝试使用命令行方法")
            return self._get_battery_android_shell()
    ...
    def _get_battery_android_shell(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"获取Android电池信息失败: {e!s}")
    ...
    def _get_battery_ios(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"获取iOS电池信息失败: {e!s}")
    ...
    def _get_battery_macos(self) -> dict[str, Any]:
        ...
        try:
            ...
            output = subprocess.check_output(
                ["pmset", "-g", "batt"], stderr=subprocess.PIPE, universal_newlines=True
            )

            # 解析输出
            percentage_match = re.search(r"(\d+)%", output)
            charging_match = re.search(
                r"(charging|discharging|charged)", output, re.IGNORECASE
            )

            level = int(percentage_match.group(1)) if percentage_match else 100
            is_charging = (
                charging_match and "discharging" not in charging_match.group(1).lower()
            )
            ...
            logger.error(f"获取MacOS电池信息失败: {e!s}")
    ...
    def _get_battery_linux(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self._is_command_available("upower"):
                ...
                level = int(percentage_match.group(1)) if percentage_match else 100
                is_charging = state_match and state_match.group(1).lower() in [
                    "charging",
                    "fully-charged",
                ]
            ...
            # 尝试使用acpi命令
                ...
                output = subprocess.check_output(
                    ["acpi", "-b"], stderr=subprocess.PIPE, universal_newlines=True
                )
                ...
                level = int(percentage_match.group(1)) if percentage_match else 100
                is_charging = charging_match and charging_match.group(1) in [
                    "Charging",
                    "Full",
                ]
            ...
            logger.error(f"获取Linux电池信息失败: {e!s}")
    ...
    def _get_battery_windows(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"获取Windows电池信息失败: {e!s}")
...
def get_battery_info(force_refresh: bool = False) -> dict[str, Any]:
    ...
    if os.environ.get("TEST_ENVIRONMENT") == "true":
        ...
        charging = os.environ.get("MOCK_BATTERY_CHARGING", "true").lower() in (
            "true",
            "1",
            "yes",
        )
