...
def main() -> None:
    ...
    try:
        ...
        if (
            app.edge_computing
            and hasattr(app.edge_computing, "enabled")
            and app.edge_computing.enabled
        ):
            logging.info("  - 边缘计算服务")
        ...
        if app.dialect_service:
            dialects = (
                len(app.dialect_service.supported_dialects)
                if hasattr(app.dialect_service, "supported_dialects")
                else 0
            )
        ...
        if (
            ...
            and hasattr(app.backup_scheduler, "backup_enabled")
            and app.backup_scheduler.backup_enabled
        ):
            logging.info("  - 备份与容灾服务")

        if (
            app.background_collection
            and hasattr(app.background_collection, "enabled")
            and app.background_collection.enabled
        ):
            collection_types = (
                app.config.background_collection.collection_types.to_dict().keys()
                if hasattr(app.config.background_collection, "collection_types")
                else []
            )
            logging.info(
                f"  - 后台数据采集服务 (支持 {len(collection_types)} 种数据类型)"
            )
            if collection_types:
                for ct in collection_types:
                    display_name = (
                        app.config.background_collection.collection_types.get(
                            ct, {}
                        ).get("display_name", ct)
                    )
        ...
        logging.exception(f"服务启动失败: {e!s}")
        if app:
            app.stop()
        sys.exit(1)
