...
# 导入模拟管理器
from internal.service.mock_managers import MockModelManager, MockCacheManager
...
class DesktopCapabilitiesTester:
    ...
    async def test_integration_capabilities(self):
        ...
        try:
            ...
            print(f"    ✅ 屏幕内容分析: 发现 {len(screen_result.get('ui_elements', []))} 个可交互元素")
            ...
            focused_element = ui_elements[0] if ui_elements else {"type": "button", "text": "确定"}
            print(f"    ✅ 焦点元素: {focused_element.get('type', 'unknown')} - {focused_element.get('text', 'N/A')}")
