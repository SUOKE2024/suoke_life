...
raise
    ...
    async def _read_contract_abi() -> list[dict[str, Any]]:
    """读取合约ABI文件"""
try:
...
return self._get_default_abi()
    ...
    def _get_default_abi() -> list[dict[str, Any]]:
    """获取默认ABI（简化版本）"""
return [
{
"inputs": [],
...
"type": "function"
    ...
    async def store_health_data() -> str:
    """存储健康数据到区块链"""
try:
...
raise
    ...
    async def verify_health_data() -> bool:
    """验证健康数据完整性"""
try:
    if "HealthDataStorage" not in self.contracts:
    raise ValueError("HealthDataStorage合约未加载")
...
raise
    ...
    async def _send_transaction() -> str:
    """发送交易"""
if not self.account:
...
return tx_hash.hex()
    ...
    async def wait_for_transaction_receipt() -> TransactionReceipt:
    """等待交易确认"""
try:
...
return Decimal(self.w3.from_wei(balance_wei, 'ether'))
    ...
    async def get_transaction() -> dict[str, Any]:
    """获取交易信息"""
try:
