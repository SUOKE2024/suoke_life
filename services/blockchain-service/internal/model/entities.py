...
class AccessPolicy(BaseModel):
    ...
    expiration_time: datetime | None = None  # 过期时间
    created_at: datetime = Field(default_factory=datetime.utcnow)
    updated_at: datetime = Field(default_factory=datetime.utcnow)
    revoked: bool = False                 # 是否已撤销
    revocation_time: datetime | None = None  # 撤销时间
    revocation_reason: str | None = None  # 撤销原因
...
class HealthDataCheckpoint(BaseModel):
    ...
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    user_id: str
    data_type: DataType | None = None  # 如果为None，表示所有类型
    start_time: datetime
    end_time: datetime
