...
class ConnectionPool:
    ...
    def _init_node_connection(self, node: BlockchainNode):
        ...
        try:
            ...
            node.status = NodeStatus.OFFLINE
            self.logger.error(f"初始化节点连接失败 {node.endpoint}: {str(e)}")
        ...
        **kwargs
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                self.logger.warning(f"操作失败 (尝试 {attempt + 1}/{max_retries + 1}): {str(e)}")
    ...
    async def _check_node_health(self, node: BlockchainNode):
        ...
        try:
            ...
            self.logger.debug(f"节点健康检查失败 {node.endpoint}: {str(e)}")
