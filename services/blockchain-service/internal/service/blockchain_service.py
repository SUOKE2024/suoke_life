...
from internal.model.entities import (
    AccessLevel, AccessPolicy, AuthorizationRecord, DataType, 
    HealthDataRecord, Transaction, TransactionStatus, VerificationRecord
)
...
class BlockchainService(BaseService):
    ...
    def _init_contracts(self):
        ...
        try:
            ...
            # 加载合约ABI
            try:
                with open(f"{contracts_dir}/HealthDataStorage.json", "r") as f:
                    health_data_abi = json.load(f)
                
                with open(f"{contracts_dir}/ZKPVerifier.json", "r") as f:
                    zkp_verifier_abi = json.load(f)
                
                with open(f"{contracts_dir}/AccessControl.json", "r") as f:
                    access_control_abi = json.load(f)
                
                with open(f"{contracts_dir}/SuoKeLifeContractFactory.json", "r") as f:
                    factory_abi = json.load(f)
            except FileNotFoundError as e:
                self.logger.error(f"合约ABI文件不存在: {str(e)}")
                raise
            except json.JSONDecodeError as e:
                self.logger.error(f"解析合约ABI文件失败: {str(e)}")
                raise
            ...
            self.logger.error(f"智能合约初始化失败: {str(e)}")
            raise
            ...
            async def wrapper(*args, **kwargs):
                ...
                while retries < max_tries:
                    try:
                        ...
                        cls_logger.warning(f"{cls}.{func_name} 操作失败，第 {retries} 次重试，等待 {wait_time:.2f} 秒: {str(e)}")
    ...
    async def store_health_data(
    ...
    ) -> Tuple[bool, str, Optional[HealthDataRecord]]:
        """
        将健康数据存储到区块链
        ...
        try:
            ...
            self.logger.error(f"健康数据存储失败: {str(e)}")
            return False, f"健康数据存储失败: {str(e)}", None
    ...
    async def verify_health_data(
        ...
        try:
            ...
            self.logger.error(f"健康数据验证异常: {str(e)}")
            return False, f"健康数据验证异常: {str(e)}", datetime.now()
    ...
    async def verify_with_zkp(
        ...
        try:
            ...
            self.logger.error(f"零知识证明验证异常: {str(e)}")
            return False, f"零知识证明验证异常: {str(e)}", {"error": str(e)}
    ...
    async def authorize_access(
        ...
        try:
            ...
            self.logger.error(f"授权访问失败: {str(e)}")
            return False, f"授权访问失败: {str(e)}", None
    ...
    async def revoke_access(
        ...
        try:
            ...
            self.logger.error(f"撤销访问异常: {str(e)}")
            return False, f"撤销访问异常: {str(e)}"
    ...
    async def get_health_data_records(
        ...
        try:
            ...
            self.logger.error(f"获取健康数据记录失败: {str(e)}")
            return [], 0
    ...
    async def get_blockchain_status(
        ...
        try:
            ...
            self.logger.error(f"获取区块链状态失败: {str(e)}")
            # 返回最基本的错误信息
    ...
    async def generate_health_proof(
        ...
        try:
            ...
            self.logger.error(f"健康证明生成失败: {str(e)}")
            return False, f"健康证明生成失败: {str(e)}", {}
    ...
    async def get_transaction_status(self, tx_hash: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"获取交易状态失败: {str(e)}")
            return {
                "hash": tx_hash,
                "status": "unknown",
                "error": str(e),
    ...
    async def deploy_contract_suite(self) -> Tuple[bool, str, Dict[str, str]]:
        ...
        try:
            ...
            self.logger.error(f"合约套件部署失败: {str(e)}")
            return False, f"合约套件部署失败: {str(e)}", {}
    ...
    async def verify_contract_addresses(self) -> Tuple[bool, str, Dict[str, bool]]:
        ...
        try:
            ...
            self.logger.error(f"验证合约地址失败: {str(e)}")
            return False, f"验证合约地址失败: {str(e)}", {} 