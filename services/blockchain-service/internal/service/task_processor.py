    ...
    async def _worker(self, priority: TaskPriority):
        ...
        while self.is_running:
            try:
                ...
                self.logger.error(f"工作协程错误 ({priority.name}): {str(e)}")
        ...
        try:
            ...
            if task.retry_count < task.max_retries:
                ...
                self.logger.warning(
                    ...
                    f"({task.retry_count}/{task.max_retries}): {task.task_id}, 错误: {str(e)}"
                )
                ...
                self.logger.error(
                    f"任务最终失败: {task.task_id}, 错误: {str(e)}\n"
    ...
    async def _cleanup_completed_tasks(self):
        ...
        while self.is_running:
            try:
                ...
                self.logger.error(f"清理任务时出错: {str(e)}")
