...
class EnhancedMonitoringService:
    ...
    async def _monitoring_loop(self):
        ...
        while self.is_monitoring:
            try:
                ...
                self.logger.error(f"监控循环错误: {e!s}")
    ...
    async def _check_alerts(self):
        ...
        for metric_name, rule in self.alert_rules.items():
            ...
            # 检查是否违反阈值
            violation = False
            if (condition == "greater_than" and metric_value > threshold) or (condition == "less_than" and metric_value < threshold) or (condition == "equals" and abs(metric_value - threshold) < 0.001):
                violation = True
    ...
    async def _trigger_alert(self, metric_name: str, value: float, rule: dict[str, Any]):
        ...
        for callback in self.alert_callbacks:
            try:
                ...
                self.logger.error(f"告警回调执行失败: {e!s}")
    ...
    def _calculate_trend(self, values: list[float], timestamps: list[float]) -> MetricTrend:
        ...
        numerator = sum((x - mean_x) * (y - mean_y) for x, y in zip(timestamps, values, strict=False))
    ...
    def _predict_future_value(self, values: list[float], timestamps: list[float]) -> float | None:
        """
        预测未来值
        ...
        try:
            ...
            self.logger.warning(f"预测计算失败: {e!s}")
            return None
                    ...
                    for insight in insights:
                        ...
                        for callback in self.insight_callbacks:
                            try:
                                ...
                                self.logger.error(f"洞察回调执行失败: {e!s}")

                        self.logger.info(f"生成性能洞察: {insight.title}")

            except Exception as e:
                self.logger.error(f"洞察规则执行失败: {e!s}")
