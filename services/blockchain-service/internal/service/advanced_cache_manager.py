...
class AdvancedCacheManager:
        ...
        with self.l1_cache_lock:
            ...
            if cache_item.compressed:
                try:
                    ...
                    self.logger.warning(f"解压缩失败: {e!s}")
                    return None
    ...
    async def _get_from_l2(self, key: str) -> Any:
        ...
        try:
            ...
            self.logger.warning(f"从L2缓存获取失败: {e!s}")
            return None
    ...
    async def _set_to_l1(self, key: str, value: Any, ttl: int | None = None) -> bool:
        ...
        try:
                ...
                if isinstance(value, (dict, list, tuple)):
                    ...
                    if size_bytes > self.compression_threshold:
                        try:
                            ...
                            self.logger.warning(f"压缩失败: {e!s}")
            ...
            self.logger.warning(f"设置L1缓存失败: {e!s}")
            return False

    async def _set_to_l2(self, key: str, value: Any, ttl: int | None = None) -> bool:
        """设置到L2 Redis缓存"""
        try:
            # 使用基础缓存服务
            parts = key.split(":", 2)
            ...
            self.logger.warning(f"设置L2缓存失败: {e!s}")
            return False
    ...
    async def _delete_from_l2(self, key: str) -> bool:
        ...
        try:
            ...
            self.logger.warning(f"从L2缓存删除失败: {e!s}")
            return False
    ...
    async def _cleanup_loop(self):
        ...
        while self.is_running:
            try:
                ...
                self.logger.error(f"清理循环错误: {e!s}")
    ...
    async def _preload_cache(self):
        ...
        try:
            ...
            self.logger.error(f"缓存预热失败: {e!s}")
