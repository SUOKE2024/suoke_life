...
from ..models.health_data import HealthDataPoint, DeviceInfo
...
class CrossDeviceHealthManager(BaseAIService):
    ...
    async def unified_health_query(self, user_id: str, user_request: str) -> Dict[str, Any]:
        """统一健康数据查询 - MCP核心功能"""
        try:
            ...
            return {
                ...
                'data_summary': {
                    ...
                    'devices_involved': list(set(d.get('device_type') for d in health_data)),
    ...
    async def gather_cross_device_data(self, user_id: str, intent: HealthIntent) -> List[Dict[str, Any]]:
        """跨设备数据收集"""
        all_health_data = []
    ...
    async def fetch_device_data(self, connector, user_id: str, device_info: Dict, intent: HealthIntent) -> List[Dict[str, Any]]:
        """从单个设备获取数据"""
        try:
            # 根据意图确定数据获取参数
            fetch_params = {
    ...
    async def collaborative_analysis(self, user_id: str, health_data: List[Dict], intent: HealthIntent) -> Dict[str, Any]:
        """四智能体协作分析"""
        analysis_results = {}
    ...
    async def xiaoai_health_analysis(self, user_id: str, health_data: List[Dict], intent: HealthIntent) -> Dict[str, Any]:
        """小艾的中医健康分析"""
        # 发布事件给小艾智能体
    ...
    async def xiaoke_service_analysis(self, user_id: str, health_data: List[Dict], intent: HealthIntent) -> Dict[str, Any]:
        """小克的服务资源分析"""
        await self.event_bus.publish(
    ...
    async def laoke_knowledge_analysis(self, user_id: str, health_data: List[Dict], intent: HealthIntent) -> Dict[str, Any]:
        """老克的知识支持分析"""
        await self.event_bus.publish(
    ...
    async def soer_lifestyle_analysis(self, user_id: str, health_data: List[Dict], intent: HealthIntent) -> Dict[str, Any]:
        """索儿的生活方式分析"""
        await self.event_bus.publish(
    ...
    async def generate_consensus_analysis(self, analysis_results: Dict[str, Any]) -> Dict[str, Any]:
        """生成智能体共识分析"""
        # 提取所有智能体的建议
        all_recommendations = []
        confidence_scores = []
        ...
        # 生成综合建议
        consensus_recommendations = await self.merge_recommendations(all_recommendations)
    ...
    async def merge_recommendations(self, recommendations: List[Dict]) -> List[Dict]:
        ...
        for rec_type, recs in grouped_recs.items():
            if len(recs)==1:
                ...
                merged_rec = {
                    'type': rec_type,
                    'content': f"综合{len(recs)}个智能体建议：" + "; ".join([r['content'] for r in recs]),
                    'priority': max([r.get('priority', 'low') for r in recs], key=lambda x: ['low', 'medium', 'high'].index(x)),
                    'supporting_agents': [r['source_agent'] for r in recs],
                    'consensus_strength': len(recs) / len(set(r['source_agent'] for r in recommendations))
                }
                merged_recommendations.append(merged_rec)
        
        return merged_recommendations
    
    async def generate_unified_response(self, analysis: Dict[str, Any], intent: HealthIntent) -> str:
        """生成统一响应"""
        response_parts = []
        ...
        if 'consensus' in analysis:
            ...
            response_parts.append(f"基于跨设备数据分析，置信度 {consensus['consensus_confidence']:.2f}")
        ...
        if 'xiaoai' in analysis and analysis['xiaoai'].get('success', True):
            ...
            if 'constitution_analysis' in xiaoai_result:
                ...
                response_parts.append(f"中医体质分析：{constitution['primary_constitution']}")
    ...
    async def get_user_connected_devices(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        # 暂时返回模拟数据
        return [
            {'device_type': 'apple_health', 'device_id': 'iphone_12', 'status': 'active'},
            {'device_type': 'fitbit', 'device_id': 'fitbit_versa_3', 'status': 'active'},
            {'device_type': 'xiaomi_health', 'device_id': 'mi_band_6', 'status': 'active'}
        ]
    ...
    def summarize_health_data(self, health_data: List[Dict]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'data_types': list(set(d.get('data_type') for d in health_data if d.get('data_type'))),
            'devices': list(set(d.get('device_type') for d in health_data if d.get('device_type'))),
            'time_span': None
        }
