...
class EnhancedTCMDiagnosisModel:
    ...
    async def diagnose(
        self, health_data: Dict[str, Any], user_profile: Dict[str, Any]
        ...
        try:
            ...
            logger.info(
                f"诊断完成: {result.syndrome_type.value}, 置信度: {final_confidence:.3f}"
            )
            return result
    ...
    def _analyze_symptoms(
        self, features: List[float], health_data: Dict[str, Any]
    ) -> Dict[str, float]:
        """症状分析"""
        symptoms_analysis = {}
    ...
    async def _generate_recommendations(
        ...
        base_recommendations = {
            TCMSyndromeType.QI_DEFICIENCY: [
                {
                    "type": "diet",
                    "content": "多食用山药、大枣、黄芪等补气食物",
                    "priority": 1,
                },
                {
                    "type": "exercise",
                    "content": "适量有氧运动，如太极拳、八段锦",
                    "priority": 2,
                },
                {
                    "type": "lifestyle",
                    "content": "保证充足睡眠，避免过度劳累",
                    "priority": 1,
                },
            ],
            ...
            TCMSyndromeType.YIN_DEFICIENCY: [
                {
                    "type": "diet",
                    "content": "多食用银耳、百合、枸杞等滋阴食物",
                    "priority": 1,
                },
                {"type": "exercise", "content": "瑜伽、冥想等静态运动", "priority": 2},
                {
                    "type": "lifestyle",
                    "content": "避免辛辣刺激食物，保持心情平和",
                    "priority": 1,
                },
            ],
        }
    ...
    def _generate_personalized_advice(
        self, prediction: int, user_profile: Dict[str, Any], health_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """生成个性化建议"""
        syndrome = list(TCMSyndromeType)[prediction]
    ...
    async def get_model_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_accuracy": sum(self.model_accuracy.values())
            / len(self.model_accuracy),
...
class OptimizedConsensusAlgorithm:
    ...
    async def _weighted_voting(
        ...
        if weighted_scores:
            ...
            final_confidence = (
                weighted_scores[best_recommendation] / total_weight
                if total_weight > 0
                else 0
            )
    ...
    async def _confidence_based_consensus(
        ...
        sorted_decisions = sorted(
            agent_decisions, key=lambda x: x.get("confidence", 0), reverse=True
        )
...
class PersonalizedRecommendationEngine:
    ...
    async def generate_recommendations(
        self, user_id: str, health_data: Dict[str, Any], context: Dict[str, Any]
        ...
        try:
            ...
            # 健康管理推荐
            health_recs = await self._generate_health_recommendations(
                user_needs, health_data
            )
            recommendations.extend(health_recs)

            # 生活方式推荐
            lifestyle_recs = await self._generate_lifestyle_recommendations(
                user_needs, user_profile
            )
            recommendations.extend(lifestyle_recs)

            # 个性化排序
            sorted_recommendations = self._personalized_ranking(
    ...
    async def _generate_health_recommendations(
        self, user_needs: Dict[str, Any], health_data: Dict[str, Any]
    ) -> List[PersonalizedRecommendation]:
        """生成健康管理推荐"""
        recommendations = []
    ...
    async def _generate_lifestyle_recommendations(
        self, user_needs: Dict[str, Any], user_profile: Dict[str, Any]
    ) -> List[PersonalizedRecommendation]:
        """生成生活方式推荐"""
        recommendations = []
    ...
    def _personalized_ranking(
        ...
        def ranking_score(rec):
            ...
            # 基于实施难度调整
            difficulty_factor = {"简单": 1.1, "中等": 1.0, "困难": 0.9}.get(
                rec.implementation_difficulty, 1.0
            )
    ...
    async def update_user_feedback(
        self, user_id: str, recommendation_id: str, feedback: Dict[str, Any]
    ):
        """更新用户反馈"""
