...
class XiaomiAdapter(BaseAdapter):
    ...
    async def get_auth_url(self, user_id: str, redirect_uri: str, scopes: List[str]) -> str:
        return f"https://account.xiaomi.com/oauth2/authorize?user_id={user_id}"
    
    async def handle_auth_callback(self, code: str, state: str) -> AuthResult:
        return AuthResult(success=True, access_token="xiaomi_mock_token")
    
    async def refresh_token(self, refresh_token: str) -> AuthResult:
        return AuthResult(success=True, access_token="xiaomi_refreshed_token")
    
    async def revoke_access(self, access_token: str) -> bool:
        return True
    
    async def get_user_info(self, access_token: str) -> Dict[str, Any]:
        return {"id": "xiaomi_user_123", "platform": "xiaomi"}
    ...
    def _normalize_activity_data(self, raw_data: Dict[str, Any], user_id: str) -> ActivityData:
        return ActivityData(user_id=user_id, source=DataSource.XIAOMI, record_date=date.today())
    
    def _normalize_sleep_data(self, raw_data: Dict[str, Any], user_id: str) -> SleepData:
        return SleepData(user_id=user_id, source=DataSource.XIAOMI, sleep_date=date.today())
    
    def _normalize_heart_rate_data(self, raw_data: Dict[str, Any], user_id: str) -> HeartRateData:
        return HeartRateData(user_id=user_id, source=DataSource.XIAOMI, record_date=date.today(), record_time=datetime.now(), heart_rate=70) 