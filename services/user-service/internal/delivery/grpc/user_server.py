...
from api.grpc import user_pb2, user_pb2_grpc
from internal.service.user_service import UserService
from internal.model.user import (
    CreateUserRequest, UpdateUserRequest, UserStatus, UserRole,
    VerifyUserRequest, UpdateUserPreferencesRequest, BindDeviceRequest
)
from pkg.errors.exceptions import UserNotFoundError, UserAlreadyExistsError, DeviceAlreadyBoundError
...
class UserServicer(user_pb2_grpc.UserServiceServicer):
    ...
    def _user_to_pb(self, user):
        ...
        for role in user.roles:
            response.roles.append(str(role.value) if hasattr(role, 'value') else str(role))
