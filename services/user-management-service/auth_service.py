...
class AuthService:
    ...
    async def authenticate(self, username: str, password: str) -> Optional[Dict[str, Any]]:
        ...
        # 验证密码
        if not await self.password_manager.verify_password(password, user.get("password_hash", "")):
            return None
    ...
    async def authorize(self, token: str, required_permissions: List[str] = None) -> Optional[Dict[str, Any]]:
        """用户授权"""
        if not self._initialized:
            await self.initialize()
