...
class HealthMetricQuery(BaseModel):
    ...
    aggregation: str = Field(
        default="raw", description="聚合方式: raw, daily, weekly, monthly"
    )
...
# 健康目标管理端点
@router.post(
    " / goals", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED
)
...
async def create_health_goal(
    ...
    try:
        ...
        return {"message": "健康目标创建成功", "goal": goal_data}
...
async def get_health_goals(
    ...
    try:
        ...
        return {"user_id": user_id, "goals": mock_goals, "total_count": len(mock_goals)}
...
async def update_health_goal(
    ...
    try:
        ...
        updated_goal = {
            ...
            "unit": "steps",
            "deadline": (
                goal_update.deadline.isoformat() if goal_update.deadline else None
            ),
            "description": goal_update.description or "每日步数目标",
            "is_active": (
                goal_update.is_active if goal_update.is_active is not None else True
            ),
            "updated_at": datetime.utcnow().isoformat(),
        }

        return {"message": "健康目标更新成功", "goal": updated_goal}
...
async def delete_health_goal(
    ...
    try:
        ...
        # 模拟目标删除
        return {"message": "健康目标删除成功", "goal_id": goal_id, "user_id": user_id}
