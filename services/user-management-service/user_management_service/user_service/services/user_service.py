...
class UserService:
    ...
    async def create_user(
        self, username: str, email: str, phone: Optional[str] = None, **kwargs
    ) -> User:
        """创建新用户"""
        try:
            # 检查用户是否已存在
    ...
    async def bind_device(
        self, user_id: uuid.UUID, device_id: str, device_name: str, device_type: str
    ) -> UserDevice:
        """绑定用户设备"""
        try:
            user = await self.get_user_by_id(user_id)
    ...
    async def update_user_health_summary(
        self, user_id: uuid.UUID, health_data: Dict[str, Any]
    ) -> HealthSummary:
        """更新用户健康摘要"""
        try:
            user = await self.get_user_by_id(user_id)
