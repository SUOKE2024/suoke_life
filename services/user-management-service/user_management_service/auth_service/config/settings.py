...
class EmailSettings(BaseSettings):
    ...
    sendgrid_api_key: Optional[str] = Field(
        default=None, description="SendGrid API密钥"
    )
...
class JWTSettings(BaseSettings):
    ...
    secret_key: str = Field(
        default="test - secret - key - for - testing - only - not - for - production",
        description="JWT密钥",
    )
    algorithm: str = Field(default="HS256", description="JWT算法")
    access_token_expire_minutes: int = Field(
        default=60, description="访问令牌过期时间(分钟)"
    )
...
class LoggingSettings(BaseSettings):
    ...
    file_path: str = Field(
        default="logs / auth - service.log", description="日志文件路径"
    )
...
class Settings(BaseSettings):
    ...
    oauth_state_expire_minutes: int = Field(
        default=10, description="OAuth状态参数过期时间(分钟)"
    )
    oauth_callback_timeout_seconds: int = Field(
        default=300, description="OAuth回调超时时间(秒)"
    )
