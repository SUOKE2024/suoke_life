...
class TokenBucket:
    ...
    def __init__(self, capacity: int, refill_rate: float, burst: int | None = None):
        """初始化令牌桶
...
class RateLimiter:
    ...
    async def _cleanup_expired_limiters(self) -> None:
        ...
        if len(self._limiters) > 5000:
            await self._force_cleanup_oldest_limiters()

    async def _force_cleanup_oldest_limiters(self) -> None:
        """强制清理最旧的限制器"""
