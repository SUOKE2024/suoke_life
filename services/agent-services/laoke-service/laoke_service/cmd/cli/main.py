...
from laoke_service.core.config import get_settings, Settings
from laoke_service.core.logging import setup_logging, get_logger
...
@click.pass_context
def export(ctx: click.Context, output: Optional[str]):
    """导出配置模板"""
    settings: Settings = ctx.obj["settings"]
...
@click.pass_context
def test(ctx: click.Context, message: str, type: str):
    """测试智能体响应"""
    settings: Settings = ctx.obj["settings"]
...
def serve(ctx: click.Context, host: str, port: int, reload: bool):
    """启动服务器（快捷命令）"""
    from laoke_service.cmd.server.main import main as server_main
    
    # 调用服务器主函数
    server_main.callback(host, port, 1, reload, "info")


@cli.command()
def version():
    """显示版本信息"""
    from laoke_service import __version__, __description__
