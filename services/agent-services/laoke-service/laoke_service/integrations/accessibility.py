...
class AccessibilityClient:
    ...
    async def _make_request(
        ...
        if not self.enabled:
            raise AccessibilityServiceException(
                "Accessibility service is disabled", conversion_type="general"
            )

        url = f"{self.base_url.rstrip('/')}/{endpoint.lstrip('/')}"
        headers = {"Content-Type": "application/json", "User-Agent": "Laoke-Agent/1.0"}
        ...
        try:
            ...
            raise AccessibilityServiceException(
                f"Accessibility service request failed: {e}", conversion_type="network"
            )
        ...
        try:
            ...
            if not isinstance(e, AccessibilityServiceException):
                e = AccessibilityServiceException(
                    f"TTS conversion failed: {e}", conversion_type="tts"
                )
            raise e
    ...
    async def speech_to_text(self, request: STTRequest) -> STTResponse:
        """语音转文本"""
        try:
            data = {"language": request.language, "format": request.format}
            ...
            if not isinstance(e, AccessibilityServiceException):
                e = AccessibilityServiceException(
                    f"STT conversion failed: {e}", conversion_type="stt"
                )
            raise e
    ...
    async def get_user_profile(self, user_id: str) -> Optional[AccessibilityProfile]:
        ...
        try:
            ...
            profile = AccessibilityProfile(
                user_id=user_id,
                enabled_features=[
                    AccessibilityFeature(f) for f in response["enabled_features"]
                ],
            ...
            if e.status == 404:
                return None
            raise AccessibilityServiceException(
                f"Failed to get user profile: {e}", conversion_type="profile"
            )
        except Exception as e:
            if not isinstance(e, AccessibilityServiceException):
                e = AccessibilityServiceException(
    ...
    async def update_user_profile(
        ...
        try:
            ...
            response = await self._make_request(
                "PUT", f"/users/{profile.user_id}/profile", data=data
            )
            ...
            if not isinstance(e, AccessibilityServiceException):
                e = AccessibilityServiceException(
                    f"Failed to update user profile: {e}", conversion_type="profile"
                )
            raise e

    async def convert_response_for_user(
        self, user_id: str, text_response: str, preferred_format: Optional[str] = None
    ) -> Dict[str, Any]:
        """根据用户需求转换响应格式"""
        try:
            # 获取用户配置
            ...
            result = {"text": text_response, "accessibility_features": []}
            ...
            if AccessibilityFeature.TEXT_TO_SPEECH in profile.enabled_features:
                ...
                tts_request = TTSRequest(
                    ...
                    voice=(
                        TTSVoice(
                            profile.tts_preferences.get(
                                "voice", TTSVoice.FEMALE_STANDARD.value
                            )
                        )
                        if profile.tts_preferences
                        else TTSVoice.FEMALE_STANDARD
                    ),
                    speed=(
                        TTSSpeed(
                            profile.tts_preferences.get("speed", TTSSpeed.NORMAL.value)
                        )
                        if profile.tts_preferences
                        else TTSSpeed.NORMAL
                    ),
                    language=(
                        profile.tts_preferences.get("language", "zh-CN")
                        if profile.tts_preferences
                        else "zh-CN"
                    ),
                )
            ...
            if AccessibilityFeature.SCREEN_READER in profile.enabled_features:
                ...
                # 添加结构化数据
                result["structured_content"] = self._create_structured_content(
                    text_response
                )
    ...
    def _create_structured_content(self, text: str) -> Dict[str, Any]:
        ...
        for line in lines:
            ...
            if line.endswith("：") or line.endswith(":"):
                ...
                structured["content"].append(
                    {"type": "list_item", "text": line[1:].strip()}
                )
            else:
                structured["content"].append({"type": "paragraph", "text": line})
