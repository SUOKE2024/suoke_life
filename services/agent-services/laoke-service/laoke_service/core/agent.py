        ...
        try:
            ...
            if "timeout" in error_str:
                ...
                raise AIModelException(
                    f"OpenAI API quota exceeded: {e}", model, "quota", e
                )
            else:
...
class LaokeAgent:
    ...
    def _create_ai_client(self) -> AIModelClient:
        ...
        if "gpt" in models_config.primary_model.lower():
            return OpenAIClient(
                api_key=models_config.api_key, base_url=models_config.base_url
            )
        else:
            # 默认使用OpenAI兼容接口
            return OpenAIClient(
    ...
    async def _cleanup_expired_sessions(self) -> None:
        ...
        async with self.session_lock:
            ...
            if expired_sessions:
                log_business_event("sessions_cleaned", count=len(expired_sessions))
        ...
        async with self.session_lock:
            ...
            if active_sessions >= self.config.max_concurrent_sessions:
                raise SessionException(
                    "Maximum concurrent sessions exceeded", error_type="limit_exceeded"
                )
            ...
            context = ConversationContext(
                user_id=user_id, session_id=session_id, metadata=metadata or {}
            )

            # 添加系统消息
            system_message = Message(
                role=MessageRole.SYSTEM, content=self.config.conversation.system_prompt
            )
            ...
            log_business_event(
                "session_created", user_id=user_id, session_id=session_id
            )
    ...
    async def terminate_session(self, session_id: str) -> None:
        ...
        async with self.session_lock:
            if session_id in self.sessions:
                ...
                log_business_event(
                    "session_terminated", user_id=context.user_id, session_id=session_id
                )
    ...
    def _validate_message(self, content: str) -> None:
        ...
        if len(content) > max_tokens:
            raise ContentException(
                f"消息内容过长，最大允许{max_tokens}个字符", error_type="too_long"
            )

    async def chat(
        self, session_id: str, message: str, stream: bool = False, **kwargs
    ) -> str:
        """对话接口"""
        try:
            # 验证消息
    ...
    async def _generate_response(self, context: ConversationContext, **kwargs) -> str:
        ...
        try:
            ...
            assistant_message = Message(
                role=MessageRole.ASSISTANT, content=response_content, metadata=metadata
            )
