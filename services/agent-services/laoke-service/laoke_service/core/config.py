...
class AIConfig(BaseModel):
    ...
    # 本地模型配置
    local_model_path: Optional[Path] = Field(default=None, description="本地模型路径")
...
class MonitoringConfig(BaseModel):
    ...
    otel_endpoint: Optional[str] = Field(default=None, description="OpenTelemetry 端点")
...
class Settings(BaseSettings):
    ...
    def validate_config_path(cls, v: Union[str, Path, None]) -> Optional[Path]:
        """验证配置文件路径"""
        if v is None:
            return None
        path = Path(v)
