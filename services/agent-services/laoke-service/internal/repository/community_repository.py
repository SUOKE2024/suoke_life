...
class CommunityRepository:
    ...
    async def find_post_by_id(self, post_id: str) -> dict[str, Any] | None:
        """
        根据ID获取社区帖子详情
    ...
    async def create_post(self, post_data: dict[str, Any]) -> str | None:
        """
        创建社区帖子
    ...
    async def add_like(self, post_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            result = await self.likes.update_one(
                ...
                {"$set": {"created_at": datetime.now(UTC).isoformat()}},
                upsert=True
            )
            ...
            for tag in tags:
                if increment > 0:
                    ...
                    await self.tags.update_one(
                        {"name": tag},
                        {"$inc": {"count": increment}, "$setOnInsert": {"created_at": datetime.now(UTC).isoformat()}},
                        upsert=True
                    )
                else:
                    # 减少标签计数
    ...
    async def _ensure_category_exists(self, category: str) -> None:
        ...
        try:
            await self.categories.update_one(
                {"name": category},
                {"$setOnInsert": {"created_at": datetime.now(UTC).isoformat()}},
                upsert=True
            )
        except PyMongoError as e:
