...
class KnowledgeRepository:
    ...
    async def find_article_by_id(self, article_id: str) -> Optional[Dict[str, Any]]:
        """
        根据ID查找知识文章
        ...
        try:
            ...
            logger.error(f"查找知识文章失败: {str(e)}")
            metrics.increment_counter("knowledge_repo_errors", {"method": "find_article_by_id"})
            return None
    ...
    async def create_learning_path(self, path_data: Dict[str, Any]) -> Optional[str]:
        """
        创建学习路径
    ...
    async def find_learning_path_by_id(self, path_id: str) -> Optional[Dict[str, Any]]:
        """
        根据ID查找学习路径
