...
class AgentManager:
    ...
    def __init__(self, 
        ...
        # 活跃会话管理 - 使用弱引用避免内存泄漏
        self._active_sessions: weakref.WeakValueDictionary = weakref.WeakValueDictionary()
    ...
    @asynccontextmanager
    async def _get_session(self, user_id: str, session_id: Optional[str] = None):
        """获取或创建会话的上下文管理器"""
        if not session_id:
    ...
    async def process_request(self, 
        ...
        try:
            ...
            logger.error(f"请求处理失败，用户ID: %s, 错误: %s", user_id, str(e))
