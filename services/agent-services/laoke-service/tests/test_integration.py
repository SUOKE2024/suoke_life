...
API集成测试"""
    ...
    def test_chat_endpoint(self, mock_chat, client):
        ...
        response = client.post("/sessions/session123/chat", json={
            "message": "你好",
            "stream": False
        })
...
class TestAccessibilityIntegration:
    ...
    def mock_accessibility_client(self):
        """模拟无障碍服务客户端"""
        with patch('laoke_service.integrations.accessibility.get_accessibility_client') as mock:
            client = Mock()
    ...
    async def test_text_to_speech_integration(self, mock_accessibility_client):
        ...
        from laoke_service.integrations.accessibility import convert_text_to_speech, TTSVoice, TTSSpeed
    ...
    async def test_accessibility_service_disabled(self):
        """测试无障碍服务禁用"""
        with patch('laoke_service.integrations.accessibility.get_config') as mock_config:
            # 模拟服务禁用
            config = Mock()
...
class TestEndToEndWorkflow:
    ...
    async def setup_agent(self):
        ...
        # 模拟AI响应
        with patch('laoke_service.core.agent.OpenAIClient.generate_response') as mock_generate:
...
class TestErrorHandling:
    ...
    def test_validation_error_handling(self, client):
        """测试验证错误处理"""
        # 空消息
        response = client.post("/sessions/test/chat", json={
            "message": "",
            "stream": False
        })
