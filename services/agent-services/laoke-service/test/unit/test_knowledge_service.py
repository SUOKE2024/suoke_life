...
import pytest
from internal.knowledge.knowledge_graph import KnowledgeGraphService
...
class TestKnowledgeService(unittest.TestCase):
    ...
    def setUp(self) -> None:
        """测试前初始化"""
        # 模拟知识图谱依赖
        self.mock_knowledge_graph = MagicMock(spec=KnowledgeGraphService)
        ...
        # 初始化知识服务
        with patch(
            "internal.knowledge.knowledge_service.Config", return_value=self.mock_config
        ):
    ...
    @pytest.mark.asyncio
    async def test_get_article_by_id(self) -> None:
        """测试通过ID获取文章"""
        # 设置模拟返回值
        mock_article = {
            "id": "123",
    ...
    async def test_search_knowledge(self) -> None:
        ...
        self.mock_knowledge_graph.generate_embeddings.assert_called_once_with(
            "中医基础理论"
        )
        self.mock_knowledge_graph.search_similar_content.assert_called_once_with(
            mock_embeddings, limit=5, threshold=0.7
        )
    ...
    @pytest.mark.asyncio
    async def test_rate_article(self) -> None:
        """测试文章评分功能"""
        # 设置模拟数据
        article_id = "123"
        rating = 4
