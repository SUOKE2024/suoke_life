...
class Settings(BaseSettings):
    ...
    # PostgreSQL 配置 (可选)
    postgres_url: Optional[str] = Field(default=None, description="PostgreSQL 连接 URL")
    ...
    # 营养分析配置
    nutrition_api_key: Optional[str] = Field(default=None, description="营养数据 API 密钥")

    # 微服务配置
    user_management_service_url: str = Field(
        default="http://localhost:8001", description="用户管理服务 URL"
    )
    tcm_constitution_service_url: str = Field(
        default="http://localhost:8010", description="中医体质服务 URL"
    )
    tcm_meridian_service_url: str = Field(
        default="http://localhost:8011", description="中医经络服务 URL"
    )
    tcm_herbal_service_url: str = Field(
        default="http://localhost:8012", description="中医药材服务 URL"
    )
    unified_health_service_url: str = Field(
        default="http://localhost:8002", description="统一健康数据服务 URL"
    )
