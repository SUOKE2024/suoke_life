...
async def analyze_nutrition(
    request: FoodAnalysisRequest, nutrition_service: NutritionService = Depends()
) -> NutritionAnalysis:
    """
    分析食物营养成分
...
async def generate_diet_plan(
    request: DietPlanRequest, nutrition_service: NutritionService = Depends()
) -> DietPlan:
    """
    生成个性化膳食计划
...
async def search_food_database(
    query: str, limit: int = 10, nutrition_service: NutritionService = Depends()
) -> list[dict[str, Any]]:
    """
    搜索食物数据库
    ...
    """
    try:
        results = await nutrition_service.search_food_database(query=query, limit=limit)
...
async def get_nutrition_recommendations(
    user_id: str, nutrition_service: NutritionService = Depends()
) -> dict[str, Any]:
    """
    获取个性化营养建议
...
async def get_nutrition_history(
    user_id: str, days: int = 7, nutrition_service: NutritionService = Depends()
) -> list[dict[str, Any]]:
    """
    获取用户营养历史记录
