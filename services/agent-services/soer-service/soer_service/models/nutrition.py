...
class NutrientInfo(BaseModel):
    ...
    daily_value_percent: float | None = Field(None, description="每日推荐值百分比")
...
class FoodItem(BaseModel):
    ...
    unit: str = Field(..., description="单位")
    calories_per_unit: float | None = Field(None, description="每单位热量")
    nutrients: list[NutrientInfo] = Field(
        default_factory=list, description="营养素列表"
    )
    food_id: str | None = Field(None, description="食物数据库ID")
...
class NutritionAnalysis(BaseModel):
    ...
    # 中医营养学分析
    tcm_analysis: dict[str, Any] = Field(
        default_factory=dict, description="中医营养分析"
    )
...
class MealPlan(BaseModel):
    ...
    recipe_url: str | None = Field(None, description="食谱链接")
...
class DietPlan(BaseModel):
    ...
    # 中医膳食理论
    tcm_principles: dict[str, Any] = Field(
        default_factory=dict, description="中医膳食原则"
    )
...
class FoodDatabase(BaseModel):
    ...
    seasonal_suitability: list[str] = Field(
        default_factory=list, description="适宜季节"
    )
