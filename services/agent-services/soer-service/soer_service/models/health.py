...
class VitalSigns(BaseModel):
    """生命体征模型"""
    heart_rate: Optional[float] = Field(None, description="心率(bpm)")
    systolic_bp: Optional[float] = Field(None, description="收缩压(mmHg)")
    diastolic_bp: Optional[float] = Field(None, description="舒张压(mmHg)")
    blood_sugar: Optional[float] = Field(None, description="血糖(mg/dL)")
...
class ActivityMetrics(BaseModel):
    """活动指标模型"""
    daily_steps: Optional[int] = Field(None, description="每日步数")
    ...
    active_minutes: Optional[int] = Field(None, description="活跃时长(分钟)")
    sedentary_time: Optional[int] = Field(None, description="久坐时间(分钟)")
...
class SleepMetrics(BaseModel):
    ...
    wake_up_count: Optional[int] = Field(None, description="夜间醒来次数")
    sleep_quality_score: Optional[float] = Field(None, description="睡眠质量评分")


class MentalHealthMetrics(BaseModel):
    """心理健康指标模型"""
    stress_level: Optional[int] = Field(None, description="压力水平(1-10)")
    mood_score: Optional[int] = Field(None, description="情绪评分(1-10)")
    anxiety_level: Optional[int] = Field(None, description="焦虑水平(1-10)")
    energy_level: Optional[int] = Field(None, description="精力水平(1-10)")
    focus_level: Optional[int] = Field(None, description="专注度(1-10)")
...
class HealthAnalysis(BaseModel):
    ...
    physical_metrics: PhysicalMetrics = Field(default_factory=PhysicalMetrics, description="身体指标")
    ...
    mental_health_metrics: MentalHealthMetrics = Field(default_factory=MentalHealthMetrics, description="心理健康指标")
    ...
    # 中医体质分析
    tcm_constitution: Dict[str, Any] = Field(default_factory=dict, description="中医体质分析")
...
class HealthRecommendation(BaseModel):
    ...
    # 中医建议
    tcm_guidance: Optional[Dict[str, Any]] = Field(None, description="中医指导")
...
class HealthGoal(BaseModel):
    ...
    achievement_date: Optional[datetime] = Field(None, description="达成日期")
