...
class Exercise(BaseModel):
    ...
    calories_burned: Optional[float] = Field(None, description="消耗热量")
    description: Optional[str] = Field(None, description="运动描述")
...
class ExercisePlan(BaseModel):
    ...
    weekly_schedule: Dict[str, List[str]] = Field(default_factory=dict, description="每周安排")
    
    # 目标设定
    fitness_goals: List[str] = Field(default_factory=list, description="健身目标")
    target_calories_per_week: Optional[float] = Field(None, description="每周目标消耗热量")
    ...
    # 中医运动理论
    tcm_exercise_principles: Dict[str, Any] = Field(default_factory=dict, description="中医运动原则")
...
class WorkoutSession(BaseModel):
    ...
    user_id: str = Field(..., description="用户ID")
    plan_id: Optional[str] = Field(None, description="所属计划ID")
    ...
    # 主观感受
    perceived_exertion: Optional[int] = Field(None, description="主观疲劳度(1-10)")
    mood_before: Optional[int] = Field(None, description="运动前情绪(1-10)")
    mood_after: Optional[int] = Field(None, description="运动后情绪(1-10)")
    energy_level: Optional[int] = Field(None, description="精力水平(1-10)")
...
class SleepData(BaseModel):
    ...
    sleep_onset_time: Optional[time] = Field(None, description="入睡时间")
    ...
    sleep_latency: Optional[int] = Field(None, description="入睡潜伏期(分钟)")
    
    # 睡眠阶段
    light_sleep_duration: Optional[float] = Field(None, description="浅睡眠时长(小时)")
    deep_sleep_duration: Optional[float] = Field(None, description="深睡眠时长(小时)")
    rem_sleep_duration: Optional[float] = Field(None, description="REM睡眠时长(小时)")
    ...
    # 主观评价
    sleep_quality_rating: Optional[int] = Field(None, description="睡眠质量评分(1-10)")
    morning_mood: Optional[int] = Field(None, description="晨起情绪(1-10)")
    
    # 环境因素
    room_temperature: Optional[float] = Field(None, description="室温(°C)")
    noise_level: Optional[str] = Field(None, description="噪音水平")
    light_exposure: Optional[str] = Field(None, description="光照情况")
...
class SleepAnalysis(BaseModel):
    ...
    sleep_recommendations: List[str] = Field(default_factory=list, description="睡眠建议")
    
    # 中医睡眠理论
    tcm_sleep_analysis: Dict[str, Any] = Field(default_factory=dict, description="中医睡眠分析")
...
class StressAssessment(BaseModel):
    ...
    user_id: str = Field(..., description="用户ID")
    assessment_date: datetime = Field(default_factory=datetime.now, description="评估时间")
    ...
    # 生理指标
    heart_rate_variability: Optional[float] = Field(None, description="心率变异性")
    cortisol_level: Optional[float] = Field(None, description="皮质醇水平")
    ...
    # 应对策略
    current_coping_strategies: List[str] = Field(default_factory=list, description="当前应对策略")
    effective_strategies: List[str] = Field(default_factory=list, description="有效策略")
    
    # 建议
    stress_management_recommendations: List[str] = Field(default_factory=list, description="压力管理建议")
    
    # 中医情志调节
    tcm_emotional_regulation: Dict[str, Any] = Field(default_factory=dict, description="中医情志调节")
...
class HabitTracker(BaseModel):
    ...
    last_completed: Optional[datetime] = Field(None, description="最后完成时间") 