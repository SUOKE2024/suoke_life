...
class AgentResponse(BaseModel):
    ...
    agent_personality: AgentPersonality = Field(default=AgentPersonality.FRIENDLY, description="智能体个性")
    ...
    # 个性化信息
    personalization_factors: Dict[str, Any] = Field(default_factory=dict, description="个性化因素")
    
    # 中医相关
    tcm_insights: Optional[Dict[str, Any]] = Field(None, description="中医见解")
...
class ConversationHistory(BaseModel):
    ...
    sentiment_analysis: Dict[str, float] = Field(default_factory=dict, description="情感分析")
    user_satisfaction: Optional[int] = Field(None, description="用户满意度(1-5)")
...
class AgentConfiguration(BaseModel):
    ...
    # 个性化设置
    personality: AgentPersonality = Field(default=AgentPersonality.FRIENDLY, description="个性类型")
    expertise_level: ExpertiseLevel = Field(default=ExpertiseLevel.INTERMEDIATE, description="专业水平")
    ...
    # 功能设置
    enabled_capabilities: List[str] = Field(default_factory=list, description="启用的能力")
    notification_preferences: Dict[str, bool] = Field(default_factory=dict, description="通知偏好")
...
class InteractionAnalytics(BaseModel):
    ...
    # 话题分析
    top_topics: List[Dict[str, Any]] = Field(default_factory=list, description="热门话题")
    topic_distribution: Dict[str, float] = Field(default_factory=dict, description="话题分布")
    
    # 满意度分析
    average_satisfaction: float = Field(..., description="平均满意度")
    satisfaction_trend: List[Dict[str, Any]] = Field(default_factory=list, description="满意度趋势")
    
    # 使用模式
    peak_usage_hours: List[int] = Field(default_factory=list, description="使用高峰时段")
