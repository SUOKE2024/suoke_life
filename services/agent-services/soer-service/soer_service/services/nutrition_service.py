...
class NutritionService(BaseService):
    ...
    async def analyze_nutrition(
        self, food_items: list[FoodItem], user_id: str, meal_type: str
    ) -> NutritionAnalysis:
        """
        分析食物营养成分
        ...
        # 计算宏量营养素
        total_calories = sum(
            item.calories_per_unit * item.amount
            for item in food_items
            if item.calories_per_unit
        )
        total_protein = sum(
            next((n.amount for n in item.nutrients if n.name == "蛋白质"), 0)
            * item.amount
            for item in food_items
        )
        total_carbs = sum(
            next((n.amount for n in item.nutrients if n.name == "碳水化合物"), 0)
            * item.amount
            for item in food_items
        )
        total_fat = sum(
            next((n.amount for n in item.nutrients if n.name == "脂肪"), 0)
            * item.amount
            for item in food_items
        )
        ...
        # 计算营养评分
        nutrition_score = await self._calculate_nutrition_score(
            macro_nutrients, micro_nutrients
        )
    ...
    async def generate_diet_plan(
        ...
        # 计算营养目标
        macro_targets = await self._calculate_macro_targets(
            target_calories, user_profile
        )
    ...
    async def get_nutrition_recommendations(self, user_id: str) -> dict[str, Any]:
        """获取个性化营养建议"""
        # 获取用户最近的营养数据
        await self._get_recent_nutrition_analysis(user_id)
        ...
        recommendations = {
            ...
            "weekly_goals": ["每周至少吃5种不同颜色的蔬菜", "减少加工食品摄入"],
            "tcm_guidance": {
                "constitution": "平和质",
    ...
    async def _calculate_nutrition_score(
        self, macro_nutrients: MacroNutrients, micro_nutrients: MicroNutrients
    ) -> float:
        """计算营养评分"""
        # 简化的评分算法
        base_score = 70.0
    ...
    async def _generate_nutrition_recommendations(
        self, macro_nutrients: MacroNutrients, user_id: str
    ) -> list[str]:
        """生成营养建议"""
        recommendations = []
    ...
    async def _analyze_tcm_nutrition(
        self, food_items: list[FoodItem], user_id: str
    ) -> dict[str, Any]:
        """中医营养分析"""
        return {
            "food_nature": "平性为主",
    ...
    async def _calculate_macro_targets(
        self, target_calories: int, user_profile: dict[str, Any]
    ) -> MacroNutrients:
        """计算宏量营养素目标"""
