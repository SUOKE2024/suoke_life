...
from ..models.agent import (
    AgentMessage, AgentResponse, ConversationHistory,
    AgentConfiguration, MessageType, MessageRole, AgentPersonality
)
...
class AgentService(BaseService):
    ...
    async def process_message(
        ...
        Returns:
            智能体响应
        """
        self.logger.info(f"处理用户消息: 用户={user_id}, 内容长度={len(message_content)}")
        ...
        # 计算置信度
        confidence_score = await self._calculate_confidence_score(intent, response_content)
        ...
        # 更新对话历史
        await self._update_conversation_history(conversation_id, user_message, agent_response)
        
        # 记录操作日志
        await self.log_operation("process_message", user_id, {
            "intent": intent,
    ...
    async def _analyze_user_intent(self, message: str, context: Dict[str, Any]) -> str:
        ...
        if any(keyword in message_lower for keyword in ["营养", "饮食", "食物", "热量"]):
            return "nutrition_inquiry"
        elif any(keyword in message_lower for keyword in ["健康", "体检", "症状", "身体"]):
            return "health_consultation"
        elif any(keyword in message_lower for keyword in ["运动", "锻炼", "健身", "减肥"]):
            return "exercise_advice"
        elif any(keyword in message_lower for keyword in ["睡眠", "失眠", "休息"]):
            return "sleep_consultation"
        elif any(keyword in message_lower for keyword in ["压力", "焦虑", "情绪", "心情"]):
            return "emotional_support"
        elif any(keyword in message_lower for keyword in ["中医", "体质", "穴位", "经络"]):
    ...
    async def _get_user_configuration(self, user_id: str) -> AgentConfiguration:
        ...
        # 尝试从数据库获取
        config_data = await self.mongodb.user_configurations.find_one({"user_id": user_id})
    ...
    async def _generate_suggestions(self, intent: str, user_config: AgentConfiguration) -> List[str]:
        """生成建议列表"""
        suggestions_map = {
            "nutrition_inquiry": [
                "查看今日营养摄入",
    ...
    async def _get_tcm_insights(self, message: str, user_id: str) -> Optional[Dict[str, Any]]:
        """获取中医见解"""
        # 如果消息涉及中医相关内容，提供中医见解
        message_lower = message.lower()
        
        if any(keyword in message_lower for keyword in ["中医", "体质", "穴位", "经络", "养生"]):
            return {
    ...
    async def _get_conversation_from_db(self, conversation_id: str) -> Optional[ConversationHistory]:
        """从数据库获取对话"""
        try:
    ...
    async def _get_conversation_messages(self, conversation_id: str) -> List[Dict[str, Any]]:
        """获取对话消息"""
        try:
            messages = await self.mongodb.agent_messages.find(
                {"conversation_id": conversation_id}
