...
from fastapi import FastAPI, APIRouter, HTTPException, Depends, Query, Body, Path
...
from api.rest.schemas import (
    User, HealthGoal, HealthPlanRequest, HealthPlanResponse, 
    LifestyleRecommendationRequest, LifestyleRecommendationResponse,
    SensorDataRequest, SensorDataResponse, NutritionRequest, NutritionResponse,
    AbnormalPatternRequest, AbnormalPatternResponse, HealthTrendRequest,
    HealthTrendResponse, SleepRecommendationRequest, SleepRecommendationResponse,
    EmotionalStateRequest, EmotionalStateResponse
)
...
async def generate_diet_recommendations(
    ...
    preferences: Optional[Dict[str, Any]] = Body(None, description="用户偏好"),
    services: Dict = Depends(get_services)
):
    """
    生成个性化饮食推荐
