...
class DietRecommendation(BaseModel):
    ...
    meal_distribution: dict[str, float] = Field(
        ..., description="膳食分配比例，如早餐、午餐、晚餐的比例"
    )
...
class ExerciseRecommendation(BaseModel):
    ...
    tcm_exercises: list[str] = Field(
        default_factory=list, description="中医养生功法推荐"
    )
...
class SleepRecommendation(BaseModel):
    ...
    pre_sleep_routine: list[str] = Field(
        default_factory=list, description="睡前建议活动"
    )
    environment_tips: list[str] = Field(
        default_factory=list, description="睡眠环境建议"
    )
    avoid_activities: list[str] = Field(
        default_factory=list, description="睡前应避免的活动"
    )
...
class StressManagement(BaseModel):
    ...
    relaxation_techniques: list[str] = Field(
        default_factory=list, description="放松技巧"
    )
    meditation_practice: dict[str, Any] | None = Field(None, description="冥想练习")
    emotional_regulation: list[str] = Field(
        default_factory=list, description="情绪调节方法"
    )
    leisure_activities: list[str] = Field(
        default_factory=list, description="推荐休闲活动"
    )
    tcm_emotion_regulation: list[str] = Field(
        default_factory=list, description="中医情志调节法"
    )
...
class SupplementRecommendation(BaseModel):
    ...
    supplements: list[dict[str, Any]] = Field(
        default_factory=list, description="补充剂列表及用量"
    )
...
class EnvironmentalSuggestion(BaseModel):
    ...
    living_environment: list[str] = Field(
        default_factory=list, description="生活环境建议"
    )
    work_environment: list[str] = Field(
        default_factory=list, description="工作环境建议"
    )
    seasonal_adjustments: dict[str, list[str]] = Field(
        default_factory=dict, description="季节性调整建议"
    )
...
class DailySchedule(BaseModel):
    ...
    tcm_timing_principles: list[str] = Field(
        default_factory=list, description="中医时辰养生原则"
    )
...
class HealthPlan(BaseModel):
    ...
    exercise_recommendations: ExerciseRecommendation = Field(
        ..., description="运动建议"
    )
    ...
    supplement_recommendations: SupplementRecommendation | None = Field(
        None, description="营养补充建议"
    )
    environmental_suggestions: EnvironmentalSuggestion = Field(
        ..., description="环境建议"
    )
