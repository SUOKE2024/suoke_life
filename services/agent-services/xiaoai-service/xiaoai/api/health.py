...
async def get_system_metrics(hours: int = Query(1, ge=1, le=24, description="指标时间范围（小时）")):
    """
    获取系统指标
...
async def get_performance_report(hours: int = Query(1, ge=1, le=24, description="报告时间范围（小时）")):
    """
    获取详细的性能报告
...
async def readiness_check():
    ...
    try:
        ...
        if unhealthy_critical:
            raise HTTPException(
                status_code=503,
                detail=f"关键组件不健康: {unhealthy_critical}"
            )
