...
# 导入服务定义
try:
    from ...api.grpc import xiaoai_service_pb2_grpc
except ImportError:
    print("警告: gRPC代码未生成，使用模拟模式")
    xiaoai_service_pb2_grpc = None
...
# 导入集成客户端
try:
    from ...integration.look_service.client import get_look_service_client
    from ...integration.listen_service.client import get_listen_service_client
    from ...integration.inquiry_service.client import get_inquiry_service_client
    from ...integration.palpation_service.client import get_palpation_service_client
except ImportError:
    print("警告: 集成服务客户端未找到，使用模拟模式")
    get_look_service_client = lambda: None
    get_listen_service_client = lambda: None
    get_inquiry_service_client = lambda: None
    get_palpation_service_client = lambda: None

# 导入核心组件
try:
    from ..four_diagnosis.feature_extractor import FeatureExtractor
    from ..four_diagnosis.multimodal_fusion import MultimodalFusion
    from ..four_diagnosis.syndrome_analyzer import SyndromeAnalyzer
    from ..four_diagnosis.recommendation.health_advisor import HealthAdvisor
    from ..agent.model_factory import get_model_factory
except ImportError as e:
    print(f"警告: 核心组件导入失败: {e}")
    FeatureExtractor = None
    MultimodalFusion = None
    SyndromeAnalyzer = None
    HealthAdvisor = None
    get_model_factory = lambda: None
...
try:
    ...
    from ..utils.config_loader import get_config as get_config_manager
    get_metrics_collector = lambda: None

# 导入健康检查和弹性组件
try:
    from ..observability.health_check import setup_health_checker
    from ..resilience.circuit_breaker import get_circuit_breaker
except ImportError:
    setup_health_checker = lambda: None
    get_circuit_breaker = lambda: None
