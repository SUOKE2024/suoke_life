...
@click.pass_context
def server(
    ctx: click.Context, host: str, port: int, workers: int, reload: bool
) -> None:
    """启动小艾智能体服务器"""
...
def worker(ctx: click.Context, concurrency: int, queue: str) -> None:
    ...
    run_worker(concurrency=concurrency, queue=queue, config=ctx.obj.get("config"))
...
def health(checkonly: bool) -> None:
    ...
    if check_only: sys.exit(0 if result["healthy"] else 1)
