...
def health_check() -> Dict[str, Any]:
    ...
    try:
        ...
        checks = [
            ...
            ("message_queue", _check_message_queue_health),
            ("ai_models", _check_ai_models_health),
            ("external_services", _check_external_services_health),
        ...
        for check_name, check_func in checks:
            try:
                ...
                result["healthy"] = False
                result["issues"].append(f"{check_name}: 检查失败 - {str(e)}")
        ...
        result["healthy"] = False
        result["issues"].append(f"健康检查执行失败: {str(e)}")
...
def _check_ai_models_health() -> Dict[str, Any]:
    ...
    try:
        ...
        total_memory = sum(model["memory_mb"] for model in models)
        loaded_count = sum(1 for model in models if model["status"] == "loaded")
        
        return {
            "healthy": loaded_count == len(models),
