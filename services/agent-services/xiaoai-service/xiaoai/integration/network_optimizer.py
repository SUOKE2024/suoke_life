...
class NetworkConfig:
    ...
    http2_max_connections: int = 100
    http2_keepalive_expiry: int = 30
...
class ConnectionInfo:
    ...
    connection_type: str = "websocket"  # websocket, http, http2
...
class ConnectionPool:
    ...
    async def add_websocket_connection(self, websocket: WebSocketServerProtocol, user_id: str = None) -> str:
        """添加WebSocket连接"""
        ...
        for conn_info in user_connections:
            if conn_info.connection_id in self.websocket_connections:
                ...
                try:
                    ...
                    await self.remove_connection(conn_info.connection_id)
...
class RateLimiter:
    ...
    async def check_bandwidth_limit(self, client_id: str, bytes_count: int) -> bool:
        """检查带宽限制"""
        if not self.config.rate_limit_enabled:
            return True
        ...
        async with self.lock:
            ...
            # 计算当前带宽使用
            current_bytes = sum(record[1] for record in bandwidth)
            max_bytes = self.config.max_bandwidth_mbps * 1024 * 1024  # MB转字节
            
            if current_bytes + bytes_count > max_bytes:
                return False
            
            # 记录新的带宽使用
            bandwidth.append((now, bytes_count))
            return True

class WebSocketManager:
    """WebSocket管理器"""
    
    def __init__(self, config: NetworkConfig, connection_pool: ConnectionPool):
        self.config = config
    ...
    async def handle_websocket(self, websocket: WebSocketServerProtocol, path: str):
        """处理WebSocket连接"""
        connection_id = await self.connection_pool.add_websocket_connection(websocket)
        ...
        try:
            ...
            async for message in websocket:
                try:
                    ...
                    # 更新连接统计
                    conn_info = await self.connection_pool.get_connection_info(connection_id)
                    if conn_info:
                    ...
                    if message_type in self.message_handlers:
                        ...
                        response = await handler(data, connection_id)
    ...
    async def start_server(self):
        ...
        try:
            self.server = await websockets.serve(
                ...
                max_size=self.config.send_buffer_size,
...
class HTTP2Client:
    ...
    async def get_client(self, base_url: str) -> httpx.AsyncClient:
        ...
        if not HTTP2_AVAILABLE:
            raise Exception("HTTP/2不可用")
        
        async with self.lock:
            if base_url not in self.clients:
                # 创建新的HTTP/2客户端
                limits = httpx.Limits(
                    max_connections=self.config.http2_max_connections,
...
class NetworkOptimizer:
    ...
    def __init__(self, config: NetworkConfig = None):
        ...
        self.data_compressor = DataCompressor(self.config)
        self.rate_limiter = RateLimiter(self.config)
        self.websocket_manager = WebSocketManager(self.config, self.connection_pool)
        self.http2_client = HTTP2Client(self.config)
    ...
    async def initialize(self):
        ...
        try:
            # 注册默认消息处理器
            self.websocket_manager.register_handler("ping", self._handle_ping)
            self.websocket_manager.register_handler("device_request", self._handle_device_request)
    ...
    async def _handle_ping(self, data: Dict[str, Any], connection_id: str) -> Dict[str, Any]:
        """处理ping消息"""
        return {
            "type": "pong",
            "timestamp": time.time(),
    ...
    async def _handle_device_request(self, data: Dict[str, Any], connection_id: str) -> Dict[str, Any]:
        """处理设备请求"""
        # 这里可以集成设备管理器
        return {
    ...
    async def _handle_chat_message(self, data: Dict[str, Any], connection_id: str) -> Dict[str, Any]:
        """处理聊天消息"""
        # 这里可以集成智能体管理器
        return {
            "type": "chat_response",
    ...
    async def send_to_user(self, user_id: str, message: Dict[str, Any], compress: bool = True) -> bool:
        ...
        try:
            ...
            if compress and len(message_data) > self.config.compression_threshold:
                message_data = await self.data_compressor.compress_data(message_data)
                message["compressed"] = True
            
            # 广播到用户的所有连接
            sent_count = await self.connection_pool.broadcast_to_user(user_id, message)
            
            # 更新统计
            self.stats["total_messages"] += sent_count
            self.stats["total_bytes_sent"] += len(message_data) * sent_count
    ...
    async def make_http2_request(self, url: str, method: str = "GET", **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            # 获取客户端
            client = await self.http2_client.get_client(base_url)
    ...
    async def get_network_stats(self) -> Dict[str, Any]:
        """获取网络统计信息"""
        connection_stats = await self.connection_pool.get_stats()
        
        return {
            "connections": connection_stats,
            "compression": self.data_compressor.compression_stats,
    ...
    async def optimize_connection(self, connection_id: str) -> Dict[str, Any]:
        ...
        # 推荐HTTP/2
        if conn_info.connection_type == "http" and self.config.http2_enabled:
            recommendations.append("升级到HTTP/2")
        
        # 推荐WebSocket
        if conn_info.messages_sent > 10 and conn_info.connection_type == "http":
