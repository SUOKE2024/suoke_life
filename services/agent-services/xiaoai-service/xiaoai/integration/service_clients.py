...
from ..utils.exceptions import ServiceUnavailableError, CommunicationError
...
class BaseServiceClient(ABC):
    ...
    async def _http_health_check(self) -> bool:
        ...
        try:
            url = f"http://{self.endpoint.host}:{self.endpoint.port}{self.endpoint.health_check_path}"
...
class LookServiceClient(BaseServiceClient):
    ...
    async def _grpc_analyze(self, user_id: str, session_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """gRPC望诊分析"""
        ...
        return {
            ...
            "tongue_analysis": json.loads(response.tongue_analysis) if response.tongue_analysis else {},
    ...
    async def _http_analyze(self, user_id: str, session_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """HTTP望诊分析"""
...
class ListenServiceClient(BaseServiceClient):
    ...
    async def _grpc_analyze(self, user_id: str, session_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "confidence": response.confidence,
            "features": json.loads(response.features) if response.features else {},
            "voice_analysis": json.loads(response.voice_analysis) if response.voice_analysis else {},
            "breathing_analysis": json.loads(response.breathing_analysis) if response.breathing_analysis else {},
            "cough_analysis": json.loads(response.cough_analysis) if response.cough_analysis else {}
        }
...
class InquiryServiceClient(BaseServiceClient):
    ...
    async def _grpc_analyze(self, user_id: str, session_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "confidence": response.confidence,
            "features": json.loads(response.features) if response.features else {},
            "conversation_analysis": json.loads(response.conversation_analysis) if response.conversation_analysis else {},
            "symptom_analysis": json.loads(response.symptom_analysis) if response.symptom_analysis else {},
            "risk_assessment": json.loads(response.risk_assessment) if response.risk_assessment else {}
        }
...
class PalpationServiceClient(BaseServiceClient):
    ...
    async def _grpc_analyze(self, user_id: str, session_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "confidence": response.confidence,
            "features": json.loads(response.features) if response.features else {},
            "pulse_analysis": json.loads(response.pulse_analysis) if response.pulse_analysis else {},
            "pressure_analysis": json.loads(response.pressure_analysis) if response.pressure_analysis else {},
            "acupoint_analysis": json.loads(response.acupoint_analysis) if response.acupoint_analysis else {}
        }
...
class CalculationServiceClient(BaseServiceClient):
    ...
    async def _grpc_analyze(self, user_id: str, session_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "confidence": response.confidence,
            "features": json.loads(response.features) if response.features else {},
            "biomarker_analysis": json.loads(response.biomarker_analysis) if response.biomarker_analysis else {},
            "risk_calculation": json.loads(response.risk_calculation) if response.risk_calculation else {},
            "trend_analysis": json.loads(response.trend_analysis) if response.trend_analysis else {}
        }
