    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if circuit['state'] == 'OPEN':
                ...
                if current_time - circuit['last_failure_time'] > recovery_time:
                    ...
                    if fallback:
                        ...
                        raise CircuitBreakerError(f"断路器 {circuit_id} 打开，服务暂时不可用", circuit_id)
            ...
            try:
                ...
                # 其他异常也视为失败
                logger.warning(f"断路器 {circuit_id} 操作失败: {str(e)}")
                _handle_failure(circuit_id, current_time)
                raise
...
def rate_limiter(max_calls: int = 10, time_period: int = 1, limiter_id: str = None):
    """
    速率限制器装饰器
    限制一段时间内的最大调用次数
        ...
        async def wrapper(*args, **kwargs):
            ...
            while attempt < max_attempts:
                try:
                    ...
                    logger.warning(f"操作失败，将在 {backoff:.2f} 秒后重试 ({attempt}/{max_attempts}): {str(e)}")
...
def bulkhead(max_concurrent: int = 10, max_queue: int = 5, bulkhead_id: str = None):
    """
    舱壁隔离装饰器
    限制并发执行数量，防止资源过载
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                try:
                    ...
                    raise Exception(f"系统繁忙，请稍后重试 (舱壁 {bulkhead_id} 队列已满)")
...
def timeout(seconds: float, timeout_id: str = None):
    """
    超时装饰器
    为函数添加超时限制
    ...
    def decorator(func):
        nonlocal timeout_id
        timeout_id = timeout_id or func.__name__
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                raise asyncio.TimeoutError(f"操作 {timeout_id} 超时，请稍后重试")
...
def get_circuit_breaker_status(circuit_id: str = None) -> Dict[str, Any]:
    """
    获取断路器状态
    ...
    with _circuit_breakers_lock:
        if circuit_id:
            return _circuit_breakers.get(circuit_id, {'state': 'UNKNOWN'})
        else:
            return {k: v.copy() for k, v in _circuit_breakers.items()}


def get_rate_limiter_status(limiter_id: str = None) -> Dict[str, Any]:
    """
    获取限流器状态
    ...
    with _rate_limiters_lock:
        if limiter_id:
            limiter = _rate_limiters.get(limiter_id, None)
            if limiter:
                # 清理过期的调用记录
                current_time = time.time()
...
def reset_circuit_breaker(circuit_id: str = None) -> bool:
    """
    重置断路器状态
