...
class VoiceEncoder(ModalityEncoder):
    ...
    pass
        ...
        # 取最后一个时间步的隐藏状态
        lasthidden = (
            hidden[-2:].transpose(0, 1).contiguous().view(-1, self.rnn.hidden_size * 2)
        )
...
class InquiryEncoder(ModalityEncoder):
    ...
    def __init__(
        self, vocab_size=10000, embedding_dim=128, hidden_dim=256, output_dim=256:
    ):
    pass
        super().__init__(
            input_dim=embeddingdim, hidden_dim=hiddendim, output_dim=outputdim
        )
        ...
        encoderlayer = nn.TransformerEncoderLayer(
            d_model=embeddingdim, nhead=8, dim_feedforward=hiddendim, dropout=0.1
        )
        self.self.transformer = nn.TransformerEncoder(encoderlayer, num_layers=4)
    ...
    pass
            ...
            attentionmask = (
                ...
                .masked_fill(attentionmask == 0, float("-inf"))
                .masked_fill(attentionmask == 1, 0.0)
            )

        output = self.self.transformer(x, src_key_padding_mask=attentionmask)
...
class CrossModalAttention(nn.Module):
    ...
    pass
        ...
        Args: query_modal: 查询模态特征 [batchsize, query_dim]
    ...
    pass
        ...
        # 重塑为多头形式
        q = q.view(batchsize, 1, self.numheads, self.headdim).transpose(
            1, 2
        )  # [batchsize, numheads, 1, head_dim]
        k = k.view(-1, 1, self.numheads, self.headdim).transpose(
            1, 2
        )  # [num_modals*batchsize, numheads, 1, head_dim]
        v = v.view(-1, 1, self.numheads, self.headdim).transpose(
            1, 2
        )  # [num_modals*batchsize, numheads, 1, head_dim]
        ...
        )  # [batchsize, numheads, 1, num_modals*batch_size]
...
class MultimodalFusionModule(nn.Module):
    ...
    pass
        ...
        self.globalfusion = nn.MultiheadAttention(
            embed_dim=featuredim, num_heads=8, dropout=0.1
        )
...
:
    ...
    pass
            ...
            fusion_features["inquiry"] = self.inquiry_fusion(
                features["inquiry"], context
            )
...
:
    ...
    pass
        """全局特征融合"""
        # 将所有融合特征堆叠为序列
        modalsequence = torch.stack(
            list(fusion_features.values()), dim=0
        )  # [nummodals, batchsize, dim]

        # 应用自注意力机制
        attnoutput, _ = self.global_fusion(modalsequence, modalsequence, modal_sequence)
    ...
    pass
        ...
        features, availablemodalities = self.encode_modalities(
            tonguedata, voicedata, pulsedata, inquirydata, voicelengths, inquiry_mask
        )
...
:
        return {"tcm_features": tcmfeatures, "constitution_logits": constitution_logits}
