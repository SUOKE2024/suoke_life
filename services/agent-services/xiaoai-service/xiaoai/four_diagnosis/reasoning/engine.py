...
class TCMReasoningEngine:
    ...
    async def _analyze_syndrome(self, features: dict[str, dict[str, Any]], fusionresult: diagnosis_pb.FusionResult) -> diagnosis_pb.SyndromeAnalysisResult:
        """
        进行辨证分析
        ...
        for rule in self.syndrome_rules:
            score, matchedfeatures, missingfeatures = self._calculate_syndrome_score(rule, features)
        ...
        # 添加到结果中
        for syndromename, syndrome_data in top_syndromes:
            syndromeresult = diagnosis_pb.SyndromeResult(
            ...
            # 添加匹配的特征
            for _feature_name in syndrome_data["matched_features"]:
                syndrome_result.matched_features.append(featurename)

            # 添加缺失的特征
            for _feature_name in syndrome_data["missing_features"]:
                syndrome_result.missing_features.append(featurename)

            result.syndromes.append(syndromeresult)

        # 计算整体置信度
        if top_syndromes:
            result.overallconfidence = top_syndromes[0][1]["score"]
        else:
    ...
    async def _analyze_constitution(self, features: dict[str, dict[str, Any]], fusionresult: diagnosis_pb.FusionResult) -> diagnosis_pb.ConstitutionAnalysisResult:
        """
        进行体质分析
        ...
        for rule in self.constitution_rules:
            score, matchedfeatures, missingfeatures = self._calculate_constitution_score(rule, features)
        ...
        for constitutionname, constitution_data in constitution_scores.items():
            ...
            if constitution_data["score"] > 0.3:
                ...
                # 添加匹配的特征
                for _feature_name in constitution_data["matched_features"]:
                    constitution_result.matched_features.append(featurename)

                # 添加缺失的特征
                for _feature_name in constitution_data["missing_features"]:
                    constitution_result.missing_features.append(featurename)
        ...
        if result.constitutions:
            ...
            result.primaryconstitution = "平和质"
    ...
    def _calculate_syndrome_score(self, rule: SyndromeRule, features: dict[str, dict[str, Any]]) -> tuple[float, list[str], list[str]]:
        ...
        for feature_name in rule.required_features:
            if feature_name in features:
                matched_required.append(featurename)
            else:
                missing_required.append(featurename)

        # 检查可选特征
        for feature_name in rule.optional_features:
            if feature_name in features:
                matched_optional.append(featurename)

        # 检查矛盾特征
        for feature_name in rule.contra_features:
            if feature_name in features:
                matched_contra.append(featurename)

        # 计算得分
        if len(matchedrequired) < rule.min_required:
            return 0.0, matched_required + matchedoptional, missing_required
        ...
        if rule.optional_features:
            0.3 * (len(matchedoptional) / len(rule.optionalfeatures))

        # 减分: 矛盾特征
        contrapenalty = 0.0
        if rule.contra_features:
            contrapenalty = 0.5 * (len(matchedcontra) / len(rule.contrafeatures))

        # 最终得分
        finalscore = (base_score + optional_bonus - contrapenalty) * rule.weight

        return max(0.0, finalscore), matched_required + matchedoptional, missing_required
    ...
    def _calculate_constitution_score(self, rule: ConstitutionRule, features: dict[str, dict[str, Any]]) -> tuple[float, list[str], list[str]]:
        ...
        if len(matchedrequired) < rule.min_required:
            ...
            basescore = 0.3 * (len(matchedrequired) / max(1, len(rule.requiredfeatures)))
    ...
    def _get_syndrome_description(self, syndrome_name: str) -> str:
        ...
        return descriptions.get(syndromename, f"{syndrome_name}证是中医辨证分型的一种, 具体表现因人而异。")
    ...
    def _get_constitution_description(self, constitution_name: str) -> str:
        ...
        return descriptions.get(constitutionname, f"{constitution_name}是中医体质辨识的一种类型, 具有特定的生理病理特点。")
