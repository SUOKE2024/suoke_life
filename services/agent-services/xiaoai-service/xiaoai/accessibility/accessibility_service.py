...
class AccessibilityService:
    ...
    async def initialize(self, config: Optional[AccessibilityConfig] = None):
        ...
        try:
            ...
            self.config = config or AccessibilityConfig(
                mode=AccessibilityMode.MULTI_MODAL
            )
    ...
    async def recognize_microphone_input(self, timeout: float = 5.0, language: str = "zh-CN") -> Optional[str]:
        """识别麦克风输入"""
    ...
    async def recognize_hand_gestures(self, image_data: Union[bytes, np.ndarray]) -> List[Dict[str, Any]]:
        """识别手势"""
    ...
    async def recognize_sign_language(self, video_frames: List[np.ndarray]) -> Optional[str]:
        ...
        try:
            ...
            for frame in video_frames:
                ...
                frame_data = {
                    "hands": hand_results.multi_hand_landmarks if hand_results.multi_hand_landmarks else [],
    ...
    async def update_config(self, new_config: AccessibilityConfig):
        ...
        try:
            ...
            if (self.config.sign_language or self.config.gesture_control) and not self.hands_detector:
                await self._initialize_sign_language()
