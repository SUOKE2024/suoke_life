...
class HealthChecker:
    ...
    def __init__(self):
        ...
        self.dependency_checkers: Dict[str, Callable[[], Awaitable[DependencyStatus]]] = {}
        self.check_interval = 30  # 健康检查间隔秒数
    ...
    def start_background_checks(self):
        ...
        if self.bg_task is None:
            self.bg_task = asyncio.create_task(self._background_check_loop())
    ...
    async def _background_check_loop(self):
        ...
        try:
            ...
            logger.error(f"后台健康检查任务异常: {str(e)}")
    
    def register_dependency(self, name: str, check_func: Callable[[], Awaitable[DependencyStatus]]):
        """
        注册依赖服务检查函数
    ...
    async def _check_dependencies(self):
        ...
        # 为每个依赖服务创建检查任务
        for name, check_func in self.dependency_checkers.items():
            check_tasks.append(self._check_dependency(name, check_func))
        
        # 并行执行所有检查
        if check_tasks:
            await asyncio.gather(*check_tasks)
    
    async def _check_dependency(self, name: str, check_func: Callable[[], Awaitable[DependencyStatus]]):
        """
        检查单个依赖服务
        ...
        try:
            ...
            # 更新延迟
            self.health_status.dependencies[name].latency_ms = elapsed_time
            
            logger.debug(f"依赖服务 {name} 健康检查完成: {status.status.name}")
            
        except Exception as e:
            # 发生异常，标记为不可用
            logger.error(f"依赖服务 {name} 健康检查异常: {str(e)}")
            self.health_status.dependencies[name] = DependencyStatus(
                name=name,
                status=service_pb.HealthCheckResponse.Status.NOT_SERVING,
                error_message=str(e),
    ...
    def _update_system_info(self):
        ...
        try:
            ...
            logger.error(f"更新系统信息异常: {str(e)}")
            self.health_status.details["error"] = str(e)
    
    def _determine_overall_status(self):
        """确定整体健康状态"""
        # 默认为可用
        overall_status = service_pb.HealthCheckResponse.Status.SERVING
        
        # 如果有任何关键依赖不可用，则整体不可用
        critical_dependencies = {"look-service", "listen-service", "inquiry-service", "palpation-service"}
        ...
        for name, status in self.health_status.dependencies.items():
            if name in critical_dependencies and status.status == service_pb.HealthCheckResponse.Status.NOT_SERVING:
                overall_status = service_pb.HealthCheckResponse.Status.NOT_SERVING
    ...
    try:
        ...
        return DependencyStatus(
            ...
            status=service_pb.HealthCheckResponse.Status.SERVING if health_response.status else 
                   service_pb.HealthCheckResponse.Status.NOT_SERVING,
            last_check_time=time.time()
        )
    except Exception as e:
        return DependencyStatus(
...
async def setup_health_checker():
    ...
    # 注册各种依赖服务的健康检查
    health_checker.register_dependency("look-service", check_look_service_health)
    health_checker.register_dependency("listen-service", check_listen_service_health)
    health_checker.register_dependency("inquiry-service", check_inquiry_service_health)
    health_checker.register_dependency("palpation-service", check_palpation_service_health)
