...
class ConstitutionAnalyzer:
    ...
    async def analyze(self, diagnosis_results: List[DiagnosisResult]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"体质分析完成，主导体质：{dominant_constitution.constitution_type.value if dominant_constitution else '未确定'}")
            
            return {
                "dominant_constitution": self._constitution_score_to_dict(dominant_constitution) if dominant_constitution else None,
                "constitution_scores": [self._constitution_score_to_dict(score) for score in constitution_scores],
    ...
    async def _extract_constitution_features(self, diagnosis_results: List[DiagnosisResult]) -> Dict[str, Any]:
        """提取体质相关特征"""
        features = {
    ...
    def _extract_constitution_symptoms(self, conversation: Dict[str, Any]) -> List[str]:
        ...
        constitution_symptoms = [
            "畏寒怕冷", "手足不温", "容易疲乏", "容易出汗", "手足心热",
            "口燥咽干", "形体肥胖", "容易困倦", "痰多", "面垢油腻",
            "口苦口干", "肤色晦暗", "容易出现瘀斑", "情绪不稳定",
            "胸胁胀满", "善太息", "过敏体质", "鼻塞流涕"
        ]
    ...
    async def _calculate_constitution_scores(self, features: Dict[str, Any]) -> List[ConstitutionScore]:
        """计算各体质得分"""
        constitution_scores = []
        ...
        for constitution_type, constitution_features in self.constitution_features.items():
            ...
            constitution_scores.append(ConstitutionScore(
    ...
    def _feature_matches_constitution(self, feature: ConstitutionFeature, extracted_features: Dict[str, Any]) -> bool:
        """检查特征是否匹配体质"""
        ...
        if "舌淡红苔薄白" in feature_name and tongue.get("color")=="淡红" and "薄白" in tongue.get("coating", ""):
            return True
        if "舌淡红" in feature_name and tongue.get("color")=="淡红":
            return True
        if "舌淡胖" in feature_name and tongue.get("color")=="淡" and "胖" in tongue.get("shape", ""):
            return True
        if "舌红少津" in feature_name and tongue.get("color")=="红" and "少津" in tongue.get("moisture", ""):
            return True
        ...
        if "脉和缓有力" in feature_name and "和缓" in pulse.get("rhythm", "") and "有力" in pulse.get("strength", ""):
            return True
        if "脉弱" in feature_name and "弱" in pulse.get("strength", ""):
            return True
        if "脉沉迟" in feature_name and "沉" in pulse.get("depth", "") and "迟" in pulse.get("rate", ""):
            return True
        if "脉细数" in feature_name and "细" in pulse.get("strength", "") and "数" in pulse.get("rate", ""):
            return True
        if "脉滑" in feature_name and "滑" in pulse.get("rhythm", ""):
            return True
        if "脉滑数" in feature_name and "滑" in pulse.get("rhythm", "") and "数" in pulse.get("rate", ""):
            return True
        if "脉涩" in feature_name and "涩" in pulse.get("rhythm", ""):
            return True
        if "脉弦" in feature_name and "弦" in pulse.get("strength", ""):
            return True
        if "脉濡弱" in feature_name and "濡" in pulse.get("strength", "") and "弱" in pulse.get("strength", ""):
            return True
    ...
    def _determine_dominant_constitution(self, constitution_scores: List[ConstitutionScore]) -> Optional[ConstitutionScore]:
        """确定主导体质"""
    ...
    def _determine_constitution_tendency(self, constitution_scores: List[ConstitutionScore]) -> List[ConstitutionType]:
        """确定体质倾向"""
        tendency = []
        for score in constitution_scores:
            if (score.normalized_score>=self.scoring_rules["tendency_threshold"] and 
                score.confidence>=self.scoring_rules["min_confidence"]):
    ...
    async def _generate_health_guidance(
        ...
        return {
            ...
            "additional_notes": self._generate_additional_notes(constitution_type, constitution_tendency)
        }
    
    def _generate_additional_notes(
        self, 
        main_constitution: ConstitutionType,
        constitution_tendency: List[ConstitutionType]
    ) -> List[str]:
        """生成额外注意事项"""
        notes = []
        ...
        if len(constitution_tendency) > 1:
            if ConstitutionType.QI_DEFICIENCY in constitution_tendency and ConstitutionType.YANG_DEFICIENCY in constitution_tendency:
                notes.append("气阳两虚，需要温补气阳，避免过度劳累")
            
            if ConstitutionType.YIN_DEFICIENCY in constitution_tendency and ConstitutionType.DAMP_HEAT in constitution_tendency:
                notes.append("阴虚湿热并存，需要滋阴清热，避免辛辣燥热食物")
            
            if ConstitutionType.QI_STAGNATION in constitution_tendency and ConstitutionType.BLOOD_STASIS in constitution_tendency:
