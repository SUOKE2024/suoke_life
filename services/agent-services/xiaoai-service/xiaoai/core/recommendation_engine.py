...
class RecommendationEngine:
    ...
    def _load_recommendation_templates(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "diet_yang_deficiency": {
                ...
                "specific_foods": {
                    "recommended": ["羊肉", "韭菜", "生姜", "桂圆", "核桃", "红枣", "小米"],
            ...
            "diet_yin_deficiency": {
                ...
                "specific_foods": {
                    "recommended": ["百合", "银耳", "枸杞", "梨", "蜂蜜", "芝麻", "鸭肉"],
            ...
            "diet_qi_deficiency": {
                ...
                "specific_foods": {
                    "recommended": ["山药", "大枣", "小米", "南瓜", "土豆", "鸡肉", "鲫鱼"],
    ...
    async def _generate_prevention_recommendations(
        ...
        if user_profile and "age" in user_profile:
            ...
            if age>=65:
                recommendations.append(
                    self._create_elderly_care_recommendation()
                )
            elif age>=40:
                recommendations.append(
                    self._create_middle_age_care_recommendation()
                )
    ...
    async def _check_interactions(self, recommendations: List[Recommendation]) -> List[Recommendation]:
        """检查相互作用"""
        # 简单的相互作用检查
        for i, rec1 in enumerate(recommendations):
            for j, rec2 in enumerate(recommendations[i+1:], i+1):
                # 检查是否有冲突的建议
                if (rec1.type==RecommendationType.DIET and 
                    rec2.type==RecommendationType.DIET):
                    # 检查饮食建议是否冲突
    ...
    async def _prioritize_and_optimize(self, recommendations: List[Recommendation]) -> List[Recommendation]:
        """排序和优化建议"""
        # 按优先级和置信度排序
    ...
    def _create_follow_up_schedule(self, recommendations: List[Recommendation]) -> Dict[str, str]:
        ...
        # 根据建议类型确定随访时间
        has_high_priority = any(rec.priority==Priority.URGENT or rec.priority==Priority.HIGH 
