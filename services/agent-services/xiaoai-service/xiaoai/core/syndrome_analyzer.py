...
class SyndromeAnalyzer:
    ...
    def _parse_symptoms(self, chief_complaint: str) -> List[str]:
        ...
        # 简单的症状关键词提取
        symptom_keywords = [
            "畏寒", "四肢不温", "精神萎靡", "五心烦热", "盗汗", "口干",
            "神疲乏力", "气短懒言", "自汗", "刺痛", "痛有定处", "胸闷",
            "痰多", "身重", "胸胁胀痛", "情志抑郁", "善太息", "食少腹胀",
            "便溏", "倦怠乏力", "腰膝酸冷", "阳痿", "小便清长", "心悸",
            "失眠", "健忘", "面色淡白", "咳嗽气短", "声音低微", "易感冒"
        ]
    ...
    def _feature_matches(self, feature: str, extracted_features: Dict[str, Any]) -> bool:
        ...
        if feature=="舌淡" and tongue_features.get("color") in ["淡红", "淡白"]:
            return True
        if feature=="舌红少苔" and tongue_features.get("color")=="红" and "少苔" in tongue_features.get("coating", ""):
            return True
        ...
        if feature=="脉沉迟" and pulse_features.get("depth")=="沉" and "迟" in pulse_features.get("rate", ""):
            return True
        if feature=="脉细数" and "细" in pulse_features.get("strength", "") and "数" in pulse_features.get("rate", ""):
            return True
        ...
        if feature=="脉弦" and "弦" in pulse_features.get("strength", ""):
            return True
        if feature=="脉缓弱" and "缓" in pulse_features.get("rate", "") and "弱" in pulse_features.get("strength", ""):
            return True
        
        return False
    
    async def _identify_syndromes(self, syndrome_scores: Dict[str, float]) -> Tuple[List[SyndromePattern], List[SyndromePattern]]:
        """识别主要证型和次要证型"""
        # 按分数排序
