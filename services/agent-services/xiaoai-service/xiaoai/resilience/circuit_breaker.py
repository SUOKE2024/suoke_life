...
class CircuitState(Enum):
    ...
    HALFOPEN = 'HALF_OPEN'    # 半开状态, 允许部分请求通过进行测试


@dataclass
class CircuitBreakerConfig:
    """断路器配置"""
    failurethreshold: int = 5         # 触发断路器的失败次数阈值
    successthreshold: int = 2         # 恢复正常所需的成功次数
    timeoutseconds: int = 30          # 断路器打开后的超时时间(秒)
    excludeexceptions: list[type] = None  # 不计入失败次数的异常类型列表
...
async def with_retry(
    ...
    retryexceptions: list[type] | None = None
) -> T:
    """
    重试机制包装器
    ...
    while True:
        try:
            ...
            logger.warning(f"操作失败 (重试 {retries}/{max_retries}), 等待 {delay:.2f} 秒后重试. 异常: {e!s}")
