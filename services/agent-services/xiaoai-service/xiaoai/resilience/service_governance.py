    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            return {
                ...
                'utilization': 1 - (self.tokens / self.config.max_requests)
            }
...
class LoadBalancer:
    ...
    def _least_connections(self) -> str:
        ...
        with self.lock:
            ...
            selected_endpoint = self.endpoints[0]
...
class ServiceGovernance:
    ...
    async def call_service(self, service_name: str, func: Callable, *args, **kwargs):
        ...
        if rate_limiter and not await rate_limiter.acquire_async(timeout=1.0):
            raise Exception(f"服务 {service_name} 请求被限流")
        ...
        try:
            ...
            if load_balancer and endpoint:
                ...
                load_balancer.record_request(endpoint, response_time, success)
    ...
    async def call_http_service(self, service_name: str, method: str, url: str, 
        ...
        async with self.call_service(service_name, http_call) as result:
            return result
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        health['summary']['total_services'] = len(service_names)
        
        for service_name in service_names:
            service_health = {'status': 'healthy', 'issues': []}
...
def get_service_governance() -> ServiceGovernance:
    ...
    if _service_governance is None:
        _service_governance = ServiceGovernance()
    ...
    def decorator(func):
        async def wrapper(*args, **kwargs):
            ...
            if config and service_name not in governance.circuit_breakers:
                governance.register_circuit_breaker(service_name, config)
            
            async with governance.call_service(service_name, func, *args, **kwargs) as result:
                return result
...
def with_rate_limit(service_name: str, config: RateLimiterConfig = None):
    """限流装饰器"""
    def decorator(func):
        async def wrapper(*args, **kwargs):
            governance = get_service_governance()
            ...
            if config and service_name not in governance.rate_limiters:
                governance.register_rate_limiter(service_name, config)
