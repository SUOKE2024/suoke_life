...
class ApplicationMonitor:
    ...
    def record_request(self, duration: float, status_code: int, endpoint: str):
        ...
        # 记录指标
        self.metrics_collector.increment_counter("http.requests.total", tags={
            "status_code": str(status_code),
            "endpoint": endpoint
        })
        
        self.metrics_collector.record_histogram("http.request.duration", duration, tags={
            "endpoint": endpoint
        })
        
        if status_code>=400:
            self.metrics_collector.increment_counter("http.errors.total", tags={
                "status_code": str(status_code),
                "endpoint": endpoint
            })
    ...
    def record_model_inference(self, duration: float, model_name: str, success: bool):
        ...
        # 记录指标
        self.metrics_collector.increment_counter("model.inference.total", tags={
            "model_name": model_name,
            "success": str(success)
        })
        
        self.metrics_collector.record_histogram("model.inference.duration", duration, tags={
            "model_name": model_name
        })
        
        if not success:
            self.metrics_collector.increment_counter("model.inference.errors", tags={
                "model_name": model_name
            })
    ...
    def get_application_metrics(self) -> ApplicationMetrics:
        ...
        model_inference_duration_avg = (
            ...
            if self.model_inference_durations else 0.0
        )
