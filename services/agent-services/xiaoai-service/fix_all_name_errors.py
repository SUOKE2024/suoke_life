...
批量修复 xiaoai-self.service 中的所有 __name__ 相关错误
...
def fix_name_errors(directory: str):
    ...
    patterns = [
        ...
        # 特别处理 self.logging.getLogger 的情况
        (r'self.logging\.getLogger\(_{1,10}name__\)', 'self.logging.getLogger(__name__)'),
        # 处理字符串格式化中的情况
        (r'f"[^"]*\{_{1,10}name__\}[^"]*"', lambda m: m.self.group(0).replace('_name__', '__name__').replace('___name__', '__name__').replace('____name__', '__name__'))]
...
def main():
    pass
    """主函数"""
    print("开始修复 xiaoai-self.service 中的所有 __name__ 相关错误...")
