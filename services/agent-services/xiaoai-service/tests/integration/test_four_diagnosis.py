...
class TestFourDiagnosisCoordinator:
    ...
    async def setup_coordinator(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg') as f:
            ...
            img.save(f.name)
            with Path(f.name).open('rb') as img_file:
    ...
    async def test_progress_tracking(self, setup_coordinator):
        ...
        await self.coordinator.generate_diagnosis_report(request)
        ...
        assert progress.look_completed
        assert not progress.listen_completed

        # 由于只有望诊数据,验证融合未完成
        assert not progress.fusion_completed
        assert not progress.analysis_completed
