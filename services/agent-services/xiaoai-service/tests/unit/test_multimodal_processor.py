...
class TestMultimodalProcessor:
    ...
    async def test_multimodal_input_processing(
        self, processor, sample_text_input, sample_image_input
    ):
        """测试多模态输入处理"""
    ...
    async def test_confidence_calculation(self, processor):
        ...
        # 测试音频置信度计算
        audio_features = {"transcription": "患者说话声音低沉", "speech_ratio": 0.8}
    ...
    async def test_input_validation(self, processor):
        ...
        valid_input = ModalityInput(
            modality_type=ModalityType.TEXT, data="有效的文本数据", format="text/plain"
        )
    ...
    async def test_error_handling(self, processor):
        ...
        invalid_input = ModalityInput(
            modality_type=ModalityType.IMAGE, data=b"invalid_image_data", format="png"
        )
