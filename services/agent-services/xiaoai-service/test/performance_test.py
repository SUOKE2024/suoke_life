...
# 导入Proto生成的类
from xiaoai_service.protos import (
    four_diagnosis_pb2 as diagnosis_pb,
    xiaoai_service_pb2 as service_pb,
    xiaoai_service_pb2_grpc as service_grpc
)
...
class PerformanceTester:
    ...
    async def run_performance_tests(self, request_counts=[1, 5, 10, 20]):
        """
        运行所有性能测试
    ...
    def _generate_performance_report(self, all_results, request_counts):
        ...
        plt.savefig(os.path.join(report_dir, f'performance_report_{time.strftime("%Y%m%d_%H%M%S")}.png'))
        ...
        # 保存文本报告
        with open(os.path.join(report_dir, f'performance_report_{time.strftime("%Y%m%d_%H%M%S")}.md'), 'w', encoding='utf-8') as f:
            f.write(report_text)
