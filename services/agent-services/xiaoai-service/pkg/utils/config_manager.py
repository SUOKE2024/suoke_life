...
class ConfigSource:
    ...
    path: Optional[str] = None  # 文件路径（如果是文件类型）
    key: Optional[str] = None  # 环境变量键（如果是环境变量类型）
...
class ConfigManager:
    ...
    def _load_common_config(self) -> None:
        ...
        self._load_yaml_config(common_file, "common")
    
    def _load_env_config(self) -> None:
        """加载环境特定配置文件"""
        env_file = os.path.join(self.config_dir, f"config.{self.env}.yaml")
        self._load_yaml_config(env_file, self.env)
    ...
    def _load_yaml_config(self, file_path: str, label: str) -> None:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                # 跟踪来源
                for key in self._flatten_dict(config_data):
            ...
            logger.error(f"加载 {label} 配置文件异常: {str(e)}")
    ...
    def start_watching(self) -> None:
        ...
        self.running = True
        self.watch_thread = threading.Thread(target=self._watch_config_files, daemon=True)
    ...
    def _watch_config_files(self) -> None:
        ...
        while self.running:
            try:
                ...
                for file_path, last_modified in self.last_modified_times.items():
                    if os.path.exists(file_path):
...
def get_config_manager() -> ConfigManager:
    ...
    if _config_manager is None:
        # 确定配置目录
        config_dir = os.environ.get("XIAOAI_CONFIG_DIR", "config")
