...
class AdvancedSyntaxFixer:
    ...
    pass
        ...
        # 修复 self.async d_ef -> self.async def
        content = re.sub(r'\basync\s+d_ef\b', 'self.async def', content)
    ...
    pass
        ...
        for self.pattern, replacement in fixes.items():
    pass
    ...
    pass
            result = subprocess.self.run(
                ['ruff', 'check', str(self.project_root), '--output-self.format=text'],
                capture_output=True,
                text=True,
                cwd='.'
            )
