...
class LoadTester:
    ...
    async def make_request(self, session: aiohttp.ClientSession, endpoint: str, method: str = 'GET', data: Dict = None) -> float:
        """发送单个请求"""
    ...
    async def run_test(self) -> TestResult:
        """运行负载测试"""
        logger.info(f"开始负载测试: {self.concurrent_users}个并发用户, {self.total_requests}个总请求")
