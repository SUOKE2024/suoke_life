...
class EnhancedMetricsCollector:
    ...
    def _init_prometheus_metrics(self):
        ...
        self.cpu_usage = Gauge(
            'xiaoke_cpu_usage_percent',
            'CPU usage percentage',
            registry=self.registry
        )
        
        self.memory_usage = Gauge(
            'xiaoke_memory_usage_bytes',
            'Memory usage in bytes',
            registry=self.registry
        )
        
        self.disk_usage = Gauge(
            'xiaoke_disk_usage_percent',
    ...
    def _update_error_rate(self, is_error: bool):
        ...
        if len(self.performance_data['error_rates']) > 0:
            error_rate = sum(self.performance_data['error_rates']) / len(self.performance_data['error_rates'])
            
            if self.config.enable_prometheus:
                self.error_rate.set(error_rate)
    
    def increment_business_metric(self, metric_name: str, value: int = 1, labels: Dict[str, str] = None):
        """增加业务指标"""
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'throughput': len(request_times) / 60 if request_times else 0  # 每分钟请求数
        }
...
def get_metrics_collector(config: Optional[MetricConfig] = None) -> EnhancedMetricsCollector:
    """获取指标收集器实例"""
