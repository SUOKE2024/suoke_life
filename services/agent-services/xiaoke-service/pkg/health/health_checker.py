...
class HealthChecker(ABC):
    ...
    def _update_status(self, result: HealthCheckResult):
        ...
        if result.status == HealthStatus.HEALTHY:
            ...
            # 检查是否从不健康状态恢复
            if (self.current_status != HealthStatus.HEALTHY and 
...
class DatabaseHealthChecker(HealthChecker):
    ...
    def __init__(self, name: str, connection_string: str, 
                 db_type: str = "postgresql", config: HealthCheckConfig = None):
        """
        初始化数据库健康检查器
...
class HTTPHealthChecker(HealthChecker):
    ...
    def __init__(self, name: str, url: str, expected_status: int = 200,
                 headers: Dict[str, str] = None, config: HealthCheckConfig = None):
        """
        初始化HTTP健康检查器
...
class CustomHealthChecker(HealthChecker):
    ...
    def __init__(self, name: str, check_function: Callable, config: HealthCheckConfig = None):
        """
        初始化自定义健康检查器
...
class HealthCheckManager:
    ...
    def register_database_checker(self, name: str, connection_string: str,
                                db_type: str = "postgresql", config: HealthCheckConfig = None):
        """注册数据库健康检查器"""
        checker = DatabaseHealthChecker(name, connection_string, db_type, config)
        self.register_checker(checker)
    
    def register_redis_checker(self, name: str, redis_url: str, config: HealthCheckConfig = None):
        """注册Redis健康检查器"""
        checker = RedisHealthChecker(name, redis_url, config)
        self.register_checker(checker)
    
    def register_http_checker(self, name: str, url: str, expected_status: int = 200,
                            headers: Dict[str, str] = None, config: HealthCheckConfig = None):
        """注册HTTP健康检查器"""
        checker = HTTPHealthChecker(name, url, expected_status, headers, config)
        self.register_checker(checker)
    
    def register_custom_checker(self, name: str, check_function: Callable,
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'summary': {
                ...
                'enabled_checkers': len([c for c in self.checkers.values() if c.config.enabled]),
                'healthy_checkers': len([c for c in self.checkers.values() if c.current_status == HealthStatus.HEALTHY]),
                'unhealthy_checkers': len([c for c in self.checkers.values() if c.current_status == HealthStatus.UNHEALTHY]),
                'degraded_checkers': len([c for c in self.checkers.values() if c.current_status == HealthStatus.DEGRADED])
            }
        }
