...
class QueueManager:
    ...
    async def enqueue_task(self, task_name: str, payload: Dict[str, Any], 
                          config: TaskConfig = None, queue_name: str = "default") -> str:
        """
        将任务加入队列
    ...
    async def start_workers(self, num_workers: int = 4, queue_names: List[str] = None):
        ...
        logger.info("启动了 %d 个工作进程，处理队列: %s", len(self.workers), queue_names)
    ...
    async def _dequeue_redis(self, queue_name: str) -> Optional[Dict[str, Any]]:
        ...
        task_json = await self.redis_client.brpop(
            [f"priority:{queue_name}", f"queue:{queue_name}"],
            timeout=1
        )
    ...
    async def _process_task(self, task_data: Dict[str, Any], worker_name: str):
        ...
        logger.info("工作进程 %s 开始处理任务: %s (%s)", worker_name, task_name, task_id)
