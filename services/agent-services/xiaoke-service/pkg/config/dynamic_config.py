...
class DynamicConfigManager:
    ...
    async def _load_file_config(self, file_path: str, format_type: ConfigFormat):
        ...
        try:
            ...
            await self._update_config(
                data, ConfigSource.FILE, f"从文件加载: {file_path}"
            )
    ...
    async def _load_environment_config(self):
        ...
        if env_config:
            await self._update_config(
                env_config, ConfigSource.ENVIRONMENT, "从环境变量加载"
            )
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(self.remote_config_url) as response:
                        if response.status == 200:
                            ...
                            logger.warning(
                                "远程配置请求失败，状态码: %d", response.status
                            )
            ...
            for file_path in self.watched_files.keys():
                ...
                if dir_path not in watched_dirs:
                    self.file_observer.schedule(
                        self.file_handler, dir_path, recursive=False
                    )
    ...
    async def rollback_to_version(self, version: str) -> bool:
        ...
        await self._update_config(
            target_version.data, target_version.source, f"回滚到版本: {version}"
        )
    ...
    async def export_config(
        self, file_path: str, format_type: ConfigFormat = ConfigFormat.JSON
    ):
        """
        导出配置到文件
