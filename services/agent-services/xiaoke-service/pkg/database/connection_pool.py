...
class ConnectionPoolManager:
    ...
    def __init__(self, pg_config: PostgreSQLConfig = None, 
    ...
    async def _initialize_postgresql(self):
        ...
        try:
            # 构建连接字符串
            dsn = f"postgresql://{self.pg_config.username}:{self.pg_config.password}@" \
            ...
            logger.info("PostgreSQL连接池创建成功，最小连接数: %d, 最大连接数: %d", 
                       self.pg_config.min_size, self.pg_config.max_size)
