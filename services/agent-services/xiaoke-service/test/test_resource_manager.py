...
class TestResourceManager:
    ...
    def resource_manager(self):
        """创建资源管理器实例"""
        with (
            patch("internal.scheduler.resource_manager.get_config"),
    ...
    async def test_schedule_resources(self, resource_manager):
        ...
        mock_resources = [
            {
                ...
                "metadata": {"experience_years": "15", "hospital": "北京中医医院"},
            },
            {
                "id": "doc2",
                "name": "李医生",
                ...
                "description": "专注于肝胆调理",
                "price": 180.0,
                "available_times": ["2023-07-01T14:00", "2023-07-01T16:00"],
        ...
        # 模拟存储库方法
        resource_manager.resource_repo.get_resources_by_type = AsyncMock(
        ...
        assert (
            result["resources"][0]["score"] > result["resources"][1]["score"]
        )  # 第一个医生的分数应更高
    ...
    async def test_manage_appointment(self, resource_manager):
        ...
        # 模拟存储库方法
        resource_manager.resource_repo.get_resource_by_id = AsyncMock(
            return_value=mock_doctor
        )
        resource_manager.appointment_repo.get_appointments_by_time = AsyncMock(
            return_value=[]
        )
        resource_manager.appointment_repo.create_appointment = AsyncMock(
            return_value="appt123"
        )
        ...
        resource_manager.appointment_repo.get_appointments_by_time = AsyncMock(
            return_value=mock_existing_appointment
        )
        resource_manager._find_next_available_time = AsyncMock(
            return_value=datetime.fromisoformat("2023-07-01T10:00")
        )
    ...
    def test_calculate_constitution_match(self, resource_manager):
        ...
        score1 = resource_manager._calculate_constitution_match(
            resource1, "QI_DEFICIENCY"
        )
        assert score1 == 1.0

        # 适合所有体质
        resource2 = {"supported_constitution_types": ["ALL"]}
        score2 = resource_manager._calculate_constitution_match(
            resource2, "QI_DEFICIENCY"
        )
        assert score2 == 0.8
        ...
        score3 = resource_manager._calculate_constitution_match(
            resource3, "QI_DEFICIENCY"
        )
        assert score3 == 0.3

        # 无体质信息
        resource4 = {}
        score4 = resource_manager._calculate_constitution_match(
            resource4, "QI_DEFICIENCY"
        )
        assert score4 == 0.5
