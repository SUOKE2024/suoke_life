...
class TestXiaoKeService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 注入被测试对象中的模拟依赖
        with mock.patch('internal.delivery.xiaoke_service_impl.ResourceManager', return_value=self.resource_manager_mock), \
             mock.patch('internal.delivery.xiaoke_service_impl.ProductManager', return_value=self.product_manager_mock), \
             mock.patch('internal.delivery.xiaoke_service_impl.SubscriptionRepository', return_value=self.subscription_repo_mock):
    ...
    def test_check_inventory(self):
        ...
        # 准备请求对象
        request = xiaoke_service_pb2.InventoryCheckRequest(
            product_id="PROD-001"
        )
    ...
    def test_reserve_resources_unavailable(self):
        ...
        # 模拟资源不可用的情况
        self.resource_manager_mock.reserve_resource.side_effect = Exception("Resource not available")
        ...
        self.assertEqual(response.resource_id, "RES-001")
        self.assertEqual(response.status, xiaoke_service_pb2.ReservationStatus.UNAVAILABLE)
    ...
    def test_cancel_subscription(self):
        ...
        # 验证结果
        self.assertEqual(response.subscription_id, "SUB-12345")
        self.assertEqual(response.status, xiaoke_service_pb2.SubscriptionStatus.CANCELLED)
