...
from services.agent_services.xiaoke_service.internal.service.enhanced_resource_service import (
    get_resource_service, ResourceRequest, ProductRequest, ConstitutionType
)
...
class TestXiaokeServiceIntegration:
    ...
    async def test_caching_functionality(self, resource_service):
        ...
        assert len(result1.matched_resources) == len(result2.matched_resources)  # 结果相同
        ...
        print(f"✅ 缓存功能测试通过 - 首次: {first_time:.3f}s, 缓存: {second_time:.3f}s")
    ...
    async def test_parallel_processing(self, resource_service):
        ...
        print(f"✅ 并行处理测试通过 - 5个请求总时间: {total_time:.3f}s, 平均: {avg_time:.3f}s")
    ...
    async def test_circuit_breaker_failure_handling(self, resource_service):
        """测试断路器故障处理"""
        # 模拟外部服务故障
        with patch.object(resource_service, '_call_external_api', side_effect=Exception("外部服务故障")):
            request = ResourceRequest(
    ...
    async def test_rate_limiter_functionality(self, resource_service):
        ...
        print(f"✅ 限流器功能测试通过 - 成功: {success_count}, 限流: {rate_limited_count}")
    ...
    async def test_performance_under_load(self, resource_service):
        ...
        print(f"✅ 负载性能测试通过 - 100个请求总时间: {total_time:.3f}s, 平均: {avg_time:.3f}s")
    ...
    def test_service_health_status(self, resource_service):
        ...
        # 验证必要的统计信息
        required_fields = [
            "total_requests", "success_rate", "avg_processing_time",
            "cache_hit_rate", "active_connections"
        ]
