...
class TestERPClient(unittest.TestCase):
    ...
    def test_check_doctor_availability(self):
        ...
        self.assertTrue(kwargs['url'].endswith(f"/api/doctors/{doctor_id}/availability"))
    ...
    def test_create_appointment(self):
        ...
        appointment_time = (datetime.now() + timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S")
        ...
        mock_response_data = {
            ...
            "meeting_link": f"https://meeting.hospital.com/{uuid.uuid4().hex}" if appointment_type == "ONLINE_CONSULTATION" else "",
            "location": "线上问诊" if appointment_type == "ONLINE_CONSULTATION" else "主诊室101"
        }
        ...
        mock_response_data = {
            ...
            "inventory": {
                ...
                "prod456": {
                    ...
                    "reserved_quantity": 0,
                    "restock_date": (datetime.now() + timedelta(days=7)).strftime("%Y-%m-%d")
                }
            }
        }
            ...
            "trace_records": [
                {
                    "stage_name": "种植",
                    "location": "黑龙江省五常市",
                    "timestamp": (datetime.now() - timedelta(days=100)).strftime("%Y-%m-%dT%H:%M:%S"),
                    "operator": "张农夫",
                    "details": {
                        "soil_type": "黑土",
                        "weather": "适宜",
                ...
                },
                {
                    "stage_name": "收割",
                    "location": "黑龙江省五常市",
                    "timestamp": (datetime.now() - timedelta(days=60)).strftime("%Y-%m-%dT%H:%M:%S"),
                    "operator": "李收割",
                    "details": {
                ...
                },
                {
                    "stage_name": "包装",
                    "location": "哈尔滨市包装厂",
                    "timestamp": (datetime.now() - timedelta(days=30)).strftime("%Y-%m-%dT%H:%M:%S"),
                    "operator": "王包装",
                    "details": {
    ...
    def test_error_handling(self):
        ...
        with self.assertRaises(ERPError) as context:
            self.client.check_doctor_availability("nonexistent", "2023-01-01T00:00:00", "2023-01-02T00:00:00")
    ...
    def test_connection_error(self):
        """测试连接错误"""
        # 配置模拟连接错误
        self.mock_session.side_effect = requests.exceptions.ConnectionError("无法连接到服务器")
        
        # 执行测试
        with self.assertRaises(ERPError) as context:
            self.client.check_doctor_availability("doc123", "2023-01-01T00:00:00", "2023-01-02T00:00:00")
