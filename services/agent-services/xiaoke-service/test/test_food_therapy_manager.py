...
class TestFoodTherapyManager:
    ...
    def food_therapy_manager(self):
        """创建食疗管理器实例"""
        with (
            patch("internal.agent.food_therapy_manager.get_config"),
    ...
    async def test_get_products_for_constitution(self, food_therapy_manager):
        ...
        # 模拟存储库方法
        food_therapy_manager.food_repo.get_foods_by_constitution = AsyncMock(
    ...
    async def test_generate_diet_plan(self, food_therapy_manager):
        """测试生成食疗方案"""
        # 模拟获取适合体质的产品方法
        food_therapy_manager.get_products_for_constitution = AsyncMock(
            return_value=[
                {
                    "id": "food1",
                    "name": "黑木耳",
        ...
        # 模拟食物药物相互作用方法
        food_therapy_manager.food_repo.get_food_drug_interactions = AsyncMock(
            return_value=[]
        )
    ...
    async def test_check_food_medicine_pairing(self, food_therapy_manager):
        """测试检查食物药物配伍"""
        # 模拟食物和药物信息
        mock_food = {"id": "food1", "name": "黑木耳"}
        mock_medicine = {"id": "med1", "name": "阿司匹林"}
        ...
        # 模拟存储库方法
        food_therapy_manager.food_repo.get_food_by_id = AsyncMock(
            return_value=mock_food
        )
        food_therapy_manager.food_repo.get_medicine_by_id = AsyncMock(
            return_value=mock_medicine
        )
        food_therapy_manager.food_repo.get_food_medicine_interaction = AsyncMock(
        ...
        food_therapy_manager.food_repo.get_medicine_by_id.assert_called_once_with(
            "med1"
        )
    ...
    async def test_recommend_recipes(self, food_therapy_manager):
        ...
        food_therapy_manager.food_repo.get_recipes_by_foods = AsyncMock(
