...
class ERPClient:
    ...
    def __init__(self, api_url=None, api_key=None, timeout=30, max_retries=3):
        ...
        logger.info(f"初始化ERP客户端，API地址: {self.api_url}，超时时间: {self.timeout}秒")
    ...
    def _make_api_request(self, method: str, endpoint: str, payload=None) -> Dict[str, Any]:
        """
        发起API请求
        ...
        try:
            ...
            if method.upper() == "GET":
                ...
                response = self.session.delete(url, headers=headers, params=payload, timeout=self.timeout)
            else:
                raise ValueError(f"不支持的HTTP方法: {method}")
            
            # 记录请求延迟
            latency = time.time() - start_time
            endpoint_name = endpoint.split('?')[0].split('/')[2] if len(endpoint.split('/')) > 2 else endpoint
            ...
            logger.error(f"ERP API请求错误: {method} {endpoint}, 错误: {str(e)}", exc_info=True)
            ...
            logger.error(f"ERP API响应解析错误: {method} {endpoint}, 错误: {str(e)}", exc_info=True)
            raise ERPError(message=f"ERP API响应解析失败: {str(e)}")
            
        finally:
            if status == "success":
                logger.debug(f"ERP API请求完成: {method} {endpoint}, 耗时: {time.time() - start_time:.3f}秒")
