...
class XiaoKeServiceServicer(xiaoke_service_pb2_grpc.XiaoKeServiceServicer):
    ...
    def ScheduleMedicalResource(self, request, context):
        """处理医疗资源调度请求"""
        logger.info(f"接收到医疗资源调度请求: user_id={request.user_id}, resource_type={request.resource_type}")
        ...
        try:
            ...
            for resource in scheduled_resources['resources']:
                resources.append(xiaoke_service_pb2.MedicalResource(
    ...
    def ManageAppointment(self, request, context):
        """处理预约管理请求"""
        logger.info(f"接收到预约管理请求: user_id={request.user_id}, doctor_id={request.doctor_id}")
    ...
    def CustomizeProduct(self, request, context):
        """处理农产品定制请求"""
        logger.info(f"接收到农产品定制请求: user_id={request.user_id}, constitution_type={request.constitution_type}")
        ...
        try:
            ...
            for product in customization_result['products']:
                customized_products.append(xiaoke_service_pb2.CustomizedProduct(
    ...
    def TraceProduct(self, request, context):
        ...
        try:
            ...
            for record in trace_result['trace_records']:
                ...
                trace_records.append(xiaoke_service_pb2.TraceRecord(
    ...
    def ProcessPayment(self, request, context):
        """处理支付请求"""
        logger.info(f"接收到支付处理请求: user_id={request.user_id}, order_id={request.order_id}")
        ...
        try:
            ...
            metrics.record_payment(payment_method=request.payment_method, status="FAILED")
            context.set_code(grpc.StatusCode.INTERNAL)
    ...
    def ManageSubscription(self, request, context):
        """处理订阅管理请求"""
        logger.info(f"接收到订阅管理请求: user_id={request.user_id}, action={request.action}")
        ...
        try:
            ...
            start_date.FromDatetime(datetime.fromisoformat(subscription_result['start_date']))
            
            end_date = Timestamp()
            end_date.FromDatetime(datetime.fromisoformat(subscription_result['end_date']))
    ...
    def RecommendProducts(self, request, context):
        """处理商品推荐请求"""
        logger.info(f"接收到商品推荐请求: user_id={request.user_id}, constitution_type={request.constitution_type}")
        ...
        try:
            ...
            for product in recommendation_result['seasonal_products']:
                seasonal_products.append(xiaoke_service_pb2.RecommendedProduct(
                    product_id=product['id'],
                    name=product['name'],
                    description=product['description'],
            ...
            for product in recommendation_result['constitution_specific_products']:
                constitution_specific_products.append(xiaoke_service_pb2.RecommendedProduct(
                    product_id=product['id'],
                    name=product['name'],
                    description=product['description'],
                    category=product['category'],
            ...
            for product in recommendation_result['personalized_products']:
                personalized_products.append(xiaoke_service_pb2.RecommendedProduct(
                    product_id=product['id'],
                    name=product['name'],
                    description=product['description'],
                    category=product['category'],
            ...
            return xiaoke_service_pb2.ProductRecommendationResponse(
                ...
                recommendation_explanation=recommendation_result.get('recommendation_explanation', {})
            )
