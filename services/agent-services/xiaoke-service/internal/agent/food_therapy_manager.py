...
class FoodTherapyManager:
    ...
    def _load_prompt_templates(self) -> Dict[str, str]:
        ...
        for key, filename in templates_files.items():
            ...
            try:
                ...
                # 设置一个简单的备用提示语
                templates[key] = f"你是小克，索克生活APP的医疗资源调度智能体中负责{key}的专家。请根据用户信息提供专业建议。"
    ...
    async def generate_diet_plan(self, user_id: str, constitution_type: str,
                            health_conditions: List[str] = None,
                            preferences: List[str] = None,
                            allergies: List[str] = None,
                            current_medications: List[str] = None,
                            plan_duration: int = 7) -> Dict[str, Any]:
        """
        生成个性化食疗方案
        ...
        try:
            ...
            if current_medications:
                food_drug_interactions = await self.food_repo.get_food_drug_interactions(
                    current_medications
                )
            ...
            for food in suitable_foods:
                # 过滤过敏源
                if allergies:
                    if any(allergen in food.get('allergens', []) for allergen in allergies):
                        continue
            ...
            for day in range(plan_duration):
                breakfast_foods = self._select_meal_foods(filtered_foods, 'BREAKFAST', 2)
                ...
                daily_menus.append({
                    ...
                    'dinner': dinner_foods,
                    'snack': snack_foods,
                    'hydration': self._generate_hydration_recommendation(constitution_type, season)
                })
            
            # 生成食疗方案总结
            summary = {
                'principle': self._get_diet_principle(constitution_type),
                'focus_nutrients': self._get_focus_nutrients(constitution_type, health_conditions),
                'foods_to_favor': self._get_top_foods_to_favor(filtered_foods, 5),
                'foods_to_avoid': self._get_foods_to_avoid(constitution_type),
                'special_notes': self._get_special_notes(constitution_type, health_conditions, allergies, current_medications)
            }
    ...
    async def analyze_food_medicine_pairing(self, medicine_data: Dict[str, Any], 
        ...
        try:
            ...
            template_params = {
                ...
                "constitution_analysis": user_data.get("constitution_analysis", "未提供体质分析信息")
            }
            ...
            # 构建消息列表
            messages = [
                {"role": "system", "content": "你是小克，索克生活APP的医疗资源调度智能体中负责食药配伍分析的专家。"},
                {"role": "user", "content": prompt}
            ]
            
            # 调用LLM生成食药配伍分析
            analysis_text, response_meta = await self.model_factory.generate_chat_completion(
    ...
    async def adjust_seasonal_diet(self, user_data: Dict[str, Any], 
                                  season_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        根据季节调整饮食建议
        ...
        try:
            ...
            template_params = {
                ...
                "current_diet_plan": user_data.get("current_diet_plan", "无现有食疗方案"),
                "health_goals": user_data.get("health_goals", "保持健康"),
                "current_season": season_data.get("current_season", self._get_current_season()),
                "upcoming_season": season_data.get("upcoming_season", self._get_next_season()),
            ...
            # 构建消息列表
            messages = [
                {"role": "system", "content": "你是小克，索克生活APP的医疗资源调度智能体中负责季节性食疗调整的专家。"},
                {"role": "user", "content": prompt}
            ]
            
            # 调用LLM生成季节性饮食调整建议
            adjustment_text, response_meta = await self.model_factory.generate_chat_completion(
                model=self.primary_model,
                messages=messages,
                temperature=0.3,
                max_tokens=1536
    ...
    async def recommend_recipes(self, user_id: str, constitution_type: str,
                           health_conditions: List[str] = None,
                           preferences: List[str] = None,
        ...
        try:
            ...
            if preferences:
                for recipe in recipes:
                    # 计算偏好匹配度
                    preference_match = sum(1 for pref in preferences 
            ...
            for recipe in top_recipes:
                recipe['constitution_benefits'] = self._get_recipe_constitution_benefits(
                    recipe, constitution_type
                )
    ...
    async def analyze_food_safety(self, food_data: Dict[str, Any], 
                                 user_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        分析食品安全性并提供建议
        ...
        try:
            ...
            # 构建消息列表
            messages = [
                {"role": "system", "content": "你是小克，索克生活APP的医疗资源调度智能体中负责食品安全分析的专家。"},
                {"role": "user", "content": prompt}
            ]
    ...
    async def design_medicinal_diet_formulation(self, user_data: Dict[str, Any], 
        ...
        try:
            ...
            template_params = {
                ...
                "medication_history": health_data.get("medication_history", "无用药情况"),
                "dietary_preferences": user_data.get("dietary_preferences", "无特殊饮食偏好"),
            ...
            # 构建消息列表
            messages = [
                {"role": "system", "content": "你是小克，索克生活APP的医疗资源调度智能体中负责药膳方剂设计的专家。"},
                {"role": "user", "content": prompt}
            ]
            
            # 调用LLM生成药膳方剂
            formulation_text, response_meta = await self.model_factory.generate_chat_completion(
                model=self.primary_model,
                messages=messages,
    ...
    async def rate_food_for_constitution(self, food_data: Dict[str, Any], 
                                        user_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        对食材进行体质适应性评分
        ...
        try:
            ...
            template_params = {
                ...
                "current_season": user_data.get("season", self._get_current_season()),
                "constitution_profile": user_data.get("constitution_profile", "未提供体质详情"),
            ...
            # 构建消息列表
            messages = [
                {"role": "system", "content": "你是小克，索克生活APP的医疗资源调度智能体中负责食材体质评分的专家。"},
                {"role": "user", "content": prompt}
            ]
            
            # 调用LLM生成食材体质评分
            rating_text, response_meta = await self.model_factory.generate_chat_completion(
                model=self.primary_model,
                messages=messages,
                temperature=0.3,
                max_tokens=2048
    ...
    async def get_products_for_constitution(self, constitution_type: str,
                                       health_conditions: List[str] = None,
                                       season: str = None,
        ...
        try:
            ...
            if season:
                for food in filtered_foods:
                    if 'ALL' in food.get('seasons', []) or season in food.get('seasons', []):
            ...
            for food in seasonal_filtered_foods:
                # 计算评分
                score = self._calculate_food_score(food, constitution_type, health_conditions, season)
    ...
    def _calculate_food_score(self, food: Dict[str, Any], constitution_type: str,
                         health_conditions: List[str] = None,
        ...
        # 健康状况匹配分数 (0-3分)
        if health_conditions:
            matching_benefits = sum(1 for condition in health_conditions 
    ...
    def _select_meal_foods(self, foods: List[Dict[str, Any]], 
                      meal_type: str, count: int) -> List[Dict[str, Any]]:
        """为特定餐点选择食物"""
        # 按餐点类型过滤
        suitable_for_meal = []
        for food in foods:
            if meal_type in food.get('suitable_meals', []) or 'ALL' in food.get('suitable_meals', []):
    ...
    def _generate_hydration_recommendation(self, constitution_type: str, season: str) -> Dict[str, Any]:
        """生成饮水建议"""
    ...
    def _get_focus_nutrients(self, constitution_type: str, 
                        health_conditions: List[str] = None) -> List[Dict[str, str]]:
        """获取重点营养素"""
        base_nutrients = []
        ...
        if health_conditions:
            for condition in health_conditions:
                if condition == "高血压":
                    base_nutrients.append({"name": "钾", "reason": "平衡钠钾，降低血压"})
                    base_nutrients.append({"name": "镁", "reason": "舒张血管，降低血压"})
                elif condition == "糖尿病":
                    base_nutrients.append({"name": "铬", "reason": "改善胰岛素敏感性"})
                    base_nutrients.append({"name": "低GI碳水化合物", "reason": "稳定血糖"})
                elif condition == "贫血":
    ...
    def _get_top_foods_to_favor(self, foods: List[Dict[str, Any]], 
        ...
        for food in sorted_foods[:count]:
            top_foods.append({
                "name": food['name'],
                "reason": food.get('top_benefit', '适合体质')
            })
    ...
    def _get_special_notes(self, constitution_type: str, 
                      health_conditions: List[str] = None,
                      allergies: List[str] = None,
    ...
    def _get_recipe_constitution_benefits(self, recipe: Dict[str, Any], 
                                     constitution_type: str) -> str:
        """获取食谱对特定体质的益处"""
        # 如果食谱已有针对该体质的益处描述，直接返回
        if 'constitution_benefits' in recipe and constitution_type in recipe['constitution_benefits']:
        ...
        if constitution_type == "QI_DEFICIENCY":
            ...
            if "化痰" in recipe.get('properties', []) or "祛湿" in recipe.get('properties', []):
                return "健脾化痰，祛湿利水，改善痰湿症状"
            else:
                return "有助于改善体内水湿代谢"
        elif constitution_type == "DAMP_HEAT":
            if "清热" in recipe.get('properties', []) or "利湿" in recipe.get('properties', []):
