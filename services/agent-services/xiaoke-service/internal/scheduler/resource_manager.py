...
class ResourceManager:
    ...
    def __init__(self):
        ...
        self.weights = self.scheduler_config.get('weights', {
    ...
    async def schedule_resources(self, user_id: str, resource_type: str, 
                              constitution_type: str, location: str,
                              requirements: List[str], page_size: int = 10, 
        ...
        try:
            ...
            # 获取所有可用资源
            all_resources = await self.resource_repo.get_resources_by_type(resource_type)
    ...
    async def manage_appointment(self, user_id: str, doctor_id: str, 
                              ...
                              symptoms: str, constitution_type: str,
        ...
        try:
            ...
            # 检查医生可用性
            is_available = await self._check_doctor_availability(doctor_id, preferred_datetime)
            
            # 创建预约
            appointment_id = str(uuid.uuid4())
            status = AppointmentStatus.CONFIRMED if is_available else AppointmentStatus.PENDING
            
            # 如果是线上咨询，生成会议链接
            meeting_link = ""
            if appointment_type == "ONLINE_CONSULTATION" and is_available:
                meeting_link = f"https://meeting.suoke.life/consultation/{appointment_id}"
            
            # 确认时间，如果不可用，则推荐下一个可用时间
            confirmed_time = preferred_time
            if not is_available:
                next_available = await self._find_next_available_time(doctor_id, preferred_datetime)
                if next_available:
    ...
    def _score_resources(self, resources: List[Dict[str, Any]], 
                       constitution_type: str, location: str,
        ...
        for resource in resources:
            # 计算各维度的匹配分数
            constitution_score = self._calculate_constitution_match(resource, constitution_type)
    ...
    def _calculate_constitution_match(self, resource: Dict[str, Any], 
    ...
    def _calculate_location_proximity(self, resource: Dict[str, Any], 
                                    location: str) -> float:
        """计算位置接近度"""
        # 简化实现，实际应使用地理坐标计算距离
    ...
    def _check_requirements(self, resource: Dict[str, Any], 
    ...
    async def _check_doctor_availability(self, doctor_id: str, 
                                      preferred_time: datetime) -> bool:
        """检查医生在指定时间是否可用"""
        # 获取医生可用时间
        doctor = await self.resource_repo.get_resource_by_id(doctor_id)
        ...
        for time_slot in available_times:
            if time_slot.startswith(preferred_time_str):
                # 检查该时间是否已被预约
                existing_appointments = await self.appointment_repo.get_appointments_by_time(
    ...
    async def _find_next_available_time(self, doctor_id: str, 
