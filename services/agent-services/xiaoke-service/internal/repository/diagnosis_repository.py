...
class DiagnosisRepository:
    ...
    async def init_indexes(self):
        ...
        try:
            ...
            await self.symptom_patterns.create_index([("name", "text"), ("description", "text")])
            
            # 中医证型索引
            await self.tcm_syndromes.create_index("syndrome_id", unique=True)
            await self.tcm_syndromes.create_index("type")
            await self.tcm_syndromes.create_index([("name", "text"), ("description", "text")])
            ...
            # 用户健康记录索引
            await self.health_records.create_index([("user_id", 1), ("record_date", -1)])
            ...
            await self.knowledge_base.create_index([("title", "text"), ("content", "text")])
    ...
    async def create_symptom_pattern(self, pattern_data: Dict[str, Any]) -> Optional[str]:
        """
        创建症状模式
        ...
        try:
            ...
            # 设置ID和创建时间
            pattern_data["pattern_id"] = pattern_data.get("pattern_id", str(uuid.uuid4()))
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "create_symptom_pattern"})
            return None
    
    @metrics.measure_execution_time("diagnosis_repo_find_symptom_patterns")
    async def find_symptom_patterns(self, 
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "find_symptom_patterns"})
            return []
    ...
    async def create_tcm_syndrome(self, syndrome_data: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            # 设置ID和创建时间
            syndrome_data["syndrome_id"] = syndrome_data.get("syndrome_id", str(uuid.uuid4()))
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "create_tcm_syndrome"})
            return None
    
    @metrics.measure_execution_time("diagnosis_repo_find_tcm_syndromes")
    async def find_tcm_syndromes(self, 
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "find_tcm_syndromes"})
            return []
    ...
    async def get_tcm_syndrome(self, syndrome_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_tcm_syndrome"})
            return None
    
    @metrics.measure_execution_time("diagnosis_repo_save_user_diagnosis")
    async def save_user_diagnosis(self, user_id: str, diagnosis_data: Dict[str, Any]) -> Optional[str]:
        """
        保存用户诊断结果
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "save_user_diagnosis"})
            return None
    ...
    async def get_user_diagnosis_history(self, 
        ...
        """
        try:
            cursor = self.user_diagnosis.find({"user_id": user_id}) \
                .sort("created_at", -1) \
                .skip(offset) \
                .limit(limit)
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_user_diagnosis_history"})
            return []
    
    @metrics.measure_execution_time("diagnosis_repo_get_diagnosis_details")
    async def get_diagnosis_details(self, diagnosis_id: str) -> Optional[Dict[str, Any]]:
        """
        获取诊断详情
        ...
        """
        try:
            diagnosis = await self.user_diagnosis.find_one({"diagnosis_id": diagnosis_id})
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_diagnosis_details"})
            return None
    
    @metrics.measure_execution_time("diagnosis_repo_save_health_record")
    async def save_health_record(self, user_id: str, record_data: Dict[str, Any]) -> Optional[str]:
        """
        保存用户健康记录
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "save_health_record"})
            return None
    
    @metrics.measure_execution_time("diagnosis_repo_get_user_health_records")
    async def get_user_health_records(self, 
                                    user_id: str, 
        ...
        try:
            ...
            cursor = self.health_records.find(query) \
                .sort("record_date", -1) \
                .limit(limit)
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_user_health_records"})
            return []
    ...
    async def get_latest_health_record(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_latest_health_record"})
            return None
    
    @metrics.measure_execution_time("diagnosis_repo_create_knowledge_item")
    async def create_knowledge_item(self, knowledge_data: Dict[str, Any]) -> Optional[str]:
        """
        创建诊断知识条目
        ...
        try:
            ...
            # 设置ID和创建时间
            knowledge_data["knowledge_id"] = knowledge_data.get("knowledge_id", str(uuid.uuid4()))
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "create_knowledge_item"})
            return None
    ...
    async def search_knowledge(self, 
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "search_knowledge"})
            return []
    ...
    async def get_related_knowledge(self, 
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_related_knowledge"})
            return []
    
    @metrics.measure_execution_time("diagnosis_repo_get_user_constitution_type")
    async def get_user_constitution_type(self, user_id: str) -> Optional[Dict[str, Any]]:
        """
        获取用户的体质类型
        基于最近的诊断结果
        ...
        try:
            ...
            constitution_data = {
                ...
                "treatment_suggestions": latest_diagnosis.get("treatment_suggestions", [])
            }
            
            return constitution_data
        except PyMongoError as e:
            logger.error(f"获取用户体质类型失败: {str(e)}")
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_user_constitution_type"})
            return None
    
    @metrics.measure_execution_time("diagnosis_repo_find_similar_diagnoses")
    async def find_similar_diagnoses(self, 
                                   symptoms: List[str], 
                                   limit: int = 5) -> List[Dict[str, Any]]:
        """
        查找相似的诊断记录
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "find_similar_diagnoses"})
            return []
    
    @metrics.measure_execution_time("diagnosis_repo_analyze_symptom_trend")
    async def analyze_symptom_trend(self, 
                                  user_id: str, 
                                  symptom: str,
        ...
        try:
            ...
            pipeline = [
                ...
                # 按日期分组
                {"$group": {
                    "_id": {"$substr": ["$record_date", 0, 10]},  # 按日期分组 (YYYY-MM-DD)
                    "count": {"$sum": 1},
                    "records": {"$push": {
                        "record_id": "$record_id",
            ...
            diagnosis_pipeline = [
                ...
                # 按日期分组
                {"$group": {
                    "_id": {"$substr": ["$created_at", 0, 10]},  # 按日期分组 (YYYY-MM-DD)
                    "count": {"$sum": 1},
                    "diagnoses": {"$push": {
                        "diagnosis_id": "$diagnosis_id",
                        "created_at": "$created_at",
            ...
            trend_data = {
                ...
                "total_occurrences": sum(r["count"] for r in results) + sum(r["count"] for r in diagnosis_results),
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "analyze_symptom_trend"})
            return {"error": str(e)}
    
    @metrics.measure_execution_time("diagnosis_repo_get_common_user_symptoms")
    async def get_common_user_symptoms(self, user_id: str, limit: int = 10) -> List[Dict[str, str]]:
        """
        获取用户常见症状
        ...
        try:
            ...
            metrics.increment_counter("diagnosis_repo_errors", {"method": "get_common_user_symptoms"})
            return [] 