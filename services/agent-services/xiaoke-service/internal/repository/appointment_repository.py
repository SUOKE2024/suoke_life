...
class AppointmentRepository:
    ...
    async def update_appointment(self, appointment_id: str, 
    ...
    async def delete_appointment(self, appointment_id: str) -> bool:
        ...
        try:
            # 执行删除
            result = await self.appointments_collection.delete_one({'_id': appointment_id})
    ...
    async def get_appointments_by_user(self, user_id: str, 
                                  status: str = None,
                                  page: int = 1,
        ...
        try:
            ...
            # 执行查询
            cursor = self.appointments_collection.find(query).sort('created_at', -1).skip(skip).limit(page_size)
    ...
    async def get_appointments_by_time(self, doctor_id: str, 
                                  time_slot: str) -> List[Dict[str, Any]]:
        """
        获取指定时间段的预约
    ...
    async def get_appointments_by_date_range(self, doctor_id: str,
                                       start_date: str,
