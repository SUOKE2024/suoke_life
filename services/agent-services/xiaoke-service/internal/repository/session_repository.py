...
class SessionRepository:
    ...
    def __init__(self):
        ...
        self.dialogue_records = self.db.dialogue_records  # 对话记录，包含更丰富的上下文信息
    ...
    async def update_diagnosis_state(self, session_id: str, diagnosis_state: Dict[str, Any]) -> bool:
        """
        更新诊断状态
        ...
        try:
            ...
            metrics.increment_counter("session_repo_errors", {"method": "update_diagnosis_state"})
            return False
    ...
    async def save_message(self, 
        ...
        try:
            ...
            message_data = {
                ...
                "created_at": now,
                "metadata": metadata or {},
                "importance": metadata.get("importance", 0) if metadata else 0,  # 消息重要性评分
                "topics": metadata.get("topics", []) if metadata else [],  # 消息主题标签
                "diagnosis_step": metadata.get("diagnosis_step", "") if metadata else ""  # 诊断步骤
            }
    ...
    async def get_diagnosis_messages(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            query = {
                "session_id": session_id,
                "metadata.diagnosis_related": True
            }
            ...
            metrics.increment_counter("session_repo_errors", {"method": "get_diagnosis_messages"})
            return []
    ...
    async def get_session_summary(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            summary = {
                ...
                "diagnosis_state": diagnosis_state,
                "last_message": latest_messages[0]["content"] if latest_messages else None,
                "last_message_time": latest_messages[0]["created_at"] if latest_messages else None,
    ...
    async def _create_memory_anchor(self, user_id: str, content: str, topic: str, context: Dict[str, Any]) -> Optional[str]:
        """
        创建记忆锚点（重要的长期记忆）
        ...
        try:
            ...
            logger.debug(f"创建记忆锚点: anchor_id={anchor_id}, user_id={user_id}, topic={topic}")
            return anchor_id
        except PyMongoError as e:
            logger.error(f"创建记忆锚点失败: {str(e)}")
            metrics.increment_counter("session_repo_errors", {"method": "create_memory_anchor"})
            return None
    
    @metrics.measure_execution_time("session_repo_update_dialogue_record")
    async def _update_dialogue_record(self, 
                                     user_id: str, 
                                     session_id: str,
                                     message_id: str,
                                     role: str,
                                     ...
                                     diagnosis_step: str,
                                     symptoms: List[str],
                                     diagnosis_data: Dict[str, Any]) -> Optional[str]:
        """
        更新对话记录集合，用于追踪诊断过程
        ...
        try:
            ...
            logger.debug(f"更新对话记录: record_id={record_id}, user_id={user_id}, step={diagnosis_step}")
            return record_id
        except PyMongoError as e:
            logger.error(f"更新对话记录失败: {str(e)}")
            metrics.increment_counter("session_repo_errors", {"method": "update_dialogue_record"})
            return None
    
    @metrics.measure_execution_time("session_repo_get_memory_anchors")
    async def get_memory_anchors(self, 
                               user_id: str, 
        ...
        try:
            ...
            cursor = self.memory_anchors.find(query) \
                .sort("importance", -1) \
                .limit(limit)
            ...
            metrics.increment_counter("session_repo_errors", {"method": "get_memory_anchors"})
            return []
    ...
    async def get_dialogue_record(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            metrics.increment_counter("session_repo_errors", {"method": "get_dialogue_record"})
            return None
    
    @metrics.measure_execution_time("session_repo_get_user_dialogue_records")
    async def get_user_dialogue_records(self, user_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        获取用户的所有对话记录
        ...
        """
        try:
            cursor = self.dialogue_records.find({"user_id": user_id}) \
                .sort("updated_at", -1) \
                .limit(limit)
            ...
            metrics.increment_counter("session_repo_errors", {"method": "get_user_dialogue_records"})
            return []
    ...
    async def delete_expired_sessions(self, days: int = 90) -> int:
        ...
        try:
            ...
            if session_ids:
                ...
                await self.dialogue_records.delete_many({"session_id": {"$in": session_ids}})
            
            # 删除过期会话
            result = await self.sessions.delete_many({"updated_at": {"$lt": expire_date}})
