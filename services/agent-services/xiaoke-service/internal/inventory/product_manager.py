...
class ProductManager:
    ...
    def __init__(self):
        ...
        self.weights = self.recommendation_config.get(
            "weights",
            {
        ...
        self.min_confidence = self.recommendation_config.get(
            "min_confidence_threshold", 0.6
        )
        self.max_recommendations = self.recommendation_config.get(
            "max_recommendations", 10
        )

        # 检查区块链溯源特性是否启用
        self.blockchain_tracing_enabled = self.config.get_section("feature_flags").get(
            "blockchain_tracing", True
        )
        ...
        self.blockchain_repo = (
            BlockchainRepository() if self.blockchain_tracing_enabled else None
        )
    ...
    async def customize_products(
        ...
        try:
            ...
            constitution_products = (
                await self.food_therapy_manager.get_products_for_constitution(
            ...
            customized_products = self._customize_product_selection(
                constitution_products, seasonal_products, preferences, quantity
            )

            # 处理包装请求
            packaged_products = await self._apply_packaging_preferences(
                customized_products, packaging_preference
            )
            ...
            return {
                ...
                "metadata": {"constitution_type": constitution_type, "season": season},
            }
    ...
    async def trace_product(
        ...
        try:
            ...
            if self.blockchain_tracing_enabled and self.blockchain_repo:
                ...
                # 生成区块链验证URL
                blockchain_url = self.blockchain_repo.get_verification_url(
                    product_id, effective_batch_id
                )

            else:
                # 使用传统数据库溯源
                db_records = await self.product_repo.get_trace_records(
                    product_id, effective_batch_id
                )
    ...
    async def process_payment(
        ...
        try:
            ...
            logger.info(
                f"处理支付请求: 用户={user_id}, 订单={order_id}, 金额={amount}{currency}"
            )
            ...
            if abs(order["total_price"] - amount) > 0.01:  # 允许1分钱的舍入误差
                raise ValueError(
                    f"支付金额不匹配: 应为 {order['total_price']}，实际为 {amount}"
                )
    ...
    async def recommend_products(
        ...
        try:
            ...
            # 获取季节性产品
            seasonal_products = await self.product_repo.get_seasonal_products(
                season, effective_max
            )
    ...
    def _customize_product_selection(
        ...
        filtered_products.sort(
            key=lambda p: (p.get("preference_score", 0), p.get("score", 0)),
            reverse=True,
        )
        ...
        for product in products:
            ...
            if packaging_preference == "ECO_FRIENDLY":
                product_copy["description"] = (
                    f"环保包装 - {product_copy['description']}"
                )
                # 环保包装略贵
                ...
                product_copy["description"] = (
                    f"精美礼盒 - {product_copy['description']}"
                )
                # 礼品包装更贵
                ...
                product_copy["description"] = (
                    f"简易包装 - {product_copy['description']}"
                )
                # 简易包装略便宜
    ...
    async def _get_personalized_recommendations(
        ...
        try:
            ...
            for product in candidate_products:
                ...
                # 季节匹配分数
                season_score = self._calculate_season_match_score(product, season)
    ...
    def _calculate_preference_match_score(
        ...
        for pref in preferences:
            if pref.lower() in product.get("tags", []) or pref.lower() in product.get(
                "categories", []
            ):
        ...
        if user_history:
            ...
            # 检查用户是否购买过同类产品
                ...
                cat in user_history.get("purchased_categories", [])
    ...
    def _calculate_health_match_score(
        ...
        # 检查有多少健康条件能得到改善
        benefit_matches = sum(
            1 for condition in health_conditions if condition in health_benefits
        )
