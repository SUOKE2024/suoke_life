...
from pkg.messaging.queue_manager import (
    get_queue_manager, QueueType, TaskPriority, 
    async_task, TaskConfig
)
from pkg.distributed.lock_manager import (
    get_lock_manager, LockType, LockConfig
)
from pkg.health.health_checker import (
    get_health_check_manager, HealthCheckConfig,
    DatabaseHealthChecker, RedisHealthChecker, HTTPHealthChecker
)
from pkg.config.dynamic_config import (
    get_config_manager, ConfigFormat
)
from pkg.cache.cache_manager import (
    get_cache_manager, CacheStrategy
)
from pkg.resilience.retry_manager import (
    get_retry_manager, RetryConfig, CircuitBreakerConfig
)
...
class XiaokeServiceIntegration:
    ...
    async def _initialize_queue_manager(self):
        ...
        await self.queue_manager.register_task_handler(
            ...
            self._process_medical_request_task
        )
        ...
        await self.queue_manager.register_task_handler(
            ...
            self._update_resource_status_task
        )
    ...
    async def _on_health_status_change(self, old_status, new_status):
        ...
        elif new_status.value == "healthy" and old_status and old_status.value == "unhealthy":
    ...
    # 任务处理器
    async def _process_medical_request_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """处理医疗请求任务"""
        ...
        async with self.lock_manager.acquire_lock(lock_key, LockType.EXCLUSIVE, lock_config):
            # 从缓存获取用户信息
    ...
    async def _send_notification_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """发送通知任务"""
        user_id = task_data.get("user_id")
    ...
    async def _update_resource_status_task(self, task_data: Dict[str, Any]) -> Dict[str, Any]:
        """更新资源状态任务"""
    ...
    async def schedule_medical_resource(self, user_id: str, requirements: Dict[str, Any]) -> Dict[str, Any]:
        """调度医疗资源（装饰器方式）"""
    ...
    async def _process_medical_request(self, user_info: Dict[str, Any], request_data: Dict[str, Any]) -> Dict[str, Any]:
        """处理医疗请求"""
        # 模拟医疗请求处理
    ...
    async def run_demo(self):
        ...
        try:
            ...
            result1 = await self.schedule_medical_resource(
                "user_123",
                {"type": "consultation", "specialty": "中医内科"}
            )
            ...
            logger.info("系统状态: %s", json.dumps(system_status, indent=2, ensure_ascii=False))
