...
export const AdvancedHealthDashboard: React.FC<AdvancedHealthDashboardProps> = ({
  ...
  useEffect(() => {
    ...
    // 启动脉冲动画
    const pulseAnimationLoop = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
      Animated.sequence([
  ...
  const handleAlertPress = useMemo(() => useMemo(() => useCallback((alert: HealthAlert) => {
    onAlertPress?.(alert), []), []);
  }, [onAlertPress]);

  const handleInsightPress = useMemo(() => useMemo(() => useCallback((insight: AIInsight) => {
  ...
  const getAlertColor = useMemo(() => useMemo(() => useCallback( (type: string) => {, []), []), []);
    switch (type) {
...
const renderPeriodSelector = useMemo(() => useMemo(() => () => (
  ...
  // TODO: 将内联组件移到组件外部
const renderViewSelector = useMemo(() => useMemo(() => () => (
  ...
  const renderHealthScore = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    const overallScore = useMemo(() => useMemo(() => Math.round(
  ...
  const renderMetricCard = useMemo(() => useMemo(() => (metric: HealthMetric, index: number) => (
    <Animated.View
      key={metric.id}
      style={[
        styles.metricCard,
  ...
  // TODO: 将内联组件移到组件外部
const renderAlerts = useMemo(() => useMemo(() => () => (
  ...
  // TODO: 将内联组件移到组件外部
const renderInsights = useMemo(() => useMemo(() => () => (
