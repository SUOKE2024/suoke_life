...
export const BlockchainDataManager: React.FC<BlockchainDataManagerProps> = ({
  ...
  const dispatch = useMemo(() => useMemo(() => useAppDispatch(), []), []);
  ...
  const handleUploadData = useMemo(() => useMemo(() => useCallback(async (request: DataUploadRequest) => {
    setLoading(true), []), []);
    try {
      // 模拟上链过程
  ...
  const handleVerifyData = useMemo(() => useMemo(() => useCallback(async (hash: string) => {
    setLoading(true), []), []);
    try {
      // 模拟验证过程
  ...
  const getDataTypeIcon = useMemo(() => useMemo(() => useCallback( (dataType: string) => {, []), []), []);
    switch (dataType) {
  ...
  const getDataTypeLabel = useMemo(() => useMemo(() => useCallback( (dataType: string) => {, []), []), []);
    switch (dataType) {
  ...
  const filteredRecords = useMemo(() => useMemo(() => selectedTab === 'all' 
    ? records 
    : records.filter(record => record.dataType === selectedTab), []), []);

  // TODO: 将内联组件移到组件外部
const renderTabSelector = useMemo(() => useMemo(() => () => (
  ...
  const renderStatsCards = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    const totalRecords = useMemo(() => useMemo(() => records.length, []), []);
    const confirmedRecords = useMemo(() => useMemo(() => records.filter(r => r.status === 'confirmed').length, []), []);
    const encryptedRecords = useMemo(() => useMemo(() => records.filter(r => r.encrypted).length, []), []);
    const sharedRecords = useMemo(() => useMemo(() => records.filter(r => r.shared).length, []), []);
  ...
  const renderRecordCard = useMemo(() => useMemo(() => (record: BlockchainRecord, index: number) => (
    <Animated.View
      key={record.id}
      style={[
        styles.recordCard,
  ...
  // TODO: 将内联组件移到组件外部
const renderUploadModal = useMemo(() => useMemo(() => () => (
    <Modal
