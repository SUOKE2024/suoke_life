...
export const HealthDashboardEnhanced: React.FC = () => {
  ...
  const diagnosisResults = useMemo(() => useMemo(() => useAppSelector(selectDiagnosisResults), []), []);
  const diagnosisSessions = useMemo(() => useMemo(() => useAppSelector(selectDiagnosisSessions), []), []);
  ...
  const loadDiagnosisHistory = useMemo(() => useMemo(() => async () => {
    try {
  ...
  // TODO: 将内联组件移到组件外部
const renderOverviewCards = useMemo(() => useMemo(() => () => (
  ...
  const renderChart = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    const selectedMetricData = useMemo(() => useMemo(() => healthMetrics.find(m => m.id === selectedMetric), []), []);
  ...
  // TODO: 将内联组件移到组件外部
const renderConstitutionAnalysis = useMemo(() => useMemo(() => () => (
  ...
  // TODO: 将内联组件移到组件外部
const renderDiagnosisHistory = useMemo(() => useMemo(() => () => (
  ...
  const getTrendColor = useMemo(() => useMemo(() => useCallback( (trend: string) => {, []), []), []);
    switch (trend) {
