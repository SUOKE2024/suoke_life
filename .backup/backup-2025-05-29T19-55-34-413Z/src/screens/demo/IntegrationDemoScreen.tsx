...
export const IntegrationDemoScreen: React.FC = () => {
  ...
  const updateServiceStatus = useMemo(() => useMemo(() => async () => {
    try {
      setServiceStatus({
  ...
  const runCompleteDemo = useMemo(() => useMemo(() => async () => {
    if (isRunning) {return, []), []);}
    ...
    try {
      ...
      // 重置所有步骤状态
      const resetSteps = useMemo(() => useMemo(() => demoSteps.map(step => ({ ...step, status: 'pending' as const, result: undefined })), []), []);
  ...
  const executeStep = useMemo(() => useMemo(() => async (stepIndex: number, stepFunction: () => Promise<any>) => {
    setCurrentStep(stepIndex), []), []);
    
    // 更新步骤状态为运行中
    const updatedSteps = useMemo(() => useMemo(() => [...demoSteps], []), []);
    updatedSteps[stepIndex].status = 'running';
    setDemoSteps(updatedSteps);

    try {
      const result = useMemo(() => useMemo(() => await stepFunction(), []), []);
  ...
  const initializeServices = useMemo(() => useMemo(() => async () => {
    console.log('🔧 开始初始化所有服务...'), []), []);

    const results = useMemo(() => useMemo(() => {
      fiveDiagnosis: '',
      mlTraining: '',
      agentCoordination: '',
    }, []) // TODO: 检查依赖项, []);
  ...
  const demonstrateFiveDiagnosis = useMemo(() => useMemo(() => async () => {
    console.log('🔍 开始五诊算法演示...'), []), []);

    const diagnosisInput = useMemo(() => useMemo(() => {
    ...
    const result = useMemo(() => useMemo(() => await fiveDiagnosisService.performDiagnosis(diagnosisInput), []), []);
  ...
  const demonstrateAgentCollaboration = useMemo(() => useMemo(() => async () => {
    console.log('🤝 开始智能体协作演示...'), []), []);

    const diagnosisData = useMemo(() => useMemo(() => {
      userId: 'demo_user_001',
    ...
    const collaboration = useMemo(() => useMemo(() => await agentCoordinationService.performCollaborativeDiagnosis(
      'demo_user_001',
      diagnosisData
    ), []), []);
  ...
  const demonstrateMLTraining = useMemo(() => useMemo(() => async () => {
    console.log('🎯 开始机器学习训练演示...'), []), []);

    // 模拟训练数据
    const trainingData = useMemo(() => useMemo(() => [
      {
        id: 'train_001',
    ...
    const modelConfig = useMemo(() => useMemo(() => {
    ...
    try {
      const trainingTask = useMemo(() => useMemo(() => await mlTrainingService.createTrainingTask(
  ...
  const performIntegrationAnalysis = useMemo(() => useMemo(() => async () => {
    console.log('📊 开始集成分析...'), []), []);

    // 获取所有系统的状态
    const systemStatus = useMemo(() => useMemo(() => {
      fiveDiagnosis: fiveDiagnosisService.getServiceStatus(),
      mlTraining: mlTrainingService.getServiceStatus(),
      agentCoordination: agentCoordinationService.getServiceStatus(),
    }, []) // TODO: 检查依赖项, []);
    ...
    // 模拟集成分析结果
    const integrationResult = useMemo(() => useMemo(() => {
      systemHealth: {
        overall: 'excellent',
  ...
  const renderStepCard = useMemo(() => useMemo(() => useCallback( (step: DemoStep, index: number) => {, []), []), []);
  ...
  // TODO: 将内联组件移到组件外部
const renderServiceStatus = useMemo(() => useMemo(() => () => (
