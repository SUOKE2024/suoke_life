...
export const DiagnosisModal: React.FC<DiagnosisModalProps> = ({
  ...
  const loading = useMemo(() => useMemo(() => useAppSelector(selectDiagnosisLoading), []), []);
  const currentSession = useMemo(() => useMemo(() => useAppSelector(selectCurrentSession), []), []);
  ...
  const initializeDiagnosisSteps = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
  ...
  const completeStep = useMemo(() => useMemo(() => async (stepData: any) => {
    if (!sessionId) {return, []), []);}

    try {
      const result = useMemo(() => useMemo(() => await dispatch(
        submitDiagnosisData({
          sessionId,
          type: diagnosisType,
          data: {
      ...
      if (submitDiagnosisData.fulfilled.match(result)) {
        // 更新步骤状态
        const updatedSteps = useMemo(() => useMemo(() => [...steps], []), []);
  ...
  const completeDiagnosis = useMemo(() => useMemo(() => async () => {
    if (!sessionId) {return, []), []);}

    try {
      const result = useMemo(() => useMemo(() => await dispatch(completeDiagnosisSession(sessionId)), []), []);
  ...
  const renderStepContent = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    const step = useMemo(() => useMemo(() => steps[currentStep], []), []);
    if (!step) {return null;}
  ...
  // TODO: 将内联组件移到组件外部
const renderProgressBar = useMemo(() => useMemo(() => () => (
