...
export const EcoLifestyleNavigator: React.FC<EcoLifestyleNavigatorProps> = ({
  ...
  const handleServiceSelect = useMemo(() => useMemo(() => useCallback( (service: EcoService) => {, []), []), []);
    setSelectedService(service);
  };

  const startService = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
  ...
  // TODO: 将内联组件移到组件外部
const renderCategoryTabs = useMemo(() => useMemo(() => () => (
    <View style={styles.categoryTabs}>
      ...
      <TouchableOpacity
        ...
        onPress={() => setSelectedCategory('food_agriculture')}
      >
      ...
      <TouchableOpacity
        ...
        onPress={() => setSelectedCategory('mountain_wellness')}
      >
  ...
  const renderServiceCard = useMemo(() => useMemo(() => (service: EcoService) => (
    <TouchableOpacity
      key={service.id}
      style={[
        styles.serviceCard,
  ...
  const renderServiceDetails = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
