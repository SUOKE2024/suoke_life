...
// 热门话题
const HOT_TOPICS = useMemo(() => useMemo(() => [
...
const ExploreScreen: React.FC = () => {
  ...
  // 过滤内容
  const filteredContent = useMemo(() => useMemo(() => LAOKE_WISDOM.filter((item) => {
  ...
  // 精选内容
  const featuredContent = useMemo(() => useMemo(() => LAOKE_WISDOM.filter(item => item.featured), []), []);

  // 与老克对话
  const chatWithLaoke = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    Alert.alert(
      '与老克对话',
  ...
  // 开始与老克对话
  const startLaokeChat = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    setAgentChatVisible(true);
    console.log('Starting chat with Laoke agent');
  };

  // 查看内容详情
  const viewContent = useMemo(() => useMemo(() => useCallback( (item: ContentItem) => {, []), []), []);
    Alert.alert(
      item.title,
  ...
  // 开始学习
  const startLearning = useMemo(() => useMemo(() => useCallback( (item: ContentItem) => {, []), []), []);
    Alert.alert('开始学习', `正在为您准备《${item.title}》的学习内容...`);
    console.log(`Starting learning: ${item.id}`);
  };

  // 获取难度文本
  const getDifficultyText = useMemo(() => useMemo(() => useCallback( (difficulty: string) => {, []), []), []);
    switch (difficulty) {
  ...
  // 获取内容类型文本
  const getContentTypeText = useMemo(() => useMemo(() => useCallback( (type: ContentType) => {, []), []), []);
    switch (type) {
  ...
  // 渲染分类标签
  const renderCategoryTab = useMemo(() => useMemo(() => useCallback( (category: CategoryType | 'all') => {, []), []), []);
    const isSelected = useMemo(() => useMemo(() => selectedCategory === category, []), []);
    const config = useMemo(() => useMemo(() => category === 'all'
  ...
  // 渲染内容卡片
  const renderContentCard = useMemo(() => useMemo(() => useCallback( ({ item }: { item: ContentItem }) => {, []), []), []);
    const categoryConfig = useMemo(() => useMemo(() => CATEGORIES[item.category], []), []);
  ...
  // 渲染热门话题
  const renderHotTopic = useMemo(() => useMemo(() => (topic: typeof HOT_TOPICS[0], index: number) => (
