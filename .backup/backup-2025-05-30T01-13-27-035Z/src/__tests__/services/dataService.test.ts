...
const dataService = {
  ...
  getDataStats: async () => {
    ...
    const stats = {
      ...
      cacheKeys: allKeys.filter((key: string) => key.startsWith("cache_"))
        .length,
...
describe("数据服务测试", () => {
  ...
  describe("数据备份和恢复", () => {
    ...
    it("应该拒绝无效的备份数据", async () => {
      await expect(dataService.restoreData(null)).rejects.toThrow(
        "无效的备份数据"
      );
  ...
  describe("数据压缩和解压", () => {
    ...
    it("应该拒绝无效的压缩数据", async () => {
      await expect(dataService.decompressData(null)).rejects.toThrow(
        "无效的压缩数据"
      );
