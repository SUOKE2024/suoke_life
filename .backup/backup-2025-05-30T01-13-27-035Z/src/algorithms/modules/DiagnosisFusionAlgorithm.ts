...
export class DiagnosisFusionAlgorithm {
   ...
   * 识别证候
   */
  private async identifySyndromes(
    evidence: Evidence[],
    userProfile?: UserProfile
  ...
  private calculateSyndromeConfidence(
    evidence: Evidence[],
    pattern: any
  ): number {
    // 计算证候置信度
  ...
  private calculateConstitutionScore(
    evidence: Evidence[],
    type: string,
    userProfile?: UserProfile
  ): number {
  ...
  private calculateOverallConfidence(
    ...
    const syndromeScore =
      syndromes.length > 0
        ? syndromes.reduce((sum, s) => sum + s.confidence, 0) / syndromes.length
        : 0.5;
  ...
  private async generateFollowUpAdvice(
    syndromes: SyndromeResult[],
    constitution: ConstitutionResult,
    dataQuality: DataQualityReport
  ): Promise<string[]> {
