...
export class InquiryDiagnosisAlgorithm {
   ...
   * 预处理症状数据
   */
  private async preprocessSymptoms(
    symptoms: SymptomData[]
  ): Promise<SymptomData[]> {
  ...
  private calculateOverallConfidence(
    ...
    if (analyses.syndromePatterns && analyses.syndromePatterns.length > 0) {
      const avgConfidence =
        analyses.syndromePatterns.reduce((sum, p) => sum + p.confidence, 0) /
        analyses.syndromePatterns.length;
      totalWeight += 0.3;
  ...
  private async generateComprehensiveAnalysis(
    ...
    const comprehensiveAnalysis =
      await this.knowledgeBase.generateCalculationAnalysis({
  ...
  private async extractConstitutionalFeatures(
    data: ProcessedInquiryData
  ): Promise<ConstitutionalFeatures> {
    // 从问诊数据中提取体质特征
    return {
      bodyType: "适中",
      energyLevel: "正常",
      ...
      sleepQuality: data.sleepPattern?.sleepQuality
        ? data.sleepPattern.sleepQuality > 7
          ? "良好"
          : "一般"
        : "未知",
    };
  ...
  private async extractLifestyleFeatures(
    data: ProcessedInquiryData
  ): Promise<LifestyleFeatures> {
    // 从问诊数据中提取生活方式特征
    return {
      activityLevel: data.lifestyle?.exerciseHabits || "未知",
      stressFactors: data.lifestyle?.stressLevel
        ? [`压力水平：${data.lifestyle.stressLevel}`]
        : [],
      environmentalFactors: data.lifestyle?.workEnvironment
        ? [data.lifestyle.workEnvironment]
        : [],
      socialFactors: [],
    };
  }

  private async extractEmotionalFeatures(
    data: ProcessedInquiryData,
    nlpResults: NLPResults
    ...
    return {
      ...
      emotionalStability: data.emotionalState?.stressLevel
        ? data.emotionalState.stressLevel < 5
          ? "稳定"
          : "不稳定"
        : "未知",
      stressResponse: "正常",
