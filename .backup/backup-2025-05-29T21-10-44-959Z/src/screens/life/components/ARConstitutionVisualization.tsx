...
export const ARConstitutionVisualization: React.FC<ARConstitutionVisualizationProps> = ({
  ...
  const rotationAnim = useMemo(() => useMemo(() => useRef(new Animated.ValueXY()).current, []), []);
  const scaleAnim = useMemo(() => useMemo(() => useRef(new Animated.Value(1)).current, []), []);
  ...
  const [constitutionData] = useState<ConstitutionVisualization[]>([
    ...
    },
    {
      type: 'yin_deficiency',
      name: '阴虚质',
      characteristics: ['阴液不足', '虚热内扰', '口干咽燥'],
      color: '#FF6B6B',
      intensity: 0.7,
      regions: ['肾脏', '肝脏', '心脏'],
    },
  ...
  const startARMode = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    Alert.alert(
      'AR模式',
  ...
  // TODO: 将内联组件移到组件外部
const renderConstitutionView = useMemo(() => useMemo(() => () => (
  ...
  // TODO: 将内联组件移到组件外部
const renderMeridiansView = useMemo(() => useMemo(() => () => (
    <View style={styles.viewContent}>
  ...
  // TODO: 将内联组件移到组件外部
const renderAcupointsView = useMemo(() => useMemo(() => () => (
    <View style={styles.viewContent}>
  ...
  // TODO: 将内联组件移到组件外部
const renderARView = useMemo(() => useMemo(() => () => (
    <View style={styles.viewContent}>
        ...
        <View style={styles.arCamera}>
          ...
          <View style={styles.arControls}>
            <TouchableOpacity style={styles.arButton}>
              ...
              <Text style={styles.arButtonText}>定位穴位</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.arButton}>
              <Icon name="eye" size={24} color="white" />
              <Text style={styles.arButtonText}>显示经络</Text>
            </TouchableOpacity>
          </View>
          ...
          >
            <Text style={styles.exitArText}>退出AR模式</Text>
          </TouchableOpacity>
        </View>
      ) : (
        <View style={styles.arIntro}>
          <Icon name="camera-3d" size={80} color={colors.primary} />
          <Text style={styles.arTitle}>AR体质可视化</Text>
          <Text style={styles.arDescription}>
            使用增强现实技术，在真实环境中查看经络穴位分布，
            获得更直观的中医理论学习体验。
          </Text>
          ...
          <View style={styles.arFeatures}>
            <View style={styles.featureItem}>
              ...
              <Text style={styles.featureText}>实时穴位定位</Text>
            </View>
            <View style={styles.featureItem}>
              <Icon name="gesture-tap" size={24} color={colors.primary} />
              <Text style={styles.featureText}>交互式学习</Text>
            </View>
            <View style={styles.featureItem}>
              <Icon name="chart-line" size={24} color={colors.primary} />
              <Text style={styles.featureText}>体质分析</Text>
            </View>
          </View>
          
          <TouchableOpacity style={styles.startArButton} onPress={startARMode}>
            <Icon name="camera" size={20} color="white" />
            <Text style={styles.startArText}>启动AR模式</Text>
          </TouchableOpacity>
        </View>
      )}
    </View>
      ...
      {[
        ...
        >
          <Icon 
            name={tab.icon} 
            size={20} 
            color={activeView === tab.key ? colors.primary : colors.textSecondary} 
          />
          <Text style={[
            styles.tabText,
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerContent}>
            ...
            <Text style={styles.subtitle}>经络穴位 • 体质分析 • 增强现实</Text>
          </View>
