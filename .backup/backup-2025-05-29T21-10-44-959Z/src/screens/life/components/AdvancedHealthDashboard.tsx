...
export const AdvancedHealthDashboard: React.FC<AdvancedHealthDashboardProps> = ({
  ...
  const getTrendIcon = useMemo(() => useMemo(() => useCallback( (trend: string) => {, []), []), []);
    switch (trend) {
  ...
  // TODO: 将内联组件移到组件外部
const renderPeriodSelector = useMemo(() => useMemo(() => () => (
  ...
  const renderMetricCard = useMemo(() => useMemo(() => (metric: HealthMetric) => (
    <TouchableOpacity
      key={metric.id}
  ...
  // TODO: 将内联组件移到组件外部
const renderOverviewCharts = useMemo(() => useMemo(() => () => (
    <View style={styles.chartsContainer}>
      ...
      <View style={styles.chartCard}>
        <Text style={styles.chartTitle}>睡眠分析</Text>
        <PieChart
          data={[
            ...
            { name: 'REM', population: 1.2, color: '#E74C3C', legendFontColor: colors.textSecondary },
  ...
  const renderMetricDetail = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
      ...
      >
        <View style={styles.detailContainer}>
          <View style={styles.detailHeader}>
            ...
            <Text style={styles.detailTitle}>{selectedMetric.name}详情</Text>
            <View style={styles.placeholder} />
          </View>

          <ScrollView style={styles.detailContent}>
            <View style={styles.detailMetricCard}>
              <View style={[styles.detailMetricIcon, { backgroundColor: selectedMetric.color + '20' }]}>
                <Icon name={selectedMetric.icon} size={32} color={selectedMetric.color} />
              </View>
              <Text style={styles.detailMetricValue}>
                {selectedMetric.value} {selectedMetric.unit}
              </Text>
              <Text style={[styles.detailMetricStatus, { color: getStatusColor(selectedMetric.status) }]}>
                {selectedMetric.status === 'excellent' ? '优秀' : 
            ...
            <View style={styles.detailChart}>
              <Text style={styles.detailChartTitle}>7天趋势</Text>
              <LineChart
                data={heartRateData}
                width={width - 48}
                height={200}
            ...
            <View style={styles.detailInsights}>
              <Text style={styles.detailInsightsTitle}>健康洞察</Text>
              <View style={styles.insightItem}>
                <Icon name="lightbulb" size={20} color={colors.warning} />
                <Text style={styles.insightText}>
                  您的{selectedMetric.name}在正常范围内，建议继续保持良好的生活习惯。
                </Text>
              </View>
              <View style={styles.insightItem}>
                <Icon name="target" size={20} color={colors.primary} />
                <Text style={styles.insightText}>
                  建议目标：保持当前水平，适当增加有氧运动。
                </Text>
              </View>
            </View>
          </ScrollView>
    ...
    >
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>健康仪表板</Text>
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.summarySection}>
            ...
            <View style={styles.metricsGrid}>
              {HEALTH_METRICS.map(renderMetricCard)}
            </View>
          </View>
