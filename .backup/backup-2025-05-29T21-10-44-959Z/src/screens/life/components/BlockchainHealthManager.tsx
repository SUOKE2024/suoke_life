...
export const BlockchainHealthManager: React.FC<BlockchainHealthManagerProps> = ({
  ...
  const getTypeColor = useMemo(() => useMemo(() => useCallback( (type: string) => {, []), []), []);
    switch (type) {
  ...
  const getEntityIcon = useMemo(() => useMemo(() => useCallback( (type: string) => {, []), []), []);
    switch (type) {
  ...
  const toggleRecordSharing = useMemo(() => useMemo(() => async (recordId: string) => {
    setIsLoading(true), []), []);
    try {
      // 模拟区块链操作
  ...
  const togglePermission = useMemo(() => useMemo(() => async (permissionId: string) => {
    setIsLoading(true), []), []);
    try {
      await new Promise<void>(resolve => setTimeout(() => resolve(), 1000));
...
const renderTabBar = useMemo(() => useMemo(() => () => (
  ...
  const renderRecordCard = useMemo(() => useMemo(() => (record: HealthDataRecord) => (
    <TouchableOpacity
      key={record.id}
  ...
  const renderPermissionCard = useMemo(() => useMemo(() => (permission: DataPermission) => (
  ...
  // TODO: 将内联组件移到组件外部
const renderSecuritySettings = useMemo(() => useMemo(() => () => (
