...
// 健康成就
const HEALTH_ACHIEVEMENTS: HealthAchievement[] = [
  {
    id: 'early_bird',
    title: '早起达人',
...
// 健康统计数据
const HEALTH_STATS = useMemo(() => useMemo(() => [
...
const ProfileScreen: React.FC = () => {
  ...
  // 设置项配置
  const settingsSections = useMemo(() => useMemo(() => [
    {
      title: '健康档案',
      items: [
        {
  ...
  // 与智能体对话
  const chatWithAgent = useMemo(() => useMemo(() => useCallback( (agent: AgentInteraction) => {, []), []), []);
    Alert.alert(
      ...
      [
        ...
        { text: '开始对话', onPress: () => console.log(`Chat with ${agent.agentType}`) },
      ]
    );
  };

  // 查看成就详情
  const viewAchievement = useMemo(() => useMemo(() => useCallback( (achievement: HealthAchievement) => {, []), []), []);
    const progressText = useMemo(() => useMemo(() => achievement.progress && achievement.target 
    ...
    Alert.alert(
      ...
      [
        { text: '了解更多', onPress: () => console.log(`View achievement: ${achievement.id}`) },
      ]
    );
  };

  // 使用会员特权
  const useBenefit = useMemo(() => useMemo(() => useCallback( (benefit: MemberBenefit) => {, []), []), []);
    if (!benefit.available) {
      Alert.alert('特权不可用', '该特权暂时不可用');
      return;
    }

    const usageText = useMemo(() => useMemo(() => benefit.limit 
    ...
    Alert.alert(
      ...
      [
        { text: '取消', style: 'cancel' },
        { text: '立即使用', onPress: () => console.log(`Use benefit: ${benefit.id}`) },
      ]
    );
  };
  ...
  // 渲染用户头像和基本信息
  // TODO: 将内联组件移到组件外部
const renderUserHeader = useMemo(() => useMemo(() => () => (
    <View style={styles.userHeader}>
      <View style={styles.avatarContainer}>
        ...
        <View style={[styles.healthScoreBadge, { backgroundColor: getHealthScoreColor() }]}>
          <Text style={styles.healthScoreText}>{USER_PROFILE.healthScore}</Text>
        </View>
      </View>
      ...
      <View style={styles.userInfo}>
        ...
        <Text style={styles.userDetails}>
          {USER_PROFILE.age}岁 • {USER_PROFILE.gender === 'male' ? '男' : '女'}{' '}
          • {USER_PROFILE.constitution}
        </Text>
        <View style={styles.memberInfo}>
          <Icon name="star" size={14} color="#FFD700" />
          <Text style={styles.memberLevel}>{USER_PROFILE.memberLevel}</Text>
          <Text style={styles.joinDate}>• 加入于{USER_PROFILE.joinDate}</Text>
        </View>
  ...
  // 获取健康评分颜色
  const getHealthScoreColor = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
  ...
  // 渲染健康统计
  // TODO: 将内联组件移到组件外部
const renderHealthStats = useMemo(() => useMemo(() => () => (
...
const renderTabBar = useMemo(() => useMemo(() => () => (
  ...
  // 渲染智能体交互卡片
  const renderAgentCard = useMemo(() => useMemo(() => ({ item }: { item: AgentInteraction }) => (
    <TouchableOpacity style={styles.agentCard} onPress={() => chatWithAgent(item)}>
      <View style={styles.agentHeader}>
        ...
        <View style={styles.agentInfo}>
          ...
          <Text style={styles.agentFeature}>最常使用：{item.favoriteFeature}</Text>
        </View>
        <View style={styles.agentStats}>
          <Text style={[styles.interactionCount, { color: item.color }]}>
            {item.totalInteractions}次
          </Text>
  ...
  // 渲染成就卡片
  const renderAchievementCard = useMemo(() => useMemo(() => ({ item }: { item: HealthAchievement }) => (
    <TouchableOpacity 
    ...
    >
      ...
      <View style={styles.achievementInfo}>
        ...
        {!item.unlocked && item.progress && item.target && (
  ...
  // 渲染会员特权卡片
  const renderBenefitCard = useMemo(() => useMemo(() => ({ item }: { item: MemberBenefit }) => (
    <TouchableOpacity 
  ...
  // 渲染设置项
  const renderSettingItem = useMemo(() => useMemo(() => (item: SettingItem) => (
    <TouchableOpacity
      key={item.id}
  ...
  // 渲染设置分组
  const renderSettingsSection = useMemo(() => useMemo(() => (section: (typeof settingsSections)[0]) => (
  ...
  // 退出登录
  const handleLogout = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
        ...
        <View style={styles.contentContainer}>
          ...
          {selectedTab === 'agents' && (
            <View style={styles.agentsSection}>
              <Text style={styles.sectionTitle}>智能体交互记录</Text>
              <FlatList
          ...
          {selectedTab === 'achievements' && (
            <View style={styles.achievementsSection}>
              <Text style={styles.sectionTitle}>健康成就</Text>
              <FlatList
