...
export const ServiceManagementScreen: React.FC = () => {
  ...
  const initializeServices = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    // 创建服务列表
  ...
  // 检查服务状态
  const checkServicesStatus = useMemo(() => useMemo(() => async (servicesList: ServiceInfo[]) => {
    setLoading(true), []), []);
    
    try {
      const updatedServices = useMemo(() => useMemo(() => [...(servicesList || services)], []), []);
      
      // 使用AbortController实现超时功能
      const controller = useMemo(() => useMemo(() => new AbortController(), []), []);
      const timeoutId = useMemo(() => useMemo(() => setTimeout(() => controller.abort(), 3000), []), []);
      
      // 并行检查所有服务
      const checkPromises = useMemo(() => useMemo(() => updatedServices.map(async (service) => {
        try {
      ...
      // 等待所有检查完成
      const results = useMemo(() => useMemo(() => await Promise.all(checkPromises), []), []);
      clearTimeout(timeoutId);
      
      // 更新服务状态
      results.forEach(result => {
        const index = useMemo(() => useMemo(() => updatedServices.findIndex(s => s.id === result.id), []), []);
        if (index !== -1) {
  ...
  // 启动服务
  const startService = useMemo(() => useMemo(() => async (serviceId: string) => {
    // 更新服务状态为启动中
  ...
  // 停止服务
  const stopService = useMemo(() => useMemo(() => async (serviceId: string) => {
    // 更新服务状态为停止中
    setServices(prev => prev.map(s => 
      s.id === serviceId 
  ...
  // 重启服务
  const restartService = useMemo(() => useMemo(() => async (serviceId: string) => {
    // 更新服务状态为停止中
    setServices(prev => prev.map(s => 
      s.id === serviceId 
  ...
  // 启动所有服务
  const startAllServices = useMemo(() => useMemo(() => async () => {
    if (loading) {return, []), []);}
  ...
  // 停止所有服务
  const stopAllServices = useMemo(() => useMemo(() => async () => {
    if (loading) {return, []), []);}
  ...
  // 刷新服务状态
  const refreshServicesStatus = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    checkServicesStatus(services);
  };

  // 返回处理
  const handleBack = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    navigation.goBack();
  };

  // 获取服务状态样式
  const getStatusStyle = useMemo(() => useMemo(() => useCallback( (status: string) => {, []), []), []);
    switch (status) {
      case 'running':
  ...
  // 渲染服务项
  const renderServiceItem = useMemo(() => useMemo(() => useCallback( (service: ServiceInfo) => {, []), []), []);
    const isDisabled = useMemo(() => useMemo(() => service.status === 'starting' || service.status === 'stopping', []), []);
