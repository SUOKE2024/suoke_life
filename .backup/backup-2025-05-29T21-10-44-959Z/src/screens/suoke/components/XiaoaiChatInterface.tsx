...
const XiaoaiChatInterface: React.FC<XiaoaiChatInterfaceProps> = ({
  ...
  const sendMessage = useMemo(() => useMemo(() => async () => {
    ...
    try {
      ...
      // 调用小艾智能体
      const response = useMemo(() => useMemo(() => await xiaoaiAgent.chat(userMessage.content, context), []), []);
  ...
  const showActionButtons = useMemo(() => useMemo(() => useCallback( (actions: any[]) => {, []), []), []);
    const actionTexts = useMemo(() => useMemo(() => actions.map(action => action.prompt).join('\n\n'), []), []);
  ...
  const handleActionAccepted = useMemo(() => useMemo(() => useCallback( (actions: any[]) => {, []), []), []);
    // 处理用户接受的操作建议
  ...
  const handleSuggestionPress = useMemo(() => useMemo(() => useCallback( (suggestion: string) => {, []), []), []);
