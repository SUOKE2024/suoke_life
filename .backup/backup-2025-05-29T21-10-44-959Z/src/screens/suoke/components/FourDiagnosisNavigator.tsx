...
export const FourDiagnosisNavigator: React.FC<FourDiagnosisNavigatorProps> = ({
  visible,
  onClose,
  onDiagnosisSelect,
}) => {
  ...
  const handleMethodSelect = useMemo(() => useMemo(() => useCallback( (method: DiagnosisMethod) => {, []), []), []);
  ...
  const startDiagnosis = useMemo(() => useMemo(() => async () => {
  ...
  const renderMethodCard = useMemo(() => useMemo(() => (method: DiagnosisMethod) => (
    <TouchableOpacity
      key={method.id}
      style={[
        styles.methodCard,
  ...
  const renderSelectedMethodDetails = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    if (!selectedMethod) {return null;}

    return (
      <View style={styles.detailsContainer}>
        <Text style={styles.detailsTitle}>诊断详情</Text>
        
        <View style={styles.detailsCard}>
          <View style={styles.detailsHeader}>
            <View style={[styles.detailsIcon, { backgroundColor: selectedMethod.color + '20' }]}>
              <Icon name={selectedMethod.icon} size={24} color={selectedMethod.color} />
            </View>
            <View style={styles.detailsInfo}>
              <Text style={styles.detailsName}>{selectedMethod.name}</Text>
              <Text style={styles.detailsAccuracy}>准确率: {selectedMethod.accuracy}%</Text>
            </View>
          </View>
          ...
          <View style={styles.detailsFeatures}>
            <Text style={styles.detailsFeaturesTitle}>功能特点:</Text>
    ...
    >
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>四诊系统</Text>
          <View style={styles.placeholder} />
        </View>

        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.introSection}>
            <Text style={styles.introTitle}>中医四诊合参</Text>
            <Text style={styles.introDescription}>
              运用现代AI技术结合传统中医理论，通过望、闻、问、切四种诊断方法，
              为您提供全面的健康评估和个性化建议。
            </Text>
          </View>

          <View style={styles.methodsSection}>
            <Text style={styles.sectionTitle}>选择诊断方法</Text>
            {DIAGNOSIS_METHODS.map(renderMethodCard)}
          </View>
            ...
            >
              {isLoading ? (
                ...
                <>
                  ...
                  <Text style={styles.startButtonText}>开始{selectedMethod.name}</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
