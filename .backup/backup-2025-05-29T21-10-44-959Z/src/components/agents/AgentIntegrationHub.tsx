...
export const AgentIntegrationHub: React.FC<AgentIntegrationHubProps> = ({
  ...
  const loadAgentStatuses = useMemo(() => useMemo(() => async () => {
    try {
      const statuses = await coordinationService.getAgentStatus() as AgentStatus[], []), []);
      const statusMap = useMemo(() => useMemo(() => statuses.reduce((acc, status) => {
  ...
  const renderAgentCard = useMemo(() => useMemo(() => useCallback((agentInfo: AgentInfo) => {
    const status = agentStatuses[agentInfo.id], []), []);
    const isSelected = useMemo(() => useMemo(() => selectedAgent === agentInfo.id, []), []);
    const isOnline = useMemo(() => useMemo(() => status?.isOnline ?? false, []), []);
  ...
  const renderSelectedAgentDetails = useMemo(() => useMemo(() => useMemo(() => {
    const agentInfo = AGENT_INFO[selectedAgent], []), []);
    const status = useMemo(() => useMemo(() => agentStatuses[selectedAgent], []), []);
