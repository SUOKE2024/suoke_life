...
const ContactsList: React.FC<ContactsListProps> = ({
  ...
  // 过滤联系人
  const filteredContacts = useMemo(() => useMemo(() => useMemo(() => {
  ...
  // 分组联系人
  const groupedContacts = useMemo(() => useMemo(() => useMemo(() => {
    if (!groupByType) {
      return [{ title: '全部联系人', data: filteredContacts }], []), []);
    }

    const groups = useMemo(() => useMemo(() => {
  ...
  const getContactTypeIcon = useMemo(() => useMemo(() => useCallback( (type: Contact['type']) => {, []), []), []);
    switch (type) {
      case 'agent':
        return 'robot';
      case 'doctor':
  ...
  const getContactTypeColor = useMemo(() => useMemo(() => useCallback( (type: Contact['type']) => {, []), []), []);
    switch (type) {
      case 'agent':
  ...
  const renderContactItem = useMemo(() => useMemo(() => ({ item }: { item: Contact }) => (
    <TouchableOpacity
  ...
  const renderSectionHeader = useMemo(() => useMemo(() => ({ section }: { section: { title: string } }) => (
  ...
  const handleStartChat = useMemo(() => useMemo(() => useCallback( (contact: Contact) => {, []), []), []);
    onContactPress(contact);
  };

  const handleBookAppointment = useMemo(() => useMemo(() => useCallback( (contact: Contact) => {, []), []), []);
    Alert.alert(
      '预约医生',
  ...
  // TODO: 将内联组件移到组件外部
const renderEmptyState = useMemo(() => useMemo(() => () => (
