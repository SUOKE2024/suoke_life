...
const Input: React.FC<InputProps> = ({
  ...
  const getKeyboardType = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    switch (type) {
      case 'email':
  ...
  const textInputStyle = useMemo(() => useMemo(() => [
    styles.input,
    styles[`${size}Input`],
  ...
  const handleFocus = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    setIsFocused(true);
    onFocus?.();
  };

  const handleBlur = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
    setIsFocused(false);
    onBlur?.();
  };

  const togglePasswordVisibility = useMemo(() => useMemo(() => useCallback( () => {, []), []), []);
