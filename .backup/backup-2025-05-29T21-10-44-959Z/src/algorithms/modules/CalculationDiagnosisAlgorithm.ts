...
export class CalculationDiagnosisAlgorithm {
  ...
  public async analyze(
    ...
    try {
      ...
      // 五行分析
      const fiveElements = await this.analyzeFiveElements(
        lunarData,
        userProfile
      );
  ...
  private async preprocessData(
    data: CalculationData
  ): Promise<ProcessedCalculationData> {
    // 验证日期时间格式
   ...
   * 计算农历数据
   */
  private async calculateLunarData(
  ...
  private async generateComprehensiveAnalysis(analyses: any): Promise<string> {
    ...
    // 五行分析总结
    const fiveElementsSummary = this.summarizeFiveElements(
      analyses.fiveElements
    );
    ...
    // 体质分析总结
    const constitutionSummary = this.summarizeConstitution(
      analyses.constitution
    );
    ...
    const comprehensiveAnalysis =
      await this.knowledgeBase.generateCalculationAnalysis(analyses);
  ...
  private getFiveElementsRecommendations(
