...
export class FiveDiagnosisExample {
  ...
  private displayResults(result: any): void {
    ...
    if (result.fusionResult) {
      console.log("🔸 融合分析:");
      console.log(
        `  置信度: ${(result.fusionResult.confidence * 100).toFixed(1)}%`
      );
      ...
      if (
        ...
        result.fusionResult.primarySyndromes.forEach(
          (syndrome: any, index: number) => {
            console.log(
              `    ${index + 1}. ${syndrome.name} (置信度: ${(
                syndrome.confidence * 100
              ).toFixed(1)}%)`
            );
          }
        );
    ...
    if (result.qualityReport) {
      console.log("🔸 质量评估:");
      console.log(
        `  数据质量: ${(result.qualityReport.score * 100).toFixed(1)}%`
      );
      console.log(
        `  可靠性: ${result.qualityReport.valid ? "可靠" : "需要改进"}`
      );
      console.log("");
    }
