...
class DeviceIntegrationTester {
  ...
  async runFullIntegrationTest(): Promise<IntegrationTestReport> {
    ...
    try {
      ...
      console.log(
        `✅ 集成测试完成: ${passedTests}/${totalTests} 通过 (${report.overallResult.passRate.toFixed(
          1
        )}%)`
      );
      return report;
    ...
    tests.push(
      await this.runTest("应用启动性能", async () => {
        const startupMetrics =
          await performanceMonitor.testAppStartupPerformance();
    ...
    tests.push(
      await this.runTest("内存泄漏测试", async () => {
        ...
        if (memoryTest.averageGrowthPerIteration > 10 * 1024 * 1024) {
          ...
          throw new Error(
            `内存增长过快: ${
              memoryTest.averageGrowthPerIteration / (1024 * 1024)
            }MB per iteration`
          );
        }
        return memoryTest;
