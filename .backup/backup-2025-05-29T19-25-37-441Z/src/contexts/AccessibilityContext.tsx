...
export const AccessibilityProvider: React.FC<AccessibilityProviderProps> = ({ children }) => {
  ...
  const updateConfig = async (updates: Partial<AccessibilityConfig>) => {
    ...
    try {
      await AsyncStorage.setItem('accessibility_config', JSON.stringify(newConfig));
    } catch (error) {
  ...
  const speak = (text: string, options: SpeechOptions = {}) => {
    if (!config.voiceNavigationEnabled && !isScreenReaderEnabled) {return;}
...
export const accessibilityUtils = {
  ...
  // 检查触摸目标尺寸
  checkTouchTargetSize: (width: number, height: number, minSize: number = 44): boolean => {
  ...
  checkContrastRatio: (foreground: string, background: string, level: 'AA' | 'AAA' = 'AA'): boolean => {
    const contrast = accessibilityUtils.calculateContrast(foreground, background);
