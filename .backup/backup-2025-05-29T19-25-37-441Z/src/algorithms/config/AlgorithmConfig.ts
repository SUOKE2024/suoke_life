...
export class AlgorithmConfig {
   ...
   * 初始化知识库配置
   */
  private initializeKnowledgeBaseConfig(config?: Partial<KnowledgeBaseConfig>): KnowledgeBaseConfig {
    return {
   ...
   * 初始化质量控制配置
   */
  private initializeQualityControlConfig(config?: Partial<QualityControlConfig>): QualityControlConfig {
    return {
      enabled: config?.enabled ?? true,
      checks: {
  ...
  public update(newConfig: Partial<AlgorithmConfigOptions>): void {
    ...
    Object.assign(this, new AlgorithmConfig({
      ...this.toJSON(),
      ...newConfig,
    }));
  }
  ...
  public validate(): { valid: boolean; errors: string[] } {
    ...
    confidenceThresholds.forEach((threshold, index) => {
      if (threshold < 0 || threshold > 1) {
        errors.push(`置信度阈值必须在0-1之间，诊法${index}的阈值为${threshold}`);
      }
    });
