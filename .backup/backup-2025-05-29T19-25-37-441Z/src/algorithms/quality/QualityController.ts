...
export class QualityController {
  ...
  public async validate(input: ValidationInput): Promise<QualityReport> {
    ...
    try {
      ...
      for (const [ruleId, rule] of Array.from(this.validationRules.entries())) {
        if (!this.isRuleEnabled(ruleId)) {continue;}
      ...
      // 生成警告和建议
      this.generateWarningsAndAdvice(input, issues, warnings, medicalAdvice, followUpRecommendations);
  ...
  private checkDataCompleteness(input: ValidationInput): ValidationResult {
    ...
    // 检查各诊法数据
    const diagnosisTypes = ['lookingData', 'listeningData', 'inquiryData', 'palpationData', 'calculationData'];
  ...
  private checkConfidenceThreshold(input: ValidationInput): ValidationResult {
    ...
    if (diagnosisResults.looking) {confidences.push(diagnosisResults.looking.confidence);}
    if (diagnosisResults.listening) {confidences.push(diagnosisResults.listening.confidence);}
    if (diagnosisResults.inquiry) {confidences.push(diagnosisResults.inquiry.confidence);}
    if (diagnosisResults.palpation) {confidences.push(diagnosisResults.palpation.confidence);}
    if (diagnosisResults.calculation) {confidences.push(diagnosisResults.calculation.confidence);}
