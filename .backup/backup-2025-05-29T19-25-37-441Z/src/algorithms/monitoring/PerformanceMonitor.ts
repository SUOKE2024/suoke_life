...
export class PerformanceMonitor {
  ...
  public recordAccuracy(accuracy: number, diagnosisType: string): void {
    if (!this.config.enabled || !this.config.metrics.accuracy) {return;}
  ...
  public recordError(errorType: string, details?: any): void {
    if (!this.config.enabled || !this.config.metrics.errors) {return;}
  ...
  public recordResourceUsage(cpu: number, memory: number, disk: number): void {
    if (!this.config.enabled || !this.config.metrics.usage) {return;}
  ...
  public generateReport(): any {
    ...
    return {
      ...
      summary: {
        totalRequests: this.events.filter(e => e.type === 'response_time').length,
  ...
  private updateAccuracyMetrics(): void {
    ...
    if (accuracyEvents.length === 0) {return;}
    ...
    for (const type of diagnosisTypes) {
      ...
      if (typeEvents.length > 0) {
        this.metrics.accuracy.diagnosisAccuracy[type as keyof typeof this.metrics.accuracy.diagnosisAccuracy] = 
