...
describe('健康算法测试', () => {
  ...
  describe('心率分析', () => {
    it('应该正确分类心率', () => {
      const classifyHeartRate = (heartRate: number, age: number) => {
        ...
        if (percentage < 50) {return '休息';}
        if (percentage < 60) {return '轻度';}
        if (percentage < 70) {return '中度';}
        if (percentage < 85) {return '高强度';}
        return '最大强度';
      };
    ...
    it('应该计算目标心率区间', () => {
      const calculateTargetHeartRate = (age: number, intensity: 'low' | 'moderate' | 'high') => {
        const maxHeartRate = 220 - age;
        ...
        const lower = restingHeartRate + (maxHeartRate - restingHeartRate) * lowerPercent;
        const upper = restingHeartRate + (maxHeartRate - restingHeartRate) * upperPercent;
  ...
  describe('血压分析', () => {
    it('应该正确分类血压', () => {
      const classifyBloodPressure = (systolic: number, diastolic: number) => {
        ...
        if (systolic < 130 && diastolic < 80) {return '血压升高';}
        if (systolic < 140 || diastolic < 90) {return '高血压1期';}
        if (systolic < 180 || diastolic < 120) {return '高血压2期';}
        return '高血压危象';
      };
  ...
  describe('睡眠分析', () => {
    it('应该分析睡眠质量', () => {
      const analyzeSleepQuality = (duration: number, deepSleepPercent: number, efficiency: number) => {
        let score = 0;
        ...
        // 深睡眠比例评分 (15-25%最佳)
        if (deepSleepPercent >= 15 && deepSleepPercent <= 25) {score += 30;}
        else if (deepSleepPercent >= 10 && deepSleepPercent <= 30) {score += 20;}
        else {score += 10;}

        // 睡眠效率评分 (85%以上最佳)
        if (efficiency >= 85) {score += 30;}
        else if (efficiency >= 75) {score += 20;}
        else {score += 10;}
  ...
  describe('运动分析', () => {
    ...
    it('应该分析运动强度', () => {
      const analyzeExerciseIntensity = (heartRate: number, age: number) => {
        ...
        if (percentage < 50) {return { intensity: '很轻', zone: 1 };}
        if (percentage < 60) {return { intensity: '轻度', zone: 2 };}
        if (percentage < 70) {return { intensity: '中度', zone: 3 };}
        if (percentage < 80) {return { intensity: '高强度', zone: 4 };}
        return { intensity: '最大强度', zone: 5 };
      };

      expect(analyzeExerciseIntensity(90, 30)).toEqual({ intensity: '很轻', zone: 1 });
      expect(analyzeExerciseIntensity(130, 30)).toEqual({ intensity: '中度', zone: 3 });
      expect(analyzeExerciseIntensity(170, 30)).toEqual({ intensity: '最大强度', zone: 5 });
    });
  });
  ...
  describe('健康风险评估', () => {
    it('应该评估心血管风险', () => {
      const assessCardiovascularRisk = (
        ...
        // 年龄风险
        if (age > 65) {risk += 3;}
        else if (age > 55) {risk += 2;}
        else if (age > 45) {risk += 1;}

        // 血压风险
        if (systolic > 160) {risk += 3;}
        else if (systolic > 140) {risk += 2;}
        else if (systolic > 120) {risk += 1;}

        // 胆固醇风险
        if (cholesterol > 240) {risk += 2;}
        ...
        if (risk >= 8) {return '高风险';}
        if (risk >= 5) {return '中等风险';}
        if (risk >= 2) {return '低风险';}
        return '很低风险';
      };

      expect(assessCardiovascularRisk(70, 170, 250, true, true)).toBe('高风险');
      expect(assessCardiovascularRisk(50, 130, 180, false, false)).toBe('低风险');
      expect(assessCardiovascularRisk(30, 110, 160, false, false)).toBe('很低风险');
    });
    ...
    it('应该计算健康评分', () => {
      const calculateHealthScore = (metrics: {
        ...
        // BMI评分
        if (metrics.bmi < 18.5 || metrics.bmi > 30) {score -= 20;}
        else if (metrics.bmi > 25) {score -= 10;}

        // 血压评分
        if (metrics.bloodPressure.systolic > 140 || metrics.bloodPressure.diastolic > 90) {score -= 20;}
        else if (metrics.bloodPressure.systolic > 120) {score -= 5;}

        // 心率评分
        if (metrics.heartRate > 100 || metrics.heartRate < 50) {score -= 10;}

        // 睡眠评分
        if (metrics.sleepHours < 6 || metrics.sleepHours > 10) {score -= 15;}
        else if (metrics.sleepHours < 7 || metrics.sleepHours > 9) {score -= 5;}

        // 运动评分
        if (metrics.exerciseMinutes < 150) {score -= 15;} // WHO推荐每周150分钟
