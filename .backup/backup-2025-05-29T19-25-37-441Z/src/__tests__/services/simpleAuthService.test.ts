...
describe('简化认证服务测试', () => {
  ...
  describe('权限检查', () => {
    it('应该能够检查用户权限', () => {
      ...
      expect(hasPermission(adminUser, 'doctor')).toBe(true); // admin有所有权限
      expect(hasPermission(doctorUser, 'doctor')).toBe(true);
      expect(hasPermission(regularUser, 'doctor')).toBe(false);
    });

    it('应该能够检查资源访问权限', () => {
      const canAccessResource = (userId: string, resourceOwnerId: string, userRoles: string[]) => {
        return userId === resourceOwnerId || userRoles.includes('admin');
      };

      expect(canAccessResource('123', '123', ['user'])).toBe(true); // 自己的资源
      expect(canAccessResource('123', '456', ['admin'])).toBe(true); // 管理员
      expect(canAccessResource('123', '456', ['user'])).toBe(false); // 无权限
    });
  });
  ...
  describe('安全功能', () => {
    it('应该能够检测可疑登录', () => {
      const detectSuspiciousLogin = (loginHistory: any[]) => {
        ...
        const locationDiff = recent[0].location !== recent[1].location;
        
        // 如果在5分钟内从不同地点登录，认为可疑
        return timeDiff < 5 * 60 * 1000 && locationDiff;
      };
