...
import { AgentCoordinator, AgentType, AgentTask, AgentCoordinationResult } from './AgentCoordinator';
...
export class AgentManager {
  ...
  async executeTask(task: AgentTask): Promise<AgentCoordinationResult> {
    ...
    try {
      ...
      this.log('debug', `任务 ${task.taskId} 执行完成, 耗时: ${executionTime}ms`);
   ...
   * 获取智能体状态
   */
  async getAgentStatus(agentType?: AgentType): Promise<Map<AgentType, AgentStatus>> {
   ...
   * 更新任务指标
   */
  private updateTaskMetrics(task: AgentTask, result: AgentCoordinationResult, executionTime: number): void {
    const agentTypes = task.requiredAgents || [this.getDefaultAgentForTask(task)];
    ...
    for (const agentType of agentTypes) {
      ...
      if (metrics) {
        ...
        // 更新平均响应时间
        metrics.averageResponseTime = (metrics.averageResponseTime * (metrics.totalTasks - 1) + executionTime) / metrics.totalTasks;
  ...
  private log(level: 'debug' | 'info' | 'warn' | 'error', message: string): void {
    ...
    if (messageLevel >= configLevel) {
      ...
      console.log(`[${timestamp}] [${level.toUpperCase()}] [AgentManager] ${message}`);
    }
  }
