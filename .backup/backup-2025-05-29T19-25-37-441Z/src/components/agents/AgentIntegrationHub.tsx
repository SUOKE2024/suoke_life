...
export const AgentIntegrationHub: React.FC<AgentIntegrationHubProps> = ({
  ...
  const handleStartChat = useCallback((agentId: AgentType) => {
    onStartChat?.(agentId);
  }, [onStartChat]);

  const handleViewDetails = useCallback((agentId: AgentType) => {
    onViewDetails?.(agentId);
  }, [onViewDetails]);

  const renderAgentCard = useCallback((agentInfo: AgentInfo) => {
  ...
  const renderSelectedAgentDetails = useMemo(() => {
