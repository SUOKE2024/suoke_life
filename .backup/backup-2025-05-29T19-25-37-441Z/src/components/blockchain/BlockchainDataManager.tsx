...
export const BlockchainDataManager: React.FC<BlockchainDataManagerProps> = ({
  ...
  const handleUploadData = useCallback(async (request: DataUploadRequest) => {
    setLoading(true);
    try {
      // 模拟上链过程
  ...
  const handleVerifyData = useCallback(async (hash: string) => {
    setLoading(true);
    try {
      // 模拟验证过程
  ...
  const renderStatsCards = () => {
    ...
    const encryptedRecords = records.filter(r => r.encrypted).length;
  ...
  const renderRecordCard = (record: BlockchainRecord, index: number) => (
    <Animated.View
      key={record.id}
      style={[
        styles.recordCard,
