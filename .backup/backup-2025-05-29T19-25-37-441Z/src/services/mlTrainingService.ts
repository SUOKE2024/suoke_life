...
export class MLTrainingService {
  ...
  async createDataset(
    ...
    try {
      console.log('📊 创建训练数据集...', { name, dataCount: trainingData.length });
  ...
  async startTraining(
    ...
    try {
      console.log('🚀 开始模型训练...', { name, modelType: modelConfig.modelType });
  ...
  async getModelPerformance(
    ...
    try {
      const response = await apiClient.get(`/ml/monitoring/performance/${modelId}?start=${timeRange.start}&end=${timeRange.end}`);
      return response.data;
    } catch (error) {
  ...
  async optimizeHyperparameters(
    ...
    try {
      ...
      const response = await apiClient.post('/ml/optimization/hyperparameters', {
        baseConfig,
        datasetId,
        optimizationConfig,
        timestamp: Date.now()
  ...
  async getFeatureImportance(modelId: string): Promise<{
    features: Array<{
      name: string;
      importance: number;
      category: 'looking' | 'listening' | 'inquiry' | 'palpation' | 'calculation';
    }>;
    correlationMatrix: number[][];
    featureNames: string[];
  }> {
    try {
      const response = await apiClient.get(`/ml/analysis/feature-importance/${modelId}`);
      return response.data;
    } catch (error) {
  ...
  getServiceStatus(): {
    ...
    const averageAccuracy = completedTasks.length > 0 
      ? completedTasks.reduce((sum, task) => sum + (task.metrics.accuracy || 0), 0) / completedTasks.length
      : 0;

    return {
      isInitialized: this.isInitialized,
      activeTrainingTasks: activeTasks.filter(task => task.status === 'running').length,
  ...
  async createTrainingTask(
    ...
    try {
      console.log('🚀 创建训练任务...', { modelName, dataSize: trainingData.length });
