...
class ApiClient {
  ...
  private async refreshToken(refreshToken: string): Promise<ApiResponse> {
    const response = await axios.post(`${API_CONFIG.SERVICES.AUTH}/auth/refresh`, {
      refreshToken,
    });
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        if (attempt < retries) {
          ...
          console.log(`[API] 请求失败，${delay}ms后重试 (${attempt}/${retries}):`, error.message);
   ...
   * GET请求
   */
  async get<T = any>(url: string, params?: any, config?: Partial<ApiRequest>): Promise<ApiResponse<T>> {
   ...
   * 上传文件
   */
  async upload<T = any>(url: string, file: any, config?: Partial<ApiRequest>): Promise<ApiResponse<T>> {
  ...
  async healthCheck(): Promise<boolean> {
    try {
      const response = await this.get('/health', undefined, { retries: 1, timeout: 5000 });
