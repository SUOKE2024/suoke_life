...
export interface GraphQLClientEvents {
  ...
  requestEnd: { request: GraphQLRequest; response: GraphQLResponse; duration: number };
...
export class GraphQLClient extends EventEmitter {
      ...
      switch (message.type) {
        ...
        case 'data':
          if (message.id === subscriptionId) {
            this.emit('subscriptionData', { subscription: subscriptionId, data: message.payload });
        ...
        case 'error':
          if (message.id === subscriptionId) {
            this.emit('subscriptionError', { subscription: subscriptionId, error: message.payload });
   ...
   * 生成缓存键
   */
  private generateCacheKey(request: GraphQLRequest, customKey?: string): string {
    if (customKey) return customKey;
    
    const queryHash = this.hashString(request.query);
    const variablesHash = request.variables ? this.hashString(JSON.stringify(request.variables)) : '';
   ...
   * 生成订阅ID
   */
  private generateSubscriptionId(subscription: string, variables?: GraphQLVariables): string {
