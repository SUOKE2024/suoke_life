...
const AGENT_ROLES: Record<AgentType, AgentRole> = {
  xiaoai: {
    id: 'xiaoai',
    name: '小艾',
    description: '索克生活APP首页聊天频道版主，提供语音引导、交互、问诊及无障碍服务',
    specialties: ['语音交互', '多模态理解', '中医望诊', '智能问诊', '无障碍服务'],
    capabilities: ['实时语音交互', '舌诊面色分析', '健康档案管理', '导盲导医', '手语识别'],
    personality: {
      style: '温和亲切',
      tone: '耐心细致',
      approach: '多模态交互'
  ...
  xiaoke: {
    ...
    capabilities: ['智能匹配预约', '个性化推荐', '区块链溯源', '第三方集成', '健康商品推荐'],
    personality: {
      style: '专业高效',
      tone: '积极主动',
      approach: '服务导向'
  ...
  laoke: {
    ...
    capabilities: ['RAG知识检索', '个性化学习', 'AR/VR教学', '内容审核', '玉米迷宫NPC'],
    personality: {
      style: '博学睿智',
      tone: '循循善诱',
      approach: '知识传承'
  ...
  soer: {
    ...
    capabilities: ['习惯培养', '多设备融合', '环境感知', '养生计划', '情绪疏导'],
    personality: {
      style: '贴心温暖',
      tone: '关怀体贴',
      approach: '全方位陪伴'
...
export class AgentCoordinationService {
  ...
  async createCollaborationTask(
    ...
    try {
      ...
      const estimatedDuration = this.estimateTaskDuration(complexity, requiredAgents.length);
   ...
   * 获取智能体状态
   */
  async getAgentStatus(agentId?: AgentType): Promise<AgentStatus | AgentStatus[]> {
    if (agentId) {
  ...
  getServiceStatus(): {
    ...
    const onlineAgents = Array.from(this.agentStatuses.values()).filter(agent => agent.isOnline);
  ...
  async performCollaborativeDiagnosis(
    ...
    try {
      ...
      const taskId = await this.createCollaborationTask(
        ...
        userId,
        `session_${Date.now()}`,
        `协作诊断: ${diagnosisData.diagnosisResult?.primarySyndrome || '综合分析'}`,
        'high'
      );
  ...
  private async processAgentResponses(
    ...
    // 生成最终决策
    const finalDecision = await this.generateFinalDecision(responses, agentContributions);
    
    // 处理冲突（如果有）
    const conflictResolution = consensusReached ? undefined : 
  ...
  private calculateAgentContributions(responses: AgentResponse[]): {
    ...
    const totalConfidence = responses.reduce((sum, r) => sum + r.content.confidence, 0);
  ...
  private async generateFinalDecision(
    ...
    const avgConfidence = responses.reduce((sum, r) => sum + r.content.confidence, 0) / responses.length;
    
    return {
      recommendation: this.synthesizeRecommendations(allRecommendations, contributions),
  ...
  private async generateAgentRecommendations(
    ...
    Object.keys(collaborationResult.agentContributions).forEach(agentId => {
      ...
      recommendations[agentId] = {
        ...
        recommendations: this.generateSpecificRecommendations(agentId as AgentType, diagnosisResult),
  ...
  private async generateFinalRecommendation(
    ...
    return `基于您的${syndrome}证候和${constitution}体质特点，经过四位专家的综合分析，我们建议：` +
