...
class AgentService {
   ...
   * 发送消息给智能体
   */
  async sendMessage(agentId: string, content: string, type: string = 'text'): Promise<Message> {
    try {
   ...
   * 获取智能体建议
   */
  async getAgentSuggestions(agentId: string, category?: string): Promise<AgentSuggestion[]> {
    try {
   ...
   * 获取聊天历史
   */
  async getChatHistory(agentId: string, limit: number = 50): Promise<Message[]> {
    try {
  ...
  async healthCheck(): Promise<{ status: string; timestamp: number }> {
    try {
      const response: ApiResponse<{ status: string; timestamp: number }> = await apiClient.get('/agents/health');
