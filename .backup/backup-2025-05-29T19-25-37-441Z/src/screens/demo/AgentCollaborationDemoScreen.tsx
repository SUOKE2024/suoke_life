...
export const AgentCollaborationDemoScreen: React.FC = () => {
  ...
  const runCollaborationScenario = async (scenario: CollaborationScenario) => {
  ...
  const simulateAgentCollaboration = async (scenario: CollaborationScenario) => {
  ...
  const simulateAgentThinking = async (agentId: AgentType, scenario: CollaborationScenario) => {
  ...
  const simulateAgentResponse = async (agentId: AgentType, scenario: CollaborationScenario) => {
    const agent = agents.find(a => a.id === agentId);
    if (!agent) {return;}
    ...
    const response = responses[scenario.id as keyof typeof responses]?.[agentId] || 
  ...
  const simulateCollaborativeDecision = async (participants: AgentType[], scenario: CollaborationScenario) => {
    // 设置所有参与者为协作状态
    setAgents(prev => prev.map(a => 
    ...
    const finalDecision = finalDecisions[scenario.id as keyof typeof finalDecisions] || 
                         '智能体协作完成，已为您提供综合性解决方案。';

    addToLog('system', `✅ 协作决策: ${finalDecision}`, 'collaboration');
  };

  const addToLog = (agentId: AgentType | 'system', message: string, type: 'thinking' | 'response' | 'collaboration') => {
  ...
  const renderScenarioCard = (scenario: CollaborationScenario) => {
