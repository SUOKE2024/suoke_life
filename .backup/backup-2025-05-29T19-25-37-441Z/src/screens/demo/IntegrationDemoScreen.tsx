...
export const IntegrationDemoScreen: React.FC = () => {
  ...
  const runCompleteDemo = async () => {
    ...
    try {
      ...
      // 重置所有步骤状态
      const resetSteps = demoSteps.map(step => ({ ...step, status: 'pending' as const, result: undefined }));
  ...
  const executeStep = async (stepIndex: number, stepFunction: () => Promise<any>) => {
  ...
  const demonstrateFiveDiagnosis = async () => {
  ...
  const demonstrateAgentCollaboration = async () => {
  ...
  const demonstrateMLTraining = async () => {
    ...
    try {
      const trainingTask = await mlTrainingService.createTrainingTask(
  ...
  const performIntegrationAnalysis = async () => {
    console.log('📊 开始集成分析...');
  ...
  const renderStepCard = (step: DemoStep, index: number) => {
