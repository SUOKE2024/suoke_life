...
export class MemoryOptimizer {
  ...
  detectLeaks(): MemoryLeak[] {
    ...
    if (memoryGrowth > 0.5) { // 50%增长阈值
      leaks.push({
        type: 'memory',
        name: 'heap',
        count: 0,
        growth: memoryGrowth,
        severity: memoryGrowth > 2 ? 'critical' : memoryGrowth > 1 ? 'high' : 'medium',
        suggestion: `内存使用量持续增长 (${(memoryGrowth * 100).toFixed(1)}%)，可能存在内存泄漏`,
      });
    }
   ...
   * 计算增长率
   */
  private calculateGrowthRate(name: string, type: 'component' | 'listener'): number {
  ...
  getOptimizationSuggestions(): MemoryOptimizationSuggestion[] {
    ...
    if (latestSnapshot && latestSnapshot.jsHeapSizeLimit > 0) {
      const usageRate = latestSnapshot.usedJSHeapSize / latestSnapshot.jsHeapSizeLimit;
      
      if (usageRate > 0.8) {
        suggestions.push({
          type: 'warning',
          priority: 'high',
          message: `内存使用率过高 (${(usageRate * 100).toFixed(1)}%)，建议清理缓存或优化代码`,
        ...
        suggestions.push({
          type: 'optimization',
          priority: 'medium',
          message: `内存使用率较高 (${(usageRate * 100).toFixed(1)}%)，建议监控内存使用情况`,
        });
      }
    }
