...
// 动画类型
export type AnimationType = 'fadeIn' | 'fadeOut' | 'slideInLeft' | 'slideInRight' | 
  'slideInUp' | 'slideInDown' | 'slideOutLeft' | 'slideOutRight' | 'slideOutUp' | 
  'slideOutDown' | 'scaleIn' | 'scaleOut' | 'rotateIn' | 'rotateOut' | 'bounce' | 
...
export const createAnimatedValueXY = (initialValue: { x: number; y: number } = { x: 0, y: 0 }): Animated.ValueXY => {
...
// 序列动画
export const animateSequence = (animations: Animated.CompositeAnimation[]): Animated.CompositeAnimation => {
  return Animated.sequence(animations);
};

// 并行动画
export const animateParallel = (animations: Animated.CompositeAnimation[]): Animated.CompositeAnimation => {
...
export const animations = {
  ...
  bounce: (
    ...
    return animateSequence([
      animateValue(animatedValue, 1.2, { duration: 150, easing: EASING.EASE_OUT }),
      animateValue(animatedValue, 0.9, { duration: 100, easing: EASING.EASE_IN }),
      animateValue(animatedValue, 1, { duration: 100, easing: EASING.EASE_OUT }),
    ]);
  },
  ...
  pulse: (
    ...
    const pulseAnimation = animateSequence([
      animateValue(animatedValue, 1.1, { duration: 300, easing: EASING.EASE_OUT }),
      animateValue(animatedValue, 1, { duration: 300, easing: EASING.EASE_IN }),
    ]);

    return config.loop ? animateLoop(pulseAnimation) : pulseAnimation;
  },
  ...
  flip: (
    ...
    return animateSequence([
      animateValue(animatedValue, 0.5, { duration: 150, easing: EASING.EASE_IN }),
      animateValue(animatedValue, 1, { duration: 150, easing: EASING.EASE_OUT }),
    ]);
  },
};
...
export const gestureAnimations = {
  ...
  createSwipeAnimation: (
    ...
    const toValue = direction === 'left' || direction === 'up' ? -distance : distance;
...
export const loadingAnimations = {
  ...
  createRotateLoading: (animatedValue: Animated.Value) => {
    ...
    return animateLoop(rotateAnimation);
  },
  ...
  createPulseLoading: (animatedValue: Animated.Value) => {
    ...
    const pulseAnimation = animateSequence([
      animateValue(animatedValue, 1, { duration: 500, easing: EASING.EASE_OUT }),
      animateValue(animatedValue, 0.5, { duration: 500, easing: EASING.EASE_IN }),
    ]);
    return animateLoop(pulseAnimation);
  },
  ...
  createWaveLoading: (animatedValues: Animated.Value[]) => {
    const waveAnimations = animatedValues.map((value, index) => {
      ...
      const waveAnimation = animateSequence([
        animateValue(value, 1, { duration: 300, delay: index * 100 }),
        animateValue(value, 0, { duration: 300 }),
      ]);
      return animateLoop(waveAnimation);
    });
    return animateParallel(waveAnimations);
  },
};
...
export const transitionAnimations = {
  ...
  slideTransition: (
    ...
    const distance = direction === 'horizontal' ? screenWidth : screenHeight;
    return {
...
export const performanceUtils = {
  ...
  shouldUseNativeDriver: (animationType: string): boolean => {
    ...
    return !unsupportedTypes.some(type => animationType.includes(type));
  },
