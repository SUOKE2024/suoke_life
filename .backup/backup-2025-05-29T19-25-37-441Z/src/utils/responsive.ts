...
export const breakpoints = {
  ...
  // 检查是否为中等屏幕
  isMedium: (): boolean => SCREEN_WIDTH >= BREAKPOINTS.sm && SCREEN_WIDTH < BREAKPOINTS.md,
  
  // 检查是否为大屏幕
  isLarge: (): boolean => SCREEN_WIDTH >= BREAKPOINTS.md && SCREEN_WIDTH < BREAKPOINTS.lg,
...
export const orientationListener = {
  ...
  getCurrentOrientation: (): Orientation => {
    return breakpoints.isLandscape() ? Orientation.LANDSCAPE : Orientation.PORTRAIT;
  },
};
