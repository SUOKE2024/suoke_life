...
class CodeQualityChecker:
    ...
    def _check_unused_imports(self, file_path: Path, tree: ast.AST, content: str):
        ...
        if unused_imports:
            self.issues["unused_imports"].append(
                {"file": str(file_path), "imports": unused_imports}
            )
    ...
    def _check_duplicate_code(self, file_path: Path, content: str):
        ...
        if duplicates:
            self.issues["duplicate_lines"].append(
                {"file": str(file_path), "duplicates": duplicates}
            )
    ...
    def _check_todos(self, file_path: Path, content: str):
        ...
        if todos:
            self.issues["todos"].append({"file": str(file_path), "todos": todos})
    ...
    def _check_console_logs(self, file_path: Path, content: str):
        ...
        for i, line in enumerate(lines, 1):
            if "console.log" in line and not line.strip().startswith("//"):
                console_logs.append({"line": i, "content": line.strip()})

        if console_logs:
            self.issues["console_logs"].append(
                {"file": str(file_path), "logs": console_logs}
            )
        ...
        try:
            ...
            if common_deps:
                self.issues["duplicate_dependencies"].append(
                    {"file": str(package_json), "duplicates": list(common_deps)}
                )
            ...
            self.issues["package_json_errors"].append(
                {"file": str(package_json), "error": str(e)}
            )
        ...
        try:
            ...
            if duplicates:
                self.issues["duplicate_python_deps"].append(
                    {"file": str(requirements_txt), "duplicates": duplicates}
                )
            ...
            self.issues["requirements_errors"].append(
                {"file": str(requirements_txt), "error": str(e)}
            )
...
"""
    ...
    def _calculate_quality_score(self) -> int:
        """计算代码质量评分"""
        total_files = self.stats.get("python_files_checked", 0) + self.stats.get(
            "js_files_checked", 0
        )
        if total_files == 0:
            return 100
...
def main():
    ...
    parser.add_argument(
        "--output", default="code_quality_report.md", help="输出报告文件名"
    )
