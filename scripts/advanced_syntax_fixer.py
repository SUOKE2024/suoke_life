...
class AdvancedSyntaxFixer:
                ...
                if "from" in line and "import" in line:
                    ...
                    if len(parts) > 2:
                        ...
                        for j, part in enumerate(parts[2:], 1):
                            lines.insert(
                                line_idx + j,
                                f"from {parts[0].replace('from ', '').strip()} import {part.strip()}",
                            )
    ...
    def fix_file_comprehensive(self, file_path: Path) -> bool:
        ...
        try:
            ...
            while iteration < max_iterations:
                error_type, error_line, error_text = self.analyze_syntax_error(
                    file_path, content
                )
                ...
                if "unexpected indent" in error_type:
                    ...
                    if "function definition" in error_type:
                        content = self.fix_function_definition_block(
                            content, error_line
                        )
    ...
    def fix_files_from_error_list(self, error_files: List[str]):
        ...
        logger.info(
            f"修复完成! 处理了 {self.files_processed} 个文件，成功修复 {self.issues_fixed} 个文件"
        )
