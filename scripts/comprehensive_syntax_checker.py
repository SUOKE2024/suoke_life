...
class ComprehensiveSyntaxChecker:
        ...
        for line in lines:
            ...
            if " import " in line and line.count(" import ") > 1:
                ...
                if len(parts) > 2:
                    ...
                    for part in parts[2:]:
                        fixed_lines.append(
                            f"from {parts[0].replace('from ', '')} import {part}"
                        )
                    continue
    ...
    def generate_report(
        self,
        all_issues: Dict[str, List[SyntaxIssue]],
        output_file: str = "syntax_check_report.json",
    ):
        """生成检查报告"""
...
def main():
    ...
    parser.add_argument("--no-fix", action="store_true", help="仅检查，不修复问题")
    parser.add_argument("--service", help="指定要检查的微服务名称")
