...
class DeploymentConfigUpdater:
    ...
    def create_backup(self):
        ...
        if self.docker_compose_file.exists():
            backup_file = (
                self.backup_dir
                / f"docker-compose.microservices.yml.{os.popen('date +%Y%m%d_%H%M%S').read().strip()}"
            )
    ...
    def _create_merged_service_config(self, service_name):
        ...
        if service_name == "user-management-service":
            ...
            # 工具服务可能需要用户管理和通信服务
            base_config["depends_on"].extend(
                ["user-management-service", "communication-service"]
            )
    ...
    def _create_k8s_deployments(self):
        ...
        for service_name in set(self.service_merges.values()):
                            ...
                            "containers": [
                                {
                                    ...
                                    "env": [
                                        ...
                                        {
                                            "name": "DATABASE_URL",
                                            "value": "postgresql://suoke:suoke123@postgres:5432/suoke_db",
                                        },
                                        {
                                            "name": "REDIS_URL",
            ...
            with open(deployment_file, "w", encoding="utf-8") as f:
                yaml.dump(
                    deployment_config, f, default_flow_style=False, allow_unicode=True
                )
