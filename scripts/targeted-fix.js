          ...
          if (nextLine) {
            ...
            if (nextPropertyMatch && !trimmedValue.endsWith(',') && !trimmedValue.endsWith(';')) {
              ...
              if (currentIndent === nextIndent) {
                ...
                fixCount++;
                hasChanges = true;
                console.log(`  修复第${lineNumber}行: ${prop}: ${trimmedValue} -> ${prop}: ${trimmedValue},`);
              }
            }
            // 如果下一行是对象结束且当前行有逗号，移除逗号
              ...
              fixCount++;
              hasChanges = true;
              console.log(`  修复第${lineNumber}行: 移除多余逗号`);
            }
          }
        }
          ...
          if (nextLine) {
            ...
            if (nextInterfaceMatch && !trimmedType.endsWith(',') && !trimmedType.endsWith(';')) {
              ...
              if (currentIndent === nextIndent) {
                ...
                fixCount++;
                hasChanges = true;
                console.log(`  修复第${lineNumber}行: 接口属性添加分号`);
              }
            }
          }
        }
...
console.log(`📁 处理 ${Object.keys(targetFiles).length} 个目标文件\n`);
...
console.log(`\n📊 针对性修复报告`);
console.log(`==================================================`);
console.log(`📁 处理文件数: ${processedFileCount}`);
