...
""",
    ...
    def analyze_project(self) -> List[GILIssue]:
        ...
        for file_path in python_files:
            ...
            if file_issues:
                print(
                    f"  📄 {file_path.relative_to(self.project_root)}: {len(file_issues)} 个问题"
                )
    ...
    def generate_report(
        self, output_file: str = "gil_optimization_report.json"
    ) -> Dict[str, Any]:
        """生成优化报告"""
        # 按严重程度分组
    ...
    def print_summary(self):
        ...
        if high_count > 0:
            ...
            for i, issue in enumerate(
                [i for i in self.issues if i.severity == "high"][:5], 1
            ):
                print(
                    f"  {i}. {issue.file_path}:{issue.line_number} - {issue.description}"
                )
