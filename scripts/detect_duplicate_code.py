...
class DuplicateCodeDetector:
    ...
    def scan_project(self) -> Dict[str, Any]:
        ...
        stats['total_duplicates'] = sum(len(blocks) for blocks in self.duplicates.values())
    ...
    def _extract_function_block(self, node: ast.FunctionDef, content: str, file_path: str):
        """提取函数代码块"""
        lines = content.split('\n')
        ...
        if end_line - start_line >= self.min_lines:
            ...
            block = CodeBlock(block_content, file_path, start_line, end_line, "function")
    ...
    def _analyze_file_by_lines(self, file_path: str):
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                if line and not line.startswith('#'):
                    ...
                    if len(current_block) >= self.min_lines:
                        block_content = '\n'.join(current_block)
                        block = CodeBlock(block_content, file_path, start_line, i, "block")
                        self.code_blocks.append(block)
                    current_block = []
            ...
            if len(current_block) >= self.min_lines:
                block_content = '\n'.join(current_block)
                block = CodeBlock(block_content, file_path, start_line, len(lines), "block")
                self.code_blocks.append(block)
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'summary': {
                ...
                'total_duplicates': sum(len(blocks) for blocks in self.duplicates.values()),
        ...
        for group_id, blocks in sorted_duplicates:
            group_info = {
                ...
                'total_duplicate_lines': sum(block.end_line - block.start_line for block in blocks),
                'locations': []
            }
    ...
    def _generate_recommendations(self) -> List[Dict[str, Any]]:
        ...
        # 高优先级：大量重复的代码块
        high_priority = [
            (group_id, blocks) for group_id, blocks in self.duplicates.items()
...
def main():
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        if report['duplicate_groups']:
            f.write("## 🔍 重复代码组\n\n")
            for i, group in enumerate(report['duplicate_groups'][:10], 1):  # 只显示前10组
                ...
                for location in group['locations']:
                    f.write(f"- `{location['file']}` (行 {location['start_line']}-{location['end_line']})\n")
    ...
    print(f"📊 发现 {summary['duplicate_groups']} 组重复代码，潜在节省 {summary['potential_savings']} 行代码")
